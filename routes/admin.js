...
    'relatorios-section': { view: true, create: true, edit: true },
    'sales-report-section': { view: true, create: true, edit: true }, // <-- ADICIONADO (Estava em falta no seu original)
...
    'users-section': { view: true, create: true, edit: true },
    'ausencias-section': { view: true, create: true, edit: true }, // <-- ADICIONADO (Estava em falta no seu original)
    'loyalty-section': { view: true, create: true, edit: true } // <-- ADICIONADO (Novo Add-on)
};
...
        // ####################################################################
        // ### (VERIFICAÇÃO) Esta parte usa o masterPermissions atualizado
        // ####################################################################
        
...
            permissions: masterPermissions, // <-- Isto agora inclui todas as permissões
...
    try {
        const { auth } = req;
        const listUsersResult = await auth.listUsers(1000); // <-- ERRO DE DIGITAÇÃO REMOVIDO DAQUI
