const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-CUkhX2g-.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/firebase-config-C2tbVz-J.js","assets/styles-vhIfPenq.css"])))=>i.map(i=>d[i]);
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${i.status}) em ${r}:`,l),new Error(l)}return i.json()}catch(i){throw console.error(`Falha de rede ao tentar acessar ${r}:`,i.message),i.message.includes("Failed to fetch")||i.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${ra}. Verifique se o servidor backend está rodando.`):i}}const Do=(e,t,a,o=null)=>{let s=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(s+=`&professionalId=${o}`),I(s)},Ws=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const s=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return I(s)},Js=e=>I("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Gs=(e,t)=>I(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ra=e=>I(`/api/appointments/${e}`,{method:"DELETE"}),Qs=e=>I(`/api/appointments/${e}/reopen`,{method:"POST"}),Ys=(e,t)=>I(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let G;async function Xs(){if(!G)try{G=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Zs(){if(!G){console.warn("AudioContext não inicializado. O som não será tocado.");return}G.state==="suspended"&&G.resume();const e=G.createOscillator(),t=G.createGain();e.connect(t),t.connect(G.destination),e.type="sine",e.frequency.setValueAtTime(800,G.currentTime),t.gain.setValueAtTime(0,G.currentTime),t.gain.linearRampToValueAtTime(.3,G.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,G.currentTime+.2),e.start(G.currentTime),e.stop(G.currentTime+.2)}function g(e,t,a="info",o=!1){const s=document.getElementById("toast-container");if(!s)return;o&&Zs();const r=document.createElement("div"),i={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},n={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};r.className=`toast ${i[a]||i.info}`,r.innerHTML=`
    ...
    `;const o=t.querySelector("#btnGenerateQr");o&&o.addEventListener("click",async()=>{o.disabled=!0,o.innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Gerando...';const r="https://us-central1-kairos-agenda-us.cloudfunctions.net/whatsapp/api/whatsapp/connect";try{const n=await(await fetch(r,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({establishmentId:J})})).json();n.qrcode?(t.querySelector("#whatsappStatusArea").classList.add("hidden"),t.querySelector("#qrCodeDisplayArea").classList.remove("hidden"),t.querySelector("#qrCodeImage").src=n.qrcode):n.message&&n.message.includes("já está conectado")?(t.querySelector("#whatsappStatusArea").classList.add("hidden"),t.querySelector("#qrCodeDisplayArea").classList.add("hidden"),t.querySelector("#connectedStatusArea").classList.remove("hidden"),g("Aviso","O WhatsApp já estava conectado nesta unidade.","success")):g("Erro na API",n.error||"Desconhecido","error")}catch(i){console.error(i),g("Erro de Conexão","Não foi possível acessar o servidor Kairós.","error")}finally{o.disabled=!1,o.innerHTML='<i class="bi bi-phone-vibrate"></i> Gerar QR Code'}});const s=t.querySelector("#btnDisconnectWhatsapp");s&&s.addEventListener("click",()=>{confirm("Tem certeza que deseja DESCONECTAR o bot desta unidade? O sistema não responderá mais os clientes via WhatsApp automaticamente.")&&g("Aviso",'Para desconectar, por favor vá ao aplicativo do seu celular (Aparelhos Conectados) e clique em "Desconectar".',"info")})}async function xl(e,t){const a=e.loyaltyProgram||{},o=a.pointsPerVisit||1;let s=[],r=[],i=[];try{[s,r,i]=await Promise.all([Se(J),Ut(J),$a(J)])}catch(d){console.error("Erro ao carregar dados para fidelidade:",d)}t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md border border-gray-100">
             <div class="flex justify-between items-center mb-6">
    ...
    `,xe.querySelector('button[data-action="back-to-menu"]').addEventListener("click",s=>{s.preventDefault(),fs({id:J})});const o=document.getElementById("settings-content-detail");switch(e){case"personal-data":cl(V,o);break;case"change-password":ul(V,o);break;case"change-email":ml(V,o);break;case"branding":pl(V,o);break;case"booking":gl(V,o);break;case"working-hours":bl(V,o);break;case"whatsapp-bot":fl(V,o);break;case"loyalty":await xl(V,o);break;case"financial":await vl(V,o);break;case"support":hl(V,o);break;case"cancellation":yl(V,o);break;default:o.innerHTML='<div class="p-4 text-center">Módulo em construção.</div>'}}async function fs(e={}){xe.innerHTML=`
    ...
    `;try{J=e.id||m.establishmentId,V=await ye(J);const t=e.id?`<button onclick="window.navigateTo('establishments-section')" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold py-2 px-4 rounded-lg shadow-sm transition-colors flex items-center gap-2">
        ...
        `,xe.querySelectorAll("div[data-section]").forEach(s=>{s.addEventListener("click",r=>{kl(s.dataset.section)})}),xe.querySelectorAll(".module-toggle").forEach(s=>{s.addEventListener("change",async()=>{const r=s.dataset.module;try{const n={...(await ye(J)).modules,[r]:s.checked};await xa(J,{modules:n}),g("Módulos","Módulos atualizados com sucesso.","success")}catch(i){s.checked=!s.checked,g("Erro",i.message,"error")}})})}catch(t){xe.innerHTML=`
