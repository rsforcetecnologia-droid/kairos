import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{initializeApp as jo}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";import{getAuth as Ho,EmailAuthProvider as Oo,reauthenticateWithCredential as zo,verifyBeforeUpdateEmail as Vo,updatePassword as Uo,updateProfile as Jo,onAuthStateChanged as _o,signOut as Ea}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{getFirestore as Wo,addDoc as lo,collection as Ke,getDocs as co,query as uo,orderBy as mo,doc as da,deleteDoc as Go,getDoc as Yo,updateDoc as Xo,where as Qo,onSnapshot as Zo}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";const Ko={apiKey:"AIzaSyAlJaPEW5-yOb-8wkB8EJZhAML2M2yI8Ao",authDomain:"kairos-system.firebaseapp.com",projectId:"kairos-system",storageBucket:"kairos-system.appspot.com",messagingSenderId:"603994960586",appId:"1:603994960586:web:30d2c030eed3c55eccfa33",measurementId:"G-SVHFXKV5EC"},po=jo(Ko),J=Ho(po),de=Wo(po),p={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function es(e,t,a){p.establishmentId=e,p.establishmentName=t,p.userPermissions=a}const Gt="https://kairos-service-603994960586.southamerica-east1.run.app";console.log("🚀 API configurada para Produção:",Gt);async function ts(){const e=J.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function w(e,t={}){const a=await ts();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const s=`${Gt}${e}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${s}`);try{const r=await fetch(s,{...t,headers:{...a,...t.headers}});if(!r.ok){const n=(await r.json().catch(()=>({message:r.statusText}))).message||`Erro na API: ${r.status}`;if(n.includes("FAILED_PRECONDITION")&&n.includes("requires an index")){const i=/(https:\/\/[^\s]+)/,d=n.match(i),l=d?d[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${r.status}) em ${s}:`,n),new Error(n)}return r.json()}catch(r){throw console.error(`Falha de rede ao tentar acessar ${s}:`,r.message),r.message.includes("Failed to fetch")||r.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${Gt}. Verifique sua conexão com a internet.`):r}}const as=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),w(r)},os=(e,t,a)=>{const s=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return w(s)},ss=({establishmentId:e,professionalId:t,serviceIds:a,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${s}`;return w(r)},rs=e=>w("/api/appointments",{method:"POST",body:JSON.stringify(e)}),ca=(e,t)=>w(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),ns=e=>w(`/api/appointments/${e}`,{method:"DELETE"}),is=e=>w(`/api/appointments/${e}/reopen`,{method:"POST"}),ls=(e,t)=>w(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let F;async function ds(){if(!F)try{F=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function cs(){if(!F){console.warn("AudioContext não inicializado. O som não será tocado.");return}F.state==="suspended"&&F.resume();const e=F.createOscillator(),t=F.createGain();e.connect(t),t.connect(F.destination),e.type="sine",e.frequency.setValueAtTime(800,F.currentTime),t.gain.setValueAtTime(0,F.currentTime),t.gain.linearRampToValueAtTime(.3,F.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,F.currentTime+.2),e.start(F.currentTime),e.stop(F.currentTime+.2)}function g(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&cs();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},d={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;const d=r.querySelector("[data-close-modal]");d&&(d.onclick=n);const l=r.querySelector('[data-action="close-modal"]');return l&&(l.onclick=n),r.addEventListener("click",u=>{u.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Ct(e){const t=document.getElementById(e);t&&(t.style.display="none")}function us(){document.body.addEventListener("click",()=>{F||ds()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const s=t.dataset.target;if(s){const r=document.getElementById(s);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const s=document.getElementById("genericModal");s&&(s.style.display="none")}})}async function $a(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const s=await os(p.establishmentId,t,a);if(s.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=s.map(r=>`
        ...
        `).join("")}catch(s){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${s.message}</p>`}}function ms(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const s=`
    ...
    `,{modalElement:r}=N({title:"Histórico de Cancelamentos",contentHTML:s,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",$a),$a()}const z=document.getElementById("sidebar"),Ia=document.getElementById("sidebarToggle"),Fe=document.getElementById("mainContent"),ps=document.querySelectorAll(".sidebar-link"),Ca=document.getElementById("hamburger-menu-btn"),Le=document.getElementById("mobile-overlay");function vt(e){!z||!Fe||(z.classList.toggle("collapsed",e),Fe.classList.toggle("sidebar-collapsed-shift",e))}function gs(){!z||!Le||(z.classList.add("mobile-open"),Le.classList.add("visible"))}function it(){!z||!Le||(z.classList.remove("mobile-open"),Le.classList.remove("visible"))}function bs(){vt(!z.classList.contains("collapsed"))}function fs(e,t,a){if(!z||!Fe)return;Fe.classList.add("main-content-shift"),window.innerWidth>=768?vt(z.classList.contains("collapsed")):(Fe.classList.remove("main-content-shift","sidebar-collapsed-shift"),it()),Ia&&Ia.addEventListener("click",r=>{r.stopPropagation(),bs()}),z.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&z.classList.contains("collapsed")&&vt(!1)}),z.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||vt(!0))}),Ca&&Ca.addEventListener("click",r=>{r.stopPropagation(),gs()}),Le&&Le.addEventListener("click",r=>{r.stopPropagation(),it()});let s=0;z.addEventListener("touchstart",r=>{s=r.changedTouches[0].screenX},{passive:!0}),z.addEventListener("touchend",r=>{const o=r.changedTouches[0].screenX;s-o>50&&it()},{passive:!0}),ps.forEach(r=>{const o=r.getAttribute("data-target"),n=o.replace("-section",""),i=a?.[n]!==!1,d=t===null||t[o]?.view===!0;if(!i||!d){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",l=>{l.preventDefault(),o&&typeof e=="function"&&e(o),window.innerWidth<768&&it()})})}const et=e=>{const t=e||p.establishmentId;return t?w(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},vs=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},hs=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ys=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},G=e=>w(`/api/professionals/${e}`),xs=e=>w(`/api/professionals/details/${e}`),ws=e=>w("/api/professionals",{method:"POST",body:JSON.stringify(e)}),ks=(e,t)=>w(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ss=e=>w(`/api/professionals/${e}`,{method:"DELETE"});var Be;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Be||(Be={}));class Nt extends Error{constructor(t,a,s){super(t),this.message=t,this.code=a,this.data=s}}const Es=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},$s=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},s=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:Es(e),o=()=>r()!=="web",n=c=>{const m=l.get(c);return!!(m?.platforms.has(r())||i(c))},i=c=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===c)},d=c=>e.console.error(c),l=new Map,u=(c,m={})=>{const b=l.get(c);if(b)return console.warn(`Capacitor plugin "${c}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(c);let k;const h=async()=>(!k&&f in m?k=typeof m[f]=="function"?k=await m[f]():k=m[f]:t!==null&&!k&&"web"in m&&(k=typeof m.web=="function"?k=await m.web():k=m.web),k),y=(T,P)=>{var A,O;if(v){const ae=v?.methods.find(_=>P===_.name);if(ae)return ae.rtype==="promise"?_=>a.nativePromise(c,P.toString(),_):(_,rt)=>a.nativeCallback(c,P.toString(),_,rt);if(T)return(A=T[P])===null||A===void 0?void 0:A.bind(T)}else{if(T)return(O=T[P])===null||O===void 0?void 0:O.bind(T);throw new Nt(`"${c}" plugin is not implemented on ${f}`,Be.Unimplemented)}},E=T=>{let P;const A=(...O)=>{const ae=h().then(_=>{const rt=y(_,T);if(rt){const nt=rt(...O);return P=nt?.remove,nt}else throw new Nt(`"${c}.${T}()" is not implemented on ${f}`,Be.Unimplemented)});return T==="addListener"&&(ae.remove=async()=>P()),ae};return A.toString=()=>`${T.toString()}() { [capacitor code] }`,Object.defineProperty(A,"name",{value:T,writable:!1,configurable:!1}),A},$=E("addListener"),L=E("removeListener"),B=(T,P)=>{const A=$({eventName:T},P),O=async()=>{const _=await A;L({eventName:T,callbackId:_},P)},ae=new Promise(_=>A.then(()=>_({remove:O})));return ae.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await O()},ae},H=new Proxy({},{get(T,P){switch(P){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?B:$;case"removeListener":return L;default:return E(P)}}});return s[c]=H,l.set(c,{name:c,proxy:H,platforms:new Set([...Object.keys(m),...v?[f]:[]])}),H};return a.convertFileSrc||(a.convertFileSrc=c=>c),a.getPlatform=r,a.handleError=d,a.isNativePlatform=o,a.isPluginAvailable=n,a.registerPlugin=u,a.Exception=Nt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Is=e=>e.Capacitor=$s(e),We=Is(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),ua=We.registerPlugin;class go{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let s=!1;this.listeners[t]||(this.listeners[t]=[],s=!0),this.listeners[t].push(a);const o=this.windowListeners[t];o&&!o.registered&&this.addWindowListener(o),s&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,s){const r=this.listeners[t];if(!r){if(s){let o=this.retainedEventArguments[t];o||(o=[]),o.push(a),this.retainedEventArguments[t]=o}return}r.forEach(o=>o(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:s=>{this.notifyListeners(a,s)}}}unimplemented(t="not implemented"){return new We.Exception(t,Be.Unimplemented)}unavailable(t="not available"){return new We.Exception(t,Be.Unavailable)}async removeListener(t,a){const s=this.listeners[t];if(!s)return;const r=s.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(s=>{this.notifyListeners(t,s)}))}}const La=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Ba=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class Cs extends go{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(s=>{if(s.length<=0)return;let[r,o]=s.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Ba(r).trim(),o=Ba(o).trim(),a[r]=o}),a}async setCookie(t){try{const a=La(t.key),s=La(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,o=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${s||""}${r}; path=${o}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}ua("CapacitorCookies",{web:()=>new Cs});const Ls=async e=>new Promise((t,a)=>{const s=new FileReader;s.onload=()=>{const r=s.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},s.onerror=r=>a(r),s.readAsDataURL(e)}),Bs=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,o,n)=>(r[o]=e[t[n]],r),{})},Ts=(e,t=!0)=>e?Object.entries(e).reduce((s,r)=>{const[o,n]=r;let i,d;return Array.isArray(n)?(d="",n.forEach(l=>{i=t?encodeURIComponent(l):l,d+=`${o}=${i}&`}),d.slice(0,-1)):(i=t?encodeURIComponent(n):n,d=`${o}=${i}`),`${s}&${d}`},"").substr(1):null,Ms=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Bs(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const o=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))o.set(n,i);a.body=o.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const o=new FormData;if(e.data instanceof FormData)e.data.forEach((i,d)=>{o.append(d,i)});else for(const i of Object.keys(e.data))o.append(i,e.data[i]);a.body=o;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Ds extends go{async request(t){const a=Ms(t,t.webFetchExtra),s=Ts(t.params,t.shouldEncodeUrlParams),r=s?`${t.url}?${s}`:t.url,o=await fetch(r,a),n=o.headers.get("content-type")||"";let{responseType:i="text"}=o.ok?t:{};n.includes("application/json")&&(i="json");let d,l;switch(i){case"arraybuffer":case"blob":l=await o.blob(),d=await Ls(l);break;case"json":d=await o.json();break;case"document":case"text":default:d=await o.text()}const u={};return o.headers.forEach((c,m)=>{u[m]=c}),{data:d,headers:u,status:o.status,url:o.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}ua("CapacitorHttp",{web:()=>new Ds});const oe=ua("PushNotifications",{}),Ps=(e,t,a)=>w(`/api/analytics/${e}?startDate=${t}&endDate=${a}`),As=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:s})=>{let r=`/api/reports/sales/${e}?startDate=${t}&endDate=${a}`;return s&&s!=="all"&&(r+=`&cashierSessionId=${s}`),w(r)},qs=(e,t,a)=>w(`/api/analytics/${e}/monthly-details?year=${t}&month=${a}`),Ns=(e,t,a,s)=>{const r=`/api/analytics/${e}/daily-details?year=${t}&month=${a}&day=${s}`;return w(r)},Fs=(e,t,a,s)=>{const r=`/api/analytics/${e}/professional-details?year=${t}&month=${a}&professionalId=${s}`;return w(r)},Rs=()=>w("/api/reports/summary",{method:"GET"}),De=e=>w(`/api/services/${e}`),js=e=>w("/api/services",{method:"POST",body:JSON.stringify(e)}),Hs=(e,t)=>w(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),Os=e=>w(`/api/services/${e}`,{method:"DELETE"}),zs=(e,t)=>w(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),Vs=e=>w(`/api/services/stats/most-popular/${e}`),Lt=(e,t,a,s="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${s}`;return w(r)},Bt=e=>w("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ma=e=>w(`/api/blockages/${e}`,{method:"DELETE"}),bo=e=>w("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),tt=e=>w(`/api/clients/${e}`),pa=e=>w("/api/clients",{method:"POST",body:JSON.stringify(e)}),Us=(e,t)=>w(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Js=e=>w(`/api/clients/${e}`,{method:"DELETE"}),_s=(e,t,a)=>{const s=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return w(s)},Ws=(e,t,a)=>{const s=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return w(s)},Gs=(e,t,a,s)=>w("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:s})}),Ta=document.getElementById("content");let Ma=!1;const Yt=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let at=[],Tt=[],Ge={},ee=[],C={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},S={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Ys(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function fo(e){const t=new Date(e);if(t.setHours(0,0,0,0),C.currentView==="week"&&C.weekViewDays===7){const a=t.getDay(),s=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(s))}return t}function St(){const e=document.getElementById("profSelectorContainer"),t=C.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(o=>C.showInactiveProfs||o.status!=="inactive");t&&(a=a.filter(o=>o.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(o=>{const n=C.selectedProfessionalId===o.id,i=o.name==="Todos"?"Todos":o.name.split(" ")[0],d=o.name==="Todos"?"T":o.name.charAt(0).toUpperCase(),l=o.status!=="inactive",u=Yt[0],c=o.id!=="all"&&p.professionalColors.get(o.id)||u,m=o.photo||`https://placehold.co/64x64/${c.main?.replace("#","")||"E0E7FF"}/${c.light?.replace("#","")||"4F46E5"}?text=${d}`,b=o.id==="all"?"#e0e7ff":c.light,f=o.id==="all"?"#4f46e5":c.main,k=`border: 3px solid ${n?c.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+c.border:"none"};`;return`
        ...
        `}).join("")}function Xs(e,t,a,s,r){const o=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${s} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,l=encodeURIComponent(d);return`https://wa.me/${o}?text=${l}`}function Qs(e){const t=document.getElementById("agenda-view");if(e.sort((s,r)=>new Date(s.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(s=>{const r=new Date(s.startTime),o=new Date(s.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=o.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=p.professionalColors.get(s.professionalId)||{};if(s.type==="blockage")return`
                ...
                </div>`;const l=s.status==="completed",u=l?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",c=l?"Finalizado":"Aberto",m=JSON.stringify(s).replace(/'/g,"&apos;"),b=s.redeemedReward?.points>0,f=s.hasRewards&&!b,v=Xs(s.clientPhone,s.clientName,s.serviceName,s.professionalName,s.startTime);return`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",l=>{l.target.checked&&(S.data.redeemedReward=null)})}async function or(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!S.data.time||S.data.selectedServiceIds.length===0||!S.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const s=S.data.selectedServiceIds.map(l=>{const u=at.find(c=>c.id===l);return{id:u.id,name:u.name,price:u.price,duration:u.duration,bufferTime:u.bufferTime||0,photo:u.photo||null}}),[r,o]=S.data.time.split(":"),n=new Date(`${S.data.date}T${r}:${o}:00`),i={establishmentId:p.establishmentId,clientName:S.data.clientName,clientPhone:S.data.clientPhone,services:s,professionalId:S.data.professionalId,startTime:n.toISOString(),redeemedReward:S.data.redeemedReward},d=t.querySelector("#appointmentId").value;d&&(i.id=d);try{d?await ca(d,i):await rs(i),g(`Agendamento ${d?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",X()}catch(l){g(l.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function sr(e){return`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",rr)}function ir(){nr()}function lr(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",s=`
    ...
    `;return{title:a,content:s}}function dr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:e,content:a}}function cr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Tt.map(a=>{const s=S.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
    ...
    `;return{title:e,content:t}}function ur(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,s=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=S.data.date||s,o=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:o}}function mr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),s=at.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=S.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>vo(r.dataset.serviceId,r))})}function pr(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),s=Tt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=S.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(o=>{o.addEventListener("click",()=>{const n=parseInt(o.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),d=a.querySelector("#apptClientPhone");if(S.data.clientName=i.value.trim(),S.data.clientPhone=d.value.trim(),!S.data.clientName||!S.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if(S.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!S.data.professionalId)return g("Atenção","Selecione um profissional.","error");Xt(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(o=>{o.addEventListener("click",()=>Xt(parseInt(o.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if(S.step===4&&r&&r.addEventListener("submit",or),a.style.display="flex",S.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>vo(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>mr(i.target.value))}if(S.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>ho(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>pr(i.target.value))}if(S.step===1){const o=a.querySelector("#clientSearchInput");o&&(o.addEventListener("input",i=>Pa(i.target.value)),S.data.clientName&&S.data.clientPhone&&Pa(`${S.data.clientName} ${S.data.clientPhone}`));const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",ir)}if(S.step===4){const o=a.querySelector("#apptDate");o&&o.addEventListener("change",Da),Da(),ar()}}async function yo(e={}){C.currentDate=e.targetDate?new Date(e.targetDate):C.currentDate||new Date,C.scrollToAppointmentId=e.scrollToAppointmentId||null,C.profSearchTerm="",window.innerWidth<768&&(C.currentView="list"),Ta.innerHTML=`
        <section>
        ...
        </section>`,document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),C.currentView=a.dataset.view;const s=document.getElementById("week-days-toggle");if(C.currentView==="week"){if(s.style.display="flex",window.innerWidth<768){C.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="3"]');r&&r.classList.add("active")}}else s.style.display="none";X()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active")),a.classList.add("active"),C.weekViewDays=parseInt(a.dataset.days,10),X()})}),document.getElementById("todayBtn").addEventListener("click",()=>{C.currentDate=new Date,X()});const t=a=>{const s=parseInt(a.currentTarget.dataset.amount,10),r=C.currentView==="week"?ga():1,o=new Date(C.currentDate);o.setDate(o.getDate()+s*r),C.currentDate=o,X()};document.getElementById("prevBtn").addEventListener("click",t),document.getElementById("nextBtn").addEventListener("click",t),document.getElementById("profSearchInput").addEventListener("input",a=>{C.profSearchTerm=a.target.value,St()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{C.showInactiveProfs=a.target.checked,St(),X()}),Ma||(Ta.addEventListener("click",async a=>{const s=a.target.closest("[data-action]");if(a.target.closest('[data-action="select-professional"]')){const d=a.target.closest('[data-action="select-professional"]').dataset.profId,l=C.selectedProfessionalId===d&&d!=="all";if(C.selectedProfessionalId=l?"all":d,d!=="all"){const u=document.getElementById("profSearchInput");u&&(u.value=""),C.profSearchTerm=""}await X();return}if(!s)return;const r=s.dataset.action;let o=null;const n=a.target.closest("[data-appointment]");switch(n&&(o=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":Qt();break;case"edit-appointment":if(!o)return;if(o.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}o.hasRewards&&!o.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),Qt(o);break;case"delete-appointment":{const i=s.dataset.id;if(await M("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await ns(i),g("Agendamento apagado!","success"),X()}catch(l){g(`Não foi possível apagar: ${l.message}`,"error")}break}case"open-comanda":if(o){o.hasRewards&&!o.redeemedReward&&o.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=o.status==="completed"?"finalizadas":"em-atendimento",d={selectedAppointmentId:o.id,initialFilter:i};if(i==="finalizadas"){let l=o.startTime;if(o.transaction&&o.transaction.paidAt){const u=o.transaction.paidAt;typeof u=="object"&&u._seconds?l=new Date(u._seconds*1e3):l=u}d.filterDate=l}j("comandas-section",d)}break}}),Ma=!0),await er(),await X()}const gr=(e,t=null,a=1,s=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${s}`;return t&&(r+=`&date=${t}`),w(r)},br=e=>w("/api/sales",{method:"POST",body:JSON.stringify(e)}),fr=e=>w(`/api/sales/${e}/reopen`,{method:"POST"}),vr=e=>w(`/api/sales/${e}`,{method:"DELETE"}),Mt=e=>w(`/api/products/${e}`),hr=e=>w("/api/products",{method:"POST",body:JSON.stringify(e)}),yr=(e,t)=>w(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),xr=e=>w(`/api/products/${e}`,{method:"DELETE"}),wr=(e,t)=>w(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),kr=({startDate:e,endDate:t,productId:a,categoryId:s})=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("productId",a),s&&s!=="all"&&r.append("categoryId",s),w(`/api/products/stock-history/report?${r.toString()}`)},xo=()=>w("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),Sr=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),w("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},Er=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),w(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},$r=()=>w("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),Ir=e=>w(`/api/cashier/report/${e}`),wo=e=>w(`/api/packages/${e}`),Cr=e=>w("/api/packages",{method:"POST",body:JSON.stringify(e)}),Lr=(e,t)=>w(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),Br=e=>w(`/api/packages/${e}`,{method:"DELETE"});let x={allComandas:[],catalog:{services:[],products:[],packages:[]},clients:[],activeFilter:"atendimento",selectedComandaId:null,isCashierOpen:!1,activeCashierSessionId:null,paging:{page:1,limit:12,total:0}},ye=null,xe=null;function Tr(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function ge(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function Mr(){const e=new Date().toISOString().split("T")[0];xe.innerHTML=`
    ...
    `}function Pr(){N({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",Ar)}async function Ar(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector('button[type="submit"]'),s={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim()||null,phone:t.querySelector("#regClientPhone").value.trim(),dob:`${t.querySelector("#regClientDobDay").value.trim()}/${t.querySelector("#regClientDobMonth").value.trim()}`,notes:t.querySelector("#regClientNotes").value.trim()||null};if(!s.name||!s.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{const r=await pa(s);x.clients.push({id:r.id,...s}),g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",ko(r.id)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar Cliente"}}function qr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");return}const{modalElement:e,close:t}=N({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{let r="";const o=e.querySelector("#add-item-content"),n={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>',package:'<svg class="w-6 h-6 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7m8 4v10M4 7v10l8 4" /></svg>'},i=()=>{const d=r.toLowerCase(),l=c=>c.filter(m=>m.name.toLowerCase().includes(d)),u={"modal-service-list":{items:l(x.catalog.services),type:"service"},"modal-product-list":{items:l(x.catalog.products),type:"product"},"modal-package-list":{items:l(x.catalog.packages),type:"package"}};for(const[c,{items:m,type:b}]of Object.entries(u)){const f=o.querySelector(`#${c}`);f&&(f.innerHTML=m.map(v=>`
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{o>1&&(o--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{o++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await Hr(r,o),t()}};e.addEventListener("click",r=>{const o=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(o){const{itemType:i,itemId:d}=o.dataset,u=(x.catalog[i+"s"]||[]).find(c=>c.id===d);u&&s({...u,type:i})}else n&&a()}),a()}async function ko(e=null){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");return}if(!x.clients||x.clients.length===0)try{x.clients=await tt(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de clientes.","error");return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await G(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de profissionais.","error");return}const t=x.clients.map(n=>{const i=n.id===e?"selected":"";return`<option value="${n.id}" ${i}>${n.name} - ${n.phone}</option>`}).join(""),a=p.professionals.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""),s=`
    ...
    `,{modalElement:r}=N({title:"Nova Venda Avulsa",contentHTML:s,maxWidth:"max-w-md"});r.querySelector("#new-sale-form").addEventListener("submit",Vr);const o=r.querySelector('[data-action="new-client-from-sale"]');o&&o.addEventListener("click",n=>{n.preventDefault(),r.style.display="none",Pr()})}function Nr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de finalizar pagamentos.","error");return}const e=x.allComandas.find(l=>l.id===x.selectedComandaId);if(!e)return;const a=[...e.services||[],...e.comandaItems||[],...e.items||[]].reduce((l,u)=>l+(u.price||0),0);let s=[],r={remainingAmount:a,selectedMethod:"dinheiro",installments:1,amountReceived:""};const o=()=>{const l=document.getElementById("payment-list"),u=document.getElementById("remaining-amount"),c=document.getElementById("finalize-checkout-btn"),m=document.getElementById("change-container"),b=document.getElementById("installments-container"),f=document.getElementById("payment-value"),v=document.getElementById("payment-controls"),k=s.reduce((y,E)=>y+E.value,0);r.remainingAmount=a-k,l.innerHTML=s.map((y,E)=>`
    ...
    `,{modalElement:d}=N({title:"Finalizar Pagamento",contentHTML:i,maxWidth:"max-w-md"});document.getElementById("payment-value").value=r.remainingAmount.toFixed(2),d.addEventListener("click",l=>{const u=l.target.closest(".payment-method-btn");u&&(r.selectedMethod=u.dataset.method,r.installments=1,document.getElementById("installments-select").value=1,o()),l.target.closest("#add-payment-btn")&&n(),l.target.closest('[data-action="remove-payment"]')&&(s.splice(parseInt(l.target.closest('[data-action="remove-payment"]').dataset.paymentIndex,10),1),o()),l.target.closest("#finalize-checkout-btn")&&zr(e,a,s)}),d.addEventListener("change",l=>{l.target.id==="installments-select"&&(r.installments=parseInt(l.target.value,10))}),d.addEventListener("input",l=>{l.target.id==="amount-received"&&(r.amountReceived=l.target.value,o())}),o()}async function Fr(){const e=`
    ...
    `,{modalElement:t}=N({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const s=document.getElementById("initial-amount"),r=s.value.trim(),o=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),s.focus();return}try{const i={establishmentId:p.establishmentId,initialAmount:parseFloat(n.toFixed(2))};o&&(i.notes=o);const d=await Sr(i);x.isCashierOpen=!0,x.activeCashierSessionId=d.id,await ba(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function Rr(){const e=x.activeCashierSessionId;if(e)try{const t=await Ir(e),a=`
        ...
        `,{modalElement:s}=N({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});s.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const o=parseFloat(document.getElementById("final-amount").value);if(isNaN(o)||o<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await Er(e,o),x.isCashierOpen=!1,x.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await ba(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function jr(e){x.activeFilter!==e&&(x.activeFilter=e,x.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),ge(),await Z(),x.selectedComandaId=null,W())}function Zt(e){x.selectedComandaId=e,le(),Tr(),W()}async function Hr(e,t){const a=x.allComandas.find(r=>r.id===x.selectedComandaId);if(!a)return;const s=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...s),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),W(),le();return}try{await ca(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),W(),le()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function Or(e,t){const a=x.allComandas.find(o=>o.id===x.selectedComandaId);if(!a)return;let s=!1,r=(a.comandaItems||[]).findIndex(o=>o.id===e&&o.type===t);if(r>-1)a.comandaItems.splice(r,1),s=!0;else{let o=(a.services||[]).findIndex(n=>n.id===e);if(o>-1)a.services.splice(o,1),s=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),s=!0)}}if(s){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),W(),le();return}try{await ca(a.id,a),g("Sucesso","Item removido!","success"),W(),le()}catch(o){g("Erro",`Não foi possível remover o item: ${o.message}`,"error"),await Z()}}}async function zr(e,t,a){const s=e.type==="appointment",r=[...e.services||[],...e.comandaItems||[],...e.items||[]],o={payments:a,totalAmount:t,items:r,cashierSessionId:x.activeCashierSessionId};try{s?await ls(e.id,o):(o.clientName=e.clientName,o.professionalId=e.professionalId,o.clientPhone=e.clientPhone,await br(o)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",ge(),x.selectedComandaId=null,await Z()}catch(n){g("Erro no Checkout",n.message,"error")}}async function Vr(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,s=x.clients.find(n=>n.id===t),r=p.professionals.find(n=>n.id===a);if(!s||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const o={id:`temp-${Date.now()}`,type:"walk-in",clientName:s.name,clientPhone:s.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};x.allComandas.unshift(o),x.selectedComandaId=o.id,document.getElementById("genericModal").style.display="none",Zt(o.id)}async function Z(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=x.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await xo();if(x.isCashierOpen=!!a,x.activeCashierSessionId=a?a.id:null,Dr(),!x.isCashierOpen&&x.activeFilter==="atendimento"){le(),W();return}const s=await gr(p.establishmentId,t,x.paging.page,x.paging.limit);if(x.allComandas=s.data||s,x.paging.total=s.total||s.length,x.catalog.services.length===0){const[r,o,n,i,d]=await Promise.all([De(p.establishmentId),Mt(p.establishmentId),wo(p.establishmentId),tt(p.establishmentId),G(p.establishmentId)]);x.catalog={services:r,products:o,packages:n},x.clients=i,p.professionals=d}le(),x.selectedComandaId,W()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function ba(e={}){xe=document.getElementById("content");try{const t=await xo();x.isCashierOpen=!!t,x.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),x.isCashierOpen=!1}x.selectedComandaId=e.selectedAppointmentId||null,Mr(),ye&&(xe.removeEventListener("click",ye),xe.removeEventListener("change",ye)),ye=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&x.activeFilter==="finalizadas"){x.paging.page=1,await Z();return}if(a){if(a.matches("[data-filter]"))jr(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}Zt(a.dataset.comandaId)}else if(a.matches("[data-action]")){const s=a.dataset.action,r=a.dataset.id||x.selectedComandaId;switch(s){case"back-to-list":{ge(),x.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(o=>o.classList.remove("selected")),W();break}case"new-sale":ko();break;case"add-item":qr();break;case"checkout":Nr();break;case"open-cashier":Fr();break;case"close-cashier":await Rr();break;case"view-sales-report":j("sales-report-section");break;case"remove-item":await Or(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await M("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await is(r);const n=x.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete x.allComandas[n].transaction,delete x.allComandas[n].cashierSessionId,delete x.allComandas[n].redeemedReward,x.allComandas[n].status="confirmed"),x.selectedComandaId=null,ge(),g("Sucesso!","Comanda reaberta para edição.","success"),await Z()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await M("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await fr(r),g("Sucesso!","Venda revertida."),ge(),x.selectedComandaId=null,await Z()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const o=a.dataset.id,n=a.dataset.date;j("agenda-section",{scrollToAppointmentId:o,targetDate:new Date(n)});break}case"delete-walk-in":{if(await M("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))x.allComandas=x.allComandas.filter(n=>n.id!==r),x.selectedComandaId=null,le(),W(),g("Sucesso","Venda avulsa removida.","success"),ge();else try{await vr(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),x.selectedComandaId=null,ge(),await Z()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},xe.addEventListener("click",ye),xe.addEventListener("change",ye),e.initialFilter&&(x.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(x.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${x.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",x.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await Z(),x.selectedComandaId&&Zt(x.selectedComandaId)}const Dt=document.getElementById("content");let ie={},Ur=null,Jr=null,q={year:null,month:null,monthName:null,professionalId:null,professionalName:null},qa=[];const we=["#4f46e5","#22c55e","#f97316","#06b6d4","#e11d48","#6366f1","#84cc16","#f59e0b"];function fa(){Object.values(ie).forEach(e=>e?.destroy()),ie={}}function _r(e){const t=document.getElementById("kpi-revenue"),a=document.getElementById("kpi-transactions"),s=document.getElementById("kpi-popular-item");t&&(t.textContent=`R$ ${e.totalRevenue.toFixed(2)}`),a&&(a.textContent=e.totalTransactions),s&&(s.textContent=e.mostPopularItem||"N/A"),document.querySelectorAll(".kpi-loader").forEach(r=>r.classList.add("hidden")),document.querySelectorAll(".kpi-value").forEach(r=>r.classList.remove("hidden"))}function Wr(e){const t="Detalhes da Transação",a=`
    ...
    `;N({title:t,contentHTML:a,maxWidth:"max-w-md"})}async function Ye(e,t,a){q={year:e,month:t,monthName:a},Dt.innerHTML=`
        <section>
        ...
        </section>`,document.getElementById("backToMainBtn").addEventListener("click",Kt);try{const s=await qs(p.establishmentId,e,t);Jr=s,document.querySelector(".loader-container").classList.add("hidden"),document.getElementById("monthly-details-view").classList.remove("hidden");const r=s.revenueByDay.map(m=>m.day),o=s.revenueByDay.map(m=>m.revenue),n=s.salesByProfessional.map(m=>m.name),i=s.salesByProfessional.map(m=>m.count),d=s.topItems.map(m=>m.name),l=s.topItems.map(m=>m.count),u=["Agendamentos","Vendas Avulsas"],c=[s.revenueByTransactionType.appointment,s.revenueByTransactionType.sales];fa(),ie.monthlyRevenue=new Chart(document.getElementById("monthlyRevenueChart").getContext("2d"),{type:"bar",data:{labels:r,datasets:[{label:"Receita Diária",data:o,backgroundColor:we[0]}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0}},onClick:(m,b)=>{if(b.length>0){const f=r[b[0].index];Gr(e,t,f)}}}}),ie.salesByProfessional=new Chart(document.getElementById("salesByProfessionalChart").getContext("2d"),{type:"pie",data:{labels:n,datasets:[{label:"Vendas",data:i,backgroundColor:we,hoverOffset:16}]},options:{responsive:!0,plugins:{legend:{position:"right"}},onClick:(m,b)=>{if(b.length>0){const f=b[0].index,v=s.salesByProfessional[f];v&&Yr(e,t,v.id,v.name)}}}}),ie.topItems=new Chart(document.getElementById("topItemsChart").getContext("2d"),{type:"bar",data:{labels:d,datasets:[{label:"Itens Vendidos",data:l,backgroundColor:we[2]}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{display:!1}},scales:{x:{beginAtZero:!0}}}}),ie.revenueByType=new Chart(document.getElementById("revenueByTypeChart").getContext("2d"),{type:"doughnut",data:{labels:u,datasets:[{label:"Receita por Tipo",data:c,backgroundColor:[we[1],we[3]],hoverOffset:16}]},options:{responsive:!0,plugins:{legend:{position:"right"}}}})}catch(s){g("Erro",`Não foi possível carregar os detalhes do mês: ${s.message}`,"error"),Kt()}}async function Gr(e,t,a){q={...q,day:a},Dt.innerHTML=`
        <section>
            <div class="flex justify-between items-center mb-6">
        ...
        `).join(""),document.querySelectorAll("#daily-transactions-list tr").forEach(o=>{o.addEventListener("click",n=>{const i=n.currentTarget.getAttribute("data-index"),d=qa[i];d&&Wr(d)})})}catch(s){g("Erro",`Não foi possível carregar os detalhes diários: ${s.message}`,"error"),Ye(q.year,q.month,q.monthName)}}async function Yr(e,t,a,s){q={year:e,month:t,professionalId:a,professionalName:s},Dt.innerHTML=`
        <section>
            <div class="flex justify-between items-center mb-6">
                <button id="backToMonthlyBtn" class="py-2 px-4 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300">
                    < Voltar
        ...
        `).join("")}catch(r){g("Erro",`Não foi possível carregar os detalhes do profissional: ${r.message}`,"error"),Ye(q.year,q.month,q.monthName)}}async function Xr(e,t,a){const s=t.getElementsAtEventForMode(e,"nearest",{intersect:!0},!1);if(s.length>0){const o=s[0].index,n=a[o];n&&await Ye(n.year,n.monthIndex,n.month)}}async function Na(){const e=document.getElementById("main-reports-view"),t=document.getElementById("startDate"),a=document.getElementById("endDate");if(!e||!t||!a){console.error("Elementos essenciais para o relatório não foram encontrados no DOM.");return}e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';const s=t.value,r=a.value;if(!s||!r)return e.innerHTML='<p class="text-red-500 p-8 text-center">Selecione as datas para gerar o relatório.</p>',g("Atenção","Por favor, selecione as datas de início e fim.","error");try{const o=await Ps(p.establishmentId,s,r);Ur=o,e.innerHTML=`
            ...
            </div>`,_r(o.kpis),setTimeout(()=>{const n=document.getElementById("transactionsByMonthChart");n&&(fa(),ie.main=new Chart(n.getContext("2d"),{type:"bar",data:{labels:o.transactionsByMonth.map(i=>i.month),datasets:[{label:"Receita Total",data:o.transactionsByMonth.map(i=>i.revenue),backgroundColor:we[0],borderRadius:5}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{callbacks:{label:i=>`R$ ${i.raw.toFixed(2)}`}}},scales:{y:{beginAtZero:!0}},onClick:(i,d)=>Xr(i,ie.main,o.transactionsByMonth)}}))},50)}catch(o){g("Erro",`Não foi possível carregar os relatórios: ${o.message}`,"error"),e.innerHTML='<p class="text-red-500 p-8 text-center">Erro ao carregar os dados dos relatórios. Por favor, tente novamente.</p>'}}async function Kt(){fa();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Dt.innerHTML=`
        <section>
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Na),await Na()}const Pt=(e,t="products")=>w(`/api/${t}/categories/${e}`),So=(e,t="products")=>w(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),Eo=(e,t="products")=>w(`/api/${t}/categories/${e}`,{method:"DELETE"}),Qr="audit_logs",Te=async(e,t,a,s,r,o=null)=>{try{if(!t)return;await lo(Ke(de,Qr),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:s,description:r,details:o,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},pe=document.getElementById("content");let se=null,Re="services",fe="all";function Me(){const e=J.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function Zr(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await So({establishmentId:p.establishmentId,name:s},"services"),Te(p.establishmentId,Me(),"Categorias (Serviços)","Criou",`Criou categoria: ${s}`),a.value="",g("Sucesso","Categoria criada!","success"),await va(),await ot()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function Kr(e){if(await M("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await Eo(e,"services"),Te(p.establishmentId,Me(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await va(),await ot()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function va(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await Pt(p.establishmentId,"services");p.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),ha(),ke()}function sn(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function ot(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,s,r]=await Promise.all([De(p.establishmentId),G(p.establishmentId),Pt(p.establishmentId,"services"),Vs(p.establishmentId)]);p.services=(t||[]).filter(Boolean),p.professionals=(a||[]).filter(Boolean),p.serviceCategories=(s||[]).filter(Boolean),p.mostPopularService=r||{name:"N/A",count:0},p.services.forEach(o=>{o.active===void 0&&(o.active=!0)}),$o(Re)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function $o(e){if(document.getElementById("services-content-container")){if(Re===e&&document.getElementById("services-content-container").children.length>1){Re==="services"&&(ha(),ke());return}Re=e,fe="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?on():e==="reports"&&sn()}}function rn(){se&&(pe.removeEventListener("click",se),pe.removeEventListener("input",se),pe.removeEventListener("change",se)),se=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),o=r.dataset.id,n=r.checked;try{await zs(o,n);const i=p.services.findIndex(d=>d.id===o);i>-1&&(p.services[i].active=n),Te(p.establishmentId,Me(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${o}) para ${n?"Ativo":"Inativo"}`),ke(),ha()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,ke()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){ke();return}if(!a)return;if(a.hasAttribute("data-view")){$o(a.dataset.view);return}switch(a.dataset.action){case"new-service":Fa();break;case"edit-service":const r=JSON.parse(a.dataset.service);Fa(r);break;case"manage-categories":en();break;case"filter-service":const o=a.dataset.filterType;if(o==="popular")return;fe=o==="total"?"all":o,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,l=i===fe||i==="total"&&fe==="all";n.classList.toggle("ring-2",l),n.classList.toggle("ring-indigo-500",l),n.classList.toggle("shadow-lg",l)}),ke();break}},pe.addEventListener("click",se),pe.addEventListener("input",se),pe.addEventListener("change",se)}async function nn(){pe.innerHTML=`
        ...
        </section>`,rn();try{(!p.professionals||p.professionals.length===0)&&(p.professionals=await G(p.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}Re="services",fe="all",await ot()}const Io="suppliers",Co="purchases",ya=async()=>{try{const e=await co(Ke(de,Io)),t=[];return e.forEach(a=>{t.push({id:a.id,...a.data()})}),t}catch(e){throw console.error("Erro ao buscar fornecedores:",e),e}},ln=async e=>{try{const t=da(de,Io,e);return await Go(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},dn=async e=>{try{const t={...e,createdAt:new Date};return{id:(await lo(Ke(de,Co),t)).id,...t}}catch(t){throw console.error("Erro ao registrar compra:",t),t}},cn=async e=>{try{const t=uo(Ke(de,Co),mo("createdAt","desc")),a=await co(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},ce=document.getElementById("content");let re=null,je="products",Q="all";async function un(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await So({establishmentId:p.establishmentId,name:s},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await xa(),await st()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function mn(e){if(await M("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await Eo(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await xa(),await st()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function xa(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await Pt(p.establishmentId,"products");p.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${a.name}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function pn(){N({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),wa(),Xe()}function vn(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const s=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
        ...
        </div>`;const r=document.getElementById("productFilterReport"),o=document.getElementById("categoryFilterReport");r&&p.products&&(r.innerHTML+=p.products.map(n=>`<option value="${n.id}">${n.name}</option>`).join("")),o&&p.categories&&(o.innerHTML+=p.categories.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""))}async function hn(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value};try{const a=await kr(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function st(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,s]=await Promise.all([Mt(p.establishmentId),Pt(p.establishmentId,"products"),ya()]);p.products=(t||[]).filter(Boolean),p.categories=(a||[]).filter(Boolean),p.suppliers=(s||[]).filter(Boolean),Lo(je)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function Lo(e){if(document.getElementById("products-content-container")){if(je===e&&document.getElementById("products-content-container").children.length>1){je==="products"&&(wa(),Xe());return}je=e,Q="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?fn():e==="movements"&&vn()}}async function yn(){ce.innerHTML=`
    ...
    `;a.forEach(o=>{const n=JSON.stringify(o).replace(/"/g,"&quot;");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=s+r}function En(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",s=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),o=`
    ...
    `;N({title:"",contentHTML:o,maxWidth:"max-w-md"})}async function He(){const e=document.getElementById("purchasesContainer");if(e)if(D.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Mt(p.establishmentId),ya()]);D.allSuppliers=a||[];const s=t.filter(i=>{const d=parseInt(i.currentStock||0),l=parseInt(i.minStock||0);return d<=l});if(D.productsToBuy=s,s.length===0){e.innerHTML=`
            ...
            `,ea()}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao calcular compras.</p>'}}else D.step===2&&$n(e)}function $n(e){if(!D.finalOrders||Object.keys(D.finalOrders).length===0){D.step=1,He();return}let t="",a=0;for(const[s,r]of Object.entries(D.finalOrders)){let o=0,n=r.items.map(u=>{const c=u.qty*u.cost;return o+=c,`
    ...
    `}async function In(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await cn(p.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(o=>{const n=new Date(o.createdAt.seconds*1e3).toLocaleDateString("pt-BR");a+=`
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function Cn(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;N({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",Sn),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Ct("genericModal"))},50)}function Ln(){ue.innerHTML=`
...
Aguardo confirmação.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(n)}&body=${encodeURIComponent(i)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=p.establishmentId,dn(s).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const a=e.target.closest(".supplier-order-card"),s=a.dataset.supplierName,r=[];a.querySelectorAll(".border-b").forEach(n=>{const i=n.querySelector(".text-gray-800"),d=n.querySelector(".text-gray-600");if(i&&d){const l=i.innerText,u=d.innerText.split(" x ")[0];r.push({name:l,qty:u})}}),Ha("Pedido de Compra",s,r,!1)}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);Cn(s)}},ue.addEventListener("click",ne),ue.addEventListener("input",ne),ue.addEventListener("change",ne),lt("list")}function lt(e){ht=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),xn()}const Ft=document.getElementById("content"),za={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let te=new Set,dt=null,Se=null;function Bn(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Tn(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",s=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,r=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function Rt(){const e=document.getElementById("genericModal");e.style.display="none",Se&&e.removeEventListener("click",Se)}async function Mn(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},s=p.services||await De(p.establishmentId),r=p.professionals||await G(p.establishmentId),o=`
        ...
        </div>`;t.innerHTML=o,t.style.display="flex",Dn(a,s),An(a),qn(a,r),Fn(a)}function Dn(e,t){const a=document.getElementById("professionalForm"),s=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(m,b)=>{const f=b+1,v=f==s[1]?"selected":"",k=new Date(0,b).toLocaleString("pt-BR",{month:"long"});return`<option value="${f}" ${v}>${k.charAt(0).toUpperCase()+k.slice(1)}</option>`}).join(""),o=e.status||"active";a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${e.notes||""}</textarea></div>`;const n=document.getElementById("profPhotoInput"),i=document.getElementById("profPhotoButton"),d=document.getElementById("profPhotoPreview"),l=document.getElementById("profPhotoBase64"),u=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,c=e.photo||"";i&&i.addEventListener("click",()=>n.click()),n&&(n.onchange=async()=>{const m=n.files[0];if(m){d.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const b=await Pn(m,800,800,"image/jpeg",.8);d.src=b,l.value=b}catch(b){g("Erro de Imagem",b.message||"Não foi possível processar a imagem.","error"),d.src=u,l.value=c,n.value=""}}})}function Pn(e,t=800,a=800,s="image/jpeg",r=.8){return new Promise((o,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=d=>{const l=new Image;l.onload=()=>{let u=l.width,c=l.height;u>c?u>t&&(c*=t/u,u=t):c>a&&(u*=a/c,c=a);const m=document.createElement("canvas");m.width=u,m.height=c,m.getContext("2d").drawImage(l,0,0,u,c);const f=m.toDataURL(s,r);o(f)},l.onerror=u=>n(new Error("Não foi possível carregar a imagem.")),l.src=d.target.result},i.onerror=d=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function An(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',Nn(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function qn(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
        ...
        `).join("")}catch(s){a.innerHTML=`<p class="text-red-500">${s.message}</p>`}}}function Fn(e){const t=document.getElementById("genericModal");Se&&t.removeEventListener("click",Se),Se=async a=>{const s=a.target.closest("button[data-action]");if(!s){const o=a.target.closest(".tab-link");o&&(t.querySelectorAll(".tab-link").forEach(n=>n.classList.remove("active")),o.classList.add("active"),t.querySelectorAll(".tab-content").forEach(n=>n.classList.add("hidden")),document.getElementById(o.dataset.tab).classList.remove("hidden"));return}const r=s.dataset.action;switch(a.stopPropagation(),r){case"close-modal":Rt();break;case"delete-professional":const o=s.dataset.id;if(await M("Excluir Profissional",`Tem certeza que deseja excluir ${e.name}? Esta ação não pode ser desfeita.`))try{await Ss(o),g("Sucesso!","Profissional excluído.","success"),Rt(),Et()}catch(v){g("Erro",`Não foi possível excluir: ${v.message}`,"error")}break;case"save-professional":const i=document.getElementById("professionalForm"),d=s,l=document.getElementById("profScheduleContainer"),u=Array.from(i.querySelectorAll("#profServicesContainer input:checked")).map(v=>v.value),c={};l&&l.querySelectorAll(".day-schedule-card").forEach(v=>{const k=v.querySelector('[data-field="active"]').dataset.day;c[k]={active:v.querySelector('[data-field="active"]').checked,start:v.querySelector('[data-field="start"]').value,end:v.querySelector('[data-field="end"]').value,breakStart:v.querySelector('[data-field="breakStart"]').value,breakEnd:v.querySelector('[data-field="breakEnd"]').value}});const m={...e,id:i.querySelector("#professionalId").value||void 0,name:i.querySelector("#profName").value,specialty:i.querySelector("#profSpecialty").value,photo:i.querySelector("#profPhotoBase64").value,services:u,workingHours:c,phone:i.querySelector("#profPhone").value,dob:`${i.querySelector("#profDobDay").value}/${i.querySelector("#profDobMonth").value}`,receivesCommission:i.querySelector("#profCommission").value==="sim",showOnAgenda:i.querySelector("#profShowOnAgenda").value==="sim",orderOnAgenda:parseInt(i.querySelector("#profOrderOnAgenda").value)||1,notes:i.querySelector("#profNotes").value,status:i.querySelector("#profStatus").value};d.disabled=!0,d.textContent="A salvar...";try{m.id?(await ks(m.id,m),g("Sucesso!","Profissional atualizado.","success")):(delete m.id,await ws(m),g("Sucesso!","Profissional criado.","success")),Rt(),Et()}catch(v){g("Erro",v.message,"error"),d.disabled=!1,d.textContent="Salvar"}break;case"delete-blockage":const b=s.dataset.id;if(await M("Apagar Bloqueio","Tem certeza?"))try{await ma(b),g("Bloqueio removido.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";Oe(e.id,v)}catch(v){g("Erro",v.message,"error")}break;case"batch-delete-blockage":const f=JSON.parse(s.dataset.ids);if(await M("Apagar em Lote",`Tem certeza que deseja apagar ${f.length} bloqueios com este motivo?`))try{await bo(f),g("Bloqueios removidos.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";Oe(e.id,v)}catch(v){g("Erro",v.message,"error")}break}},t.addEventListener("click",Se)}function ta(){const e=document.getElementById("batch-actions-container"),t=document.getElementById("selected-count");!e||!t||(te.size>0?(t.textContent=`${te.size} selecionado(s)`,e.classList.remove("hidden")):e.classList.add("hidden"))}function Rn(){M("Excluir em Lote",`Tem certeza que deseja excluir ${te.size} profissionais? Esta ação não pode ser desfeita.`).then(async e=>{if(e)try{await(void 0)(Array.from(te)),g("Sucesso!",`${te.size} profissionais foram excluídos.`,"success"),te.clear(),ta(),Et()}catch(t){g("Erro",`Não foi possível excluir em lote: ${t.message}`,"error")}})}function Ae(){const e=document.getElementById("professionalsList");if(!e)return;if(!p.professionals){e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=Bn();return}const t=document.getElementById("showInactiveProfToggle").checked,a=document.getElementById("profSearchInput").value.toLowerCase(),s=p.professionals.filter(r=>{const o=r.name.toLowerCase().includes(a),n=t||r.status!=="inactive";return o&&n});e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=Tn(s)}async function Et(){te.clear(),Ft.innerHTML=`
        ...
        </section>`,dt&&Ft.removeEventListener("click",dt),dt=t=>{const a=t.target.closest('[data-action="open-professional-modal"]'),s=t.target.closest('[data-action="batch-delete"]');if(a){t.preventDefault();let o={};if(a.dataset.professional)try{o=JSON.parse(a.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}Mn(o);return}if(s){Rn();return}const r=t.target.closest(".professional-checkbox");if(r){const o=r.dataset.id;r.checked?te.add(o):te.delete(o),Ae(),ta();return}},Ft.addEventListener("click",dt),document.getElementById("profSearchInput").addEventListener("input",Ae),document.getElementById("showInactiveProfToggle").addEventListener("change",Ae);const e=document.getElementById("professionalsList");p.professionals=null,p.services=null,Ae();try{const[t,a]=await Promise.all([G(p.establishmentId),De(p.establishmentId)]);p.professionals=t,p.services=a,Ae(),ta()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}const jt=document.getElementById("content");let K=[],ze={},R=null,aa="list",U="all",oa="O Estabelecimento";const jn=(e,t)=>`Olá, ${e}! Nós da ${t} desejamos a você um Feliz Aniversário! Esperamos que seu dia seja maravilhoso. Venha comemorar conosco! 🎉🎂`,Hn=(e,t)=>`Oi, ${e}! Faz um tempo que não te vemos aqui no(a) ${t}. Sentimos sua falta! Temos novidades/ofertas especiais para você. Que tal agendar seu horário?`,On=[{value:30,label:"30 dias"},{value:60,label:"60 dias"},{value:90,label:"90 dias"},{value:120,label:"120 dias"}];function zn(){return Math.floor(Math.random()*140)+10}function Vn(e){if(!e.dob)return!1;const t=e.dob.split("/");if(t.length!==2)return!1;const a=new Date,s=a.getDate(),r=a.getMonth()+1,o=parseInt(t[0],10),n=parseInt(t[1],10);return o===s&&n===r}const Un=[{value:99,label:"Aniversariantes de Hoje"},{value:0,label:"Todos os meses (com DOB)"},{value:1,label:"Janeiro"},{value:2,label:"Fevereiro"},{value:3,label:"Março"},{value:4,label:"Abril"},{value:5,label:"Maio"},{value:6,label:"Junho"},{value:7,label:"Julho"},{value:8,label:"Agosto"},{value:9,label:"Setembro"},{value:10,label:"Outubro"},{value:11,label:"Novembro"},{value:12,label:"Dezembro"}];function Va(){return Un.map(e=>{let t="";return e.value===99&&(t="selected"),`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function Ua(){return On.map(e=>{const t=e.value===90?"selected":"";return`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function Jn(e,t){const a=`w-5 h-5 ${t} mr-2`;switch(e){case"cadastro":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>`;case"agendamentos":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>`;case"historico":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v2a2 2 0 01-2 2h-2.586a1 1 0 00-.707.293l-1.414 1.414a1 1 0 01-1.414 0l-1.414-1.414A1 1 0 009.586 17H7a2 2 0 01-2-2v-2a2 2 0 012-2h12z" /></svg>`;case"fidelidade":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z" /><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.5a9.5 9.5 0 1019 0 9.5 9.5 0 00-19 0z" /></svg>`;default:return""}}function _n(e="cadastro"){const t=[{id:"cadastro",label:"Cadastro"},{id:"agendamentos",label:"Próximos Agend."},{id:"historico",label:"Histórico"},{id:"fidelidade",label:"Fidelidade"}],a=document.getElementById("client-detail-tabs");a&&(a.innerHTML=t.map(s=>{const r=e===s.id,o=r?"text-indigo-600":"text-gray-500";return`
        ...
        `}).join(""),a.querySelectorAll(".tab-btn").forEach(s=>{s.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),sa(s.dataset.tab)})}))}async function sa(e){_n(e);const t=document.getElementById("client-detail-content");if(t)switch(t.innerHTML='<form id="client-form" class="p-6 space-y-4"><div class="loader mx-auto my-8"></div></form>',e){case"cadastro":t.innerHTML=Wn(R);break;case"agendamentos":case"historico":try{const s=await _s(p.establishmentId,R.name,R.phone);t.innerHTML=Gn(s,e)}catch(s){console.error("Erro ao carregar histórico do cliente:",s),t.innerHTML=`<form id="client-form" class="p-6 space-y-4"><p class="text-center text-red-500">Erro ao carregar o histórico: ${s.message}</p></form>`}break;case"fidelidade":const a=await Ws(p.establishmentId,R.name,R.phone);t.innerHTML=Yn(R,a);break;default:t.innerHTML='<form id="client-form" class="p-6 space-y-4"><p class="p-4 text-center text-gray-500">Secção não implementada.</p></form>'}}function Wn(e){const t=e?.dob?e.dob.split("/"):["",""];return`
    ...
    `}function Yn(e,t){const a=e.loyaltyPoints||0;let s='<p class="text-sm text-gray-500">O programa de fidelidade não está ativo.</p>';ze.enabled&&ze.tiers&&(s=ze.tiers.map(o=>{const n=a>=o.points;return`
    ...
    `}function ra(e){R=e,aa="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",s=`
    ...
    `,r=window.innerWidth<768,o=r?"max-w-full":"max-w-3xl";if(N({title:a,contentHTML:s,maxWidth:o}),r){const u=document.getElementById("genericModal");if(u){const c=u.querySelector(`.${o.replace(":","\\:")}`);c&&(c.style.height="auto",c.style.maxHeight="85vh",c.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&n.addEventListener("click",async u=>{const c=u.target.closest("[data-action]");if(!c)return;switch(c.dataset.action){case"redeem-reward":{const b=parseInt(c.dataset.points,10),f=c.dataset.reward;if(await M("Confirmar Resgate",`Deseja resgatar "${f}" por ${b} pontos?`))try{await Gs(p.establishmentId,R.name,R.phone,{points:b,reward:f}),g("Prémio resgatado com sucesso!","success"),K=await tt(p.establishmentId);const h=K.find(y=>y.id===R.id);h&&(R=h),sa("fidelidade")}catch(k){g(`Erro ao resgatar: ${k.message}`,"error")}break}case"open-comanda-from-history":{const b=c.dataset.appointmentId;b&&(document.getElementById("genericModal").style.display="none",j("comandas-section",{selectedAppointmentId:b,initialFilter:"finalizada"}));break}case"view-appointment":{const b=c.dataset.appointmentId,f=c.dataset.appointmentDate;b&&f&&(document.getElementById("genericModal").style.display="none",j("agenda-section",{targetDate:f,scrollToAppointmentId:b}));break}}}),sa("cadastro");const i=document.getElementById("client-form");i&&i.addEventListener("submit",u=>{u.preventDefault(),Xn()});const d=document.getElementById("cancelDetailViewBtn");d&&d.addEventListener("click",u=>{u.preventDefault(),document.getElementById("genericModal").style.display="none",qt()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await Qn()})}async function Xn(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await Us(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await pa(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await qt()}catch(s){g("Erro",`Não foi possível salvar: ${s.message}`,"error")}}async function Qn(){if(!R||!R.id)return;if(await M("Excluir Cliente",`Tem certeza que deseja excluir ${R.name}? Esta ação é irreversível.`))try{await Js(R.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await qt()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function yt(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const s=U==="inactive",r=U==="birthdays";e.forEach(o=>{const n=document.createElement("div");n.className="client-card bg-white rounded-lg shadow p-4 flex flex-col cursor-pointer",n.dataset.clientId=o.id;const i=o.loyaltyPoints||0,d=ze.enabled?`${i} pts`:`R$ ${i.toFixed(2)}`;let l="";const c=`https://wa.me/55${o.phone?o.phone.replace(/\D/g,""):""}?text=`;if(s){const m=encodeURIComponent(Hn(o.name,oa));l=`
                    ...
                    </a>`}else if(r&&Vn(o)){const b=encodeURIComponent(jn(o.name,oa));l=`
    ...
    `;try{const[u,c]=await Promise.all([tt(p.establishmentId),et(p.establishmentId)]);K=u,ze=c.loyaltyProgram||{enabled:!1},oa=c.name||"O Estabelecimento";const m=xt("",U);yt(m,K.length)}catch{document.getElementById("clientsList").innerHTML='<p class="text-red-500 col-span-full text-center">Erro ao carregar dados dos clientes.</p>'}const e=document.getElementById("filter-sheet"),t=document.getElementById("filter-overlay"),a=document.getElementById("openFilterSheetBtn"),s=document.getElementById("closeFilterSheetBtn"),r=()=>{e.classList.add("show"),t.classList.remove("hidden")},o=()=>{e.classList.remove("show"),t.classList.add("hidden")};a&&a.addEventListener("click",r),s&&s.addEventListener("click",o),t&&t.addEventListener("click",o);const n=u=>{const c=u.target.closest(".client-filter-btn");c&&(Zn(c.dataset.filterKey),window.innerWidth<768&&o())},i=document.getElementById("desktop-filter-bar"),d=document.getElementById("mobile-filter-list");i&&i.addEventListener("click",n),d&&d.addEventListener("click",n);const l=u=>{const c=document.getElementById(u);c&&c.addEventListener("change",()=>{if(U==="birthdays"||U==="inactive"){const m=document.getElementById("clientSearchInput").value,b=xt(m,U);yt(b,K.length)}})};l("birthMonthFilter"),l("mobileBirthMonthFilter"),l("inactiveDaysFilter"),l("mobileInactiveDaysFilter"),jt.addEventListener("click",async u=>{const c=u.target.closest("[data-action]"),m=u.target.closest(".client-card");if(aa==="list"){if(c){const b=c.dataset.action;b==="new-client"?ra(null):b==="print-list"&&window.print()}else if(m){const b=m.dataset.clientId,f=K.find(v=>v.id===b);f&&ra(f)}}}),jt.addEventListener("input",u=>{if(u.target.id==="clientSearchInput"){const c=u.target.value,m=xt(c,U);yt(m,K.length)}})}const Qe=()=>w("/api/financial/natures"),Kn=e=>w("/api/financial/natures",{method:"POST",body:JSON.stringify(e)}),ei=e=>w(`/api/financial/natures/${e}`,{method:"DELETE"}),Ze=()=>w("/api/financial/cost-centers"),ti=e=>w("/api/financial/cost-centers",{method:"POST",body:JSON.stringify(e)}),ai=e=>w(`/api/financial/cost-centers/${e}`,{method:"DELETE"}),Bo=(e,t)=>w(`/api/financial/${e}`,{method:"POST",body:JSON.stringify(t)}),To=(e,t={})=>{let a=`/api/financial/${e}`;const s=new URLSearchParams;t.startDate&&s.append("startDate",t.startDate),t.endDate&&s.append("endDate",t.endDate),t.natureId&&s.append("natureId",t.natureId),t.costCenterId&&s.append("costCenterId",t.costCenterId);const r=s.toString();return r&&(a+=`?${r}`),w(a)},Mo=(e,t,a)=>w(`/api/financial/${e}/${t}`,{method:"PUT",body:JSON.stringify(a)}),Do=(e,t)=>w(`/api/financial/${e}/${t}`,{method:"DELETE"}),Po=(e,t,a)=>w(`/api/financial/${e}/${t}/status`,{method:"PATCH",body:JSON.stringify({status:"paid",paymentDate:a})}),oi=e=>Bo("payables",e),si=e=>To("payables",e),ri=(e,t)=>Mo("payables",e,t),ni=e=>Do("payables",e),ii=(e,t)=>Po("payables",e,t),li=e=>Bo("receivables",e),di=e=>To("receivables",e),ci=(e,t)=>Mo("receivables",e,t),ui=e=>Do("receivables",e),mi=(e,t)=>Po("receivables",e,t),pi=(e,t)=>w(`/api/financial/cash-flow?startDate=${e}&endDate=${t}`),gi=()=>w("/api/financial/today-summary"),be=document.getElementById("content"),Ht={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"},bi={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}},Ao=[{id:"personal-data",icon:"M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z",label:"Dados Gerais"},{id:"branding",icon:"M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z",label:"Identidade e Cores"},{id:"booking",icon:"M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z",label:"Agendamento Online"},{id:"working-hours",icon:"M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z",label:"Horário de Funcionamento"},{id:"loyalty",icon:"M5 5a2 2 0 012-2h10a2 2 0 012 2v1h2a1 1 0 011 1v3a1 1 0 01-1 1h-2v1a2 2 0 01-2 2H7a2 2 0 01-2-2v-1H3a1 1 0 01-1-1V7a1 1 0 011-1h2V5z",label:"Plano de Fidelidade"},{id:"financial",icon:"M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z",label:"Integração Financeira"},{id:"change-password",icon:"M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z",label:"Alterar senha"},{id:"change-email",icon:"M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207",label:"Alterar E-mail de Acesso"}];let V=null;function Ja(e,t,a){return new Promise((s,r)=>{const o=new FileReader;o.readAsDataURL(e),o.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const d=document.createElement("canvas");let l=i.width,u=i.height;l>t&&(u*=t/l,l=t),d.width=l,d.height=u,d.getContext("2d").drawImage(i,0,0,l,u);const m=e.type==="image/png"&&t<500?"image/png":"image/jpeg";s(d.toDataURL(m,a))},i.onerror=d=>r(d)},o.onerror=n=>r(n)})}function _a(e,t=null){let a='<option value="">-- Selecione (Opcional) --</option>';const s=n=>{const i=new Map,d=[];return n&&(n.forEach(l=>i.set(l.id,{...l,children:[]})),i.forEach(l=>{l.parentId&&i.has(l.parentId)?i.get(l.parentId).children.push(l):d.push(l)})),d},r=(n,i="")=>{const d=n.id===t?"selected":"";a+=`<option value="${n.id}" ${d}>${i}${n.name}</option>`,n.children.forEach(l=>r(l,i+"— "))};return s(e).forEach(n=>r(n)),a}async function Pe(e,t){const a=t.target.querySelector('button[type="submit"]');a&&(a.disabled=!0,a.textContent="A Salvar...");try{const s=V||await et(p.establishmentId),r=[],{ownerName:o,...n}=e;if(o&&o!==p.userName){const d=J.currentUser;d&&r.push(Jo(d,{displayName:o}).then(()=>{p.userName=o}))}const i={...s,...n};if(r.push(vs(p.establishmentId,i)),await Promise.all(r),V=i,g("Sucesso","Definições salvas com sucesso! A página será recarregada para aplicar o novo tema.","success"),n.themeColor)setTimeout(()=>window.location.reload(),1500);else{const d=document.getElementById("panelEstablishmentName");n.name&&d&&(d.textContent=n.name,p.establishmentName=n.name)}}catch(s){g("Erro",`Não foi possível salvar: ${s.message}`,"error")}finally{a&&(a.disabled=!1,a.textContent="Salvar")}}function fi(e,t){t.innerHTML=`
    ...
    `,t.querySelector("#personal-data-form").addEventListener("submit",a=>{a.preventDefault();const s={ownerName:t.querySelector("#ownerName").value,name:t.querySelector("#establishmentName").value,phone:t.querySelector("#establishmentPhone").value,document:t.querySelector("#establishmentCnpjCpf").value,email:t.querySelector("#establishmentEmail").value,address:t.querySelector("#establishmentAddress").value,website:t.querySelector("#establishmentWebsite").value};Pe(s,a)})}function vi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const s=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!s||!r){g("Erro","Preencha todos os campos.","error");return}const o=t.querySelector('button[form="change-email-form"]');o.disabled=!0,o.textContent="A verificar...";try{const n=J.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Oo.credential(n.email,r);await zo(n,i),o.textContent="A enviar link...",await Vo(n,s),o.textContent="A atualizar BD...",await ys(p.establishmentId,s),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{o.disabled=!1,o.textContent="Salvar Novo E-mail"}})}function yi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;const a=t.querySelector("#establishmentTimezone");if(e.timezone)a.value=e.timezone;else try{const o=Intl.DateTimeFormat().resolvedOptions().timeZone;Array.from(a.options).some(i=>i.value===o)?a.value=o:a.value="America/Sao_Paulo"}catch{a.value="America/Sao_Paulo"}const s=t.querySelector("#establishmentWorkingHoursContainer"),r=e.workingHours||{};Object.keys(Ht).forEach(o=>{const n=r[o]||{},i=Ht[o],d=n.active!==!1,l=document.createElement("div");l.className=`day-schedule-card p-4 rounded-lg ${d?"bg-gray-50":"bg-gray-100 disabled"}`,l.innerHTML=`
            ...
            </div>`,s.appendChild(l)}),s.addEventListener("change",o=>{const n=o.target.closest('.day-schedule-card input[type="checkbox"]');n&&n.closest(".day-schedule-card").classList.toggle("disabled",!n.checked)}),t.querySelector("#working-hours-form").addEventListener("submit",o=>{o.preventDefault();const n={};Object.keys(Ht).forEach(d=>{n[d]={active:t.querySelector(`#est-${d}-active`).checked,start:t.querySelector(`#est-${d}-start`).value,end:t.querySelector(`#est-${d}-end`).value,breakStart:t.querySelector(`#est-${d}-breakStart`).value,breakEnd:t.querySelector(`#est-${d}-breakEnd`).value}});const i=t.querySelector("#establishmentTimezone").value;Pe({workingHours:n,timezone:i},o)})}function ki(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
             <div class="flex justify-between items-center mb-6">
    ...
    `;try{const[a,s]=await Promise.all([Qe(),Ze()]),r=e.financialIntegration||{};t.querySelector("#financialNatureId").innerHTML=_a(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=_a(s,r.defaultCentroDeCustoId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const s={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null}};Pe(s,a)})}function Ei(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <h3 class="text-xl font-bold text-gray-800">${e}</h3>
            <p class="mt-4 text-gray-500">Esta secção ainda não foi implementada.</p>
        </div>
    `}function qo(e="indigo",t){const a=t.querySelector("#color-palette-container"),s=t.querySelector("#establishmentThemeColor");!a||!s||(a.innerHTML="",Object.entries(bi).forEach(([r,o])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{s.value=r,qo(r,t)}),a.appendChild(i)}),s.value=e)}function $i(e,t){const a=t.querySelector("#slotIntervalContainer"),s=t.querySelector("#establishmentSlotInterval");if(!a||!s)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(o=>{const n=o.value===e;return`<button type="button" data-value="${o.value}" 
    ...
    `,be.querySelector('button[data-action="back-to-list"]').addEventListener("click",s=>{s.preventDefault(),No()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":fi(V,a);break;case"change-password":vi(V,a);break;case"change-email":hi(V,a);break;case"branding":yi(V,a);break;case"booking":xi(V,a);break;case"working-hours":wi(V,a);break;case"loyalty":ki(V,a);break;case"financial":await Si(V,a);break;default:Ei(t?t.label:"Definições",a)}}async function No(){if(be.innerHTML=`
    ...
    `,be.querySelector("#settings-menu-list").addEventListener("click",s=>{const r=s.target.closest("button[data-section]");r&&(s.preventDefault(),Ii(r.dataset.section))});const a=be.querySelector('[data-action="go-to-my-profile"]');a&&a.addEventListener("click",s=>{s.preventDefault(),j("my-profile-section")})}const Ve=document.getElementById("content");async function Ie(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,s=document.getElementById("filterEndDate")?.value,r=await Lt(p.establishmentId,a||new Date().toISOString().split("T")[0],s||new Date().toISOString().split("T")[0],e),o=document.getElementById("filterReason")?.value.toLowerCase(),n=o?r.filter(d=>d.reason&&d.reason.toLowerCase().includes(o)):r,i=n.reduce((d,l)=>{const u=l.reason||"Sem motivo";return d[u]||(d[u]=[]),d[u].push(l),d},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([d,l])=>{const u=document.createElement("div");u.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let c=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                    ...
                    </div>`;u.innerHTML+=y}),t.appendChild(u)})}catch(a){t.innerHTML=`<p class="text-center text-red-500">Erro: ${a.message}</p>`}}}async function Ci(e){e.preventDefault();const t=e.target,a=t.querySelector("#blockageProfId").value,s=t.querySelector("#blockageDate").value,r=t.querySelector("#blockageEndDate").value||s,o=t.querySelector("#blockageStartTime").value,n=t.querySelector("#blockageEndTime").value,i={establishmentId:p.establishmentId,professionalId:a,startTime:new Date(`${s}T${o}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:t.querySelector("#blockageReason").value};try{await Bt(i),t.reset(),g("Sucesso","Bloqueio adicionado com sucesso!","success"),Ie(a)}catch(d){g("Erro",`Não foi possível criar o bloqueio: ${d.message}`,"error")}}async function Li(e){e.preventDefault();const t=e.target,a=Array.from(t.querySelectorAll('input[name="batch-professionals"]:checked')).map(u=>u.value);if(a.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const s=t.querySelector("#batchBlockageDate").value,r=t.querySelector("#batchBlockageEndDate").value||s,o=t.querySelector("#batchBlockageStartTime").value,n=t.querySelector("#batchBlockageEndTime").value,i=t.querySelector("#batchBlockageReason").value,d=t.querySelector('button[type="submit"]');d.disabled=!0,d.textContent="Aguarde...";const l=a.map(u=>{const c={establishmentId:p.establishmentId,professionalId:u,startTime:new Date(`${s}T${o}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:i};return Bt(c)});try{await Promise.all(l),g("Sucesso",`${a.length} bloqueios foram criados com sucesso!`,"success"),t.reset(),t.querySelectorAll('input[name="batch-professionals"]:checked').forEach(c=>c.checked=!1);const u=document.getElementById("blockageProfId").value;u&&Ie(u)}catch(u){g("Erro",`Ocorreu um erro: ${u.message}`,"error")}finally{d.disabled=!1,d.textContent="Adicionar Bloqueio em Lote"}}function Bi(e){Ve.addEventListener("submit",t=>{t.target.id==="blockageForm"&&Ci(t),t.target.id==="batchBlockageForm"&&Li(t)}),Ve.addEventListener("input",t=>{t.target.matches("#filterStartDate, #filterEndDate, #filterReason")&&Ie(e)}),Ve.addEventListener("click",async t=>{const a=t.target.closest("button[data-action]");if(!a)return;const s=a.dataset.action;if(s==="back-to-professionals")j("profissionais-section");else if(s==="delete-blockage"){if(await M("Apagar Bloqueio","Tem a certeza que deseja apagar este bloqueio?"))try{await ma(a.dataset.id),g("Sucesso","Bloqueio removido.","success"),Ie(e)}catch(o){g("Erro",`Não foi possível remover o bloqueio: ${o.message}`,"error")}}else if(s==="batch-delete-blockage"){const r=JSON.parse(a.dataset.ids);if(await M("Apagar Lote de Bloqueios",`Tem certeza que deseja apagar ${r.length} bloqueios de uma vez?`))try{await bo(r),g("Sucesso",`${r.length} bloqueios removidos.`,"success"),Ie(e)}catch(n){g("Erro",`Não foi possível apagar os bloqueios: ${n.message}`,"error")}}})}async function Ti(e){const{professionalId:t,professionalName:a}=e;if(!t||!a){Ve.innerHTML='<p class="text-red-500 p-8">Erro: ID do profissional não fornecido.</p>';return}Ve.innerHTML=`
        <section>
        ...
        </section>`,Bi(t),await Ie(t);const s=document.getElementById("batchProfSelectionContainer");try{const r=await G(p.establishmentId);s.innerHTML=r.map(o=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{s.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const Mi=e=>w(`/api/users/${e}`),Di=e=>w("/api/users",{method:"POST",body:JSON.stringify(e)}),Pi=(e,t)=>w(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ai=e=>w(`/api/users/${e}`,{method:"DELETE"}),qi=(e,t)=>w(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),Ni=(e,t)=>w(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),me=document.getElementById("content"),Fi={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},Ri={view:"Visualizar",create:"Criar",edit:"Editar"};let qe=null,Ne=null;function ji(e){const t=document.getElementById("usersListContainer");if(!t)return;const a=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const s=a?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${s}</p>`;return}e.sort((s,r)=>(s.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(s=>{const r=JSON.stringify(s).replace(/'/g,"&apos;"),o=s.status==="active",n=p.professionals.find(u=>u.id===s.professionalId),i=n?n.name:"N/A",d=n?n.name.charAt(0):s.name.charAt(0),l=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(d)}`;return`
    ...
    `}).join("")}function wt(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=p.users:t=p.users.filter(a=>a.status==="active"),ji(t)}function Hi(e={}){return Object.entries(Fi).map(([t,a])=>{const s=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,o=Object.entries(Ri).map(([i,d])=>`
    ...
    `,qe&&me.removeEventListener("click",qe),Ne&&me.removeEventListener("change",Ne),qe=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("click",qe);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":Wa();break;case"edit-user":const s=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));Wa(s);break;case"back-to-list":$t();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await M("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await Ai(r),g("Usuário excluído com sucesso!","success"),$t()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Ne=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("change",Ne);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")wt();else if(t){e.stopPropagation();const a=t.dataset.userId,s=t.checked?"active":"inactive";try{await Ni(a,s),g(`Usuário ${s==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=p.users.findIndex(o=>o.id===a);r>-1&&(p.users[r].status=s,wt())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,wt()}}},me.addEventListener("click",qe),me.addEventListener("change",Ne),await Oi()}const zi=document.getElementById("content");let Ga={},na=null;function Vi(){Object.values(Ga).forEach(e=>e?.destroy()),Ga={}}function Ui(e,t){const{jsPDF:a}=window.jspdf,s=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(s.setFontSize(18),s.text(e,s.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];s.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const o=s.lastAutoTable?s.lastAutoTable.finalY+20:60;s.text("Detalhes das Vendas",20,o),s.autoTable({html:`#${t}`,startY:o+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),s.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function Ya(e){const t=document.getElementById("genericModal"),a=(e.payments||[]).map(s=>`
    ...
    `,t.style.display="flex"}function Ji(e){const{summary:t,transactions:a}=e;document.getElementById("summary-revenue").textContent=`R$ ${t.totalRevenue.toFixed(2)}`,document.getElementById("summary-transactions").textContent=t.totalTransactions,document.getElementById("summary-avg-ticket").textContent=`R$ ${t.averageTicket.toFixed(2)}`;const s=document.getElementById("paymentSummaryTableBody"),r=Object.entries(t.paymentMethodTotals).sort(([,i],[,d])=>d-i);s.innerHTML=r.map(([i,d])=>`
        <tr class="border-b">
    ...
    `).join(""),o.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const d=i.dataset.transactionIndex,l=na.transactions[d];l&&Ya(l)})}),n.innerHTML=a.map((i,d)=>`
    ...
    `).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const d=i.dataset.transactionIndex,l=na.transactions[d];l&&Ya(l)})})}async function Xa(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const s=t.value,r=a.value;if(!s||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const o=document.getElementById("cashierSessionFilter").value,n=await As({establishmentId:p.establishmentId,startDate:s,endDate:r,cashierSessionId:o});na=n,e.innerHTML=`
        ...
        `,Ji(n)}catch(o){g("Erro",`Não foi possível carregar o relatório: ${o.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${o.message}</p>`}}async function _i(){Vi();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];zi.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Xa),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const s=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,o=`Relatorio_Vendas_${s}_a_${r}`;Ui(o,"transactionsTable")});try{const s=await $r(),r=document.getElementById("cashierSessionFilter");s.forEach(o=>{const n=new Date(o.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=o.closedByName||"N/A";r.innerHTML+=`<option value="${o.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await Xa()}const Wi=document.getElementById("content");let I={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},Ot=null,ct=null,ut=null;function ka(e){const t=new Map,a=[];return e&&(e.forEach(s=>t.set(s.id,{...s,children:[]})),t.forEach(s=>{s.parentId&&t.has(s.parentId)?t.get(s.parentId).children.push(s):a.push(s)})),a}function Fo(e,t,a){if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const s=(r,o=0)=>{const n="— ".repeat(o);return`
    ...
    `,e.style.display="flex"}function Xi(){const e=document.getElementById("genericModal");e.innerHTML=`
        <div class="modal-content max-w-lg">
            <div class="flex justify-between items-center mb-6">
    ...
    `,e.style.display="flex"}function mt(e,t="all"){let a='<option value="all">Todos</option>';const s=n=>{const i=new Map,d=[];return n&&(n.forEach(l=>i.set(l.id,{...l,children:[]})),i.forEach(l=>{l.parentId&&i.has(l.parentId)?i.get(l.parentId).children.push(l):d.push(l)})),d},r=(n,i=0)=>{const d=i>0?"— ".repeat(i):"",l=n.id===t?"selected":"";a+=`<option value="${n.id}" ${l}>${d}${n.name}</option>`,n.children.forEach(u=>r(u,i+1))};return s(e).forEach(n=>r(n)),a}async function ve(){const e=document.getElementById("financial-content"),t=document.getElementById("filterStartDate")?.value,a=document.getElementById("filterEndDate")?.value,s=document.getElementById("filterNaturezaId")?.value,r=document.getElementById("filterCostCenterId")?.value;if(!t||!a){try{const[i,d]=await Promise.all([Qe(),Ze()]);I={...I,natures:i,costCenters:d},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(I.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(I.costCenters))}catch(i){g("Erro",`Não foi possível carregar os dados base: ${i.message}`,"error")}ia(),to();return}const o=document.getElementById("payables-list"),n=document.getElementById("receivables-list");o&&(o.innerHTML='<div class="loader mx-auto"></div>'),n&&(n.innerHTML='<div class="loader mx-auto"></div>');try{const i={startDate:t,endDate:a};s&&s!=="all"&&(i.natureId=s),r&&r!=="all"&&(i.costCenterId=r);const[d,l,u,c]=await Promise.all([si(i),di(i),Qe(),Ze()]),m=l.previousBalance-d.previousBalance;I={...I,payables:d.entries,receivables:l.entries,natures:u,costCenters:c,previousBalance:m,filterNaturezaId:s,filterCostCenterId:r},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(I.natures,I.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(I.costCenters,I.filterCostCenterId)),ia(),to()}catch(i){g("Erro",`Não foi possível carregar os dados: ${i.message}`,"error"),e&&(e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function Qi(e,t,a=null){e.preventDefault();const s=e.target,r=s.querySelector('[name="status"]').checked,o=s.querySelector('[name="paymentDate"]').value,n=parseFloat(s.querySelector('[name="amount"]').value),i=parseInt(s.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!o){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const d={description:s.querySelector('[name="description"]').value,amount:n,dueDate:s.querySelector('[name="dueDate"]').value,naturezaId:s.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:s.querySelector('[name="centroDeCustoId"]').value||null,notes:s.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?o:null,installments:a?1:i};try{a?(await(t==="payable"?ri(a,d):ci(a,d)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?oi(d):li(d)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await ve()}catch(l){g("Erro",`Não foi possível salvar: ${l.message}`,"error")}}async function Zi(e,t){if(await M("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?ni(t):ui(t)),g("Sucesso","Lançamento excluído!","success"),await ve()}catch(s){g("Erro",`Falha ao excluir: ${s.message}`,"error")}}async function Ki(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?ii(t,a):mi(t,a)),g("Sucesso","Lançamento atualizado!","success"),await ve()}catch(s){g("Erro",`Falha ao atualizar status: ${s.message}`,"error")}}function eo(e){const t=I.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function ia(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(I.natures.map(i=>[i.id,i.name])),s=new Map(I.costCenters.map(i=>[i.id,i.name])),r=eo(I.payables),o=eo(I.receivables),n=(i,d)=>{const l=i.status==="paid",u=JSON.stringify(i).replace(/'/g,"&apos;"),c=i.naturezaId?a.get(i.naturezaId):"N/A",m=i.centroDeCustoId?s.get(i.centroDeCustoId):"N/A";let b=d==="payable"?"text-red-600":"text-green-600";const f=l?"bg-gray-200 text-gray-600":d==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",v=l?"Finalizado":"Pendente";return l&&(b="text-gray-500"),`
    ...
    `,{modalElement:o}=N({title:"✨ Novo Cálculo de Comissões",contentHTML:r,maxWidth:"max-w-xl"}),n=o.querySelector("#calculation-form"),i=o.querySelector("#calc-professionals-all"),d=o.querySelectorAll(".professional-checkbox");i.addEventListener("change",l=>{d.forEach(u=>{u.checked=l.target.checked})}),d.forEach(l=>{l.addEventListener("change",()=>{l.checked?Array.from(d).every(c=>c.checked)&&(i.checked=!0):i.checked=!1})}),n.addEventListener("submit",l=>{l.preventDefault();const u=Array.from(d).filter(m=>m.checked).map(m=>m.value);if(u.length===0){g("Atenção","Selecione pelo menos um profissional para o cálculo.","error");return}ll({professionalIds:u});const c=o.querySelector("[data-close-modal]");c?c.click():o.style.display="none"})}async function il(e){if(await M("Excluir Relatório","Tem a certeza que deseja excluir permanentemente este relatório de comissão? Esta ação não pode ser desfeita."))try{await sl(e),g("Sucesso!","Relatório de comissão excluído.","success"),kt()}catch(a){g("Erro",`Não foi possível excluir: ${a.message}`,"error")}}async function ll(e){const{professionalIds:t}=e,a=document.getElementById("calc-start-date")?.value,s=document.getElementById("calc-end-date")?.value,r={services:document.getElementById("calc-type-services")?.checked,products:document.getElementById("calc-type-products")?.checked,packages:document.getElementById("calc-type-packages")?.checked};if(!a||!s){g("Erro","As datas não foram capturadas corretamente.","error");return}j("commissions-section",{view:"results",isLoading:!0});try{const o=await tl({professionalIds:t,startDate:a,endDate:s,calculationTypes:r});Ue=o;const n=`${new Date(a+"T00:00:00").toLocaleDateString("pt-BR")} a ${new Date(s+"T00:00:00").toLocaleDateString("pt-BR")}`;Ro=n,ul(o,n)}catch(o){g("Erro",`Não foi possível calcular: ${o.message}`,"error"),j("commissions-section",{view:"history"})}}async function dl(){if(Ue.length===0){g("Erro","Não há resultados para salvar.","error");return}const e=Ro;if(await M("Salvar Relatórios",`Tem a certeza que deseja salvar ${Ue.length} relatório(s) de comissão para o período de ${e}?`))try{const a=Ue.map(s=>al({professionalId:s.professionalId,professionalName:s.professionalName,period:e,reportData:s}));await Promise.all(a),g("Sucesso!","Relatórios de comissão salvos.","success"),j("commissions-section",{view:"history"})}catch(a){g("Erro",`Não foi possível salvar: ${a.message}`,"error")}}async function kt(){const e=document.getElementById("commissionHistory");if(!e)return;const t=document.getElementById("filter-professional")?.value||"",a=document.getElementById("filter-month")?.value||"";Ee={},t&&t!=="all"&&(Ee.professionalId=t),a&&(Ee.period=a),e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const s=await ol(Ee);if(s.length===0){e.innerHTML='<p class="text-center text-gray-500 py-8">Nenhum relatório de comissão salvo encontrado para os filtros.</p>';return}e.innerHTML=`
            <div class="space-y-3">
                ${s.map(r=>`
    ...
    `,document.getElementById("filter-professional").value=s,document.getElementById("filter-month").value=r,document.getElementById("filter-professional").addEventListener("change",kt),document.getElementById("filter-month").addEventListener("change",kt),kt()}function ul(e,t){Ue=e;const a=e.reduce((s,r)=>s+r.summary.totalCommission,0);Ce.innerHTML=`
        <section>
    ...
    `}async function ml(e={}){const{view:t="history",isLoading:a=!1}=e;if(pt&&Ce.removeEventListener("click",pt),pt=s=>{const r=s.target.closest("button[data-action]");if(!r)return;switch(r.dataset.action){case"open-calculator":nl();break;case"back-to-history":j("commissions-section",{view:"history"});break;case"save-reports":dl();break;case"generate-receipt":const n=JSON.parse(r.dataset.report.replace(/&apos;/g,"'"));rl(n);break;case"delete-report":const i=r.dataset.id;il(i);break}},Ce.addEventListener("click",pt),a){Ce.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A calcular comissões...</p></div>';return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await G(p.establishmentId)}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}(t==="history"||t==="main")&&cl()}const Vt=document.getElementById("content");let he={allPackages:[],catalogForModal:{services:[],products:[]}},gt=null,$e=null;function pl(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function gl(){const e=document.getElementById("packagesListContainer");if(e){if(he.allPackages.length===0){e.innerHTML=`
        ...
        `}).join("")}}function ao(){const e=document.getElementById("genericModal");e.style.display="none",$e&&e.removeEventListener("click",$e)}async function oo(e=null){const t=document.getElementById("genericModal"),a=!!e,s=e?JSON.parse(JSON.stringify(e.items||[])):[],r=`
    ...
    `;t.innerHTML=r,t.style.display="flex";const o=t.querySelector("#package-items-list"),n=(d,l)=>{const u=l.querySelector("#originalPrice"),c=d.reduce((m,b)=>m+b.price*b.quantity,0);u&&(u.textContent=`R$ ${c.toFixed(2)}`)},i=d=>{d.length===0?o.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':o.innerHTML=d.map((l,u)=>{const c=l.type==="service",m=c?"Serviço":"Produto",b=c?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
            ...
            `}).join(""),n(d,t)};i(s),o.addEventListener("change",d=>{if(d.target.classList.contains("quantity-input")){const l=parseInt(d.target.dataset.index,10),u=parseInt(d.target.value,10);u>0&&s[l]&&(s[l].quantity=u,i(s))}}),o.addEventListener("click",d=>{if(d.target.classList.contains("remove-item-btn")){const l=parseInt(d.target.dataset.index,10);s.splice(l,1),i(s)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>bl(d=>{const l=s.find(u=>u.id===d.id&&u.type===d.type);l?l.quantity++:s.push({...d,quantity:1}),i(s)}),$e&&t.removeEventListener("click",$e),$e=async d=>{const l=d.target.closest("button[data-action]");if(!l)return;const u=l.dataset.action;if(d.stopPropagation(),u==="close-modal"&&ao(),u==="save-package"){const c=l,m={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:s,originalPrice:s.reduce((b,f)=>b+f.price*f.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null};if(!m.name||!m.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(m.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}c.disabled=!0,c.textContent="A salvar...";try{a?await Lr(m.id,m):(delete m.id,await Cr(m)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),ao(),await Sa()}catch(b){g("Erro",`Não foi possível salvar o pacote: ${b.message}`,"error"),c.disabled=!1,c.textContent="Salvar Pacote"}}},t.addEventListener("click",$e)}function bl(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const s={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=d=>{const l=t.toLowerCase(),u=he.catalogForModal.services.filter(f=>f.name.toLowerCase().includes(l)),c=he.catalogForModal.products.filter(f=>f.name.toLowerCase().includes(l)),m=u.map(f=>`
    ...
    `,document.body.appendChild(a);const o=a.querySelector("#item-selection-list"),n=a.querySelector("#item-search-input"),i=()=>{a.remove()};r(o),n.addEventListener("input",()=>{t=n.value,r(o)}),a.addEventListener("click",d=>{const l=d.target.closest('[data-action="select-item"]'),u=d.target.closest('[data-action="close-selection-modal"]');if(l){const{itemType:c,itemId:m}=l.dataset,f=(he.catalogForModal[c+"s"]||[]).find(v=>v.id===m);f&&(e({...f,type:c}),i())}else(u||d.target===a)&&i()})}async function Sa(){Vt.innerHTML=`
    ...
    `,gt&&Vt.removeEventListener("click",gt),gt=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const s=e.target.closest('[data-action="delete-package"]');if(s){const r=s.dataset.id;M("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async o=>{if(o)try{await Br(r),g("Sucesso!","Pacote excluído.","success"),await Sa()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")oo(null);else if(a==="edit-package"){const s=JSON.parse(t.dataset.package);oo(s)}},Vt.addEventListener("click",gt);try{const[e,t,a]=await Promise.all([wo(p.establishmentId),De(p.establishmentId),Mt(p.establishmentId)]);he.allPackages=e,he.catalogForModal={services:t.filter(s=>s.active),products:a},gl()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const fl=document.getElementById("content");let vl=null;async function hl(){fl.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await yl()}async function yl(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=p.userProfessionalId;if(t){const a=await xs(t);vl=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo),e.innerHTML=`
            ...
            `,xl(a.id),document.getElementById("my-blocks-filter").addEventListener("change",r=>It(a.id,r.target.value)),It(a.id,"future")}else e.innerHTML=`
        ...
        `}}function xl(e){const t=document.getElementById("block-schedule-form");t.addEventListener("submit",async a=>{a.preventDefault();const s=t.querySelector("#blockDate").value,r=t.querySelector("#blockStartTime").value,o=t.querySelector("#blockEndTime").value,n=t.querySelector("#blockReason").value;if(!s||!r||!o){g("Erro","Por favor, preencha a data e os horários de início e fim.","error");return}if(r>=o){g("Erro","A hora de início deve ser anterior à hora de fim.","error");return}const i=new Date(`${s}T${r}:00`),d=new Date(`${s}T${o}:00`),l=t.querySelector('button[type="submit"]');l.disabled=!0,l.textContent="A bloquear...";try{await Bt({establishmentId:p.establishmentId,professionalId:e,reason:n||"Bloqueado (Meu Perfil)",startTime:i.toISOString(),endTime:d.toISOString()}),g("Sucesso","Agenda bloqueada com sucesso!","success"),t.reset();const u=document.getElementById("my-blocks-filter").value;It(e,u)}catch(u){console.error("Erro ao bloquear agenda:",u),g("Erro",`Não foi possível bloquear a agenda: ${u.message}`,"error")}finally{l.disabled=!1,l.textContent="Bloquear Agenda"}})}async function It(e,t="future"){const a=document.getElementById("my-blocks-list");a.innerHTML='<p class="text-gray-500">A carregar bloqueios...</p>';try{const s=new Date;let r,o;t==="history"?(o=new Date,r=new Date,r.setFullYear(r.getFullYear()-1)):(r=new Date,o=new Date,o.setFullYear(o.getFullYear()+1));let i=(await Lt(p.establishmentId,r.toISOString(),o.toISOString(),e)).map(d=>({...d,startTime:new Date(d.startTime),endTime:new Date(d.endTime)}));t==="history"?i=i.filter(d=>d.endTime<s).sort((d,l)=>l.startTime-d.startTime):i=i.filter(d=>d.endTime>=s).sort((d,l)=>d.startTime-l.startTime),i.length>0?(a.innerHTML=i.map(d=>{const l=d.startTime.toLocaleDateString("pt-BR"),u=`${d.startTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${d.endTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`;return`
                ...
                `}).join(""),a.querySelectorAll(".remove-block-btn").forEach(d=>{d.addEventListener("click",async l=>{const u=l.currentTarget.dataset.blockId;if(confirm("Tem certeza que deseja remover este bloqueio?"))try{await ma(u),g("Sucesso","Bloqueio removido.","success"),It(e,t)}catch(c){console.error("Erro ao remover bloqueio:",c),g("Erro",`Não foi possível remover o bloqueio: ${c.message}`,"error")}})})):a.innerHTML=`<p class="text-gray-500 py-4 text-center">Nenhum bloqueio ${t==="history"?"no histórico recente":"futuro agendado"}.</p>`}catch(s){console.error("Erro ao carregar bloqueios:",s),a.innerHTML=`<p class="text-red-500">Erro ao carregar bloqueios: ${s.message}</p>`}}const bt=document.getElementById("loadingScreen"),Ut=document.getElementById("dashboardContent"),Jt=document.getElementById("content"),so=document.getElementById("notificationBell"),_t=document.getElementById("notificationBadge"),ft=document.getElementById("notificationPanel"),ro=document.getElementById("notificationList"),Wt=document.getElementById("profileMenuButton"),Y=document.getElementById("profileDropdown"),wl=document.getElementById("profileName"),kl=document.getElementById("profileEmail"),Sl=document.getElementById("logoutButton"),El=document.getElementById("cancellationHistoryBtn"),no=document.getElementById("myProfileLink"),io={indigo:{main:"#4f46e5",hover:"#4338ca",light:"#e0e7ff",text:"#ffffff"},blue:{main:"#2563eb",hover:"#1d4ed8",light:"#dbeafe",text:"#ffffff"},sky:{main:"#0284c7",hover:"#0369a1",light:"#e0f2fe",text:"#ffffff"},teal:{main:"#0d9488",hover:"#0f766e",light:"#ccfbf1",text:"#ffffff"},emerald:{main:"#059669",hover:"#047857",light:"#d1fae5",text:"#ffffff"},green:{main:"#16a34a",hover:"#15803d",light:"#dcfce7",text:"#ffffff"},lime:{main:"#65a30d",hover:"#4d7c0f",light:"#ecfccb",text:"#ffffff"},amber:{main:"#d97706",hover:"#b45309",light:"#fef3c7",text:"#1f2937"},orange:{main:"#ea580c",hover:"#c2410c",light:"#ffedd5",text:"#ffffff"},red:{main:"#dc2626",hover:"#b91c1c",light:"#fee2e2",text:"#ffffff"},rose:{main:"#e11d48",hover:"#be123c",light:"#ffe4e6",text:"#ffffff"},pink:{main:"#db2777",hover:"#be185d",light:"#fce7f3",text:"#ffffff"},fuchsia:{main:"#c026d3",hover:"#a21caf",light:"#fae8ff",text:"#ffffff"},purple:{main:"#7c3aed",hover:"#6d28d9",light:"#ede9fe",text:"#ffffff"},violet:{main:"#8b5cf6",hover:"#7c3aed",light:"#ddd6fe",text:"#ffffff"},gray:{main:"#4b5563",hover:"#374151",light:"#f3f4f6",text:"#ffffff"},black:{main:"#111827",hover:"#000000",light:"#e5e7eb",text:"#ffffff"}};let Je=null,_e=[];const $l={"agenda-section":yo,"comandas-section":ba,"relatorios-section":Kt,"servicos-section":nn,"produtos-section":yn,"suppliers-section":Ln,"profissionais-section":Et,"clientes-section":qt,"estabelecimento-section":No,"ausencias-section":Ti,"users-section":$t,"sales-report-section":_i,"financial-section":el,"commissions-section":ml,"packages-section":Sa,"my-profile-section":hl};function Il(e){const t=io[e]||io.indigo,s=(o=>{const n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(o);return n?`${parseInt(n[1],16)}, ${parseInt(n[2],16)}, ${parseInt(n[3],16)}`:"79, 70, 229"})(t.main);document.body.style.setProperty("--theme-main",t.main);const r=document.getElementById("dynamic-theme-styles");r.innerHTML=`
        :root {
    ...
    `}function la(){const e=_e.filter(t=>!t.read).length;if(e>0?(_t.textContent=e,_t.classList.remove("hidden")):_t.classList.add("hidden"),_e.length===0){ro.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}ro.innerHTML=_e.map(t=>`
    ...
    `).join("")}function Cl(e){Je&&Je();const t=Ke(de,"establishments",e,"notifications"),a=uo(t,Qo("timestamp",">=",new Date),mo("timestamp","desc"));Je=Zo(a,s=>{s.docChanges().forEach(r=>{if(r.type==="added"){const o=r.doc.data();_e.unshift({title:o.title,message:o.message,time:o.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(o.title,o.message,"info",!0),la();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(o.type==="cancellation"||o.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),yo())}})},s=>{console.error("Erro no listener de notificações em tempo real:",s),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function j(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=p.enabledModules?.[a]!==!1,o=p.userPermissions===null||p.userPermissions[e]?.view===!0;if(!r||!o){Jt.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const s=$l[e];s?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),Jt.innerHTML="",s(t)):(Jt.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function Ll(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),s=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),o=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(o&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!o&&!n))try{const i=await Rs();o&&s&&(s.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),o&&s&&(s.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function Bl(e){try{We.getPlatform()==="android"&&(await oe.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await oe.checkPermissions();if(t.receive==="prompt"&&(t=await oe.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await oe.register(),oe.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const s=da(de,"users",e);await Xo(s,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(s){console.error("Erro ao salvar token FCM:",s)}}),oe.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),oe.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),oe.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),j("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function Tl(){We.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),us(),so.addEventListener("click",e=>{e.stopPropagation(),ft.classList.toggle("hidden"),ft.classList.contains("hidden")||(_e.forEach(t=>t.read=!0),la())}),El.addEventListener("click",()=>{ms()}),Wt.addEventListener("click",e=>{e.stopPropagation(),Y.classList.toggle("active"),Y.classList.contains("active")?Y.classList.remove("hidden"):setTimeout(()=>Y.classList.add("hidden"),200)}),no&&no.addEventListener("click",e=>{e.preventDefault(),j("my-profile-section"),Y.classList.remove("active"),Y.classList.add("hidden")}),document.addEventListener("click",e=>{!ft.contains(e.target)&&e.target!==so&&ft.classList.add("hidden"),!Y.contains(e.target)&&e.target!==Wt&&Y.classList.contains("active")&&(Y.classList.remove("active"),setTimeout(()=>Y.classList.add("hidden"),200))}),_o(J,async e=>{if(e)try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const s=await et(a.establishmentId);p.enabledModules=s.modules,Il(s.themeColor||"indigo");let r=null,o=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const l=da(de,"users",e.uid),u=await Yo(l);if(u.exists()){const c=u.data();r=a.role==="employee"?c.permissions||{}:null,o=c.name||o,n=c.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}p.userProfessionalId=n,Bl(e.uid);const i=o||e.email;es(a.establishmentId,s.name,r),Wt.textContent=i.charAt(0).toUpperCase(),wl.textContent=i,kl.textContent=e.email;const d=()=>{Je&&Je(),Ea(J).then(()=>window.location.href="/login.html")};Sl.addEventListener("click",l=>{l.preventDefault(),d()}),fs(j,r,p.enabledModules),Ll(r),Cl(a.establishmentId),la(),bt.classList.add("fade-out"),Ut.style.display="flex",setTimeout(()=>{bt.style.display="none"},500),j("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),bt.classList.add("fade-out"),setTimeout(()=>{bt.style.display="none"},500),Ut.innerHTML=`
                ...
                `,Ut.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{Ea(J).then(()=>window.location.href="/login.html")})}else window.location.href="/login.html"})}Tl();
