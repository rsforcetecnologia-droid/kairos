...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${s}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${s}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${Kt}. Verifique sua conexão com a internet.`):n}}const ns=(e,t,a,o=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(r+=`&professionalId=${o}`),y(r)},is=(e,t,a)=>{const o=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return y(o)},ls=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return y(r)},ds=e=>y("/api/appointments",{method:"POST",body:JSON.stringify(e)}),ga=(e,t)=>y(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),cs=e=>y(`/api/appointments/${e}`,{method:"DELETE"}),us=e=>y(`/api/appointments/${e}/reopen`,{method:"POST"}),ms=(e,t)=>y(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let R;async function ps(){if(!R)try{R=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function gs(){if(!R){console.warn("AudioContext não inicializado. O som não será tocado.");return}R.state==="suspended"&&R.resume();const e=R.createOscillator(),t=R.createGain();e.connect(t),t.connect(R.destination),e.type="sine",e.frequency.setValueAtTime(800,R.currentTime),t.gain.setValueAtTime(0,R.currentTime),t.gain.linearRampToValueAtTime(.3,R.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,R.currentTime+.2),e.start(R.currentTime),e.stop(R.currentTime+.2)}function g(e,t,a="info",o=!1){const r=document.getElementById("toast-container");if(!r)return;o&&gs();const s=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};s.className=`toast ${n[a]||n.info}`,s.innerHTML=`
    ...
    `;const l=r.querySelector("[data-close-modal]");l&&(l.onclick=n);const d=r.querySelector('[data-action="close-modal"]');return d&&(d.onclick=n),r.addEventListener("click",u=>{u.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Tt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function bs(){document.body.addEventListener("click",()=>{R||ps()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const o=t.dataset.target;if(o){const r=document.getElementById(o);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const o=document.getElementById("genericModal");o&&(o.style.display="none")}})}async function Da(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const o=await is(p.establishmentId,t,a);if(o.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=o.map(r=>`
        ...
        `).join("")}catch(o){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${o.message}</p>`}}function fs(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const o=`
    ...
    `,{modalElement:r}=N({title:"Histórico de Cancelamentos",contentHTML:o,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Da),Da()}const z=document.getElementById("sidebar"),Ma=document.getElementById("sidebarToggle"),Re=document.getElementById("mainContent"),vs=document.querySelectorAll(".sidebar-link"),Pa=document.getElementById("hamburger-menu-btn"),Ie=document.getElementById("mobile-overlay");function ft(e){!z||!Re||(z.classList.toggle("collapsed",e),Re.classList.toggle("sidebar-collapsed-shift",e))}function hs(){!z||!Ie||(z.classList.add("mobile-open"),Ie.classList.add("visible"))}function ot(){!z||!Ie||(z.classList.remove("mobile-open"),Ie.classList.remove("visible"))}function xs(){ft(!z.classList.contains("collapsed"))}function ys(e,t,a){if(!z||!Re)return;Re.classList.add("main-content-shift"),window.innerWidth>=768?ft(z.classList.contains("collapsed")):(Re.classList.remove("main-content-shift","sidebar-collapsed-shift"),ot()),Ma&&Ma.addEventListener("click",r=>{r.stopPropagation(),xs()}),z.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&z.classList.contains("collapsed")&&ft(!1)}),z.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||ft(!0))}),Pa&&Pa.addEventListener("click",r=>{r.stopPropagation(),hs()}),Ie&&Ie.addEventListener("click",r=>{r.stopPropagation(),ot()});let o=0;z.addEventListener("touchstart",r=>{o=r.changedTouches[0].screenX},{passive:!0}),z.addEventListener("touchend",r=>{const s=r.changedTouches[0].screenX;o-s>50&&ot()},{passive:!0}),vs.forEach(r=>{const s=r.getAttribute("data-target"),n=s.replace("-section",""),i=a?.[n]!==!1,l=t===null||t[s]?.view===!0;if(!i||!l){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",d=>{d.preventDefault(),s&&typeof e=="function"&&e(s),window.innerWidth<768&&ot()})})}const Qe=e=>{const t=e||p.establishmentId;return t?y(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ws=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ks=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Ss=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},_=e=>y(`/api/professionals/${e}`),$s=e=>y(`/api/professionals/details/${e}`),Es=e=>y("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Cs=(e,t)=>y(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Is=e=>y(`/api/professionals/${e}`,{method:"DELETE"});var Le;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Le||(Le={}));class Ot extends Error{constructor(t,a,o){super(t),this.message=t,this.code=a,this.data=o}}const Ls=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},Ts=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},o=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:Ls(e),s=()=>r()!=="web",n=c=>{const m=d.get(c);return!!(m?.platforms.has(r())||i(c))},i=c=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===c)},l=c=>e.console.error(c),d=new Map,u=(c,m={})=>{const b=d.get(c);if(b)return console.warn(`Capacitor plugin "${c}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(c);let w;const h=async()=>(!w&&f in m?w=typeof m[f]=="function"?w=await m[f]():w=m[f]:t!==null&&!w&&"web"in m&&(w=typeof m.web=="function"?w=await m.web():w=m.web),w),k=(M,q)=>{var F,O;if(v){const se=v?.methods.find(W=>q===W.name);if(se)return se.rtype==="promise"?W=>a.nativePromise(c,q.toString(),W):(W,tt)=>a.nativeCallback(c,q.toString(),W,tt);if(M)return(F=M[q])===null||F===void 0?void 0:F.bind(M)}else{if(M)return(O=M[q])===null||O===void 0?void 0:O.bind(M);throw new Ot(`"${c}" plugin is not implemented on ${f}`,Le.Unimplemented)}},$=M=>{let q;const F=(...O)=>{const se=h().then(W=>{const tt=k(W,M);if(tt){const at=tt(...O);return q=at?.remove,at}else throw new Ot(`"${c}.${M}()" is not implemented on ${f}`,Le.Unimplemented)});return M==="addListener"&&(se.remove=async()=>q()),se};return F.toString=()=>`${M.toString()}() { [capacitor code] }`,Object.defineProperty(F,"name",{value:M,writable:!1,configurable:!1}),F},E=$("addListener"),I=$("removeListener"),D=(M,q)=>{const F=E({eventName:M},q),O=async()=>{const W=await F;I({eventName:M,callbackId:W},q)},se=new Promise(W=>F.then(()=>W({remove:O})));return se.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await O()},se},H=new Proxy({},{get(M,q){switch(q){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?D:E;case"removeListener":return I;default:return $(q)}}});return o[c]=H,d.set(c,{name:c,proxy:H,platforms:new Set([...Object.keys(m),...v?[f]:[]])}),H};return a.convertFileSrc||(a.convertFileSrc=c=>c),a.getPlatform=r,a.handleError=l,a.isNativePlatform=s,a.isPluginAvailable=n,a.registerPlugin=u,a.Exception=Ot,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Bs=e=>e.Capacitor=Ts(e),_e=Bs(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),ba=_e.registerPlugin;class xo{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let o=!1;this.listeners[t]||(this.listeners[t]=[],o=!0),this.listeners[t].push(a);const s=this.windowListeners[t];s&&!s.registered&&this.addWindowListener(s),o&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,o){const r=this.listeners[t];if(!r){if(o){let s=this.retainedEventArguments[t];s||(s=[]),s.push(a),this.retainedEventArguments[t]=s}return}r.forEach(s=>s(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:o=>{this.notifyListeners(a,o)}}}unimplemented(t="not implemented"){return new _e.Exception(t,Le.Unimplemented)}unavailable(t="not available"){return new _e.Exception(t,Le.Unavailable)}async removeListener(t,a){const o=this.listeners[t];if(!o)return;const r=o.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(o=>{this.notifyListeners(t,o)}))}}const Aa=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),qa=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class Ds extends xo{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(o=>{if(o.length<=0)return;let[r,s]=o.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=qa(r).trim(),s=qa(s).trim(),a[r]=s}),a}async setCookie(t){try{const a=Aa(t.key),o=Aa(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,s=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${o||""}${r}; path=${s}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}ba("CapacitorCookies",{web:()=>new Ds});const Ms=async e=>new Promise((t,a)=>{const o=new FileReader;o.onload=()=>{const r=o.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},o.onerror=r=>a(r),o.readAsDataURL(e)}),Ps=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,s,n)=>(r[s]=e[t[n]],r),{})},As=(e,t=!0)=>e?Object.entries(e).reduce((o,r)=>{const[s,n]=r;let i,l;return Array.isArray(n)?(l="",n.forEach(d=>{i=t?encodeURIComponent(d):d,l+=`${s}=${i}&`}),l.slice(0,-1)):(i=t?encodeURIComponent(n):n,l=`${s}=${i}`),`${o}&${l}`},"").substr(1):null,qs=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Ps(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const s=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))s.set(n,i);a.body=s.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const s=new FormData;if(e.data instanceof FormData)e.data.forEach((i,l)=>{s.append(l,i)});else for(const i of Object.keys(e.data))s.append(i,e.data[i]);a.body=s;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Ns extends xo{async request(t){const a=qs(t,t.webFetchExtra),o=As(t.params,t.shouldEncodeUrlParams),r=o?`${t.url}?${o}`:t.url,s=await fetch(r,a),n=s.headers.get("content-type")||"";let{responseType:i="text"}=s.ok?t:{};n.includes("application/json")&&(i="json");let l,d;switch(i){case"arraybuffer":case"blob":d=await s.blob(),l=await Ms(d);break;case"json":l=await s.json();break;case"document":case"text":default:l=await s.text()}const u={};return s.headers.forEach((c,m)=>{u[m]=c}),{data:l,headers:u,status:s.status,url:s.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}ba("CapacitorHttp",{web:()=>new Ns});const re=ba("PushNotifications",{}),Fs=(e,t,a="all",o="all")=>{const r={startDate:e,endDate:t};a&&a!=="all"&&(r.professionalId=a),o&&o!=="all"&&(r.costCenterId=o);const s=new URLSearchParams(r).toString();return y(`/api/reports/indicators?${s}`)},Rs=(e,t="all")=>{const a={date:e};t&&t!=="all"&&(a.professionalId=t);const o=new URLSearchParams(a).toString();return y(`/api/reports/appointments/list?${o}`)},js=()=>y("/api/financial/cost-centers"),Hs=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:o})=>{let r=`/api/reports/sales/${e}?startDate=${t}&endDate=${a}`;return o&&o!=="all"&&(r+=`&cashierSessionId=${o}`),y(r)},Os=()=>y("/api/reports/summary",{method:"GET"}),Me=e=>y(`/api/services/${e}`),zs=e=>y("/api/services",{method:"POST",body:JSON.stringify(e)}),Vs=(e,t)=>y(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),Us=e=>y(`/api/services/${e}`,{method:"DELETE"}),Js=(e,t)=>y(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),_s=e=>y(`/api/services/stats/most-popular/${e}`),Bt=(e,t,a,o="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${o}`;return y(r)},Dt=e=>y("/api/blockages",{method:"POST",body:JSON.stringify(e)}),fa=e=>y(`/api/blockages/${e}`,{method:"DELETE"}),yo=e=>y("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),Mt=e=>y(`/api/clients/${e}`),va=e=>y("/api/clients",{method:"POST",body:JSON.stringify(e)}),Ws=(e,t)=>y(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Gs=e=>y(`/api/clients/${e}`,{method:"DELETE"}),Na=(e,t,a)=>{const o=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return y(o)},Ys=(e,t,a)=>{const o=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return y(o)},Xs=(e,t,a,o)=>y("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:o})}),Fa=document.getElementById("content");let Ra=!1;const ea=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let Ze=[],Pt=[],We={},ae=[],L={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},S={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Qs(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function wo(e){const t=new Date(e);if(t.setHours(0,0,0,0),L.currentView==="week"&&L.weekViewDays===7){const a=t.getDay(),o=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(o))}return t}function wt(){const e=document.getElementById("profSelectorContainer"),t=L.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(s=>L.showInactiveProfs||s.status!=="inactive");t&&(a=a.filter(s=>s.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(s=>{const n=L.selectedProfessionalId===s.id,i=s.name==="Todos"?"Todos":s.name.split(" ")[0],l=s.name==="Todos"?"T":s.name.charAt(0).toUpperCase(),d=s.status!=="inactive",u=ea[0],c=s.id!=="all"&&p.professionalColors.get(s.id)||u,m=s.photo||`https://placehold.co/64x64/${c.main?.replace("#","")||"E0E7FF"}/${c.light?.replace("#","")||"4F46E5"}?text=${l}`,b=s.id==="all"?"#e0e7ff":c.light,f=s.id==="all"?"#4f46e5":c.main,w=`border: 3px solid ${n?c.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+c.border:"none"};`;return`
            <div class="prof-card ${n?"selected":""} ${d?"":"opacity-50"}" 
                 data-action="select-professional" 
                 data-prof-id="${s.id}">
                ${s.id==="all"?`<div class="prof-card-all-placeholder" style="background-color: ${b}; color: ${f}; ${w}">
                           ${l}
                          </div>`:`<img src="${m}" alt="${s.name}" class="prof-card-photo" style="${w}" />`}
                <span class="prof-card-name">${i}</span>
            </div>
        `}).join("")}function Zs(e,t,a,o,r){const s=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${o} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,d=encodeURIComponent(l);return`https://wa.me/${s}?text=${d}`}function Ks(e){const t=document.getElementById("agenda-view");if(e.sort((o,r)=>new Date(o.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(o=>{const r=new Date(o.startTime),s=new Date(o.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=s.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=p.professionalColors.get(o.professionalId)||{};if(o.type==="blockage")return`
                ...
                </div>`;const d=o.status==="completed",u=d?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",c=d?"Finalizado":"Aberto",m=JSON.stringify(o).replace(/'/g,"&apos;"),b=o.redeemedReward?.points>0,f=o.hasRewards&&!b,v=Zs(o.clientPhone,o.clientName,o.serviceName,o.professionalName,o.startTime);return`
            <div class="appointment-list-card" data-appointment='${m}' style="border-left-color: ${l.border};">
                ...
                <div class="status-info">
                    ...
                    `;const w=JSON.stringify(m).replace(/'/g,"&apos;"),h=m.redeemedReward?.points>0,k=m.hasRewards&&!h,$=m.status==="completed";return`
                    <div class="p-2 rounded-lg border-l-4 flex flex-col cursor-pointer" 
                         style="background-color: ${v.bg}; border-left-color: ${v.border};"
                         data-action="open-comanda" data-appointment='${w}'>
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",ir)}function dr(){lr()}function cr(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",o=`
    ...
    `;return{title:a,content:o}}function ur(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${Ze.map(o=>{const r=S.data.selectedServiceIds.includes(o.id),s=o.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
    ...
    `;return{title:e,content:a}}function mr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Pt.map(a=>{const o=S.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
    ...
    `;return{title:e,content:t}}function pr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,o=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=S.data.date||o,s=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:s}}function gr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),o=Ze.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=S.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            <div class="service-card p-3 bg-white rounded-lg border-2 border-gray-200 cursor-pointer transition-all hover:bg-gray-50 ${s?"selected border-blue-500":""}" data-service-id="${r.id}">
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>ko(r.dataset.serviceId,r))})}function br(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),o=Pt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=S.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
             ...
             </div>`}).join(""),t.querySelectorAll(".professional-modal-card").forEach(r=>{r.addEventListener("click",()=>So(r.dataset.professionalId,r))})}async function aa(e=null,t=!1){const a=document.getElementById("appointmentModal");if(!t){const s=e?.startTime?new Date(e.startTime).toISOString().split("T")[0]:new Date().toISOString().split("T")[0],n=e?.startTime?new Date(e.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",hour12:!1}):null;S={step:1,data:{id:e?.id||null,clientName:e?.clientName||"",clientPhone:e?.clientPhone||"",selectedServiceIds:e?.services?.map(i=>i.id)||[],professionalId:e?.professionalId||null,professionalName:e?.professionalName||"",date:s,time:n,redeemedReward:e?.redeemedReward||null,clientHasRewards:e?.hasRewards||!1,clientLoyaltyPoints:0}}}if(!p.services||!p.professionals||!ae||We.enabled===void 0){g("Erro","Os dados da agenda ainda não foram carregados. Tente novamente em alguns segundos.","error");return}if(Ze=p.services,Pt=p.professionals.filter(s=>s.status==="active"),S.data.clientName&&S.data.clientPhone){const s=ae.find(n=>n.phone===S.data.clientPhone&&n.name===S.data.clientName);s&&(S.data.clientLoyaltyPoints=s.loyaltyPoints||0)}let o={title:"Erro",content:"<p>Etapa não encontrada.</p>"};switch(S.step){case 1:o=cr(e);break;case 2:o=ur();break;case 3:o=mr();break;case 4:o=pr(e);break}a.innerHTML=`
        ...
        </section>`,document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),L.currentView=a.dataset.view;const o=document.getElementById("week-days-toggle");if(L.currentView==="week"){if(o.style.display="flex",window.innerWidth<768){L.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="3"]');r&&r.classList.add("active")}}else o.style.display="none";K()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active")),a.classList.add("active"),L.weekViewDays=parseInt(a.dataset.days,10),K()})}),document.getElementById("todayBtn").addEventListener("click",()=>{L.currentDate=new Date,K()});const t=a=>{const o=parseInt(a.currentTarget.dataset.amount,10),r=L.currentView==="week"?ha():1,s=new Date(L.currentDate);s.setDate(s.getDate()+o*r),L.currentDate=s,K()};document.getElementById("prevBtn").addEventListener("click",t),document.getElementById("nextBtn").addEventListener("click",t),document.getElementById("profSearchInput").addEventListener("input",a=>{L.profSearchTerm=a.target.value,wt()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{L.showInactiveProfs=a.target.checked,wt(),K()}),Ra||(Fa.addEventListener("click",async a=>{const o=a.target.closest("[data-action]");if(a.target.closest('[data-action="select-professional"]')){const l=a.target.closest('[data-action="select-professional"]').dataset.profId,d=L.selectedProfessionalId===l&&l!=="all";if(L.selectedProfessionalId=d?"all":l,l!=="all"){const u=document.getElementById("profSearchInput");u&&(u.value=""),L.profSearchTerm=""}await K();return}if(!o)return;const r=o.dataset.action;let s=null;const n=a.target.closest("[data-appointment]");switch(n&&(s=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":aa();break;case"edit-appointment":if(!s)return;if(s.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}s.hasRewards&&!s.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),aa(s);break;case"delete-appointment":{const i=o.dataset.id;if(await A("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await cs(i),g("Agendamento apagado!","success"),K()}catch(d){g(`Não foi possível apagar: ${d.message}`,"error")}break}case"open-comanda":if(s){s.hasRewards&&!s.redeemedReward&&s.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=s.status==="completed"?"finalizadas":"em-atendimento",l={selectedAppointmentId:s.id,initialFilter:i};if(i==="finalizadas"){let d=s.startTime;if(s.transaction&&s.transaction.paidAt){const u=s.transaction.paidAt;typeof u=="object"&&u._seconds?d=new Date(u._seconds*1e3):d=u}l.filterDate=d}Y("comandas-section",l)}break}}),Ra=!0),await ar(),await K()}const fr=(e,t=null,a=1,o=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${o}`;return t&&(r+=`&date=${t}`),y(r)},vr=e=>y("/api/sales",{method:"POST",body:JSON.stringify(e)}),hr=e=>y(`/api/sales/${e}/reopen`,{method:"POST"}),xr=e=>y(`/api/sales/${e}`,{method:"DELETE"}),At=e=>y(`/api/products/${e}`),yr=e=>y("/api/products",{method:"POST",body:JSON.stringify(e)}),wr=(e,t)=>y(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),kr=e=>y(`/api/products/${e}`,{method:"DELETE"}),Sr=(e,t)=>y(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),$r=({startDate:e,endDate:t,productId:a,categoryId:o})=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("productId",a),o&&o!=="all"&&r.append("categoryId",o),y(`/api/products/stock-history/report?${r.toString()}`)},Eo=()=>y("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),Er=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),y("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},Cr=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),y(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},Ir=()=>y("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),Lr=e=>y(`/api/cashier/report/${e}`),Co=e=>y(`/api/packages/${e}`),Tr=e=>y("/api/packages",{method:"POST",body:JSON.stringify(e)}),Br=(e,t)=>y(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),Dr=e=>y(`/api/packages/${e}`,{method:"DELETE"});let x={allComandas:[],catalog:{services:[],products:[],packages:[]},clients:[],activeFilter:"atendimento",selectedComandaId:null,isCashierOpen:!1,activeCashierSessionId:null,paging:{page:1,limit:12,total:0}},xe=null,ye=null;function qt(e){if(e.status==="completed"){const r=e.comandaItems||e.items||[];return r.length>0?r:e.services||[]}const t=e.services||[],a=[...e.comandaItems||[],...e.items||[]];let o=[...t];return a.forEach(r=>{if(r.type==="product"){o.push(r);return}o.some(n=>{const i=n.id&&r.id&&n.id===r.id,l=n.name&&r.name&&n.name===r.name;return i||l})||o.push(r)}),o}function Mr(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function ge(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function Pr(){const e=new Date().toISOString().split("T")[0];ye.innerHTML=`
    ...
    `}function Ar(){const e=document.getElementById("cashier-controls");e&&(x.isCashierOpen?e.innerHTML=`
    ...
    `;if(!x.isCashierOpen){e.innerHTML=`
        ...
        `;return}const a=x.allComandas.find(d=>d.id===x.selectedComandaId);if(!a){e.innerHTML=`
            <div class="hidden lg:flex flex-col items-center justify-center h-full text-center text-gray-400">
        ...
        `;return}const o=qt(a),r=a.status==="completed",s=a.type==="walk-in"||a.id.startsWith("temp-"),n=s?"":`<button data-action="go-to-appointment" data-id="${a.id}" data-date="${a.startTime}" 
           ...
           </button>`,i=o.reduce((d,u)=>{const c=`${u.type}-${u.id||u.name}`;return d[c]||(d[c]={...u,quantity:0}),d[c].quantity+=1,d},{}),l=Object.values(i).reduce((d,u)=>d+(u.price||0)*u.quantity,0);e.innerHTML=`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",Nr)}async function Nr(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector('button[type="submit"]'),o={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim()||null,phone:t.querySelector("#regClientPhone").value.trim(),dob:`${t.querySelector("#regClientDobDay").value.trim()}/${t.querySelector("#regClientDobMonth").value.trim()}`,notes:t.querySelector("#regClientNotes").value.trim()||null};if(!o.name||!o.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{const r=await va(o);x.clients.push({id:r.id,...o}),g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",Io(r.id)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar Cliente"}}function Fr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");return}const{modalElement:e,close:t}=N({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{let r="";const s=e.querySelector("#add-item-content"),n={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>',package:'<svg class="w-6 h-6 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7m8 4v10M4 7v10l8 4" /></svg>'},i=()=>{const l=r.toLowerCase(),d=c=>c.filter(m=>m.name.toLowerCase().includes(l)),u={"modal-service-list":{items:d(x.catalog.services),type:"service"},"modal-product-list":{items:d(x.catalog.products),type:"product"},"modal-package-list":{items:d(x.catalog.packages),type:"package"}};for(const[c,{items:m,type:b}]of Object.entries(u)){const f=s.querySelector(`#${c}`);f&&(f.innerHTML=m.map(v=>`
            ...
            </div>`,i(),s.querySelector("#item-search-input").addEventListener("input",l=>{r=l.target.value,i()})},o=r=>{let s=1;const n=e.querySelector("#add-item-content"),i=()=>{document.getElementById("quantity-display").textContent=s,document.getElementById("quantity-minus-btn").disabled=s<=1};n.innerHTML=`
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{s>1&&(s--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{s++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await zr(r,s),t()}};e.addEventListener("click",r=>{const s=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(s){const{itemType:i,itemId:l}=s.dataset,u=(x.catalog[i+"s"]||[]).find(c=>c.id===l);u&&o({...u,type:i})}else n&&a()}),a()}async function Io(e=null){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");return}if(!x.clients||x.clients.length===0)try{x.clients=await Mt(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de clientes.","error");return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await _(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de profissionais.","error");return}const t=x.clients.map(n=>{const i=n.id===e?"selected":"";return`<option value="${n.id}" ${i}>${n.name} - ${n.phone}</option>`}).join(""),a=p.professionals.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""),o=`
    ...
    `,{modalElement:r}=N({title:"Nova Venda Avulsa",contentHTML:o,maxWidth:"max-w-md"});r.querySelector("#new-sale-form").addEventListener("submit",Jr);const s=r.querySelector('[data-action="new-client-from-sale"]');s&&s.addEventListener("click",n=>{n.preventDefault(),r.style.display="none",qr()})}function Rr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de finalizar pagamentos.","error");return}const e=x.allComandas.find(c=>c.id===x.selectedComandaId);if(!e)return;const t=qt(e),a=new Set,o=[];for(const c of t){if(c.type==="service"&&c.id){if(a.has(c.id))continue;a.add(c.id)}o.push(c)}const r=o.reduce((c,m)=>c+(m.price||0),0);let s=[],n={remainingAmount:r,selectedMethod:"dinheiro",installments:1,amountReceived:""};const i=()=>{const c=document.getElementById("payment-list"),m=document.getElementById("remaining-amount"),b=document.getElementById("finalize-checkout-btn"),f=document.getElementById("change-container"),v=document.getElementById("installments-container"),w=document.getElementById("payment-value"),h=document.getElementById("payment-controls"),k=s.reduce((E,I)=>E+I.value,0);n.remainingAmount=r-k,c.innerHTML=s.map((E,I)=>`
    ...
    `,{modalElement:t}=N({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const o=document.getElementById("initial-amount"),r=o.value.trim(),s=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),o.focus();return}try{const i={establishmentId:p.establishmentId,initialAmount:parseFloat(n.toFixed(2))};s&&(i.notes=s);const l=await Er(i);x.isCashierOpen=!0,x.activeCashierSessionId=l.id,await xa(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function Hr(){const e=x.activeCashierSessionId;if(e)try{const t=await Lr(e),a=`
        ...
        `,{modalElement:o}=N({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});o.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const s=parseFloat(document.getElementById("final-amount").value);if(isNaN(s)||s<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await Cr(e,s),x.isCashierOpen=!1,x.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await xa(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function Or(e){x.activeFilter!==e&&(x.activeFilter=e,x.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),ge(),await te(),x.selectedComandaId=null,G())}function oa(e){x.selectedComandaId=e,de(),Mr(),G()}async function zr(e,t){const a=x.allComandas.find(r=>r.id===x.selectedComandaId);if(!a)return;const o=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...o),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),G(),de();return}try{await ga(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),G(),de()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function Vr(e,t){const a=x.allComandas.find(s=>s.id===x.selectedComandaId);if(!a)return;let o=!1,r=(a.comandaItems||[]).findIndex(s=>s.id===e&&s.type===t);if(r>-1)a.comandaItems.splice(r,1),o=!0;else{let s=(a.services||[]).findIndex(n=>n.id===e);if(s>-1)a.services.splice(s,1),o=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),o=!0)}}if(o){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),G(),de();return}try{await ga(a.id,a),g("Sucesso","Item removido!","success"),G(),de()}catch(s){g("Erro",`Não foi possível remover o item: ${s.message}`,"error"),await te()}}}async function Ur(e,t,a){const o=e.type==="appointment",r=qt(e),s=new Set,n=[];for(const l of r){if(l.type==="service"&&l.id){if(s.has(l.id))continue;s.add(l.id)}n.push(l)}const i={payments:a,totalAmount:t,items:n,cashierSessionId:x.activeCashierSessionId};try{o?await ms(e.id,i):(i.clientName=e.clientName,i.professionalId=e.professionalId,i.clientPhone=e.clientPhone,await vr(i)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",ge(),x.selectedComandaId=null,await te()}catch(l){g("Erro no Checkout",l.message,"error")}}async function Jr(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,o=x.clients.find(n=>n.id===t),r=p.professionals.find(n=>n.id===a);if(!o||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const s={id:`temp-${Date.now()}`,type:"walk-in",clientName:o.name,clientPhone:o.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};x.allComandas.unshift(s),x.selectedComandaId=s.id,document.getElementById("genericModal").style.display="none",oa(s.id)}async function te(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=x.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await Eo();if(x.isCashierOpen=!!a,x.activeCashierSessionId=a?a.id:null,Ar(),!x.isCashierOpen&&x.activeFilter==="atendimento"){de(),G();return}const o=await fr(p.establishmentId,t,x.paging.page,x.paging.limit);if(x.allComandas=o.data||o,x.paging.total=o.total||o.length,x.catalog.services.length===0){const[r,s,n,i,l]=await Promise.all([Me(p.establishmentId),At(p.establishmentId),Co(p.establishmentId),Mt(p.establishmentId),_(p.establishmentId)]);x.catalog={services:r,products:s,packages:n},x.clients=i,p.professionals=l}de(),x.selectedComandaId,G()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function xa(e={}){ye=document.getElementById("content");try{const t=await Eo();x.isCashierOpen=!!t,x.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),x.isCashierOpen=!1}x.selectedComandaId=e.selectedAppointmentId||null,Pr(),xe&&(ye.removeEventListener("click",xe),ye.removeEventListener("change",xe)),xe=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&x.activeFilter==="finalizadas"){x.paging.page=1,await te();return}if(a){if(a.matches("[data-filter]"))Or(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}oa(a.dataset.comandaId)}else if(a.matches("[data-action]")){const o=a.dataset.action,r=a.dataset.id||x.selectedComandaId;switch(o){case"back-to-list":{ge(),x.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(s=>s.classList.remove("selected")),G();break}case"new-sale":Io();break;case"add-item":Fr();break;case"checkout":Rr();break;case"open-cashier":jr();break;case"close-cashier":await Hr();break;case"view-sales-report":Y("sales-report-section");break;case"remove-item":await Vr(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await A("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await us(r);const n=x.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete x.allComandas[n].transaction,delete x.allComandas[n].cashierSessionId,delete x.allComandas[n].redeemedReward,x.allComandas[n].status="confirmed"),x.selectedComandaId=null,ge(),g("Sucesso!","Comanda reaberta para edição.","success"),await te()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await A("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await hr(r),g("Sucesso!","Venda revertida."),ge(),x.selectedComandaId=null,await te()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const s=a.dataset.id,n=a.dataset.date;Y("agenda-section",{scrollToAppointmentId:s,targetDate:new Date(n)});break}case"delete-walk-in":{if(await A("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))x.allComandas=x.allComandas.filter(n=>n.id!==r),x.selectedComandaId=null,de(),G(),g("Sucesso","Venda avulsa removida.","success"),ge();else try{await xr(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),x.selectedComandaId=null,ge(),await te()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},ye.addEventListener("click",xe),ye.addEventListener("change",xe),e.initialFilter&&(x.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(x.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${x.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",x.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await te(),x.selectedComandaId&&oa(x.selectedComandaId)}const sa=document.getElementById("content");let Ee={};const st=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],B={startDate:new Date(new Date().getFullYear(),0,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,currentTab:"dashboards",apptViewMode:"year",apptSelectedMonth:null};async function _r(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function Wr(){sa.innerHTML='<div class="flex flex-col items-center justify-center h-64"><div class="loader mb-4"></div><p class="text-gray-500">Carregando inteligência de dados...</p></div>';try{await _r();const[e,t]=await Promise.all([_(p.establishmentId),js().catch(()=>[])]);B.professionalsList=e,B.costCentersList=t,Gr(),await Lo()}catch(e){sa.innerHTML=`
    ...
    `,document.getElementById("btn-filter").onclick=Yr,document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{B.currentTab=a.dataset.tab,za(),To()}}),za()}function za(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===B.currentTab;e.className=t?"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-bold bg-white text-indigo-600 shadow-sm transition-all whitespace-nowrap":"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition-all whitespace-nowrap"})}async function Yr(){B.startDate=document.getElementById("report-start").value,B.endDate=document.getElementById("report-end").value,B.selectedProfessional=document.getElementById("report-prof").value,B.selectedCostCenter=document.getElementById("report-cost").value,B.apptViewMode="year",B.apptSelectedMonth=null,await Lo()}async function Lo(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader"></div></div>';try{const t=await Fs(B.startDate,B.endDate,B.selectedProfessional,B.selectedCostCenter);B.data=t,To()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function To(){const e=document.getElementById("report-content");switch(B.currentTab){case"dashboards":Xr(e);break;case"appointments":ya(e);break;case"dre":en(e);break}}function Xr(e){const{dreSimple:t,charts:a}=B.data,o=t||{grossRevenue:0,netProfit:0,variableCosts:0},r=B.data.totalAppointments||0,s=r>0?o.grossRevenue/r:0;e.innerHTML=`
    ...
    `,rt("chart-monthly","bar","Receita Mensal",a.salesMonthly.labels,a.salesMonthly.data,st[0]),rt("chart-profs","doughnut","Total Vendas",a.professionals.labels,a.professionals.data,st),rt("chart-daily","line","Vendas Diárias",a.salesDaily.labels,a.salesDaily.data,st[4]),rt("chart-products","bar","Total Vendido",a.products.labels,a.products.data,st[1])}function ya(e){const{charts:t}=B.data;let a='<h3 class="font-bold text-gray-700 text-lg">Visão Anual/Mensal</h3>',o="Clique na barra do Mês para detalhar os dias";B.apptViewMode==="month"&&(a=`
    ...
    `,Qr();const r=document.getElementById("btn-back-year");r&&(r.onclick=()=>{B.apptViewMode="year",B.apptSelectedMonth=null,ya(e)})}function Qr(){const e=document.getElementById("chart-appointments").getContext("2d");Ee["chart-appointments"]&&Ee["chart-appointments"].destroy();const{charts:t}=B.data;let a,o;if(B.apptViewMode==="year")a=t.appointmentsMonthly.labels,o=t.appointmentsMonthly.data;else{const s={jan:"01",fev:"02",mar:"03",abr:"04",mai:"05",jun:"06",jul:"07",ago:"08",set:"09",out:"10",nov:"11",dez:"12"},[n,i]=B.apptSelectedMonth.split("/"),l=s[n.toLowerCase()]||n,d=i.length===2?"20"+i:i,u=Object.entries(t.appointmentsDaily.data).map(([c,m],b)=>{const f=t.appointmentsDaily.labels[b];return f.includes(`/${l}/${d}`)?{label:f,value:m}:null}).filter(Boolean);a=u.map(c=>c.label),o=u.map(c=>c.value)}const r={type:"bar",data:{labels:a,datasets:[{label:"Volume de Agendamentos",data:o,backgroundColor:B.apptViewMode==="year"?"#3b82f6":"#8b5cf6",borderRadius:4,hoverBackgroundColor:"#1e40af"}]},options:{responsive:!0,maintainAspectRatio:!1,onClick:(s,n)=>{if(n.length>0){const i=n[0].index,l=a[i];Zr(l)}},plugins:{legend:{display:!1},tooltip:{callbacks:{label:s=>` ${s.raw} Agendamentos`}}}}};Ee["chart-appointments"]=new Chart(e,r)}function Zr(e){B.apptViewMode==="year"?(B.apptViewMode="month",B.apptSelectedMonth=e,ya(document.getElementById("report-content"))):B.apptViewMode==="month"&&Kr(e)}async function Kr(e){const[t,a,o]=e.split("/"),r=`${o}-${a}-${t}`;N({title:"Carregando...",contentHTML:'<div class="p-8 text-center"><div class="loader mx-auto"></div></div>',maxWidth:"max-w-md"});try{const s=await Rs(r,B.selectedProfessional);let n="";s.length===0?n='<div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">Nenhum agendamento encontrado para este dia.</div>':n=`
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",an),t.addEventListener("click",o=>{const r=o.target.closest('button[data-action="delete-category"]');r&&(o.preventDefault(),on(r.dataset.id))}))}wa()}async function rn(e){e.preventDefault();const t=e.target.closest("#serviceModal"),a=t.querySelector("#serviceId").value,o={},r=t.querySelector('input[name="commissionType"]:checked').value;r==="custom"&&t.querySelectorAll(".professional-commission-row").forEach(n=>{const i=n.dataset.profId;if(n.querySelector('input[type="checkbox"]').checked){const d=parseFloat(n.querySelector('input[type="number"]').value);o[i]=isNaN(d)?0:d}});const s={establishmentId:p.establishmentId,name:t.querySelector("#serviceName").value,price:parseFloat(t.querySelector("#servicePrice").value),duration:parseInt(t.querySelector("#serviceDurationMinutes").value,10),bufferTime:parseInt(t.querySelector("#serviceBufferTimeMinutes").value,10)||0,categoryId:t.querySelector("#serviceCategory").value||null,commissionRate:parseFloat(t.querySelector("#serviceCommissionRate").value)||0,active:t.querySelector("#serviceStatus").value==="true",photo:t.querySelector("#servicePhotoBase64").value,notes:t.querySelector("#serviceNotes").value,commissionType:r,professionalCommissions:o};try{a?(await Vs(a,s),Te(p.establishmentId,Be(),"Serviços","Editou",`Editou o serviço: ${s.name}`)):(await zs(s),Te(p.establishmentId,Be(),"Serviços","Criou",`Criou novo serviço: ${s.name}`)),document.getElementById("serviceModal").style.display="none",g("Sucesso",`Serviço ${a?"atualizado":"adicionado"} com sucesso!`,"success"),await Ke()}catch(n){g("Erro",n.message,"error")}}function nn(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=l=>{const d=new Image;d.onload=()=>{let u=d.width,c=d.height;u>c?u>t&&(c*=t/u,u=t):c>a&&(u*=a/c,c=a);const m=document.createElement("canvas");m.width=u,m.height=c,m.getContext("2d").drawImage(d,0,0,u,c);const f=m.toDataURL(o,r);s(f)},d.onerror=u=>n(new Error("Não foi possível carregar a imagem.")),d.src=l.target.result},i.onerror=l=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Va(e=null){const t=document.getElementById("serviceModal"),a=p.serviceCategories||[],o=e?.duration||0,r=e?.bufferTime||0,s=a.map(h=>`<option value="${h.id}" ${e?.categoryId===h.id?"selected":""}>${h.name}</option>`).join("");t.innerHTML=`
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",vn),t.addEventListener("click",o=>{const r=o.target.closest('button[data-action="delete-category"]');r&&hn(r.dataset.id)}))}$a()}async function yn(e){if(!e)return;if(await A("Apagar Produto","Tem a certeza que deseja apagar este produto? Esta ação não pode ser revertida."))try{await kr(e),g("Sucesso","Produto apagado com sucesso!","success"),await et()}catch(a){g("Erro",`Não foi possível apagar o produto: ${a.message}`,"error")}}async function wn(e){const t=e.querySelector("#productId").value,a=parseInt(e.querySelector("#productCurrentStock").value),o=parseInt(e.querySelector("#productMinStock").value),r=parseInt(e.querySelector("#productMaxStock").value),s=e.querySelectorAll("#selectedSuppliersList .selected-supplier-item"),n=Array.from(s).map(l=>l.dataset.id),i={establishmentId:p.establishmentId,name:e.querySelector("#productName").value,price:parseFloat(e.querySelector("#productPrice").value),costPrice:parseFloat(e.querySelector("#productCostPrice").value)||0,commissionRate:parseFloat(e.querySelector("#productCommissionRate").value)||0,currentStock:isNaN(a)?0:a,minStock:isNaN(o)?0:o,maxStock:isNaN(r)?0:r,categoryId:e.querySelector("#productCategory").value||null,photo:e.querySelector("#productPhotoBase64").value,supplierIds:n};try{t?await wr(t,i):await yr(i),document.getElementById("productModal").style.display="none",g("Sucesso",`Produto ${t?"atualizado":"adicionado"} com sucesso!`,"success"),await et()}catch(l){throw new Error(l.message)}}function Ua(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=l=>{const d=new Image;d.onload=()=>{let u=d.width,c=d.height;u>c?u>t&&(c*=t/u,u=t):c>a&&(u*=a/c,c=a);const m=document.createElement("canvas");m.width=u,m.height=c,m.getContext("2d").drawImage(d,0,0,u,c);const f=m.toDataURL(o,r);s(f)},d.onerror=u=>n(new Error("Não foi possível carregar a imagem.")),d.src=l.target.result},i.onerror=l=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Ja(e=null){const t=document.getElementById("productModal"),a=p.categories||[],o=p.suppliers||[],r=a.map(w=>`<option value="${w.id}" ${e?.categoryId===w.id?"selected":""}>${w.name}</option>`).join("");let s=new Set(e?.supplierIds||[]);t.innerHTML=`
    <div class="modal-content max-w-3xl overflow-y-auto max-h-[90vh]">
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
    ...
    </div>`;const n=t.querySelector("#productCategory"),i=t.querySelector("#productPhotoInput");t.querySelector("#productPhotoButton").addEventListener("click",()=>i.click()),n.innerHTML='<option value="">Sem categoria</option>'+(p.categories||[]).map(w=>`<option value="${w.id}" ${e?.categoryId===w.id?"selected":""}>${w.name}</option>`).join(""),e&&(n.value=e.categoryId||"");const l=t.querySelector("#productPhotoPreview");t.querySelector("#productPhotoBase64");const d=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",u=e?.photo||"";i.onchange=async()=>{const w=i.files[0];if(w){l.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const h=await Ua(w,800,800,"image/jpeg",.8),$=h.length*3/4,E=1e3*1024;if($>E)throw new Error("A imagem é muito grande mesmo após a compressão.");preview.src=h,base64Input.value=h}catch(h){console.error("Erro ao processar imagem:",h),g("Erro de Imagem",h.message||"Não foi possível processar a imagem.","error"),preview.src=d,base64Input.value=u,v.value=""}}};const c=t.cloneNode(!0);t.parentNode.replaceChild(c,t);const m=()=>{const w=c.querySelector("#modalSupplierSearch"),h=c.querySelector("#supplierSearchResults"),k=c.querySelector("#selectedSuppliersList"),$=w.value.toLowerCase();if($.length>0){const E=o.filter(I=>I.name.toLowerCase().includes($)&&!s.has(I.id));E.length>0?(h.classList.remove("hidden"),h.innerHTML=E.map(I=>`
                ...
                `).join("")):(h.classList.remove("hidden"),h.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else h.classList.add("hidden");s.size>0?(k.innerHTML="",s.forEach(E=>{const I=o.find(D=>D.id===E);I&&(k.innerHTML+=`
    ...
    `;a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=o+r}function Bn(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",o=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),s=`
    ...
    `;N({title:"",contentHTML:s,maxWidth:"max-w-md"})}async function Oe(){const e=document.getElementById("purchasesContainer");if(e)if(P.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([At(p.establishmentId),Sa()]);P.allSuppliers=a||[];const o=t.filter(d=>{const u=parseInt(d.currentStock||0),c=parseInt(d.minStock||0);return u<=c});if(P.productsToBuy=o,o.length===0){e.innerHTML=`
                ...
                `;return}let r='<div class="flex flex-col gap-3 md:hidden">',s="";o.forEach(d=>{const u=parseInt(d.minStock)||0,c=parseInt(d.currentStock)||0,m=Math.max(u-c,1),b=parseFloat(d.costPrice||0);let f='<option value="">Selecione...</option>';P.allSuppliers.length>0?P.allSuppliers.forEach(v=>{const h=d.supplierIds&&d.supplierIds.includes(v.id)?"selected":"";f+=`<option value="${v.id}" ${h}>${v.name}</option>`}):f='<option value="">Sem fornecedores</option>',r+=`
        ...
        `}).join("");o+=f;const w=encodeURIComponent(JSON.stringify({supplierId:m,supplierName:b.info.name,totalAmount:f,items:b.items})),h=encodeURIComponent(JSON.stringify({name:b.info.name,phone:b.info.phone,email:b.info.email})),k=encodeURIComponent(JSON.stringify(b.items));a+=`
    ...
    `,le&&(ue.removeEventListener("click",le),ue.removeEventListener("input",le),ue.removeEventListener("change",le)),le=e=>{if(e.target.closest("#tab-btn-list")&&nt("list"),e.target.closest("#tab-btn-purchases")&&nt("purchases"),e.target.closest("#tab-btn-history")&&nt("history"),e.target.id==="toggle-quote-mode"&&(P.isQuoteMode=e.target.checked,Oe()),e.target.id==="supplierSearchInput"&&Rt(),e.target.closest("#btn-new-supplier")&&_a(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),o=JSON.parse(a.dataset.supplier);Bn(o)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&Ln(t.dataset.id),a==="edit"&&_a(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&ra(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(o=>o.checked=a),ra()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),o={};let r=!1;if(a.forEach(s=>{if(s.offsetParent===null||!s.querySelector(".row-select").checked)return;r=!0;let i="Produto";const l=s.querySelector("td:nth-child(2)"),d=s.querySelector(".font-bold");l?i=l.innerText:d&&(i=d.innerText);const u=parseInt(s.querySelector(".qty-input").value),c=parseFloat(s.dataset.cost),b=s.querySelector(".supplier-select").value;if(b){if(!o[b]){const f=P.allSuppliers.find(v=>v.id===b);o[b]={info:f,items:[]}}o[b].items.push({name:i,qty:u,cost:c})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}P.finalOrders=o,P.step=2,Oe()}if(e.target.closest("#btn-back-step1")&&(P.step=1,Oe()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),o=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),s=parseFloat(a.dataset.total),n=P.isQuoteMode;if(o.phone){const i=o.phone.replace(/\D/g,"");let l="";n?(l=`Olá *${o.name}*, tudo bem?
...
`,r.forEach(u=>{l+=`- ${u.qty}x ${u.name}
`}),l+=`
Aguardo confirmação.`);const d=`https://wa.me/${i}?text=${encodeURIComponent(l)}`;window.open(d,"_blank"),g("Aberto","WhatsApp aberto.","success")}else if(o.email){const i=n?`Solicitação de Cotação - ${p.establishmentName||"Empresa"}`:`Pedido de Compra - ${p.establishmentName||"Empresa"}`;let l=`Olá ${o.name},
...
Aguardo retorno.`;const d=`mailto:${o.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),o=JSON.parse(decodeURIComponent(a.dataset.order));o.establishmentId=p.establishmentId,bn(o).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const o=e.target.closest(".supplier-order-card").dataset.supplierId,r=P.finalOrders[o];r?An(r,P.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),o=JSON.parse(a.dataset.purchase);Pn(o)}},ue.addEventListener("click",le),ue.addEventListener("input",le),ue.addEventListener("change",le),nt("list")}function nt(e){vt=e,["list","purchases","history"].forEach(a=>{const o=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(o.classList.add("border-indigo-500","text-indigo-600"),o.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(o.classList.remove("border-indigo-500","text-indigo-600"),o.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),Cn()}const zt=document.getElementById("content"),Wa={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let oe=new Set,it=null,ke=null;function Nn(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Fn(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,r=JSON.stringify(t).replace(/'/g,"&apos;");return`
        ...
        </div>`;t.innerHTML=s,t.style.display="flex",jn(a,o),On(a),zn(a,r),Un(a)}function jn(e,t){const a=document.getElementById("professionalForm"),o=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(m,b)=>{const f=b+1,v=f==o[1]?"selected":"",w=new Date(0,b).toLocaleString("pt-BR",{month:"long"});return`<option value="${f}" ${v}>${w.charAt(0).toUpperCase()+w.slice(1)}</option>`}).join(""),s=e.status||"active";a.innerHTML=`
        ...
        </div>`;const o=document.getElementById("batchBlockageForm");o&&o.addEventListener("submit",async s=>{s.preventDefault();const n=Array.from(s.target.querySelectorAll('input[name="batch-professionals"]:checked')).map(b=>b.value);if(n.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const i=s.target.batchBlockageStartDate.value,l=s.target.batchBlockageEndDate.value||i,d=s.target.batchBlockageStartTime.value,u=s.target.batchBlockageEndTime.value,c=s.target.batchBlockageReason.value;if(!i||!d||!u)return g("Atenção","Preencha Data de Início, Início e Fim.","error");const m=n.map(b=>{const f={professionalId:b,establishmentId:p.establishmentId,startTime:new Date(`${i}T${d}`).toISOString(),endTime:new Date(`${l}T${u}`).toISOString(),reason:c};return Dt(f)});try{await Promise.all(m),g("Sucesso!",`${n.length} bloqueios foram criados.`);const b=document.getElementById("prof-blockages-filter").value;ze(e.id,b)}catch(b){g("Erro",b.message,"error")}}),document.getElementById("prof-blockages-filter").addEventListener("change",s=>ze(e.id,s.target.value)),await ze(e.id,"future")}function Vn(e,t){e.innerHTML=Object.keys(Wa).map(a=>{const o=t[a]||{},r=o.active!==!1;return`
    ...
    `}function ti(e,t){const a=t==="agendamentos"?"Próximos Agendamentos":"Histórico de Visitas",o=t==="agendamentos"?"Nenhum agendamento futuro.":"Nenhum histórico de visitas.",r=new Date;r.setHours(0,0,0,0);const s=t==="agendamentos",n=(e||[]).filter(i=>{const l=new Date(i.date),d=new Date(l);d.setHours(0,0,0,0);const u=d.getTime()===r.getTime(),c=d>r,m=d<r,b=["completed","finalized","finished","paid"].includes((i.status||"").toLowerCase());return s?!b&&(c||u):m||b});return n.sort((i,l)=>s?new Date(i.date).getTime()-new Date(l.date).getTime():new Date(l.date).getTime()-new Date(i.date).getTime()),n.length===0?`<div class="p-6 text-center text-gray-500">${o}</div>`:`
    ...
    `}function ai(e,t){const a=(e||[]).filter(l=>{const d=(l.status||"").toLowerCase();return d==="completed"||d==="finished"||d==="paid"||d==="finalized"});let o=0;a.forEach(l=>{o+=Ca(l)});const r=Q&&Q.conversionRate?parseFloat(Q.conversionRate):qo,s=Math.floor(o*r);let n=0;(t||[]).forEach(l=>{l.type==="redeem"&&(n+=l.points||0)});const i=s-n;return{totalSpent:o,totalPointsEarned:s,totalPointsRedeemed:n,currentBalance:i}}function oi(e,t,a){if(!Q||!Q.enabled)return`<div class="p-8 text-center text-gray-500">
        ...
        </div>`;const o=ai(t,a);e.loyaltyPoints=o.currentBalance,e.totalSpent=o.totalSpent;const r=(Q.tiers||[]).map(l=>{const d=o.currentBalance>=l.points;return`
            ...
            </div>`}).join("")||'<p class="text-sm text-gray-400 italic">Nenhum prêmio configurado.</p>',s=[],n=Q.conversionRate||qo;(t||[]).forEach(l=>{const d=(l.status||"").toLowerCase();if(d==="completed"||d==="finished"||d==="paid"||d==="finalized"){const u=Ca(l),c=Math.floor(u*n);c>0&&s.push({type:"earn",desc:`Serviço/Compra (R$ ${u.toFixed(2)})`,points:c,date:l.date||l.createdAt})}}),(a||[]).forEach(l=>{s.push({type:"redeem",desc:`Resgate: ${l.reward||"Prêmio"}`,points:l.points,date:l.timestamp||l.date})}),s.sort((l,d)=>new Date(d.date)-new Date(l.date));const i=s.length>0?s.map(l=>`
    ...
    `).join(""):'<p class="text-xs text-center text-gray-400 py-4">Sem movimentações.</p>';return`
        <div class="p-4 space-y-6">
            <div class="bg-gradient-to-r from-indigo-600 to-blue-500 rounded-xl p-5 text-white shadow-lg text-center relative overflow-hidden">
                ...
                <p class="text-5xl font-extrabold mb-1">${o.currentBalance}</p>
                <p class="text-sm opacity-90">Total Gasto: R$ ${o.totalSpent.toFixed(2)}</p>
            </div>
    ...
    `}function No(e){j=e,ia="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",o=`
    ...
    `,r=window.innerWidth<768,s=r?"max-w-full":"max-w-3xl";if(N({title:a,contentHTML:o,maxWidth:s}),r){const d=document.getElementById("genericModal");if(d){const u=d.querySelector(`.${s.replace(":","\\:")}`);u&&(u.style.height="auto",u.style.maxHeight="85vh",u.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&(n.onclick=async d=>{const u=d.target.closest("[data-action]");if(!u)return;switch(u.dataset.action){case"redeem-reward":{const m=parseInt(u.dataset.points,10),b=u.dataset.reward;if(await A("Confirmar Resgate",`Deseja resgatar "${b}" por ${m} pontos?`))try{await Xs(p.establishmentId,j.name,j.phone,{points:m,reward:b}),g("Prêmio resgatado com sucesso!","success"),await da("fidelidade")}catch(v){g(`Erro ao resgatar: ${v.message}`,"error")}break}case"open-comanda-from-history":{const m=u.dataset.appointmentId;m&&(document.getElementById("genericModal").style.display="none",Y("comandas-section",{selectedAppointmentId:m,initialFilter:"finalizada"}));break}case"view-appointment":{const m=u.dataset.appointmentId,b=u.dataset.appointmentDate;m&&b&&(document.getElementById("genericModal").style.display="none",Y("agenda-section",{targetDate:b,scrollToAppointmentId:m}));break}case"save-client":{d.preventDefault(),Xa();break}}}),da("cadastro"),setTimeout(()=>{const d=document.getElementById("client-form");d&&d.addEventListener("submit",u=>{u.preventDefault(),Xa()})},500);const i=document.getElementById("cancelDetailViewBtn");i&&i.addEventListener("click",d=>{d.preventDefault(),document.getElementById("genericModal").style.display="none",jt()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await si()})}async function Xa(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await Ws(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await va(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await jt()}catch(o){g("Erro",`Não foi possível salvar: ${o.message}`,"error")}}async function si(){if(!j||!j.id)return;if(await A("Excluir Cliente",`Tem certeza que deseja excluir ${j.name}? Esta ação é irreversível.`))try{await Gs(j.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await jt()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function ht(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",a.className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 content-start p-2 pb-20",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const o=U==="inactive",r=U==="birthdays";e.forEach(s=>{const n=document.createElement("div");n.className="bg-white rounded-lg border border-gray-200 shadow-sm p-3 hover:shadow-md cursor-pointer transition-all flex flex-col justify-between h-full relative group",n.dataset.clientId=s.id;const i=s.loyaltyPoints||0,l=s.totalSpent||0;let d="";const c=`https://wa.me/55${s.phone?s.phone.replace(/\D/g,""):""}?text=`;if(o){const m=encodeURIComponent(Wn(s.name,la));d=`<a href="${c+m}" target="_blank" class="absolute top-2 right-2 text-blue-500 bg-blue-50 p-1 rounded-full z-10 hover:bg-blue-100"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM16.64 16.64C16.64 16.64 15.11 17.58 14.54 17.76C13.97 17.94 13.06 18.06 10.66 17.06C8.26 16.06 6.38 13.62 6.38 13.62C6.38 13.62 4.96 11.72 4.96 9.76C4.96 7.8 6.04 6.88 6.04 6.88C6.04 6.88 6.32 6.56 6.6 6.56C6.88 6.56 7.16 6.56 7.16 6.56C7.38 6.56 7.62 6.46 7.86 7.02C8.1 7.58 8.68 9.02 8.68 9.02C8.68 9.02 8.78 9.24 8.64 9.48C8.5 9.72 8.36 9.88 8.16 10.1C7.96 10.32 7.74 10.4 8.02 10.88C8.3 11.36 9.26 12.92 10.68 14.18C11.62 15.02 12.56 15.36 12.94 15.54C13.32 15.72 13.6 15.66 13.84 15.38C14.08 15.1 14.62 14.34 14.62 14.34C14.62 14.34 14.88 14.06 15.18 14.12C15.48 14.18 16.94 14.9 16.94 14.9C16.94 14.9 17.2 15.04 17.3 15.22C17.4 15.4 17.4 16.28 16.64 16.64Z"/></svg></a>`}else if(r&&Xn(s)){const m=encodeURIComponent(_n(s.name,la));d=`<a href="${c+m}" target="_blank" class="absolute top-2 right-2 text-green-500 bg-green-50 p-1 rounded-full z-10 hover:bg-green-100"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M... (mesmo icone)"/></svg></a>`}n.innerHTML=`
                ${d}
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div class="min-w-0">
                        <p class="font-bold text-gray-800 text-sm truncate leading-tight">${s.name}</p>
                        <p class="text-[10px] text-gray-500 truncate">${s.phone}</p>
                    </div>
                </div>
            ...
            `,n.addEventListener("click",()=>No(s)),a.appendChild(n)})}else a.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum cliente encontrado com os filtros aplicados.</p>'}function xt(e="",t="all"){const a=e.toLowerCase(),o=a.length>0;let r=0,s=90;const n=window.innerWidth<768;if(t==="birthdays"){const l=n?"mobileBirthMonthFilter":"birthMonthFilter",d=document.getElementById(l);d&&(r=parseInt(d.value,10))}else if(t==="inactive"){const l=n?"mobileInactiveDaysFilter":"inactiveDaysFilter",d=document.getElementById(l);d&&(s=parseInt(d.value,10))}let i=Se.filter(l=>!o||l.name.toLowerCase().includes(a)||(l.phone||"").includes(a));switch(t){case"birthdays":const l=new Date,d=l.getDate(),u=l.getMonth()+1;return i.filter(c=>{if(!c.dob)return!1;const m=c.dob.split("/");if(m.length!==2)return!1;const b=parseInt(m[0],10),f=parseInt(m[1],10);return r===99?b===d&&f===u:r===0?f>=1&&f<=12:f===r});case"inactive":return i.filter(c=>(c.lastAppointmentDaysAgo||Yn())>s);case"credit":return i.filter(c=>(c.loyaltyPoints||0)>0);case"all":default:return i}}async function ri(e){const t=document.getElementById("birthMonthFilterContainer"),a=document.getElementById("mobileBirthMonthFilterContainer"),o=document.getElementById("inactiveDaysFilterContainer"),r=document.getElementById("mobileInactiveDaysFilterContainer"),s=e==="birthdays",n=e==="inactive";if([t,a].forEach(d=>d?.classList.toggle("hidden",!s)),[o,r].forEach(d=>d?.classList.toggle("hidden",!n)),s&&U!=="birthdays"&&document.querySelectorAll("#birthMonthFilter, #mobileBirthMonthFilter").forEach(d=>d.value=99),n&&U!=="inactive"&&document.querySelectorAll("#inactiveDaysFilter, #mobileInactiveDaysFilter").forEach(d=>d.value=90),U===e&&!s&&!n)return;U=e,document.querySelectorAll(".client-filter-btn").forEach(d=>{d.classList.remove("bg-white","text-indigo-600","shadow"),d.classList.add("bg-gray-100","text-gray-600")}),document.querySelectorAll(`[data-filter-key="${e}"]`).forEach(d=>{d.classList.remove("bg-gray-100","text-gray-600"),d.classList.add("bg-white","text-indigo-600","shadow")});const i=document.getElementById("clientSearchInput").value,l=xt(i,U);ht(l,Se.length)}async function jt(){ia="list",Ut.innerHTML=`
    ...
    `;try{const[u,c]=await Promise.all([Mt(p.establishmentId),Qe(p.establishmentId)]);Se=u,Q=c.loyaltyProgram||{enabled:!1},la=c.name||"O Estabelecimento";const m=xt("",U);ht(m,Se.length)}catch{const c=document.getElementById("clientsList");c&&(c.innerHTML='<p class="text-red-500 col-span-full text-center mt-10">Erro ao carregar dados dos clientes.</p>')}const e=document.getElementById("filter-sheet"),t=document.getElementById("filter-overlay"),a=document.getElementById("openFilterSheetBtn"),o=document.getElementById("closeFilterSheetBtn"),r=()=>{e.classList.remove("translate-y-full"),t.classList.remove("hidden")},s=()=>{e.classList.add("translate-y-full"),t.classList.add("hidden")};a&&a.addEventListener("click",r),o&&o.addEventListener("click",s),t&&t.addEventListener("click",s);const n=u=>{const c=u.target.closest(".client-filter-btn");if(c){ri(c.dataset.filterKey);const m=c.dataset.filterKey;window.innerWidth<768&&m!=="birthdays"&&m!=="inactive"&&s()}},i=document.getElementById("desktop-filter-bar"),l=document.getElementById("mobile-filter-list");i&&i.addEventListener("click",n),l&&l.addEventListener("click",n);const d=u=>{const c=document.getElementById(u);c&&c.addEventListener("change",()=>{if(U==="birthdays"||U==="inactive"){const m=document.getElementById("clientSearchInput").value,b=xt(m,U);ht(b,Se.length)}})};d("birthMonthFilter"),d("mobileBirthMonthFilter"),d("inactiveDaysFilter"),d("mobileInactiveDaysFilter"),Ut.addEventListener("click",async u=>{const c=u.target.closest("[data-action]");ia==="list"&&c&&c.dataset.action==="new-client"&&No(null)}),Ut.addEventListener("input",u=>{if(u.target.id==="clientSearchInput"){const c=u.target.value,m=xt(c,U);ht(m,Se.length)}})}const Ye=()=>y("/api/financial/natures"),ni=e=>y("/api/financial/natures",{method:"POST",body:JSON.stringify(e)}),ii=e=>y(`/api/financial/natures/${e}`,{method:"DELETE"}),Xe=()=>y("/api/financial/cost-centers"),li=e=>y("/api/financial/cost-centers",{method:"POST",body:JSON.stringify(e)}),di=e=>y(`/api/financial/cost-centers/${e}`,{method:"DELETE"}),Fo=(e,t)=>y(`/api/financial/${e}`,{method:"POST",body:JSON.stringify(t)}),Ro=(e,t={})=>{let a=`/api/financial/${e}`;const o=new URLSearchParams;t.startDate&&o.append("startDate",t.startDate),t.endDate&&o.append("endDate",t.endDate),t.natureId&&o.append("natureId",t.natureId),t.costCenterId&&o.append("costCenterId",t.costCenterId);const r=o.toString();return r&&(a+=`?${r}`),y(a)},jo=(e,t,a)=>y(`/api/financial/${e}/${t}`,{method:"PUT",body:JSON.stringify(a)}),Ho=(e,t)=>y(`/api/financial/${e}/${t}`,{method:"DELETE"}),Oo=(e,t,a)=>y(`/api/financial/${e}/${t}/status`,{method:"PATCH",body:JSON.stringify({status:"paid",paymentDate:a})}),ci=e=>Fo("payables",e),ui=e=>Ro("payables",e),mi=(e,t)=>jo("payables",e,t),pi=e=>Ho("payables",e),gi=(e,t)=>Oo("payables",e,t),bi=e=>Fo("receivables",e),fi=e=>Ro("receivables",e),vi=(e,t)=>jo("receivables",e,t),hi=e=>Ho("receivables",e),xi=(e,t)=>Oo("receivables",e,t),yi=(e,t)=>y(`/api/financial/cash-flow?startDate=${e}&endDate=${t}`),wi=()=>y("/api/financial/today-summary"),be=document.getElementById("content"),Jt={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"},ki={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}},zo=[{id:"personal-data",icon:"M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z",label:"Dados Gerais"},{id:"branding",icon:"M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z",label:"Identidade e Cores"},{id:"booking",icon:"M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z",label:"Agendamento Online"},{id:"working-hours",icon:"M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z",label:"Horário de Funcionamento"},{id:"loyalty",icon:"M5 5a2 2 0 012-2h10a2 2 0 012 2v1h2a1 1 0 011 1v3a1 1 0 01-1 1h-2v1a2 2 0 01-2 2H7a2 2 0 01-2-2v-1H3a1 1 0 01-1-1V7a1 1 0 011-1h2V5z",label:"Plano de Fidelidade"},{id:"financial",icon:"M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z",label:"Integração Financeira"},{id:"change-password",icon:"M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z",label:"Alterar senha"},{id:"change-email",icon:"M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207",label:"Alterar E-mail de Acesso"}];let V=null;function Qa(e,t,a){return new Promise((o,r)=>{const s=new FileReader;s.readAsDataURL(e),s.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const l=document.createElement("canvas");let d=i.width,u=i.height;d>t&&(u*=t/d,d=t),l.width=d,l.height=u,l.getContext("2d").drawImage(i,0,0,d,u);const m=e.type==="image/png"&&t<500?"image/png":"image/jpeg";o(l.toDataURL(m,a))},i.onerror=l=>r(l)},s.onerror=n=>r(n)})}function lt(e,t=null){let a='<option value="">-- Selecione (Opcional) --</option>';const o=n=>{const i=new Map,l=[];return n&&(n.forEach(d=>i.set(d.id,{...d,children:[]})),i.forEach(d=>{d.parentId&&i.has(d.parentId)?i.get(d.parentId).children.push(d):l.push(d)})),l},r=(n,i="")=>{const l=n.id===t?"selected":"";a+=`<option value="${n.id}" ${l}>${i}${n.name}</option>`,n.children.forEach(d=>r(d,i+"— "))};return o(e).forEach(n=>r(n)),a}async function Pe(e,t){const a=t.target.querySelector('button[type="submit"]');a&&(a.disabled=!0,a.textContent="A Salvar...");try{const o=V||await Qe(p.establishmentId),r=[],{ownerName:s,...n}=e;if(s&&s!==p.userName){const l=J.currentUser;l&&r.push(Zo(l,{displayName:s}).then(()=>{p.userName=s}))}const i={...o,...n};if(r.push(ws(p.establishmentId,i)),await Promise.all(r),V=i,g("Sucesso","Definições salvas com sucesso! A página será recarregada para aplicar o novo tema.","success"),n.themeColor)setTimeout(()=>window.location.reload(),1500);else{const l=document.getElementById("panelEstablishmentName");n.name&&l&&(l.textContent=n.name,p.establishmentName=n.name)}}catch(o){g("Erro",`Não foi possível salvar: ${o.message}`,"error")}finally{a&&(a.disabled=!1,a.textContent="Salvar")}}function Si(e,t){t.innerHTML=`
    ...
    `,t.querySelector("#personal-data-form").addEventListener("submit",a=>{a.preventDefault();const o={ownerName:t.querySelector("#ownerName").value,name:t.querySelector("#establishmentName").value,phone:t.querySelector("#establishmentPhone").value,document:t.querySelector("#establishmentCnpjCpf").value,email:t.querySelector("#establishmentEmail").value,address:t.querySelector("#establishmentAddress").value,website:t.querySelector("#establishmentWebsite").value};Pe(o,a)})}function $i(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!o||!r){g("Erro","Preencha todos os campos.","error");return}const s=t.querySelector('button[form="change-email-form"]');s.disabled=!0,s.textContent="A verificar...";try{const n=J.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Go.credential(n.email,r);await Yo(n,i),s.textContent="A enviar link...",await Xo(n,o),s.textContent="A atualizar BD...",await Ss(p.establishmentId,o),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{s.disabled=!1,s.textContent="Salvar Novo E-mail"}})}function Ci(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;const a=t.querySelector("#establishmentTimezone");if(e.timezone)a.value=e.timezone;else try{const s=Intl.DateTimeFormat().resolvedOptions().timeZone;Array.from(a.options).some(i=>i.value===s)?a.value=s:a.value="America/Sao_Paulo"}catch{a.value="America/Sao_Paulo"}const o=t.querySelector("#establishmentWorkingHoursContainer"),r=e.workingHours||{};Object.keys(Jt).forEach(s=>{const n=r[s]||{},i=Jt[s],l=n.active!==!1,d=document.createElement("div");d.className=`day-schedule-card p-4 rounded-lg ${l?"bg-gray-50":"bg-gray-100 disabled"}`,d.innerHTML=`
            ...
            </div>`,o.appendChild(d)}),o.addEventListener("change",s=>{const n=s.target.closest('.day-schedule-card input[type="checkbox"]');n&&n.closest(".day-schedule-card").classList.toggle("disabled",!n.checked)}),t.querySelector("#working-hours-form").addEventListener("submit",s=>{s.preventDefault();const n={};Object.keys(Jt).forEach(l=>{n[l]={active:t.querySelector(`#est-${l}-active`).checked,start:t.querySelector(`#est-${l}-start`).value,end:t.querySelector(`#est-${l}-end`).value,breakStart:t.querySelector(`#est-${l}-breakStart`).value,breakEnd:t.querySelector(`#est-${l}-breakEnd`).value}});const i=t.querySelector("#establishmentTimezone").value;Pe({workingHours:n,timezone:i},s)})}function Ti(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
             <div class="flex justify-between items-center mb-6">
    ...
    `;try{const[a,o]=await Promise.all([Ye(),Xe()]),r=e.financialIntegration||{},s=e.commissionConfig||{};t.querySelector("#financialNatureId").innerHTML=lt(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=lt(o,r.defaultCentroDeCustoId),t.querySelector("#commissionNatureId").innerHTML=lt(a,s.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=lt(o,s.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const o={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};Pe(o,a)})}function Di(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <h3 class="text-xl font-bold text-gray-800">${e}</h3>
            <p class="mt-4 text-gray-500">Esta secção ainda não foi implementada.</p>
        </div>
    `}function Vo(e="indigo",t){const a=t.querySelector("#color-palette-container"),o=t.querySelector("#establishmentThemeColor");!a||!o||(a.innerHTML="",Object.entries(ki).forEach(([r,s])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{o.value=r,Vo(r,t)}),a.appendChild(i)}),o.value=e)}function Mi(e,t){const a=t.querySelector("#slotIntervalContainer"),o=t.querySelector("#establishmentSlotInterval");if(!a||!o)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(s=>{const n=s.value===e;return`<button type="button" data-value="${s.value}" 
    ...
    `,be.querySelector("#settings-menu-list").addEventListener("click",o=>{const r=o.target.closest("button[data-section]");r&&(o.preventDefault(),Pi(r.dataset.section))});const a=be.querySelector('[data-action="go-to-my-profile"]');a&&a.addEventListener("click",o=>{o.preventDefault(),Y("my-profile-section")})}const Ve=document.getElementById("content");async function Ce(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,o=document.getElementById("filterEndDate")?.value,r=await Bt(p.establishmentId,a||new Date().toISOString().split("T")[0],o||new Date().toISOString().split("T")[0],e),s=document.getElementById("filterReason")?.value.toLowerCase(),n=s?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(s)):r,i=n.reduce((l,d)=>{const u=d.reason||"Sem motivo";return l[u]||(l[u]=[]),l[u].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const u=document.createElement("div");u.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let c=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <h4 class="font-bold text-gray-700">${l} (${d.length})</h4>`;if(d.length>1){const m=JSON.stringify(d.map(b=>b.id));c+=`<button data-action="batch-delete-blockage" data-ids='${m}' class="text-xs text-red-600 hover:text-red-800 font-semibold flex items-center gap-1">
                                    Apagar Lote
                                </button>`}c+="</div>",u.innerHTML=c,d.forEach(m=>{const b=new Date(m.startTime),f=new Date(m.endTime),v=b.toLocaleDateString("pt-BR"),w=f.toLocaleDateString("pt-BR"),k=`
                    <div class="bg-white p-3 rounded-md flex items-center justify-between shadow-sm">
                        <div>
                            <p class="font-medium text-gray-800 text-sm">
                                ${v===w?`${v} | ${b.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`:`De ${v} às ${b.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}<br>Até ${w} às ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`}
                            </p>
                        </div>
        ...
        </section>`,Ni(t),await Ce(t);const o=document.getElementById("batchProfSelectionContainer");try{const r=await _(p.establishmentId);o.innerHTML=r.map(s=>`
            <div class="flex items-center">
    ...
    `}).join("")}function yt(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=p.users:t=p.users.filter(a=>a.status==="active"),_i(t)}function Wi(e={}){return Object.entries(Ui).map(([t,a])=>{const o=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,s=Object.entries(Ji).map(([i,l])=>`
    ...
    `}).join("")}async function Za(e=null){document.getElementById("user-list-view").classList.add("hidden");const t=document.getElementById("user-form-view");t.classList.remove("hidden");let a=p.professionals;if(!a||a.length===0)try{a=await _(p.establishmentId),p.professionals=a}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error")}const o=h=>a.find(k=>k.id===h),r=(h,k)=>{const E=o(h)?.photo,I=k.charAt(0).toUpperCase();return{photoSrc:E||`https://placehold.co/128x128/E2E8F0/4A5568?text=${I}`,initials:I,photoUrl:E||""}},s=e?.professionalId,n=e?.name||"Novo Usuário",i=r(s,n),l=o(s),d=h=>{let k='<option value="">-- Não Associado a um Profissional --</option>';return k+=a.map($=>`<option value="${$.id}" ${$.id===h?"selected":""}>${$.name} (${$.specialty||"N/A"})</option>`).join(""),k},u=e!==null;t.querySelector("#userFormTitle").textContent=u?`Editar Usuário: ${e.name}`:"Novo Usuário";const c=t.querySelector("#userForm");c.innerHTML=`
    ...
    `,qe&&me.removeEventListener("click",qe),Ne&&me.removeEventListener("change",Ne),qe=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("click",qe);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":Za();break;case"edit-user":const o=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));Za(o);break;case"back-to-list":St();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await A("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await Oi(r),g("Usuário excluído com sucesso!","success"),St()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Ne=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("change",Ne);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")yt();else if(t){e.stopPropagation();const a=t.dataset.userId,o=t.checked?"active":"inactive";try{await Vi(a,o),g(`Usuário ${o==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=p.users.findIndex(s=>s.id===a);r>-1&&(p.users[r].status=o,yt())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,yt()}}},me.addEventListener("click",qe),me.addEventListener("change",Ne),await Gi()}const Yi=document.getElementById("content");let Ka={},ca=null;function Xi(){Object.values(Ka).forEach(e=>e?.destroy()),Ka={}}function Qi(e,t){const{jsPDF:a}=window.jspdf,o=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(o.setFontSize(18),o.text(e,o.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];o.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const s=o.lastAutoTable?o.lastAutoTable.finalY+20:60;o.text("Detalhes das Vendas",20,s),o.autoTable({html:`#${t}`,startY:s+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),o.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function eo(e){const t=document.getElementById("genericModal"),a=(e.payments||[]).map(o=>`
    ...
    `,t.style.display="flex"}function Zi(e){const{summary:t,transactions:a}=e;document.getElementById("summary-revenue").textContent=`R$ ${t.totalRevenue.toFixed(2)}`,document.getElementById("summary-transactions").textContent=t.totalTransactions,document.getElementById("summary-avg-ticket").textContent=`R$ ${t.averageTicket.toFixed(2)}`;const o=document.getElementById("paymentSummaryTableBody"),r=Object.entries(t.paymentMethodTotals).sort(([,i],[,l])=>l-i);o.innerHTML=r.map(([i,l])=>`
        <tr class="border-b">
    ...
    `).join(""),s.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=ca.transactions[l];d&&eo(d)})}),n.innerHTML=a.map((i,l)=>`
    ...
    `).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const l=i.dataset.transactionIndex,d=ca.transactions[l];d&&eo(d)})})}async function to(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const o=t.value,r=a.value;if(!o||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const s=document.getElementById("cashierSessionFilter").value,n=await Hs({establishmentId:p.establishmentId,startDate:o,endDate:r,cashierSessionId:s});ca=n,e.innerHTML=`
        ...
        `,Zi(n)}catch(s){g("Erro",`Não foi possível carregar o relatório: ${s.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${s.message}</p>`}}async function Ki(){Xi();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Yi.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",to),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const o=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,s=`Relatorio_Vendas_${o}_a_${r}`;Qi(s,"transactionsTable")});try{const o=await Ir(),r=document.getElementById("cashierSessionFilter");o.forEach(s=>{const n=new Date(s.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=s.closedByName||"N/A";r.innerHTML+=`<option value="${s.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await to()}const el=document.getElementById("content");let C={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},_t=null,dt=null,ct=null;function Ia(e){const t=new Map,a=[];return e&&(e.forEach(o=>t.set(o.id,{...o,children:[]})),t.forEach(o=>{o.parentId&&t.has(o.parentId)?t.get(o.parentId).children.push(o):a.push(o)})),a}function Jo(e,t,a){if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const o=(r,s=0)=>{const n="— ".repeat(s);return`
        ...
        </div>`,t.style.display="flex";const n=t.querySelector("#hierarchyList"),i=t.querySelector("#itemParent"),l=u=>{const c=Ia(u);Jo(n,c,e),i.innerHTML='<option value="">-- Nível Principal --</option>';const m=(b,f="",v=0)=>{const w=v>0?"— ".repeat(v):"";i.innerHTML+=`<option value="${b.id}">${w}${b.name}</option>`,b.children.forEach(h=>m(h,f+"— "))};c.forEach(b=>m(b))},d=await r();C[s]=d,l(d),t.querySelector("#hierarchyForm").addEventListener("submit",async u=>{u.preventDefault();const c=t.querySelector("#itemName").value,m=i.value,b=a?ni:li;try{await b({name:c,parentId:m||null});const f=await r();C[s]=f,l(f),t.querySelector("#hierarchyForm").reset(),await ve()}catch(f){g("Erro",`Não foi possível criar: ${f.message}`,"error")}})}function tl(e){const t=document.getElementById("cashFlowChart");if(!t)return;const a=t.getContext("2d");_t&&_t.destroy();const o=e.payables.map(r=>r*-1);_t=new Chart(a,{type:"bar",data:{labels:e.labels,datasets:[{label:"Receitas",data:e.receivables,backgroundColor:"rgba(74, 222, 128, 0.6)",borderColor:"rgba(34, 197, 94, 1)",borderWidth:1,yAxisID:"y"},{label:"Despesas",data:o,backgroundColor:"rgba(248, 113, 113, 0.6)",borderColor:"rgba(239, 68, 68, 1)",borderWidth:1,yAxisID:"y"},{label:"Saldo Acumulado",data:e.expectedBalance,type:"line",borderColor:"rgba(59, 130, 246, 1)",backgroundColor:"rgba(59, 130, 246, 0.2)",borderWidth:3,pointRadius:4,pointBackgroundColor:"rgba(59, 130, 246, 1)",fill:!0,tension:.1,yAxisID:"y1"}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{x:{stacked:!0},y:{type:"linear",display:!0,position:"left",stacked:!0,title:{display:!0,text:"Movimentações (R$)"}},y1:{type:"linear",display:!0,position:"right",title:{display:!0,text:"Saldo Acumulado (R$)"},grid:{drawOnChartArea:!1}}},plugins:{tooltip:{callbacks:{label:function(r){let s=r.dataset.label||"";if(s&&(s+=": "),r.parsed.y!==null){const n=Math.abs(r.parsed.y);s+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(n)}return s}}}}}})}async function oo(){const e=document.getElementById("cash-flow-chart-container"),t=document.getElementById("cashFlowStartDate").value,a=document.getElementById("cashFlowEndDate").value;if(!t||!a){g("Atenção","Por favor, selecione as datas de início e fim.","error");return}e.innerHTML='<div class="loader mx-auto my-10"></div>';try{const o=await yi(t,a);e.innerHTML='<canvas id="cashFlowChart"></canvas>',tl(o)}catch(o){e.innerHTML=`<p class="text-red-500 text-center">Erro ao carregar dados do gráfico: ${o.message}</p>`}}function so(){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const e=document.getElementById("genericModal"),t=new Date,a=new Date(t.getFullYear(),t.getMonth(),1).toISOString().split("T")[0],o=t.toISOString().split("T")[0];e.innerHTML=`
    ...
    `,e.style.display="flex",e.querySelector("#generateCashFlowBtn").addEventListener("click",oo),oo()}function al(){const e=document.getElementById("genericModal"),t=C.payables.filter(c=>c.status==="pending").reduce((c,m)=>c+m.amount,0),a=C.receivables.filter(c=>c.status==="pending").reduce((c,m)=>c+m.amount,0),o=a-t,r=C.payables.filter(c=>c.status==="paid").reduce((c,m)=>c+m.amount,0),s=C.receivables.filter(c=>c.status==="paid").reduce((c,m)=>c+m.amount,0),n=s-r,i=C.previousBalance||0,l=i+n,d=c=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(c),u=c=>c>=0?"text-green-600":"text-red-600";e.innerHTML=`
                ...
                <div class="bg-gray-50 p-4 rounded-lg shadow-inner">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                        ...
                        <div class="bg-white p-3 rounded-lg shadow-lg border-b-4 ${u(o)==="text-green-600"?"border-green-600":"border-red-600"}">
                            ...
                            <p class="text-2xl font-bold ${u(o)}">${d(o)}</p>
                        </div>
                    </div>
                </div>
    ...
    `,e.style.display="flex"}function ut(e,t="all"){let a='<option value="all">Todos</option>';const o=n=>{const i=new Map,l=[];return n&&(n.forEach(d=>i.set(d.id,{...d,children:[]})),i.forEach(d=>{d.parentId&&i.has(d.parentId)?i.get(d.parentId).children.push(d):l.push(d)})),l},r=(n,i=0)=>{const l=i>0?"— ".repeat(i):"",d=n.id===t?"selected":"";a+=`<option value="${n.id}" ${d}>${l}${n.name}</option>`,n.children.forEach(u=>r(u,i+1))};return o(e).forEach(n=>r(n)),a}async function ve(){const e=document.getElementById("financial-content"),t=document.getElementById("filterStartDate")?.value,a=document.getElementById("filterEndDate")?.value,o=document.getElementById("filterNaturezaId")?.value,r=document.getElementById("filterCostCenterId")?.value;if(!t||!a){try{const[i,l]=await Promise.all([Ye(),Xe()]);C={...C,natures:i,costCenters:l},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=ut(C.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=ut(C.costCenters))}catch(i){g("Erro",`Não foi possível carregar os dados base: ${i.message}`,"error")}ua(),no();return}const s=document.getElementById("payables-list"),n=document.getElementById("receivables-list");s&&(s.innerHTML='<div class="loader mx-auto"></div>'),n&&(n.innerHTML='<div class="loader mx-auto"></div>');try{const i={startDate:t,endDate:a};o&&o!=="all"&&(i.natureId=o),r&&r!=="all"&&(i.costCenterId=r);const[l,d,u,c]=await Promise.all([ui(i),fi(i),Ye(),Xe()]),m=d.previousBalance-l.previousBalance;C={...C,payables:l.entries,receivables:d.entries,natures:u,costCenters:c,previousBalance:m,filterNaturezaId:o,filterCostCenterId:r},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=ut(C.natures,C.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=ut(C.costCenters,C.filterCostCenterId)),ua(),no()}catch(i){g("Erro",`Não foi possível carregar os dados: ${i.message}`,"error"),e&&(e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function sl(e,t,a=null){e.preventDefault();const o=e.target,r=o.querySelector('[name="status"]').checked,s=o.querySelector('[name="paymentDate"]').value,n=parseFloat(o.querySelector('[name="amount"]').value),i=parseInt(o.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!s){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const l={description:o.querySelector('[name="description"]').value,amount:n,dueDate:o.querySelector('[name="dueDate"]').value,naturezaId:o.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:o.querySelector('[name="centroDeCustoId"]').value||null,notes:o.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?s:null,installments:a?1:i};try{a?(await(t==="payable"?mi(a,l):vi(a,l)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?ci(l):bi(l)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await ve()}catch(d){g("Erro",`Não foi possível salvar: ${d.message}`,"error")}}async function rl(e,t){if(await A("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?pi(t):hi(t)),g("Sucesso","Lançamento excluído!","success"),await ve()}catch(o){g("Erro",`Falha ao excluir: ${o.message}`,"error")}}async function nl(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?gi(t,a):xi(t,a)),g("Sucesso","Lançamento atualizado!","success"),await ve()}catch(o){g("Erro",`Falha ao atualizar status: ${o.message}`,"error")}}function ro(e){const t=C.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function ua(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(C.natures.map(i=>[i.id,i.name])),o=new Map(C.costCenters.map(i=>[i.id,i.name])),r=ro(C.payables),s=ro(C.receivables),n=(i,l)=>{const d=i.status==="paid",u=JSON.stringify(i).replace(/'/g,"&apos;"),c=i.naturezaId?a.get(i.naturezaId):"N/A",m=i.centroDeCustoId?o.get(i.centroDeCustoId):"N/A";let b=l==="payable"?"text-red-600":"text-green-600";const f=d?"bg-gray-200 text-gray-600":l==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",v=d?"Finalizado":"Pendente";return d&&(b="text-gray-500"),`
    ...
    `;const r=document.getElementById("main-fab-btn"),s=document.getElementById("fab-menu");if(r&&s){r.addEventListener("click",v=>{v.stopPropagation(),s.classList.toggle("hidden"),r.classList.toggle("rotate-45")});const m=s.querySelector('button[data-action="open-modal"][data-type="receivable"]'),b=s.querySelector('button[data-action="open-modal"][data-type="payable"]'),f=s.querySelector('button[data-action="open-cash-flow-modal"]');m&&m.addEventListener("click",v=>{v.stopPropagation(),Wt("receivable")}),b&&b.addEventListener("click",v=>{v.stopPropagation(),Wt("payable")}),f&&f.addEventListener("click",v=>{v.stopPropagation(),so()})}dt&&document.body.removeEventListener("click",dt),ct&&document.getElementById("genericModal").removeEventListener("click",ct);const n=()=>{const m=document.getElementById("filterStartDate"),b=document.getElementById("filterEndDate"),f=document.getElementById("filterNaturezaId"),v=document.getElementById("filterCostCenterId");C.startDate=m.value,C.endDate=b.value,C.filterNaturezaId=f.value,C.filterCostCenterId=v.value;const w=document.getElementById("advanced-filters");w&&w.classList.contains("hidden")===!1&&window.innerWidth<768&&w.classList.add("hidden"),ve()},i=m=>{const b=m.target.closest("[data-status-filter]");if(!b)return;const f=b.dataset.statusFilter;C.currentFilter=f,document.querySelectorAll("[data-status-filter]").forEach(v=>{v.classList.remove("bg-blue-100","text-blue-800"),v.classList.add("bg-gray-100","text-gray-600")}),b.classList.remove("bg-gray-100","text-gray-600"),b.classList.add("bg-blue-100","text-blue-800"),ua()},l=m=>{const b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("btn-payables-view"),w=document.getElementById("btn-receivables-view");window.innerWidth>=1024&&C.currentListView===m||(m==="payables"?(b.classList.remove("hidden"),f.classList.add("hidden"),v&&(v.classList.remove("bg-gray-200"),v.classList.add("bg-red-100","border","border-red-500")),w&&(w.classList.remove("bg-green-100","border","border-green-500"),w.classList.add("bg-gray-200"))):(b.classList.add("hidden"),f.classList.remove("hidden"),v&&(v.classList.remove("bg-red-100","border","border-red-500"),v.classList.add("bg-gray-200")),w&&(w.classList.remove("bg-gray-200"),w.classList.add("bg-green-100","border","border-green-500"))),C.currentListView=m)};document.getElementById("applyDateFilterBtn").addEventListener("click",n),document.getElementById("filterNaturezaId").addEventListener("change",()=>{C.filterNaturezaId=document.getElementById("filterNaturezaId").value}),document.getElementById("filterCostCenterId").addEventListener("change",()=>{C.filterCostCenterId=document.getElementById("filterCostCenterId").value}),document.querySelectorAll("[data-status-filter]").forEach(m=>{m.addEventListener("click",i)}),dt=m=>{const b=m.target.closest("button[data-action]");if(!b)return;const{action:f,type:v,id:w}=b.dataset;f==="edit"?Wt(v,JSON.parse(b.dataset.item.replace(/&apos;/g,"'"))):f==="delete"?rl(v,w):f==="mark-as-paid"?nl(v,w):f==="manage-natures"?ao("nature"):f==="manage-cost-centers"?ao("cost-center"):f==="open-cash-flow-modal"?so():f==="toggle-filters"?document.getElementById("advanced-filters")?.classList.toggle("hidden"):f==="open-indicators-modal"?al():f==="open-settings-modal"?ol():f==="toggle-list-view"&&l(b.dataset.list)},ct=m=>{const b=m.target.closest('button[data-action^="delete-"]');if(b){const f=b.dataset.action.split("-")[1];d(f,b.dataset.id)}},document.body.addEventListener("click",dt),document.getElementById("genericModal").addEventListener("click",ct);async function d(m,b){const f=m==="nature",v=f?ii:di,w=f?Ye:Xe,h=f?"natures":"costCenters",k=document.getElementById("hierarchyList");if(await A("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await v(b);const E=await w();C[h]=E,Jo(k,Ia(E),m),await ve()}catch(E){g("Erro",`Não foi possível apagar: ${E.message}`,"error")}}const u=()=>{const m=window.innerWidth<1024,b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("list-toggle-buttons");b&&f&&(b.classList.remove("hidden"),f.classList.remove("hidden"),m?(b.classList.remove("lg:col-span-1"),f.classList.remove("lg:col-span-1"),v?.classList.remove("hidden"),l(C.currentListView)):(b.classList.add("lg:col-span-1"),f.classList.add("lg:col-span-1"),v?.classList.add("hidden"),b.classList.remove("hidden"),f.classList.remove("hidden")))};u(),window.addEventListener("resize",u);const c=document.querySelector(`[data-status-filter="${C.currentFilter}"]`);c&&(document.querySelectorAll("[data-status-filter]").forEach(m=>{m.classList.remove("bg-blue-100","text-blue-800"),m.classList.add("bg-gray-100","text-gray-600")}),c.classList.remove("bg-gray-100","text-gray-600"),c.classList.add("bg-blue-100","text-blue-800"));try{const m=await wi(),b=document.getElementById("summary-today-payables");b&&(b.textContent=`R$ ${m.totalPayables.toFixed(2)}`);const f=document.getElementById("summary-today-receivables");f&&(f.textContent=`R$ ${m.totalReceivables.toFixed(2)}`)}catch{g("Erro","Não foi possível carregar o resumo do dia.","error")}await ve()}const ll=e=>y("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),dl=e=>y("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),cl=(e,t)=>{const a=new URLSearchParams({startDate:e,endDate:t}).toString();return y(`/api/commissions/stats?${a}`)},ul=(e={})=>{Object.keys(e).forEach(o=>(e[o]===void 0||e[o]===null||e[o]==="")&&delete e[o]);const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return y(a)},ml=e=>y(`/api/commissions/report/${e}`,{method:"DELETE"}),$t=new Date,io=new Date($t.getFullYear(),$t.getMonth(),1),T={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:io.toISOString().split("T")[0],dashEndDate:$t.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:io.toISOString().split("T")[0],histEndDate:$t.toISOString().split("T")[0],histProfessionalId:"all"};let mt=null;const Fe=document.getElementById("content");async function pl(){try{T.professionals=await _(p.establishmentId)}catch(e){console.error("Erro profissionais",e)}vl(),gl(),Ht(),Et("dashboard")}function gl(){mt&&Fe.removeEventListener("click",mt),mt=e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.action,o=t.dataset.id,r=t.dataset.idx;switch(a){case"tab-nav":Et(t.dataset.tab);break;case"toggle-all-profs":bl();break;case"back-to-filters":T.calculationResult=null,Ct(document.getElementById("commissions-content"));break;case"view-preview-items":wl(r);break;case"save-final-report":kl();break;case"start-new-calc":Et("calculator");break;case"print-receipt":Sl(o);break;case"delete-report":$l(o);break;case"filter-dashboard":Ht();break;case"filter-history":La();break}},Fe.addEventListener("click",mt),Fe.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;xl(t)}},Fe.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),fl())}}async function Ht(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(T.dashStartDate=e.value),t&&(T.dashEndDate=t.value);const a=document.getElementById("dashboard-stats-container");a&&(a.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const o=await cl(T.dashStartDate,T.dashEndDate);T.dashStats={revenue:o.totalRevenue||0,commissions:o.totalCommissionsPaid||0},T.currentTab==="dashboard"&&_o(document.getElementById("commissions-content"))}catch(o){console.error(o),a&&(a.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function La(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),a=document.getElementById("hist-prof");e&&(T.histStartDate=e.value),t&&(T.histEndDate=t.value),a&&(T.histProfessionalId=a.value);const o=document.getElementById("history-list-container");if(o){o.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await ul({startDate:T.histStartDate,endDate:T.histEndDate,professionalId:T.histProfessionalId});T.historyData=r,Wo(o,r)}catch{o.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function bl(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(a=>a.checked);e.forEach(a=>a.checked=!t)}async function fl(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(s=>s.value);if(e.length===0)return g("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},a=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),o=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");T.periodString=`${a} a ${o}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const s=await ll(t);T.calculationResult=s.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),Ct(r)}catch(s){g("Erro",s.message,"error"),T.calculationResult=null,Ct(r)}}function vl(){Fe.innerHTML=`
    ...
    `}function hl(e){const t=T.professionals.map(a=>`<option value="${a.id}" ${T.histProfessionalId===a.id?"selected":""}>${a.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `}function xl(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),a=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let o=0,r=0;if(t.forEach(s=>{s.value&&(o=parseFloat(s.value))}),a.forEach(s=>{s.value&&(r=parseFloat(s.value))}),T.calculationResult&&T.calculationResult[e]){const s=T.calculationResult[e];s.extraDebit=o,s.extraCredit=r,s.finalValue=s.summary.totalCommission-o+r,t.forEach(i=>{i!==document.activeElement&&(i.value=o||"")}),a.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${s.finalValue.toFixed(2)}`),yl()}}function yl(){const e=T.calculationResult.reduce((a,o)=>a+o.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(a=>a.innerText=`R$ ${e.toFixed(2)}`)}function wl(e){const t=T.calculationResult[e];if(!t)return;const a=t.items.map(o=>`
    ...
    `).join("");N({title:"Detalhes da Comissão",contentHTML:`<div class="bg-gray-50 p-3 rounded-lg mb-4 flex justify-between items-center"><div><p class="text-xs text-gray-500">Profissional</p><p class="font-bold text-gray-800">${t.professionalName}</p></div><div class="text-right"><p class="text-xs text-gray-500">Total Itens</p><p class="font-bold text-gray-800">${t.items.length}</p></div></div><div class="border rounded-lg overflow-hidden max-h-[60vh] overflow-y-auto">${a}</div>`,maxWidth:"max-w-md"})}async function kl(){const e=T.calculationResult.length;if(await A("Confirmar",`Gerar ${e} relatórios?`))try{const a=T.calculationResult.map(o=>dl({professionalId:o.professionalId,professionalName:o.professionalName,period:T.periodString,reportData:{...o,summary:{...o.summary,finalValue:o.finalValue,extraDebit:o.extraDebit||0,extraCredit:o.extraCredit||0,notes:o.notes||""}}}));await Promise.all(a),g("Sucesso","Pagamentos registrados!","success"),T.calculationResult=null,Ht(),Et("history")}catch(a){g("Erro",a.message,"error")}}function Sl(e){const t=T.historyData.find(a=>a.id===e);t&&El(t)}async function $l(e){if(await A("Excluir","Deseja remover este registro?"))try{await ml(e),g("Sucesso","Registro removido.","success"),La(),Ht()}catch(a){g("Erro",a.message,"error")}}function El(e){const{jsPDF:t}=window.jspdf;if(!t)return g("Erro","PDF lib não carregada.","error");const a=new t,o=a.internal.pageSize.getWidth()/2;a.setFontSize(18),a.setFont(void 0,"bold"),a.text("RECIBO DE PAGAMENTO DE COMISSÃO",o,20,{align:"center"}),a.setFontSize(12),a.setFont(void 0,"normal"),a.text(`Profissional: ${e.professionalName}`,15,40),a.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),a.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const s=a.lastAutoTable.finalY+10;a.setFontSize(14),a.setFont(void 0,"bold"),a.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,s,{align:"right"}),a.save(`Recibo_${e.professionalName}.pdf`)}function Ct(e){if(T.calculationResult){const t=T.calculationResult,a=t.reduce((s,n)=>s+n.finalValue,0),o=t.map((s,n)=>`
            ...
            </div>`}else{const t=new Date().toISOString().split("T")[0],a=new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],o=T.professionals.map(r=>`
            ...
            </form>`}}const Gt=document.getElementById("content");let he={allPackages:[],catalogForModal:{services:[],products:[]}},pt=null,$e=null;function Cl(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `}).join("")}}function lo(){const e=document.getElementById("genericModal");e.style.display="none",$e&&e.removeEventListener("click",$e)}async function co(e=null){const t=document.getElementById("genericModal"),a=!!e,o=e?JSON.parse(JSON.stringify(e.items||[])):[],r=`
    ...
    `;t.innerHTML=r,t.style.display="flex";const s=t.querySelector("#package-items-list"),n=(l,d)=>{const u=d.querySelector("#originalPrice"),c=l.reduce((m,b)=>m+b.price*b.quantity,0);u&&(u.textContent=`R$ ${c.toFixed(2)}`)},i=l=>{l.length===0?s.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':s.innerHTML=l.map((d,u)=>{const c=d.type==="service",m=c?"Serviço":"Produto",b=c?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
    ...
    `,document.body.appendChild(a);const s=a.querySelector("#item-selection-list"),n=a.querySelector("#item-search-input"),i=()=>{a.remove()};r(s),n.addEventListener("input",()=>{t=n.value,r(s)}),a.addEventListener("click",l=>{const d=l.target.closest('[data-action="select-item"]'),u=l.target.closest('[data-action="close-selection-modal"]');if(d){const{itemType:c,itemId:m}=d.dataset,f=(he.catalogForModal[c+"s"]||[]).find(v=>v.id===m);f&&(e({...f,type:c}),i())}else(u||l.target===a)&&i()})}async function Ta(){Gt.innerHTML=`
    ...
    `).join("")}function Hl(e){Ue&&Ue();const t=De(Z,"establishments",e,"notifications"),a=fo(t,as("timestamp",">=",new Date),vo("timestamp","desc"));Ue=os(a,o=>{o.docChanges().forEach(r=>{if(r.type==="added"){const s=r.doc.data();Je.unshift({title:s.title,message:s.message,time:s.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(s.title,s.message,"info",!0),ma();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(s.type==="cancellation"||s.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),$o())}})},o=>{console.error("Erro no listener de notificações em tempo real:",o),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function Y(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=p.enabledModules?.[a]!==!1,s=p.userPermissions===null||p.userPermissions[e]?.view===!0;if(!r||!s){Xt.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const o=Rl[e];o?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),Xt.innerHTML="",o(t)):(Xt.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function Ol(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),o=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),s=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(s&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!s&&!n))try{const i=await Os();s&&o&&(o.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),s&&o&&(o.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function zl(e){try{_e.getPlatform()==="android"&&(await re.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await re.checkPermissions();if(t.receive==="prompt"&&(t=await re.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await re.register(),re.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const o=Lt(Z,"users",e);await ho(o,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(o){console.error("Erro ao salvar token FCM:",o)}}),re.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),re.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),re.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),Y("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function Vl(){_e.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),bs(),uo.addEventListener("click",e=>{e.stopPropagation(),bt.classList.toggle("hidden"),bt.classList.contains("hidden")||(Je.forEach(t=>t.read=!0),ma())}),Fl.addEventListener("click",()=>{fs()}),Zt.addEventListener("click",e=>{e.stopPropagation(),X.classList.toggle("active"),X.classList.contains("active")?X.classList.remove("hidden"):setTimeout(()=>X.classList.add("hidden"),200)}),po&&po.addEventListener("click",e=>{e.preventDefault(),Y("my-profile-section"),X.classList.remove("active"),X.classList.add("hidden")}),document.addEventListener("click",e=>{!bt.contains(e.target)&&e.target!==uo&&bt.classList.add("hidden"),!X.contains(e.target)&&e.target!==Zt&&X.classList.contains("active")&&(X.classList.remove("active"),setTimeout(()=>X.classList.add("hidden"),200))}),Ko(J,async e=>{if(e)try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const o=await Qe(a.establishmentId);p.enabledModules=o.modules,jl(o.themeColor||"indigo");let r=null,s=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const d=Lt(Z,"users",e.uid),u=await ts(d);if(u.exists()){const c=u.data();r=a.role==="employee"?c.permissions||{}:null,s=c.name||s,n=c.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}p.userProfessionalId=n,zl(e.uid);const i=s||e.email;ss(a.establishmentId,o.name,r),Zt.textContent=i.charAt(0).toUpperCase(),Al.textContent=i,ql.textContent=e.email;const l=()=>{Ue&&Ue(),Ba(J).then(()=>window.location.href="/login.html")};Nl.addEventListener("click",d=>{d.preventDefault(),l()}),ys(Y,r,p.enabledModules),Ol(r),Hl(a.establishmentId),ma(),gt.classList.add("fade-out"),Yt.style.display="flex",setTimeout(()=>{gt.style.display="none"},500),Y("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),gt.classList.add("fade-out"),setTimeout(()=>{gt.style.display="none"},500),Yt.innerHTML=`
                ...
                `,Yt.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{Ba(J).then(()=>window.location.href="/login.html")})}else window.location.href="/login.html"})}Vl();
