...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${s}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${s}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${la}. Verifique se o servidor backend está rodando.`):n}}const Ro=(e,t,a,o=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(r+=`&professionalId=${o}`),w(r)},Js=(e,t,a)=>{const o=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return w(o)},Ws=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return w(r)},Gs=e=>w("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Fo=(e,t)=>w(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ua=e=>w(`/api/appointments/${e}`,{method:"DELETE"}),Ys=e=>w(`/api/appointments/${e}/reopen`,{method:"POST"}),Qs=(e,t)=>w(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let _;async function Xs(){if(!_)try{_=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Zs(){if(!_){console.warn("AudioContext não inicializado. O som não será tocado.");return}_.state==="suspended"&&_.resume();const e=_.createOscillator(),t=_.createGain();e.connect(t),t.connect(_.destination),e.type="sine",e.frequency.setValueAtTime(800,_.currentTime),t.gain.setValueAtTime(0,_.currentTime),t.gain.linearRampToValueAtTime(.3,_.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,_.currentTime+.2),e.start(_.currentTime),e.stop(_.currentTime+.2)}function g(e,t,a="info",o=!1){const r=document.getElementById("toast-container");if(!r)return;o&&Zs();const s=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};s.className=`toast ${n[a]||n.info}`,s.innerHTML=`
    ...
    `,r.appendChild(s),s.querySelector(".toast-close").addEventListener("click",()=>s.remove()),setTimeout(()=>{s.remove()},4e3)}function j(e,t){const a=document.getElementById("genericModal");return new Promise(o=>{a.innerHTML=`
    ...
    `;const l=r.querySelector("[data-close-modal]");l&&(l.onclick=n);const d=r.querySelector('[data-action="close-modal"]');return d&&(d.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function jt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function Ks(){document.body.addEventListener("click",()=>{_||Xs()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const o=t.dataset.target;if(o){const r=document.getElementById(o);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const o=document.getElementById("genericModal");o&&(o.style.display="none")}})}async function _a(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const o=await Js(u.establishmentId,t,a);if(o.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=o.map(r=>`
        ...
        `).join("")}catch(o){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${o.message}</p>`}}function er(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const o=`
    ...
    `,{modalElement:r}=G({title:"Histórico de Cancelamentos",contentHTML:o,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",_a),_a()}const Q=document.getElementById("sidebar"),Ja=document.getElementById("sidebarToggle"),Ge=document.getElementById("mainContent"),tr=document.querySelectorAll(".sidebar-link"),Wa=document.getElementById("hamburger-menu-btn"),Ne=document.getElementById("mobile-overlay");function kt(e){!Q||!Ge||(Q.classList.toggle("collapsed",e),Ge.classList.toggle("sidebar-collapsed-shift",e))}function ar(){!Q||!Ne||(Q.classList.add("mobile-open"),Ne.classList.add("visible"))}function ut(){!Q||!Ne||(Q.classList.remove("mobile-open"),Ne.classList.remove("visible"))}function or(){kt(!Q.classList.contains("collapsed"))}function sr(e,t,a){if(!Q||!Ge)return;Ge.classList.add("main-content-shift"),window.innerWidth>=768?kt(Q.classList.contains("collapsed")):(Ge.classList.remove("main-content-shift","sidebar-collapsed-shift"),ut()),Ja&&Ja.addEventListener("click",r=>{r.stopPropagation(),or()}),Q.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&Q.classList.contains("collapsed")&&kt(!1)}),Q.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||kt(!0))}),Wa&&Wa.addEventListener("click",r=>{r.stopPropagation(),ar()}),Ne&&Ne.addEventListener("click",r=>{r.stopPropagation(),ut()});let o=0;Q.addEventListener("touchstart",r=>{o=r.changedTouches[0].screenX},{passive:!0}),Q.addEventListener("touchend",r=>{const s=r.changedTouches[0].screenX;o-s>50&&ut()},{passive:!0}),tr.forEach(r=>{const s=r.getAttribute("data-target"),n=s.replace("-section",""),i=a?.[n]!==!1,l=t===null||t[s]?.view===!0;if(!i||!l){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",d=>{d.preventDefault(),s&&typeof e=="function"&&e(s),window.innerWidth<768&&ut()})})}const Se=e=>{const t=e||u.establishmentId;return t?w(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},St=(e,t)=>{const a=e||u.establishmentId;return a?w(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},rr=(e,t)=>{const a=e||u.establishmentId;return a?w(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},nr=(e,t)=>{const a=e||u.establishmentId;return a?w(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Z=e=>w(`/api/professionals/${e}`),ir=e=>w(`/api/professionals/details/${e}`),jo=e=>w("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Lt=(e,t)=>w(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ga=(e,t)=>Lt(e,{services:t}),Ho=e=>w(`/api/professionals/${e}`,{method:"DELETE"}),lr=e=>{const t=e.map(a=>Ho(a));return Promise.all(t)},$e=e=>w(`/api/services/${e}`),Oo=e=>w("/api/services",{method:"POST",body:JSON.stringify(e)}),dr=(e,t)=>w(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),cr=e=>w(`/api/services/${e}`,{method:"DELETE"}),ur=(e,t)=>w(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),mr=e=>w(`/api/services/stats/most-popular/${e}`),Ht=e=>w(`/api/products/${e}`),zo=e=>w("/api/products",{method:"POST",body:JSON.stringify(e)}),pr=(e,t)=>w(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),gr=e=>w(`/api/products/${e}`,{method:"DELETE"}),br=(e,t)=>w(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),fr=({startDate:e,endDate:t,productId:a,categoryId:o,establishmentId:r})=>{const s=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&s.append("productId",a),o&&o!=="all"&&s.append("categoryId",o),r&&s.append("establishmentId",r),w(`/api/products/stock-history/report?${s.toString()}`)},vr={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}};function Ya(e,t,a){return new Promise((o,r)=>{const s=new FileReader;s.readAsDataURL(e),s.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const l=document.createElement("canvas");let d=i.width,c=i.height;d>t&&(c*=t/d,d=t),l.width=d,l.height=c,l.getContext("2d").drawImage(i,0,0,d,c);const p=e.type==="image/png"&&t<500?"image/png":"image/jpeg";o(l.toDataURL(p,a))},i.onerror=l=>r(l)},s.onerror=n=>r(n)})}let de=null;const $t=[{id:"company_data",title:"Identidade do Negócio",icon:"🏢",description:"Configure os dados da sua empresa."},{id:"branding",title:"Sua Marca",icon:"🎨",description:"Logo e cores (Opcional)."},{id:"time_config",title:"O Relógio",icon:"⏱️",description:"Tempo padrão entre agendamentos."},{id:"first_service",title:"O Menu",icon:"✂️",description:"Seu principal serviço."},{id:"first_prof",title:"Sua Equipe",icon:"💇",description:"Cadastre o primeiro profissional."},{id:"first_product",title:"O Estoque",icon:"🧴",description:"Cadastre um produto (opcional)."}];let K=0,Tt=[];async function hr(){try{console.log("Iniciando verificação de Onboarding para ID:",u.establishmentId);const e=await Se(u.establishmentId),t=await Z(u.establishmentId),a=await $e(u.establishmentId);Tt=a||[];const o=e&&e.name&&(e.phone||e.address),r=e&&(e.logo||e.themeColor&&e.themeColor!=="indigo"),s=e&&e.slotInterval>0,n=a&&a.length>0,i=t&&t.length>0;if(console.log("Status Onboarding:",{hasCompanyData:o,hasBranding:r,hasTimeConfig:s,hasService:n,hasProf:i}),o&&s&&i&&n)return;if(!o)K=0;else if(!r&&!s)K=1;else if(!s)K=2;else if(!n)K=3;else if(!i)K=4;else if(K===0)return;xr(),$a(K)}catch(e){console.error("Erro ao verificar onboarding:",e)}}function xr(){document.getElementById("onboarding-overlay")||(de=document.createElement("div"),de.id="onboarding-overlay",de.className="fixed inset-0 bg-gray-900 bg-opacity-95 z-[9999] flex items-center justify-center p-4 overflow-y-auto",de.style.cssText="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(17, 24, 39, 0.95); z-index: 9999; display: flex; align-items: center; justify-content: center;",de.innerHTML=`
    ...
    `,document.body.appendChild(de),Sa())}function Sa(){const e=Math.round(K/$t.length*100),t=document.getElementById("progress-bar"),a=document.getElementById("progress-text");t&&(t.style.width=`${e}%`),a&&(a.innerText=`${e}%`)}function $a(e){const t=document.getElementById("onboarding-step-content"),a=$t[e];if(!a){Qa(t);return}let o="";if(a.id==="company_data")o=`
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="themeColor" class="mt-1 w-full p-2 border rounded text-sm bg-white">
                            ${Object.entries(vr).map(([s,n])=>`<option value="${s}">${n.name}</option>`).join("")}
                        </select>
                    </div>
                    <div>
        ...
        `;else if(a.id==="first_prof"){const r=Tt.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""),s=Tt.length>0;o=`
            <form id="step-form" class="space-y-3">
    ...
    `,document.getElementById("next-step-btn").addEventListener("click",()=>yr(a.id)),document.getElementById("skip-btn")&&document.getElementById("skip-btn").addEventListener("click",()=>{e===$t.length-1?Qa(t):(K++,Sa(),$a(K))}),a.id==="branding"){const r=document.getElementById("logo-input"),s=document.getElementById("bg-input");r&&(r.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Ya(i,200,.8);document.getElementById("logo-base64").value=l,document.getElementById("logo-preview").innerHTML=`<img src="${l}" class="w-full h-full object-contain rounded">`}catch(l){console.error("Erro logo",l)}}),s&&(s.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Ya(i,1024,.7);document.getElementById("bg-base64").value=l}catch(l){console.error("Erro bg",l)}})}}function Qa(e){e.innerHTML=`
    ...
    `;const t=document.getElementById("progress-bar"),a=document.getElementById("progress-text");t&&(t.style.width="100%"),a&&(a.innerText="100%"),document.getElementById("finish-onboarding-btn").onclick=()=>{de&&de.remove(),window.location.reload()}}async function yr(e){const t=document.getElementById("step-form");if(!t.reportValidity())return;const a=document.getElementById("next-step-btn"),o=a.innerHTML;a.disabled=!0,a.innerHTML="Salvando...";const r=new FormData(t),s=Object.fromEntries(r.entries());try{if(e==="company_data")await St(u.establishmentId,{name:s.name,phone:s.phone,email:s.email,address:s.address,zipCode:s.zipCode});else if(e==="branding"){const n={};s.logoBase64&&(n.logo=s.logoBase64),s.bgBase64&&(n.backgroundImage=s.bgBase64),s.themeColor&&(n.themeColor=s.themeColor),s.primaryColor&&(n.primaryColor=s.primaryColor),Object.keys(n).length>0&&await St(u.establishmentId,n)}else if(e==="time_config"){const n=parseInt(s.slotInterval);await St(u.establishmentId,{slotInterval:n})}else if(e==="first_service"){const n=await Oo({establishmentId:u.establishmentId,name:s.name,price:parseFloat(s.price),duration:parseInt(s.duration),active:!0});n&&Tt.push(n)}else if(e==="first_prof"){const n=await jo({establishmentId:u.establishmentId,name:s.name,specialty:s.role,active:!0,commissionRate:0});if(s.serviceId&&n&&n.id)try{Ga?await Ga(n.id,[s.serviceId]):Lt&&await Lt(n.id,{services:[s.serviceId]})}catch(i){console.warn("Não foi possível vincular o serviço automaticamente.",i)}}else e==="first_product"&&await zo({establishmentId:u.establishmentId,name:s.name,price:parseFloat(s.salePrice),stock:parseInt(s.stock),active:!0});g("Sucesso","Passo concluído!","success"),K++,Sa(),$a(K)}catch(n){g("Erro","Erro ao salvar: "+n.message,"error"),a.disabled=!1,a.innerHTML=o}}var qe;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(qe||(qe={}));class Qt extends Error{constructor(t,a,o){super(t),this.message=t,this.code=a,this.data=o}}const wr=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},kr=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},o=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:wr(e),s=()=>r()!=="web",n=m=>{const p=d.get(m);return!!(p?.platforms.has(r())||i(m))},i=m=>{var p;return(p=a.PluginHeaders)===null||p===void 0?void 0:p.find(b=>b.name===m)},l=m=>e.console.error(m),d=new Map,c=(m,p={})=>{const b=d.get(m);if(b)return console.warn(`Capacitor plugin "${m}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(m);let x;const k=async()=>(!x&&f in p?x=typeof p[f]=="function"?x=await p[f]():x=p[f]:t!==null&&!x&&"web"in p&&(x=typeof p.web=="function"?x=await p.web():x=p.web),x),C=(L,B)=>{var N,F;if(v){const z=v?.methods.find(A=>B===A.name);if(z)return z.rtype==="promise"?A=>a.nativePromise(m,B.toString(),A):(A,Y)=>a.nativeCallback(m,B.toString(),A,Y);if(L)return(N=L[B])===null||N===void 0?void 0:N.bind(L)}else{if(L)return(F=L[B])===null||F===void 0?void 0:F.bind(L);throw new Qt(`"${m}" plugin is not implemented on ${f}`,qe.Unimplemented)}},S=L=>{let B;const N=(...F)=>{const z=k().then(A=>{const Y=C(A,L);if(Y){const Ee=Y(...F);return B=Ee?.remove,Ee}else throw new Qt(`"${m}.${L}()" is not implemented on ${f}`,qe.Unimplemented)});return L==="addListener"&&(z.remove=async()=>B()),z};return N.toString=()=>`${L.toString()}() { [capacitor code] }`,Object.defineProperty(N,"name",{value:L,writable:!1,configurable:!1}),N},E=S("addListener"),T=S("removeListener"),R=(L,B)=>{const N=E({eventName:L},B),F=async()=>{const A=await N;T({eventName:L,callbackId:A},B)},z=new Promise(A=>N.then(()=>A({remove:F})));return z.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await F()},z},O=new Proxy({},{get(L,B){switch(B){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?R:E;case"removeListener":return T;default:return S(B)}}});return o[m]=O,d.set(m,{name:m,proxy:O,platforms:new Set([...Object.keys(p),...v?[f]:[]])}),O};return a.convertFileSrc||(a.convertFileSrc=m=>m),a.getPlatform=r,a.handleError=l,a.isNativePlatform=s,a.isPluginAvailable=n,a.registerPlugin=c,a.Exception=Qt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Sr=e=>e.Capacitor=kr(e),he=Sr(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),Ea=he.registerPlugin;class Vo{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let o=!1;this.listeners[t]||(this.listeners[t]=[],o=!0),this.listeners[t].push(a);const s=this.windowListeners[t];s&&!s.registered&&this.addWindowListener(s),o&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,o){const r=this.listeners[t];if(!r){if(o){let s=this.retainedEventArguments[t];s||(s=[]),s.push(a),this.retainedEventArguments[t]=s}return}r.forEach(s=>s(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:o=>{this.notifyListeners(a,o)}}}unimplemented(t="not implemented"){return new he.Exception(t,qe.Unimplemented)}unavailable(t="not available"){return new he.Exception(t,qe.Unavailable)}async removeListener(t,a){const o=this.listeners[t];if(!o)return;const r=o.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(o=>{this.notifyListeners(t,o)}))}}const Xa=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Za=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class $r extends Vo{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(o=>{if(o.length<=0)return;let[r,s]=o.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Za(r).trim(),s=Za(s).trim(),a[r]=s}),a}async setCookie(t){try{const a=Xa(t.key),o=Xa(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,s=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${o||""}${r}; path=${s}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}Ea("CapacitorCookies",{web:()=>new $r});const Er=async e=>new Promise((t,a)=>{const o=new FileReader;o.onload=()=>{const r=o.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},o.onerror=r=>a(r),o.readAsDataURL(e)}),Ir=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,s,n)=>(r[s]=e[t[n]],r),{})},Cr=(e,t=!0)=>e?Object.entries(e).reduce((o,r)=>{const[s,n]=r;let i,l;return Array.isArray(n)?(l="",n.forEach(d=>{i=t?encodeURIComponent(d):d,l+=`${s}=${i}&`}),l.slice(0,-1)):(i=t?encodeURIComponent(n):n,l=`${s}=${i}`),`${o}&${l}`},"").substr(1):null,Lr=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Ir(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const s=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))s.set(n,i);a.body=s.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const s=new FormData;if(e.data instanceof FormData)e.data.forEach((i,l)=>{s.append(l,i)});else for(const i of Object.keys(e.data))s.append(i,e.data[i]);a.body=s;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Tr extends Vo{async request(t){const a=Lr(t,t.webFetchExtra),o=Cr(t.params,t.shouldEncodeUrlParams),r=o?`${t.url}?${o}`:t.url,s=await fetch(r,a),n=s.headers.get("content-type")||"";let{responseType:i="text"}=s.ok?t:{};n.includes("application/json")&&(i="json");let l,d;switch(i){case"arraybuffer":case"blob":d=await s.blob(),l=await Er(d);break;case"json":l=await s.json();break;case"document":case"text":default:l=await s.text()}const c={};return s.headers.forEach((m,p)=>{c[p]=m}),{data:l,headers:c,status:s.status,url:s.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}Ea("CapacitorHttp",{web:()=>new Tr});const W=Ea("PushNotifications",{}),Pr="BDA-IaH_jjWBRwHbuFcB56I25jKHpchx34yZtv_6iIo_yV2tz_yIZYS3hfntDaN5Slf4ch8ZEJCIt4D7LIWX4mY";let Ka=!1;async function Uo(){if(window.Capacitor&&window.Capacitor.isNativePlatform()){try{await W.removeAllListeners(),await W.addListener("registration",async a=>{Wo(a.value,!0)}),await W.addListener("pushNotificationReceived",a=>{console.log("[Push Nativo] Recebido em Foreground (Visual gerido pelo Firestore):",a)}),await W.addListener("pushNotificationActionPerformed",a=>{const o=a.notification.data;console.log("Notificação clicada (Ação):",o)});let t=await W.checkPermissions();t.receive==="prompt"&&(t=await W.requestPermissions()),t.receive==="granted"&&await W.register()}catch(t){console.error("[Push Nativo] Erro:",t)}return}"Notification"in window&&Notification.permission==="granted"&&Jo()}async function _o(){try{return await Notification.requestPermission()==="granted"?(console.log("Permissão concedida pelo utilizador."),await Jo(),!0):(console.warn("Permissão de notificações negada."),!1)}catch(e){return console.error("Erro ao pedir permissão Web:",e),!1}}async function Jo(){if("serviceWorker"in navigator)try{const e=await navigator.serviceWorker.register("/firebase-messaging-sw.js");await e.update();const t=await zs(Oa,{vapidKey:Pr,serviceWorkerRegistration:e});t?(console.log("[Push Web] Token validado."),await Wo(t,!1)):console.warn("[Push Web] Token veio vazio."),Ka||(Vs(Oa,a=>{console.log("[Push Web] Recebido em Foreground (Silencioso):",a)}),Ka=!0)}catch(e){console.error("[Push Web] Falha no registo:",e)}else console.warn("Navegador sem suporte a Service Worker.")}async function Wo(e,t){const a=V.currentUser;if(!a){console.warn("Usuário não logado. Token não salvo.");return}const o=ce(U,"users",a.uid);try{const r=await Do(o);if(r.exists()){const n=r.data().fcmTokens||[];if(n.length===1&&n[0]===e){console.log("Token já sincronizado e único. Nenhuma ação necessária.");return}}await wa(o,{fcmTokens:[e],lastLoginAt:new Date().toISOString(),platform:t?"android_native":"pwa_web"}),console.log("Token atualizado (Sessão Única garantida).")}catch(r){if(r.code==="not-found")try{await Fs(o,{email:a.email,fcmTokens:[e],platform:t?"android_native":"pwa_web",createdAt:new Date().toISOString()},{merge:!0}),console.log("Utilizador criado e token inicial salvo.")}catch(s){console.error("Erro ao criar user:",s)}else console.error("Erro ao atualizar token:",r)}}const Br=(e,t,a="all",o="all")=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("professionalId",a),o&&o!=="all"&&r.append("costCenterId",o),w(`/api/reports/indicators?${r.toString()}`)},Dr=e=>e?w(`/api/financial/cost-centers/${e}`):Promise.resolve([]),Mr=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:o})=>{const r=new URLSearchParams({startDate:t,endDate:a});return o&&o!=="all"&&r.append("cashierSessionId",o),e&&r.append("establishmentId",e),w(`/api/reports/sales?${r.toString()}`)},Ar=()=>w("/api/reports/summary",{method:"GET"}),Ot=(e,t,a,o="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${o}`;return w(r)},zt=e=>w("/api/blockages",{method:"POST",body:JSON.stringify(e)}),Ia=e=>w(`/api/blockages/${e}`,{method:"DELETE"}),Go=e=>w("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),Ca=e=>e?String(e).replace(/\D/g,""):"",nt=(e,t="",a=20,o={})=>{const r=new URLSearchParams;return t&&r.append("search",t),a&&r.append("limit",a),o.hasLoyalty&&r.append("hasLoyalty","true"),o.birthMonth&&r.append("birthMonth",o.birthMonth),o.inactiveDays&&r.append("inactiveDays",o.inactiveDays),w(`/api/clients/${e}?${r.toString()}`)},da=(e,t)=>{const a=encodeURIComponent(t);return w(`/api/clients/details/${e}/${a}`)},Nr=e=>{if(!e.phone)throw new Error("Telefone é obrigatório");const t=Ca(e.phone),a={...e,phone:t,id:t};return w(`/api/clients/${t}`,{method:"PUT",body:JSON.stringify(a)})},qr=(e,t)=>{const a=Ca(t);return w(`/api/clients/full-history/${e}?phone=${a}`)},Yo=e=>{const t=encodeURIComponent(e);return w(`/api/clients/${t}`,{method:"DELETE"})},Rr=(e,t,a,o)=>w("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientPhone:Ca(t),points:a,rewardName:o})});function h(e){return e==null?"":String(e).replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}function Qo(e,t=800,a=800,o=.7){return new Promise((r,s)=>{if(!e.type.match(/image.*/))return s(new Error("O ficheiro selecionado não é uma imagem."));const n=new FileReader;n.readAsDataURL(e),n.onload=i=>{const l=new Image;l.src=i.target.result,l.onload=()=>{let d=l.width,c=l.height;d>c?d>t&&(c*=t/d,d=t):c>a&&(d*=a/c,c=a);const m=document.createElement("canvas");m.width=d,m.height=c,m.getContext("2d").drawImage(l,0,0,d,c);const b=m.toDataURL("image/jpeg",o);r(b)},l.onerror=d=>s(new Error("Erro ao carregar a imagem para processamento."))},n.onerror=i=>s(new Error("Erro ao ler o ficheiro."))})}const eo=document.getElementById("content");let to=!1;const ca=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let it=[],Vt=[],Re={},Me=[],I={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null,isSelectionMode:!1,selectedItems:new Set},$={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Fr(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function Xo(e){const t=new Date(e);if(t.setHours(0,0,0,0),I.currentView==="week"&&I.weekViewDays===7){const a=t.getDay(),o=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(o))}return t}function Pt(){const e=document.getElementById("profSelectorContainer"),t=I.profSearchTerm.toLowerCase();if(!e||!u.professionals)return;let a=u.professionals.filter(s=>I.showInactiveProfs||s.status!=="inactive");t&&(a=a.filter(s=>s.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(s=>{const n=I.selectedProfessionalId===s.id,i=s.name==="Todos"?"Todos":s.name.split(" ")[0],l=s.name==="Todos"?"T":s.name.charAt(0).toUpperCase(),d=s.status!=="inactive",c=h(i),m=ca[0],p=s.id!=="all"&&u.professionalColors.get(s.id)||m,b=s.photo||`https://placehold.co/64x64/${p.main?.replace("#","")||"E0E7FF"}/${p.light?.replace("#","")||"4F46E5"}?text=${l}`,f=s.id==="all"?"#e0e7ff":p.light,v=s.id==="all"?"#4f46e5":p.main,k=`border: 3px solid ${n?p.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+p.border:"none"};`;return`
            <div class="prof-card ${n?"selected":""} ${d?"":"opacity-50"}" 
                 data-action="select-professional" 
                 data-prof-id="${s.id}">
                ${s.id==="all"?`<div class="prof-card-all-placeholder" style="background-color: ${f}; color: ${v}; ${k}">
                           ${l}
                          </div>`:`<img src="${b}" alt="${c}" class="prof-card-photo" style="${k}" />`}
                <span class="prof-card-name">${c}</span>
            </div>
        `}).join("")}function jr(e,t,a,o,r){const s=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${o} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,d=encodeURIComponent(l);return`https://wa.me/${s}?text=${d}`}function Hr(e){const t=document.getElementById("agenda-view");if(!t)return;if(e.sort((o,r)=>new Date(o.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(o=>{const r=new Date(o.startTime),s=new Date(o.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=s.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=u.professionalColors.get(o.professionalId)||{},d=h(o.reason),c=h(o.professionalName),m=h(o.clientName),p=h(o.serviceName),b=I.selectedItems.has(o.id),f=I.isSelectionMode?`<div class="flex items-center justify-center pr-3 border-r border-gray-200 mr-3">
                ...
                </div>`;const v=o.status==="completed",x=v?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",k=v?"Finalizado":"Aberto",C=JSON.stringify(o).replace(/'/g,"&apos;"),S=o.redeemedReward?.points>0,E=o.hasRewards&&!S,T=jr(o.clientPhone,o.clientName,o.serviceName,o.professionalName,o.startTime),R=I.isSelectionMode?"":'data-action="open-comanda"';return`
            <div class="appointment-list-card" data-appointment='${C}' style="border-left-color: ${l.border};">
                ...
                <div class="status-info">
                    ...
                    <div class="card-actions flex gap-1 items-center">
                        ${v?`
                            <button data-action="edit-appointment" data-appointment='${C}' class="action-btn opacity-40 cursor-not-allowed" title="Finalizado - Não editável" disabled><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L16.732 3.732z"></path></svg></button>
                        `:`
                            ...
                            <button data-action="edit-appointment" data-appointment='${C}' class="action-btn" title="Editar Agendamento"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L16.732 3.732z"></path></svg></button>
                        `}
            ...
            </div>`}).join("");t.innerHTML=`<div class="list-view-container space-y-2 pb-24">${a}</div>`}function La(){return window.innerWidth<768&&I.currentView==="week"?3:I.weekViewDays}function Or(e){const t=document.getElementById("agenda-view");if(!t)return;const a=["Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado"],o=Xo(I.currentDate),r=La();let s=`<div class="grid divide-x divide-gray-200 min-h-[60vh]" style="grid-template-columns: repeat(${r}, minmax(0, 1fr));">`;for(let n=0;n<r;n++){const i=new Date(o);i.setDate(i.getDate()+n);const l=new Date,d=i.toDateString()===l.toDateString(),c=e.filter(p=>new Date(p.startTime).toDateString()===i.toDateString()).sort((p,b)=>new Date(p.startTime)-new Date(b.startTime));let m='<div class="p-1 space-y-2">';c.length>0?m+=c.map(p=>{const f=new Date(p.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),v=u.professionalColors.get(p.professionalId)||{bg:"#e5e7eb",border:"#9ca3af"},x=h(p.reason),k=h(p.professionalName),C=h(p.clientName),S=h(p.serviceName);if(p.type==="blockage")return`
                    ...
                    `;const E=JSON.stringify(p).replace(/'/g,"&apos;"),T=p.redeemedReward?.points>0,R=p.hasRewards&&!T,O=p.status==="completed";return`
                    <div class="p-2 rounded-lg border-l-4 flex flex-col cursor-pointer" 
                        ...
                        <div class="mt-1 min-w-0">
                            <p class="font-semibold text-sm text-gray-800 truncate">${R?"🎁 ":""}${C}</p>
        ...
        `}s+="</div>",t.innerHTML=s}function Zo(){const e=u.allEvents.filter(t=>I.selectedProfessionalId==="all"||t.professionalId===I.selectedProfessionalId);I.currentView==="list"?Hr(e):Or(e),Ta()}function Ta(){const e=document.getElementById("batch-delete-container"),t=document.querySelector('[data-action="new-appointment"]');e&&(I.isSelectionMode&&I.selectedItems.size>0?(e.innerHTML=`
        ...
        `,e.style.display="block",t&&(t.style.display="none")):(e.style.display="none",t&&(t.style.display="flex")))}async function te(){const e=document.getElementById("agenda-view");if(!e)return;I.selectedItems.clear(),Ta(),e.innerHTML='<div class="loader mx-auto my-10"></div>';let t,a;const o=document.getElementById("weekRange");if(o){if(I.currentView==="list")t=new Date(I.currentDate),t.setHours(0,0,0,0),a=new Date(I.currentDate),a.setHours(23,59,59,999),o.textContent=Fr(t);else{const r=La();t=Xo(new Date(I.currentDate)),a=new Date(t),a.setDate(t.getDate()+(r-1)),a.setHours(23,59,59,999),o.textContent=`${t.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} - ${a.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}`}try{const r=await Ro(u.establishmentId,t.toISOString(),a.toISOString(),I.selectedProfessionalId==="all"?null:I.selectedProfessionalId),s=await Ot(u.establishmentId,t.toISOString(),a.toISOString(),I.selectedProfessionalId);if(!document.getElementById("agenda-view"))return;const n=s.map(l=>{let d=l.professionalName;if(!d&&l.professionalId){const c=u.professionals?u.professionals.find(m=>m.id===l.professionalId):null;c&&(d=c.name)}return{...l,type:"blockage",professionalName:d||"Não identificado"}}),i=[...r.map(l=>({...l,type:"appointment"})),...n];if(u.allEvents=i,Pt(),Zo(),I.scrollToAppointmentId){const l=document.querySelector(`[data-appointment*='"id":"${I.scrollToAppointmentId}"']`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.style.transition="background-color 0.5s ease-in-out",l.style.backgroundColor="#e0e7ff",setTimeout(()=>{l.style.backgroundColor=""},2500)),I.scrollToAppointmentId=null}}catch(r){document.getElementById("agenda-view")&&(document.getElementById("agenda-view").innerHTML='<div class="p-6 text-center text-red-600">Falha ao carregar dados.</div>',g("Erro na Agenda",`Não foi possível carregar a agenda: ${r.message}`,"error"))}}}async function zr(){try{const[e,t,a]=await Promise.all([u.professionals&&u.professionals.length>0?Promise.resolve(u.professionals):Z(u.establishmentId),u.services&&u.services.length>0?Promise.resolve(u.services):$e(u.establishmentId),Re.enabled!==void 0?Promise.resolve(null):Se(u.establishmentId)]);(!u.professionals||u.professionals.length===0)&&(u.professionals=e||[]),(!u.services||u.services.length===0)&&(u.services=t||[]),Me=[],a&&(Re=a.loyaltyProgram||{enabled:!1}),u.professionals.forEach((o,r)=>{u.professionalColors.set(o.id,ca[r%ca.length])}),Pt()}catch(e){console.error("Erro ao popular filtros e dependências do modal:",e),g("Atenção","Não foi possível pré-carregar os dados para agendamento. A abertura do modal pode ser lenta.","error")}}function ua(e){e<1||e>4||($.step=e,ma(null,!0))}function Ko(e,t){const a=document.getElementById("multiServiceToggle"),o=a&&a.checked,r=t.classList.contains("selected"),s=$.data.selectedServiceIds.indexOf(e);if(r)t.classList.remove("selected","border-blue-500"),s>-1&&$.data.selectedServiceIds.splice(s,1);else{if(!o){$.data.selectedServiceIds=[];const n=document.getElementById("apptServicesContainer");n&&n.querySelectorAll(".service-card.selected").forEach(i=>{i.classList.remove("selected","border-blue-500")})}t.classList.add("selected","border-blue-500"),$.data.selectedServiceIds.push(e)}}function es(e,t){const a=document.querySelector(".professional-step-cards");if(!a)return;a.querySelectorAll(".professional-modal-card").forEach(r=>r.classList.remove("selected","border-blue-500")),t.classList.add("selected","border-blue-500");const o=Vt.find(r=>r.id===e);$.data.professionalId=e,$.data.professionalName=o?o.name:"N/A"}function Vr(e,t){const a=document.getElementById("availableTimesContainer");a&&(a.querySelectorAll(".time-slot-card").forEach(o=>o.classList.remove("selected")),t.classList.add("selected"),$.data.time=e)}async function ao(){const e=document.getElementById("apptTotalDuration"),t=document.getElementById("availableTimesContainer");if(!e||!t)return;const a=$.data.professionalId,o=$.data.selectedServiceIds,r=document.getElementById("apptDate").value;$.data.date=r;const s=o.reduce((n,i)=>{const l=it.find(d=>d.id===i);return n+(l?l.duration+(l.bufferTime||0):0)},0);if(e.textContent=`${s} min`,s===0||!a||!r){t.innerHTML='<p class="col-span-full text-center text-gray-500">Selecione serviço, profissional e data.</p>';return}t.innerHTML='<div class="loader mx-auto col-span-full"></div>';try{let n=await Ws({establishmentId:u.establishmentId,professionalId:a,serviceIds:o,date:r});const i=new Date;if(new Date(r+"T00:00:00").toDateString()===i.toDateString()){const d=i.getHours()*60+i.getMinutes();n=n.filter(c=>{const[m,p]=c.split(":").map(Number);return m*60+p>=d})}if(t.innerHTML="",n.length>0){if(n.forEach(d=>{const c=document.createElement("button");c.type="button",c.className=`time-slot-card p-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200 transition ${$.data.time===d?"selected":""}`,c.textContent=d,c.addEventListener("click",()=>Vr(d,c)),t.appendChild(c)}),$.data.time){const d=t.querySelector(`[data-action="time-slot"][data-time="${$.data.time}"]`);d&&d.classList.add("selected")}}else t.innerHTML='<p class="col-span-full text-center text-gray-500">Nenhum horário disponível.</p>'}catch(n){console.error("Erro ao buscar horários:",n),t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao buscar horários.</p>'}}function Ur(){const e=document.getElementById("loyaltyRewardsContainer");if(!e)return;const{clientHasRewards:t,clientLoyaltyPoints:a,redeemedReward:o}=$.data,{enabled:r,rewards:s}=Re;if(!r||!t||!s||s.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden");const n=s.filter(l=>a>=l.points);let i=`
        <h4 class="text-md font-semibold text-gray-700 mb-2">🎁 Prêmios Disponíveis (${a} pontos)</h4>
    `;n.length>0?(i+='<div class="space-y-2">',i+=n.map(l=>{const d=o?.reward===l.reward,c=h(l.reward);return`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",l=>{l.target.checked&&($.data.redeemedReward=null)})}async function _r(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!$.data.time||$.data.selectedServiceIds.length===0||!$.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const o=$.data.selectedServiceIds.map(d=>{const c=it.find(m=>m.id===d);return{id:c.id,name:c.name,price:c.price,duration:c.duration,bufferTime:c.bufferTime||0,photo:c.photo||null}}),[r,s]=$.data.time.split(":"),n=new Date(`${$.data.date}T${r}:${s}:00`),i={establishmentId:u.establishmentId,clientName:$.data.clientName,clientPhone:$.data.clientPhone,services:o,professionalId:$.data.professionalId,startTime:n.toISOString(),redeemedReward:$.data.redeemedReward},l=t.querySelector("#appointmentId").value;l&&(i.id=l);try{l?await Fo(l,i):await Gs(i),g(`Agendamento ${l?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",te()}catch(d){g(d.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function ts(e){const t=$.data.clientName===e.name&&$.data.clientPhone===e.phone,a=h(e.name),o=h(e.phone);return`
    ...
    `}async function Jr(e){const t=document.getElementById("clientSearchResults");if(!t)return;const a=e.trim();if(a.length<3){t.innerHTML='<p class="text-sm text-gray-500">Digite pelo menos 3 caracteres para buscar clientes existentes.</p>';return}t.innerHTML='<div class="loader-small mx-auto my-2"></div>';try{const o=await nt(u.establishmentId,a);if(Me=o,o.length===0){t.innerHTML='<p class="text-sm text-gray-500">Nenhum cliente encontrado com este termo.</p>';return}t.innerHTML=o.map(ts).join(""),t.querySelectorAll('[data-action="select-client"]').forEach(r=>{r.addEventListener("click",s=>{const n=r.dataset.clientName,i=r.dataset.clientPhone,l=parseInt(r.dataset.loyaltyPoints||"0",10);$.data.clientName=n,$.data.clientPhone=i,$.data.clientLoyaltyPoints=l;const d=Re,c=Math.min(...(d?.rewards||[]).map(m=>m.points));$.data.clientHasRewards=d.enabled&&c!==1/0&&$.data.clientLoyaltyPoints>=c,document.getElementById("apptClientName").value=n,document.getElementById("apptClientPhone").value=i,document.querySelectorAll(".client-search-card").forEach(m=>m.classList.remove("selected","border-blue-500")),r.classList.add("selected","border-blue-500")})})}catch(o){console.error("Erro na busca de clientes:",o),t.innerHTML='<p class="text-sm text-red-500">Erro ao buscar clientes.</p>'}}async function Wr(e){e.preventDefault();const t=document.getElementById("clientRegistrationForm"),a=t.querySelector('button[type="submit"]'),o={establishmentId:u.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim(),phone:t.querySelector("#regClientPhone").value.trim(),dobDay:t.querySelector("#regClientDobDay").value.trim(),dobMonth:t.querySelector("#regClientDobMonth").value.trim(),notes:t.querySelector("#regClientNotes").value.trim()};if(!o.name||!o.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{await(void 0)(o),Me.push({name:o.name,phone:o.phone,loyaltyPoints:0}),$.data.clientName=o.name,$.data.clientPhone=o.phone,$.data.clientHasRewards=!1,$.data.clientLoyaltyPoints=0,g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",ua(1)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar"}}function Gr(){G({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",Wr)}function Yr(){Gr()}function Qr(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",o=`
        <div class="p-5 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="apptClientName" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="Nome Completo" value="${h($.data.clientName)}">
                </div>
                <div>
                    <label for="apptClientPhone" class="block text-sm font-medium text-gray-700">Telemóvel</label>
                    <input type="tel" id="apptClientPhone" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="(XX) XXXXX-XXXX" value="${h($.data.clientPhone)}">
                </div>
            </div>
    ...
    `;return{title:a,content:o}}function Xr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${it.map(o=>{const r=$.data.selectedServiceIds.includes(o.id),s=o.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S",n=h(o.name);return`
    ...
    `;return{title:e,content:a}}function Zr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Vt.map(a=>{const o=$.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",s=h(a.name);return`
    ...
    `;return{title:e,content:t}}function Kr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,o=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=$.data.date||o,s=h($.data.clientName),n=h($.data.professionalName),i=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:i}}function en(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),o=it.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=$.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            <div class="service-card p-3 bg-white rounded-lg border-2 border-gray-200 cursor-pointer transition-all hover:bg-gray-50 ${s?"selected border-blue-500":""}" data-service-id="${r.id}">
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>Ko(r.dataset.serviceId,r))})}function tn(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),o=Vt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=$.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",i=h(r.name);return`
             ...
             </div>`}).join(""),t.querySelectorAll(".professional-modal-card").forEach(r=>{r.addEventListener("click",()=>es(r.dataset.professionalId,r))})}async function ma(e=null,t=!1){const a=document.getElementById("appointmentModal");if(!t){const s=e?.startTime?new Date(e.startTime).toISOString().split("T")[0]:new Date().toISOString().split("T")[0],n=e?.startTime?new Date(e.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",hour12:!1}):null;if($={step:1,data:{id:e?.id||null,clientName:e?.clientName||"",clientPhone:e?.clientPhone||"",selectedServiceIds:e?.services?.map(i=>i.id)||[],professionalId:e?.professionalId||null,professionalName:e?.professionalName||"",date:s,time:n,redeemedReward:e?.redeemedReward||null,clientHasRewards:e?.hasRewards||!1,clientLoyaltyPoints:0}},e&&e.clientName)try{const i=await nt(u.establishmentId,e.clientName),l=i.find(d=>d.phone===e.clientPhone);l&&($.data.clientLoyaltyPoints=l.loyaltyPoints||0,Me=i)}catch(i){console.warn("Não foi possível carregar pontos do cliente para edição:",i)}}if(!u.services||!u.professionals||Re.enabled===void 0){g("Erro","Os dados da agenda ainda não foram carregados. Tente novamente em alguns segundos.","error");return}if(it=u.services,Vt=u.professionals.filter(s=>s.status==="active"),$.data.clientLoyaltyPoints>0){const s=Re,n=Math.min(...(s?.rewards||[]).map(i=>i.points));$.data.clientHasRewards=s.enabled&&n!==1/0&&$.data.clientLoyaltyPoints>=n}let o={title:"Erro",content:"<p>Etapa não encontrada.</p>"};switch($.step){case 1:o=Qr(e);break;case 2:o=Xr();break;case 3:o=Zr();break;case 4:o=Kr(e);break}a.innerHTML=`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(s=>{s.addEventListener("click",()=>{const n=parseInt(s.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),l=a.querySelector("#apptClientPhone");if($.data.clientName=i.value.trim(),$.data.clientPhone=l.value.trim(),!$.data.clientName||!$.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if($.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!$.data.professionalId)return g("Atenção","Selecione um profissional.","error");ua(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(s=>{s.addEventListener("click",()=>ua(parseInt(s.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if($.step===4&&r&&r.addEventListener("submit",_r),a.style.display="flex",$.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>Ko(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>en(i.target.value))}if($.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>es(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>tn(i.target.value))}if($.step===1){const s=a.querySelector("#clientSearchInput");if(s&&(s.addEventListener("input",i=>Jr(i.target.value)),$.data.clientName&&$.data.clientPhone&&Me.length>0)){const i=document.getElementById("clientSearchResults");i&&(i.innerHTML=Me.map(ts).join(""))}const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",Yr)}if($.step===4){const s=a.querySelector("#apptDate");s&&s.addEventListener("change",ao),ao(),Ur()}}async function as(e={}){I.currentDate=e.targetDate?new Date(e.targetDate):I.currentDate||new Date,I.scrollToAppointmentId=e.scrollToAppointmentId||null,I.profSearchTerm="",I.isSelectionMode=!1,I.selectedItems.clear(),window.innerWidth<768&&(I.currentView="list"),eo.innerHTML=`
        <section>
        ...
        </section>`;const t=document.getElementById("btn-toggle-select");t.addEventListener("click",()=>{I.isSelectionMode=!I.isSelectionMode,I.isSelectionMode||I.selectedItems.clear(),t.classList.toggle("bg-blue-100",I.isSelectionMode),t.classList.toggle("text-blue-700",I.isSelectionMode),Zo()}),document.querySelectorAll(".view-btn[data-view]").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(s=>s.classList.remove("active")),o.classList.add("active"),I.currentView=o.dataset.view;const r=document.getElementById("week-days-toggle");if(I.currentView==="week"){if(r.style.display="flex",window.innerWidth<768){I.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(n=>n.classList.remove("active"));const s=document.querySelector('.week-days-btn[data-days="3"]');s&&s.classList.add("active")}}else r.style.display="none";te()})}),document.querySelectorAll(".week-days-btn").forEach(o=>{o.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(r=>r.classList.remove("active")),o.classList.add("active"),I.weekViewDays=parseInt(o.dataset.days,10),te()})}),document.getElementById("todayBtn").addEventListener("click",()=>{I.currentDate=new Date,te()});const a=o=>{const r=parseInt(o.currentTarget.dataset.amount,10),s=I.currentView==="week"?La():1,n=new Date(I.currentDate);n.setDate(n.getDate()+r*s),I.currentDate=n,te()};document.getElementById("prevBtn").addEventListener("click",a),document.getElementById("nextBtn").addEventListener("click",a),document.getElementById("profSearchInput").addEventListener("input",o=>{I.profSearchTerm=o.target.value,Pt()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",o=>{I.showInactiveProfs=o.target.checked,Pt(),te()}),to||(eo.addEventListener("click",async o=>{const r=o.target.closest("[data-action]");if(o.target.dataset.action==="toggle-select-item"){const l=o.target.dataset.id;o.target.checked?I.selectedItems.add(l):I.selectedItems.delete(l),Ta();return}if(r&&r.dataset.action==="batch-delete"){const l=I.selectedItems.size;if(await j("Excluir em Lote",`Tem certeza que deseja excluir ${l} agendamento(s)? Esta ação não pode ser desfeita.`)){const c=Array.from(I.selectedItems);let m=0;try{await Promise.all(c.map(async p=>{try{await Ua(p),m++}catch(b){console.error(`Falha ao excluir ${p}`,b)}})),g(`${m} agendamento(s) excluído(s).`,"success"),I.selectedItems.clear(),I.isSelectionMode=!1,document.getElementById("btn-toggle-select").classList.remove("bg-blue-100","text-blue-700"),te()}catch{g("Erro ao processar exclusão em lote.","error")}}return}if(o.target.closest('[data-action="select-professional"]')){const d=o.target.closest('[data-action="select-professional"]').dataset.profId,c=I.selectedProfessionalId===d&&d!=="all";if(I.selectedProfessionalId=c?"all":d,d!=="all"){const m=document.getElementById("profSearchInput");m&&(m.value=""),I.profSearchTerm=""}await te();return}if(!r)return;const s=r.dataset.action;let n=null;const i=o.target.closest("[data-appointment]");switch(i&&(n=JSON.parse(i.dataset.appointment.replace(/&apos;/g,"'"))),s){case"new-appointment":ma();break;case"edit-appointment":if(I.isSelectionMode||!n)return;if(n.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}n.hasRewards&&!n.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),ma(n);break;case"delete-appointment":{if(I.isSelectionMode)return;const l=r.dataset.id;if(await j("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await Ua(l),g("Agendamento apagado!","success"),te()}catch(c){g(`Não foi possível apagar: ${c.message}`,"error")}break}case"open-comanda":if(I.isSelectionMode)return;if(n){n.hasRewards&&!n.redeemedReward&&n.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const l=n.status==="completed"?"finalizadas":"em-atendimento",d={selectedAppointmentId:n.id,initialFilter:l};if(l==="finalizadas"){let c=n.startTime;if(n.transaction&&n.transaction.paidAt){const m=n.transaction.paidAt;typeof m=="object"&&m._seconds?c=new Date(m._seconds*1e3):c=m}d.filterDate=c}ae("comandas-section",d)}break}}),to=!0),await zr(),await te()}const an=(e,t=null,a=1,o=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${o}`;return t&&(r+=`&date=${t}`),w(r)},on=e=>w("/api/sales",{method:"POST",body:JSON.stringify(e)}),sn=e=>w(`/api/sales/${e}`,{method:"DELETE"}),rn=()=>w("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),nn=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),w("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},ln=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),w(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},dn=()=>w("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),cn=e=>w(`/api/cashier/report/${e}`),os=e=>w(`/api/packages/${e}`),un=e=>w("/api/packages",{method:"POST",body:JSON.stringify(e)}),mn=(e,t)=>w(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),pn=e=>w(`/api/packages/${e}`,{method:"DELETE"});let y={allComandas:[],catalog:{services:[],products:[],packages:[]},activeFilter:"atendimento",selectedComandaId:null,viewMode:"items",isCashierOpen:!1,activeCashierSessionId:null,loyaltySettings:null,paging:{page:1,limit:10,total:0},checkoutState:{payments:[],selectedMethod:"dinheiro",installments:1,amountReceived:""}},Ie=null,Le=null,oo=null;function ss(e,t){return function(...a){clearTimeout(oo),oo=setTimeout(()=>e.apply(this,a),t)}}async function so(e,t="stay"){if(!e||!e.id)return;e._localUpdatedAt=Date.now(),e._cachedItems=null,ke(),t==="checkout"&&(y.viewMode="checkout",y.checkoutState.payments||(y.checkoutState.payments=[]),y.checkoutState.selectedMethod="dinheiro",y.checkoutState.amountReceived="",X());const a=document.createElement("div");a.id="saving-overlay",a.className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm",a.innerHTML=`
    ...
    `,document.body.appendChild(a);try{e.type==="walk-in"&&String(e.id).startsWith("temp-"),await Fo(e.id,e),document.body.contains(a)&&document.body.removeChild(a),t!=="checkout"&&g("Sucesso","Comanda atualizada!","success")}catch(o){document.body.contains(a)&&document.body.removeChild(a),console.error("Erro ao salvar:",o),g("Erro","Falha ao salvar no servidor (Verifique conexão): "+o.message,"warning")}}function lt(e){if(!e._cachedItems){let t=[];if(e.status==="completed"){const a=e.comandaItems||e.items||[];t=a.length>0?a:e.services||[]}else{const a=(e.services||[]).map(n=>({...n,_source:"original_service"})),o=e.comandaItems||[],r=e.items||[],s=[...o,...r].map(n=>({...n,_source:"extra"}));t=[...a,...s]}return e._cachedItems=t,e._cachedTimestamp=Date.now(),t}return e._cachedItems}function gn(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function Te(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function bn(){const e=new Date().toISOString().split("T")[0];Le.innerHTML=`
    ...
    `,Ut()}function Ut(){const e=document.getElementById("cashier-alert-box"),t=document.getElementById("btn-new-sale");y.isCashierOpen?(e&&(e.innerHTML=""),t&&(t.classList.remove("opacity-50","cursor-not-allowed"),t.disabled=!1)):(e&&(e.innerHTML=`
        ...
        `),t&&(t.classList.add("opacity-50","cursor-not-allowed"),t.disabled=!0)),fn()}function fn(){const e=document.getElementById("cashier-controls");e&&(y.isCashierOpen?e.innerHTML=`
        ...
        `:e.innerHTML=`
            ...
            <button data-action="view-sales-report" class="py-2 px-4 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 text-sm transition">Relatório</button>
        `)}function ke(){const e=document.getElementById("comandas-list"),t=document.getElementById("pagination-container");if(!e)return;if(!y.isCashierOpen&&y.activeFilter==="atendimento"){e.innerHTML=`
        ...
        `,t&&(t.innerHTML="");return}const o={atendimento:"confirmed",finalizadas:"completed"}[y.activeFilter],r=y.allComandas.filter(n=>n.status===o);if(r.length===0){e.innerHTML='<p class="text-center text-gray-400 py-10 text-sm">Nenhuma venda nesta página.</p>',ro(t);return}const s=document.createDocumentFragment();r.forEach(n=>{const l=lt(n).reduce((x,k)=>x+(k.price||0),0),d=n.id===y.selectedComandaId,c=new Date(n.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),m=n.type==="walk-in"||typeof n.id=="string"&&n.id.startsWith("temp-"),p=h(n.clientName||"Cliente sem nome"),b=h(n.professionalName||"Sem profissional"),f=m?'<span class="text-[10px] font-bold uppercase text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md border border-blue-200">Avulso</span>':'<span class="text-[10px] font-bold uppercase text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-md border border-indigo-200">Agenda</span>',v=document.createElement("div");v.className=`comanda-card cursor-pointer ${d?"selected":""}`,v.dataset.action="select-comanda",v.dataset.comandaId=n.id,v.innerHTML=`
            <div class="flex justify-between items-start mb-1 pointer-events-none">
                <p class="font-bold text-gray-800 truncate max-w-[70%] text-sm">${p}</p>
        ...
        `,s.appendChild(v)}),e.innerHTML="",e.appendChild(s),ro(t)}function ro(e){if(!e)return;e.innerHTML="";const{page:t,total:a,limit:o}=y.paging,r=Math.ceil((a||0)/o);if(r===0)return;const s=document.createElement("div");s.className="flex gap-2 justify-center items-center w-full",s.innerHTML=`
    ...
    `,e.appendChild(s),s.querySelectorAll("button[data-page]").forEach(n=>{n.onclick=i=>{i.stopPropagation();const l=parseInt(n.dataset.page,10);l>0&&l<=r&&(y.paging.page=l,se())}})}function X(){const e=document.getElementById("comanda-detail-container");if(!e)return;const t=y.allComandas.find(p=>p.id===y.selectedComandaId);if(y.viewMode==="checkout"&&t){vn(t,e);return}const a=`
    ...
    `;if(!y.isCashierOpen){e.innerHTML=`
        ...
        `;return}const o=lt(t),r=t.status==="completed",s=t.type==="walk-in"||typeof t.id=="string"&&t.id.startsWith("temp-"),n=o.reduce((p,b)=>{const f=`${b.type}-${b.id||b.name}`;return p[f]||(p[f]={...b,quantity:0,sources:[]}),p[f].quantity+=1,b._source&&p[f].sources.push(b._source),p},{}),i=Object.values(n).reduce((p,b)=>p+(b.price||0)*b.quantity,0),l=h(t.clientName||"Cliente sem nome"),d=h(t.professionalName||"Profissional não atribuído"),c=`
    ...
    `;e.innerHTML=`
        ...
        <div class="flex-grow overflow-y-auto p-4 pb-24 custom-scrollbar"> 
            ...
            <div class="space-y-3">
                ...
                ${Object.values(n).map(p=>{const f=p.sources&&p.sources.includes("original_service")&&p.quantity<=1;return`
                    <div class="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-100 shadow-sm ${p.isReward?"border-yellow-200 bg-yellow-50":""}">
                        <div class="flex items-center gap-3 w-full">
                            <div class="flex-grow min-w-0">
                                <p class="text-sm font-semibold text-gray-800 line-clamp-1">${p.isReward?"🎁 ":""}${h(p.name)}</p>
                            ...
                            ${r?`<span class="flex items-center justify-center w-8 h-8 bg-gray-100 text-gray-700 font-bold text-sm rounded-lg">${p.quantity}x</span>`:`
                                <div class="flex items-center bg-gray-100 rounded-lg p-1 gap-3">
                                    <button data-action="decrease-qty" data-item-id="${p.id}" data-item-type="${p.type}" class="w-6 h-6 flex items-center justify-center rounded bg-white text-gray-600 shadow-sm hover:bg-red-50 hover:text-red-600 disabled:opacity-30 disabled:hover:bg-white disabled:hover:text-gray-600" ${f?'disabled title="Mínimo 1"':""}>-</button>
        ...
        ${r?"":m}
    `,!r&&(t.clientId||t.clientName)&&hn(t,e.querySelector("#loyalty-container"))}function vn(e,t){const o=lt(e).reduce((l,d)=>l+(d.price||0),0),r=y.checkoutState,s=r.payments.reduce((l,d)=>l+d.value,0),n=Math.max(0,o-s);(!r.amountReceived||n>0)&&(r.amountReceived=n.toFixed(2));const i=`
        <div class="mobile-only-header">
    ...
    `;t.innerHTML=`
        ...
        <div class="flex-grow overflow-y-auto p-4 pb-24 custom-scrollbar">
            <div class="text-center mb-6 bg-gray-50 p-6 rounded-xl border border-gray-100">
                ...
                <p class="text-5xl font-extrabold text-gray-800 mt-2">R$ ${o.toFixed(2)}</p>
    ...
    `,t.querySelector("#checkout-amount")?.addEventListener("input",l=>{r.amountReceived=l.target.value}),t.querySelector("#checkout-installments")?.addEventListener("change",l=>{r.installments=parseInt(l.target.value,10)})}async function hn(e,t){if(!t)return;const a=y.loyaltySettings;if(!a||!a.enabled)return;let o=null;try{if(e.clientId)o=await da(u.establishmentId,e.clientId);else if(e.clientName){const i=await nt(u.establishmentId,e.clientName,1);i&&i.length>0&&(o=i[0])}}catch(i){console.warn("Erro ao buscar dados de fidelidade",i)}if(!o||o.loyaltyPoints===void 0)return;const r=Number(o.loyaltyPoints)||0,n=(a.tiers||a.rewards||[]).filter(i=>{const l=Number(i.costPoints||i.points||0);return l>0&&r>=l});if(n.length>0){const i=document.createElement("div");i.className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-xl p-4 shadow-sm flex justify-between items-center animate-fade-in",i.innerHTML=`
        ...
        `;const l=document.createElement("button");l.innerText="Resgatar",l.className="text-xs font-bold bg-yellow-500 text-white px-4 py-2 rounded-lg shadow hover:bg-yellow-600 transition-colors",l.onclick=()=>xn(n),i.appendChild(l),t.innerHTML="",t.appendChild(i)}}function xn(e,t){const a=`
        <div class="space-y-4">
            <p class="text-sm text-gray-600 mb-4">O cliente possui pontos suficientes para resgatar os seguintes itens:</p>
            <div class="space-y-2 max-h-96 overflow-y-auto custom-scrollbar">
                ${e.map(s=>{const n=s.costPoints||s.points||0,i=s.name||s.reward;return`
    ...
    `,{modalElement:o,close:r}=G({title:"🎁 Resgatar Prémio",contentHTML:a,maxWidth:"max-w-md"});o.addEventListener("click",s=>{const n=s.target.closest('[data-action="select-reward"]');if(n){const i=n.dataset.rewardId,l=e.find(d=>d.id&&d.id==i||(d.name||d.reward)==i);l&&(yn(l),r())}})}async function yn(e,t){const a=Number(e.costPoints||e.points||0),o=e.name||e.reward,r={id:e.serviceId||e.productId||`reward-${Date.now()}`,name:`${o}`,price:0,type:e.serviceId?"service":"product",isReward:!0,pointsCost:a};await Pa(r,1)}function wn(){if(!y.isCashierOpen)return g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");const{modalElement:e,close:t}=G({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{const r=e.querySelector("#add-item-content");r.innerHTML=`
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{s>1&&(s--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{s++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await Pa(r,s),t()}};e.addEventListener("click",r=>{const s=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(s){const{itemType:i,itemId:l}=s.dataset,c=(y.catalog[i+"s"]||[]).find(m=>m.id===l);c&&o({...c,type:i})}else n&&a()}),a()}async function pa(e=null){if(!y.isCashierOpen)return g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");if(!u.professionals||u.professionals.length===0)try{u.professionals=await Z(u.establishmentId)}catch{return g("Erro","Não foi possível carregar profissionais.","error")}const a=`
    ...
    `,{modalElement:o}=G({title:"Nova Venda Avulsa",contentHTML:a,maxWidth:"max-w-md"}),r=o.querySelector("#client-search"),s=o.querySelector("#client-suggestions"),n=o.querySelector("#selected-client-id");e&&(n.value=e.id,r.value=`${e.name} (${e.phone||"Sem tel"})`,r.classList.add("bg-green-50","border-green-300","text-green-800")),r.addEventListener("input",ss(async l=>{const d=l.target.value.trim();if(n.value="",r.classList.remove("bg-green-50","border-green-300","text-green-800"),d.length<2){s.classList.add("hidden");return}try{s.innerHTML='<li class="p-2 text-xs text-gray-500">Buscando...</li>',s.classList.remove("hidden");const c=await nt(u.establishmentId,d,10);c.length===0?s.innerHTML='<li class="p-2 text-xs text-gray-500">Nenhum cliente encontrado</li>':s.innerHTML=c.map(m=>`<li data-client-id="${m.id}" data-client-name="${m.name}" data-client-phone="${m.phone}" class="p-2 hover:bg-indigo-50 cursor-pointer border-b last:border-0 transition-colors"><div class="font-bold text-sm text-gray-800">${h(m.name)}</div><div class="text-xs text-gray-500">${m.phone||"Sem telefone"}</div></li>`).join("")}catch{s.classList.add("hidden")}},400)),s.addEventListener("click",l=>{const d=l.target.closest("li[data-client-id]");d&&(n.value=d.dataset.clientId,n.dataset.name=d.dataset.clientName,n.dataset.phone=d.dataset.clientPhone,r.value=`${d.dataset.clientName}`,r.classList.add("bg-green-50","border-green-300","text-green-800"),s.classList.add("hidden"))}),document.addEventListener("click",l=>{!r.contains(l.target)&&!s.contains(l.target)&&s.classList.add("hidden")}),o.querySelector("#new-sale-form").addEventListener("submit",Ln);const i=o.querySelector('[data-action="new-client-from-sale"]');i&&i.addEventListener("click",l=>{l.preventDefault(),o.style.display="none",kn()})}function kn(){G({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",Sn)}async function Sn(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector("#regClientName"),r=t.querySelector("#regClientPhone").value.replace(/\D/g,"");if(!a.value||!r)return g("Erro","Nome e Telefone são obrigatórios.","error");try{const s=await(void 0)(u.establishmentId,r);if(s)g("Atenção","Cliente já cadastrado.","info"),document.getElementById("genericModal").style.display="none",pa(s);else{const n=await(void 0)({establishmentId:u.establishmentId,name:a.value,phone:r});g("Sucesso","Cliente cadastrado!","success"),document.getElementById("genericModal").style.display="none",pa(n)}}catch(s){g("Erro",s.message,"error")}}async function $n(){const e=`
    ...
    `,{modalElement:t}=G({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const o=parseFloat(document.getElementById("initial-amount").value);if(isNaN(o)||o<0)return g("Valor Inválido","Insira um valor válido.","error");try{const r=await nn({establishmentId:u.establishmentId,initialAmount:parseFloat(o.toFixed(2))});y.isCashierOpen=!0,y.activeCashierSessionId=r.id,document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto (R$ ${o.toFixed(2)})`,"success"),Ut(),await se()}catch(r){g("Erro",`Falha ao abrir caixa: ${r.message}`,"error")}})}async function En(){const e=y.activeCashierSessionId;if(e)try{const t=await cn(e),a=`
        ...
        `,{modalElement:o}=G({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});o.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const s=parseFloat(document.getElementById("final-amount").value);if(isNaN(s)||s<0)return g("Valor Inválido","Insira um valor final válido.","error");try{await ln(e,s),y.isCashierOpen=!1,y.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",Ut(),await se(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Falha ao fechar caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Falha ao carregar relatório: ${t.message}`,"error")}}async function In(e){y.activeFilter!==e&&(y.activeFilter=e,y.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),Te(),await se(),y.selectedComandaId=null,y.viewMode="items",X())}function rs(e){y.selectedComandaId=e,y.viewMode="items",ke(),gn(),X()}async function Pa(e,t){const a=y.allComandas.find(r=>r.id===y.selectedComandaId);if(!a)return;const o=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type,isReward:e.isReward||!1,pointsCost:e.pointsCost||0}));a.comandaItems=a.comandaItems||[],a.comandaItems.push(...o),a._cachedItems=null,a._localUpdatedAt=Date.now(),X(),ke()}async function no(e,t){const a=y.allComandas.find(s=>s.id===y.selectedComandaId);if(!a)return;let o=!1,r=(a.comandaItems||[]).findIndex(s=>s.id===e&&s.type===t);if(r>-1)a.comandaItems.splice(r,1),o=!0;else{let s=(a.services||[]).findIndex(n=>n.id===e);s>-1&&a.services.filter(i=>i.id===e).length>1&&(a.services.splice(s,1),o=!0)}o&&(a._cachedItems=null,a._localUpdatedAt=Date.now(),X(),ke())}async function Cn(e){if(y.isProcessing)return;y.isProcessing=!0;const t=e.type==="appointment",o=lt(e),{payments:r}=y.checkoutState,s=r.reduce((c,m)=>c+m.value,0);let n=0;const i=y.loyaltySettings;if(i&&i.enabled)if(i.type==="visit")n=Number(i.pointsPerVisit)||1;else{const c=Number(i.pointsPerCurrency)||10;c>0&&(n=Math.floor(s/c))}const l={payments:r,totalAmount:Number(s),items:o,cashierSessionId:y.activeCashierSessionId,loyaltyPointsEarned:n},d=document.createElement("div");d.className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm",d.innerHTML='<div class="bg-white p-6 rounded-2xl shadow-2xl flex flex-col items-center"><div class="loader mb-4 border-t-indigo-600"></div><p>Finalizando venda...</p></div>',document.body.appendChild(d);try{t?await Qs(e.id,l):(l.establishmentId=u.establishmentId,l.clientId=e.clientId,l.clientName=e.clientName,l.professionalId=e.professionalId,e.clientPhone&&(l.clientPhone=e.clientPhone),await on(l));let c="Venda finalizada com sucesso!";n>0&&(c+=` Cliente ganhou ${n} pontos!`),g("Sucesso!",c,"success"),Te(),y.selectedComandaId=null,y.viewMode="items",await se()}catch(c){g("Erro no Checkout",c.message,"error")}finally{document.body.contains(d)&&document.body.removeChild(d),y.isProcessing=!1}}async function Ln(e){e.preventDefault();const t=document.getElementById("selected-client-id"),a=document.getElementById("new-sale-professional").value,o=t.value,r=document.getElementById("client-search").value,s=t.dataset.phone||"";if(!o)return g("Erro","Selecione um cliente válido.","error");const n=u.professionals.find(l=>l.id===a);if(!n)return g("Erro","Selecione um profissional válido.","error");const i={id:`temp-${Date.now()}`,type:"walk-in",clientId:o,clientName:r.split("(")[0].trim(),clientPhone:s,professionalId:n.id,professionalName:n.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};y.allComandas.unshift(i),y.selectedComandaId=i.id,y.viewMode="items",document.getElementById("genericModal").style.display="none",rs(i.id)}async function se(){const e=document.getElementById("comandas-list");(!e.hasChildNodes()||e.innerHTML.includes("loader"))&&(e.innerHTML='<div class="loader mx-auto mt-10"></div>');const t=y.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await rn();if(y.isCashierOpen=!!a,y.activeCashierSessionId=a?a.id:null,Ut(),!y.isCashierOpen&&y.activeFilter==="atendimento"){ke(),X();return}try{const r=await(void 0)(u.establishmentId);r&&r.loyaltyProgram&&(y.loyaltySettings=r.loyaltyProgram)}catch{}const o=await an(u.establishmentId,t,y.paging.page,y.paging.limit);if(y.allComandas=o.data||o,y.paging.total=o.total||o.length,y.catalog.services.length===0){const[r,s,n,i]=await Promise.all([$e(u.establishmentId),Ht(u.establishmentId),os(u.establishmentId),Z(u.establishmentId)]);y.catalog={services:r,products:s,packages:n},u.professionals=i}ke(),X()}catch(a){g("Erro",`Não foi possível carregar os dados: ${a.message}`,"error")}}async function Tn(e={}){Le=document.getElementById("content"),y.selectedComandaId=e.selectedAppointmentId||null,y.viewMode="items",bn(),Ie&&(Le.removeEventListener("click",Ie),Le.removeEventListener("change",Ie)),Ie=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id]");if(t.target.id==="filter-date"&&y.activeFilter==="finalizadas"){y.paging.page=1,await se();return}if(a){if(a.matches("[data-filter]"))In(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}rs(a.dataset.comandaId)}else if(a.matches("[data-action]")){const r=a.dataset.action,s=a.dataset.id||y.selectedComandaId,n=y.allComandas.find(i=>i.id===s);switch(r){case"back-to-list":Te(),y.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(v=>v.classList.remove("selected")),X();break;case"new-sale":pa();break;case"add-item":wn();break;case"open-cashier":$n();break;case"close-cashier":await En();break;case"view-sales-report":ae("sales-report-section");break;case"go-to-checkout":await so(n,"checkout");break;case"back-to-items":y.viewMode="items",X();break;case"save-comanda":await so(n,"stay");break;case"select-method":y.checkoutState.selectedMethod=a.dataset.method,y.checkoutState.installments=1,X();break;case"add-payment-checkout":const i=document.getElementById("checkout-amount");let l=parseFloat(i.value);const c=lt(n).reduce((v,x)=>v+(x.price||0),0),m=y.checkoutState.payments.reduce((v,x)=>v+x.value,0),p=c-m;if(isNaN(l)||l<=0){g("Valor inválido","Insira um valor maior que zero.","error");break}if(l>p+.05){g("Valor inválido","Valor excede o restante.","error");break}const b={method:y.checkoutState.selectedMethod,value:l};["credito","crediario"].includes(y.checkoutState.selectedMethod)&&y.checkoutState.installments>1&&(b.installments=y.checkoutState.installments),y.checkoutState.payments.push(b),y.checkoutState.selectedMethod="dinheiro",y.checkoutState.installments=1,y.checkoutState.amountReceived="",X();break;case"remove-payment-checkout":const f=parseInt(a.dataset.index,10);y.checkoutState.payments.splice(f,1),X();break;case"finalize-checkout":await Cn(n);break;case"increase-qty":{const k=(y.catalog[a.dataset.itemType+"s"]||[]).find(C=>C.id===a.dataset.itemId)||{id:a.dataset.itemId,name:"Item",price:0,type:a.dataset.itemType};await Pa(k,1);break}case"decrease-qty":{await no(a.dataset.itemId,a.dataset.itemType);break}case"remove-item":await no(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await j("Reabrir Comanda","Tem certeza? O pagamento será estornado."))try{await Ys(s);const x=y.allComandas.findIndex(k=>k.id===s);x!==-1&&(y.allComandas[x].status="confirmed",delete y.allComandas[x].transaction),y.selectedComandaId=null,Te(),await se(),g("Sucesso!","Comanda reaberta.","success")}catch(x){g("Erro",x.message,"error")}break}case"go-to-appointment":{const v=a.dataset.id,x=a.dataset.date;ae("agenda-section",{scrollToAppointmentId:v,targetDate:new Date(x)});break}case"delete-walk-in":{if(await j("Excluir Venda","Confirma a exclusão desta venda avulsa?"))if(s.startsWith("temp-"))y.allComandas=y.allComandas.filter(x=>x.id!==s),y.selectedComandaId=null,ke(),X(),Te();else try{await sn(s),g("Sucesso","Venda excluída.","success"),y.selectedComandaId=null,Te(),await se()}catch(x){g("Erro",x.message,"error")}break}}}}},Le.addEventListener("click",Ie),Le.addEventListener("change",Ie),e.initialFilter&&(y.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(y.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${y.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",y.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await se()}const ga=document.getElementById("content");let Xt={};const mt=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],q={startDate:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,appointmentsData:[],currentTab:"dashboards"};async function Pn(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function Bn(){ga.innerHTML='<div class="flex flex-col items-center justify-center h-64"><div class="loader mb-4"></div><p class="text-gray-500">A carregar inteligência de dados...</p></div>';try{await Pn();const[e,t]=await Promise.all([Z(u.establishmentId),Dr(u.establishmentId).catch(()=>[])]);q.professionalsList=e||[],q.costCentersList=t||[],Dn(),await ns()}catch(e){console.error("Erro no loadReportsPage:",e),ga.innerHTML=`
            ...
            </div>`}}function Dn(){const e=q.professionalsList.map(a=>`<option value="${a.id}">${h(a.name)}</option>`).join(""),t=q.costCentersList.map(a=>`<option value="${a.id}">${h(a.name)}</option>`).join("");ga.innerHTML=`
    ...
    `,document.getElementById("btn-filter").onclick=Mn,document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{q.currentTab=a.dataset.tab,io(),is()}}),io()}function io(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===q.currentTab;e.className=t?"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-bold bg-white text-indigo-600 shadow-sm transition-all whitespace-nowrap":"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition-all whitespace-nowrap"})}async function Mn(){q.startDate=document.getElementById("report-start").value,q.endDate=document.getElementById("report-end").value,q.selectedProfessional=document.getElementById("report-prof").value,q.selectedCostCenter=document.getElementById("report-cost").value,await ns()}async function ns(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader"></div></div>';try{const t=Br(q.startDate,q.endDate,q.selectedProfessional,q.selectedCostCenter),a=q.selectedProfessional==="all"?null:q.selectedProfessional,o=new Date(q.startDate+"T00:00:00").toISOString(),r=new Date(q.endDate+"T23:59:59").toISOString(),s=Ro(u.establishmentId,o,r,a).catch(l=>[]),[n,i]=await Promise.all([t,s]);q.data=n,q.appointmentsData=Array.isArray(i)?i:[],is()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function is(){const e=document.getElementById("report-content");if(q.data)switch(q.currentTab){case"dashboards":An(e);break;case"appointments":Nn(e);break;case"dre":qn(e);break}}function An(e){const{dreSimple:t,charts:a}=q.data,o=t||{grossRevenue:0,netProfit:0,variableCosts:0},r=q.data.totalAppointments||0,s=r>0?o.grossRevenue/r:0;e.innerHTML=`
    ...
    `,Je("chart-monthly","bar","Receita Mensal",a.salesMonthly.labels,a.salesMonthly.data,mt[0]);const n=a.professionals.labels.map(l=>h(l));Je("chart-profs","doughnut","Total Vendas",n,a.professionals.data,mt),Je("chart-daily","line","Vendas Diárias",a.salesDaily.labels,a.salesDaily.data,mt[4]);const i=a.products.labels.map(l=>h(l));Je("chart-products","bar","Total Vendido",i,a.products.data,mt[1])}function Nn(e){const t=q.appointmentsData,a=t.length;let o=0,r=0,s=0,n=0;const i={},l={};let d=new Date(q.startDate);const c=new Date(q.endDate);for(;d<=c;)i[d.toISOString().split("T")[0]]=0,d.setDate(d.getDate()+1);t.forEach(f=>{const v=parseFloat(f.totalAmount||f.price||0),x=(f.status||"").toLowerCase();let k=f.startTime?(f.startTime.toDate?f.startTime.toDate():new Date(f.startTime)).toISOString().split("T")[0]:"";const C=f.professionalName||"Sem Profissional";l[C]||(l[C]={name:C,count:0,value:0}),["cancelled","cancelado","no-show"].includes(x)?(r++,n+=v):(["completed","finalized","paid"].includes(x)&&o++,s+=v,k&&i.hasOwnProperty(k)&&i[k]++,l[C].count++,l[C].value+=v)});const m=Object.keys(i).sort(),p=m.map(f=>i[f]),b=Object.values(l).sort((f,v)=>v.count-f.count);e.innerHTML=`
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        ${b.map(f=>`
                            <tr>
                                <td class="p-3 text-gray-800 font-medium">${h(f.name)}</td>
    ...
    `,Je("dailyApptChart","line","Agendamentos",m.map(f=>f.split("-").reverse().slice(0,2).join("/")),p,"#4f46e5")}function qn(e){const{dreFinancial:t}=q.data,a=Object.entries(t.revenues).map(([r,s])=>`
    ...
    `}function Je(e,t,a,o,r,s){const n=document.getElementById(e);if(!n)return;const i=n.getContext("2d");Xt[e]&&Xt[e].destroy();const l={type:t,data:{labels:o,datasets:[{label:a,data:r,backgroundColor:Array.isArray(s)?s:t==="line"?"rgba(79, 70, 229, 0.1)":s,borderColor:Array.isArray(s)?"#fff":s,borderWidth:2,fill:t==="line",tension:.3,borderRadius:t==="bar"?4:0,pointBackgroundColor:"#fff",pointBorderColor:s,pointHoverBackgroundColor:s}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:t==="doughnut",position:"bottom"},tooltip:{backgroundColor:"rgba(17, 24, 39, 0.9)",padding:10,cornerRadius:8,callbacks:{label:d=>{let c=d.dataset.label||"";return c&&(c+=": "),d.parsed.y!==null?c+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(d.parsed.y):c+=d.raw,c}}}},scales:t==="doughnut"?{}:{y:{beginAtZero:!0,grid:{color:"#f3f4f6"},ticks:{font:{size:11}}},x:{grid:{display:!1},ticks:{font:{size:11}}}}}};Xt[e]=new Chart(i,l)}const _t=(e,t="products")=>w(`/api/${t}/categories/${e}`),ls=(e,t="products")=>w(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),ds=(e,t="products")=>w(`/api/${t}/categories/${e}`,{method:"DELETE"}),Rn="audit_logs",Fe=async(e,t,a,o,r,s=null)=>{try{if(!t)return;await Mo(ue(U,Rn),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:o,description:r,details:s,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},fe=document.getElementById("content");let ne=null,Ye="services",xe="all";function je(){const e=V.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function Fn(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await ls({establishmentId:u.establishmentId,name:o},"services"),Fe(u.establishmentId,je(),"Categorias (Serviços)","Criou",`Criou categoria: ${o}`),a.value="",g("Sucesso","Categoria criada!","success"),await Ba(),await dt()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function jn(e){if(await j("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await ds(e,"services"),Fe(u.establishmentId,je(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await Ba(),await dt()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function Ba(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await _t(u.establishmentId,"services");u.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                ...
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria criada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function Hn(){G({title:"Gerir Categorias de Serviços",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",Fn),t.addEventListener("click",o=>{const r=o.target.closest('button[data-action="delete-category"]');r&&(o.preventDefault(),jn(r.dataset.id))}))}Ba()}async function On(e){e.preventDefault();const t=e.target.closest("#serviceModal"),a=t.querySelector("#serviceId").value,o={},r=t.querySelector('input[name="commissionType"]:checked').value;r==="custom"&&t.querySelectorAll(".professional-commission-row").forEach(n=>{const i=n.dataset.profId;if(n.querySelector('input[type="checkbox"]').checked){const d=parseFloat(n.querySelector('input[type="number"]').value);o[i]=isNaN(d)?0:d}});const s={establishmentId:u.establishmentId,name:t.querySelector("#serviceName").value,price:parseFloat(t.querySelector("#servicePrice").value),duration:parseInt(t.querySelector("#serviceDurationMinutes").value,10),bufferTime:parseInt(t.querySelector("#serviceBufferTimeMinutes").value,10)||0,categoryId:t.querySelector("#serviceCategory").value||null,commissionRate:parseFloat(t.querySelector("#serviceCommissionRate").value)||0,active:t.querySelector("#serviceStatus").value==="true",photo:t.querySelector("#servicePhotoBase64").value,notes:t.querySelector("#serviceNotes").value,commissionType:r,professionalCommissions:o};try{a?(await dr(a,s),Fe(u.establishmentId,je(),"Serviços","Editou",`Editou o serviço: ${s.name}`)):(await Oo(s),Fe(u.establishmentId,je(),"Serviços","Criou",`Criou novo serviço: ${s.name}`)),document.getElementById("serviceModal").style.display="none",g("Sucesso",`Serviço ${a?"atualizado":"adicionado"} com sucesso!`,"success"),await dt()}catch(n){g("Erro",n.message,"error")}}function lo(e=null){const t=document.getElementById("serviceModal"),a=u.serviceCategories||[],o=e?.duration||0,r=e?.bufferTime||0,s=h(e?.name||""),n=h(e?.notes||""),i=e?s:"Novo Serviço",l=a.map(E=>`<option value="${E.id}" ${e?.categoryId===E.id?"selected":""}>${h(E.name)}</option>`).join("");t.innerHTML=`
            ...
            <div class="mt-6 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-3">
    ...
    </div>`,t.style.display="flex",t.addEventListener("click",async E=>{const T=E.target.closest("button[data-action]");if(!T)return;const R=T.dataset.action,O=T.dataset.id;if(R==="close-modal"&&(t.style.display="none"),R==="delete-service"){if(!O)return;if(t.style.display="none",await j("Apagar Serviço","Tem a certeza que deseja apagar este serviço?"))try{const B=u.services.find(N=>N.id===O)?.name||"Desconhecido";await cr(O),Fe(u.establishmentId,je(),"Serviços","Excluiu",`Excluiu o serviço: ${B}`),g("Sucesso","Serviço apagado com sucesso!","success"),await dt()}catch(B){g("Erro",`Não foi possível apagar o serviço: ${B.message}`,"error")}else t.style.display="flex"}});const d=t.querySelectorAll(".tab-btn"),c=t.querySelectorAll(".tab-content");d.forEach(E=>{E.addEventListener("click",()=>{d.forEach(T=>{T.classList.remove("border-indigo-500","text-indigo-600"),T.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),E.classList.add("border-indigo-500","text-indigo-600"),E.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),c.forEach(T=>T.classList.add("hidden")),document.getElementById(`tab-content-${E.dataset.tab}`).classList.remove("hidden")})});const m=t.querySelectorAll('input[name="commissionType"]'),p=document.getElementById("defaultCommissionRateContainer"),b=document.getElementById("professionalCommissionsContainer");function f(){const E=t.querySelector('input[name="commissionType"]:checked').value;p&&(p.style.display=E==="default"?"block":"none"),b&&(b.style.display=E==="custom"?"block":"none")}m.forEach(E=>E.addEventListener("change",f));const v=document.getElementById("professionalCommissionsList");v&&(v.innerHTML=(u.professionals||[]).map(E=>{const T=e?.professionalCommissions?.[E.id]!==void 0,R=e?.professionalCommissions?.[E.id]||0;return`
                <div class="professional-commission-row flex items-center justify-between p-2 rounded-md ${T?"bg-blue-50":""}" data-prof-id="${E.id}">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <img src="${E.photo||`https://placehold.co/40x40/E2E8F0/4A5568?text=${h(E.name.charAt(0))}`}" class="w-8 h-8 rounded-full object-cover">
                        <span class="text-sm font-medium">${h(E.name)}</span>
                    </label>
                    <div class="flex items-center gap-1">
                        <input type="number" value="${R}" class="w-20 p-1 border rounded-md text-sm text-center" ${T?"":"disabled"}>
                        <span class="text-sm font-semibold">%</span>
                    </div>
                </div>
            `}).join(""),v.querySelectorAll('input[type="checkbox"]').forEach(E=>{E.addEventListener("change",T=>{const R=T.target.closest(".professional-commission-row");R.querySelector('input[type="number"]').disabled=!T.target.checked,R.classList.toggle("bg-blue-50",T.target.checked)})})),f();const x=t.querySelector("#serviceForm"),k=t.querySelector("#servicePhotoInput"),C=t.querySelector("#servicePhotoPreview"),S=t.querySelector("#servicePhotoBase64");t.querySelector("#servicePhotoButton").addEventListener("click",()=>k.click()),k.onchange=async()=>{const E=k.files[0];if(E){C.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const T=await Qo(E,800,800,.8),O=T.length*3/4,L=1e3*1024;if(O>L)throw new Error("A imagem é muito grande mesmo após a compressão.");C.src=T,S.value=T}catch(T){console.error("Erro ao processar imagem:",T),g("Erro de Imagem",T.message||"Não foi possível processar a imagem.","error"),C.src=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",S.value=e?.photo||"",k.value=""}}},x.addEventListener("submit",On)}function Pe(){const e=document.getElementById("servicesList");if(!e)return;const t=document.getElementById("serviceSearchInput")?.value.toLowerCase()||"",a=document.getElementById("serviceCategoryFilter")?.value||"all",o=new Map((u.serviceCategories||[]).map(s=>[s.id,s.name]));let r=(u.services||[]).filter(Boolean);if(xe!=="all"){const s=xe==="active";r=r.filter(n=>n.active!==!1===s)}r=r.filter(s=>{const n=s.name.toLowerCase().includes(t),i=a==="all"||s.categoryId===a;return n&&i}),e.innerHTML="",r.length>0?r.forEach(s=>{const n=document.createElement("div"),i=JSON.stringify(s).replace(/'/g,"&apos;");n.className=`service-card bg-white rounded-lg shadow-md flex overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-lg hover:bg-gray-50 ${s.active!==!1?"opacity-100":"opacity-50 bg-gray-100"} sm:flex-col`,n.dataset.action="edit-service",n.dataset.service=i;const l=h(s.name),d=h(o.get(s.categoryId)||"N/A"),c=s.photo||`https://placehold.co/200x200/E2E8F0/4A5568?text=${encodeURIComponent(s.name.charAt(0))}`;n.innerHTML=`
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(u.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${h(a.name)}</option>`)),Da(),Pe()}function Vn(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function dt(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,o,r]=await Promise.all([$e(u.establishmentId),Z(u.establishmentId),_t(u.establishmentId,"services"),mr(u.establishmentId)]);u.services=(t||[]).filter(Boolean),u.professionals=(a||[]).filter(Boolean),u.serviceCategories=(o||[]).filter(Boolean),u.mostPopularService=r||{name:"N/A",count:0},u.services.forEach(s=>{s.active===void 0&&(s.active=!0)}),cs(Ye)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function cs(e){if(document.getElementById("services-content-container")){if(Ye===e&&document.getElementById("services-content-container").children.length>1){Ye==="services"&&(Da(),Pe());return}Ye=e,xe="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?zn():e==="reports"&&Vn()}}function Un(){ne&&(fe.removeEventListener("click",ne),fe.removeEventListener("input",ne),fe.removeEventListener("change",ne)),ne=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),s=r.dataset.id,n=r.checked;try{await ur(s,n);const i=u.services.findIndex(l=>l.id===s);i>-1&&(u.services[i].active=n),Fe(u.establishmentId,je(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${s}) para ${n?"Ativo":"Inativo"}`),Pe(),Da()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,Pe()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){Pe();return}if(!a)return;if(a.hasAttribute("data-view")){cs(a.dataset.view);return}switch(a.dataset.action){case"new-service":lo();break;case"edit-service":const r=JSON.parse(a.dataset.service);lo(r);break;case"manage-categories":Hn();break;case"filter-service":const s=a.dataset.filterType;if(s==="popular")return;xe=s==="total"?"all":s,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,d=i===xe||i==="total"&&xe==="all";n.classList.toggle("ring-2",d),n.classList.toggle("ring-indigo-500",d),n.classList.toggle("shadow-lg",d)}),Pe();break}},fe.addEventListener("click",ne),fe.addEventListener("input",ne),fe.addEventListener("change",ne)}async function _n(){fe.innerHTML=`
        ...
        </section>`,Un();try{(!u.professionals||u.professionals.length===0)&&(u.professionals=await Z(u.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),u.professionals=[]}Ye="services",xe="all",await dt()}const Jt="suppliers",Ma="purchases",us="financial_payables",Aa=async e=>{try{const t=Ft(ue(U,Jt),at("establishmentId","==",e)),a=await ka(t),o=[];return a.forEach(r=>{o.push({id:r.id,...r.data()})}),o}catch(t){throw console.error("Erro ao buscar fornecedores:",t),t}},Jn=async e=>{try{return{id:(await Mo(ue(U,Jt),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},Wn=async(e,t)=>{try{const a=ce(U,Jt,e);return await wa(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},Gn=async e=>{try{const t=ce(U,Jt,e);return await js(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},Yn=async(e,t=null)=>{try{const a=No(U),o=ce(ue(U,Ma)),r={...e,createdAt:Va()};if(a.set(o,r),t&&t.defaultNatureId&&t.defaultCostCenterId){const s=ce(ue(U,us)),n=new Date().toISOString().split("T")[0],i={establishmentId:e.establishmentId,description:`Compra - ${e.supplierName}`,amount:parseFloat(e.totalAmount),dueDate:n,naturezaId:t.defaultNatureId,centroDeCustoId:t.defaultCostCenterId,notes:`Gerado automaticamente pelo Pedido de Compra. Itens: ${e.items.length}`,status:"pending",paymentDate:null,purchaseId:o.id,createdAt:Va()};a.set(s,i)}return await a.commit(),{id:o.id,...r}}catch(a){throw console.error("Erro ao registrar compra com integração:",a),a}},Qn=async(e,t)=>{try{const a=No(U),o=ce(U,Ma,e);a.delete(o);const r=Ft(ue(U,us),at("purchaseId","==",e),at("establishmentId","==",t));return(await ka(r)).forEach(n=>{a.delete(n.ref)}),await a.commit(),!0}catch(a){throw console.error("Erro ao excluir compra e financeiro:",a),a}},Xn=async e=>{try{const t=Ft(ue(U,Ma),at("establishmentId","==",e),Ao("createdAt","desc")),a=await ka(t),o=[];return a.forEach(r=>{o.push({id:r.id,...r.data()})}),o}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},pe=document.getElementById("content");let ie=null,Qe="products",oe="all";async function Zn(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await ls({establishmentId:u.establishmentId,name:o},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await Na(),await ct()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function Kn(e){if(await j("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await ds(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await Na(),await ct()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function Na(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await _t(u.establishmentId,"products");u.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${h(a.name)}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function ei(){G({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
    ...
    <div class="modal-content max-w-3xl overflow-y-auto max-h-[90vh]">
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
    ...
    </div>`;const f=t.querySelector("#productCategory"),v=t.querySelector("#productPhotoInput");t.querySelector("#productPhotoButton").addEventListener("click",()=>v.click()),f.innerHTML='<option value="">Sem categoria</option>'+(u.categories||[]).map(L=>`<option value="${L.id}" ${e?.categoryId===L.id?"selected":""}>${h(L.name)}</option>`).join(""),e&&(f.value=e.categoryId||"");const x=t.querySelector("#productPhotoPreview");t.querySelector("#productPhotoBase64");const k=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",C=e?.photo||"";v.onchange=async()=>{const L=v.files[0];if(L){x.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const B=await co(L,800,800,"image/jpeg",.8),F=B.length*3/4,z=1e3*1024;if(F>z)throw new Error("A imagem é muito grande mesmo após a compressão.");preview.src=B,base64Input.value=B}catch(B){console.error("Erro ao processar imagem:",B),g("Erro de Imagem",B.message||"Não foi possível processar a imagem.","error"),preview.src=k,base64Input.value=C,O.value=""}}};const S=t.cloneNode(!0);t.parentNode.replaceChild(S,t);const E=()=>{const L=S.querySelector("#modalSupplierSearch"),B=S.querySelector("#supplierSearchResults"),N=S.querySelector("#selectedSuppliersList"),F=L.value.toLowerCase();if(F.length>0){const z=o.filter(A=>A.name.toLowerCase().includes(F)&&!s.has(A.id));z.length>0?(B.classList.remove("hidden"),B.innerHTML=z.map(A=>`
                ...
                `).join("")):(B.classList.remove("hidden"),B.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else B.classList.add("hidden");s.size>0?(N.innerHTML="",s.forEach(z=>{const A=o.find(Y=>Y.id===z);A&&(N.innerHTML+=`
                    ...
                    `)})):N.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};S.querySelector("#modalSupplierSearch").addEventListener("input",E),S.addEventListener("click",L=>{const B=L.target.closest("[data-add-supplier]");if(B){const F=B.dataset.addSupplier;s.add(F),S.querySelector("#modalSupplierSearch").value="",E()}const N=L.target.closest("[data-remove-supplier]");if(N){const F=N.dataset.removeSupplier;s.delete(F),E()}}),E(),S.addEventListener("click",async L=>{const B=L.target.closest("button[data-action]");if(!B)return;const N=B.dataset.action,F=S.querySelector("#productId").value;if(N==="close-modal"&&(S.style.display="none"),N==="delete-product"){if(!F)return;S.style.display="none",await ti(F)}if(N==="save-product-modal"){const z=S.querySelector("#productForm");if(z){if(!z.querySelector("#productName").value||!z.querySelector("#productPrice").value){g("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const A=B.closest('button[data-action="save-product-modal"]');A.disabled=!0,A.textContent="A salvar...";try{await ai(z)}catch(Y){g("Erro",`Falha ao salvar: ${Y.message}`,"error"),A.disabled=!1,A.textContent="Salvar Alterações"}}}if(N==="adjust-stock-modal"){L.preventDefault();const z=S.querySelector("#stockAdjustmentAmount"),A=S.querySelector("#stockAdjustmentReason"),Y=parseInt(z.value,10),Ee=parseInt(B.dataset.change,10);if(!Y||Y<=0){g("Erro","Por favor, insira uma quantidade válida.","error");return}const Yt=Y*Ee,Ps=A.value||(Yt>0?"Entrada manual":"Saída manual");try{await br(F,{change:Yt,reason:Ps});const Oe=u.products.findIndex(ze=>ze.id===F);if(Oe>-1){const ze=u.products[Oe].currentStock+Yt;u.products[Oe].currentStock=ze,S.querySelector("#currentStockDisplay").textContent=ze,S.querySelector("#productCurrentStock").value=ze,z.value="",A.value="",g("Sucesso","Estoque atualizado!","success"),qa(),ot()}}catch(Oe){g("Erro de Stock",Oe.message,"error")}}});const T=S.querySelectorAll(".tab-btn"),R=S.querySelectorAll(".tab-content");T.forEach(L=>{L.addEventListener("click",B=>{B.preventDefault(),T.forEach(N=>{N.classList.remove("border-indigo-500","text-indigo-600"),N.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),L.classList.add("border-indigo-500","text-indigo-600"),L.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),R.forEach(N=>N.classList.add("hidden")),document.getElementById(`tab-content-${L.dataset.tab}`).classList.remove("hidden")})});const O=S.querySelector("#productPhotoInput");S.querySelector("#productPhotoButton").addEventListener("click",()=>O.click()),O.onchange=async()=>{const L=O.files[0];if(!L)return;const B=S.querySelector("#productPhotoPreview"),N=S.querySelector("#productPhotoBase64");B.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const F=await co(L,800,800,"image/jpeg",.8),A=F.length*3/4,Y=1e3*1024;if(A>Y)throw new Error("A imagem é muito grande mesmo após a compressão.");B.src=F,N.value=F}catch(F){console.error("Erro ao processar imagem:",F),g("Erro de Imagem",F.message||"Não foi possível processar a imagem.","error"),B.src=k,N.value=C,O.value=""}},S.style.display="flex"}function oi(){const e=document.getElementById("products-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(u.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${h(a.name)}</option>`)),qa(),ot()}function si(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const o=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
        ...
        </div>`;const r=document.getElementById("productFilterReport"),s=document.getElementById("categoryFilterReport");r&&u.products&&(r.innerHTML+=u.products.map(n=>`<option value="${n.id}">${h(n.name)}</option>`).join("")),s&&u.categories&&(s.innerHTML+=u.categories.map(n=>`<option value="${n.id}">${h(n.name)}</option>`).join(""))}async function ri(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value,establishmentId:u.establishmentId};try{const a=await fr(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function ct(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,o]=await Promise.all([Ht(u.establishmentId),_t(u.establishmentId,"products"),Aa(u.establishmentId)]);u.products=(t||[]).filter(Boolean),u.categories=(a||[]).filter(Boolean),u.suppliers=(o||[]).filter(Boolean),ms(Qe)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function ms(e){if(document.getElementById("products-content-container")){if(Qe===e&&document.getElementById("products-content-container").children.length>1){Qe==="products"&&(qa(),ot());return}Qe=e,oe="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?oi():e==="movements"&&si()}}async function ni(){pe.innerHTML=`
        ...
        </section>`,ie&&(pe.removeEventListener("click",ie),pe.removeEventListener("input",ie),pe.removeEventListener("change",ie)),ie=async e=>{const t=e.target;if(t.id==="productSearchInput"||t.id==="productCategoryFilter"){ot();return}const a=e.target.closest("button[data-action], button[data-view], .indicator-card[data-action], .product-card[data-action]");if(!a||e.target.closest('[data-action-stop-propagation="true"]'))return;if(a.hasAttribute("data-view")){ms(a.dataset.view);return}switch(a.dataset.action){case"new-product":uo();break;case"edit-product":uo(JSON.parse(a.dataset.product));break;case"manage-product-categories":ei();break;case"generate-report":await ri();break;case"filter-stock":const r=a.dataset.filterType;oe=oe===r?"all":r,document.querySelectorAll(".indicator-card").forEach(s=>{s.classList.toggle("ring-2",s.dataset.filterType===oe),s.classList.toggle("ring-indigo-500",s.dataset.filterType===oe),s.classList.toggle("shadow-lg",s.dataset.filterType===oe)}),ot();break}},pe.addEventListener("click",ie),pe.addEventListener("input",ie),pe.addEventListener("change",ie),Qe="products",oe="all",await ct()}const ge=document.getElementById("content");let le=null,Et="list",H={step:1,productsToBuy:[],allSuppliers:[],finalOrders:{},isQuoteMode:!1};async function ii(){Et==="list"?Wt():Et==="purchases"?(H.step=1,Xe()):Et==="history"&&ps()}async function li(){try{const e=await Aa(u.establishmentId);return u.suppliers=e||[],H.allSuppliers=e,!0}catch(e){return console.error(e),!1}}async function di(e){if(await j("Excluir Fornecedor","Tem a certeza? Isso removerá o vínculo com os produtos."))try{await Gn(e),g("Sucesso","Fornecedor excluído.","success"),jt("genericModal"),Wt()}catch(t){g("Erro","Erro ao excluir: "+t.message,"error")}}async function ci(e){e.preventDefault();const t=e.target,a=t.querySelector("#supId").value,o={name:t.querySelector("#supName").value,contactName:t.querySelector("#supContact").value,email:t.querySelector("#supEmail").value,phone:t.querySelector("#supPhone").value,taxId:t.querySelector("#supTaxId").value,category:t.querySelector("#supCategory").value,establishmentId:u.establishmentId},r=t.querySelector('button[type="submit"]');r.disabled=!0,r.textContent="A salvar...";try{a?(await Wn(a,o),g("Sucesso","Fornecedor atualizado!","success")):(await Jn(o),g("Sucesso","Fornecedor criado!","success")),jt("genericModal"),Wt()}catch(s){g("Erro","Erro ao salvar: "+s.message,"error")}finally{r.disabled=!1,r.textContent="Salvar"}}async function Wt(){const e=document.getElementById("suppliersList");if(!e)return;e.innerHTML='<div class="loader mx-auto my-8"></div>',await li();const t=document.getElementById("supplierSearchInput")?.value.toLowerCase()||"",a=u.suppliers.filter(s=>s.name.toLowerCase().includes(t)||s.contactName&&s.contactName.toLowerCase().includes(t));if(e.innerHTML="",a.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum fornecedor encontrado.</div>';return}let o='<div class="flex flex-col gap-2 md:hidden">';a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;"),i=h(s.name),l=h(s.category||"Geral"),d=h(s.contactName||"");o+=`
    ...
    `;a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;"),i=h(s.name),l=h(s.taxId||"Sem doc."),d=h(s.email||"-"),c=h(s.phone||"-"),m=h(s.category||"Geral");r+=`
            <tr class="hover:bg-gray-50">
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=o+r}function ui(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",o=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),s=h(e.name),n=h(e.category||"Fornecedor"),i=h(e.contactName||""),l=h(e.phone||""),d=`
    ...
    `;G({title:"",contentHTML:d,maxWidth:"max-w-md"})}async function Xe(){const e=document.getElementById("purchasesContainer");if(e)if(H.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Ht(u.establishmentId),Aa(u.establishmentId)]);H.allSuppliers=a||[];const o=t.filter(d=>{const c=parseInt(d.currentStock||0),m=parseInt(d.minStock||0);return c<=m});if(H.productsToBuy=o,o.length===0){e.innerHTML=`
                ...
                `;return}let r='<div class="flex flex-col gap-3 md:hidden">',s="";o.forEach(d=>{const c=parseInt(d.minStock)||0,m=parseInt(d.currentStock)||0,p=Math.max(c-m,1),b=parseFloat(d.costPrice||0),f=h(d.name);let v='<option value="">Selecione...</option>';H.allSuppliers.length>0?H.allSuppliers.forEach(x=>{const C=d.supplierIds&&d.supplierIds.includes(x.id)?"selected":"";v+=`<option value="${x.id}" ${C}>${h(x.name)}</option>`}):v='<option value="">Sem fornecedores</option>',r+=`
            ...
            `,ba()}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao calcular compras.</p>'}}else H.step===2&&mi(e)}function mi(e){if(!H.finalOrders||Object.keys(H.finalOrders).length===0){H.step=1,Xe();return}const t=H.isQuoteMode;let a="",o=0;const r=t?"border-indigo-100":"border-gray-200",s=t?"bg-indigo-50 border-indigo-100":"bg-gray-50 border-gray-200",n=t?"bg-blue-100 text-blue-700":"bg-green-100 text-green-700",i=t?"hidden":"flex",l=t?"Cotações Prontas":"Pedidos Prontos",d=t?"text-indigo-600":"text-green-600",c=t?"bg-indigo-50 border-indigo-100":"bg-green-50 border-green-100",m=t?"text-indigo-800":"text-green-800";for(const[p,b]of Object.entries(H.finalOrders)){let f=0,v=b.items.map(T=>{const R=T.qty*T.cost;return f+=R,`
            <div class="flex justify-between py-2 border-b border-gray-50 text-sm">
                <span class="text-gray-800 font-medium">${h(T.name)}</span>
                <div class="text-right">
        ...
        `}).join("");o+=f;const x=encodeURIComponent(JSON.stringify({supplierId:p,supplierName:b.info.name,totalAmount:f,items:b.items})),k=encodeURIComponent(JSON.stringify({name:b.info.name,phone:b.info.phone,email:b.info.email})),C=encodeURIComponent(JSON.stringify(b.items)),S=h(b.info.name),E=h(b.info.email||"");a+=`
    ...
    `}async function ps(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await Xn(u.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(s=>{const n=new Date(s.createdAt.seconds*1e3).toLocaleDateString("pt-BR"),i=h(s.supplierName);a+=`
            ...
            <tr class="hover:bg-gray-50 border-b border-gray-100">
                ...
                <td class="p-3 font-medium text-gray-800">${h(s.supplierName)}</td>
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function pi(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;G({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",ci),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>jt("genericModal"))},50)}function bi(){ge.innerHTML=`
    ...
    `,le&&(ge.removeEventListener("click",le),ge.removeEventListener("input",le),ge.removeEventListener("change",le)),le=e=>{if(e.target.closest("#tab-btn-list")&&pt("list"),e.target.closest("#tab-btn-purchases")&&pt("purchases"),e.target.closest("#tab-btn-history")&&pt("history"),e.target.id==="toggle-quote-mode"&&(H.isQuoteMode=e.target.checked,Xe()),e.target.id==="supplierSearchInput"&&Wt(),e.target.closest("#btn-new-supplier")&&mo(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),o=JSON.parse(a.dataset.supplier);ui(o)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&di(t.dataset.id),a==="edit"&&mo(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&ba(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(o=>o.checked=a),ba()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),o={};let r=!1;if(a.forEach(s=>{if(s.offsetParent===null||!s.querySelector(".row-select").checked)return;r=!0;let i="Produto";const l=s.querySelector("td:nth-child(2)"),d=s.querySelector(".font-bold");l?i=l.innerText:d&&(i=d.innerText);const c=parseInt(s.querySelector(".qty-input").value),m=parseFloat(s.dataset.cost),b=s.querySelector(".supplier-select").value;if(b){if(!o[b]){const f=H.allSuppliers.find(v=>v.id===b);o[b]={info:f,items:[]}}o[b].items.push({name:i,qty:c,cost:m})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}H.finalOrders=o,H.step=2,Xe()}if(e.target.closest("#btn-back-step1")&&(H.step=1,Xe()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),o=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),s=parseFloat(a.dataset.total),n=H.isQuoteMode;if(o.phone){const i=o.phone.replace(/\D/g,"");let l="";n?(l=`Olá *${o.name}*, tudo bem?
...
Aguardo retorno.`;const d=`mailto:${o.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order");if(a.disabled)return;const o=JSON.parse(decodeURIComponent(a.dataset.order));o.establishmentId=u.establishmentId,a.disabled=!0,a.textContent="A processar...",Se(u.establishmentId).then(r=>{const s=r.purchaseConfig||null;return Yn(o,s)}).then(()=>{g("Sucesso","Compra registrada e integrada ao financeiro!","success"),a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{a.disabled=!1,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Salvar',g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-delete-purchase")){const o=e.target.closest(".btn-delete-purchase").dataset.id;j("Excluir Compra","Isto apagará o registo histórico E o lançamento financeiro associado. Deseja continuar?").then(async r=>{if(r)try{await Qn(o,u.establishmentId),g("Sucesso","Compra e financeiro excluídos.","success"),ps()}catch(s){g("Erro","Erro ao excluir: "+s.message,"error")}})}if(e.target.closest(".btn-print-order")){const o=e.target.closest(".supplier-order-card").dataset.supplierId,r=H.finalOrders[o];r?gi(r,H.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),o=JSON.parse(a.dataset.purchase);pi(o)}},ge.addEventListener("click",le),ge.addEventListener("input",le),ge.addEventListener("change",le),pt("list")}function pt(e){Et=e,["list","purchases","history"].forEach(a=>{const o=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(o.classList.add("border-indigo-500","text-indigo-600"),o.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(o.classList.remove("border-indigo-500","text-indigo-600"),o.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),ii()}const Zt=document.getElementById("content"),po={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let re=new Set,gt=null,Be=null;function fi(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function vi(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",o=h(t.name),r=h(t.specialty||"Especialidade"),s=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,n=JSON.stringify(t).replace(/'/g,"&apos;");return`
        ...
        </div>`;t.innerHTML=n,t.style.display="flex",xi(a,r),yi(a),wi(a,s),Si(a)}function xi(e,t){const a=document.getElementById("professionalForm"),o=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(x,k)=>{const C=k+1,S=C==o[1]?"selected":"",E=new Date(0,k).toLocaleString("pt-BR",{month:"long"});return`<option value="${C}" ${S}>${E.charAt(0).toUpperCase()+E.slice(1)}</option>`}).join(""),s=e.status||"active",n=h(e.name||""),i=h(e.specialty||""),l=h(e.phone||""),d=h(e.notes||"");a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${d}</textarea></div>`;const c=document.getElementById("profPhotoInput"),m=document.getElementById("profPhotoButton"),p=document.getElementById("profPhotoPreview"),b=document.getElementById("profPhotoBase64"),f=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,v=e.photo||"";m&&m.addEventListener("click",()=>c.click()),c&&(c.onchange=async()=>{const x=c.files[0];if(x){p.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const k=await Qo(x,800,800,.8),S=k.length*3/4,E=1e3*1024;if(S>E)throw new Error("A imagem é muito grande mesmo após a compressão.");p.src=k,b.value=k}catch(k){g("Erro de Imagem",k.message||"Não foi possível processar a imagem.","error"),p.src=f,b.value=v,c.value=""}}})}function yi(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',ki(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function wi(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
            ...
            <div>
                ...
                <form id="batchBlockageForm" class="p-4 bg-white rounded-lg shadow-inner space-y-3 mb-4">
                    ...
                    <div id="batchProfSelectionContainer" class="max-h-32 overflow-y-auto p-2 border rounded-md space-y-2">
                        ${t.map(s=>`<label class="flex items-center"><input type="checkbox" name="batch-professionals" value="${s.id}" class="rounded mr-2" ${s.id===e.id?"checked":""}><span>${h(s.name)}</span></label>`).join("")}
                    </div>
                    <div class="grid grid-cols-2 gap-2">
        ...
        `).join("")}catch(o){a.innerHTML=`<p class="text-red-500">${o.message}</p>`}}}function Si(e){const t=document.getElementById("genericModal");Be&&t.removeEventListener("click",Be),Be=async a=>{const o=a.target.closest("button[data-action]");if(!o){const s=a.target.closest(".tab-link");s&&(t.querySelectorAll(".tab-link").forEach(n=>n.classList.remove("active")),s.classList.add("active"),t.querySelectorAll(".tab-content").forEach(n=>n.classList.add("hidden")),document.getElementById(s.dataset.tab).classList.remove("hidden"));return}const r=o.dataset.action;switch(a.stopPropagation(),r){case"close-modal":Kt();break;case"delete-professional":const s=o.dataset.id;if(await j("Excluir Profissional",`Tem certeza que deseja excluir ${e.name}? Esta ação não pode ser desfeita.`))try{await Ho(s),g("Sucesso!","Profissional excluído.","success"),Kt(),Bt()}catch(v){g("Erro",`Não foi possível excluir: ${v.message}`,"error")}break;case"save-professional":const i=document.getElementById("professionalForm"),l=o,d=document.getElementById("profScheduleContainer"),c=Array.from(i.querySelectorAll("#profServicesContainer input:checked")).map(v=>v.value),m={};d&&d.querySelectorAll(".day-schedule-card").forEach(v=>{const x=v.querySelector('[data-field="active"]').dataset.day;m[x]={active:v.querySelector('[data-field="active"]').checked,start:v.querySelector('[data-field="start"]').value,end:v.querySelector('[data-field="end"]').value,breakStart:v.querySelector('[data-field="breakStart"]').value,breakEnd:v.querySelector('[data-field="breakEnd"]').value}});const p={...e,id:i.querySelector("#professionalId").value||void 0,name:i.querySelector("#profName").value,specialty:i.querySelector("#profSpecialty").value,photo:i.querySelector("#profPhotoBase64").value,services:c,workingHours:m,phone:i.querySelector("#profPhone").value,dob:`${i.querySelector("#profDobDay").value}/${i.querySelector("#profDobMonth").value}`,receivesCommission:i.querySelector("#profCommission").value==="sim",showOnAgenda:i.querySelector("#profShowOnAgenda").value==="sim",orderOnAgenda:parseInt(i.querySelector("#profOrderOnAgenda").value)||1,notes:i.querySelector("#profNotes").value,status:i.querySelector("#profStatus").value,establishmentId:u.establishmentId};l.disabled=!0,l.textContent="A salvar...";try{p.id?(await Lt(p.id,p),g("Sucesso!","Profissional atualizado.","success")):(delete p.id,await jo(p),g("Sucesso!","Profissional criado.","success")),Kt(),Bt()}catch(v){g("Erro",v.message,"error"),l.disabled=!1,l.textContent="Salvar"}break;case"delete-blockage":const b=o.dataset.id;if(await j("Apagar Bloqueio","Tem certeza?"))try{await Ia(b),g("Bloqueio removido.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";Ze(e.id,v)}catch(v){g("Erro",v.message,"error")}break;case"batch-delete-blockage":const f=JSON.parse(o.dataset.ids);if(await j("Apagar em Lote",`Tem certeza que deseja apagar ${f.length} bloqueios com este motivo?`))try{await Go(f),g("Bloqueios removidos.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";Ze(e.id,v)}catch(v){g("Erro",v.message,"error")}break}},t.addEventListener("click",Be)}function fa(){const e=document.getElementById("batch-actions-container"),t=document.getElementById("selected-count");!e||!t||(re.size>0?(t.textContent=`${re.size} selecionado(s)`,e.classList.remove("hidden")):e.classList.add("hidden"))}function $i(){j("Excluir em Lote",`Tem certeza que deseja excluir ${re.size} profissionais? Esta ação não pode ser desfeita.`).then(async e=>{if(e)try{await lr(Array.from(re)),g("Sucesso!",`${re.size} profissionais foram excluídos.`,"success"),re.clear(),fa(),Bt()}catch(t){g("Erro",`Não foi possível excluir em lote: ${t.message}`,"error")}})}function Ve(){const e=document.getElementById("professionalsList");if(!e)return;if(!u.professionals){e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=fi();return}const t=document.getElementById("showInactiveProfToggle").checked,a=document.getElementById("profSearchInput").value.toLowerCase(),o=u.professionals.filter(r=>{const s=r.name.toLowerCase().includes(a),n=t||r.status!=="inactive";return s&&n});e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=vi(o)}async function Bt(){re.clear(),Zt.innerHTML=`
        ...
        </section>`,gt&&Zt.removeEventListener("click",gt),gt=t=>{const a=t.target.closest('[data-action="open-professional-modal"]'),o=t.target.closest('[data-action="batch-delete"]');if(a){t.preventDefault();let s={};if(a.dataset.professional)try{s=JSON.parse(a.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}hi(s);return}if(o){$i();return}const r=t.target.closest(".professional-checkbox");if(r){const s=r.dataset.id;r.checked?re.add(s):re.delete(s),Ve(),fa();return}},Zt.addEventListener("click",gt),document.getElementById("profSearchInput").addEventListener("input",Ve),document.getElementById("showInactiveProfToggle").addEventListener("change",Ve);const e=document.getElementById("professionalsList");u.professionals=null,u.services=null,Ve();try{const[t,a]=await Promise.all([Z(u.establishmentId),$e(u.establishmentId)]);u.professionals=t,u.services=a,Ve(),fa()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}let P={clients:[],establishment:null,searchTimeout:null,currentClient:null,history:[],historyLimit:10,filters:{hasLoyalty:!1,birthMonth:"",inactiveDays:""},showFilters:!1,selectionMode:!1,selectedClients:new Set};const gs=e=>e?e.replace(/\D/g,""):"",Dt=e=>{if(!e)return"";const t=e.replace(/\D/g,"");return t.length>10?t.replace(/^(\d\d)(\d{5})(\d{4}).*/,"($1) $2-$3"):t.replace(/^(\d\d)(\d{4})(\d{0,4}).*/,"($1) $2-$3")},Ei=e=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(e||0),va=e=>{if(!e)return"-";try{const t=new Date(e);return isNaN(t.getTime())?"-":t.toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",year:"2-digit",hour:"2-digit",minute:"2-digit"})}catch{return"-"}},Ii=()=>{P.showFilters=!P.showFilters;const e=document.getElementById("filter-panel");e&&e.classList.toggle("hidden",!P.showFilters)},bs=()=>{P.selectionMode=!P.selectionMode,P.selectedClients.clear(),hs(),Ra()},fs=e=>{P.selectedClients.has(e)?P.selectedClients.delete(e):P.selectedClients.add(e),Ra();const t=document.getElementById(`card-${e}`);if(t){const a=t.querySelector(".client-checkbox");a&&(a.checked=P.selectedClients.has(e)),P.selectedClients.has(e)?t.classList.add("ring-2","ring-indigo-500","bg-indigo-50"):t.classList.remove("ring-2","ring-indigo-500","bg-indigo-50")}},Ra=()=>{const e=document.getElementById("header-normal"),t=document.getElementById("header-selection"),a=document.getElementById("selected-count");P.selectionMode?(e.classList.add("hidden"),t.classList.remove("hidden"),a&&(a.textContent=`${P.selectedClients.size} selecionado(s)`)):(e.classList.remove("hidden"),t.classList.add("hidden"))},Ci=async()=>{const e=P.selectedClients.size;if(e!==0&&await j("Excluir Clientes",`Tem certeza que deseja excluir ${e} clientes selecionados? Esta ação não pode ser desfeita.`)){const t=document.getElementById("btn-bulk-delete");t&&(t.disabled=!0,t.textContent="Excluindo...");try{const a=Array.from(P.selectedClients).map(o=>Yo(o));await Promise.all(a),g("Sucesso",`${e} clientes excluídos com sucesso!`,"success"),bs(),setTimeout(()=>me(),500)}catch(a){console.error(a),g("Erro","Ocorreu um erro ao excluir alguns clientes.","error"),t&&(t.disabled=!1,t.textContent="Excluir")}}},Li=()=>{const e=document.getElementById("filter-loyalty"),t=document.getElementById("filter-month"),a=document.getElementById("filter-inactive");P.filters.hasLoyalty=e?e.checked:!1,P.filters.birthMonth=t?t.value:"",P.filters.inactiveDays=a?a.value:"",me()},Ti=()=>{const e=document.getElementById("filter-loyalty"),t=document.getElementById("filter-month"),a=document.getElementById("filter-inactive");e&&(e.checked=!1),t&&(t.value=""),a&&(a.value=""),P.filters={hasLoyalty:!1,birthMonth:"",inactiveDays:""},me()},vs=async(e=null)=>{P.currentClient=e,P.historyLimit=10;const t=!e;if(G({title:"",contentHTML:(o=>`
        <div class="h-[80vh] flex flex-col bg-gray-50 rounded-lg overflow-hidden">
            <div class="bg-white border-b px-6 py-4 flex justify-between items-center shrink-0">
                <div>
                    ...
                    ${t?"":`<p class="text-sm text-gray-500">${Dt(o.phone)}</p>`}
                </div>
                ${t?"":`
    ...
    `)(e||{}),maxWidth:"max-w-3xl"}),window.switchTab=o=>It(o,t),window.handleSave=Pi,window.handleDelete=Bi,window.loadMoreHistory=()=>{P.historyLimit+=10,It("history",t)},!t&&e.id)try{if(typeof da=="function"){const o=await da(u.establishmentId,e.id);if(o){P.currentClient=o;const r=document.querySelector(".bg-indigo-100.text-indigo-700");r&&(r.textContent=`${o.loyaltyPoints||0} pts`)}}P.history=await qr(u.establishmentId,e.phone)}catch(o){console.error("Aviso: Não foi possível atualizar detalhes do cliente",o),P.history=[]}It("details",t)},It=(e,t)=>{const a=document.getElementById("modal-body");if(!a)return;document.querySelectorAll('[id^="tab-btn-"]').forEach(s=>{s.classList.remove("border-indigo-600","text-indigo-600"),s.classList.add("border-transparent","text-gray-500")});const o=document.getElementById(`tab-btn-${e}`);o&&(o.classList.add("border-indigo-600","text-indigo-600"),o.classList.remove("border-transparent","text-gray-500"));const r=P.currentClient||{};if(e==="details"){if(a.innerHTML=`
        ...
        `,t){const s=document.getElementById("input-phone");s&&(s.oninput=n=>n.target.value=Dt(n.target.value))}}else if(e==="appointments"){const s=new Date,n=P.history.filter(i=>{const l=new Date(i.date);return i.type==="appointment"&&l>=s&&i.status!=="cancelled"&&i.status!=="cancelado"&&i.status!=="completed"&&i.status!=="finalizado"});if(n.sort((i,l)=>new Date(i.date)-new Date(l.date)),!n.length){a.innerHTML=`<div class="text-center py-10">
        ...
        `}else if(e==="history"){const s=[...P.history].filter(l=>l.type==="appointment"&&(l.status==="completed"||l.status==="finalizado")).sort((l,d)=>new Date(d.date)-new Date(l.date)),n=s.slice(0,P.historyLimit),i=s.length>P.historyLimit;if(!n.length){a.innerHTML='<div class="text-center text-gray-400 mt-10">Nenhum histórico de agendamento finalizado.</div>';return}a.innerHTML=`
            <div class="space-y-3">
                ${n.map(l=>`
            ...
            <div class="space-y-2">
                ${l.map(d=>{const c=d.costPoints||d.points||0,m=n>=c,p=d.reward||d.name;return`
                        <div class="flex justify-between items-center p-3 border rounded ${m?"bg-green-50 border-green-200":"bg-gray-50 opacity-70"}">
                            <div>
                                <p class="font-bold text-gray-800">${h(p)}</p>
                                <p class="text-xs text-gray-500">${c} pontos necessários</p>
                            </div>
                            <button onclick="window.handleRedeem('${c}', '${h(p)}')" ${m?"":"disabled"} 
        ...
        `,window.handleRedeem=async(d,c)=>{if(await j("Resgatar",`Trocar ${d} pontos por "${c}"?`))try{await Rr(u.establishmentId,r.phone,d,c),g("Sucesso","Resgate realizado!","success"),r.loyaltyPoints=(r.loyaltyPoints||0)-parseInt(d),P.history.unshift({type:"loyalty",description:`Resgate: ${c}`,date:new Date().toISOString(),value:-d,isPoints:!0,status:"completed"}),It("loyalty",!1),me()}catch(m){g("Erro",m.message||"Erro ao resgatar","error")}}}},Pi=async()=>{const e=document.getElementById("form-client"),t=new FormData(e),a=t.get("phone").replace(/\D/g,"");if(a.length<10){g("Erro","Telefone inválido","error");return}const o={name:t.get("name"),phone:a,email:t.get("email"),birthDate:t.get("birthDate"),notes:t.get("notes"),establishmentId:u.establishmentId};try{await Nr(o),g("Sucesso","Cliente salvo!","success"),document.getElementById("genericModal").style.display="none",me()}catch(r){g("Erro","Erro ao salvar cliente","error"),console.error(r)}},Bi=async()=>{if(await j("Excluir","Tem certeza? Isso apagará o histórico deste cliente."))try{const e=P.currentClient.id||gs(P.currentClient.phone);await Yo(e),g("Sucesso","Cliente removido","success"),document.getElementById("genericModal").style.display="none",setTimeout(()=>me(),500)}catch(e){g("Erro","Erro ao remover","error"),console.error(e)}},hs=()=>{const e=document.getElementById("clients-grid");if(e){if(e.innerHTML="",P.clients.length===0){e.innerHTML=`
        ...
        `;return}P.clients.forEach(t=>{const a=document.createElement("div"),o=t.id||gs(t.phone),r=P.selectedClients.has(o);a.id=`card-${o}`,a.className=`bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group relative ${r?"ring-2 ring-indigo-500 bg-indigo-50":""}`,a.onclick=l=>{l.target.type!=="checkbox"&&(P.selectionMode?fs(o):vs(t))};const s=t.lastVisit||t.lastServiceDate||t.lastAppointmentDate;let n="Nunca visitou";if(s){const l=va(s);l&&l!=="-"&&(n=`Última visita: ${l.split(" ")[0]}`)}const i=P.selectionMode?`
        ...
        `,e.appendChild(a)})}},me=async(e="")=>{const t=document.getElementById("clients-grid");t&&(t.innerHTML='<div class="col-span-full flex justify-center py-10"><div class="loader"></div></div>');const a=e||document.getElementById("search-input")?.value||"";try{if(P.clients=await nt(u.establishmentId,a,100,P.filters),P.filters.inactiveDays){const o=parseInt(P.filters.inactiveDays,10);if(!isNaN(o)&&o>0){const r=new Date,s=new Date(r.setDate(r.getDate()-o));P.clients=P.clients.filter(n=>{const i=n.lastVisit||n.lastServiceDate;if(!i)return!0;const l=new Date(i);return isNaN(l.getTime())?!0:l<s})}}hs(),Ra()}catch(o){console.error(o),t&&(t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao carregar lista.</p>')}},Di=async()=>{const e=document.getElementById("content");e.innerHTML=`
    ...
    `,window.openNewClient=()=>vs(null),window.toggleFilterPanel=Ii,window.toggleSelectionMode=bs,window.toggleClientSelection=fs,window.handleBulkDelete=Ci,window.applyFilters=Li,window.clearFilters=Ti,window.navigateTo=ae;const t=document.getElementById("search-input");t&&t.addEventListener("input",a=>{clearTimeout(P.searchTimeout),P.searchTimeout=setTimeout(()=>{me(a.target.value)},400)});try{const[a]=await Promise.all([Se(u.establishmentId),me()]);P.establishment=a}catch(a){console.error("Erro inicialização clientes",a)}},st=e=>w(`/api/financial/natures/${e}`),Mi=e=>w("/api/financial/natures",{method:"POST",body:JSON.stringify(e)}),Ai=e=>w(`/api/financial/natures/${e}`,{method:"DELETE"}),rt=e=>w(`/api/financial/cost-centers/${e}`),Ni=e=>w("/api/financial/cost-centers",{method:"POST",body:JSON.stringify(e)}),qi=e=>w(`/api/financial/cost-centers/${e}`,{method:"DELETE"}),xs=(e,t)=>w(`/api/financial/${e}`,{method:"POST",body:JSON.stringify(t)}),ys=(e,t={})=>{let a=`/api/financial/${e}`;const o=new URLSearchParams;t.establishmentId&&o.append("establishmentId",t.establishmentId),t.startDate&&o.append("startDate",t.startDate),t.endDate&&o.append("endDate",t.endDate),t.natureId&&o.append("natureId",t.natureId),t.costCenterId&&o.append("costCenterId",t.costCenterId),t.status&&o.append("status",t.status);const r=o.toString();return r&&(a+=`?${r}`),w(a)},ws=(e,t,a)=>w(`/api/financial/${e}/${t}`,{method:"PUT",body:JSON.stringify(a)}),ks=(e,t)=>w(`/api/financial/${e}/${t}`,{method:"DELETE"}),Ss=(e,t,a)=>w(`/api/financial/${e}/${t}/status`,{method:"PATCH",body:JSON.stringify({status:"paid",paymentDate:a})}),Ri=e=>xs("payables",e),Fi=e=>ys("payables",e),ji=(e,t)=>ws("payables",e,t),Hi=e=>ks("payables",e),Oi=(e,t)=>Ss("payables",e,t),zi=e=>xs("receivables",e),Vi=e=>ys("receivables",e),Ui=(e,t)=>ws("receivables",e,t),_i=e=>ks("receivables",e),Ji=(e,t)=>Ss("receivables",e,t),Wi=(e,t,a)=>w(`/api/financial/cash-flow?establishmentId=${e}&startDate=${t}&endDate=${a}`),Gi=e=>w(`/api/financial/today-summary/${e}`),ve=document.getElementById("content"),ea={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"},Yi={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}},$s=[{id:"personal-data",icon:"M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z",label:"Dados Gerais"},{id:"branding",icon:"M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z",label:"Identidade e Cores"},{id:"booking",icon:"M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z",label:"Agendamento Online"},{id:"working-hours",icon:"M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z",label:"Horário de Funcionamento"},{id:"loyalty",icon:"M5 5a2 2 0 012-2h10a2 2 0 012 2v1h2a1 1 0 011 1v3a1 1 0 01-1 1h-2v1a2 2 0 01-2 2H7a2 2 0 01-2-2v-1H3a1 1 0 01-1-1V7a1 1 0 011-1h2V5z",label:"Plano de Fidelidade"},{id:"financial",icon:"M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z",label:"Integração Financeira"},{id:"change-password",icon:"M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z",label:"Alterar senha"},{id:"change-email",icon:"M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207",label:"Alterar E-mail de Acesso"},{id:"support",icon:"M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z",label:"Suporte e Ajuda"},{id:"cancellation",icon:"M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z",label:"Cancelar Assinatura"}];let J=null;function go(e,t,a){return new Promise((o,r)=>{const s=new FileReader;s.readAsDataURL(e),s.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const l=document.createElement("canvas");let d=i.width,c=i.height;d>t&&(c*=t/d,d=t),l.width=d,l.height=c,l.getContext("2d").drawImage(i,0,0,d,c);const p=e.type==="image/png"&&t<500?"image/png":"image/jpeg";o(l.toDataURL(p,a))},i.onerror=l=>r(l)},s.onerror=n=>r(n)})}function Ce(e,t=null){let a='<option value="">-- Selecione (Opcional) --</option>';const o=n=>{const i=new Map,l=[];return n&&(n.forEach(d=>i.set(d.id,{...d,children:[]})),i.forEach(d=>{d.parentId&&i.has(d.parentId)?i.get(d.parentId).children.push(d):l.push(d)})),l},r=(n,i="")=>{const l=n.id===t?"selected":"";a+=`<option value="${n.id}" ${l}>${i}${h(n.name)}</option>`,n.children.forEach(d=>r(d,i+"— "))};return o(e).forEach(n=>r(n)),a}async function He(e,t){const a=t.target.querySelector('button[type="submit"]');a&&(a.disabled=!0,a.textContent="A Salvar...");try{const o=J||await Se(u.establishmentId),r=[],{ownerName:s,...n}=e;if(s&&s!==u.userName){const l=V.currentUser;l&&r.push(Ns(l,{displayName:s}).then(()=>{u.userName=s}))}const i={...o,...n};if(r.push(St(u.establishmentId,i)),await Promise.all(r),J=i,g("Sucesso","Definições salvas com sucesso! A página será recarregada para aplicar o novo tema.","success"),n.themeColor)setTimeout(()=>window.location.reload(),1500);else{const l=document.getElementById("panelEstablishmentName");n.name&&l&&(l.textContent=n.name,u.establishmentName=n.name)}}catch(o){g("Erro",`Não foi possível salvar: ${o.message}`,"error")}finally{a&&(a.disabled=!1,a.textContent="Salvar")}}function Qi(e,t){const a=h(e.name||""),o=h(e.phone||""),r=h(e.document||""),s=h(e.email||""),n=h(e.address||""),i=h(e.website||""),l=h(u.userName||"");t.innerHTML=`
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!o||!r){g("Erro","Preencha todos os campos.","error");return}const s=t.querySelector('button[form="change-email-form"]');s.disabled=!0,s.textContent="A verificar...";try{const n=V.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Bs.credential(n.email,r);await Ds(n,i),s.textContent="A enviar link...",await Ms(n,o),s.textContent="A atualizar BD...",await nr(u.establishmentId,o),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{s.disabled=!1,s.textContent="Salvar Novo E-mail"}})}function Ki(e,t){const a=h(e.welcomeMessage||"");t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#loyaltyEnabled").checked=a.enabled||!1;const n=t.querySelectorAll('input[name="loyaltyType"]'),i=t.querySelector("#loyalty-config-amount"),l=t.querySelector("#loyalty-config-visit");n.forEach(m=>{m.addEventListener("change",p=>{const b=p.target.value;n.forEach(f=>{const v=f.closest("label");f.checked?v.classList.add("border-indigo-500","bg-indigo-50","ring-1","ring-indigo-500"):v.classList.remove("border-indigo-500","bg-indigo-50","ring-1","ring-indigo-500")}),b==="amount"?(i.classList.remove("hidden"),l.classList.add("hidden")):(i.classList.add("hidden"),l.classList.remove("hidden"))})});const d=t.querySelector("#loyaltyTiersContainer"),c=(m={})=>{const p=document.createElement("div"),b=h(m.reward||"");return p.className="loyalty-tier-row group bg-white md:bg-transparent p-3 md:p-0 border md:border-0 rounded-lg shadow-sm md:shadow-none relative",p.innerHTML=`
    ...
    `;try{const[a,o]=await Promise.all([st(u.establishmentId),rt(u.establishmentId)]),r=e.financialIntegration||{},s=e.commissionConfig||{},n=e.purchaseConfig||{};t.querySelector("#financialNatureId").innerHTML=Ce(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=Ce(o,r.defaultCentroDeCustoId),t.querySelector("#purchaseNatureId").innerHTML=Ce(a,n.defaultNatureId),t.querySelector("#purchaseCostCenterId").innerHTML=Ce(o,n.defaultCostCenterId),t.querySelector("#commissionNatureId").innerHTML=Ce(a,s.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=Ce(o,s.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const o={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},purchaseConfig:{defaultNatureId:t.querySelector("#purchaseNatureId").value||null,defaultCostCenterId:t.querySelector("#purchaseCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};He(o,a)})}function sl(e,t){const a="5516997859430",o=encodeURIComponent("Olá, preciso de ajuda com o sistema Kairos."),r=`https://wa.me/${a}?text=${o}`;t.innerHTML=`
    ...
    `}function rl(e,t){const a="5516997859430",o=encodeURIComponent("Olá, gostaria de solicitar o cancelamento da minha assinatura."),r=`https://wa.me/${a}?text=${o}`,s="sistemakairosagenda@gmail.com";t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex flex-col md:flex-row items-center justify-between mb-6 border-b pb-4">
                <div>
    ...
    `}function Es(e="indigo",t){const a=t.querySelector("#color-palette-container"),o=t.querySelector("#establishmentThemeColor");!a||!o||(a.innerHTML="",Object.entries(Yi).forEach(([r,s])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{o.value=r,Es(r,t)}),a.appendChild(i)}),o.value=e)}function il(e,t){const a=t.querySelector("#slotIntervalContainer"),o=t.querySelector("#establishmentSlotInterval");if(!a||!o)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(s=>{const n=s.value===e;return`<button type="button" data-value="${s.value}" 
    ...
    `,ve.querySelector('button[data-action="back-to-list"]').addEventListener("click",o=>{o.preventDefault(),Is()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":Qi(J,a);break;case"change-password":Xi(J,a);break;case"change-email":Zi(J,a);break;case"branding":Ki(J,a);break;case"booking":el(J,a);break;case"working-hours":tl(J,a);break;case"loyalty":al(J,a);break;case"financial":await ol(J,a);break;case"support":sl(J,a);break;case"cancellation":rl(J,a);break;default:nl(t?t.label:"Definições",a)}}async function Is(){if(ve.innerHTML=`
    ...
    `,!J)try{J=await Se(u.establishmentId)}catch{g("Erro Fatal","Não foi possível carregar os dados do estabelecimento.","error"),ve.innerHTML='<p class="text-red-500">Erro ao carregar dados.</p>';return}const e=V.currentUser;e&&e.displayName&&(u.userName=e.displayName);const t=h(u.userName||V.currentUser.email);let o=`https://placehold.co/96x96/E2E8F0/4A5568?text=${t?t.charAt(0).toUpperCase():"U"}`;e&&e.photoURL&&(o=e.photoURL),ve.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd" /></svg>
                Definições
    ...
    `,ve.querySelector("#settings-menu-list").addEventListener("click",s=>{const n=s.target.closest("button[data-section]");n&&(s.preventDefault(),ll(n.dataset.section))});const r=ve.querySelector('[data-action="go-to-my-profile"]');r&&r.addEventListener("click",s=>{s.preventDefault(),ae("my-profile-section")})}const Ke=document.getElementById("content");async function Ae(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,o=document.getElementById("filterEndDate")?.value,r=await Ot(u.establishmentId,a||new Date().toISOString().split("T")[0],o||new Date().toISOString().split("T")[0],e),s=document.getElementById("filterReason")?.value.toLowerCase(),n=s?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(s)):r,i=n.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let p=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <h4 class="font-bold text-gray-700">${h(l)} (${d.length})</h4>`;if(d.length>1){const b=JSON.stringify(d.map(f=>f.id));p+=`<button data-action="batch-delete-blockage" data-ids='${b}' class="text-xs text-red-600 hover:text-red-800 font-semibold flex items-center gap-1">
                                    Apagar Lote
                                </button>`}p+="</div>",c.innerHTML=p,d.forEach(b=>{const f=new Date(b.startTime),v=new Date(b.endTime),x=f.toLocaleDateString("pt-BR"),k=v.toLocaleDateString("pt-BR"),S=`
                    <div class="bg-white p-3 rounded-md flex items-center justify-between shadow-sm">
                        <div>
                            <p class="font-medium text-gray-800 text-sm">
                                ${x===k?`${x} | ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${v.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`:`De ${x} às ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}<br>Até ${k} às ${v.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`}
                            </p>
                        </div>
                    ...
                    </div>`;c.innerHTML+=S}),t.appendChild(c)})}catch(a){t.innerHTML=`<p class="text-center text-red-500">Erro: ${a.message}</p>`}}}async function dl(e){e.preventDefault();const t=e.target,a=t.querySelector("#blockageProfId").value,o=t.querySelector("#blockageDate").value,r=t.querySelector("#blockageEndDate").value||o,s=t.querySelector("#blockageStartTime").value,n=t.querySelector("#blockageEndTime").value,i={establishmentId:u.establishmentId,professionalId:a,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:t.querySelector("#blockageReason").value};try{await zt(i),t.reset(),g("Sucesso","Bloqueio adicionado com sucesso!","success"),Ae(a)}catch(l){g("Erro",`Não foi possível criar o bloqueio: ${l.message}`,"error")}}async function cl(e){e.preventDefault();const t=e.target,a=Array.from(t.querySelectorAll('input[name="batch-professionals"]:checked')).map(c=>c.value);if(a.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const o=t.querySelector("#batchBlockageDate").value,r=t.querySelector("#batchBlockageEndDate").value||o,s=t.querySelector("#batchBlockageStartTime").value,n=t.querySelector("#batchBlockageEndTime").value,i=t.querySelector("#batchBlockageReason").value,l=t.querySelector('button[type="submit"]');l.disabled=!0,l.textContent="Aguarde...";const d=a.map(c=>{const m={establishmentId:u.establishmentId,professionalId:c,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:i};return zt(m)});try{await Promise.all(d),g("Sucesso",`${a.length} bloqueios foram criados com sucesso!`,"success"),t.reset(),t.querySelectorAll('input[name="batch-professionals"]:checked').forEach(m=>m.checked=!1);const c=document.getElementById("blockageProfId").value;c&&Ae(c)}catch(c){g("Erro",`Ocorreu um erro: ${c.message}`,"error")}finally{l.disabled=!1,l.textContent="Adicionar Bloqueio em Lote"}}function ul(e){Ke.addEventListener("submit",t=>{t.target.id==="blockageForm"&&dl(t),t.target.id==="batchBlockageForm"&&cl(t)}),Ke.addEventListener("input",t=>{t.target.matches("#filterStartDate, #filterEndDate, #filterReason")&&Ae(e)}),Ke.addEventListener("click",async t=>{const a=t.target.closest("button[data-action]");if(!a)return;const o=a.dataset.action;if(o==="back-to-professionals")ae("profissionais-section");else if(o==="delete-blockage"){if(await j("Apagar Bloqueio","Tem a certeza que deseja apagar este bloqueio?"))try{await Ia(a.dataset.id),g("Sucesso","Bloqueio removido.","success"),Ae(e)}catch(s){g("Erro",`Não foi possível remover o bloqueio: ${s.message}`,"error")}}else if(o==="batch-delete-blockage"){const r=JSON.parse(a.dataset.ids);if(await j("Apagar Lote de Bloqueios",`Tem certeza que deseja apagar ${r.length} bloqueios de uma vez?`))try{await Go(r),g("Sucesso",`${r.length} bloqueios removidos.`,"success"),Ae(e)}catch(n){g("Erro",`Não foi possível apagar os bloqueios: ${n.message}`,"error")}}})}async function ml(e){const{professionalId:t,professionalName:a}=e;if(!t||!a){Ke.innerHTML='<p class="text-red-500 p-8">Erro: ID do profissional não fornecido.</p>';return}const o=h(a);Ke.innerHTML=`
        <section>
        ...
        </section>`,ul(t),await Ae(t);const r=document.getElementById("batchProfSelectionContainer");try{const s=await Z(u.establishmentId);r.innerHTML=s.map(n=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{r.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const pl=e=>w(`/api/users/${e}`),gl=e=>w("/api/users",{method:"POST",body:JSON.stringify(e)}),bl=(e,t)=>w(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),fl=e=>w(`/api/users/${e}`,{method:"DELETE"}),vl=(e,t)=>w(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),hl=(e,t)=>w(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),be=document.getElementById("content"),xl={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},yl={view:"Visualizar",create:"Criar",edit:"Editar"};let Ue=null,_e=null;function wl(e){const t=document.getElementById("usersListContainer");if(!t)return;const a=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const o=a?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${o}</p>`;return}e.sort((o,r)=>(o.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(o=>{const r=JSON.stringify(o).replace(/'/g,"&apos;"),s=o.status==="active",n=u.professionals.find(c=>c.id===o.professionalId),i=n?n.name:"N/A",l=n?n.name.charAt(0):o.name.charAt(0),d=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(l)}`;return`
    ...
    `}).join("")}function Ct(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=u.users:t=u.users.filter(a=>a.status==="active"),wl(t)}function kl(e={}){return Object.entries(xl).map(([t,a])=>{const o=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,s=Object.entries(yl).map(([i,l])=>`
    ...
    `}).join("")}async function bo(e=null){document.getElementById("user-list-view").classList.add("hidden");const t=document.getElementById("user-form-view");t.classList.remove("hidden");let a=u.professionals;if(!a||a.length===0)try{a=await Z(u.establishmentId),u.professionals=a}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error")}const o=k=>a.find(C=>C.id===k),r=(k,C)=>{const E=o(k)?.photo,T=C.charAt(0).toUpperCase();return{photoSrc:E||`https://placehold.co/128x128/E2E8F0/4A5568?text=${T}`,initials:T,photoUrl:E||""}},s=e?.professionalId,n=e?.name||"Novo Usuário",i=r(s,n),l=o(s),d=k=>{let C='<option value="">-- Não Associado a um Profissional --</option>';return C+=a.map(S=>`<option value="${S.id}" ${S.id===k?"selected":""}>${S.name} (${S.specialty||"N/A"})</option>`).join(""),C},c=e!==null;t.querySelector("#userFormTitle").textContent=c?`Editar Usuário: ${e.name}`:"Novo Usuário";const m=t.querySelector("#userForm");m.innerHTML=`
    ...
    `;const p=window.innerWidth<768,b=m.querySelector(".bg-white");if(p&&b){b.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const k=m.closest("section");k&&(k.style.padding="0",k.style.margin="0"),b.classList.add("p-4")}const f=m.querySelector("#userProfessionalId"),v=m.querySelector("#userPhotoPreview"),x=m.querySelector("#profPhotoName");if(f.addEventListener("change",k=>{const C=k.target.value,S=o(C),E=S?S.name:"Selecione um profissional",T=r(C,n);v.src=T.photoSrc,x.textContent=E,m.querySelector("#professionalPhotoUrl").value=T.photoUrl}),m.addEventListener("submit",async k=>{k.preventDefault();const C=e?.email,S=m.querySelector("#userEmail").value,E={};m.querySelectorAll('input[type="checkbox"]').forEach(O=>{const L=O.dataset.module,B=O.dataset.permission;E[L]||(E[L]={}),E[L][B]=O.checked});const T=m.querySelector("#userProfessionalId").value||null,R={name:m.querySelector("#userName").value,permissions:E,professionalId:T,establishmentId:u.establishmentId};try{c?(C!==S&&(R.email=S),await bl(e.id,R),g("Usuário atualizado com sucesso!","success")):(R.email=m.querySelector("#userEmail").value,R.password=m.querySelector("#userPassword").value,await gl(R),g("Usuário criado com sucesso!","success")),Mt()}catch(O){g(`Erro: ${O.message}`,"error")}}),c){const k=m.querySelector("#password-change-container"),C=k.querySelector('[data-action="show-password-form"]'),S=k.querySelector("#password-form"),E=S.querySelector('[data-action="save-password"]'),T=S.querySelector('[data-action="cancel-password-change"]');C.addEventListener("click",()=>{C.classList.add("hidden"),S.classList.remove("hidden")}),T.addEventListener("click",()=>{C.classList.remove("hidden"),S.classList.add("hidden"),S.querySelector("#userNewPassword").value=""}),E.addEventListener("click",async()=>{const R=S.querySelector("#userNewPassword").value;if(!R||R.length<6){g("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await j("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{E.disabled=!0,E.textContent="Aguarde...",await vl(e.id,R),g("Sucesso!","A senha do usuário foi alterada.","success"),C.classList.remove("hidden"),S.classList.add("hidden"),S.querySelector("#userNewPassword").value=""}catch(L){g("Erro",`Não foi possível alterar a senha: ${L.message}`,"error")}finally{E.disabled=!1,E.textContent="Salvar Nova Senha"}})}}async function Sl(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,a]=await Promise.all([pl(u.establishmentId),Z(u.establishmentId)]);u.users=t,u.professionals=a,Ct()}catch{g("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function Mt(){be.innerHTML=`
    ...
    `,Ue&&be.removeEventListener("click",Ue),_e&&be.removeEventListener("change",_e),Ue=async e=>{if(!document.getElementById("user-list-view")){be.removeEventListener("click",Ue);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":bo();break;case"edit-user":const o=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));bo(o);break;case"back-to-list":Mt();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await j("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await fl(r),g("Usuário excluído com sucesso!","success"),Mt()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},_e=async e=>{if(!document.getElementById("user-list-view")){be.removeEventListener("change",_e);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")Ct();else if(t){e.stopPropagation();const a=t.dataset.userId,o=t.checked?"active":"inactive";try{await hl(a,o),g(`Usuário ${o==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=u.users.findIndex(s=>s.id===a);r>-1&&(u.users[r].status=o,Ct())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,Ct()}}},be.addEventListener("click",Ue),be.addEventListener("change",_e),await Sl()}const $l=document.getElementById("content");let fo={},ha=null;function El(){Object.values(fo).forEach(e=>e?.destroy()),fo={}}function Il(e,t){if(!window.jspdf){g("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:a}=window.jspdf,o=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(o.setFontSize(18),o.text(e,o.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];o.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const s=o.lastAutoTable?o.lastAutoTable.finalY+20:60;o.text("Detalhes das Vendas",20,s),o.autoTable({html:`#${t}`,startY:s+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),o.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function vo(e){const t=document.getElementById("genericModal"),a=h(e.client),o=h(e.items),r=h(e.responsavelCaixa||"N/A"),s=(e.payments||[]).map(n=>`
    ...
    `,t.style.display="flex"}function Cl(e){const{summary:t,transactions:a}=e;document.getElementById("summary-revenue").textContent=`R$ ${t.totalRevenue.toFixed(2)}`,document.getElementById("summary-transactions").textContent=t.totalTransactions,document.getElementById("summary-avg-ticket").textContent=`R$ ${t.averageTicket.toFixed(2)}`;const o=document.getElementById("paymentSummaryTableBody"),r=Object.entries(t.paymentMethodTotals).sort(([,i],[,l])=>l-i);o.innerHTML=r.map(([i,l])=>`
        <tr class="border-b">
    ...
    `}).join(""),s.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=ha.transactions[l];d&&vo(d)})}),n.innerHTML=a.map((i,l)=>{const d=h(i.client),c=h(i.items),m=h(i.type);return`
    ...
    `}).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const l=i.dataset.transactionIndex,d=ha.transactions[l];d&&vo(d)})})}async function ho(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const o=t.value,r=a.value;if(!o||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const s=document.getElementById("cashierSessionFilter").value,n=await Mr({establishmentId:u.establishmentId,startDate:o,endDate:r,cashierSessionId:s});ha=n,e.innerHTML=`
        ...
        `,Cl(n)}catch(s){g("Erro",`Não foi possível carregar o relatório: ${s.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${h(s.message)}</p>`}}async function Ll(){El();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];$l.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",ho),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const o=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,s=`Relatorio_Vendas_${o}_a_${r}`;Il(s,"transactionsTable")});try{const o=await dn(u.establishmentId),r=document.getElementById("cashierSessionFilter");o&&o.length>0&&o.forEach(s=>{const n=new Date(s.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=h(s.closedByName||"N/A");r.innerHTML+=`<option value="${s.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await ho()}const Tl=document.getElementById("content");let D={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},ta=null,bt=null,ft=null;function Fa(e){const t=new Map,a=[];return e&&(e.forEach(o=>t.set(o.id,{...o,children:[]})),t.forEach(o=>{o.parentId&&t.has(o.parentId)?t.get(o.parentId).children.push(o):a.push(o)})),a}function Cs(e,t,a){if(!e)return;if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const o=(r,s=0)=>{const n="— ".repeat(s);return`
        ...
        `};e.innerHTML=t.map(r=>o(r)).join("")}async function xo(e){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const t=document.getElementById("genericModal"),a=e==="nature",o=`Gerir ${a?"Naturezas Financeiras":"Centros de Custo"}`,r=a?st:rt,s=a?"natures":"costCenters";t.innerHTML=`
        ...
        </div>`,t.style.display="flex";const n=t.querySelector("#hierarchyList"),i=t.querySelector("#itemParent"),l=c=>{const m=Fa(c);if(Cs(n,m,e),i){i.innerHTML='<option value="">-- Nível Principal --</option>';const p=(b,f="",v=0)=>{const x=v>0?"— ".repeat(v):"";i.innerHTML+=`<option value="${b.id}">${x}${b.name}</option>`,b.children.forEach(k=>p(k,f+"— "))};m.forEach(b=>p(b))}};try{const c=await r(u.establishmentId);D[s]=c,l(c)}catch(c){console.error(c)}const d=t.querySelector("#hierarchyForm");d&&d.addEventListener("submit",async c=>{c.preventDefault();const m=t.querySelector("#itemName");if(!m)return;const p=m.value,b=i.value,f=a?Mi:Ni;try{await f({name:p,parentId:b||null,establishmentId:u.establishmentId});const v=await r(u.establishmentId);D[s]=v,l(v),d.reset(),await ye()}catch(v){g("Erro",`Não foi possível criar: ${v.message}`,"error")}})}function Pl(e){const t=document.getElementById("cashFlowChart");if(!t)return;const a=t.getContext("2d");ta&&ta.destroy();const o=e.payables.map(r=>r*-1);ta=new Chart(a,{type:"bar",data:{labels:e.labels,datasets:[{label:"Receitas",data:e.receivables,backgroundColor:"rgba(74, 222, 128, 0.6)",borderColor:"rgba(34, 197, 94, 1)",borderWidth:1,yAxisID:"y"},{label:"Despesas",data:o,backgroundColor:"rgba(248, 113, 113, 0.6)",borderColor:"rgba(239, 68, 68, 1)",borderWidth:1,yAxisID:"y"},{label:"Saldo Acumulado",data:e.expectedBalance,type:"line",borderColor:"rgba(59, 130, 246, 1)",backgroundColor:"rgba(59, 130, 246, 0.2)",borderWidth:3,pointRadius:4,pointBackgroundColor:"rgba(59, 130, 246, 1)",fill:!0,tension:.1,yAxisID:"y1"}]},options:{responsive:!0,maintainAspectRatio:!1,scales:{x:{stacked:!0},y:{type:"linear",display:!0,position:"left",stacked:!0,title:{display:!0,text:"Movimentações (R$)"}},y1:{type:"linear",display:!0,position:"right",title:{display:!0,text:"Saldo Acumulado (R$)"},grid:{drawOnChartArea:!1}}},plugins:{tooltip:{callbacks:{label:function(r){let s=r.dataset.label||"";if(s&&(s+=": "),r.parsed.y!==null){const n=Math.abs(r.parsed.y);s+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(n)}return s}}}}}})}async function yo(){const e=document.getElementById("cash-flow-chart-container"),t=document.getElementById("cashFlowStartDate"),a=document.getElementById("cashFlowEndDate");if(!e||!t||!a)return;const o=t.value,r=a.value;if(!o||!r){g("Atenção","Por favor, selecione as datas de início e fim.","error");return}e.innerHTML='<div class="loader mx-auto my-10"></div>';try{const s=await Wi(u.establishmentId,o,r);document.getElementById("cash-flow-chart-container")&&(e.innerHTML='<canvas id="cashFlowChart"></canvas>',Pl(s))}catch(s){document.getElementById("cash-flow-chart-container")&&(e.innerHTML=`<p class="text-red-500 text-center">Erro ao carregar dados do gráfico: ${s.message}</p>`)}}function wo(){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const e=document.getElementById("genericModal"),t=new Date,a=new Date(t.getFullYear(),t.getMonth(),1).toISOString().split("T")[0],o=t.toISOString().split("T")[0];e.innerHTML=`
    ...
    `,e.style.display="flex";const r=e.querySelector("#generateCashFlowBtn");r&&(r.addEventListener("click",yo),yo())}function Bl(){const e=document.getElementById("genericModal"),t=D.payables.filter(m=>m.status==="pending").reduce((m,p)=>m+p.amount,0),a=D.receivables.filter(m=>m.status==="pending").reduce((m,p)=>m+p.amount,0),o=a-t,r=D.payables.filter(m=>m.status==="paid").reduce((m,p)=>m+p.amount,0),s=D.receivables.filter(m=>m.status==="paid").reduce((m,p)=>m+p.amount,0),n=s-r,i=D.previousBalance||0,l=i+n,d=m=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(m),c=m=>m>=0?"text-green-600":"text-red-600";e.innerHTML=`
    ...
    `;if(a.innerHTML=`
        ...
        </div>`,a.style.display="flex",t){const p=a.querySelector('[name="naturezaId"]');p&&(p.value=t.naturezaId||"");const b=a.querySelector('[name="centroDeCustoId"]');b&&(b.value=t.centroDeCustoId||"")}const d=a.querySelector("#status"),c=a.querySelector("#payment-date-container"),m=a.querySelector('[name="paymentDate"]');t?.status==="paid"&&(d.checked=!0,c.classList.remove("hidden"),m.value=t.paymentDate||new Date().toISOString().split("T")[0]),d.addEventListener("change",()=>{c.classList.toggle("hidden",!d.checked),m.required=d.checked}),a.querySelector("#financial-form").addEventListener("submit",p=>Ml(p,e,t?.id))}async function ql(){const e=new Date,a=new Date(e.getFullYear(),e.getMonth()-1,1).toISOString().split("T")[0],o=e.toISOString().split("T")[0];D.startDate=a,D.endDate=o,D.currentFilter="pending",D.filterNaturezaId="all",D.filterCostCenterId="all",Tl.innerHTML=`
        <section>
    ...
    `;const r=document.getElementById("main-fab-btn"),s=document.getElementById("fab-menu");if(r&&s){r.addEventListener("click",v=>{v.stopPropagation(),s.classList.toggle("hidden"),r.classList.toggle("rotate-45")});const p=s.querySelector('button[data-action="open-modal"][data-type="receivable"]'),b=s.querySelector('button[data-action="open-modal"][data-type="payable"]'),f=s.querySelector('button[data-action="open-cash-flow-modal"]');p&&p.addEventListener("click",v=>{v.stopPropagation(),aa("receivable")}),b&&b.addEventListener("click",v=>{v.stopPropagation(),aa("payable")}),f&&f.addEventListener("click",v=>{v.stopPropagation(),wo()})}bt&&document.body.removeEventListener("click",bt),ft&&document.getElementById("genericModal").removeEventListener("click",ft);const n=()=>{const p=document.getElementById("filterStartDate"),b=document.getElementById("filterEndDate"),f=document.getElementById("filterNaturezaId"),v=document.getElementById("filterCostCenterId");if(!p)return;D.startDate=p.value,D.endDate=b.value,D.filterNaturezaId=f.value,D.filterCostCenterId=v.value;const x=document.getElementById("advanced-filters");x&&x.classList.contains("hidden")===!1&&window.innerWidth<768&&x.classList.add("hidden"),ye()},i=p=>{const b=p.target.closest("[data-status-filter]");if(!b)return;const f=b.dataset.statusFilter;D.currentFilter=f,document.querySelectorAll("[data-status-filter]").forEach(v=>{v.classList.remove("bg-blue-100","text-blue-800"),v.classList.add("bg-gray-100","text-gray-600")}),b.classList.remove("bg-gray-100","text-gray-600"),b.classList.add("bg-blue-100","text-blue-800"),xa()},l=p=>{const b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("btn-payables-view"),x=document.getElementById("btn-receivables-view");b&&(window.innerWidth>=1024&&D.currentListView===p||(p==="payables"?(b.classList.remove("hidden"),f.classList.add("hidden"),v&&(v.classList.remove("bg-gray-200"),v.classList.add("bg-red-100","border","border-red-500")),x&&(x.classList.remove("bg-green-100","border","border-green-500"),x.classList.add("bg-gray-200"))):(b.classList.add("hidden"),f.classList.remove("hidden"),v&&(v.classList.remove("bg-red-100","border","border-red-500"),v.classList.add("bg-gray-200")),x&&(x.classList.remove("bg-gray-200"),x.classList.add("bg-green-100","border","border-green-500"))),D.currentListView=p))};document.getElementById("applyDateFilterBtn").addEventListener("click",n),document.getElementById("filterNaturezaId").addEventListener("change",()=>{D.filterNaturezaId=document.getElementById("filterNaturezaId").value}),document.getElementById("filterCostCenterId").addEventListener("change",()=>{D.filterCostCenterId=document.getElementById("filterCostCenterId").value}),document.querySelectorAll("[data-status-filter]").forEach(p=>{p.addEventListener("click",i)}),bt=p=>{const b=p.target.closest("button[data-action]");if(!b)return;const{action:f,type:v,id:x}=b.dataset;f==="edit"?aa(v,JSON.parse(b.dataset.item.replace(/&apos;/g,"'"))):f==="delete"?Al(v,x):f==="mark-as-paid"?Nl(v,x):f==="manage-natures"?xo("nature"):f==="manage-cost-centers"?xo("cost-center"):f==="open-cash-flow-modal"?wo():f==="toggle-filters"?document.getElementById("advanced-filters")?.classList.toggle("hidden"):f==="open-indicators-modal"?Bl():f==="open-settings-modal"?Dl():f==="toggle-list-view"&&l(b.dataset.list)},ft=p=>{const b=p.target.closest('button[data-action^="delete-"]');if(b){const f=b.dataset.action.split("-")[1];d(f,b.dataset.id)}},document.body.addEventListener("click",bt),document.getElementById("genericModal").addEventListener("click",ft);async function d(p,b){const f=p==="nature",v=f?Ai:qi,x=f?st:rt,k=f?"natures":"costCenters",C=document.getElementById("hierarchyList");if(await j("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await v(b);const E=await x(u.establishmentId);D[k]=E,Cs(C,Fa(E),p),await ye()}catch(E){g("Erro",`Não foi possível apagar: ${E.message}`,"error")}}const c=()=>{const p=window.innerWidth<1024,b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("list-toggle-buttons");b&&f&&(b.classList.remove("hidden"),f.classList.remove("hidden"),p?(b.classList.remove("lg:col-span-1"),f.classList.remove("lg:col-span-1"),v?.classList.remove("hidden"),l(D.currentListView)):(b.classList.add("lg:col-span-1"),f.classList.add("lg:col-span-1"),v?.classList.add("hidden"),b.classList.remove("hidden"),f.classList.remove("hidden")))};c(),window.addEventListener("resize",c);const m=document.querySelector(`[data-status-filter="${D.currentFilter}"]`);m&&(document.querySelectorAll("[data-status-filter]").forEach(p=>{p.classList.remove("bg-blue-100","text-blue-800"),p.classList.add("bg-gray-100","text-gray-600")}),m.classList.remove("bg-gray-100","text-gray-600"),m.classList.add("bg-blue-100","text-blue-800"));try{const p=await Gi(u.establishmentId),b=document.getElementById("summary-today-payables");b&&(b.textContent=`R$ ${p.totalPayables.toFixed(2)}`);const f=document.getElementById("summary-today-receivables");f&&(f.textContent=`R$ ${p.totalReceivables.toFixed(2)}`)}catch{}await ye()}const Rl=e=>w("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),Fl=e=>w("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),jl=(e,t)=>{const a=new URLSearchParams({startDate:e,endDate:t}).toString();return w(`/api/commissions/stats?${a}`)},Hl=(e={})=>{Object.keys(e).forEach(o=>(e[o]===void 0||e[o]===null||e[o]==="")&&delete e[o]);const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return w(a)},Ol=e=>w(`/api/commissions/report/${e}`,{method:"DELETE"}),At=new Date,$o=new Date(At.getFullYear(),At.getMonth(),1),M={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:$o.toISOString().split("T")[0],dashEndDate:At.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:$o.toISOString().split("T")[0],histEndDate:At.toISOString().split("T")[0],histProfessionalId:"all"};let ht=null;const We=document.getElementById("content");async function zl(){try{M.professionals=await Z(u.establishmentId)}catch(e){console.error("Erro profissionais",e)}Kl(),Vl(),Gt(),qt("dashboard")}function Vl(){ht&&We.removeEventListener("click",ht),ht=e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.action,o=t.dataset.id,r=t.dataset.idx;switch(a){case"tab-nav":qt(t.dataset.tab);break;case"toggle-all-profs":Ul();break;case"back-to-filters":M.calculationResult=null,Nt(document.getElementById("commissions-content"));break;case"view-preview-items":Zl(r);break;case"save-final-report":Jl();break;case"start-new-calc":qt("calculator");break;case"print-receipt":Wl(o);break;case"delete-report":Gl(o);break;case"filter-dashboard":Gt();break;case"filter-history":ja();break}},We.addEventListener("click",ht),We.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;Ql(t)}},We.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),_l())}}async function Gt(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(M.dashStartDate=e.value),t&&(M.dashEndDate=t.value);const a=document.getElementById("dashboard-stats-container");a&&(a.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const o=await jl(M.dashStartDate,M.dashEndDate);M.dashStats={revenue:o.totalRevenue||0,commissions:o.totalCommissionsPaid||0},M.currentTab==="dashboard"&&Ls(document.getElementById("commissions-content"))}catch(o){console.error(o),a&&(a.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function ja(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),a=document.getElementById("hist-prof");e&&(M.histStartDate=e.value),t&&(M.histEndDate=t.value),a&&(M.histProfessionalId=a.value);const o=document.getElementById("history-list-container");if(o){o.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await Hl({startDate:M.histStartDate,endDate:M.histEndDate,professionalId:M.histProfessionalId});M.historyData=r,Ts(o,r)}catch{o.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function Ul(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(a=>a.checked);e.forEach(a=>a.checked=!t)}async function _l(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(s=>s.value);if(e.length===0)return g("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},a=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),o=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");M.periodString=`${a} a ${o}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const s=await Rl(t);M.calculationResult=s.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),Nt(r)}catch(s){g("Erro",s.message,"error"),M.calculationResult=null,Nt(r)}}async function Jl(){const e=M.calculationResult.length;if(await j("Confirmar",`Gerar ${e} relatórios? Isso marcará as vendas como pagas.`))try{const a=M.calculationResult.map(o=>{const r=o.items.map(s=>s.originalSaleId).filter(s=>s!=null);return Fl({professionalId:o.professionalId,professionalName:o.professionalName,period:M.periodString,processedSalesIds:r,reportData:{...o,summary:{...o.summary,finalValue:o.finalValue,extraDebit:o.extraDebit||0,extraCredit:o.extraCredit||0,notes:o.notes||""}}})});await Promise.all(a),g("Sucesso","Pagamentos registrados!","success"),M.calculationResult=null,Gt(),qt("history")}catch(a){g("Erro",a.message,"error")}}function Wl(e){const t=M.historyData.find(a=>a.id===e);t&&Yl(t)}async function Gl(e){if(await j("Excluir","Deseja remover este registro? As vendas voltarão a ficar disponíveis para cálculo."))try{await Ol(e),g("Sucesso","Registro removido.","success"),ja(),Gt()}catch(a){g("Erro",a.message,"error")}}function Yl(e){const{jsPDF:t}=window.jspdf;if(!t)return g("Erro","PDF lib não carregada.","error");const a=new t,o=a.internal.pageSize.getWidth()/2;a.setFontSize(18),a.setFont(void 0,"bold"),a.text("RECIBO DE PAGAMENTO DE COMISSÃO",o,20,{align:"center"}),a.setFontSize(12),a.setFont(void 0,"normal"),a.text(`Profissional: ${e.professionalName}`,15,40),a.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),a.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const s=a.lastAutoTable.finalY+10;a.setFontSize(14),a.setFont(void 0,"bold"),a.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,s,{align:"right"}),a.save(`Recibo_${e.professionalName}.pdf`)}function Ql(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),a=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let o=0,r=0;if(t.forEach(s=>{s.value&&(o=parseFloat(s.value))}),a.forEach(s=>{s.value&&(r=parseFloat(s.value))}),M.calculationResult&&M.calculationResult[e]){const s=M.calculationResult[e];s.extraDebit=o,s.extraCredit=r,s.finalValue=s.summary.totalCommission-o+r,t.forEach(i=>{i!==document.activeElement&&(i.value=o||"")}),a.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${s.finalValue.toFixed(2)}`),Xl()}}function Xl(){const e=M.calculationResult.reduce((a,o)=>a+o.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(a=>a.innerText=`R$ ${e.toFixed(2)}`)}function Zl(e){const t=M.calculationResult[e];if(!t)return;const a=t.items.map(o=>`
    ...
    `}function ed(e){const t=M.professionals.map(a=>`<option value="${a.id}" ${M.histProfessionalId===a.id?"selected":""}>${a.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `,M.historyData.length>0?Ts(document.getElementById("history-list-container"),M.historyData):ja()}function Ts(e,t){if(t.length===0){e.innerHTML=`
    ...
    `}const oa=document.getElementById("content");let we={allPackages:[],catalogForModal:{services:[],products:[]}},xt=null,De=null;function td(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function ad(){const e=document.getElementById("packagesListContainer");if(e){if(we.allPackages.length===0){e.innerHTML=`
            ...
            </div>`;return}e.innerHTML=we.allPackages.map(t=>{const a=t.status==="active",o=JSON.stringify(t).replace(/'/g,"&apos;"),r=t.price||0,s=t.originalPrice||0,n=s>r?s-r:0,i=s>0?(s-r)/s*100:0,l=h(t.name),d=h(t.description||"Sem descrição");return`
        ...
        `}).join("")}}function Eo(){const e=document.getElementById("genericModal");e.style.display="none",De&&e.removeEventListener("click",De)}async function Io(e=null){const t=document.getElementById("genericModal"),a=!!e,o=e?JSON.parse(JSON.stringify(e.items||[])):[],r=h(e?.name||""),s=h(e?.description||""),n=e?.price||"",i=e?.commissionRate||0,l=e?.validityDays||30,d=`
    ...
    `;t.innerHTML=d,t.style.display="flex";const c=t.querySelector("#package-items-list"),m=(b,f)=>{const v=f.querySelector("#originalPrice"),x=b.reduce((k,C)=>k+C.price*C.quantity,0);v&&(v.textContent=`R$ ${x.toFixed(2)}`)},p=b=>{b.length===0?c.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':c.innerHTML=b.map((f,v)=>{const x=f.type==="service",k=x?"Serviço":"Produto",C=x?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
                <div class="flex items-center justify-between bg-white p-2 rounded shadow-sm border border-gray-200">
                    <div class="flex items-center gap-3 min-w-0">
                        <input type="number" value="${f.quantity}" min="1" class="w-12 p-1 border rounded-md text-sm quantity-input flex-shrink-0" data-index="${v}">
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full ${C}">${k}</span>
                        <span class="font-medium text-gray-800 truncate">${h(f.name)}</span>
                    </div>
            ...
            `}).join(""),m(b,t)};p(o),c.addEventListener("change",b=>{if(b.target.classList.contains("quantity-input")){const f=parseInt(b.target.dataset.index,10),v=parseInt(b.target.value,10);v>0&&o[f]&&(o[f].quantity=v,p(o))}}),c.addEventListener("click",b=>{if(b.target.classList.contains("remove-item-btn")){const f=parseInt(b.target.dataset.index,10);o.splice(f,1),p(o)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>od(b=>{const f=o.find(v=>v.id===b.id&&v.type===b.type);f?f.quantity++:o.push({...b,quantity:1}),p(o)}),De&&t.removeEventListener("click",De),De=async b=>{const f=b.target.closest("button[data-action]");if(!f)return;const v=f.dataset.action;if(b.stopPropagation(),v==="close-modal"&&Eo(),v==="save-package"){const x=f,k={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:o,originalPrice:o.reduce((C,S)=>C+S.price*S.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null,establishmentId:u.establishmentId};if(!k.name||!k.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(k.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}x.disabled=!0,x.textContent="A salvar...";try{a?await mn(k.id,k):(delete k.id,await un(k)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),Eo(),await Ha()}catch(C){g("Erro",`Não foi possível salvar o pacote: ${C.message}`,"error"),x.disabled=!1,x.textContent="Salvar Pacote"}}},t.addEventListener("click",De)}function od(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const o={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=l=>{const d=t.toLowerCase(),c=we.catalogForModal.services.filter(f=>f.name.toLowerCase().includes(d)),m=we.catalogForModal.products.filter(f=>f.name.toLowerCase().includes(d)),p=c.map(f=>`
            <button data-action="select-item" data-item-type="service" data-item-id="${f.id}" class="flex items-center gap-3 w-full p-3 bg-white border rounded-lg hover:bg-gray-50 transition">
                ...
                <span class="flex-grow text-left min-w-0 truncate">${h(f.name)}</span>
    ...
    `,xt&&oa.removeEventListener("click",xt),xt=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const o=e.target.closest('[data-action="delete-package"]');if(o){const r=o.dataset.id;j("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async s=>{if(s)try{await pn(r),g("Sucesso!","Pacote excluído.","success"),await Ha()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")Io(null);else if(a==="edit-package"){const o=JSON.parse(t.dataset.package);Io(o)}},oa.addEventListener("click",xt);try{const[e,t,a]=await Promise.all([os(u.establishmentId),$e(u.establishmentId),Ht(u.establishmentId)]);we.allPackages=e,we.catalogForModal={services:t.filter(o=>o.active),products:a},ad()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const sd=document.getElementById("content");let rd=null;async function nd(){const e=h(u.userName||"Usuário"),t=h(V.currentUser?.email||"E-mail não disponível"),a=u.userName?u.userName.charAt(0):"U";sd.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await id()}async function id(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=u.userProfessionalId;if(t){const a=await ir(t);rd=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo);const o=h(a.name);e.innerHTML=`
            ...
            `,ld(a.id),document.getElementById("my-blocks-filter").addEventListener("change",s=>Rt(a.id,s.target.value)),Rt(a.id,"future")}else e.innerHTML=`
    ...
    `).join("")}function bd(e){et&&et();const t=ue(U,"establishments",e,"notifications"),a=Ft(t,at("timestamp",">=",new Date),Ao("timestamp","desc"));et=Os(a,o=>{o.docChanges().forEach(r=>{if(r.type==="added"){const s=r.doc.data();tt.unshift({title:s.title,message:s.message,time:s.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(s.title,s.message,"info",!0),ya();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(s.type==="cancellation"||s.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),as())}})},o=>{console.error("Erro no listener de notificações em tempo real:",o)})}function ae(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=u.enabledModules?.[a]!==!1,s=u.userPermissions===null||u.userPermissions[e]?.view===!0;if(!r||!s){ra.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const o=pd[e];o?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),ra.innerHTML="",o(t)):ra.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`}async function fd(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),o=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),s=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(s&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!s&&!n))try{const i=await Ar();s&&o&&(o.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i)}}async function vd(e){try{console.log("[Nativo] Iniciando configuração de Push..."),he.getPlatform()==="android"&&(await W.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal Android criado."));let t=await W.checkPermissions();if(t.receive==="prompt"&&(t=await W.requestPermissions()),t.receive!=="granted"){alert("ERRO: Permissão de notificações negada!");return}await W.register(),W.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const o=ce(U,"users",e);await wa(o,{fcmTokens:Hs(a.value),platform:"native_mobile"}),console.log("Token FCM salvo no perfil do utilizador (Nativo).")}catch(o){alert("Erro ao salvar no Banco: "+o.message),console.error("Erro ao salvar token FCM:",o)}}),W.addListener("registrationError",a=>{alert("FALHA DE REGISTO: "+JSON.stringify(a)),console.error("Erro no registo de push notifications:",a)}),W.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),W.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),ae("agenda-section")})}catch(t){alert("Erro Fatal Push: "+t.message),console.log("Push Notifications não suportado/inicializado:",t)}}async function hd(){try{await qs(V,Rs),console.log("Persistência LOCAL configurada na inicialização.")}catch(e){console.error("Erro ao definir persistência no main.js",e)}he.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),Ks(),Co.addEventListener("click",e=>{e.stopPropagation(),wt.classList.toggle("hidden"),wt.classList.contains("hidden")||(tt.forEach(t=>t.read=!0),ya())}),md.addEventListener("click",()=>{er()}),ia.addEventListener("click",e=>{e.stopPropagation(),ee.classList.toggle("active"),ee.classList.contains("active")?ee.classList.remove("hidden"):setTimeout(()=>ee.classList.add("hidden"),200)}),To&&To.addEventListener("click",e=>{e.preventDefault(),ae("my-profile-section"),ee.classList.remove("active"),ee.classList.add("hidden")}),document.addEventListener("click",e=>{!wt.contains(e.target)&&e.target!==Co&&wt.classList.add("hidden"),!ee.contains(e.target)&&e.target!==ia&&ee.classList.contains("active")&&(ee.classList.remove("active"),setTimeout(()=>ee.classList.add("hidden"),200))}),Bo(V,async e=>{if(e){if(console.log("Usuário detectado:",e.email),!he.isNativePlatform()&&(console.log("Inicializando Web Push (PWA)..."),Uo(),"Notification"in window&&Notification.permission==="default")){const t=document.getElementById("toast-notification-request"),a=document.getElementById("btn-enable-toast"),o=document.getElementById("btn-deny-toast"),r=document.getElementById("btn-close-toast");setTimeout(()=>{t&&(t.style.display="block")},3500),a&&a.addEventListener("click",async()=>{await _o()&&t&&(t.style.display="none")});const s=()=>{t&&(t.style.display="none")};o&&o.addEventListener("click",s),r&&r.addEventListener("click",s)}try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const o=await Se(a.establishmentId);u.enabledModules=o.modules,gd(o.themeColor||"indigo");let r=null,s=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const d=ce(U,"users",e.uid),c=await Do(d);if(c.exists()){const m=c.data();r=a.role==="employee"?m.permissions||{}:null,s=m.name||s,n=m.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}u.userProfessionalId=n,he.isNativePlatform()&&vd(e.uid);const i=s||e.email;Us(a.establishmentId,o.name,r),ia.textContent=i.charAt(0).toUpperCase(),dd.textContent=i,cd.textContent=e.email;const l=()=>{et&&et(),za(V).then(()=>window.location.href="/login.html")};ud.addEventListener("click",d=>{d.preventDefault(),l()}),sr(ae,r,u.enabledModules),fd(r),bd(a.establishmentId),ya(),yt.classList.add("fade-out"),sa.style.display="flex",setTimeout(()=>{yt.style.display="none"},500),console.log("Verificando Onboarding..."),setTimeout(()=>{hr()},1500),ae("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),yt.classList.add("fade-out"),setTimeout(()=>{yt.style.display="none"},500),sa.innerHTML=`
                ...
                `,sa.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{za(V).then(()=>window.location.href="/login.html")})}}else window.location.href="/login.html"})}hd();Bo(V,async e=>{if(e){await Uo();const a="Notification"in window&&Notification.permission==="default",o=window.Capacitor&&window.Capacitor.isNativePlatform();if(a&&!o){const r=document.getElementById("toast-notification-request"),s=document.getElementById("btn-enable-toast"),n=document.getElementById("btn-deny-toast"),i=document.getElementById("btn-close-toast");setTimeout(()=>{r&&(r.style.display="block")},3500),s&&s.addEventListener("click",async()=>{await _o()&&r&&(r.style.display="none")});const l=()=>{r&&(r.style.display="none")};n&&n.addEventListener("click",l),i&&i.addEventListener("click",l)}}});
