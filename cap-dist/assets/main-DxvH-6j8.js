import"./modulepreload-polyfill-B5Qt9EMX.js";import{a as J,d as Q,o as Oo,b as uo,g as zo,s as La,u as Vo,c as Uo,q as Jo,e as _o,w as Wo,f as Go}from"./firebase-config-DnrQqYxH.js";import{EmailAuthProvider as Yo,reauthenticateWithCredential as Xo,verifyBeforeUpdateEmail as Qo,updatePassword as Zo,updateProfile as Ko}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{addDoc as ma,collection as Xe,getDocs as mo,query as es,orderBy as ts,doc as po,deleteDoc as as,updateDoc as os}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";const p={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function ss(e,t,a){p.establishmentId=e,p.establishmentName=t,p.userPermissions=a}const Qt="http://localhost:8080";console.log("🚀 API configurada para Produção (US):",Qt);async function rs(){const e=J.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function k(e,t={}){const a=await rs();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const o=e.startsWith("/")?e:`/${e}`,r=`${Qt}${o}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${r}`);try{const s=await fetch(r,{...t,headers:{...a,...t.headers}});if(!s.ok){const i=(await s.json().catch(()=>({message:s.statusText}))).message||`Erro na API: ${s.status}`;if(i.includes("FAILED_PRECONDITION")&&i.includes("requires an index")){const d=/(https:\/\/[^\s]+)/,l=i.match(d),c=l?l[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${s.status}) em ${r}:`,i),new Error(i)}return s.json()}catch(s){throw console.error(`Falha de rede ao tentar acessar ${r}:`,s.message),s.message.includes("Failed to fetch")||s.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${Qt}. Verifique sua conexão com a internet.`):s}}const ns=(e,t,a,o=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(r+=`&professionalId=${o}`),k(r)},is=(e,t,a)=>{const o=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return k(o)},ls=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return k(r)},ds=e=>k("/api/appointments",{method:"POST",body:JSON.stringify(e)}),pa=(e,t)=>k(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),cs=e=>k(`/api/appointments/${e}`,{method:"DELETE"}),us=e=>k(`/api/appointments/${e}/reopen`,{method:"POST"}),ms=(e,t)=>k(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let R;async function ps(){if(!R)try{R=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function gs(){if(!R){console.warn("AudioContext não inicializado. O som não será tocado.");return}R.state==="suspended"&&R.resume();const e=R.createOscillator(),t=R.createGain();e.connect(t),t.connect(R.destination),e.type="sine",e.frequency.setValueAtTime(800,R.currentTime),t.gain.setValueAtTime(0,R.currentTime),t.gain.linearRampToValueAtTime(.3,R.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,R.currentTime+.2),e.start(R.currentTime),e.stop(R.currentTime+.2)}function g(e,t,a="info",o=!1){const r=document.getElementById("toast-container");if(!r)return;o&&gs();const s=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},d={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};s.className=`toast ${n[a]||n.info}`,s.innerHTML=`
    ...
    `;const d=r.querySelector("[data-close-modal]");d&&(d.onclick=n);const l=r.querySelector('[data-action="close-modal"]');return l&&(l.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Bt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function bs(){document.body.addEventListener("click",()=>{R||ps()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const o=t.dataset.target;if(o){const r=document.getElementById(o);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const o=document.getElementById("genericModal");o&&(o.style.display="none")}})}async function Ba(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const o=await is(p.establishmentId,t,a);if(o.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=o.map(r=>`
    ...
    `,{modalElement:r}=N({title:"Histórico de Cancelamentos",contentHTML:o,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Ba),Ba()}const z=document.getElementById("sidebar"),Ta=document.getElementById("sidebarToggle"),Ne=document.getElementById("mainContent"),vs=document.querySelectorAll(".sidebar-link"),Da=document.getElementById("hamburger-menu-btn"),Ce=document.getElementById("mobile-overlay");function vt(e){!z||!Ne||(z.classList.toggle("collapsed",e),Ne.classList.toggle("sidebar-collapsed-shift",e))}function hs(){!z||!Ce||(z.classList.add("mobile-open"),Ce.classList.add("visible"))}function st(){!z||!Ce||(z.classList.remove("mobile-open"),Ce.classList.remove("visible"))}function xs(){vt(!z.classList.contains("collapsed"))}function ys(e,t,a){if(!z||!Ne)return;Ne.classList.add("main-content-shift"),window.innerWidth>=768?vt(z.classList.contains("collapsed")):(Ne.classList.remove("main-content-shift","sidebar-collapsed-shift"),st()),Ta&&Ta.addEventListener("click",r=>{r.stopPropagation(),xs()}),z.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&z.classList.contains("collapsed")&&vt(!1)}),z.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||vt(!0))}),Da&&Da.addEventListener("click",r=>{r.stopPropagation(),hs()}),Ce&&Ce.addEventListener("click",r=>{r.stopPropagation(),st()});let o=0;z.addEventListener("touchstart",r=>{o=r.changedTouches[0].screenX},{passive:!0}),z.addEventListener("touchend",r=>{const s=r.changedTouches[0].screenX;o-s>50&&st()},{passive:!0}),vs.forEach(r=>{const s=r.getAttribute("data-target"),n=s.replace("-section",""),i=a?.[n]!==!1,d=t===null||t[s]?.view===!0;if(!i||!d){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",l=>{l.preventDefault(),s&&typeof e=="function"&&e(s),window.innerWidth<768&&st()})})}const Qe=e=>{const t=e||p.establishmentId;return t?k(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ws=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ks=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},$s=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},_=e=>k(`/api/professionals/${e}`),Ss=e=>k(`/api/professionals/details/${e}`),Es=e=>k("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Cs=(e,t)=>k(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Is=e=>k(`/api/professionals/${e}`,{method:"DELETE"});var Ie;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Ie||(Ie={}));class jt extends Error{constructor(t,a,o){super(t),this.message=t,this.code=a,this.data=o}}const Ls=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},Bs=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},o=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:Ls(e),s=()=>r()!=="web",n=u=>{const m=l.get(u);return!!(m?.platforms.has(r())||i(u))},i=u=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===u)},d=u=>e.console.error(u),l=new Map,c=(u,m={})=>{const b=l.get(u);if(b)return console.warn(`Capacitor plugin "${u}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(u);let y;const h=async()=>(!y&&f in m?y=typeof m[f]=="function"?y=await m[f]():y=m[f]:t!==null&&!y&&"web"in m&&(y=typeof m.web=="function"?y=await m.web():y=m.web),y),x=(M,q)=>{var F,O;if(v){const se=v?.methods.find(W=>q===W.name);if(se)return se.rtype==="promise"?W=>a.nativePromise(u,q.toString(),W):(W,at)=>a.nativeCallback(u,q.toString(),W,at);if(M)return(F=M[q])===null||F===void 0?void 0:F.bind(M)}else{if(M)return(O=M[q])===null||O===void 0?void 0:O.bind(M);throw new jt(`"${u}" plugin is not implemented on ${f}`,Ie.Unimplemented)}},S=M=>{let q;const F=(...O)=>{const se=h().then(W=>{const at=x(W,M);if(at){const ot=at(...O);return q=ot?.remove,ot}else throw new jt(`"${u}.${M}()" is not implemented on ${f}`,Ie.Unimplemented)});return M==="addListener"&&(se.remove=async()=>q()),se};return F.toString=()=>`${M.toString()}() { [capacitor code] }`,Object.defineProperty(F,"name",{value:M,writable:!1,configurable:!1}),F},E=S("addListener"),T=S("removeListener"),D=(M,q)=>{const F=E({eventName:M},q),O=async()=>{const W=await F;T({eventName:M,callbackId:W},q)},se=new Promise(W=>F.then(()=>W({remove:O})));return se.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await O()},se},H=new Proxy({},{get(M,q){switch(q){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?D:E;case"removeListener":return T;default:return S(q)}}});return o[u]=H,l.set(u,{name:u,proxy:H,platforms:new Set([...Object.keys(m),...v?[f]:[]])}),H};return a.convertFileSrc||(a.convertFileSrc=u=>u),a.getPlatform=r,a.handleError=d,a.isNativePlatform=s,a.isPluginAvailable=n,a.registerPlugin=c,a.Exception=jt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Ts=e=>e.Capacitor=Bs(e),Je=Ts(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),ga=Je.registerPlugin;class go{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let o=!1;this.listeners[t]||(this.listeners[t]=[],o=!0),this.listeners[t].push(a);const s=this.windowListeners[t];s&&!s.registered&&this.addWindowListener(s),o&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,o){const r=this.listeners[t];if(!r){if(o){let s=this.retainedEventArguments[t];s||(s=[]),s.push(a),this.retainedEventArguments[t]=s}return}r.forEach(s=>s(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:o=>{this.notifyListeners(a,o)}}}unimplemented(t="not implemented"){return new Je.Exception(t,Ie.Unimplemented)}unavailable(t="not available"){return new Je.Exception(t,Ie.Unavailable)}async removeListener(t,a){const o=this.listeners[t];if(!o)return;const r=o.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(o=>{this.notifyListeners(t,o)}))}}const Ma=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Pa=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class Ds extends go{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(o=>{if(o.length<=0)return;let[r,s]=o.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Pa(r).trim(),s=Pa(s).trim(),a[r]=s}),a}async setCookie(t){try{const a=Ma(t.key),o=Ma(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,s=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${o||""}${r}; path=${s}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}ga("CapacitorCookies",{web:()=>new Ds});const Ms=async e=>new Promise((t,a)=>{const o=new FileReader;o.onload=()=>{const r=o.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},o.onerror=r=>a(r),o.readAsDataURL(e)}),Ps=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,s,n)=>(r[s]=e[t[n]],r),{})},As=(e,t=!0)=>e?Object.entries(e).reduce((o,r)=>{const[s,n]=r;let i,d;return Array.isArray(n)?(d="",n.forEach(l=>{i=t?encodeURIComponent(l):l,d+=`${s}=${i}&`}),d.slice(0,-1)):(i=t?encodeURIComponent(n):n,d=`${s}=${i}`),`${o}&${d}`},"").substr(1):null,qs=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Ps(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const s=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))s.set(n,i);a.body=s.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const s=new FormData;if(e.data instanceof FormData)e.data.forEach((i,d)=>{s.append(d,i)});else for(const i of Object.keys(e.data))s.append(i,e.data[i]);a.body=s;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Ns extends go{async request(t){const a=qs(t,t.webFetchExtra),o=As(t.params,t.shouldEncodeUrlParams),r=o?`${t.url}?${o}`:t.url,s=await fetch(r,a),n=s.headers.get("content-type")||"";let{responseType:i="text"}=s.ok?t:{};n.includes("application/json")&&(i="json");let d,l;switch(i){case"arraybuffer":case"blob":l=await s.blob(),d=await Ms(l);break;case"json":d=await s.json();break;case"document":case"text":default:d=await s.text()}const c={};return s.headers.forEach((u,m)=>{c[m]=u}),{data:d,headers:c,status:s.status,url:s.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}ga("CapacitorHttp",{web:()=>new Ns});const re=ga("PushNotifications",{}),Fs=(e,t,a="all",o="all")=>{const r={startDate:e,endDate:t};a&&a!=="all"&&(r.professionalId=a),o&&o!=="all"&&(r.costCenterId=o);const s=new URLSearchParams(r).toString();return k(`/api/reports/indicators?${s}`)},Rs=(e,t="all")=>{const a={date:e};t&&t!=="all"&&(a.professionalId=t);const o=new URLSearchParams(a).toString();return k(`/api/reports/appointments/list?${o}`)},js=()=>k("/api/financial/cost-centers"),Hs=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:o})=>{let r=`/api/reports/sales/${e}?startDate=${t}&endDate=${a}`;return o&&o!=="all"&&(r+=`&cashierSessionId=${o}`),k(r)},Os=()=>k("/api/reports/summary",{method:"GET"}),Te=e=>k(`/api/services/${e}`),zs=e=>k("/api/services",{method:"POST",body:JSON.stringify(e)}),Vs=(e,t)=>k(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),Us=e=>k(`/api/services/${e}`,{method:"DELETE"}),Js=(e,t)=>k(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),_s=e=>k(`/api/services/stats/most-popular/${e}`),Tt=(e,t,a,o="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${o}`;return k(r)},Dt=e=>k("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ba=e=>k(`/api/blockages/${e}`,{method:"DELETE"}),bo=e=>k("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),Ze=e=>k(`/api/clients/${e}`),fa=e=>k("/api/clients",{method:"POST",body:JSON.stringify(e)}),Ws=(e,t)=>k(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Gs=e=>k(`/api/clients/${e}`,{method:"DELETE"}),Ys=(e,t,a)=>{const o=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return k(o)},Xs=(e,t,a)=>{const o=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return k(o)},Qs=(e,t,a,o)=>k("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:o})}),Aa=document.getElementById("content");let qa=!1;const Zt=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let Ke=[],Mt=[],_e={},ae=[],I={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},$={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Zs(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function fo(e){const t=new Date(e);if(t.setHours(0,0,0,0),I.currentView==="week"&&I.weekViewDays===7){const a=t.getDay(),o=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(o))}return t}function kt(){const e=document.getElementById("profSelectorContainer"),t=I.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(s=>I.showInactiveProfs||s.status!=="inactive");t&&(a=a.filter(s=>s.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(s=>{const n=I.selectedProfessionalId===s.id,i=s.name==="Todos"?"Todos":s.name.split(" ")[0],d=s.name==="Todos"?"T":s.name.charAt(0).toUpperCase(),l=s.status!=="inactive",c=Zt[0],u=s.id!=="all"&&p.professionalColors.get(s.id)||c,m=s.photo||`https://placehold.co/64x64/${u.main?.replace("#","")||"E0E7FF"}/${u.light?.replace("#","")||"4F46E5"}?text=${d}`,b=s.id==="all"?"#e0e7ff":u.light,f=s.id==="all"?"#4f46e5":u.main,y=`border: 3px solid ${n?u.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+u.border:"none"};`;return`
        ...
        `}).join("")}function Ks(e,t,a,o,r){const s=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${o} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,l=encodeURIComponent(d);return`https://wa.me/${s}?text=${l}`}function er(e){const t=document.getElementById("agenda-view");if(e.sort((o,r)=>new Date(o.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(o=>{const r=new Date(o.startTime),s=new Date(o.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=s.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=p.professionalColors.get(o.professionalId)||{};if(o.type==="blockage")return`
                ...
                </div>`;const l=o.status==="completed",c=l?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",u=l?"Finalizado":"Aberto",m=JSON.stringify(o).replace(/'/g,"&apos;"),b=o.redeemedReward?.points>0,f=o.hasRewards&&!b,v=Ks(o.clientPhone,o.clientName,o.serviceName,o.professionalName,o.startTime);return`
        ...
        `}s+="</div>",t.innerHTML=s}function ar(){const e=p.allEvents.filter(t=>I.selectedProfessionalId==="all"||t.professionalId===I.selectedProfessionalId);I.currentView==="list"?er(e):tr(e)}async function Z(){const e=document.getElementById("agenda-view");if(!e)return;e.innerHTML='<div class="loader mx-auto my-10"></div>';let t,a;const o=document.getElementById("weekRange");if(I.currentView==="list")t=new Date(I.currentDate),t.setHours(0,0,0,0),a=new Date(I.currentDate),a.setHours(23,59,59,999),o.textContent=Zs(t);else{const r=va();t=fo(new Date(I.currentDate)),a=new Date(t),a.setDate(t.getDate()+(r-1)),a.setHours(23,59,59,999),o.textContent=`${t.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} - ${a.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}`}try{const r=await ns(p.establishmentId,t.toISOString(),a.toISOString(),I.selectedProfessionalId==="all"?null:I.selectedProfessionalId),n=(await Tt(p.establishmentId,t.toISOString(),a.toISOString(),I.selectedProfessionalId)).map(d=>{let l=d.professionalName;if(!l&&d.professionalId){const c=p.professionals?p.professionals.find(u=>u.id===d.professionalId):null;c&&(l=c.name)}return{...d,type:"blockage",professionalName:l||"Não identificado"}}),i=[...r.map(d=>({...d,type:"appointment"})),...n];if(p.allEvents=i,kt(),ar(),I.scrollToAppointmentId){const d=document.querySelector(`[data-appointment*='"id":"${I.scrollToAppointmentId}"']`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.style.transition="background-color 0.5s ease-in-out",d.style.backgroundColor="#e0e7ff",setTimeout(()=>{d.style.backgroundColor=""},2500)),I.scrollToAppointmentId=null}}catch(r){g("Erro na Agenda",`Não foi possível carregar a agenda: ${r.message}`,"error"),e.innerHTML='<div class="p-6 text-center text-red-600">Falha ao carregar dados.</div>'}}async function or(){try{const[e,t,a,o]=await Promise.all([p.professionals&&p.professionals.length>0?Promise.resolve(p.professionals):_(p.establishmentId),p.services&&p.services.length>0?Promise.resolve(p.services):Te(p.establishmentId),ae.length>0?Promise.resolve(ae):Ze(p.establishmentId),_e.enabled!==void 0?Promise.resolve(null):Qe(p.establishmentId)]);(!p.professionals||p.professionals.length===0)&&(p.professionals=e||[]),(!p.services||p.services.length===0)&&(p.services=t||[]),ae.length===0&&(ae=a||[]),o&&(_e=o.loyaltyProgram||{enabled:!1}),p.professionals.forEach((r,s)=>{p.professionalColors.set(r.id,Zt[s%Zt.length])}),kt()}catch(e){console.error("Erro ao popular filtros e dependências do modal:",e),g("Atenção","Não foi possível pré-carregar os dados para agendamento. A abertura do modal pode ser lenta.","error")}}function Kt(e){e<1||e>4||($.step=e,ea(null,!0))}function vo(e,t){const a=document.getElementById("multiServiceToggle"),o=a&&a.checked,r=t.classList.contains("selected"),s=$.data.selectedServiceIds.indexOf(e);if(r)t.classList.remove("selected","border-blue-500"),s>-1&&$.data.selectedServiceIds.splice(s,1);else{if(!o){$.data.selectedServiceIds=[];const n=document.getElementById("apptServicesContainer");n&&n.querySelectorAll(".service-card.selected").forEach(i=>{i.classList.remove("selected","border-blue-500")})}t.classList.add("selected","border-blue-500"),$.data.selectedServiceIds.push(e)}}function ho(e,t){const a=document.querySelector(".professional-step-cards");if(!a)return;a.querySelectorAll(".professional-modal-card").forEach(r=>r.classList.remove("selected","border-blue-500")),t.classList.add("selected","border-blue-500");const o=Mt.find(r=>r.id===e);$.data.professionalId=e,$.data.professionalName=o?o.name:"N/A"}function sr(e,t){const a=document.getElementById("availableTimesContainer");a&&(a.querySelectorAll(".time-slot-card").forEach(o=>o.classList.remove("selected")),t.classList.add("selected"),$.data.time=e)}async function Na(){const e=document.getElementById("apptTotalDuration"),t=document.getElementById("availableTimesContainer");if(!e||!t)return;const a=$.data.professionalId,o=$.data.selectedServiceIds,r=document.getElementById("apptDate").value;$.data.date=r;const s=o.reduce((n,i)=>{const d=Ke.find(l=>l.id===i);return n+(d?d.duration+(d.bufferTime||0):0)},0);if(e.textContent=`${s} min`,s===0||!a||!r){t.innerHTML='<p class="col-span-full text-center text-gray-500">Selecione serviço, profissional e data.</p>';return}t.innerHTML='<div class="loader mx-auto col-span-full"></div>';try{let n=await ls({establishmentId:p.establishmentId,professionalId:a,serviceIds:o,date:r});const i=new Date;if(new Date(r+"T00:00:00").toDateString()===i.toDateString()){const l=i.getHours()*60+i.getMinutes();n=n.filter(c=>{const[u,m]=c.split(":").map(Number);return u*60+m>=l})}if(t.innerHTML="",n.length>0){if(n.forEach(l=>{const c=document.createElement("button");c.type="button",c.className=`time-slot-card p-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200 transition ${$.data.time===l?"selected":""}`,c.textContent=l,c.addEventListener("click",()=>sr(l,c)),t.appendChild(c)}),$.data.time){const l=t.querySelector(`[data-action="time-slot"][data-time="${$.data.time}"]`);l&&l.classList.add("selected")}}else t.innerHTML='<p class="col-span-full text-center text-gray-500">Nenhum horário disponível.</p>'}catch(n){console.error("Erro ao buscar horários:",n),t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao buscar horários.</p>'}}function rr(){const e=document.getElementById("loyaltyRewardsContainer");if(!e)return;const{clientHasRewards:t,clientLoyaltyPoints:a,redeemedReward:o}=$.data,{enabled:r,rewards:s,pointsPerCurrency:n}=_e;if(!r||!t||!s||s.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden");const i=s.filter(l=>a>=l.points);let d=`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",l=>{l.target.checked&&($.data.redeemedReward=null)})}async function nr(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!$.data.time||$.data.selectedServiceIds.length===0||!$.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const o=$.data.selectedServiceIds.map(l=>{const c=Ke.find(u=>u.id===l);return{id:c.id,name:c.name,price:c.price,duration:c.duration,bufferTime:c.bufferTime||0,photo:c.photo||null}}),[r,s]=$.data.time.split(":"),n=new Date(`${$.data.date}T${r}:${s}:00`),i={establishmentId:p.establishmentId,clientName:$.data.clientName,clientPhone:$.data.clientPhone,services:o,professionalId:$.data.professionalId,startTime:n.toISOString(),redeemedReward:$.data.redeemedReward},d=t.querySelector("#appointmentId").value;d&&(i.id=d);try{d?await pa(d,i):await ds(i),g(`Agendamento ${d?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",Z()}catch(l){g(l.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function ir(e){return`
    ...
    `}async function Fa(e){const t=document.getElementById("clientSearchResults");if(!t)return;const a=e.toLowerCase().trim();if(a.length<3){t.innerHTML='<p class="text-sm text-gray-500">Digite pelo menos 3 caracteres para buscar clientes existentes.</p>';return}const o=ae.filter(r=>r.name.toLowerCase().includes(a)||r.phone.includes(a));if(o.length===0){t.innerHTML='<p class="text-sm text-gray-500">Nenhum cliente encontrado com este termo.</p>';return}t.innerHTML=o.map(ir).join(""),t.querySelectorAll('[data-action="select-client"]').forEach(r=>{r.addEventListener("click",s=>{const n=r.dataset.clientName,i=r.dataset.clientPhone,d=ae.find(l=>l.phone===i&&l.name===n);if($.data.clientName=n,$.data.clientPhone=i,d){const l=_e,c=Math.min(...(l?.rewards||[]).map(u=>u.points));$.data.clientLoyaltyPoints=d.loyaltyPoints||0,$.data.clientHasRewards=l.enabled&&c!==1/0&&$.data.clientLoyaltyPoints>=c}else $.data.clientHasRewards=!1,$.data.clientLoyaltyPoints=0;document.getElementById("apptClientName").value=n,document.getElementById("apptClientPhone").value=i,document.querySelectorAll(".client-search-card").forEach(l=>l.classList.remove("selected","border-blue-500")),r.classList.add("selected","border-blue-500")})})}async function lr(e){e.preventDefault();const t=document.getElementById("clientRegistrationForm"),a=t.querySelector('button[type="submit"]'),o={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim(),phone:t.querySelector("#regClientPhone").value.trim(),dobDay:t.querySelector("#regClientDobDay").value.trim(),dobMonth:t.querySelector("#regClientDobMonth").value.trim(),notes:t.querySelector("#regClientNotes").value.trim()};if(!o.name||!o.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{await fa(o),ae.push({name:o.name,phone:o.phone,loyaltyPoints:0}),$.data.clientName=o.name,$.data.clientPhone=o.phone,$.data.clientHasRewards=!1,$.data.clientLoyaltyPoints=0,g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",Kt(1)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar"}}function dr(){N({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",lr)}function cr(){dr()}function ur(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",o=`
    ...
    `;return{title:a,content:o}}function mr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:e,content:a}}function pr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Mt.map(a=>{const o=$.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
    ...
    `;return{title:e,content:t}}function gr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,o=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=$.data.date||o,s=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:s}}function br(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),o=Ke.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=$.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            <div class="service-card p-3 bg-white rounded-lg border-2 border-gray-200 cursor-pointer transition-all hover:bg-gray-50 ${s?"selected border-blue-500":""}" data-service-id="${r.id}">
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>vo(r.dataset.serviceId,r))})}function fr(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),o=Mt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=$.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
             ...
             </div>`}).join(""),t.querySelectorAll(".professional-modal-card").forEach(r=>{r.addEventListener("click",()=>ho(r.dataset.professionalId,r))})}async function ea(e=null,t=!1){const a=document.getElementById("appointmentModal");if(!t){const s=e?.startTime?new Date(e.startTime).toISOString().split("T")[0]:new Date().toISOString().split("T")[0],n=e?.startTime?new Date(e.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",hour12:!1}):null;$={step:1,data:{id:e?.id||null,clientName:e?.clientName||"",clientPhone:e?.clientPhone||"",selectedServiceIds:e?.services?.map(i=>i.id)||[],professionalId:e?.professionalId||null,professionalName:e?.professionalName||"",date:s,time:n,redeemedReward:e?.redeemedReward||null,clientHasRewards:e?.hasRewards||!1,clientLoyaltyPoints:0}}}if(!p.services||!p.professionals||!ae||_e.enabled===void 0){g("Erro","Os dados da agenda ainda não foram carregados. Tente novamente em alguns segundos.","error");return}if(Ke=p.services,Mt=p.professionals.filter(s=>s.status==="active"),$.data.clientName&&$.data.clientPhone){const s=ae.find(n=>n.phone===$.data.clientPhone&&n.name===$.data.clientName);s&&($.data.clientLoyaltyPoints=s.loyaltyPoints||0)}let o={title:"Erro",content:"<p>Etapa não encontrada.</p>"};switch($.step){case 1:o=ur(e);break;case 2:o=mr();break;case 3:o=pr();break;case 4:o=gr(e);break}a.innerHTML=`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(s=>{s.addEventListener("click",()=>{const n=parseInt(s.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),d=a.querySelector("#apptClientPhone");if($.data.clientName=i.value.trim(),$.data.clientPhone=d.value.trim(),!$.data.clientName||!$.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if($.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!$.data.professionalId)return g("Atenção","Selecione um profissional.","error");Kt(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(s=>{s.addEventListener("click",()=>Kt(parseInt(s.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if($.step===4&&r&&r.addEventListener("submit",nr),a.style.display="flex",$.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>vo(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>br(i.target.value))}if($.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>ho(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>fr(i.target.value))}if($.step===1){const s=a.querySelector("#clientSearchInput");s&&(s.addEventListener("input",i=>Fa(i.target.value)),$.data.clientName&&$.data.clientPhone&&Fa(`${$.data.clientName} ${$.data.clientPhone}`));const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",cr)}if($.step===4){const s=a.querySelector("#apptDate");s&&s.addEventListener("change",Na),Na(),rr()}}async function xo(e={}){I.currentDate=e.targetDate?new Date(e.targetDate):I.currentDate||new Date,I.scrollToAppointmentId=e.scrollToAppointmentId||null,I.profSearchTerm="",window.innerWidth<768&&(I.currentView="list"),Aa.innerHTML=`
        <section>
        ...
        </section>`,document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),I.currentView=a.dataset.view;const o=document.getElementById("week-days-toggle");if(I.currentView==="week"){if(o.style.display="flex",window.innerWidth<768){I.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="3"]');r&&r.classList.add("active")}}else o.style.display="none";Z()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active")),a.classList.add("active"),I.weekViewDays=parseInt(a.dataset.days,10),Z()})}),document.getElementById("todayBtn").addEventListener("click",()=>{I.currentDate=new Date,Z()});const t=a=>{const o=parseInt(a.currentTarget.dataset.amount,10),r=I.currentView==="week"?va():1,s=new Date(I.currentDate);s.setDate(s.getDate()+o*r),I.currentDate=s,Z()};document.getElementById("prevBtn").addEventListener("click",t),document.getElementById("nextBtn").addEventListener("click",t),document.getElementById("profSearchInput").addEventListener("input",a=>{I.profSearchTerm=a.target.value,kt()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{I.showInactiveProfs=a.target.checked,kt(),Z()}),qa||(Aa.addEventListener("click",async a=>{const o=a.target.closest("[data-action]");if(a.target.closest('[data-action="select-professional"]')){const d=a.target.closest('[data-action="select-professional"]').dataset.profId,l=I.selectedProfessionalId===d&&d!=="all";if(I.selectedProfessionalId=l?"all":d,d!=="all"){const c=document.getElementById("profSearchInput");c&&(c.value=""),I.profSearchTerm=""}await Z();return}if(!o)return;const r=o.dataset.action;let s=null;const n=a.target.closest("[data-appointment]");switch(n&&(s=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":ea();break;case"edit-appointment":if(!s)return;if(s.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}s.hasRewards&&!s.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),ea(s);break;case"delete-appointment":{const i=o.dataset.id;if(await A("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await cs(i),g("Agendamento apagado!","success"),Z()}catch(l){g(`Não foi possível apagar: ${l.message}`,"error")}break}case"open-comanda":if(s){s.hasRewards&&!s.redeemedReward&&s.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=s.status==="completed"?"finalizadas":"em-atendimento",d={selectedAppointmentId:s.id,initialFilter:i};if(i==="finalizadas"){let l=s.startTime;if(s.transaction&&s.transaction.paidAt){const c=s.transaction.paidAt;typeof c=="object"&&c._seconds?l=new Date(c._seconds*1e3):l=c}d.filterDate=l}Y("comandas-section",d)}break}}),qa=!0),await or(),await Z()}const vr=(e,t=null,a=1,o=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${o}`;return t&&(r+=`&date=${t}`),k(r)},hr=e=>k("/api/sales",{method:"POST",body:JSON.stringify(e)}),xr=e=>k(`/api/sales/${e}/reopen`,{method:"POST"}),yr=e=>k(`/api/sales/${e}`,{method:"DELETE"}),Pt=e=>k(`/api/products/${e}`),wr=e=>k("/api/products",{method:"POST",body:JSON.stringify(e)}),kr=(e,t)=>k(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),$r=e=>k(`/api/products/${e}`,{method:"DELETE"}),Sr=(e,t)=>k(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),Er=({startDate:e,endDate:t,productId:a,categoryId:o})=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("productId",a),o&&o!=="all"&&r.append("categoryId",o),k(`/api/products/stock-history/report?${r.toString()}`)},yo=()=>k("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),Cr=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),k("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},Ir=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),k(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},Lr=()=>k("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),Br=e=>k(`/api/cashier/report/${e}`),wo=e=>k(`/api/packages/${e}`),Tr=e=>k("/api/packages",{method:"POST",body:JSON.stringify(e)}),Dr=(e,t)=>k(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),Mr=e=>k(`/api/packages/${e}`,{method:"DELETE"});let w={allComandas:[],catalog:{services:[],products:[],packages:[]},clients:[],activeFilter:"atendimento",selectedComandaId:null,isCashierOpen:!1,activeCashierSessionId:null,paging:{page:1,limit:12,total:0}},xe=null,ye=null;function Pr(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function ge(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function Ar(){const e=new Date().toISOString().split("T")[0];ye.innerHTML=`
    ...
    `}function qr(){const e=document.getElementById("cashier-controls");e&&(w.isCashierOpen?e.innerHTML=`
    ...
    `}function Nr(){N({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,{modalElement:d}=N({title:"Finalizar Pagamento",contentHTML:i,maxWidth:"max-w-md"});document.getElementById("payment-value").value=r.remainingAmount.toFixed(2),d.addEventListener("click",l=>{const c=l.target.closest(".payment-method-btn");c&&(r.selectedMethod=c.dataset.method,r.installments=1,document.getElementById("installments-select").value=1,s()),l.target.closest("#add-payment-btn")&&n(),l.target.closest('[data-action="remove-payment"]')&&(o.splice(parseInt(l.target.closest('[data-action="remove-payment"]').dataset.paymentIndex,10),1),s()),l.target.closest("#finalize-checkout-btn")&&Jr(e,a,o)}),d.addEventListener("change",l=>{l.target.id==="installments-select"&&(r.installments=parseInt(l.target.value,10))}),d.addEventListener("input",l=>{l.target.id==="amount-received"&&(r.amountReceived=l.target.value,s())}),s()}async function Hr(){const e=`
    ...
    `,{modalElement:t}=N({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const o=document.getElementById("initial-amount"),r=o.value.trim(),s=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),o.focus();return}try{const i={establishmentId:p.establishmentId,initialAmount:parseFloat(n.toFixed(2))};s&&(i.notes=s);const d=await Cr(i);w.isCashierOpen=!0,w.activeCashierSessionId=d.id,await ha(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function Or(){const e=w.activeCashierSessionId;if(e)try{const t=await Br(e),a=`
        ...
        `,{modalElement:o}=N({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});o.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const s=parseFloat(document.getElementById("final-amount").value);if(isNaN(s)||s<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await Ir(e,s),w.isCashierOpen=!1,w.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await ha(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function zr(e){w.activeFilter!==e&&(w.activeFilter=e,w.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),ge(),await ee(),w.selectedComandaId=null,G())}function ta(e){w.selectedComandaId=e,de(),Pr(),G()}async function Vr(e,t){const a=w.allComandas.find(r=>r.id===w.selectedComandaId);if(!a)return;const o=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...o),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),G(),de();return}try{await pa(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),G(),de()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function Ur(e,t){const a=w.allComandas.find(s=>s.id===w.selectedComandaId);if(!a)return;let o=!1,r=(a.comandaItems||[]).findIndex(s=>s.id===e&&s.type===t);if(r>-1)a.comandaItems.splice(r,1),o=!0;else{let s=(a.services||[]).findIndex(n=>n.id===e);if(s>-1)a.services.splice(s,1),o=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),o=!0)}}if(o){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),G(),de();return}try{await pa(a.id,a),g("Sucesso","Item removido!","success"),G(),de()}catch(s){g("Erro",`Não foi possível remover o item: ${s.message}`,"error"),await ee()}}}async function Jr(e,t,a){const o=e.type==="appointment",r=[...e.services||[],...e.comandaItems||[],...e.items||[]],s={payments:a,totalAmount:t,items:r,cashierSessionId:w.activeCashierSessionId};try{o?await ms(e.id,s):(s.clientName=e.clientName,s.professionalId=e.professionalId,s.clientPhone=e.clientPhone,await hr(s)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",ge(),w.selectedComandaId=null,await ee()}catch(n){g("Erro no Checkout",n.message,"error")}}async function _r(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,o=w.clients.find(n=>n.id===t),r=p.professionals.find(n=>n.id===a);if(!o||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const s={id:`temp-${Date.now()}`,type:"walk-in",clientName:o.name,clientPhone:o.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};w.allComandas.unshift(s),w.selectedComandaId=s.id,document.getElementById("genericModal").style.display="none",ta(s.id)}async function ee(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=w.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await yo();if(w.isCashierOpen=!!a,w.activeCashierSessionId=a?a.id:null,qr(),!w.isCashierOpen&&w.activeFilter==="atendimento"){de(),G();return}const o=await vr(p.establishmentId,t,w.paging.page,w.paging.limit);if(w.allComandas=o.data||o,w.paging.total=o.total||o.length,w.catalog.services.length===0){const[r,s,n,i,d]=await Promise.all([Te(p.establishmentId),Pt(p.establishmentId),wo(p.establishmentId),Ze(p.establishmentId),_(p.establishmentId)]);w.catalog={services:r,products:s,packages:n},w.clients=i,p.professionals=d}de(),w.selectedComandaId,G()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function ha(e={}){ye=document.getElementById("content");try{const t=await yo();w.isCashierOpen=!!t,w.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),w.isCashierOpen=!1}w.selectedComandaId=e.selectedAppointmentId||null,Ar(),xe&&(ye.removeEventListener("click",xe),ye.removeEventListener("change",xe)),xe=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&w.activeFilter==="finalizadas"){w.paging.page=1,await ee();return}if(a){if(a.matches("[data-filter]"))zr(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}ta(a.dataset.comandaId)}else if(a.matches("[data-action]")){const o=a.dataset.action,r=a.dataset.id||w.selectedComandaId;switch(o){case"back-to-list":{ge(),w.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(s=>s.classList.remove("selected")),G();break}case"new-sale":ko();break;case"add-item":Rr();break;case"checkout":jr();break;case"open-cashier":Hr();break;case"close-cashier":await Or();break;case"view-sales-report":Y("sales-report-section");break;case"remove-item":await Ur(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await A("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await us(r);const n=w.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete w.allComandas[n].transaction,delete w.allComandas[n].cashierSessionId,delete w.allComandas[n].redeemedReward,w.allComandas[n].status="confirmed"),w.selectedComandaId=null,ge(),g("Sucesso!","Comanda reaberta para edição.","success"),await ee()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await A("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await xr(r),g("Sucesso!","Venda revertida."),ge(),w.selectedComandaId=null,await ee()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const s=a.dataset.id,n=a.dataset.date;Y("agenda-section",{scrollToAppointmentId:s,targetDate:new Date(n)});break}case"delete-walk-in":{if(await A("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))w.allComandas=w.allComandas.filter(n=>n.id!==r),w.selectedComandaId=null,de(),G(),g("Sucesso","Venda avulsa removida.","success"),ge();else try{await yr(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),w.selectedComandaId=null,ge(),await ee()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},ye.addEventListener("click",xe),ye.addEventListener("change",xe),e.initialFilter&&(w.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(w.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${w.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",w.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await ee(),w.selectedComandaId&&ta(w.selectedComandaId)}const aa=document.getElementById("content");let Se={};const rt=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],B={startDate:new Date(new Date().getFullYear(),0,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,currentTab:"dashboards",apptViewMode:"year",apptSelectedMonth:null};async function Wr(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function Gr(){aa.innerHTML='<div class="flex flex-col items-center justify-center h-64"><div class="loader mb-4"></div><p class="text-gray-500">Carregando inteligência de dados...</p></div>';try{await Wr();const[e,t]=await Promise.all([_(p.establishmentId),js().catch(()=>[])]);B.professionalsList=e,B.costCentersList=t,Yr(),await $o()}catch(e){aa.innerHTML=`
            ...
            </div>`}}function Yr(){const e=B.professionalsList.map(a=>`<option value="${a.id}">${a.name}</option>`).join(""),t=B.costCentersList.map(a=>`<option value="${a.id}">${a.name}</option>`).join("");aa.innerHTML=`
    ...
    `,document.getElementById("btn-filter").onclick=Xr,document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{B.currentTab=a.dataset.tab,ja(),So()}}),ja()}function ja(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===B.currentTab;e.className=t?"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-bold bg-white text-indigo-600 shadow-sm transition-all whitespace-nowrap":"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition-all whitespace-nowrap"})}async function Xr(){B.startDate=document.getElementById("report-start").value,B.endDate=document.getElementById("report-end").value,B.selectedProfessional=document.getElementById("report-prof").value,B.selectedCostCenter=document.getElementById("report-cost").value,B.apptViewMode="year",B.apptSelectedMonth=null,await $o()}async function $o(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader"></div></div>';try{const t=await Fs(B.startDate,B.endDate,B.selectedProfessional,B.selectedCostCenter);B.data=t,So()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function So(){const e=document.getElementById("report-content");switch(B.currentTab){case"dashboards":Qr(e);break;case"appointments":xa(e);break;case"dre":tn(e);break}}function Qr(e){const{dreSimple:t,charts:a}=B.data,o=t||{grossRevenue:0,netProfit:0,variableCosts:0},r=B.data.totalAppointments||0,s=r>0?o.grossRevenue/r:0;e.innerHTML=`
    ...
    `,nt("chart-monthly","bar","Receita Mensal",a.salesMonthly.labels,a.salesMonthly.data,rt[0]),nt("chart-profs","doughnut","Total Vendas",a.professionals.labels,a.professionals.data,rt),nt("chart-daily","line","Vendas Diárias",a.salesDaily.labels,a.salesDaily.data,rt[4]),nt("chart-products","bar","Total Vendido",a.products.labels,a.products.data,rt[1])}function xa(e){const{charts:t}=B.data;let a='<h3 class="font-bold text-gray-700 text-lg">Visão Anual/Mensal</h3>',o="Clique na barra do Mês para detalhar os dias";B.apptViewMode==="month"&&(a=`
    ...
    `,Zr();const r=document.getElementById("btn-back-year");r&&(r.onclick=()=>{B.apptViewMode="year",B.apptSelectedMonth=null,xa(e)})}function Zr(){const e=document.getElementById("chart-appointments").getContext("2d");Se["chart-appointments"]&&Se["chart-appointments"].destroy();const{charts:t}=B.data;let a,o;if(B.apptViewMode==="year")a=t.appointmentsMonthly.labels,o=t.appointmentsMonthly.data;else{const s={jan:"01",fev:"02",mar:"03",abr:"04",mai:"05",jun:"06",jul:"07",ago:"08",set:"09",out:"10",nov:"11",dez:"12"},[n,i]=B.apptSelectedMonth.split("/"),d=s[n.toLowerCase()]||n,l=i.length===2?"20"+i:i,c=Object.entries(t.appointmentsDaily.data).map(([u,m],b)=>{const f=t.appointmentsDaily.labels[b];return f.includes(`/${d}/${l}`)?{label:f,value:m}:null}).filter(Boolean);a=c.map(u=>u.label),o=c.map(u=>u.value)}const r={type:"bar",data:{labels:a,datasets:[{label:"Volume de Agendamentos",data:o,backgroundColor:B.apptViewMode==="year"?"#3b82f6":"#8b5cf6",borderRadius:4,hoverBackgroundColor:"#1e40af"}]},options:{responsive:!0,maintainAspectRatio:!1,onClick:(s,n)=>{if(n.length>0){const i=n[0].index,d=a[i];Kr(d)}},plugins:{legend:{display:!1},tooltip:{callbacks:{label:s=>` ${s.raw} Agendamentos`}}}}};Se["chart-appointments"]=new Chart(e,r)}function Kr(e){B.apptViewMode==="year"?(B.apptViewMode="month",B.apptSelectedMonth=e,xa(document.getElementById("report-content"))):B.apptViewMode==="month"&&en(e)}async function en(e){const[t,a,o]=e.split("/"),r=`${o}-${a}-${t}`;N({title:"Carregando...",contentHTML:'<div class="p-8 text-center"><div class="loader mx-auto"></div></div>',maxWidth:"max-w-md"});try{const s=await Rs(r,B.selectedProfessional);let n="";s.length===0?n='<div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">Nenhum agendamento encontrado para este dia.</div>':n=`
    ...
    `}function nt(e,t,a,o,r,s){const n=document.getElementById(e);if(!n)return;const i=n.getContext("2d");Se[e]&&Se[e].destroy();const d={type:t,data:{labels:o,datasets:[{label:a,data:r,backgroundColor:Array.isArray(s)?s:t==="line"?"rgba(79, 70, 229, 0.1)":s,borderColor:Array.isArray(s)?"#fff":s,borderWidth:2,fill:t==="line",tension:.3,borderRadius:t==="bar"?4:0,pointBackgroundColor:"#fff",pointBorderColor:s,pointHoverBackgroundColor:s}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:t==="doughnut",position:"bottom"},tooltip:{backgroundColor:"rgba(17, 24, 39, 0.9)",padding:10,cornerRadius:8,callbacks:{label:l=>{let c=l.dataset.label||"";return c&&(c+=": "),l.parsed.y!==null?c+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(l.parsed.y):c+=l.raw,c}}}},scales:t==="doughnut"?{}:{y:{beginAtZero:!0,grid:{color:"#f3f4f6"},ticks:{font:{size:11}}},x:{grid:{display:!1},ticks:{font:{size:11}}}}}};Se[e]=new Chart(i,d)}const At=(e,t="products")=>k(`/api/${t}/categories/${e}`),Eo=(e,t="products")=>k(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),Co=(e,t="products")=>k(`/api/${t}/categories/${e}`,{method:"DELETE"}),an="audit_logs",Le=async(e,t,a,o,r,s=null)=>{try{if(!t)return;await ma(Xe(Q,an),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:o,description:r,details:s,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},pe=document.getElementById("content");let ne=null,Fe="services",fe="all";function Be(){const e=J.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function on(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await Eo({establishmentId:p.establishmentId,name:o},"services"),Le(p.establishmentId,Be(),"Categorias (Serviços)","Criou",`Criou categoria: ${o}`),a.value="",g("Sucesso","Categoria criada!","success"),await ya(),await et()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function sn(e){if(await A("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await Co(e,"services"),Le(p.establishmentId,Be(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await ya(),await et()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function ya(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await At(p.establishmentId,"services");p.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                ...
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria criada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function rn(){N({title:"Gerir Categorias de Serviços",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),wa(),we()}function cn(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function et(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,o,r]=await Promise.all([Te(p.establishmentId),_(p.establishmentId),At(p.establishmentId,"services"),_s(p.establishmentId)]);p.services=(t||[]).filter(Boolean),p.professionals=(a||[]).filter(Boolean),p.serviceCategories=(o||[]).filter(Boolean),p.mostPopularService=r||{name:"N/A",count:0},p.services.forEach(s=>{s.active===void 0&&(s.active=!0)}),Io(Fe)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function Io(e){if(document.getElementById("services-content-container")){if(Fe===e&&document.getElementById("services-content-container").children.length>1){Fe==="services"&&(wa(),we());return}Fe=e,fe="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?dn():e==="reports"&&cn()}}function un(){ne&&(pe.removeEventListener("click",ne),pe.removeEventListener("input",ne),pe.removeEventListener("change",ne)),ne=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),s=r.dataset.id,n=r.checked;try{await Js(s,n);const i=p.services.findIndex(d=>d.id===s);i>-1&&(p.services[i].active=n),Le(p.establishmentId,Be(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${s}) para ${n?"Ativo":"Inativo"}`),we(),wa()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,we()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){we();return}if(!a)return;if(a.hasAttribute("data-view")){Io(a.dataset.view);return}switch(a.dataset.action){case"new-service":Ha();break;case"edit-service":const r=JSON.parse(a.dataset.service);Ha(r);break;case"manage-categories":rn();break;case"filter-service":const s=a.dataset.filterType;if(s==="popular")return;fe=s==="total"?"all":s,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,l=i===fe||i==="total"&&fe==="all";n.classList.toggle("ring-2",l),n.classList.toggle("ring-indigo-500",l),n.classList.toggle("shadow-lg",l)}),we();break}},pe.addEventListener("click",ne),pe.addEventListener("input",ne),pe.addEventListener("change",ne)}async function mn(){pe.innerHTML=`
        ...
        </section>`,un();try{(!p.professionals||p.professionals.length===0)&&(p.professionals=await _(p.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}Fe="services",fe="all",await et()}const qt="suppliers",Lo="purchases",ka=async()=>{try{const e=await mo(Xe(Q,qt)),t=[];return e.forEach(a=>{t.push({id:a.id,...a.data()})}),t}catch(e){throw console.error("Erro ao buscar fornecedores:",e),e}},pn=async e=>{try{return{id:(await ma(Xe(Q,qt),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},gn=async(e,t)=>{try{const a=po(Q,qt,e);return await os(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},bn=async e=>{try{const t=po(Q,qt,e);return await as(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},fn=async e=>{try{const t={...e,createdAt:new Date};return{id:(await ma(Xe(Q,Lo),t)).id,...t}}catch(t){throw console.error("Erro ao registrar compra:",t),t}},vn=async e=>{try{const t=es(Xe(Q,Lo),ts("createdAt","desc")),a=await mo(t),o=[];return a.forEach(r=>{o.push({id:r.id,...r.data()})}),o}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},ce=document.getElementById("content");let ie=null,Re="products",K="all";async function hn(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await Eo({establishmentId:p.establishmentId,name:o},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await $a(),await tt()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function xn(e){if(await A("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await Co(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await $a(),await tt()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function $a(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await At(p.establishmentId,"products");p.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${a.name}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function yn(){N({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",hn),t.addEventListener("click",o=>{const r=o.target.closest('button[data-action="delete-category"]');r&&xn(r.dataset.id)}))}$a()}async function wn(e){if(!e)return;if(await A("Apagar Produto","Tem a certeza que deseja apagar este produto? Esta ação não pode ser revertida."))try{await $r(e),g("Sucesso","Produto apagado com sucesso!","success"),await tt()}catch(a){g("Erro",`Não foi possível apagar o produto: ${a.message}`,"error")}}async function kn(e){const t=e.querySelector("#productId").value,a=parseInt(e.querySelector("#productCurrentStock").value),o=parseInt(e.querySelector("#productMinStock").value),r=parseInt(e.querySelector("#productMaxStock").value),s=e.querySelectorAll("#selectedSuppliersList .selected-supplier-item"),n=Array.from(s).map(d=>d.dataset.id),i={establishmentId:p.establishmentId,name:e.querySelector("#productName").value,price:parseFloat(e.querySelector("#productPrice").value),costPrice:parseFloat(e.querySelector("#productCostPrice").value)||0,commissionRate:parseFloat(e.querySelector("#productCommissionRate").value)||0,currentStock:isNaN(a)?0:a,minStock:isNaN(o)?0:o,maxStock:isNaN(r)?0:r,categoryId:e.querySelector("#productCategory").value||null,photo:e.querySelector("#productPhotoBase64").value,supplierIds:n};try{t?await kr(t,i):await wr(i),document.getElementById("productModal").style.display="none",g("Sucesso",`Produto ${t?"atualizado":"adicionado"} com sucesso!`,"success"),await tt()}catch(d){throw new Error(d.message)}}function Oa(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=d=>{const l=new Image;l.onload=()=>{let c=l.width,u=l.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(l,0,0,c,u);const f=m.toDataURL(o,r);s(f)},l.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),l.src=d.target.result},i.onerror=d=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function za(e=null){const t=document.getElementById("productModal"),a=p.categories||[],o=p.suppliers||[],r=a.map(y=>`<option value="${y.id}" ${e?.categoryId===y.id?"selected":""}>${y.name}</option>`).join("");let s=new Set(e?.supplierIds||[]);t.innerHTML=`
        ...
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                ...
                `).join("")):(h.classList.remove("hidden"),h.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else h.classList.add("hidden");s.size>0?(x.innerHTML="",s.forEach(E=>{const T=o.find(D=>D.id===E);T&&(x.innerHTML+=`
                    ...
                    `)})):x.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};u.querySelector("#modalSupplierSearch").addEventListener("input",m),u.addEventListener("click",y=>{const h=y.target.closest("[data-add-supplier]");if(h){const S=h.dataset.addSupplier;s.add(S),u.querySelector("#modalSupplierSearch").value="",m()}const x=y.target.closest("[data-remove-supplier]");if(x){const S=x.dataset.removeSupplier;s.delete(S),m()}}),m(),u.addEventListener("click",async y=>{const h=y.target.closest("button[data-action]");if(!h)return;const x=h.dataset.action,S=u.querySelector("#productId").value;if(x==="close-modal"&&(u.style.display="none"),x==="delete-product"){if(!S)return;u.style.display="none",await wn(S)}if(x==="save-product-modal"){const E=u.querySelector("#productForm");if(E){if(!E.querySelector("#productName").value||!E.querySelector("#productPrice").value){g("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const T=h.closest('button[data-action="save-product-modal"]');T.disabled=!0,T.textContent="A salvar...";try{await kn(E)}catch(D){g("Erro",`Falha ao salvar: ${D.message}`,"error"),T.disabled=!1,T.textContent="Salvar Alterações"}}}if(x==="adjust-stock-modal"){y.preventDefault();const E=u.querySelector("#stockAdjustmentAmount"),T=u.querySelector("#stockAdjustmentReason"),D=parseInt(E.value,10),H=parseInt(h.dataset.change,10);if(!D||D<=0){g("Erro","Por favor, insira uma quantidade válida.","error");return}const M=D*H,q=T.value||(M>0?"Entrada manual":"Saída manual");try{await Sr(S,{change:M,reason:q});const F=p.products.findIndex(O=>O.id===S);if(F>-1){const O=p.products[F].currentStock+M;p.products[F].currentStock=O,u.querySelector("#currentStockDisplay").textContent=O,u.querySelector("#productCurrentStock").value=O,E.value="",T.value="",g("Sucesso","Estoque atualizado!","success"),Sa(),We()}}catch(F){g("Erro de Stock",F.message,"error")}}});const b=u.querySelectorAll(".tab-btn"),f=u.querySelectorAll(".tab-content");b.forEach(y=>{y.addEventListener("click",h=>{h.preventDefault(),b.forEach(x=>{x.classList.remove("border-indigo-500","text-indigo-600"),x.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),y.classList.add("border-indigo-500","text-indigo-600"),y.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),f.forEach(x=>x.classList.add("hidden")),document.getElementById(`tab-content-${y.dataset.tab}`).classList.remove("hidden")})});const v=u.querySelector("#productPhotoInput");u.querySelector("#productPhotoButton").addEventListener("click",()=>v.click()),v.onchange=async()=>{const y=v.files[0];if(!y)return;const h=u.querySelector("#productPhotoPreview"),x=u.querySelector("#productPhotoBase64");h.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const S=await Oa(y,800,800,"image/jpeg",.8),T=S.length*3/4,D=1e3*1024;if(T>D)throw new Error("A imagem é muito grande mesmo após a compressão.");h.src=S,x.value=S}catch(S){console.error("Erro ao processar imagem:",S),g("Erro de Imagem",S.message||"Não foi possível processar a imagem.","error"),h.src=l,x.value=c,v.value=""}},u.style.display="flex"}function $n(){const e=document.getElementById("products-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),Sa(),We()}function Sn(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const o=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
        ...
        </div>`;const r=document.getElementById("productFilterReport"),s=document.getElementById("categoryFilterReport");r&&p.products&&(r.innerHTML+=p.products.map(n=>`<option value="${n.id}">${n.name}</option>`).join("")),s&&p.categories&&(s.innerHTML+=p.categories.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""))}async function En(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value};try{const a=await Er(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function tt(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,o]=await Promise.all([Pt(p.establishmentId),At(p.establishmentId,"products"),ka()]);p.products=(t||[]).filter(Boolean),p.categories=(a||[]).filter(Boolean),p.suppliers=(o||[]).filter(Boolean),Bo(Re)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function Bo(e){if(document.getElementById("products-content-container")){if(Re===e&&document.getElementById("products-content-container").children.length>1){Re==="products"&&(Sa(),We());return}Re=e,K="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?$n():e==="movements"&&Sn()}}async function Cn(){ce.innerHTML=`
        ...
        </section>`,ie&&(ce.removeEventListener("click",ie),ce.removeEventListener("input",ie),ce.removeEventListener("change",ie)),ie=async e=>{const t=e.target;if(t.id==="productSearchInput"||t.id==="productCategoryFilter"){We();return}const a=e.target.closest("button[data-action], button[data-view], .indicator-card[data-action], .product-card[data-action]");if(!a||e.target.closest('[data-action-stop-propagation="true"]'))return;if(a.hasAttribute("data-view")){Bo(a.dataset.view);return}switch(a.dataset.action){case"new-product":za();break;case"edit-product":za(JSON.parse(a.dataset.product));break;case"manage-product-categories":yn();break;case"generate-report":await En();break;case"filter-stock":const r=a.dataset.filterType;K=K===r?"all":r,document.querySelectorAll(".indicator-card").forEach(s=>{s.classList.toggle("ring-2",s.dataset.filterType===K),s.classList.toggle("ring-indigo-500",s.dataset.filterType===K),s.classList.toggle("shadow-lg",s.dataset.filterType===K)}),We();break}},ce.addEventListener("click",ie),ce.addEventListener("input",ie),ce.addEventListener("change",ie),Re="products",K="all",await tt()}const ue=document.getElementById("content");let le=null,ht="list",P={step:1,productsToBuy:[],allSuppliers:[],finalOrders:{},isQuoteMode:!1};async function In(){ht==="list"?Nt():ht==="purchases"?(P.step=1,je()):ht==="history"&&Pn()}async function Ln(){try{const e=await ka();return p.suppliers=e||[],P.allSuppliers=e,!0}catch(e){return console.error(e),!1}}async function Bn(e){if(await A("Excluir Fornecedor","Tem a certeza? Isso removerá o vínculo com os produtos."))try{await bn(e),g("Sucesso","Fornecedor excluído.","success"),Bt("genericModal"),Nt()}catch(t){g("Erro","Erro ao excluir: "+t.message,"error")}}async function Tn(e){e.preventDefault();const t=e.target,a=t.querySelector("#supId").value,o={name:t.querySelector("#supName").value,contactName:t.querySelector("#supContact").value,email:t.querySelector("#supEmail").value,phone:t.querySelector("#supPhone").value,taxId:t.querySelector("#supTaxId").value,category:t.querySelector("#supCategory").value},r=t.querySelector('button[type="submit"]');r.disabled=!0,r.textContent="A salvar...";try{a?(await gn(a,o),g("Sucesso","Fornecedor atualizado!","success")):(await pn(o),g("Sucesso","Fornecedor criado!","success")),Bt("genericModal"),Nt()}catch(s){g("Erro","Erro ao salvar: "+s.message,"error")}finally{r.disabled=!1,r.textContent="Salvar"}}async function Nt(){const e=document.getElementById("suppliersList");if(!e)return;e.innerHTML='<div class="loader mx-auto my-8"></div>',await Ln();const t=document.getElementById("supplierSearchInput")?.value.toLowerCase()||"",a=p.suppliers.filter(s=>s.name.toLowerCase().includes(t)||s.contactName&&s.contactName.toLowerCase().includes(t));if(e.innerHTML="",a.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum fornecedor encontrado.</div>';return}let o='<div class="flex flex-col gap-2 md:hidden">';a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;");o+=`
    ...
    `;a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=o+r}function Dn(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",o=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),s=`
    ...
    `;N({title:"",contentHTML:s,maxWidth:"max-w-md"})}async function je(){const e=document.getElementById("purchasesContainer");if(e)if(P.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Pt(p.establishmentId),ka()]);P.allSuppliers=a||[];const o=t.filter(l=>{const c=parseInt(l.currentStock||0),u=parseInt(l.minStock||0);return c<=u});if(P.productsToBuy=o,o.length===0){e.innerHTML=`
            ...
            `,oa()}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao calcular compras.</p>'}}else P.step===2&&Mn(e)}function Mn(e){if(!P.finalOrders||Object.keys(P.finalOrders).length===0){P.step=1,je();return}const t=P.isQuoteMode;let a="",o=0;const r=t?"border-indigo-100":"border-gray-200",s=t?"bg-indigo-50 border-indigo-100":"bg-gray-50 border-gray-200",n=t?"bg-blue-100 text-blue-700":"bg-green-100 text-green-700",i=t?"hidden":"flex",d=t?"Cotações Prontas":"Pedidos Prontos",l=t?"text-indigo-600":"text-green-600",c=t?"bg-indigo-50 border-indigo-100":"bg-green-50 border-green-100",u=t?"text-indigo-800":"text-green-800";for(const[m,b]of Object.entries(P.finalOrders)){let f=0,v=b.items.map(S=>{const E=S.qty*S.cost;return f+=E,`
    ...
    `}async function Pn(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await vn(p.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(s=>{const n=new Date(s.createdAt.seconds*1e3).toLocaleDateString("pt-BR");a+=`
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function An(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;N({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",Tn),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Bt("genericModal"))},50)}function Nn(){ue.innerHTML=`
    ...
    `,le&&(ue.removeEventListener("click",le),ue.removeEventListener("input",le),ue.removeEventListener("change",le)),le=e=>{if(e.target.closest("#tab-btn-list")&&it("list"),e.target.closest("#tab-btn-purchases")&&it("purchases"),e.target.closest("#tab-btn-history")&&it("history"),e.target.id==="toggle-quote-mode"&&(P.isQuoteMode=e.target.checked,je()),e.target.id==="supplierSearchInput"&&Nt(),e.target.closest("#btn-new-supplier")&&Va(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),o=JSON.parse(a.dataset.supplier);Dn(o)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&Bn(t.dataset.id),a==="edit"&&Va(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&oa(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(o=>o.checked=a),oa()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),o={};let r=!1;if(a.forEach(s=>{if(s.offsetParent===null||!s.querySelector(".row-select").checked)return;r=!0;let i="Produto";const d=s.querySelector("td:nth-child(2)"),l=s.querySelector(".font-bold");d?i=d.innerText:l&&(i=l.innerText);const c=parseInt(s.querySelector(".qty-input").value),u=parseFloat(s.dataset.cost),b=s.querySelector(".supplier-select").value;if(b){if(!o[b]){const f=P.allSuppliers.find(v=>v.id===b);o[b]={info:f,items:[]}}o[b].items.push({name:i,qty:c,cost:u})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}P.finalOrders=o,P.step=2,je()}if(e.target.closest("#btn-back-step1")&&(P.step=1,je()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),o=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),s=parseFloat(a.dataset.total),n=P.isQuoteMode;if(o.phone){const i=o.phone.replace(/\D/g,"");let d="";n?(d=`Olá *${o.name}*, tudo bem?
...
Aguardo retorno.`;const l=`mailto:${o.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(d)}`;window.location.href=l}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),o=JSON.parse(decodeURIComponent(a.dataset.order));o.establishmentId=p.establishmentId,fn(o).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const o=e.target.closest(".supplier-order-card").dataset.supplierId,r=P.finalOrders[o];r?qn(r,P.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),o=JSON.parse(a.dataset.purchase);An(o)}},ue.addEventListener("click",le),ue.addEventListener("input",le),ue.addEventListener("change",le),it("list")}function it(e){ht=e,["list","purchases","history"].forEach(a=>{const o=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(o.classList.add("border-indigo-500","text-indigo-600"),o.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(o.classList.remove("border-indigo-500","text-indigo-600"),o.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),In()}const Ht=document.getElementById("content"),Ua={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let oe=new Set,lt=null,ke=null;function Fn(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Rn(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,r=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function Ot(){const e=document.getElementById("genericModal");e.style.display="none",ke&&e.removeEventListener("click",ke)}async function jn(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},o=p.services||await Te(p.establishmentId),r=p.professionals||await _(p.establishmentId),s=`
        ...
        </div>`;t.innerHTML=s,t.style.display="flex",Hn(a,o),zn(a),Vn(a,r),Jn(a)}function Hn(e,t){const a=document.getElementById("professionalForm"),o=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(m,b)=>{const f=b+1,v=f==o[1]?"selected":"",y=new Date(0,b).toLocaleString("pt-BR",{month:"long"});return`<option value="${f}" ${v}>${y.charAt(0).toUpperCase()+y.slice(1)}</option>`}).join(""),s=e.status||"active";a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${e.notes||""}</textarea></div>`;const n=document.getElementById("profPhotoInput"),i=document.getElementById("profPhotoButton"),d=document.getElementById("profPhotoPreview"),l=document.getElementById("profPhotoBase64"),c=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,u=e.photo||"";i&&i.addEventListener("click",()=>n.click()),n&&(n.onchange=async()=>{const m=n.files[0];if(m){d.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const b=await On(m,800,800,"image/jpeg",.8);d.src=b,l.value=b}catch(b){g("Erro de Imagem",b.message||"Não foi possível processar a imagem.","error"),d.src=c,l.value=u,n.value=""}}})}function On(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=d=>{const l=new Image;l.onload=()=>{let c=l.width,u=l.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(l,0,0,c,u);const f=m.toDataURL(o,r);s(f)},l.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),l.src=d.target.result},i.onerror=d=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function zn(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',Un(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function Vn(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            </div>`}).join(""),e.querySelectorAll('input[type="checkbox"][data-field="active"]').forEach(a=>{a.addEventListener("change",o=>{const r=o.target.closest(".day-schedule-card"),s=!o.target.checked;r.classList.toggle("bg-white",!s),r.classList.toggle("bg-gray-100",s),r.classList.toggle("disabled",s),r.querySelectorAll(".time-inputs input").forEach(n=>n.disabled=s)})})}async function He(e,t="future"){const a=document.getElementById("blockagesList");if(a){a.innerHTML='<div class="loader mx-auto"></div>';try{const o=new Date;let r,s;t==="history"?(s=new Date,r=new Date,r.setFullYear(r.getFullYear()-2)):(r=new Date,s=new Date,s.setFullYear(s.getFullYear()+2));let i=(await Tt(p.establishmentId,r.toISOString(),s.toISOString(),e)).map(l=>({...l,startTime:new Date(l.startTime),endTime:new Date(l.endTime)}));t==="history"?i=i.filter(l=>l.endTime<o).sort((l,c)=>c.startTime-l.startTime):i=i.filter(l=>l.endTime>=o).sort((l,c)=>l.startTime-c.startTime);const d=i.reduce((l,c)=>{const u=c.reason||"Sem motivo";return l[u]||(l[u]=[]),l[u].push(c),l},{});if(Object.keys(d).length===0){a.innerHTML=`<p class="text-center text-gray-500 text-sm py-4">Nenhum bloqueio ${t==="history"?"no histórico":"futuro"}.</p>`;return}a.innerHTML=Object.entries(d).map(([l,c])=>`
        ...
        `).join("")}catch(o){a.innerHTML=`<p class="text-red-500">${o.message}</p>`}}}function Jn(e){const t=document.getElementById("genericModal");ke&&t.removeEventListener("click",ke),ke=async a=>{const o=a.target.closest("button[data-action]");if(!o){const s=a.target.closest(".tab-link");s&&(t.querySelectorAll(".tab-link").forEach(n=>n.classList.remove("active")),s.classList.add("active"),t.querySelectorAll(".tab-content").forEach(n=>n.classList.add("hidden")),document.getElementById(s.dataset.tab).classList.remove("hidden"));return}const r=o.dataset.action;switch(a.stopPropagation(),r){case"close-modal":Ot();break;case"delete-professional":const s=o.dataset.id;if(await A("Excluir Profissional",`Tem certeza que deseja excluir ${e.name}? Esta ação não pode ser desfeita.`))try{await Is(s),g("Sucesso!","Profissional excluído.","success"),Ot(),$t()}catch(v){g("Erro",`Não foi possível excluir: ${v.message}`,"error")}break;case"save-professional":const i=document.getElementById("professionalForm"),d=o,l=document.getElementById("profScheduleContainer"),c=Array.from(i.querySelectorAll("#profServicesContainer input:checked")).map(v=>v.value),u={};l&&l.querySelectorAll(".day-schedule-card").forEach(v=>{const y=v.querySelector('[data-field="active"]').dataset.day;u[y]={active:v.querySelector('[data-field="active"]').checked,start:v.querySelector('[data-field="start"]').value,end:v.querySelector('[data-field="end"]').value,breakStart:v.querySelector('[data-field="breakStart"]').value,breakEnd:v.querySelector('[data-field="breakEnd"]').value}});const m={...e,id:i.querySelector("#professionalId").value||void 0,name:i.querySelector("#profName").value,specialty:i.querySelector("#profSpecialty").value,photo:i.querySelector("#profPhotoBase64").value,services:c,workingHours:u,phone:i.querySelector("#profPhone").value,dob:`${i.querySelector("#profDobDay").value}/${i.querySelector("#profDobMonth").value}`,receivesCommission:i.querySelector("#profCommission").value==="sim",showOnAgenda:i.querySelector("#profShowOnAgenda").value==="sim",orderOnAgenda:parseInt(i.querySelector("#profOrderOnAgenda").value)||1,notes:i.querySelector("#profNotes").value,status:i.querySelector("#profStatus").value};d.disabled=!0,d.textContent="A salvar...";try{m.id?(await Cs(m.id,m),g("Sucesso!","Profissional atualizado.","success")):(delete m.id,await Es(m),g("Sucesso!","Profissional criado.","success")),Ot(),$t()}catch(v){g("Erro",v.message,"error"),d.disabled=!1,d.textContent="Salvar"}break;case"delete-blockage":const b=o.dataset.id;if(await A("Apagar Bloqueio","Tem certeza?"))try{await ba(b),g("Bloqueio removido.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";He(e.id,v)}catch(v){g("Erro",v.message,"error")}break;case"batch-delete-blockage":const f=JSON.parse(o.dataset.ids);if(await A("Apagar em Lote",`Tem certeza que deseja apagar ${f.length} bloqueios com este motivo?`))try{await bo(f),g("Bloqueios removidos.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";He(e.id,v)}catch(v){g("Erro",v.message,"error")}break}},t.addEventListener("click",ke)}function sa(){const e=document.getElementById("batch-actions-container"),t=document.getElementById("selected-count");!e||!t||(oe.size>0?(t.textContent=`${oe.size} selecionado(s)`,e.classList.remove("hidden")):e.classList.add("hidden"))}function _n(){A("Excluir em Lote",`Tem certeza que deseja excluir ${oe.size} profissionais? Esta ação não pode ser desfeita.`).then(async e=>{if(e)try{await(void 0)(Array.from(oe)),g("Sucesso!",`${oe.size} profissionais foram excluídos.`,"success"),oe.clear(),sa(),$t()}catch(t){g("Erro",`Não foi possível excluir em lote: ${t.message}`,"error")}})}function Me(){const e=document.getElementById("professionalsList");if(!e)return;if(!p.professionals){e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=Fn();return}const t=document.getElementById("showInactiveProfToggle").checked,a=document.getElementById("profSearchInput").value.toLowerCase(),o=p.professionals.filter(r=>{const s=r.name.toLowerCase().includes(a),n=t||r.status!=="inactive";return s&&n});e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=Rn(o)}async function $t(){oe.clear(),Ht.innerHTML=`
        ...
        </section>`,lt&&Ht.removeEventListener("click",lt),lt=t=>{const a=t.target.closest('[data-action="open-professional-modal"]'),o=t.target.closest('[data-action="batch-delete"]');if(a){t.preventDefault();let s={};if(a.dataset.professional)try{s=JSON.parse(a.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}jn(s);return}if(o){_n();return}const r=t.target.closest(".professional-checkbox");if(r){const s=r.dataset.id;r.checked?oe.add(s):oe.delete(s),Me(),sa();return}},Ht.addEventListener("click",lt),document.getElementById("profSearchInput").addEventListener("input",Me),document.getElementById("showInactiveProfToggle").addEventListener("change",Me);const e=document.getElementById("professionalsList");p.professionals=null,p.services=null,Me();try{const[t,a]=await Promise.all([_(p.establishmentId),Te(p.establishmentId)]);p.professionals=t,p.services=a,Me(),sa()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}const zt=document.getElementById("content");let te=[],Oe={},j=null,ra="list",U="all",na="O Estabelecimento";const Wn=(e,t)=>`Olá, ${e}! Nós da ${t} desejamos a você um Feliz Aniversário! Esperamos que seu dia seja maravilhoso. Venha comemorar conosco! 🎉🎂`,Gn=(e,t)=>`Oi, ${e}! Faz um tempo que não te vemos aqui no(a) ${t}. Sentimos sua falta! Temos novidades/ofertas especiais para você. Que tal agendar seu horário?`,Yn=[{value:30,label:"30 dias"},{value:60,label:"60 dias"},{value:90,label:"90 dias"},{value:120,label:"120 dias"}];function Xn(){return Math.floor(Math.random()*140)+10}function Qn(e){if(!e.dob)return!1;const t=e.dob.split("/");if(t.length!==2)return!1;const a=new Date,o=a.getDate(),r=a.getMonth()+1,s=parseInt(t[0],10),n=parseInt(t[1],10);return s===o&&n===r}const Zn=[{value:99,label:"Aniversariantes de Hoje"},{value:0,label:"Todos os meses (com DOB)"},{value:1,label:"Janeiro"},{value:2,label:"Fevereiro"},{value:3,label:"Março"},{value:4,label:"Abril"},{value:5,label:"Maio"},{value:6,label:"Junho"},{value:7,label:"Julho"},{value:8,label:"Agosto"},{value:9,label:"Setembro"},{value:10,label:"Outubro"},{value:11,label:"Novembro"},{value:12,label:"Dezembro"}];function Ja(){return Zn.map(e=>{let t="";return e.value===99&&(t="selected"),`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function _a(){return Yn.map(e=>{const t=e.value===90?"selected":"";return`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function Kn(e,t){const a=`w-5 h-5 ${t} mr-2`;switch(e){case"cadastro":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>`;case"agendamentos":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>`;case"historico":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v2a2 2 0 01-2 2h-2.586a1 1 0 00-.707.293l-1.414 1.414a1 1 0 01-1.414 0l-1.414-1.414A1 1 0 009.586 17H7a2 2 0 01-2-2v-2a2 2 0 012-2h12z" /></svg>`;case"fidelidade":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z" /><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.5a9.5 9.5 0 1019 0 9.5 9.5 0 00-19 0z" /></svg>`;default:return""}}function ei(e="cadastro"){const t=[{id:"cadastro",label:"Cadastro"},{id:"agendamentos",label:"Próximos Agend."},{id:"historico",label:"Histórico"},{id:"fidelidade",label:"Fidelidade"}],a=document.getElementById("client-detail-tabs");a&&(a.innerHTML=t.map(o=>{const r=e===o.id,s=r?"text-indigo-600":"text-gray-500";return`
        ...
        `}).join(""),a.querySelectorAll(".tab-btn").forEach(o=>{o.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),ia(o.dataset.tab)})}))}async function ia(e){ei(e);const t=document.getElementById("client-detail-content");if(t)switch(t.innerHTML='<form id="client-form" class="p-6 space-y-4"><div class="loader mx-auto my-8"></div></form>',e){case"cadastro":t.innerHTML=ti(j);break;case"agendamentos":case"historico":try{const o=await Ys(p.establishmentId,j.name,j.phone);t.innerHTML=ai(o,e)}catch(o){console.error("Erro ao carregar histórico do cliente:",o),t.innerHTML=`<form id="client-form" class="p-6 space-y-4"><p class="text-center text-red-500">Erro ao carregar o histórico: ${o.message}</p></form>`}break;case"fidelidade":const a=await Xs(p.establishmentId,j.name,j.phone);t.innerHTML=oi(j,a);break;default:t.innerHTML='<form id="client-form" class="p-6 space-y-4"><p class="p-4 text-center text-gray-500">Secção não implementada.</p></form>'}}function ti(e){const t=e?.dob?e.dob.split("/"):["",""];return`
    ...
    `}function oi(e,t){const a=e.loyaltyPoints||0;let o='<p class="text-sm text-gray-500">O programa de fidelidade não está ativo.</p>';Oe.enabled&&Oe.tiers&&(o=Oe.tiers.map(s=>{const n=a>=s.points;return`
    ...
    `}function la(e){j=e,ra="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",o=`
    ...
    `,r=window.innerWidth<768,s=r?"max-w-full":"max-w-3xl";if(N({title:a,contentHTML:o,maxWidth:s}),r){const c=document.getElementById("genericModal");if(c){const u=c.querySelector(`.${s.replace(":","\\:")}`);u&&(u.style.height="auto",u.style.maxHeight="85vh",u.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&n.addEventListener("click",async c=>{const u=c.target.closest("[data-action]");if(!u)return;switch(u.dataset.action){case"redeem-reward":{const b=parseInt(u.dataset.points,10),f=u.dataset.reward;if(await A("Confirmar Resgate",`Deseja resgatar "${f}" por ${b} pontos?`))try{await Qs(p.establishmentId,j.name,j.phone,{points:b,reward:f}),g("Prémio resgatado com sucesso!","success"),te=await Ze(p.establishmentId);const h=te.find(x=>x.id===j.id);h&&(j=h),ia("fidelidade")}catch(y){g(`Erro ao resgatar: ${y.message}`,"error")}break}case"open-comanda-from-history":{const b=u.dataset.appointmentId;b&&(document.getElementById("genericModal").style.display="none",Y("comandas-section",{selectedAppointmentId:b,initialFilter:"finalizada"}));break}case"view-appointment":{const b=u.dataset.appointmentId,f=u.dataset.appointmentDate;b&&f&&(document.getElementById("genericModal").style.display="none",Y("agenda-section",{targetDate:f,scrollToAppointmentId:b}));break}}}),ia("cadastro");const i=document.getElementById("client-form");i&&i.addEventListener("submit",c=>{c.preventDefault(),si()});const d=document.getElementById("cancelDetailViewBtn");d&&d.addEventListener("click",c=>{c.preventDefault(),document.getElementById("genericModal").style.display="none",Ft()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await ri()})}async function si(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await Ws(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await fa(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await Ft()}catch(o){g("Erro",`Não foi possível salvar: ${o.message}`,"error")}}async function ri(){if(!j||!j.id)return;if(await A("Excluir Cliente",`Tem certeza que deseja excluir ${j.name}? Esta ação é irreversível.`))try{await Gs(j.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await Ft()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function xt(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const o=U==="inactive",r=U==="birthdays";e.forEach(s=>{const n=document.createElement("div");n.className="client-card bg-white rounded-lg shadow p-4 flex flex-col cursor-pointer",n.dataset.clientId=s.id;const i=s.loyaltyPoints||0,d=Oe.enabled?`${i} pts`:`R$ ${i.toFixed(2)}`;let l="";const u=`https://wa.me/55${s.phone?s.phone.replace(/\D/g,""):""}?text=`;if(o){const m=encodeURIComponent(Gn(s.name,na));l=`
                    ...
                    </a>`}else if(r&&Qn(s)){const b=encodeURIComponent(Wn(s.name,na));l=`
    ...
    `,t.querySelector("#personal-data-form").addEventListener("submit",a=>{a.preventDefault();const o={ownerName:t.querySelector("#ownerName").value,name:t.querySelector("#establishmentName").value,phone:t.querySelector("#establishmentPhone").value,document:t.querySelector("#establishmentCnpjCpf").value,email:t.querySelector("#establishmentEmail").value,address:t.querySelector("#establishmentAddress").value,website:t.querySelector("#establishmentWebsite").value};De(o,a)})}function Ei(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!o||!r){g("Erro","Preencha todos os campos.","error");return}const s=t.querySelector('button[form="change-email-form"]');s.disabled=!0,s.textContent="A verificar...";try{const n=J.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Yo.credential(n.email,r);await Xo(n,i),s.textContent="A enviar link...",await Qo(n,o),s.textContent="A atualizar BD...",await $s(p.establishmentId,o),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{s.disabled=!1,s.textContent="Salvar Novo E-mail"}})}function Ii(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;const a=t.querySelector("#establishmentTimezone");if(e.timezone)a.value=e.timezone;else try{const s=Intl.DateTimeFormat().resolvedOptions().timeZone;Array.from(a.options).some(i=>i.value===s)?a.value=s:a.value="America/Sao_Paulo"}catch{a.value="America/Sao_Paulo"}const o=t.querySelector("#establishmentWorkingHoursContainer"),r=e.workingHours||{};Object.keys(Vt).forEach(s=>{const n=r[s]||{},i=Vt[s],d=n.active!==!1,l=document.createElement("div");l.className=`day-schedule-card p-4 rounded-lg ${d?"bg-gray-50":"bg-gray-100 disabled"}`,l.innerHTML=`
            ...
            </div>`,o.appendChild(l)}),o.addEventListener("change",s=>{const n=s.target.closest('.day-schedule-card input[type="checkbox"]');n&&n.closest(".day-schedule-card").classList.toggle("disabled",!n.checked)}),t.querySelector("#working-hours-form").addEventListener("submit",s=>{s.preventDefault();const n={};Object.keys(Vt).forEach(d=>{n[d]={active:t.querySelector(`#est-${d}-active`).checked,start:t.querySelector(`#est-${d}-start`).value,end:t.querySelector(`#est-${d}-end`).value,breakStart:t.querySelector(`#est-${d}-breakStart`).value,breakEnd:t.querySelector(`#est-${d}-breakEnd`).value}});const i=t.querySelector("#establishmentTimezone").value;De({workingHours:n,timezone:i},s)})}function Ti(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
             <div class="flex justify-between items-center mb-6">
    ...
    `;try{const[a,o]=await Promise.all([Ge(),Ye()]),r=e.financialIntegration||{},s=e.commissionConfig||{};t.querySelector("#financialNatureId").innerHTML=dt(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=dt(o,r.defaultCentroDeCustoId),t.querySelector("#commissionNatureId").innerHTML=dt(a,s.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=dt(o,s.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const o={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};De(o,a)})}function Mi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <h3 class="text-xl font-bold text-gray-800">${e}</h3>
            <p class="mt-4 text-gray-500">Esta secção ainda não foi implementada.</p>
        </div>
    `}function No(e="indigo",t){const a=t.querySelector("#color-palette-container"),o=t.querySelector("#establishmentThemeColor");!a||!o||(a.innerHTML="",Object.entries($i).forEach(([r,s])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{o.value=r,No(r,t)}),a.appendChild(i)}),o.value=e)}function Pi(e,t){const a=t.querySelector("#slotIntervalContainer"),o=t.querySelector("#establishmentSlotInterval");if(!a||!o)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(s=>{const n=s.value===e;return`<button type="button" data-value="${s.value}" 
    ...
    `,be.querySelector('button[data-action="back-to-list"]').addEventListener("click",o=>{o.preventDefault(),Fo()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":Si(V,a);break;case"change-password":Ei(V,a);break;case"change-email":Ci(V,a);break;case"branding":Ii(V,a);break;case"booking":Li(V,a);break;case"working-hours":Bi(V,a);break;case"loyalty":Ti(V,a);break;case"financial":await Di(V,a);break;default:Mi(t?t.label:"Definições",a)}}async function Fo(){if(be.innerHTML=`
    ...
    `,be.querySelector("#settings-menu-list").addEventListener("click",o=>{const r=o.target.closest("button[data-section]");r&&(o.preventDefault(),Ai(r.dataset.section))});const a=be.querySelector('[data-action="go-to-my-profile"]');a&&a.addEventListener("click",o=>{o.preventDefault(),Y("my-profile-section")})}const ze=document.getElementById("content");async function Ee(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,o=document.getElementById("filterEndDate")?.value,r=await Tt(p.establishmentId,a||new Date().toISOString().split("T")[0],o||new Date().toISOString().split("T")[0],e),s=document.getElementById("filterReason")?.value.toLowerCase(),n=s?r.filter(d=>d.reason&&d.reason.toLowerCase().includes(s)):r,i=n.reduce((d,l)=>{const c=l.reason||"Sem motivo";return d[c]||(d[c]=[]),d[c].push(l),d},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([d,l])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let u=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                    ...
                    </div>`;c.innerHTML+=x}),t.appendChild(c)})}catch(a){t.innerHTML=`<p class="text-center text-red-500">Erro: ${a.message}</p>`}}}async function qi(e){e.preventDefault();const t=e.target,a=t.querySelector("#blockageProfId").value,o=t.querySelector("#blockageDate").value,r=t.querySelector("#blockageEndDate").value||o,s=t.querySelector("#blockageStartTime").value,n=t.querySelector("#blockageEndTime").value,i={establishmentId:p.establishmentId,professionalId:a,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:t.querySelector("#blockageReason").value};try{await Dt(i),t.reset(),g("Sucesso","Bloqueio adicionado com sucesso!","success"),Ee(a)}catch(d){g("Erro",`Não foi possível criar o bloqueio: ${d.message}`,"error")}}async function Ni(e){e.preventDefault();const t=e.target,a=Array.from(t.querySelectorAll('input[name="batch-professionals"]:checked')).map(c=>c.value);if(a.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const o=t.querySelector("#batchBlockageDate").value,r=t.querySelector("#batchBlockageEndDate").value||o,s=t.querySelector("#batchBlockageStartTime").value,n=t.querySelector("#batchBlockageEndTime").value,i=t.querySelector("#batchBlockageReason").value,d=t.querySelector('button[type="submit"]');d.disabled=!0,d.textContent="Aguarde...";const l=a.map(c=>{const u={establishmentId:p.establishmentId,professionalId:c,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:i};return Dt(u)});try{await Promise.all(l),g("Sucesso",`${a.length} bloqueios foram criados com sucesso!`,"success"),t.reset(),t.querySelectorAll('input[name="batch-professionals"]:checked').forEach(u=>u.checked=!1);const c=document.getElementById("blockageProfId").value;c&&Ee(c)}catch(c){g("Erro",`Ocorreu um erro: ${c.message}`,"error")}finally{d.disabled=!1,d.textContent="Adicionar Bloqueio em Lote"}}function Fi(e){ze.addEventListener("submit",t=>{t.target.id==="blockageForm"&&qi(t),t.target.id==="batchBlockageForm"&&Ni(t)}),ze.addEventListener("input",t=>{t.target.matches("#filterStartDate, #filterEndDate, #filterReason")&&Ee(e)}),ze.addEventListener("click",async t=>{const a=t.target.closest("button[data-action]");if(!a)return;const o=a.dataset.action;if(o==="back-to-professionals")Y("profissionais-section");else if(o==="delete-blockage"){if(await A("Apagar Bloqueio","Tem a certeza que deseja apagar este bloqueio?"))try{await ba(a.dataset.id),g("Sucesso","Bloqueio removido.","success"),Ee(e)}catch(s){g("Erro",`Não foi possível remover o bloqueio: ${s.message}`,"error")}}else if(o==="batch-delete-blockage"){const r=JSON.parse(a.dataset.ids);if(await A("Apagar Lote de Bloqueios",`Tem certeza que deseja apagar ${r.length} bloqueios de uma vez?`))try{await bo(r),g("Sucesso",`${r.length} bloqueios removidos.`,"success"),Ee(e)}catch(n){g("Erro",`Não foi possível apagar os bloqueios: ${n.message}`,"error")}}})}async function Ri(e){const{professionalId:t,professionalName:a}=e;if(!t||!a){ze.innerHTML='<p class="text-red-500 p-8">Erro: ID do profissional não fornecido.</p>';return}ze.innerHTML=`
        <section>
        ...
        </section>`,Fi(t),await Ee(t);const o=document.getElementById("batchProfSelectionContainer");try{const r=await _(p.establishmentId);o.innerHTML=r.map(s=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{o.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const ji=e=>k(`/api/users/${e}`),Hi=e=>k("/api/users",{method:"POST",body:JSON.stringify(e)}),Oi=(e,t)=>k(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),zi=e=>k(`/api/users/${e}`,{method:"DELETE"}),Vi=(e,t)=>k(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),Ui=(e,t)=>k(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),me=document.getElementById("content"),Ji={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},_i={view:"Visualizar",create:"Criar",edit:"Editar"};let Pe=null,Ae=null;function Wi(e){const t=document.getElementById("usersListContainer");if(!t)return;const a=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const o=a?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${o}</p>`;return}e.sort((o,r)=>(o.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(o=>{const r=JSON.stringify(o).replace(/'/g,"&apos;"),s=o.status==="active",n=p.professionals.find(c=>c.id===o.professionalId),i=n?n.name:"N/A",d=n?n.name.charAt(0):o.name.charAt(0),l=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(d)}`;return`
    ...
    `}).join("")}function wt(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=p.users:t=p.users.filter(a=>a.status==="active"),Wi(t)}function Gi(e={}){return Object.entries(Ji).map(([t,a])=>{const o=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,s=Object.entries(_i).map(([i,d])=>`
    ...
    `,Pe&&me.removeEventListener("click",Pe),Ae&&me.removeEventListener("change",Ae),Pe=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("click",Pe);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":Ga();break;case"edit-user":const o=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));Ga(o);break;case"back-to-list":St();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await A("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await zi(r),g("Usuário excluído com sucesso!","success"),St()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Ae=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("change",Ae);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")wt();else if(t){e.stopPropagation();const a=t.dataset.userId,o=t.checked?"active":"inactive";try{await Ui(a,o),g(`Usuário ${o==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=p.users.findIndex(s=>s.id===a);r>-1&&(p.users[r].status=o,wt())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,wt()}}},me.addEventListener("click",Pe),me.addEventListener("change",Ae),await Yi()}const Xi=document.getElementById("content");let Ya={},da=null;function Qi(){Object.values(Ya).forEach(e=>e?.destroy()),Ya={}}function Zi(e,t){const{jsPDF:a}=window.jspdf,o=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(o.setFontSize(18),o.text(e,o.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];o.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const s=o.lastAutoTable?o.lastAutoTable.finalY+20:60;o.text("Detalhes das Vendas",20,s),o.autoTable({html:`#${t}`,startY:s+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),o.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function Xa(e){const t=document.getElementById("genericModal"),a=(e.payments||[]).map(o=>`
    ...
    `,t.style.display="flex"}function Ki(e){const{summary:t,transactions:a}=e;document.getElementById("summary-revenue").textContent=`R$ ${t.totalRevenue.toFixed(2)}`,document.getElementById("summary-transactions").textContent=t.totalTransactions,document.getElementById("summary-avg-ticket").textContent=`R$ ${t.averageTicket.toFixed(2)}`;const o=document.getElementById("paymentSummaryTableBody"),r=Object.entries(t.paymentMethodTotals).sort(([,i],[,d])=>d-i);o.innerHTML=r.map(([i,d])=>`
        <tr class="border-b">
    ...
    `).join(""),s.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const d=i.dataset.transactionIndex,l=da.transactions[d];l&&Xa(l)})}),n.innerHTML=a.map((i,d)=>`
    ...
    `).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const d=i.dataset.transactionIndex,l=da.transactions[d];l&&Xa(l)})})}async function Qa(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const o=t.value,r=a.value;if(!o||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const s=document.getElementById("cashierSessionFilter").value,n=await Hs({establishmentId:p.establishmentId,startDate:o,endDate:r,cashierSessionId:s});da=n,e.innerHTML=`
        ...
        `,Ki(n)}catch(s){g("Erro",`Não foi possível carregar o relatório: ${s.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${s.message}</p>`}}async function el(){Qi();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Xi.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Qa),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const o=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,s=`Relatorio_Vendas_${o}_a_${r}`;Zi(s,"transactionsTable")});try{const o=await Lr(),r=document.getElementById("cashierSessionFilter");o.forEach(s=>{const n=new Date(s.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=s.closedByName||"N/A";r.innerHTML+=`<option value="${s.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await Qa()}const tl=document.getElementById("content");let C={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},Ut=null,ct=null,ut=null;function Ea(e){const t=new Map,a=[];return e&&(e.forEach(o=>t.set(o.id,{...o,children:[]})),t.forEach(o=>{o.parentId&&t.has(o.parentId)?t.get(o.parentId).children.push(o):a.push(o)})),a}function Ro(e,t,a){if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const o=(r,s=0)=>{const n="— ".repeat(s);return`
    ...
    `,e.style.display="flex"}function mt(e,t="all"){let a='<option value="all">Todos</option>';const o=n=>{const i=new Map,d=[];return n&&(n.forEach(l=>i.set(l.id,{...l,children:[]})),i.forEach(l=>{l.parentId&&i.has(l.parentId)?i.get(l.parentId).children.push(l):d.push(l)})),d},r=(n,i=0)=>{const d=i>0?"— ".repeat(i):"",l=n.id===t?"selected":"";a+=`<option value="${n.id}" ${l}>${d}${n.name}</option>`,n.children.forEach(c=>r(c,i+1))};return o(e).forEach(n=>r(n)),a}async function ve(){const e=document.getElementById("financial-content"),t=document.getElementById("filterStartDate")?.value,a=document.getElementById("filterEndDate")?.value,o=document.getElementById("filterNaturezaId")?.value,r=document.getElementById("filterCostCenterId")?.value;if(!t||!a){try{const[i,d]=await Promise.all([Ge(),Ye()]);C={...C,natures:i,costCenters:d},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(C.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(C.costCenters))}catch(i){g("Erro",`Não foi possível carregar os dados base: ${i.message}`,"error")}ca(),ao();return}const s=document.getElementById("payables-list"),n=document.getElementById("receivables-list");s&&(s.innerHTML='<div class="loader mx-auto"></div>'),n&&(n.innerHTML='<div class="loader mx-auto"></div>');try{const i={startDate:t,endDate:a};o&&o!=="all"&&(i.natureId=o),r&&r!=="all"&&(i.costCenterId=r);const[d,l,c,u]=await Promise.all([mi(i),vi(i),Ge(),Ye()]),m=l.previousBalance-d.previousBalance;C={...C,payables:d.entries,receivables:l.entries,natures:c,costCenters:u,previousBalance:m,filterNaturezaId:o,filterCostCenterId:r},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(C.natures,C.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(C.costCenters,C.filterCostCenterId)),ca(),ao()}catch(i){g("Erro",`Não foi possível carregar os dados: ${i.message}`,"error"),e&&(e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function rl(e,t,a=null){e.preventDefault();const o=e.target,r=o.querySelector('[name="status"]').checked,s=o.querySelector('[name="paymentDate"]').value,n=parseFloat(o.querySelector('[name="amount"]').value),i=parseInt(o.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!s){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const d={description:o.querySelector('[name="description"]').value,amount:n,dueDate:o.querySelector('[name="dueDate"]').value,naturezaId:o.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:o.querySelector('[name="centroDeCustoId"]').value||null,notes:o.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?s:null,installments:a?1:i};try{a?(await(t==="payable"?pi(a,d):hi(a,d)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?ui(d):fi(d)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await ve()}catch(l){g("Erro",`Não foi possível salvar: ${l.message}`,"error")}}async function nl(e,t){if(await A("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?gi(t):xi(t)),g("Sucesso","Lançamento excluído!","success"),await ve()}catch(o){g("Erro",`Falha ao excluir: ${o.message}`,"error")}}async function il(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?bi(t,a):yi(t,a)),g("Sucesso","Lançamento atualizado!","success"),await ve()}catch(o){g("Erro",`Falha ao atualizar status: ${o.message}`,"error")}}function to(e){const t=C.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function ca(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(C.natures.map(i=>[i.id,i.name])),o=new Map(C.costCenters.map(i=>[i.id,i.name])),r=to(C.payables),s=to(C.receivables),n=(i,d)=>{const l=i.status==="paid",c=JSON.stringify(i).replace(/'/g,"&apos;"),u=i.naturezaId?a.get(i.naturezaId):"N/A",m=i.centroDeCustoId?o.get(i.centroDeCustoId):"N/A";let b=d==="payable"?"text-red-600":"text-green-600";const f=l?"bg-gray-200 text-gray-600":d==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",v=l?"Finalizado":"Pendente";return l&&(b="text-gray-500"),`
    ...
    `;const r=document.getElementById("main-fab-btn"),s=document.getElementById("fab-menu");if(r&&s){r.addEventListener("click",v=>{v.stopPropagation(),s.classList.toggle("hidden"),r.classList.toggle("rotate-45")});const m=s.querySelector('button[data-action="open-modal"][data-type="receivable"]'),b=s.querySelector('button[data-action="open-modal"][data-type="payable"]'),f=s.querySelector('button[data-action="open-cash-flow-modal"]');m&&m.addEventListener("click",v=>{v.stopPropagation(),Jt("receivable")}),b&&b.addEventListener("click",v=>{v.stopPropagation(),Jt("payable")}),f&&f.addEventListener("click",v=>{v.stopPropagation(),eo()})}ct&&document.body.removeEventListener("click",ct),ut&&document.getElementById("genericModal").removeEventListener("click",ut);const n=()=>{const m=document.getElementById("filterStartDate"),b=document.getElementById("filterEndDate"),f=document.getElementById("filterNaturezaId"),v=document.getElementById("filterCostCenterId");C.startDate=m.value,C.endDate=b.value,C.filterNaturezaId=f.value,C.filterCostCenterId=v.value;const y=document.getElementById("advanced-filters");y&&y.classList.contains("hidden")===!1&&window.innerWidth<768&&y.classList.add("hidden"),ve()},i=m=>{const b=m.target.closest("[data-status-filter]");if(!b)return;const f=b.dataset.statusFilter;C.currentFilter=f,document.querySelectorAll("[data-status-filter]").forEach(v=>{v.classList.remove("bg-blue-100","text-blue-800"),v.classList.add("bg-gray-100","text-gray-600")}),b.classList.remove("bg-gray-100","text-gray-600"),b.classList.add("bg-blue-100","text-blue-800"),ca()},d=m=>{const b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("btn-payables-view"),y=document.getElementById("btn-receivables-view");window.innerWidth>=1024&&C.currentListView===m||(m==="payables"?(b.classList.remove("hidden"),f.classList.add("hidden"),v&&(v.classList.remove("bg-gray-200"),v.classList.add("bg-red-100","border","border-red-500")),y&&(y.classList.remove("bg-green-100","border","border-green-500"),y.classList.add("bg-gray-200"))):(b.classList.add("hidden"),f.classList.remove("hidden"),v&&(v.classList.remove("bg-red-100","border","border-red-500"),v.classList.add("bg-gray-200")),y&&(y.classList.remove("bg-gray-200"),y.classList.add("bg-green-100","border","border-green-500"))),C.currentListView=m)};document.getElementById("applyDateFilterBtn").addEventListener("click",n),document.getElementById("filterNaturezaId").addEventListener("change",()=>{C.filterNaturezaId=document.getElementById("filterNaturezaId").value}),document.getElementById("filterCostCenterId").addEventListener("change",()=>{C.filterCostCenterId=document.getElementById("filterCostCenterId").value}),document.querySelectorAll("[data-status-filter]").forEach(m=>{m.addEventListener("click",i)}),ct=m=>{const b=m.target.closest("button[data-action]");if(!b)return;const{action:f,type:v,id:y}=b.dataset;f==="edit"?Jt(v,JSON.parse(b.dataset.item.replace(/&apos;/g,"'"))):f==="delete"?nl(v,y):f==="mark-as-paid"?il(v,y):f==="manage-natures"?Za("nature"):f==="manage-cost-centers"?Za("cost-center"):f==="open-cash-flow-modal"?eo():f==="toggle-filters"?document.getElementById("advanced-filters")?.classList.toggle("hidden"):f==="open-indicators-modal"?ol():f==="open-settings-modal"?sl():f==="toggle-list-view"&&d(b.dataset.list)},ut=m=>{const b=m.target.closest('button[data-action^="delete-"]');if(b){const f=b.dataset.action.split("-")[1];l(f,b.dataset.id)}},document.body.addEventListener("click",ct),document.getElementById("genericModal").addEventListener("click",ut);async function l(m,b){const f=m==="nature",v=f?li:ci,y=f?Ge:Ye,h=f?"natures":"costCenters",x=document.getElementById("hierarchyList");if(await A("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await v(b);const E=await y();C[h]=E,Ro(x,Ea(E),m),await ve()}catch(E){g("Erro",`Não foi possível apagar: ${E.message}`,"error")}}const c=()=>{const m=window.innerWidth<1024,b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("list-toggle-buttons");b&&f&&(b.classList.remove("hidden"),f.classList.remove("hidden"),m?(b.classList.remove("lg:col-span-1"),f.classList.remove("lg:col-span-1"),v?.classList.remove("hidden"),d(C.currentListView)):(b.classList.add("lg:col-span-1"),f.classList.add("lg:col-span-1"),v?.classList.add("hidden"),b.classList.remove("hidden"),f.classList.remove("hidden")))};c(),window.addEventListener("resize",c);const u=document.querySelector(`[data-status-filter="${C.currentFilter}"]`);u&&(document.querySelectorAll("[data-status-filter]").forEach(m=>{m.classList.remove("bg-blue-100","text-blue-800"),m.classList.add("bg-gray-100","text-gray-600")}),u.classList.remove("bg-gray-100","text-gray-600"),u.classList.add("bg-blue-100","text-blue-800"));try{const m=await ki(),b=document.getElementById("summary-today-payables");b&&(b.textContent=`R$ ${m.totalPayables.toFixed(2)}`);const f=document.getElementById("summary-today-receivables");f&&(f.textContent=`R$ ${m.totalReceivables.toFixed(2)}`)}catch{g("Erro","Não foi possível carregar o resumo do dia.","error")}await ve()}const dl=e=>k("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),cl=e=>k("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),ul=(e,t)=>{const a=new URLSearchParams({startDate:e,endDate:t}).toString();return k(`/api/commissions/stats?${a}`)},ml=(e={})=>{Object.keys(e).forEach(o=>(e[o]===void 0||e[o]===null||e[o]==="")&&delete e[o]);const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return k(a)},pl=e=>k(`/api/commissions/report/${e}`,{method:"DELETE"}),Et=new Date,oo=new Date(Et.getFullYear(),Et.getMonth(),1),L={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:oo.toISOString().split("T")[0],dashEndDate:Et.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:oo.toISOString().split("T")[0],histEndDate:Et.toISOString().split("T")[0],histProfessionalId:"all"};let pt=null;const qe=document.getElementById("content");async function gl(){try{L.professionals=await _(p.establishmentId)}catch(e){console.error("Erro profissionais",e)}hl(),bl(),Rt(),Ct("dashboard")}function bl(){pt&&qe.removeEventListener("click",pt),pt=e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.action,o=t.dataset.id,r=t.dataset.idx;switch(a){case"tab-nav":Ct(t.dataset.tab);break;case"toggle-all-profs":fl();break;case"back-to-filters":L.calculationResult=null,It(document.getElementById("commissions-content"));break;case"view-preview-items":kl(r);break;case"save-final-report":$l();break;case"start-new-calc":Ct("calculator");break;case"print-receipt":Sl(o);break;case"delete-report":El(o);break;case"filter-dashboard":Rt();break;case"filter-history":Ca();break}},qe.addEventListener("click",pt),qe.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;yl(t)}},qe.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),vl())}}async function Rt(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(L.dashStartDate=e.value),t&&(L.dashEndDate=t.value);const a=document.getElementById("dashboard-stats-container");a&&(a.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const o=await ul(L.dashStartDate,L.dashEndDate);L.dashStats={revenue:o.totalRevenue||0,commissions:o.totalCommissionsPaid||0},L.currentTab==="dashboard"&&jo(document.getElementById("commissions-content"))}catch(o){console.error(o),a&&(a.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function Ca(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),a=document.getElementById("hist-prof");e&&(L.histStartDate=e.value),t&&(L.histEndDate=t.value),a&&(L.histProfessionalId=a.value);const o=document.getElementById("history-list-container");if(o){o.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await ml({startDate:L.histStartDate,endDate:L.histEndDate,professionalId:L.histProfessionalId});L.historyData=r,Ho(o,r)}catch{o.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function fl(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(a=>a.checked);e.forEach(a=>a.checked=!t)}async function vl(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(s=>s.value);if(e.length===0)return g("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},a=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),o=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");L.periodString=`${a} a ${o}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const s=await dl(t);L.calculationResult=s.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),It(r)}catch(s){g("Erro",s.message,"error"),L.calculationResult=null,It(r)}}function hl(){qe.innerHTML=`
    ...
    `}function xl(e){const t=L.professionals.map(a=>`<option value="${a.id}" ${L.histProfessionalId===a.id?"selected":""}>${a.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `,L.historyData.length>0?Ho(document.getElementById("history-list-container"),L.historyData):Ca()}function Ho(e,t){if(t.length===0){e.innerHTML=`
    ...
    `}function yl(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),a=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let o=0,r=0;if(t.forEach(s=>{s.value&&(o=parseFloat(s.value))}),a.forEach(s=>{s.value&&(r=parseFloat(s.value))}),L.calculationResult&&L.calculationResult[e]){const s=L.calculationResult[e];s.extraDebit=o,s.extraCredit=r,s.finalValue=s.summary.totalCommission-o+r,t.forEach(i=>{i!==document.activeElement&&(i.value=o||"")}),a.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${s.finalValue.toFixed(2)}`),wl()}}function wl(){const e=L.calculationResult.reduce((a,o)=>a+o.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(a=>a.innerText=`R$ ${e.toFixed(2)}`)}function kl(e){const t=L.calculationResult[e];if(!t)return;const a=t.items.map(o=>`
    ...
    `).join("");N({title:"Detalhes da Comissão",contentHTML:`<div class="bg-gray-50 p-3 rounded-lg mb-4 flex justify-between items-center"><div><p class="text-xs text-gray-500">Profissional</p><p class="font-bold text-gray-800">${t.professionalName}</p></div><div class="text-right"><p class="text-xs text-gray-500">Total Itens</p><p class="font-bold text-gray-800">${t.items.length}</p></div></div><div class="border rounded-lg overflow-hidden max-h-[60vh] overflow-y-auto">${a}</div>`,maxWidth:"max-w-md"})}async function $l(){const e=L.calculationResult.length;if(await A("Confirmar",`Gerar ${e} relatórios?`))try{const a=L.calculationResult.map(o=>cl({professionalId:o.professionalId,professionalName:o.professionalName,period:L.periodString,reportData:{...o,summary:{...o.summary,finalValue:o.finalValue,extraDebit:o.extraDebit||0,extraCredit:o.extraCredit||0,notes:o.notes||""}}}));await Promise.all(a),g("Sucesso","Pagamentos registrados!","success"),L.calculationResult=null,Rt(),Ct("history")}catch(a){g("Erro",a.message,"error")}}function Sl(e){const t=L.historyData.find(a=>a.id===e);t&&Cl(t)}async function El(e){if(await A("Excluir","Deseja remover este registro?"))try{await pl(e),g("Sucesso","Registro removido.","success"),Ca(),Rt()}catch(a){g("Erro",a.message,"error")}}function Cl(e){const{jsPDF:t}=window.jspdf;if(!t)return g("Erro","PDF lib não carregada.","error");const a=new t,o=a.internal.pageSize.getWidth()/2;a.setFontSize(18),a.setFont(void 0,"bold"),a.text("RECIBO DE PAGAMENTO DE COMISSÃO",o,20,{align:"center"}),a.setFontSize(12),a.setFont(void 0,"normal"),a.text(`Profissional: ${e.professionalName}`,15,40),a.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),a.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const s=a.lastAutoTable.finalY+10;a.setFontSize(14),a.setFont(void 0,"bold"),a.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,s,{align:"right"}),a.save(`Recibo_${e.professionalName}.pdf`)}function It(e){if(L.calculationResult){const t=L.calculationResult,a=t.reduce((s,n)=>s+n.finalValue,0),o=t.map((s,n)=>`
            ...
            </form>`}}const _t=document.getElementById("content");let he={allPackages:[],catalogForModal:{services:[],products:[]}},gt=null,$e=null;function Il(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Ll(){const e=document.getElementById("packagesListContainer");if(e){if(he.allPackages.length===0){e.innerHTML=`
        ...
        `}).join("")}}function so(){const e=document.getElementById("genericModal");e.style.display="none",$e&&e.removeEventListener("click",$e)}async function ro(e=null){const t=document.getElementById("genericModal"),a=!!e,o=e?JSON.parse(JSON.stringify(e.items||[])):[],r=`
    ...
    `;t.innerHTML=r,t.style.display="flex";const s=t.querySelector("#package-items-list"),n=(d,l)=>{const c=l.querySelector("#originalPrice"),u=d.reduce((m,b)=>m+b.price*b.quantity,0);c&&(c.textContent=`R$ ${u.toFixed(2)}`)},i=d=>{d.length===0?s.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':s.innerHTML=d.map((l,c)=>{const u=l.type==="service",m=u?"Serviço":"Produto",b=u?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
            ...
            `}).join(""),n(d,t)};i(o),s.addEventListener("change",d=>{if(d.target.classList.contains("quantity-input")){const l=parseInt(d.target.dataset.index,10),c=parseInt(d.target.value,10);c>0&&o[l]&&(o[l].quantity=c,i(o))}}),s.addEventListener("click",d=>{if(d.target.classList.contains("remove-item-btn")){const l=parseInt(d.target.dataset.index,10);o.splice(l,1),i(o)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>Bl(d=>{const l=o.find(c=>c.id===d.id&&c.type===d.type);l?l.quantity++:o.push({...d,quantity:1}),i(o)}),$e&&t.removeEventListener("click",$e),$e=async d=>{const l=d.target.closest("button[data-action]");if(!l)return;const c=l.dataset.action;if(d.stopPropagation(),c==="close-modal"&&so(),c==="save-package"){const u=l,m={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:o,originalPrice:o.reduce((b,f)=>b+f.price*f.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null};if(!m.name||!m.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(m.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}u.disabled=!0,u.textContent="A salvar...";try{a?await Dr(m.id,m):(delete m.id,await Tr(m)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),so(),await Ia()}catch(b){g("Erro",`Não foi possível salvar o pacote: ${b.message}`,"error"),u.disabled=!1,u.textContent="Salvar Pacote"}}},t.addEventListener("click",$e)}function Bl(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const o={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=d=>{const l=t.toLowerCase(),c=he.catalogForModal.services.filter(f=>f.name.toLowerCase().includes(l)),u=he.catalogForModal.products.filter(f=>f.name.toLowerCase().includes(l)),m=c.map(f=>`
    ...
    `,gt&&_t.removeEventListener("click",gt),gt=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const o=e.target.closest('[data-action="delete-package"]');if(o){const r=o.dataset.id;A("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async s=>{if(s)try{await Mr(r),g("Sucesso!","Pacote excluído.","success"),await Ia()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")ro(null);else if(a==="edit-package"){const o=JSON.parse(t.dataset.package);ro(o)}},_t.addEventListener("click",gt);try{const[e,t,a]=await Promise.all([wo(p.establishmentId),Te(p.establishmentId),Pt(p.establishmentId)]);he.allPackages=e,he.catalogForModal={services:t.filter(o=>o.active),products:a},Ll()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const Tl=document.getElementById("content");let Dl=null;async function Ml(){Tl.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await Pl()}async function Pl(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=p.userProfessionalId;if(t){const a=await Ss(t);Dl=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo),e.innerHTML=`
            ...
            `,Al(a.id),document.getElementById("my-blocks-filter").addEventListener("change",r=>Lt(a.id,r.target.value)),Lt(a.id,"future")}else e.innerHTML=`
        ...
        `}}function Al(e){const t=document.getElementById("block-schedule-form");t.addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#blockDate").value,r=t.querySelector("#blockStartTime").value,s=t.querySelector("#blockEndTime").value,n=t.querySelector("#blockReason").value;if(!o||!r||!s){g("Erro","Por favor, preencha a data e os horários de início e fim.","error");return}if(r>=s){g("Erro","A hora de início deve ser anterior à hora de fim.","error");return}const i=new Date(`${o}T${r}:00`),d=new Date(`${o}T${s}:00`),l=t.querySelector('button[type="submit"]');l.disabled=!0,l.textContent="A bloquear...";try{await Dt({establishmentId:p.establishmentId,professionalId:e,reason:n||"Bloqueado (Meu Perfil)",startTime:i.toISOString(),endTime:d.toISOString()}),g("Sucesso","Agenda bloqueada com sucesso!","success"),t.reset();const c=document.getElementById("my-blocks-filter").value;Lt(e,c)}catch(c){console.error("Erro ao bloquear agenda:",c),g("Erro",`Não foi possível bloquear a agenda: ${c.message}`,"error")}finally{l.disabled=!1,l.textContent="Bloquear Agenda"}})}async function Lt(e,t="future"){const a=document.getElementById("my-blocks-list");a.innerHTML='<p class="text-gray-500">A carregar bloqueios...</p>';try{const o=new Date;let r,s;t==="history"?(s=new Date,r=new Date,r.setFullYear(r.getFullYear()-1)):(r=new Date,s=new Date,s.setFullYear(s.getFullYear()+1));let i=(await Tt(p.establishmentId,r.toISOString(),s.toISOString(),e)).map(d=>({...d,startTime:new Date(d.startTime),endTime:new Date(d.endTime)}));t==="history"?i=i.filter(d=>d.endTime<o).sort((d,l)=>l.startTime-d.startTime):i=i.filter(d=>d.endTime>=o).sort((d,l)=>d.startTime-l.startTime),i.length>0?(a.innerHTML=i.map(d=>{const l=d.startTime.toLocaleDateString("pt-BR"),c=`${d.startTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${d.endTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`;return`
                ...
                `}).join(""),a.querySelectorAll(".remove-block-btn").forEach(d=>{d.addEventListener("click",async l=>{const c=l.currentTarget.dataset.blockId;if(confirm("Tem certeza que deseja remover este bloqueio?"))try{await ba(c),g("Sucesso","Bloqueio removido.","success"),Lt(e,t)}catch(u){console.error("Erro ao remover bloqueio:",u),g("Erro",`Não foi possível remover o bloqueio: ${u.message}`,"error")}})})):a.innerHTML=`<p class="text-gray-500 py-4 text-center">Nenhum bloqueio ${t==="history"?"no histórico recente":"futuro agendado"}.</p>`}catch(o){console.error("Erro ao carregar bloqueios:",o),a.innerHTML=`<p class="text-red-500">Erro ao carregar bloqueios: ${o.message}</p>`}}const bt=document.getElementById("loadingScreen"),Wt=document.getElementById("dashboardContent"),Gt=document.getElementById("content"),no=document.getElementById("notificationBell"),Yt=document.getElementById("notificationBadge"),ft=document.getElementById("notificationPanel"),io=document.getElementById("notificationList"),Xt=document.getElementById("profileMenuButton"),X=document.getElementById("profileDropdown"),ql=document.getElementById("profileName"),Nl=document.getElementById("profileEmail"),Fl=document.getElementById("logoutButton"),Rl=document.getElementById("cancellationHistoryBtn"),lo=document.getElementById("myProfileLink"),co={indigo:{main:"#4f46e5",hover:"#4338ca",light:"#e0e7ff",text:"#ffffff"},blue:{main:"#2563eb",hover:"#1d4ed8",light:"#dbeafe",text:"#ffffff"},sky:{main:"#0284c7",hover:"#0369a1",light:"#e0f2fe",text:"#ffffff"},teal:{main:"#0d9488",hover:"#0f766e",light:"#ccfbf1",text:"#ffffff"},emerald:{main:"#059669",hover:"#047857",light:"#d1fae5",text:"#ffffff"},green:{main:"#16a34a",hover:"#15803d",light:"#dcfce7",text:"#ffffff"},lime:{main:"#65a30d",hover:"#4d7c0f",light:"#ecfccb",text:"#ffffff"},amber:{main:"#d97706",hover:"#b45309",light:"#fef3c7",text:"#1f2937"},orange:{main:"#ea580c",hover:"#c2410c",light:"#ffedd5",text:"#ffffff"},red:{main:"#dc2626",hover:"#b91c1c",light:"#fee2e2",text:"#ffffff"},rose:{main:"#e11d48",hover:"#be123c",light:"#ffe4e6",text:"#ffffff"},pink:{main:"#db2777",hover:"#be185d",light:"#fce7f3",text:"#ffffff"},fuchsia:{main:"#c026d3",hover:"#a21caf",light:"#fae8ff",text:"#ffffff"},purple:{main:"#7c3aed",hover:"#6d28d9",light:"#ede9fe",text:"#ffffff"},violet:{main:"#8b5cf6",hover:"#7c3aed",light:"#ddd6fe",text:"#ffffff"},gray:{main:"#4b5563",hover:"#374151",light:"#f3f4f6",text:"#ffffff"},black:{main:"#111827",hover:"#000000",light:"#e5e7eb",text:"#ffffff"}};let Ve=null,Ue=[];const jl={"agenda-section":xo,"comandas-section":ha,"relatorios-section":Gr,"servicos-section":mn,"produtos-section":Cn,"suppliers-section":Nn,"profissionais-section":$t,"clientes-section":Ft,"estabelecimento-section":Fo,"ausencias-section":Ri,"users-section":St,"sales-report-section":el,"financial-section":ll,"commissions-section":gl,"packages-section":Ia,"my-profile-section":Ml};function Hl(e){const t=co[e]||co.indigo,o=(s=>{const n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(s);return n?`${parseInt(n[1],16)}, ${parseInt(n[2],16)}, ${parseInt(n[3],16)}`:"79, 70, 229"})(t.main);document.body.style.setProperty("--theme-main",t.main);const r=document.getElementById("dynamic-theme-styles");r.innerHTML=`
        :root {
    ...
    `}function ua(){const e=Ue.filter(t=>!t.read).length;if(e>0?(Yt.textContent=e,Yt.classList.remove("hidden")):Yt.classList.add("hidden"),Ue.length===0){io.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}io.innerHTML=Ue.map(t=>`
    ...
    `).join("")}function Ol(e){Ve&&Ve();const t=Uo(Q,"establishments",e,"notifications"),a=Jo(t,Wo("timestamp",">=",new Date),_o("timestamp","desc"));Ve=Go(a,o=>{o.docChanges().forEach(r=>{if(r.type==="added"){const s=r.doc.data();Ue.unshift({title:s.title,message:s.message,time:s.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(s.title,s.message,"info",!0),ua();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(s.type==="cancellation"||s.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),xo())}})},o=>{console.error("Erro no listener de notificações em tempo real:",o),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function Y(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=p.enabledModules?.[a]!==!1,s=p.userPermissions===null||p.userPermissions[e]?.view===!0;if(!r||!s){Gt.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const o=jl[e];o?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),Gt.innerHTML="",o(t)):(Gt.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function zl(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),o=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),s=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(s&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!s&&!n))try{const i=await Os();s&&o&&(o.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),s&&o&&(o.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function Vl(e){try{Je.getPlatform()==="android"&&(await re.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await re.checkPermissions();if(t.receive==="prompt"&&(t=await re.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await re.register(),re.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const o=uo(Q,"users",e);await Vo(o,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(o){console.error("Erro ao salvar token FCM:",o)}}),re.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),re.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),re.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),Y("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function Ul(){Je.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),bs(),no.addEventListener("click",e=>{e.stopPropagation(),ft.classList.toggle("hidden"),ft.classList.contains("hidden")||(Ue.forEach(t=>t.read=!0),ua())}),Rl.addEventListener("click",()=>{fs()}),Xt.addEventListener("click",e=>{e.stopPropagation(),X.classList.toggle("active"),X.classList.contains("active")?X.classList.remove("hidden"):setTimeout(()=>X.classList.add("hidden"),200)}),lo&&lo.addEventListener("click",e=>{e.preventDefault(),Y("my-profile-section"),X.classList.remove("active"),X.classList.add("hidden")}),document.addEventListener("click",e=>{!ft.contains(e.target)&&e.target!==no&&ft.classList.add("hidden"),!X.contains(e.target)&&e.target!==Xt&&X.classList.contains("active")&&(X.classList.remove("active"),setTimeout(()=>X.classList.add("hidden"),200))}),Oo(J,async e=>{if(e)try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const o=await Qe(a.establishmentId);p.enabledModules=o.modules,Hl(o.themeColor||"indigo");let r=null,s=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const l=uo(Q,"users",e.uid),c=await zo(l);if(c.exists()){const u=c.data();r=a.role==="employee"?u.permissions||{}:null,s=u.name||s,n=u.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}p.userProfessionalId=n,Vl(e.uid);const i=s||e.email;ss(a.establishmentId,o.name,r),Xt.textContent=i.charAt(0).toUpperCase(),ql.textContent=i,Nl.textContent=e.email;const d=()=>{Ve&&Ve(),La(J).then(()=>window.location.href="/login.html")};Fl.addEventListener("click",l=>{l.preventDefault(),d()}),ys(Y,r,p.enabledModules),zl(r),Ol(a.establishmentId),ua(),bt.classList.add("fade-out"),Wt.style.display="flex",setTimeout(()=>{bt.style.display="none"},500),Y("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),bt.classList.add("fade-out"),setTimeout(()=>{bt.style.display="none"},500),Wt.innerHTML=`
                ...
                `,Wt.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{La(J).then(()=>window.location.href="/login.html")})}else window.location.href="/login.html"})}Ul();
