import"./modulepreload-polyfill-B5Qt9EMX.js";import{a as y}from"./firebase-config-seH-87Om.js";import{onAuthStateChanged as N,signOut as M}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{a as p,b as d,e as b,s as k,c as q}from"./utils-DTVAHnIk.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-storage.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-messaging.js";const C={super_admin:["dashboard","establishments","whatsapp","financial","team","settings"],support:["establishments","whatsapp"],financial:["dashboard","establishments","financial"],developer:["whatsapp","settings"]};let v=null;function R(){document.body.style.display="none",N(y,async e=>{if(e)try{let a=(await e.getIdTokenResult()).claims.role;a==="super-admin"&&(a="super_admin"),a&&C[a]?(v=a,console.log("🛡️ Acesso Seguro Concedido! Perfil:",v),z(v),document.body.style.display="flex"):(console.error("🚨 Intruso detectado: Conta sem cargo de gestão."),await M(y),window.location.href="admin-login.html")}catch(s){console.error("Erro ao verificar permissões de segurança:",s),window.location.href="admin-login.html"}else window.location.href="admin-login.html"})}function z(e){const s=C[e]||[];document.querySelectorAll("[data-module]").forEach(t=>{const o=t.getAttribute("data-module");s.includes(o)||t.remove()})}async function O(){try{await M(y),window.location.href="admin-login.html"}catch(e){console.error("Erro ao sair do sistema:",e)}}const F=e=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(e||0);async function U(e){e.innerHTML=`
    ...
    `}async function J(){try{r.plans=await p("/api/admin/plans")||[]}catch{d("Erro","Falha ao carregar os planos.","error")}}async function m(){const e=document.getElementById("tenants-table-body"),s=document.getElementById("pagination-container");e&&(e.innerHTML='<tr><td colspan="5" class="py-12 text-center"><div class="loader mx-auto mb-2 border-brand-500"></div></td></tr>');try{let a=`/api/admin/tenants?page=${r.currentPage}&limit=${r.limit}`;r.searchQuery&&(a+=`&search=${encodeURIComponent(r.searchQuery)}`);const t=await p(a);r.tenants=(t.data||[]).filter(o=>o.status!=="deleted"),r.totalPages=t.pagination.totalPages||1,T(),Q(s)}catch{e&&(e.innerHTML='<tr><td colspan="5" class="py-12 text-center text-rose-500">Erro ao carregar clientes.</td></tr>')}}async function A(e){try{const s=await p(`/api/admin/tenants/${e}/payments`);r.payments=s||[],W(e)}catch{document.getElementById("payments-wrapper").innerHTML='<p class="text-rose-500 text-xs font-bold text-center mt-4">Erro ao carregar pagamentos.</p>'}}function j(e){if(!e)return"-";try{const[s,a,t]=e.split("T")[0].split("-");return`${t}/${a}/${s}`}catch{return e}}function T(){const e=document.getElementById("tenants-table-body");if(!e)return;if(r.tenants.length===0){e.innerHTML='<tr><td colspan="5" class="py-16 text-center text-slate-400">Nenhum cliente encontrado.</td></tr>';return}const s=new Map(r.plans.map(t=>[t.id,t.name])),a=r.tenants.map(t=>{const o=t.status==="inactive"||t.status==="blocked",l=s.get(t.planId)||"Não Definido",i=t.nextDueDate?j(t.nextDueDate):"-",n=t.lastPaymentStatus==="overdue"?'<span class="text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded ml-1 border border-rose-100"><i class="bi bi-exclamation-circle text-[8px]"></i> Atrasado</span>':'<span class="text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded ml-1 border border-emerald-100">Em dia</span>';return`
            <tr class="hover:bg-slate-50 transition-colors group cursor-pointer border-b border-slate-100" data-action="view-tenant" data-id="${t.id}">
                ...
                <td class="p-4 text-center">
                    ...
                    <p class="text-[9px] font-bold text-slate-500 mt-1">Vence: ${i} ${n}</p>
                </td>
                <td class="p-4 text-center">
    ...
    `}function D(e,s){const a=document.getElementById("panel-content");document.querySelectorAll(".panel-tab").forEach(l=>{l.dataset.tab===r.activeTab?(l.classList.remove("border-transparent","text-slate-500"),l.classList.add("border-brand-600","text-brand-600")):(l.classList.add("border-transparent","text-slate-500"),l.classList.remove("border-brand-600","text-brand-600"))});const o=e.status==="inactive"||e.status==="blocked"||e.status==="deleted";r.activeTab==="cadastro"?(a.innerHTML=`
        ...
        `,setTimeout(()=>{const l=document.getElementById("edit-plan");l&&e.planId&&(l.value=e.planId)},50)):r.activeTab==="history"&&(a.innerHTML=`
    ...
    `}async function I(){const e=document.getElementById("plans-table-body");try{const s=new Date().getTime(),a=await p(`/api/saas/plans?_t=${s}`);x.plans=a||[],re()}catch(s){console.error("Erro ao buscar planos:",s),e&&(e.innerHTML='<tr><td colspan="7" class="py-12 text-center text-rose-500 font-bold text-sm"><i class="bi bi-exclamation-triangle block text-2xl mb-2"></i> Erro ao carregar os planos.</td></tr>')}}function re(){const e=document.getElementById("plans-table-body");if(!e)return;if(x.plans.length===0){e.innerHTML=`
            ...
            </tr>`;return}const s=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}),a=x.plans.map(t=>{const l=t.active!==!1?'<span class="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest"><i class="bi bi-check-circle-fill mr-1"></i> Ativo</span>':'<span class="bg-slate-100 text-slate-500 border border-slate-200 px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest"><i class="bi bi-pause-circle-fill mr-1"></i> Inativo</span>',i=t.intervalCount===12?"Anual":t.intervalCount===6?"Semestral":"Mensal",c=t.trialDays?`${t.trialDays} dias`:"Sem trial";return`
            <tr class="hover:bg-slate-50 transition-colors group cursor-pointer" data-action="edit-plan" data-id="${t.id}">
                ...
                <td class="p-4 text-center">
                    <span class="inline-block bg-emerald-50 text-emerald-600 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest shadow-sm">${c}</span>
                </td>
        ...
        `}).join("");e.innerHTML=a}function P(e=null){x.tempPlanId=e;const s=document.getElementById("plan-modal");document.getElementById("plan-form").reset();const t=!!e;document.getElementById("modal-title").innerText=t?"Editar Plano SaaS":"Novo Plano SaaS";const o=t?x.plans.find(n=>n.id===e):null;document.getElementById("plan-name").value=o?o.name:"",document.getElementById("plan-price").value=o?o.price:"",document.getElementById("plan-max-ests").value=o&&o.maxEstablishments||1,document.getElementById("plan-max-users").value=o&&o.maxUsers||1,document.getElementById("plan-max-profs").value=o&&o.maxProfessionals||1,document.getElementById("plan-interval").value=o&&o.intervalCount||1,document.getElementById("plan-trial").value=o&&o.trialDays!==void 0?o.trialDays:7;const l=document.getElementById("modules-grid"),i=o?.features||o?.allowedModules||{},c=Array.isArray(i)?i.reduce((n,g)=>({...n,[g]:!0}),{}):i;l.innerHTML=Object.entries(se).map(([n,g])=>{const u=!t||c[n]?"checked":"";return`
