const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-XpYsCF7_.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/firebase-config-C2tbVz-J.js","assets/styles-vhIfPenq.css"])))=>i.map(i=>d[i]);
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${i.status}) em ${r}:`,l),new Error(l)}return i.json()}catch(i){throw console.error(`Falha de rede ao tentar acessar ${r}:`,i.message),i.message.includes("Failed to fetch")||i.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${ra}. Verifique se o servidor backend está rodando.`):i}}const Do=(e,t,a,o=null)=>{let s=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(s+=`&professionalId=${o}`),I(s)},Ws=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const s=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return I(s)},Js=e=>I("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Gs=(e,t)=>I(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ra=e=>I(`/api/appointments/${e}`,{method:"DELETE"}),Ys=e=>I(`/api/appointments/${e}/reopen`,{method:"POST"}),Qs=(e,t)=>I(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let J;async function Xs(){if(!J)try{J=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Zs(){if(!J){console.warn("AudioContext não inicializado. O som não será tocado.");return}J.state==="suspended"&&J.resume();const e=J.createOscillator(),t=J.createGain();e.connect(t),t.connect(J.destination),e.type="sine",e.frequency.setValueAtTime(800,J.currentTime),t.gain.setValueAtTime(0,J.currentTime),t.gain.linearRampToValueAtTime(.3,J.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,J.currentTime+.2),e.start(J.currentTime),e.stop(J.currentTime+.2)}function g(e,t,a="info",o=!1){const s=document.getElementById("toast-container");if(!s)return;o&&Zs();const r=document.createElement("div"),i={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},n={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};r.className=`toast ${i[a]||i.info}`,r.innerHTML=`
    ...
    `,s.appendChild(r),r.querySelector(".toast-close").addEventListener("click",()=>r.remove()),setTimeout(()=>{r.remove()},4e3)}function z(e,t){const a=document.getElementById("genericModal");return new Promise(o=>{a.innerHTML=`
    ...
    `,_t()}function _t(){const e=document.getElementById("cashier-alert-box"),t=document.getElementById("btn-new-sale");y.isCashierOpen?(e&&(e.innerHTML=""),t&&(t.classList.remove("opacity-50","cursor-not-allowed"),t.disabled=!1)):(e&&(e.innerHTML=`
        ...
        `,r.appendChild(E)}),e.innerHTML="",e.appendChild(r),Qa(t)}function Qa(e){if(!e)return;e.innerHTML="";const{page:t,total:a,limit:o}=y.paging,s=Math.ceil((a||0)/o);if(s===0)return;const r=document.createElement("div");r.className="flex gap-2 justify-center items-center w-full",r.innerHTML=`
    ...
    `,e.appendChild(r),r.querySelectorAll("button[data-page]").forEach(i=>{i.onclick=n=>{n.stopPropagation();const l=parseInt(i.dataset.page,10);l>0&&l<=s&&(y.paging.page=l,ue())}})}function W(){const e=document.getElementById("comanda-detail-container");if(!e)return;const t=y.allComandas.find(x=>x.id===y.selectedComandaId);if(y.viewMode==="checkout"&&t){ki(t,e);return}const a=`
    ...
    `,{modalElement:t}=se({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const o=parseFloat(document.getElementById("initial-amount").value);if(isNaN(o)||o<0)return g("Valor Inválido","Insira um valor válido.","error");try{const s=await mi({establishmentId:m.establishmentId,initialAmount:parseFloat(o.toFixed(2))});y.isCashierOpen=!0,y.activeCashierSessionId=s.id,document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto (R$ ${o.toFixed(2)})`,"success"),_t(),await ue()}catch(s){g("Erro",`Falha ao abrir caixa: ${s.message}`,"error")}})}async function Di(){const e=y.activeCashierSessionId;if(e)try{const t=await bi(e),a=`
        ...
        <div class="flex flex-col items-center justify-center h-[calc(100vh-100px)] w-full">
        ...
        </div>`;try{await Gi();const[e,t,a]=await Promise.all([re(m.establishmentId),Lr(m.establishmentId).catch(()=>[]),Wt(m.establishmentId).catch(()=>[])]);P.professionalsList=e||[],P.costCentersList=t||[],P.naturesList=a||[],Qi(),await os()}catch(e){console.error("Erro no loadReportsPage:",e),ca.innerHTML=`
            ...
            </div>`}}function Qi(){const e=P.professionalsList.map(a=>`<option value="${a.id}">${b(a.name)}</option>`).join(""),t=P.costCentersList.map(a=>`<option value="${a.id}">${b(a.name)}</option>`).join("");ca.innerHTML=`
    ...
    `,o=w.clients.length>0?`
        <div class="px-3 sm:px-4 pb-20 pt-2 w-full">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ${w.clients.map(c=>{const u=c.totalDebt&&parseFloat(c.totalDebt)>0,p=Qn(c.lastVisit);return`
    ...
    `;try{const[a,o]=await Promise.all([Wt(G),Ea(G)]),s=e.financialIntegration||{},r=e.commissionConfig||{},i=e.purchaseConfig||{};t.querySelector("#financialNatureId").innerHTML=Ie(a,s.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=Ie(o,s.defaultCentroDeCustoId),t.querySelector("#purchaseNatureId").innerHTML=Ie(a,i.defaultNatureId),t.querySelector("#purchaseCostCenterId").innerHTML=Ie(o,i.defaultCostCenterId),t.querySelector("#commissionNatureId").innerHTML=Ie(a,r.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=Ie(o,r.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar o plano de contas da unidade.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const o={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},purchaseConfig:{defaultNatureId:t.querySelector("#purchaseNatureId").value||null,defaultCostCenterId:t.querySelector("#purchaseCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};ze(o,a)})}function vl(e,t){const a=`https://wa.me/5516997859430?text=Olá, preciso de ajuda com o sistema Kairos (Minha Unidade: ${e.name}).`;t.innerHTML=`
    ...
    `}function bs(e="indigo",t){const a=t.querySelector("#color-palette-container"),o=t.querySelector("#establishmentThemeColor");!a||!o||(a.innerHTML="",Object.entries(dl).forEach(([s,r])=>{const i=s===e,n=document.createElement("div");n.className="w-24 text-center cursor-pointer mb-4",n.innerHTML=`
        ...
        `,n.addEventListener("click",()=>{o.value=s,bs(s,t)}),a.appendChild(n)}),o.value=e)}function yl(e,t){const a=t.querySelector("#slotIntervalContainer"),o=t.querySelector("#establishmentSlotInterval");if(!a||!o)return;const s=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=s.map(r=>{const i=r.value===e;return`<button type="button" data-value="${r.value}" 
    ...
    `,xe.querySelector('button[data-action="back-to-menu"]').addEventListener("click",s=>{s.preventDefault(),fs({id:G})});const o=document.getElementById("settings-content-detail");switch(e){case"personal-data":cl(V,o);break;case"change-password":ul(V,o);break;case"change-email":ml(V,o);break;case"branding":pl(V,o);break;case"booking":gl(V,o);break;case"working-hours":bl(V,o);break;case"loyalty":await fl(V,o);break;case"financial":await xl(V,o);break;case"support":vl(V,o);break;case"cancellation":hl(V,o);break;default:o.innerHTML='<div class="p-4 text-center">Módulo em construção.</div>'}}async function fs(e={}){xe.innerHTML=`
    ...
    `;try{G=e.id||m.establishmentId,V=await ye(G);const t=e.id?`<button onclick="window.navigateTo('establishments-section')" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 font-semibold py-2 px-4 rounded-lg shadow-sm transition-colors flex items-center gap-2">
               ...
               </button>`:"",a=V.isMatriz||!V.parentId?'<span class="bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded ml-3">🏢 MATRIZ</span>':'<span class="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-1 rounded ml-3">📍 UNIDADE</span>',o=gs();xe.innerHTML=`
            <div class="max-w-5xl mx-auto w-full pb-20">
                ...
                <div class="bg-gradient-to-r from-indigo-900 to-indigo-700 rounded-xl shadow-lg p-6 mb-8 text-white flex justify-between items-center relative overflow-hidden">
                    <div class="relative z-10">
                        <h3 class="text-2xl font-bold mb-1">${b(V.name)} ${a}</h3>
                        <p class="text-indigo-200 text-sm flex items-center gap-2"><i class="bi bi-geo-alt"></i> ${b(V.address||"Morada não definida")}</p>
                    </div>
        ...
        `,xe.querySelectorAll("div[data-section]").forEach(s=>{s.addEventListener("click",r=>{wl(s.dataset.section)})}),xe.querySelectorAll(".module-toggle").forEach(s=>{s.addEventListener("change",async()=>{const r=s.dataset.module;try{const n={...(await ye(G)).modules,[r]:s.checked};await xa(G,{modules:n}),g("Módulos","Módulos atualizados com sucesso.","success")}catch(i){s.checked=!s.checked,g("Erro",i.message,"error")}})})}catch(t){xe.innerHTML=`
        ...
        `}}function kl(e){return[{key:"agenda-section",label:"Agenda Diária",icon:"bi-calendar"},{key:"comandas-section",label:"Comandas e PDV",icon:"bi-receipt"},{key:"financial-section",label:"Financeiro Completo",icon:"bi-cash-coin"},{key:"reports-section",label:"Relatórios Gerenciais",icon:"bi-graph-up"}].map(a=>`
    ...
    `).join("")}const st=document.getElementById("content");async function Re(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,o=document.getElementById("filterEndDate")?.value,s=await Ot(m.establishmentId,a||new Date().toISOString().split("T")[0],o||new Date().toISOString().split("T")[0],e),r=document.getElementById("filterReason")?.value.toLowerCase(),i=r?s.filter(l=>l.reason&&l.reason.toLowerCase().includes(r)):s,n=i.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",i.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(n).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let p=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
        ...
        </section>`,El(t),await Re(t);const s=document.getElementById("batchProfSelectionContainer");try{const r=await re(m.establishmentId);s.innerHTML=r.map(i=>`
            <div class="flex items-center">
    ...
    `}).join("")}function pa(){const t=document.getElementById("showInactiveUsersToggle")?.checked?m.users:m.users.filter(a=>a.status==="active");Rl(t)}function Nl(e={}){return Object.entries(Ml).map(([t,a])=>{const o=t==="agenda-section"||t==="comandas-section",s=e[t]?.view_all_prof===!0,r=Object.entries(Al).map(([n,l])=>`
    ...
    `,xt&&We.removeEventListener("click",xt),vt&&We.removeEventListener("change",vt),xt=async e=>{const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":lo();break;case"edit-user":const o=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));lo(o);break;case"back-to-list":Pt();break;case"delete-user":{if(e.stopPropagation(),await z("Excluir Usuário","Tem certeza? Ação irreversível."))try{await Dl(t.dataset.userId),g("Usuário excluído!","success"),Pt()}catch(s){g(`Erro: ${s.message}`,"error")}break}}},vt=async e=>{const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")pa();else if(t){e.stopPropagation();const a=t.dataset.userId,o=t.checked?"active":"inactive";try{await Bl(a,o);const s=m.users.findIndex(r=>r.id===a);s>-1&&(m.users[s].status=o,pa())}catch(s){g(`Erro: ${s.message}`,"error"),t.checked=!t.checked}}},We.addEventListener("click",xt),We.addEventListener("change",vt),await jl()}const Fl=document.getElementById("content");let co={},ga=null;function Hl(){Object.values(co).forEach(e=>e?.destroy()),co={}}function Ol(e,t){if(!window.jspdf){g("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:a}=window.jspdf,o=new a({orientation:"landscape",unit:"px",format:"a4"}),s=document.getElementById("salesReportSummaryCards");if(o.setFontSize(18),o.text(e,o.internal.pageSize.getWidth()/2,40,{align:"center"}),s){const i=[["Receita Total",s.querySelector("#summary-revenue").textContent],["Vendas Totais",s.querySelector("#summary-transactions").textContent],["Ticket Médio",s.querySelector("#summary-avg-ticket").textContent]];o.autoTable({startY:60,head:[["Métrica","Valor"]],body:i,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const r=o.lastAutoTable?o.lastAutoTable.finalY+20:60;o.text("Detalhes das Vendas",20,r),o.autoTable({html:`#${t}`,startY:r+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),o.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function uo(e){const t=document.getElementById("genericModal"),a=b(e.client),o=b(e.items),s=b(e.responsavelCaixa||"N/A"),r=(e.payments||[]).map(i=>`
    ...
    `}).join(""),i.querySelectorAll("div[data-transaction-index]").forEach(n=>{n.addEventListener("click",()=>{const l=n.dataset.transactionIndex,d=ga.transactions[l];d&&uo(d)})})}async function mo(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const o=t.value,s=a.value;if(!o||!s)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const r=document.getElementById("cashierSessionFilter").value,i=await Cr({establishmentId:m.establishmentId,startDate:o,endDate:s,cashierSessionId:r});ga=i,e.innerHTML=`
        ...
        `,zl(i)}catch(r){g("Erro",`Não foi possível carregar o relatório: ${r.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${b(r.message)}</p>`}}async function Vl(){Hl();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Fl.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",mo),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const o=document.getElementById("reportStartDate").value,s=document.getElementById("reportEndDate").value,r=`Relatorio_Vendas_${o}_a_${s}`;Ol(r,"transactionsTable")});try{const o=await gi(m.establishmentId),s=document.getElementById("cashierSessionFilter");o&&o.length>0&&o.forEach(r=>{const i=new Date(r.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),n=b(r.closedByName||"N/A");s.innerHTML+=`<option value="${r.id}">${n} - ${i}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await mo()}const Ul=document.getElementById("content");let $={payables:[],receivables:[],natures:[],costCenters:[],establishments:[],currentTab:"receivables",statusFilter:"all",startDate:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],endDate:new Date(new Date().getFullYear(),new Date().getMonth()+1,0).toISOString().split("T")[0],filterNaturezaId:"all",filterCostCenterId:"all",filterEstablishmentIds:new Set,searchQuery:"",isAdvancedFilterOpen:!1,selectedIds:new Set,isSelectionMode:!1},ht=null,yt=null;function Ba(e){const t=new Map,a=[];return e&&(e.forEach(o=>t.set(o.id,{...o,children:[]})),t.forEach(o=>{o.parentId&&t.has(o.parentId)?t.get(o.parentId).children.push(o):a.push(o)})),a}function xs(e){if(!e)return{day:"--",month:"---",full:"--/--/----"};const[t,a,o]=e.split("-"),s=new Date(t,a-1,o),r=String(s.getDate()).padStart(2,"0"),i=s.toLocaleString("pt-BR",{month:"short"}).toUpperCase().replace(".","");return{day:r,month:i,full:s.toLocaleDateString("pt-BR")}}function ne(e){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(e)}function Bt(e,t){if(t==="paid")return!1;const a=new Date;a.setHours(0,0,0,0);const[o,s,r]=e.split("-");return new Date(o,s-1,r)<a}function _l(e,t,a){if(!e)return;if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-400 text-sm py-4">Nenhum item criado.</p>';return}const o=(s,r=0)=>`
    ...
    `,a.style.display="flex";const r=a.querySelector("#modal-select-all"),i=a.querySelectorAll(".modal-item-checkbox"),n=a.querySelector("#confirm-batch-delete");r.addEventListener("change",d=>{i.forEach(c=>c.checked=d.target.checked),l()}),i.forEach(d=>d.addEventListener("change",l));function l(){const d=Array.from(i).filter(c=>c.checked).length;n.innerHTML=d>0?`<i class="bi bi-trash3"></i> Excluir ${d} Parcela(s)`:"Selecione para excluir",n.disabled=d===0,d===0?n.classList.add("opacity-50","cursor-not-allowed","bg-gray-400"):n.classList.remove("opacity-50","cursor-not-allowed","bg-gray-400")}n.addEventListener("click",async()=>{const d=Array.from(i).filter(u=>u.checked).map(u=>u.value);if(d.length===0)return;a.style.display="none",await z("Confirmar Ação",`Tem certeza que deseja apagar estas ${d.length} parcelas permanentemente?`)&&await ks(e,d)}),l()}async function ks(e,t){try{t.length===1?e==="payable"?await Oi(t[0]):await _i(t[0]):await Ko(e==="payable"?"payables":"receivables",t),g("Sucesso",`${t.length} registo(s) limpo(s) do sistema.`,"success"),$.selectedIds.clear(),Pe(),await ke()}catch(a){g("Erro",a.message,"error")}}async function Ql(e,t){const o=e==="nature"?Ri:ji;if(await z("Apagar Categoria","Tem certeza? Apagar um item pai também apagará as suas subcategorias."))try{await o(t),ba(e==="nature"?"nature":"cost-center")}catch(r){g("Erro",r.message,"error")}}function Xl(){const e=document.getElementById("genericModal");e.innerHTML=`
    ...
    `}function bd(e){const t=N.professionals.map(a=>`<option value="${a.id}" ${N.histProfessionalId===a.id?"selected":""}>${a.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `}const aa=document.getElementById("content");let he={allPackages:[],catalogForModal:{services:[],products:[]}},kt=null,Be=null;function fd(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function xd(){const e=document.getElementById("packagesListContainer");if(e){if(he.allPackages.length===0){e.innerHTML=`
    ...
    `,await kd()}async function kd(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=m.userProfessionalId;if(t){const a=await qr(t);yd=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo);const o=b(a.name);e.innerHTML=`
            ...
            `,Sd(a.id),document.getElementById("my-blocks-filter").addEventListener("change",r=>Nt(a.id,r.target.value)),Nt(a.id,"future")}else e.innerHTML=`
    ...
    `;const t=document.getElementById("hierarchy-list-container"),a=document.getElementById("est-parent");try{const s=(await Oe()).matrizes||[];if(a&&(a.innerHTML='<option value="">Nenhuma (Criar como Matriz Independente)</option>'),s.length===0)t.innerHTML=`
            ...
            `;else{let r="";s.forEach(i=>{if(a&&!i.isOrphanBranch){const l=document.createElement("option");l.value=i.id,l.textContent=i.name,a.appendChild(l)}const n=i.isMatriz||!i.parentId?'<span class="bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-1 rounded ml-3 tracking-wider">🏢 MATRIZ</span>':'<span class="bg-amber-100 text-amber-700 text-[10px] font-bold px-2 py-1 rounded ml-3 tracking-wider">📍 UNIDADE</span>';r+=`
                ...
                `}),t.innerHTML=r}xo||($d(),xo=!0)}catch(o){console.error("Erro na renderização da rede:",o),t.innerHTML=`
