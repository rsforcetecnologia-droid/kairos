import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{a as G,d as te}from"./firebase-config-BCzB3QOS.js";import{EmailAuthProvider as es,reauthenticateWithCredential as ts,verifyBeforeUpdateEmail as as,updatePassword as os,updateProfile as ss,onAuthStateChanged as rs,signOut as qa}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{addDoc as va,collection as Ae,query as ha,where as xa,getDocs as wo,orderBy as ko,doc as Dt,deleteDoc as ns,updateDoc as So,getDoc as is,onSnapshot as ls}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";const p={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function ds(e,t,a){p.establishmentId=e,p.establishmentName=t,p.userPermissions=a}const oa="https://kairos-app-407358446276.us-central1.run.app";console.log("🚀 API configurada para Produção (US):",oa);async function cs(){const e=G.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function y(e,t={}){const a=await cs();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const s=oa.replace(/\/$/,""),r=e.startsWith("/")?e:`/${e}`,o=`${s}${r}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${o}`);try{const n=await fetch(o,{...t,headers:{...a,...t.headers}});if(!n.ok){const l=(await n.json().catch(()=>({message:n.statusText}))).message||`Erro na API: ${n.status}`;if(l.includes("FAILED_PRECONDITION")&&l.includes("requires an index")){const d=/(https:\/\/[^\s]+)/,c=l.match(d),u=c?c[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${o}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${o}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${oa}. Verifique sua conexão com a internet.`):n}}const us=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),y(r)},ms=(e,t,a)=>{const s=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return y(s)},ps=({establishmentId:e,professionalId:t,serviceIds:a,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${s}`;return y(r)},gs=e=>y("/api/appointments",{method:"POST",body:JSON.stringify(e)}),ya=(e,t)=>y(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),bs=e=>y(`/api/appointments/${e}`,{method:"DELETE"}),fs=e=>y(`/api/appointments/${e}/reopen`,{method:"POST"}),vs=(e,t)=>y(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let V;async function hs(){if(!V)try{V=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function xs(){if(!V){console.warn("AudioContext não inicializado. O som não será tocado.");return}V.state==="suspended"&&V.resume();const e=V.createOscillator(),t=V.createGain();e.connect(t),t.connect(V.destination),e.type="sine",e.frequency.setValueAtTime(800,V.currentTime),t.gain.setValueAtTime(0,V.currentTime),t.gain.linearRampToValueAtTime(.3,V.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,V.currentTime+.2),e.start(V.currentTime),e.stop(V.currentTime+.2)}function g(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&xs();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;const l=r.querySelector("[data-close-modal]");l&&(l.onclick=n);const d=r.querySelector('[data-action="close-modal"]');return d&&(d.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Pt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function ys(){document.body.addEventListener("click",()=>{V||hs()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const s=t.dataset.target;if(s){const r=document.getElementById(s);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const s=document.getElementById("genericModal");s&&(s.style.display="none")}})}async function Fa(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const s=await ms(p.establishmentId,t,a);if(s.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=s.map(r=>`
        ...
        `).join("")}catch(s){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${s.message}</p>`}}function ws(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const s=`
    ...
    `,{modalElement:r}=z({title:"Histórico de Cancelamentos",contentHTML:s,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Fa),Fa()}const _=document.getElementById("sidebar"),Ra=document.getElementById("sidebarToggle"),Ve=document.getElementById("mainContent"),ks=document.querySelectorAll(".sidebar-link"),ja=document.getElementById("hamburger-menu-btn"),Be=document.getElementById("mobile-overlay");function xt(e){!_||!Ve||(_.classList.toggle("collapsed",e),Ve.classList.toggle("sidebar-collapsed-shift",e))}function Ss(){!_||!Be||(_.classList.add("mobile-open"),Be.classList.add("visible"))}function nt(){!_||!Be||(_.classList.remove("mobile-open"),Be.classList.remove("visible"))}function $s(){xt(!_.classList.contains("collapsed"))}function Es(e,t,a){if(!_||!Ve)return;Ve.classList.add("main-content-shift"),window.innerWidth>=768?xt(_.classList.contains("collapsed")):(Ve.classList.remove("main-content-shift","sidebar-collapsed-shift"),nt()),Ra&&Ra.addEventListener("click",r=>{r.stopPropagation(),$s()}),_.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&_.classList.contains("collapsed")&&xt(!1)}),_.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||xt(!0))}),ja&&ja.addEventListener("click",r=>{r.stopPropagation(),Ss()}),Be&&Be.addEventListener("click",r=>{r.stopPropagation(),nt()});let s=0;_.addEventListener("touchstart",r=>{s=r.changedTouches[0].screenX},{passive:!0}),_.addEventListener("touchend",r=>{const o=r.changedTouches[0].screenX;s-o>50&&nt()},{passive:!0}),ks.forEach(r=>{const o=r.getAttribute("data-target"),n=o.replace("-section",""),i=a?.[n]!==!1,l=t===null||t[o]?.view===!0;if(!i||!l){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",d=>{d.preventDefault(),o&&typeof e=="function"&&e(o),window.innerWidth<768&&nt()})})}const at=e=>{const t=e||p.establishmentId;return t?y(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Is=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Cs=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Ls=(e,t)=>{const a=e||p.establishmentId;return a?y(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Q=e=>y(`/api/professionals/${e}`),Ts=e=>y(`/api/professionals/details/${e}`),Bs=e=>y("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Ds=(e,t)=>y(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ps=e=>y(`/api/professionals/${e}`,{method:"DELETE"});var De;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(De||(De={}));class Jt extends Error{constructor(t,a,s){super(t),this.message=t,this.code=a,this.data=s}}const Ms=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},As=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},s=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:Ms(e),o=()=>r()!=="web",n=u=>{const m=d.get(u);return!!(m?.platforms.has(r())||i(u))},i=u=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===u)},l=u=>e.console.error(u),d=new Map,c=(u,m={})=>{const b=d.get(u);if(b)return console.warn(`Capacitor plugin "${u}" already registered. Cannot register plugins twice.`),b.proxy;const v=r(),h=i(u);let w;const $=async()=>(!w&&v in m?w=typeof m[v]=="function"?w=await m[v]():w=m[v]:t!==null&&!w&&"web"in m&&(w=typeof m.web=="function"?w=await m.web():w=m.web),w),I=(L,T)=>{var N,q;if(h){const H=h?.methods.find(A=>T===A.name);if(H)return H.rtype==="promise"?A=>a.nativePromise(u,T.toString(),A):(A,J)=>a.nativeCallback(u,T.toString(),A,J);if(L)return(N=L[T])===null||N===void 0?void 0:N.bind(L)}else{if(L)return(q=L[T])===null||q===void 0?void 0:q.bind(L);throw new Jt(`"${u}" plugin is not implemented on ${v}`,De.Unimplemented)}},E=L=>{let T;const N=(...q)=>{const H=$().then(A=>{const J=I(A,L);if(J){const we=J(...q);return T=we?.remove,we}else throw new Jt(`"${u}.${L}()" is not implemented on ${v}`,De.Unimplemented)});return L==="addListener"&&(H.remove=async()=>T()),H};return N.toString=()=>`${L.toString()}() { [capacitor code] }`,Object.defineProperty(N,"name",{value:L,writable:!1,configurable:!1}),N},k=E("addListener"),C=E("removeListener"),F=(L,T)=>{const N=k({eventName:L},T),q=async()=>{const A=await N;C({eventName:L,callbackId:A},T)},H=new Promise(A=>N.then(()=>A({remove:q})));return H.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await q()},H},j=new Proxy({},{get(L,T){switch(T){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return h?F:k;case"removeListener":return C;default:return E(T)}}});return s[u]=j,d.set(u,{name:u,proxy:j,platforms:new Set([...Object.keys(m),...h?[v]:[]])}),j};return a.convertFileSrc||(a.convertFileSrc=u=>u),a.getPlatform=r,a.handleError=l,a.isNativePlatform=o,a.isPluginAvailable=n,a.registerPlugin=c,a.Exception=Jt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Ns=e=>e.Capacitor=As(e),Xe=Ns(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),wa=Xe.registerPlugin;class $o{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let s=!1;this.listeners[t]||(this.listeners[t]=[],s=!0),this.listeners[t].push(a);const o=this.windowListeners[t];o&&!o.registered&&this.addWindowListener(o),s&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,s){const r=this.listeners[t];if(!r){if(s){let o=this.retainedEventArguments[t];o||(o=[]),o.push(a),this.retainedEventArguments[t]=o}return}r.forEach(o=>o(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:s=>{this.notifyListeners(a,s)}}}unimplemented(t="not implemented"){return new Xe.Exception(t,De.Unimplemented)}unavailable(t="not available"){return new Xe.Exception(t,De.Unavailable)}async removeListener(t,a){const s=this.listeners[t];if(!s)return;const r=s.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(s=>{this.notifyListeners(t,s)}))}}const Ha=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Oa=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class qs extends $o{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(s=>{if(s.length<=0)return;let[r,o]=s.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Oa(r).trim(),o=Oa(o).trim(),a[r]=o}),a}async setCookie(t){try{const a=Ha(t.key),s=Ha(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,o=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${s||""}${r}; path=${o}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}wa("CapacitorCookies",{web:()=>new qs});const Fs=async e=>new Promise((t,a)=>{const s=new FileReader;s.onload=()=>{const r=s.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},s.onerror=r=>a(r),s.readAsDataURL(e)}),Rs=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,o,n)=>(r[o]=e[t[n]],r),{})},js=(e,t=!0)=>e?Object.entries(e).reduce((s,r)=>{const[o,n]=r;let i,l;return Array.isArray(n)?(l="",n.forEach(d=>{i=t?encodeURIComponent(d):d,l+=`${o}=${i}&`}),l.slice(0,-1)):(i=t?encodeURIComponent(n):n,l=`${o}=${i}`),`${s}&${l}`},"").substr(1):null,Hs=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Rs(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const o=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))o.set(n,i);a.body=o.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const o=new FormData;if(e.data instanceof FormData)e.data.forEach((i,l)=>{o.append(l,i)});else for(const i of Object.keys(e.data))o.append(i,e.data[i]);a.body=o;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Os extends $o{async request(t){const a=Hs(t,t.webFetchExtra),s=js(t.params,t.shouldEncodeUrlParams),r=s?`${t.url}?${s}`:t.url,o=await fetch(r,a),n=o.headers.get("content-type")||"";let{responseType:i="text"}=o.ok?t:{};n.includes("application/json")&&(i="json");let l,d;switch(i){case"arraybuffer":case"blob":d=await o.blob(),l=await Fs(d);break;case"json":l=await o.json();break;case"document":case"text":default:l=await o.text()}const c={};return o.headers.forEach((u,m)=>{c[m]=u}),{data:l,headers:c,status:o.status,url:o.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}wa("CapacitorHttp",{web:()=>new Os});const ie=wa("PushNotifications",{}),zs=(e,t,a="all",s="all")=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("professionalId",a),s&&s!=="all"&&r.append("costCenterId",s),y(`/api/reports/indicators?${r.toString()}`)},Vs=(e,t="all")=>{const a=new URLSearchParams({date:e});return t&&t!=="all"&&a.append("professionalId",t),y(`/api/reports/appointments/list?${a.toString()}`)},Us=e=>Promise.resolve([]),Js=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:s})=>{const r=new URLSearchParams({startDate:t,endDate:a});return s&&s!=="all"&&r.append("cashierSessionId",s),e&&r.append("establishmentId",e),y(`/api/reports/sales?${r.toString()}`)},_s=()=>y("/api/reports/summary",{method:"GET"}),Ne=e=>y(`/api/services/${e}`),Ws=e=>y("/api/services",{method:"POST",body:JSON.stringify(e)}),Gs=(e,t)=>y(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ys=e=>y(`/api/services/${e}`,{method:"DELETE"}),Qs=(e,t)=>y(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),Xs=e=>y(`/api/services/stats/most-popular/${e}`),Mt=(e,t,a,s="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${s}`;return y(r)},At=e=>y("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ka=e=>y(`/api/blockages/${e}`,{method:"DELETE"}),Eo=e=>y("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),Nt=e=>y(`/api/clients/${e}`),Sa=e=>y("/api/clients",{method:"POST",body:JSON.stringify(e)}),Zs=(e,t)=>y(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ks=e=>y(`/api/clients/${e}`,{method:"DELETE"}),za=(e,t,a)=>{const s=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return y(s)},er=(e,t,a)=>{const s=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return y(s)},tr=(e,t,a,s)=>y("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:s})});function f(e){return e==null?"":String(e).replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}const Va=document.getElementById("content");let Ua=!1;const sa=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let ot=[],qt=[],Ze={},re=[],D={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},S={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function ar(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function Io(e){const t=new Date(e);if(t.setHours(0,0,0,0),D.currentView==="week"&&D.weekViewDays===7){const a=t.getDay(),s=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(s))}return t}function $t(){const e=document.getElementById("profSelectorContainer"),t=D.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(o=>D.showInactiveProfs||o.status!=="inactive");t&&(a=a.filter(o=>o.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(o=>{const n=D.selectedProfessionalId===o.id,i=o.name==="Todos"?"Todos":o.name.split(" ")[0],l=o.name==="Todos"?"T":o.name.charAt(0).toUpperCase(),d=o.status!=="inactive",c=f(i),u=sa[0],m=o.id!=="all"&&p.professionalColors.get(o.id)||u,b=o.photo||`https://placehold.co/64x64/${m.main?.replace("#","")||"E0E7FF"}/${m.light?.replace("#","")||"4F46E5"}?text=${l}`,v=o.id==="all"?"#e0e7ff":m.light,h=o.id==="all"?"#4f46e5":m.main,$=`border: 3px solid ${n?m.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+m.border:"none"};`;return`
            <div class="prof-card ${n?"selected":""} ${d?"":"opacity-50"}" 
                 data-action="select-professional" 
                 data-prof-id="${o.id}">
                ${o.id==="all"?`<div class="prof-card-all-placeholder" style="background-color: ${v}; color: ${h}; ${$}">
                           ${l}
                          </div>`:`<img src="${b}" alt="${c}" class="prof-card-photo" style="${$}" />`}
                <span class="prof-card-name">${c}</span>
            </div>
        `}).join("")}function or(e,t,a,s,r){const o=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${s} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,d=encodeURIComponent(l);return`https://wa.me/${o}?text=${d}`}function sr(e){const t=document.getElementById("agenda-view");if(e.sort((s,r)=>new Date(s.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(s=>{const r=new Date(s.startTime),o=new Date(s.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=o.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=p.professionalColors.get(s.professionalId)||{},d=f(s.reason),c=f(s.professionalName),u=f(s.clientName),m=f(s.serviceName);if(s.type==="blockage")return`
                ...
                </div>`;const b=s.status==="completed",v=b?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",h=b?"Finalizado":"Aberto",w=JSON.stringify(s).replace(/'/g,"&apos;"),$=s.redeemedReward?.points>0,I=s.hasRewards&&!$,E=or(s.clientPhone,s.clientName,s.serviceName,s.professionalName,s.startTime);return`
            <div class="appointment-list-card" data-appointment='${w}' style="border-left-color: ${l.border};">
                ...
                <div class="details-info min-w-0" data-action="open-comanda">
                    ...
                    ${$?'<p class="text-xs font-semibold text-purple-600">Resgate de Prémio</p>':""}
                </div>

                <div class="status-info">
                    <span class="status-badge ${v} mb-1">${h}</span>
                    <div class="card-actions flex gap-1 items-center">
            ...
            </div>`}).join("");t.innerHTML=`<div class="list-view-container">${a}</div>`}function $a(){return window.innerWidth<768&&D.currentView==="week"?3:D.weekViewDays}function rr(e){const t=document.getElementById("agenda-view"),a=["Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado"],s=Io(D.currentDate),r=$a();let o=`<div class="grid divide-x divide-gray-200 min-h-[60vh]" style="grid-template-columns: repeat(${r}, minmax(0, 1fr));">`;for(let n=0;n<r;n++){const i=new Date(s);i.setDate(i.getDate()+n);const l=new Date,d=i.toDateString()===l.toDateString(),c=e.filter(m=>new Date(m.startTime).toDateString()===i.toDateString()).sort((m,b)=>new Date(m.startTime)-new Date(b.startTime));let u='<div class="p-1 space-y-2">';c.length>0?u+=c.map(m=>{const v=new Date(m.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),h=p.professionalColors.get(m.professionalId)||{bg:"#e5e7eb",border:"#9ca3af"},w=f(m.reason),$=f(m.professionalName),I=f(m.clientName),E=f(m.serviceName);if(m.type==="blockage")return`
                        <div class="p-2 rounded-lg border-l-4 flex flex-col bg-red-100" style="border-left-color: ${h.border};">
                            ...
                            <div class="mt-1 min-w-0">
                                ...
                                <p class="text-xs text-red-600 truncate">com ${$}</p>
                            </div>
                        </div>
                    ...
                    <div class="p-2 rounded-lg border-l-4 flex flex-col cursor-pointer" 
                        ...
                        <div class="mt-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 truncate">com ${$||"Indefinido"}</p>
        ...
        `}o+="</div>",t.innerHTML=o}function nr(){const e=p.allEvents.filter(t=>D.selectedProfessionalId==="all"||t.professionalId===D.selectedProfessionalId);D.currentView==="list"?sr(e):rr(e)}async function ae(){const e=document.getElementById("agenda-view");if(!e)return;e.innerHTML='<div class="loader mx-auto my-10"></div>';let t,a;const s=document.getElementById("weekRange");if(D.currentView==="list")t=new Date(D.currentDate),t.setHours(0,0,0,0),a=new Date(D.currentDate),a.setHours(23,59,59,999),s.textContent=ar(t);else{const r=$a();t=Io(new Date(D.currentDate)),a=new Date(t),a.setDate(t.getDate()+(r-1)),a.setHours(23,59,59,999),s.textContent=`${t.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} - ${a.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}`}try{const r=await us(p.establishmentId,t.toISOString(),a.toISOString(),D.selectedProfessionalId==="all"?null:D.selectedProfessionalId),n=(await Mt(p.establishmentId,t.toISOString(),a.toISOString(),D.selectedProfessionalId)).map(l=>{let d=l.professionalName;if(!d&&l.professionalId){const c=p.professionals?p.professionals.find(u=>u.id===l.professionalId):null;c&&(d=c.name)}return{...l,type:"blockage",professionalName:d||"Não identificado"}}),i=[...r.map(l=>({...l,type:"appointment"})),...n];if(p.allEvents=i,$t(),nr(),D.scrollToAppointmentId){const l=document.querySelector(`[data-appointment*='"id":"${D.scrollToAppointmentId}"']`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.style.transition="background-color 0.5s ease-in-out",l.style.backgroundColor="#e0e7ff",setTimeout(()=>{l.style.backgroundColor=""},2500)),D.scrollToAppointmentId=null}}catch(r){g("Erro na Agenda",`Não foi possível carregar a agenda: ${r.message}`,"error"),e.innerHTML='<div class="p-6 text-center text-red-600">Falha ao carregar dados.</div>'}}async function ir(){try{const[e,t,a,s]=await Promise.all([p.professionals&&p.professionals.length>0?Promise.resolve(p.professionals):Q(p.establishmentId),p.services&&p.services.length>0?Promise.resolve(p.services):Ne(p.establishmentId),re.length>0?Promise.resolve(re):Nt(p.establishmentId),Ze.enabled!==void 0?Promise.resolve(null):at(p.establishmentId)]);(!p.professionals||p.professionals.length===0)&&(p.professionals=e||[]),(!p.services||p.services.length===0)&&(p.services=t||[]),re.length===0&&(re=a||[]),s&&(Ze=s.loyaltyProgram||{enabled:!1}),p.professionals.forEach((r,o)=>{p.professionalColors.set(r.id,sa[o%sa.length])}),$t()}catch(e){console.error("Erro ao popular filtros e dependências do modal:",e),g("Atenção","Não foi possível pré-carregar os dados para agendamento. A abertura do modal pode ser lenta.","error")}}function ra(e){e<1||e>4||(S.step=e,na(null,!0))}function Co(e,t){const a=document.getElementById("multiServiceToggle"),s=a&&a.checked,r=t.classList.contains("selected"),o=S.data.selectedServiceIds.indexOf(e);if(r)t.classList.remove("selected","border-blue-500"),o>-1&&S.data.selectedServiceIds.splice(o,1);else{if(!s){S.data.selectedServiceIds=[];const n=document.getElementById("apptServicesContainer");n&&n.querySelectorAll(".service-card.selected").forEach(i=>{i.classList.remove("selected","border-blue-500")})}t.classList.add("selected","border-blue-500"),S.data.selectedServiceIds.push(e)}}function Lo(e,t){const a=document.querySelector(".professional-step-cards");if(!a)return;a.querySelectorAll(".professional-modal-card").forEach(r=>r.classList.remove("selected","border-blue-500")),t.classList.add("selected","border-blue-500");const s=qt.find(r=>r.id===e);S.data.professionalId=e,S.data.professionalName=s?s.name:"N/A"}function lr(e,t){const a=document.getElementById("availableTimesContainer");a&&(a.querySelectorAll(".time-slot-card").forEach(s=>s.classList.remove("selected")),t.classList.add("selected"),S.data.time=e)}async function Ja(){const e=document.getElementById("apptTotalDuration"),t=document.getElementById("availableTimesContainer");if(!e||!t)return;const a=S.data.professionalId,s=S.data.selectedServiceIds,r=document.getElementById("apptDate").value;S.data.date=r;const o=s.reduce((n,i)=>{const l=ot.find(d=>d.id===i);return n+(l?l.duration+(l.bufferTime||0):0)},0);if(e.textContent=`${o} min`,o===0||!a||!r){t.innerHTML='<p class="col-span-full text-center text-gray-500">Selecione serviço, profissional e data.</p>';return}t.innerHTML='<div class="loader mx-auto col-span-full"></div>';try{let n=await ps({establishmentId:p.establishmentId,professionalId:a,serviceIds:s,date:r});const i=new Date;if(new Date(r+"T00:00:00").toDateString()===i.toDateString()){const d=i.getHours()*60+i.getMinutes();n=n.filter(c=>{const[u,m]=c.split(":").map(Number);return u*60+m>=d})}if(t.innerHTML="",n.length>0){if(n.forEach(d=>{const c=document.createElement("button");c.type="button",c.className=`time-slot-card p-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200 transition ${S.data.time===d?"selected":""}`,c.textContent=d,c.addEventListener("click",()=>lr(d,c)),t.appendChild(c)}),S.data.time){const d=t.querySelector(`[data-action="time-slot"][data-time="${S.data.time}"]`);d&&d.classList.add("selected")}}else t.innerHTML='<p class="col-span-full text-center text-gray-500">Nenhum horário disponível.</p>'}catch(n){console.error("Erro ao buscar horários:",n),t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao buscar horários.</p>'}}function dr(){const e=document.getElementById("loyaltyRewardsContainer");if(!e)return;const{clientHasRewards:t,clientLoyaltyPoints:a,redeemedReward:s}=S.data,{enabled:r,rewards:o,pointsPerCurrency:n}=Ze;if(!r||!t||!o||o.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden");const i=o.filter(d=>a>=d.points);let l=`
        <h4 class="text-md font-semibold text-gray-700 mb-2">🎁 Prêmios Disponíveis (${a} pontos)</h4>
    `;i.length>0?(l+='<div class="space-y-2">',l+=i.map(d=>{const c=s?.reward===d.reward,u=f(d.reward);return`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",d=>{d.target.checked&&(S.data.redeemedReward=null)})}async function cr(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!S.data.time||S.data.selectedServiceIds.length===0||!S.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const s=S.data.selectedServiceIds.map(d=>{const c=ot.find(u=>u.id===d);return{id:c.id,name:c.name,price:c.price,duration:c.duration,bufferTime:c.bufferTime||0,photo:c.photo||null}}),[r,o]=S.data.time.split(":"),n=new Date(`${S.data.date}T${r}:${o}:00`),i={establishmentId:p.establishmentId,clientName:S.data.clientName,clientPhone:S.data.clientPhone,services:s,professionalId:S.data.professionalId,startTime:n.toISOString(),redeemedReward:S.data.redeemedReward},l=t.querySelector("#appointmentId").value;l&&(i.id=l);try{l?await ya(l,i):await gs(i),g(`Agendamento ${l?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",ae()}catch(d){g(d.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function ur(e){const t=S.data.clientName===e.name&&S.data.clientPhone===e.phone,a=f(e.name),s=f(e.phone);return`
    ...
    `}async function _a(e){const t=document.getElementById("clientSearchResults");if(!t)return;const a=e.toLowerCase().trim();if(a.length<3){t.innerHTML='<p class="text-sm text-gray-500">Digite pelo menos 3 caracteres para buscar clientes existentes.</p>';return}const s=re.filter(r=>r.name.toLowerCase().includes(a)||r.phone.includes(a));if(s.length===0){t.innerHTML='<p class="text-sm text-gray-500">Nenhum cliente encontrado com este termo.</p>';return}t.innerHTML=s.map(ur).join(""),t.querySelectorAll('[data-action="select-client"]').forEach(r=>{r.addEventListener("click",o=>{const n=r.dataset.clientName,i=r.dataset.clientPhone,l=re.find(d=>d.phone===i&&d.name===n);if(S.data.clientName=n,S.data.clientPhone=i,l){const d=Ze,c=Math.min(...(d?.rewards||[]).map(u=>u.points));S.data.clientLoyaltyPoints=l.loyaltyPoints||0,S.data.clientHasRewards=d.enabled&&c!==1/0&&S.data.clientLoyaltyPoints>=c}else S.data.clientHasRewards=!1,S.data.clientLoyaltyPoints=0;document.getElementById("apptClientName").value=n,document.getElementById("apptClientPhone").value=i,document.querySelectorAll(".client-search-card").forEach(d=>d.classList.remove("selected","border-blue-500")),r.classList.add("selected","border-blue-500")})})}async function mr(e){e.preventDefault();const t=document.getElementById("clientRegistrationForm"),a=t.querySelector('button[type="submit"]'),s={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim(),phone:t.querySelector("#regClientPhone").value.trim(),dobDay:t.querySelector("#regClientDobDay").value.trim(),dobMonth:t.querySelector("#regClientDobMonth").value.trim(),notes:t.querySelector("#regClientNotes").value.trim()};if(!s.name||!s.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{await Sa(s),re.push({name:s.name,phone:s.phone,loyaltyPoints:0}),S.data.clientName=s.name,S.data.clientPhone=s.phone,S.data.clientHasRewards=!1,S.data.clientLoyaltyPoints=0,g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",ra(1)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar"}}function pr(){z({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",mr)}function gr(){pr()}function br(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",s=`
        <div class="p-5 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="apptClientName" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="Nome Completo" value="${f(S.data.clientName)}">
                </div>
                <div>
                    <label for="apptClientPhone" class="block text-sm font-medium text-gray-700">Telemóvel</label>
                    <input type="tel" id="apptClientPhone" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="(XX) XXXXX-XXXX" value="${f(S.data.clientPhone)}">
                </div>
            </div>
    ...
    `;return{title:a,content:s}}function fr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${ot.map(s=>{const r=S.data.selectedServiceIds.includes(s.id),o=s.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S",n=f(s.name);return`
    ...
    `;return{title:e,content:a}}function vr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${qt.map(a=>{const s=S.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",o=f(a.name);return`
    ...
    `;return{title:e,content:t}}function hr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,s=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=S.data.date||s,o=f(S.data.clientName),n=f(S.data.professionalName),i=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:i}}function xr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),s=ot.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=S.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            <div class="service-card p-3 bg-white rounded-lg border-2 border-gray-200 cursor-pointer transition-all hover:bg-gray-50 ${o?"selected border-blue-500":""}" data-service-id="${r.id}">
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>Co(r.dataset.serviceId,r))})}function yr(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),s=qt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=S.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",i=f(r.name);return`
             ...
             </div>`}).join(""),t.querySelectorAll(".professional-modal-card").forEach(r=>{r.addEventListener("click",()=>Lo(r.dataset.professionalId,r))})}async function na(e=null,t=!1){const a=document.getElementById("appointmentModal");if(!t){const o=e?.startTime?new Date(e.startTime).toISOString().split("T")[0]:new Date().toISOString().split("T")[0],n=e?.startTime?new Date(e.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",hour12:!1}):null;S={step:1,data:{id:e?.id||null,clientName:e?.clientName||"",clientPhone:e?.clientPhone||"",selectedServiceIds:e?.services?.map(i=>i.id)||[],professionalId:e?.professionalId||null,professionalName:e?.professionalName||"",date:o,time:n,redeemedReward:e?.redeemedReward||null,clientHasRewards:e?.hasRewards||!1,clientLoyaltyPoints:0}}}if(!p.services||!p.professionals||!re||Ze.enabled===void 0){g("Erro","Os dados da agenda ainda não foram carregados. Tente novamente em alguns segundos.","error");return}if(ot=p.services,qt=p.professionals.filter(o=>o.status==="active"),S.data.clientName&&S.data.clientPhone){const o=re.find(n=>n.phone===S.data.clientPhone&&n.name===S.data.clientName);o&&(S.data.clientLoyaltyPoints=o.loyaltyPoints||0)}let s={title:"Erro",content:"<p>Etapa não encontrada.</p>"};switch(S.step){case 1:s=br(e);break;case 2:s=fr();break;case 3:s=vr();break;case 4:s=hr(e);break}a.innerHTML=`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(o=>{o.addEventListener("click",()=>{const n=parseInt(o.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),l=a.querySelector("#apptClientPhone");if(S.data.clientName=i.value.trim(),S.data.clientPhone=l.value.trim(),!S.data.clientName||!S.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if(S.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!S.data.professionalId)return g("Atenção","Selecione um profissional.","error");ra(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(o=>{o.addEventListener("click",()=>ra(parseInt(o.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if(S.step===4&&r&&r.addEventListener("submit",cr),a.style.display="flex",S.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>Co(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>xr(i.target.value))}if(S.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>Lo(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>yr(i.target.value))}if(S.step===1){const o=a.querySelector("#clientSearchInput");o&&(o.addEventListener("input",i=>_a(i.target.value)),S.data.clientName&&S.data.clientPhone&&_a(`${S.data.clientName} ${S.data.clientPhone}`));const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",gr)}if(S.step===4){const o=a.querySelector("#apptDate");o&&o.addEventListener("change",Ja),Ja(),dr()}}async function To(e={}){D.currentDate=e.targetDate?new Date(e.targetDate):D.currentDate||new Date,D.scrollToAppointmentId=e.scrollToAppointmentId||null,D.profSearchTerm="",window.innerWidth<768&&(D.currentView="list"),Va.innerHTML=`
        <section>
        ...
        </section>`,document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),D.currentView=a.dataset.view;const s=document.getElementById("week-days-toggle");if(D.currentView==="week"){if(s.style.display="flex",window.innerWidth<768){D.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="3"]');r&&r.classList.add("active")}}else s.style.display="none";ae()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active")),a.classList.add("active"),D.weekViewDays=parseInt(a.dataset.days,10),ae()})}),document.getElementById("todayBtn").addEventListener("click",()=>{D.currentDate=new Date,ae()});const t=a=>{const s=parseInt(a.currentTarget.dataset.amount,10),r=D.currentView==="week"?$a():1,o=new Date(D.currentDate);o.setDate(o.getDate()+s*r),D.currentDate=o,ae()};document.getElementById("prevBtn").addEventListener("click",t),document.getElementById("nextBtn").addEventListener("click",t),document.getElementById("profSearchInput").addEventListener("input",a=>{D.profSearchTerm=a.target.value,$t()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{D.showInactiveProfs=a.target.checked,$t(),ae()}),Ua||(Va.addEventListener("click",async a=>{const s=a.target.closest("[data-action]");if(a.target.closest('[data-action="select-professional"]')){const l=a.target.closest('[data-action="select-professional"]').dataset.profId,d=D.selectedProfessionalId===l&&l!=="all";if(D.selectedProfessionalId=d?"all":l,l!=="all"){const c=document.getElementById("profSearchInput");c&&(c.value=""),D.profSearchTerm=""}await ae();return}if(!s)return;const r=s.dataset.action;let o=null;const n=a.target.closest("[data-appointment]");switch(n&&(o=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":na();break;case"edit-appointment":if(!o)return;if(o.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}o.hasRewards&&!o.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),na(o);break;case"delete-appointment":{const i=s.dataset.id;if(await O("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await bs(i),g("Agendamento apagado!","success"),ae()}catch(d){g(`Não foi possível apagar: ${d.message}`,"error")}break}case"open-comanda":if(o){o.hasRewards&&!o.redeemedReward&&o.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=o.status==="completed"?"finalizadas":"em-atendimento",l={selectedAppointmentId:o.id,initialFilter:i};if(i==="finalizadas"){let d=o.startTime;if(o.transaction&&o.transaction.paidAt){const c=o.transaction.paidAt;typeof c=="object"&&c._seconds?d=new Date(c._seconds*1e3):d=c}l.filterDate=d}Z("comandas-section",l)}break}}),Ua=!0),await ir(),await ae()}const wr=(e,t=null,a=1,s=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${s}`;return t&&(r+=`&date=${t}`),y(r)},kr=e=>y("/api/sales",{method:"POST",body:JSON.stringify(e)}),Sr=e=>y(`/api/sales/${e}/reopen`,{method:"POST"}),$r=e=>y(`/api/sales/${e}`,{method:"DELETE"}),Ft=e=>y(`/api/products/${e}`),Er=e=>y("/api/products",{method:"POST",body:JSON.stringify(e)}),Ir=(e,t)=>y(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),Cr=e=>y(`/api/products/${e}`,{method:"DELETE"}),Lr=(e,t)=>y(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),Tr=({startDate:e,endDate:t,productId:a,categoryId:s,establishmentId:r})=>{const o=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&o.append("productId",a),s&&s!=="all"&&o.append("categoryId",s),r&&o.append("establishmentId",r),y(`/api/products/stock-history/report?${o.toString()}`)},Bo=()=>y("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),Br=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),y("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},Dr=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),y(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},Pr=()=>y("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),Mr=e=>y(`/api/cashier/report/${e}`),Do=e=>y(`/api/packages/${e}`),Ar=e=>y("/api/packages",{method:"POST",body:JSON.stringify(e)}),Nr=(e,t)=>y(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),qr=e=>y(`/api/packages/${e}`,{method:"DELETE"});let x={allComandas:[],catalog:{services:[],products:[],packages:[]},clients:[],activeFilter:"atendimento",selectedComandaId:null,isCashierOpen:!1,activeCashierSessionId:null,paging:{page:1,limit:12,total:0}},ke=null,Se=null;function Rt(e){if(e.status==="completed"){const r=e.comandaItems||e.items||[];return r.length>0?r:e.services||[]}const t=e.services||[],a=[...e.comandaItems||[],...e.items||[]];let s=[...t];return a.forEach(r=>{if(r.type==="product"){s.push(r);return}s.some(n=>{const i=n.id&&r.id&&n.id===r.id,l=n.name&&r.name&&n.name===r.name;return i||l})||s.push(r)}),s}function Fr(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function fe(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function Rr(){const e=new Date().toISOString().split("T")[0];Se.innerHTML=`
    ...
    `}function jr(){const e=document.getElementById("cashier-controls");e&&(x.isCashierOpen?e.innerHTML=`
        ...
        `;return}const a={atendimento:"confirmed",finalizadas:"completed"}[x.activeFilter],s=x.allComandas.filter(r=>r.status===a);if(s.length===0){e.innerHTML='<p class="text-center text-gray-400 py-10 text-sm">Nenhuma venda encontrada.</p>',Wa(e);return}e.innerHTML=s.map(r=>{const n=Rt(r).reduce((b,v)=>b+(v.price||0),0),i=r.id===x.selectedComandaId,l=new Date(r.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=r.type==="walk-in"||r.id.startsWith("temp-"),c=f(r.clientName),u=f(r.professionalName),m=d?'<span class="text-[10px] font-bold uppercase text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md">Avulso</span>':'<span class="text-[10px] font-bold uppercase text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-md">Agendado</span>';return`
    ...
    `;if(!x.isCashierOpen){e.innerHTML=`
        ...
        `;return}const s=Rt(a),r=a.status==="completed",o=a.type==="walk-in"||a.id.startsWith("temp-"),n=o?"":`<button data-action="go-to-appointment" data-id="${a.id}" data-date="${a.startTime}" 
           ...
           </button>`,i=s.reduce((u,m)=>{const b=`${m.type}-${m.id||m.name}`;return u[b]||(u[b]={...m,quantity:0}),u[b].quantity+=1,u},{}),l=Object.values(i).reduce((u,m)=>u+(m.price||0)*m.quantity,0),d=f(a.clientName),c=f(a.professionalName);e.innerHTML=`
                ...
                ${Object.values(i).map(u=>`
                    <div class="flex items-center justify-between bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-800 line-clamp-1">${f(u.name)}</p>
    ...
    `}function Hr(){z({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",Or)}async function Or(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector('button[type="submit"]'),s={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim()||null,phone:t.querySelector("#regClientPhone").value.trim(),dob:`${t.querySelector("#regClientDobDay").value.trim()}/${t.querySelector("#regClientDobMonth").value.trim()}`,notes:t.querySelector("#regClientNotes").value.trim()||null};if(!s.name||!s.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{const r=await Sa(s);x.clients.push({id:r.id,...s}),g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",Po(r.id)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar Cliente"}}function zr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");return}const{modalElement:e,close:t}=z({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{let r="";const o=e.querySelector("#add-item-content"),n={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>',package:'<svg class="w-6 h-6 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7m8 4v10M4 7v10l8 4" /></svg>'},i=()=>{const l=r.toLowerCase(),d=u=>u.filter(m=>m.name.toLowerCase().includes(l)),c={"modal-service-list":{items:d(x.catalog.services),type:"service"},"modal-product-list":{items:d(x.catalog.products),type:"product"},"modal-package-list":{items:d(x.catalog.packages),type:"package"}};for(const[u,{items:m,type:b}]of Object.entries(c)){const v=o.querySelector(`#${u}`);v&&(v.innerHTML=m.map(h=>`
            ...
            <div class="text-center p-8 relative">
                ...
                <h3 class="font-bold text-2xl text-gray-800">${f(r.name)}</h3>
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{o>1&&(o--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{o++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await Wr(r,o),t()}};e.addEventListener("click",r=>{const o=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(o){const{itemType:i,itemId:l}=o.dataset,c=(x.catalog[i+"s"]||[]).find(u=>u.id===l);c&&s({...c,type:i})}else n&&a()}),a()}async function Po(e=null){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");return}if(!x.clients||x.clients.length===0)try{x.clients=await Nt(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de clientes.","error");return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await Q(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de profissionais.","error");return}const t=x.clients.map(n=>{const i=n.id===e?"selected":"";return`<option value="${n.id}" ${i}>${f(n.name)} - ${f(n.phone)}</option>`}).join(""),a=p.professionals.map(n=>`<option value="${n.id}">${f(n.name)}</option>`).join(""),s=`
    ...
    `,{modalElement:r}=z({title:"Nova Venda Avulsa",contentHTML:s,maxWidth:"max-w-md"});r.querySelector("#new-sale-form").addEventListener("submit",Qr);const o=r.querySelector('[data-action="new-client-from-sale"]');o&&o.addEventListener("click",n=>{n.preventDefault(),r.style.display="none",Hr()})}function Vr(){if(!x.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de finalizar pagamentos.","error");return}const e=x.allComandas.find(u=>u.id===x.selectedComandaId);if(!e)return;const t=Rt(e),a=new Set,s=[];for(const u of t){if(u.type==="service"&&u.id){if(a.has(u.id))continue;a.add(u.id)}s.push(u)}const r=s.reduce((u,m)=>u+(m.price||0),0);let o=[],n={remainingAmount:r,selectedMethod:"dinheiro",installments:1,amountReceived:""};const i=()=>{const u=document.getElementById("payment-list"),m=document.getElementById("remaining-amount"),b=document.getElementById("finalize-checkout-btn"),v=document.getElementById("change-container"),h=document.getElementById("installments-container"),w=document.getElementById("payment-value"),$=document.getElementById("payment-controls"),I=o.reduce((k,C)=>k+C.value,0);n.remainingAmount=r-I,u.innerHTML=o.map((k,C)=>`
            ...
            </div>`).join(""),n.remainingAmount<=.001?(m.textContent="Total Pago!",m.className="text-lg font-bold text-center mb-4 text-green-600",w.value="",b.disabled=!1,$&&($.style.display="none")):(m.textContent=`Faltam: R$ ${n.remainingAmount.toFixed(2)}`,m.className="text-lg font-bold text-center mb-4 text-red-600",w.value=n.remainingAmount.toFixed(2),b.disabled=!0,$&&($.style.display="block")),document.querySelectorAll(".payment-method-btn").forEach(k=>{k.classList.toggle("ring-2",k.dataset.method===n.selectedMethod),k.classList.toggle("ring-offset-2",k.dataset.method===n.selectedMethod)}),h.style.display=["credito","crediario"].includes(n.selectedMethod)?"block":"none",v.style.display=n.selectedMethod==="dinheiro"&&n.remainingAmount>0?"block":"none";const E=parseFloat(n.amountReceived)-n.remainingAmount;document.getElementById("change-value").textContent=`R$ ${E>0?E.toFixed(2):"0.00"}`},l=()=>{const u=document.getElementById("payment-value");let m=parseFloat(u.value);if(isNaN(m)||m<=0){g("Valor Inválido","Insira um valor de pagamento válido e maior que zero.","error");return}if(m>n.remainingAmount+.001){g("Valor Inválido","O valor excede o saldo restante.","error");return}const b={method:n.selectedMethod,value:m};["credito","crediario"].includes(n.selectedMethod)&&n.installments>1&&(b.installments=n.installments),o.push(b),n.selectedMethod="dinheiro",n.installments=1,document.getElementById("installments-select").value=1,i()},d=`
    ...
    `,{modalElement:c}=z({title:"Finalizar Pagamento",contentHTML:d,maxWidth:"max-w-md"});document.getElementById("payment-value").value=n.remainingAmount.toFixed(2),c.addEventListener("click",u=>{const m=u.target.closest(".payment-method-btn");m&&(n.selectedMethod=m.dataset.method,n.installments=1,document.getElementById("installments-select").value=1,i()),u.target.closest("#add-payment-btn")&&l(),u.target.closest('[data-action="remove-payment"]')&&(o.splice(parseInt(u.target.closest('[data-action="remove-payment"]').dataset.paymentIndex,10),1),i()),u.target.closest("#finalize-checkout-btn")&&Yr(e,r,o)}),c.addEventListener("change",u=>{u.target.id==="installments-select"&&(n.installments=parseInt(u.target.value,10))}),c.addEventListener("input",u=>{u.target.id==="amount-received"&&(n.amountReceived=u.target.value,i())}),i()}async function Ur(){const e=`
    ...
    `,{modalElement:t}=z({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const s=document.getElementById("initial-amount"),r=s.value.trim(),o=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),s.focus();return}try{const i={establishmentId:p.establishmentId,initialAmount:parseFloat(n.toFixed(2))};o&&(i.notes=o);const l=await Br(i);x.isCashierOpen=!0,x.activeCashierSessionId=l.id,await Ea(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function Jr(){const e=x.activeCashierSessionId;if(e)try{const t=await Mr(e),a=`
        ...
        `,{modalElement:s}=z({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});s.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const o=parseFloat(document.getElementById("final-amount").value);if(isNaN(o)||o<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await Dr(e,o),x.isCashierOpen=!1,x.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await Ea(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function _r(e){x.activeFilter!==e&&(x.activeFilter=e,x.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),fe(),await se(),x.selectedComandaId=null,X())}function ia(e){x.selectedComandaId=e,ue(),Fr(),X()}async function Wr(e,t){const a=x.allComandas.find(r=>r.id===x.selectedComandaId);if(!a)return;const s=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...s),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),X(),ue();return}try{await ya(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),X(),ue()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function Gr(e,t){const a=x.allComandas.find(o=>o.id===x.selectedComandaId);if(!a)return;let s=!1,r=(a.comandaItems||[]).findIndex(o=>o.id===e&&o.type===t);if(r>-1)a.comandaItems.splice(r,1),s=!0;else{let o=(a.services||[]).findIndex(n=>n.id===e);if(o>-1)a.services.splice(o,1),s=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),s=!0)}}if(s){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),X(),ue();return}try{await ya(a.id,a),g("Sucesso","Item removido!","success"),X(),ue()}catch(o){g("Erro",`Não foi possível remover o item: ${o.message}`,"error"),await se()}}}async function Yr(e,t,a){const s=e.type==="appointment",r=Rt(e),o=new Set,n=[];for(const l of r){if(l.type==="service"&&l.id){if(o.has(l.id))continue;o.add(l.id)}n.push(l)}const i={payments:a,totalAmount:t,items:n,cashierSessionId:x.activeCashierSessionId};try{s?await vs(e.id,i):(i.establishmentId=p.establishmentId,i.clientName=e.clientName,i.professionalId=e.professionalId,i.clientPhone=e.clientPhone,await kr(i)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",fe(),x.selectedComandaId=null,await se()}catch(l){g("Erro no Checkout",l.message,"error")}}async function Qr(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,s=x.clients.find(n=>n.id===t),r=p.professionals.find(n=>n.id===a);if(!s||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const o={id:`temp-${Date.now()}`,type:"walk-in",clientName:s.name,clientPhone:s.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};x.allComandas.unshift(o),x.selectedComandaId=o.id,document.getElementById("genericModal").style.display="none",ia(o.id)}async function se(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=x.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await Bo();if(x.isCashierOpen=!!a,x.activeCashierSessionId=a?a.id:null,jr(),!x.isCashierOpen&&x.activeFilter==="atendimento"){ue(),X();return}const s=await wr(p.establishmentId,t,x.paging.page,x.paging.limit);if(x.allComandas=s.data||s,x.paging.total=s.total||s.length,x.catalog.services.length===0){const[r,o,n,i,l]=await Promise.all([Ne(p.establishmentId),Ft(p.establishmentId),Do(p.establishmentId),Nt(p.establishmentId),Q(p.establishmentId)]);x.catalog={services:r,products:o,packages:n},x.clients=i,p.professionals=l}ue(),x.selectedComandaId,X()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function Ea(e={}){Se=document.getElementById("content");try{const t=await Bo();x.isCashierOpen=!!t,x.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),x.isCashierOpen=!1}x.selectedComandaId=e.selectedAppointmentId||null,Rr(),ke&&(Se.removeEventListener("click",ke),Se.removeEventListener("change",ke)),ke=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&x.activeFilter==="finalizadas"){x.paging.page=1,await se();return}if(a){if(a.matches("[data-filter]"))_r(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}ia(a.dataset.comandaId)}else if(a.matches("[data-action]")){const s=a.dataset.action,r=a.dataset.id||x.selectedComandaId;switch(s){case"back-to-list":{fe(),x.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(o=>o.classList.remove("selected")),X();break}case"new-sale":Po();break;case"add-item":zr();break;case"checkout":Vr();break;case"open-cashier":Ur();break;case"close-cashier":await Jr();break;case"view-sales-report":Z("sales-report-section");break;case"remove-item":await Gr(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await O("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await fs(r);const n=x.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete x.allComandas[n].transaction,delete x.allComandas[n].cashierSessionId,delete x.allComandas[n].redeemedReward,x.allComandas[n].status="confirmed"),x.selectedComandaId=null,fe(),g("Sucesso!","Comanda reaberta para edição.","success"),await se()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await O("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await Sr(r),g("Sucesso!","Venda revertida."),fe(),x.selectedComandaId=null,await se()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const o=a.dataset.id,n=a.dataset.date;Z("agenda-section",{scrollToAppointmentId:o,targetDate:new Date(n)});break}case"delete-walk-in":{if(await O("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))x.allComandas=x.allComandas.filter(n=>n.id!==r),x.selectedComandaId=null,ue(),X(),g("Sucesso","Venda avulsa removida.","success"),fe();else try{await $r(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),x.selectedComandaId=null,fe(),await se()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},Se.addEventListener("click",ke),Se.addEventListener("change",ke),e.initialFilter&&(x.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(x.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${x.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",x.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await se(),x.selectedComandaId&&ia(x.selectedComandaId)}const la=document.getElementById("content");let Le={};const it=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],M={startDate:new Date(new Date().getFullYear(),0,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,currentTab:"dashboards",apptViewMode:"year",apptSelectedMonth:null};async function Xr(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function Zr(){la.innerHTML='<div class="flex flex-col items-center justify-center h-64"><div class="loader mb-4"></div><p class="text-gray-500">A carregar inteligência de dados...</p></div>';try{await Xr();const[e,t]=await Promise.all([Q(p.establishmentId),Us().catch(()=>[])]);M.professionalsList=e||[],M.costCentersList=t||[],Kr(),await Mo()}catch(e){console.error("Erro no loadReportsPage:",e),la.innerHTML=`
            <div class="flex flex-col items-center justify-center h-full text-red-500">
                ...
                <p>Erro ao carregar relatórios: ${f(e.message)}</p>
            ...
            </div>`}}function Kr(){const e=M.professionalsList.map(a=>`<option value="${a.id}">${f(a.name)}</option>`).join(""),t=M.costCentersList.map(a=>`<option value="${a.id}">${f(a.name)}</option>`).join("");la.innerHTML=`
            ...
            <div class="bg-white shadow-sm border-b sticky top-0 z-20 px-4 py-4">
                <div class="max-w-7xl mx-auto flex flex-col xl:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex flex-col md:flex-row gap-2 w-full xl:w-auto">
                        ...
                        <div class="flex gap-2">
                            <input type="date" id="report-start" value="${M.startDate}" class="border rounded-lg px-2 py-2 text-sm w-full">
                            <input type="date" id="report-end" value="${M.endDate}" class="border rounded-lg px-2 py-2 text-sm w-full">
    ...
    `,document.getElementById("btn-filter").onclick=en,document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{M.currentTab=a.dataset.tab,Ga(),Ao()}}),Ga()}function Ga(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===M.currentTab;e.className=t?"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-bold bg-white text-indigo-600 shadow-sm transition-all whitespace-nowrap":"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition-all whitespace-nowrap"})}async function en(){M.startDate=document.getElementById("report-start").value,M.endDate=document.getElementById("report-end").value,M.selectedProfessional=document.getElementById("report-prof").value,M.selectedCostCenter=document.getElementById("report-cost").value,M.apptViewMode="year",M.apptSelectedMonth=null,await Mo()}async function Mo(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader"></div></div>';try{const t=await zs(M.startDate,M.endDate,M.selectedProfessional,M.selectedCostCenter);M.data=t,Ao()}catch(t){console.error(t),e.innerHTML=`
            <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded text-red-700 text-center">
                ...
                <p class="text-sm">${f(t.message||"Verifique sua conexão.")}</p>
            </div>`}}function Ao(){const e=document.getElementById("report-content");if(M.data)switch(M.currentTab){case"dashboards":tn(e);break;case"appointments":Ia(e);break;case"dre":rn(e);break}}function tn(e){const{dreSimple:t,charts:a}=M.data,s=t||{grossRevenue:0,netProfit:0,variableCosts:0},r=M.data.totalAppointments||0,o=r>0?s.grossRevenue/r:0;e.innerHTML=`
    ...
    `,lt("chart-monthly","bar","Receita Mensal",a.salesMonthly.labels,a.salesMonthly.data,it[0]);const n=a.professionals.labels.map(l=>f(l));lt("chart-profs","doughnut","Total Vendas",n,a.professionals.data,it),lt("chart-daily","line","Vendas Diárias",a.salesDaily.labels,a.salesDaily.data,it[4]);const i=a.products.labels.map(l=>f(l));lt("chart-products","bar","Total Vendido",i,a.products.data,it[1])}function Ia(e){const{charts:t}=M.data;let a='<h3 class="font-bold text-gray-700 text-lg">Visão Anual/Mensal</h3>',s="Clique na barra do Mês para detalhar os dias";M.apptViewMode==="month"&&(a=`
            <div class="flex items-center gap-4">
                ...
                <h3 class="font-bold text-gray-700 text-lg">Detalhes de ${f(M.apptSelectedMonth)}</h3>
    ...
    `;const r=document.getElementById("btn-back-year");r&&(r.onclick=()=>{M.apptViewMode="year",M.apptSelectedMonth=null,Ia(e)});try{an()}catch(o){console.error("Erro ao renderizar gráfico de agendamentos:",o)}}function an(){const e=document.getElementById("chart-appointments").getContext("2d");Le["chart-appointments"]&&Le["chart-appointments"].destroy();const{charts:t}=M.data;let a,s;if(M.apptViewMode==="year")a=t.appointmentsMonthly.labels,s=t.appointmentsMonthly.data;else{const o=M.apptSelectedMonth.match(/([a-zç\.]+)(?:[\/\s\-]*)(\d{2,4})/i);if(!o){console.error("Formato de mês não reconhecido:",M.apptSelectedMonth);return}const n=o[1].toLowerCase().replace(/\./g,"").trim(),i=o[2],l=i.length===2?"20"+i:i,u=`/${{jan:"01",fev:"02",mar:"03",abr:"04",mai:"05",jun:"06",jul:"07",ago:"08",set:"09",out:"10",nov:"11",dez:"12",janeiro:"01",fevereiro:"02",março:"03",abril:"04",maio:"05",junho:"06",julho:"07",agosto:"08",setembro:"09",outubro:"10",novembro:"11",dezembro:"12"}[n]||"01"}/${l}`,m=Object.entries(t.appointmentsDaily.data).map(([b,v],h)=>{const w=t.appointmentsDaily.labels[h];return w.includes(u)?{label:w,value:v}:null}).filter(Boolean);a=m.map(b=>b.label),s=m.map(b=>b.value)}const r={type:"bar",data:{labels:a,datasets:[{label:"Volume de Agendamentos",data:s,backgroundColor:M.apptViewMode==="year"?"#3b82f6":"#8b5cf6",borderRadius:4,hoverBackgroundColor:"#1e40af"}]},options:{responsive:!0,maintainAspectRatio:!1,onClick:(o,n)=>{if(n.length>0){const i=n[0].index,l=a[i];on(l)}},plugins:{legend:{display:!1},tooltip:{callbacks:{label:o=>` ${o.raw} Agendamentos`}}}}};Le["chart-appointments"]=new Chart(e,r)}function on(e){M.apptViewMode==="year"?(M.apptViewMode="month",M.apptSelectedMonth=e,Ia(document.getElementById("report-content"))):M.apptViewMode==="month"&&sn(e)}async function sn(e){const[t,a,s]=e.split("/"),r=`${s}-${a}-${t}`;z({title:"Carregando...",contentHTML:'<div class="p-8 text-center"><div class="loader mx-auto"></div></div>',maxWidth:"max-w-2xl"});try{const o=await Vs(r,M.selectedProfessional);let n="";o.length===0?n='<div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg">Nenhum agendamento encontrado para este dia.</div>':n=`
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 text-sm">
                            ${o.map(l=>`
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3 font-bold text-gray-900">${f(l.time||"--:--")}</td>
                                    <td class="px-4 py-3 text-indigo-700 font-medium">${f(l.professionalName||"N/A")}</td>
                                    <td class="px-4 py-3 text-gray-700">${f(l.clientName||"Cliente")}</td>
                                    <td class="px-4 py-3 text-gray-500">${f(l.serviceName||"Serviço")}</td>
                                    <td class="px-4 py-3">
            ...
            `;const i=document.querySelector(".modal-content-body");i?(i.innerHTML=n,document.querySelector(".modal-title").innerText=`Agendamentos de ${e}`):(document.getElementById("genericModal").style.display="none",z({title:`Agendamentos de ${e}`,contentHTML:n,maxWidth:"max-w-4xl"}))}catch{g("Erro","Não foi possível carregar os detalhes do dia.","error"),document.getElementById("genericModal").style.display="none"}}function rn(e){const{dreFinancial:t}=M.data,a=Object.entries(t.revenues).map(([r,o])=>`
        <tr class="text-sm text-gray-600 bg-green-50/30 hover:bg-green-50 transition-colors">
            <td class="pl-8 py-2 border-l-4 border-transparent hover:border-green-400">${f(r)}</td>
    ...
    `).join(""),s=Object.entries(t.expenses).map(([r,o])=>`
        <tr class="text-sm text-gray-600 bg-red-50/30 hover:bg-red-50 transition-colors">
            <td class="pl-8 py-2 border-l-4 border-transparent hover:border-red-400">${f(r)}</td>
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden animate-fade-in mb-10">
            <div class="bg-gray-900 text-white p-6 text-center relative overflow-hidden">
                ...
                <p class="text-sm opacity-75 mt-1">
                    ${new Date(M.startDate).toLocaleDateString("pt-BR")} até ${new Date(M.endDate).toLocaleDateString("pt-BR")}
                </p>
            </div>
    ...
    `}function lt(e,t,a,s,r,o){const n=document.getElementById(e);if(!n)return;const i=n.getContext("2d");Le[e]&&Le[e].destroy();const l={type:t,data:{labels:s,datasets:[{label:a,data:r,backgroundColor:Array.isArray(o)?o:t==="line"?"rgba(79, 70, 229, 0.1)":o,borderColor:Array.isArray(o)?"#fff":o,borderWidth:2,fill:t==="line",tension:.3,borderRadius:t==="bar"?4:0,pointBackgroundColor:"#fff",pointBorderColor:o,pointHoverBackgroundColor:o}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:t==="doughnut",position:"bottom"},tooltip:{backgroundColor:"rgba(17, 24, 39, 0.9)",padding:10,cornerRadius:8,callbacks:{label:d=>{let c=d.dataset.label||"";return c&&(c+=": "),d.parsed.y!==null?c+=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(d.parsed.y):c+=d.raw,c}}}},scales:t==="doughnut"?{}:{y:{beginAtZero:!0,grid:{color:"#f3f4f6"},ticks:{font:{size:11}}},x:{grid:{display:!1},ticks:{font:{size:11}}}}}};Le[e]=new Chart(i,l)}const jt=(e,t="products")=>y(`/api/${t}/categories/${e}`),No=(e,t="products")=>y(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),qo=(e,t="products")=>y(`/api/${t}/categories/${e}`,{method:"DELETE"}),nn="audit_logs",Pe=async(e,t,a,s,r,o=null)=>{try{if(!t)return;await va(Ae(te,nn),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:s,description:r,details:o,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},be=document.getElementById("content");let le=null,Ue="services",he="all";function Me(){const e=G.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function ln(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await No({establishmentId:p.establishmentId,name:s},"services"),Pe(p.establishmentId,Me(),"Categorias (Serviços)","Criou",`Criou categoria: ${s}`),a.value="",g("Sucesso","Categoria criada!","success"),await Ca(),await st()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function dn(e){if(await O("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await qo(e,"services"),Pe(p.establishmentId,Me(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await Ca(),await st()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function Ca(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await jt(p.establishmentId,"services");p.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                ...
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria criada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function cn(){z({title:"Gerir Categorias de Serviços",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",ln),t.addEventListener("click",s=>{const r=s.target.closest('button[data-action="delete-category"]');r&&(s.preventDefault(),dn(r.dataset.id))}))}Ca()}async function un(e){e.preventDefault();const t=e.target.closest("#serviceModal"),a=t.querySelector("#serviceId").value,s={},r=t.querySelector('input[name="commissionType"]:checked').value;r==="custom"&&t.querySelectorAll(".professional-commission-row").forEach(n=>{const i=n.dataset.profId;if(n.querySelector('input[type="checkbox"]').checked){const d=parseFloat(n.querySelector('input[type="number"]').value);s[i]=isNaN(d)?0:d}});const o={establishmentId:p.establishmentId,name:t.querySelector("#serviceName").value,price:parseFloat(t.querySelector("#servicePrice").value),duration:parseInt(t.querySelector("#serviceDurationMinutes").value,10),bufferTime:parseInt(t.querySelector("#serviceBufferTimeMinutes").value,10)||0,categoryId:t.querySelector("#serviceCategory").value||null,commissionRate:parseFloat(t.querySelector("#serviceCommissionRate").value)||0,active:t.querySelector("#serviceStatus").value==="true",photo:t.querySelector("#servicePhotoBase64").value,notes:t.querySelector("#serviceNotes").value,commissionType:r,professionalCommissions:s};try{a?(await Gs(a,o),Pe(p.establishmentId,Me(),"Serviços","Editou",`Editou o serviço: ${o.name}`)):(await Ws(o),Pe(p.establishmentId,Me(),"Serviços","Criou",`Criou novo serviço: ${o.name}`)),document.getElementById("serviceModal").style.display="none",g("Sucesso",`Serviço ${a?"atualizado":"adicionado"} com sucesso!`,"success"),await st()}catch(n){g("Erro",n.message,"error")}}function mn(e,t=800,a=800,s="image/jpeg",r=.8){return new Promise((o,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=l=>{const d=new Image;d.onload=()=>{let c=d.width,u=d.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(d,0,0,c,u);const v=m.toDataURL(s,r);o(v)},d.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),d.src=l.target.result},i.onerror=l=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Ya(e=null){const t=document.getElementById("serviceModal"),a=p.serviceCategories||[],s=e?.duration||0,r=e?.bufferTime||0,o=f(e?.name||""),n=f(e?.notes||""),i=e?o:"Novo Serviço",l=a.map(k=>`<option value="${k.id}" ${e?.categoryId===k.id?"selected":""}>${f(k.name)}</option>`).join("");t.innerHTML=`
    <div class="modal-content max-w-3xl overflow-y-auto max-h-[85vh] my-auto">
        <form id="serviceForm">
            ...
            <div class="mt-6 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-3">
                ...
                <div class="professional-commission-row flex items-center justify-between p-2 rounded-md ${C?"bg-blue-50":""}" data-prof-id="${k.id}">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <img src="${k.photo||`https://placehold.co/40x40/E2E8F0/4A5568?text=${f(k.name.charAt(0))}`}" class="w-8 h-8 rounded-full object-cover">
                        <span class="text-sm font-medium">${f(k.name)}</span>
                    </label>
            ...
            `}).join(""),h.querySelectorAll('input[type="checkbox"]').forEach(k=>{k.addEventListener("change",C=>{const F=C.target.closest(".professional-commission-row");F.querySelector('input[type="number"]').disabled=!C.target.checked,F.classList.toggle("bg-blue-50",C.target.checked)})})),v();const w=t.querySelector("#serviceForm"),$=t.querySelector("#servicePhotoInput"),I=t.querySelector("#servicePhotoPreview"),E=t.querySelector("#servicePhotoBase64");t.querySelector("#servicePhotoButton").addEventListener("click",()=>$.click()),$.onchange=async()=>{const k=$.files[0];if(k){I.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const C=await mn(k,800,800,"image/jpeg",.8),j=C.length*3/4,L=1e3*1024;if(j>L)throw new Error("A imagem é muito grande mesmo após a compressão.");I.src=C,E.value=C}catch(C){console.error("Erro ao processar imagem:",C),g("Erro de Imagem",C.message||"Não foi possível processar a imagem.","error"),I.src=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",E.value=e?.photo||"",$.value=""}}},w.addEventListener("submit",un)}function $e(){const e=document.getElementById("servicesList");if(!e)return;const t=document.getElementById("serviceSearchInput")?.value.toLowerCase()||"",a=document.getElementById("serviceCategoryFilter")?.value||"all",s=new Map((p.serviceCategories||[]).map(o=>[o.id,o.name]));let r=(p.services||[]).filter(Boolean);if(he!=="all"){const o=he==="active";r=r.filter(n=>n.active!==!1===o)}r=r.filter(o=>{const n=o.name.toLowerCase().includes(t),i=a==="all"||o.categoryId===a;return n&&i}),e.innerHTML="",r.length>0?r.forEach(o=>{const n=document.createElement("div"),i=JSON.stringify(o).replace(/'/g,"&apos;");n.className=`service-card bg-white rounded-lg shadow-md flex overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-lg hover:bg-gray-50 ${o.active!==!1?"opacity-100":"opacity-50 bg-gray-100"} sm:flex-col`,n.dataset.action="edit-service",n.dataset.service=i;const l=f(o.name),d=f(s.get(o.categoryId)||"N/A"),c=o.photo||`https://placehold.co/200x200/E2E8F0/4A5568?text=${encodeURIComponent(o.name.charAt(0))}`;n.innerHTML=`
                ...
                </div>`,e.appendChild(n)}):e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum serviço encontrado.</p>'}function La(){const e={active:0,inactive:0,total:0},t=(p.services||[]).filter(Boolean);t.forEach(n=>{n.active===!1?e.inactive++:e.active++}),e.total=t.length;const a=document.getElementById("indicator-total"),s=document.getElementById("indicator-active"),r=document.getElementById("indicator-inactive"),o=document.getElementById("indicator-popular");a&&(a.textContent=e.total),s&&(s.textContent=e.active),r&&(r.textContent=e.inactive),o&&(p.mostPopularService&&p.mostPopularService.name!=="N/A"?(o.textContent=f(p.mostPopularService.name),o.closest(".indicator-card").title=`${p.mostPopularService.name} (${p.mostPopularService.count} agendamentos)`):(o.textContent="N/A",o.closest(".indicator-card").title="Nenhum serviço agendado ainda"))}function pn(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${f(a.name)}</option>`)),La(),$e()}function gn(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function st(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,s,r]=await Promise.all([Ne(p.establishmentId),Q(p.establishmentId),jt(p.establishmentId,"services"),Xs(p.establishmentId)]);p.services=(t||[]).filter(Boolean),p.professionals=(a||[]).filter(Boolean),p.serviceCategories=(s||[]).filter(Boolean),p.mostPopularService=r||{name:"N/A",count:0},p.services.forEach(o=>{o.active===void 0&&(o.active=!0)}),Fo(Ue)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function Fo(e){if(document.getElementById("services-content-container")){if(Ue===e&&document.getElementById("services-content-container").children.length>1){Ue==="services"&&(La(),$e());return}Ue=e,he="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?pn():e==="reports"&&gn()}}function bn(){le&&(be.removeEventListener("click",le),be.removeEventListener("input",le),be.removeEventListener("change",le)),le=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),o=r.dataset.id,n=r.checked;try{await Qs(o,n);const i=p.services.findIndex(l=>l.id===o);i>-1&&(p.services[i].active=n),Pe(p.establishmentId,Me(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${o}) para ${n?"Ativo":"Inativo"}`),$e(),La()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,$e()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){$e();return}if(!a)return;if(a.hasAttribute("data-view")){Fo(a.dataset.view);return}switch(a.dataset.action){case"new-service":Ya();break;case"edit-service":const r=JSON.parse(a.dataset.service);Ya(r);break;case"manage-categories":cn();break;case"filter-service":const o=a.dataset.filterType;if(o==="popular")return;he=o==="total"?"all":o,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,d=i===he||i==="total"&&he==="all";n.classList.toggle("ring-2",d),n.classList.toggle("ring-indigo-500",d),n.classList.toggle("shadow-lg",d)}),$e();break}},be.addEventListener("click",le),be.addEventListener("input",le),be.addEventListener("change",le)}async function fn(){be.innerHTML=`
        ...
        </section>`,bn();try{(!p.professionals||p.professionals.length===0)&&(p.professionals=await Q(p.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}Ue="services",he="all",await st()}const Ht="suppliers",Ro="purchases",Ta=async e=>{try{const t=ha(Ae(te,Ht),xa("establishmentId","==",e)),a=await wo(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar fornecedores:",t),t}},vn=async e=>{try{return{id:(await va(Ae(te,Ht),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},hn=async(e,t)=>{try{const a=Dt(te,Ht,e);return await So(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},xn=async e=>{try{const t=Dt(te,Ht,e);return await ns(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},yn=async e=>{try{const t={...e,createdAt:new Date};return{id:(await va(Ae(te,Ro),t)).id,...t}}catch(t){throw console.error("Erro ao registrar compra:",t),t}},wn=async e=>{try{const t=ha(Ae(te,Ro),xa("establishmentId","==",e),ko("createdAt","desc")),a=await wo(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},me=document.getElementById("content");let de=null,Je="products",oe="all";async function kn(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await No({establishmentId:p.establishmentId,name:s},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await Ba(),await rt()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function Sn(e){if(await O("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await qo(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await Ba(),await rt()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function Ba(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await jt(p.establishmentId,"products");p.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${f(a.name)}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function $n(){z({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",kn),t.addEventListener("click",s=>{const r=s.target.closest('button[data-action="delete-category"]');r&&Sn(r.dataset.id)}))}Ba()}async function En(e){if(!e)return;if(await O("Apagar Produto","Tem a certeza que deseja apagar este produto? Esta ação não pode ser revertida."))try{await Cr(e),g("Sucesso","Produto apagado com sucesso!","success"),await rt()}catch(a){g("Erro",`Não foi possível apagar o produto: ${a.message}`,"error")}}async function In(e){const t=e.querySelector("#productId").value,a=parseInt(e.querySelector("#productCurrentStock").value),s=parseInt(e.querySelector("#productMinStock").value),r=parseInt(e.querySelector("#productMaxStock").value),o=e.querySelectorAll("#selectedSuppliersList .selected-supplier-item"),n=Array.from(o).map(l=>l.dataset.id),i={establishmentId:p.establishmentId,name:e.querySelector("#productName").value,price:parseFloat(e.querySelector("#productPrice").value),costPrice:parseFloat(e.querySelector("#productCostPrice").value)||0,commissionRate:parseFloat(e.querySelector("#productCommissionRate").value)||0,currentStock:isNaN(a)?0:a,minStock:isNaN(s)?0:s,maxStock:isNaN(r)?0:r,categoryId:e.querySelector("#productCategory").value||null,photo:e.querySelector("#productPhotoBase64").value,supplierIds:n};try{t?await Ir(t,i):await Er(i),document.getElementById("productModal").style.display="none",g("Sucesso",`Produto ${t?"atualizado":"adicionado"} com sucesso!`,"success"),await rt()}catch(l){throw new Error(l.message)}}function Qa(e,t=800,a=800,s="image/jpeg",r=.8){return new Promise((o,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=l=>{const d=new Image;d.onload=()=>{let c=d.width,u=d.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(d,0,0,c,u);const v=m.toDataURL(s,r);o(v)},d.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),d.src=l.target.result},i.onerror=l=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Xa(e=null){const t=document.getElementById("productModal"),a=p.categories||[],s=p.suppliers||[],r=a.map(L=>`<option value="${L.id}" ${e?.categoryId===L.id?"selected":""}>${f(L.name)}</option>`).join("");let o=new Set(e?.supplierIds||[]);const n=f(e?.name||""),i=e?.price||"",l=e?.costPrice||"",d=e?.commissionRate||"",c=e?.minStock||0,u=e?.maxStock||0,m=e?.currentStock||0,b=e?n:"Novo Produto";t.innerHTML=`
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                ...
                <div class="flex flex-col-reverse sm:flex-row w-full sm:w-auto gap-3">
    ...
    </div>`;const v=t.querySelector("#productCategory"),h=t.querySelector("#productPhotoInput");t.querySelector("#productPhotoButton").addEventListener("click",()=>h.click()),v.innerHTML='<option value="">Sem categoria</option>'+(p.categories||[]).map(L=>`<option value="${L.id}" ${e?.categoryId===L.id?"selected":""}>${f(L.name)}</option>`).join(""),e&&(v.value=e.categoryId||"");const w=t.querySelector("#productPhotoPreview");t.querySelector("#productPhotoBase64");const $=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",I=e?.photo||"";h.onchange=async()=>{const L=h.files[0];if(L){w.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const T=await Qa(L,800,800,"image/jpeg",.8),q=T.length*3/4,H=1e3*1024;if(q>H)throw new Error("A imagem é muito grande mesmo após a compressão.");preview.src=T,base64Input.value=T}catch(T){console.error("Erro ao processar imagem:",T),g("Erro de Imagem",T.message||"Não foi possível processar a imagem.","error"),preview.src=$,base64Input.value=I,j.value=""}}};const E=t.cloneNode(!0);t.parentNode.replaceChild(E,t);const k=()=>{const L=E.querySelector("#modalSupplierSearch"),T=E.querySelector("#supplierSearchResults"),N=E.querySelector("#selectedSuppliersList"),q=L.value.toLowerCase();if(q.length>0){const H=s.filter(A=>A.name.toLowerCase().includes(q)&&!o.has(A.id));H.length>0?(T.classList.remove("hidden"),T.innerHTML=H.map(A=>`
                    <div class="p-2 hover:bg-indigo-50 cursor-pointer border-b last:border-0 text-sm flex justify-between items-center" data-add-supplier="${A.id}">
                        <span class="font-medium">${f(A.name)}</span>
                        <span class="text-indigo-600 text-xs">+ Adicionar</span>
                    </div>
                `).join("")):(T.classList.remove("hidden"),T.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else T.classList.add("hidden");o.size>0?(N.innerHTML="",o.forEach(H=>{const A=s.find(J=>J.id===H);A&&(N.innerHTML+=`
                        <div class="selected-supplier-item flex items-center justify-between bg-white border p-2 rounded shadow-sm" data-id="${A.id}">
                            <div class="text-sm">
                                <p class="font-bold text-gray-800">${f(A.name)}</p>
                                <p class="text-xs text-gray-500">${f(A.contactName||"")} - ${f(A.phone||"")}</p>
                            </div>
                    ...
                    `)})):N.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};E.querySelector("#modalSupplierSearch").addEventListener("input",k),E.addEventListener("click",L=>{const T=L.target.closest("[data-add-supplier]");if(T){const q=T.dataset.addSupplier;o.add(q),E.querySelector("#modalSupplierSearch").value="",k()}const N=L.target.closest("[data-remove-supplier]");if(N){const q=N.dataset.removeSupplier;o.delete(q),k()}}),k(),E.addEventListener("click",async L=>{const T=L.target.closest("button[data-action]");if(!T)return;const N=T.dataset.action,q=E.querySelector("#productId").value;if(N==="close-modal"&&(E.style.display="none"),N==="delete-product"){if(!q)return;E.style.display="none",await En(q)}if(N==="save-product-modal"){const H=E.querySelector("#productForm");if(H){if(!H.querySelector("#productName").value||!H.querySelector("#productPrice").value){g("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const A=T.closest('button[data-action="save-product-modal"]');A.disabled=!0,A.textContent="A salvar...";try{await In(H)}catch(J){g("Erro",`Falha ao salvar: ${J.message}`,"error"),A.disabled=!1,A.textContent="Salvar Alterações"}}}if(N==="adjust-stock-modal"){L.preventDefault();const H=E.querySelector("#stockAdjustmentAmount"),A=E.querySelector("#stockAdjustmentReason"),J=parseInt(H.value,10),we=parseInt(T.dataset.change,10);if(!J||J<=0){g("Erro","Por favor, insira uma quantidade válida.","error");return}const Ut=J*we,Ko=A.value||(Ut>0?"Entrada manual":"Saída manual");try{await Lr(q,{change:Ut,reason:Ko});const Fe=p.products.findIndex(Re=>Re.id===q);if(Fe>-1){const Re=p.products[Fe].currentStock+Ut;p.products[Fe].currentStock=Re,E.querySelector("#currentStockDisplay").textContent=Re,E.querySelector("#productCurrentStock").value=Re,H.value="",A.value="",g("Sucesso","Estoque atualizado!","success"),Da(),Ke()}}catch(Fe){g("Erro de Stock",Fe.message,"error")}}});const C=E.querySelectorAll(".tab-btn"),F=E.querySelectorAll(".tab-content");C.forEach(L=>{L.addEventListener("click",T=>{T.preventDefault(),C.forEach(N=>{N.classList.remove("border-indigo-500","text-indigo-600"),N.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),L.classList.add("border-indigo-500","text-indigo-600"),L.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),F.forEach(N=>N.classList.add("hidden")),document.getElementById(`tab-content-${L.dataset.tab}`).classList.remove("hidden")})});const j=E.querySelector("#productPhotoInput");E.querySelector("#productPhotoButton").addEventListener("click",()=>j.click()),j.onchange=async()=>{const L=j.files[0];if(!L)return;const T=E.querySelector("#productPhotoPreview"),N=E.querySelector("#productPhotoBase64");T.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const q=await Qa(L,800,800,"image/jpeg",.8),A=q.length*3/4,J=1e3*1024;if(A>J)throw new Error("A imagem é muito grande mesmo após a compressão.");T.src=q,N.value=q}catch(q){console.error("Erro ao processar imagem:",q),g("Erro de Imagem",q.message||"Não foi possível processar a imagem.","error"),T.src=$,N.value=I,j.value=""}},E.style.display="flex"}function Cn(){const e=document.getElementById("products-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${f(a.name)}</option>`)),Da(),Ke()}function Ln(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const s=a.toISOString().split("T")[0];e.innerHTML=`
        ...
        </div>`;const r=document.getElementById("productFilterReport"),o=document.getElementById("categoryFilterReport");r&&p.products&&(r.innerHTML+=p.products.map(n=>`<option value="${n.id}">${f(n.name)}</option>`).join("")),o&&p.categories&&(o.innerHTML+=p.categories.map(n=>`<option value="${n.id}">${f(n.name)}</option>`).join(""))}async function Tn(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value,establishmentId:p.establishmentId};try{const a=await Tr(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
        ...
        </section>`,de&&(me.removeEventListener("click",de),me.removeEventListener("input",de),me.removeEventListener("change",de)),de=async e=>{const t=e.target;if(t.id==="productSearchInput"||t.id==="productCategoryFilter"){Ke();return}const a=e.target.closest("button[data-action], button[data-view], .indicator-card[data-action], .product-card[data-action]");if(!a||e.target.closest('[data-action-stop-propagation="true"]'))return;if(a.hasAttribute("data-view")){jo(a.dataset.view);return}switch(a.dataset.action){case"new-product":Xa();break;case"edit-product":Xa(JSON.parse(a.dataset.product));break;case"manage-product-categories":$n();break;case"generate-report":await Tn();break;case"filter-stock":const r=a.dataset.filterType;oe=oe===r?"all":r,document.querySelectorAll(".indicator-card").forEach(o=>{o.classList.toggle("ring-2",o.dataset.filterType===oe),o.classList.toggle("ring-indigo-500",o.dataset.filterType===oe),o.classList.toggle("shadow-lg",o.dataset.filterType===oe)}),Ke();break}},me.addEventListener("click",de),me.addEventListener("input",de),me.addEventListener("change",de),Je="products",oe="all",await rt()}const pe=document.getElementById("content");let ce=null,yt="list",R={step:1,productsToBuy:[],allSuppliers:[],finalOrders:{},isQuoteMode:!1};async function Dn(){yt==="list"?Ot():yt==="purchases"?(R.step=1,_e()):yt==="history"&&Fn()}async function Pn(){try{const e=await Ta(p.establishmentId);return p.suppliers=e||[],R.allSuppliers=e,!0}catch(e){return console.error(e),!1}}async function Mn(e){if(await O("Excluir Fornecedor","Tem a certeza? Isso removerá o vínculo com os produtos."))try{await xn(e),g("Sucesso","Fornecedor excluído.","success"),Pt("genericModal"),Ot()}catch(t){g("Erro","Erro ao excluir: "+t.message,"error")}}async function An(e){e.preventDefault();const t=e.target,a=t.querySelector("#supId").value,s={name:t.querySelector("#supName").value,contactName:t.querySelector("#supContact").value,email:t.querySelector("#supEmail").value,phone:t.querySelector("#supPhone").value,taxId:t.querySelector("#supTaxId").value,category:t.querySelector("#supCategory").value,establishmentId:p.establishmentId},r=t.querySelector('button[type="submit"]');r.disabled=!0,r.textContent="A salvar...";try{a?(await hn(a,s),g("Sucesso","Fornecedor atualizado!","success")):(await vn(s),g("Sucesso","Fornecedor criado!","success")),Pt("genericModal"),Ot()}catch(o){g("Erro","Erro ao salvar: "+o.message,"error")}finally{r.disabled=!1,r.textContent="Salvar"}}async function Ot(){const e=document.getElementById("suppliersList");if(!e)return;e.innerHTML='<div class="loader mx-auto my-8"></div>',await Pn();const t=document.getElementById("supplierSearchInput")?.value.toLowerCase()||"",a=p.suppliers.filter(o=>o.name.toLowerCase().includes(t)||o.contactName&&o.contactName.toLowerCase().includes(t));if(e.innerHTML="",a.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum fornecedor encontrado.</div>';return}let s='<div class="flex flex-col gap-2 md:hidden">';a.forEach(o=>{const n=JSON.stringify(o).replace(/"/g,"&quot;"),i=f(o.name),l=f(o.category||"Geral"),d=f(o.contactName||"");s+=`
    ...
    `;a.forEach(o=>{const n=JSON.stringify(o).replace(/"/g,"&quot;"),i=f(o.name),l=f(o.taxId||"Sem doc."),d=f(o.email||"-"),c=f(o.phone||"-"),u=f(o.category||"Geral");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=s+r}function Nn(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",s=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),o=f(e.name),n=f(e.category||"Fornecedor"),i=f(e.contactName||""),l=f(e.phone||""),d=`
    ...
    `;z({title:"",contentHTML:d,maxWidth:"max-w-md"})}async function _e(){const e=document.getElementById("purchasesContainer");if(e)if(R.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Ft(p.establishmentId),Ta(p.establishmentId)]);R.allSuppliers=a||[];const s=t.filter(d=>{const c=parseInt(d.currentStock||0),u=parseInt(d.minStock||0);return c<=u});if(R.productsToBuy=s,s.length===0){e.innerHTML=`
                ...
                `;return}let r='<div class="flex flex-col gap-3 md:hidden">',o="";s.forEach(d=>{const c=parseInt(d.minStock)||0,u=parseInt(d.currentStock)||0,m=Math.max(c-u,1),b=parseFloat(d.costPrice||0),v=f(d.name);let h='<option value="">Selecione...</option>';R.allSuppliers.length>0?R.allSuppliers.forEach(w=>{const I=d.supplierIds&&d.supplierIds.includes(w.id)?"selected":"";h+=`<option value="${w.id}" ${I}>${f(w.name)}</option>`}):h='<option value="">Sem fornecedores</option>',r+=`
            ...
            `,da()}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao calcular compras.</p>'}}else R.step===2&&qn(e)}function qn(e){if(!R.finalOrders||Object.keys(R.finalOrders).length===0){R.step=1,_e();return}const t=R.isQuoteMode;let a="",s=0;const r=t?"border-indigo-100":"border-gray-200",o=t?"bg-indigo-50 border-indigo-100":"bg-gray-50 border-gray-200",n=t?"bg-blue-100 text-blue-700":"bg-green-100 text-green-700",i=t?"hidden":"flex",l=t?"Cotações Prontas":"Pedidos Prontos",d=t?"text-indigo-600":"text-green-600",c=t?"bg-indigo-50 border-indigo-100":"bg-green-50 border-green-100",u=t?"text-indigo-800":"text-green-800";for(const[m,b]of Object.entries(R.finalOrders)){let v=0,h=b.items.map(C=>{const F=C.qty*C.cost;return v+=F,`
        ...
        `}).join("");s+=v;const w=encodeURIComponent(JSON.stringify({supplierId:m,supplierName:b.info.name,totalAmount:v,items:b.items})),$=encodeURIComponent(JSON.stringify({name:b.info.name,phone:b.info.phone,email:b.info.email})),I=encodeURIComponent(JSON.stringify(b.items)),E=f(b.info.name),k=f(b.info.email||"");a+=`
    ...
    `}async function Fn(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await wn(p.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(o=>{const n=new Date(o.createdAt.seconds*1e3).toLocaleDateString("pt-BR"),i=f(o.supplierName);a+=`
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function Rn(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;z({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",An),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Pt("genericModal"))},50)}function Hn(){pe.innerHTML=`
    ...
    `,ce&&(pe.removeEventListener("click",ce),pe.removeEventListener("input",ce),pe.removeEventListener("change",ce)),ce=e=>{if(e.target.closest("#tab-btn-list")&&dt("list"),e.target.closest("#tab-btn-purchases")&&dt("purchases"),e.target.closest("#tab-btn-history")&&dt("history"),e.target.id==="toggle-quote-mode"&&(R.isQuoteMode=e.target.checked,_e()),e.target.id==="supplierSearchInput"&&Ot(),e.target.closest("#btn-new-supplier")&&Za(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),s=JSON.parse(a.dataset.supplier);Nn(s)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&Mn(t.dataset.id),a==="edit"&&Za(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&da(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(s=>s.checked=a),da()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),s={};let r=!1;if(a.forEach(o=>{if(o.offsetParent===null||!o.querySelector(".row-select").checked)return;r=!0;let i="Produto";const l=o.querySelector("td:nth-child(2)"),d=o.querySelector(".font-bold");l?i=l.innerText:d&&(i=d.innerText);const c=parseInt(o.querySelector(".qty-input").value),u=parseFloat(o.dataset.cost),b=o.querySelector(".supplier-select").value;if(b){if(!s[b]){const v=R.allSuppliers.find(h=>h.id===b);s[b]={info:v,items:[]}}s[b].items.push({name:i,qty:c,cost:u})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}R.finalOrders=s,R.step=2,_e()}if(e.target.closest("#btn-back-step1")&&(R.step=1,_e()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),s=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),o=parseFloat(a.dataset.total),n=R.isQuoteMode;if(s.phone){const i=s.phone.replace(/\D/g,"");let l="";n?(l=`Olá *${s.name}*, tudo bem?
...
Aguardo retorno.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=p.establishmentId,yn(s).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const s=e.target.closest(".supplier-order-card").dataset.supplierId,r=R.finalOrders[s];r?jn(r,R.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);Rn(s)}},pe.addEventListener("click",ce),pe.addEventListener("input",ce),pe.addEventListener("change",ce),dt("list")}function dt(e){yt=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),Dn()}const _t=document.getElementById("content"),Ka={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let ne=new Set,ct=null,Ee=null;function On(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function zn(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",s=f(t.name),r=f(t.specialty||"Especialidade"),o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,n=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function Wt(){const e=document.getElementById("genericModal");e.style.display="none",Ee&&e.removeEventListener("click",Ee)}async function Vn(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},s=f(a.name),r=p.services||await Ne(p.establishmentId),o=p.professionals||await Q(p.establishmentId),n=`
        ...
        </div>`;t.innerHTML=n,t.style.display="flex",Un(a,r),_n(a),Wn(a,o),Yn(a)}function Un(e,t){const a=document.getElementById("professionalForm"),s=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(w,$)=>{const I=$+1,E=I==s[1]?"selected":"",k=new Date(0,$).toLocaleString("pt-BR",{month:"long"});return`<option value="${I}" ${E}>${k.charAt(0).toUpperCase()+k.slice(1)}</option>`}).join(""),o=e.status||"active",n=f(e.name||""),i=f(e.specialty||""),l=f(e.phone||""),d=f(e.notes||"");a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${d}</textarea></div>`;const c=document.getElementById("profPhotoInput"),u=document.getElementById("profPhotoButton"),m=document.getElementById("profPhotoPreview"),b=document.getElementById("profPhotoBase64"),v=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,h=e.photo||"";u&&u.addEventListener("click",()=>c.click()),c&&(c.onchange=async()=>{const w=c.files[0];if(w){m.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const $=await Jn(w,800,800,"image/jpeg",.8);m.src=$,b.value=$}catch($){g("Erro de Imagem",$.message||"Não foi possível processar a imagem.","error"),m.src=v,b.value=h,c.value=""}}})}function Jn(e,t=800,a=800,s="image/jpeg",r=.8){return new Promise((o,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=l=>{const d=new Image;d.onload=()=>{let c=d.width,u=d.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(d,0,0,c,u);const v=m.toDataURL(s,r);o(v)},d.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),d.src=l.target.result},i.onerror=l=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function _n(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',Gn(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function Wn(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
        ...
        </div>`;const s=document.getElementById("batchBlockageForm");s&&s.addEventListener("submit",async o=>{o.preventDefault();const n=Array.from(o.target.querySelectorAll('input[name="batch-professionals"]:checked')).map(b=>b.value);if(n.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const i=o.target.batchBlockageStartDate.value,l=o.target.batchBlockageEndDate.value||i,d=o.target.batchBlockageStartTime.value,c=o.target.batchBlockageEndTime.value,u=o.target.batchBlockageReason.value;if(!i||!d||!c)return g("Atenção","Preencha Data de Início, Início e Fim.","error");const m=n.map(b=>{const v={professionalId:b,establishmentId:p.establishmentId,startTime:new Date(`${i}T${d}`).toISOString(),endTime:new Date(`${l}T${c}`).toISOString(),reason:u};return At(v)});try{await Promise.all(m),g("Sucesso!",`${n.length} bloqueios foram criados.`);const b=document.getElementById("prof-blockages-filter").value;We(e.id,b)}catch(b){g("Erro",b.message,"error")}}),document.getElementById("prof-blockages-filter").addEventListener("change",o=>We(e.id,o.target.value)),await We(e.id,"future")}function Gn(e,t){e.innerHTML=Object.keys(Ka).map(a=>{const s=t[a]||{},r=s.active!==!1;return`
            <div class="day-schedule-card p-3 rounded-lg ${r?"bg-white":"bg-gray-100 disabled"} border">
                 <div class="flex justify-between items-center"><span class="font-semibold text-gray-800">${Ka[a]}</span><label class="flex items-center cursor-pointer"><div class="relative"><input type="checkbox" data-day="${a}" data-field="active" class="sr-only" ${r?"checked":""}><div class="toggle-bg block bg-gray-300 w-10 h-6 rounded-full"></div></div></label></div>
        ...
        `).join("")}catch(s){a.innerHTML=`<p class="text-red-500">${s.message}</p>`}}}function Yn(e){const t=document.getElementById("genericModal");Ee&&t.removeEventListener("click",Ee),Ee=async a=>{const s=a.target.closest("button[data-action]");if(!s){const o=a.target.closest(".tab-link");o&&(t.querySelectorAll(".tab-link").forEach(n=>n.classList.remove("active")),o.classList.add("active"),t.querySelectorAll(".tab-content").forEach(n=>n.classList.add("hidden")),document.getElementById(o.dataset.tab).classList.remove("hidden"));return}const r=s.dataset.action;switch(a.stopPropagation(),r){case"close-modal":Wt();break;case"delete-professional":const o=s.dataset.id;if(await O("Excluir Profissional",`Tem certeza que deseja excluir ${e.name}? Esta ação não pode ser desfeita.`))try{await Ps(o),g("Sucesso!","Profissional excluído.","success"),Wt(),Et()}catch(h){g("Erro",`Não foi possível excluir: ${h.message}`,"error")}break;case"save-professional":const i=document.getElementById("professionalForm"),l=s,d=document.getElementById("profScheduleContainer"),c=Array.from(i.querySelectorAll("#profServicesContainer input:checked")).map(h=>h.value),u={};d&&d.querySelectorAll(".day-schedule-card").forEach(h=>{const w=h.querySelector('[data-field="active"]').dataset.day;u[w]={active:h.querySelector('[data-field="active"]').checked,start:h.querySelector('[data-field="start"]').value,end:h.querySelector('[data-field="end"]').value,breakStart:h.querySelector('[data-field="breakStart"]').value,breakEnd:h.querySelector('[data-field="breakEnd"]').value}});const m={...e,id:i.querySelector("#professionalId").value||void 0,name:i.querySelector("#profName").value,specialty:i.querySelector("#profSpecialty").value,photo:i.querySelector("#profPhotoBase64").value,services:c,workingHours:u,phone:i.querySelector("#profPhone").value,dob:`${i.querySelector("#profDobDay").value}/${i.querySelector("#profDobMonth").value}`,receivesCommission:i.querySelector("#profCommission").value==="sim",showOnAgenda:i.querySelector("#profShowOnAgenda").value==="sim",orderOnAgenda:parseInt(i.querySelector("#profOrderOnAgenda").value)||1,notes:i.querySelector("#profNotes").value,status:i.querySelector("#profStatus").value,establishmentId:p.establishmentId};l.disabled=!0,l.textContent="A salvar...";try{m.id?(await Ds(m.id,m),g("Sucesso!","Profissional atualizado.","success")):(delete m.id,await Bs(m),g("Sucesso!","Profissional criado.","success")),Wt(),Et()}catch(h){g("Erro",h.message,"error"),l.disabled=!1,l.textContent="Salvar"}break;case"delete-blockage":const b=s.dataset.id;if(await O("Apagar Bloqueio","Tem certeza?"))try{await ka(b),g("Bloqueio removido.","success");const h=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";We(e.id,h)}catch(h){g("Erro",h.message,"error")}break;case"batch-delete-blockage":const v=JSON.parse(s.dataset.ids);if(await O("Apagar em Lote",`Tem certeza que deseja apagar ${v.length} bloqueios com este motivo?`))try{await Eo(v),g("Bloqueios removidos.","success");const h=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";We(e.id,h)}catch(h){g("Erro",h.message,"error")}break}},t.addEventListener("click",Ee)}function ca(){const e=document.getElementById("batch-actions-container"),t=document.getElementById("selected-count");!e||!t||(ne.size>0?(t.textContent=`${ne.size} selecionado(s)`,e.classList.remove("hidden")):e.classList.add("hidden"))}function Qn(){O("Excluir em Lote",`Tem certeza que deseja excluir ${ne.size} profissionais? Esta ação não pode ser desfeita.`).then(async e=>{if(e)try{await(void 0)(Array.from(ne)),g("Sucesso!",`${ne.size} profissionais foram excluídos.`,"success"),ne.clear(),ca(),Et()}catch(t){g("Erro",`Não foi possível excluir em lote: ${t.message}`,"error")}})}function je(){const e=document.getElementById("professionalsList");if(!e)return;if(!p.professionals){e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=On();return}const t=document.getElementById("showInactiveProfToggle").checked,a=document.getElementById("profSearchInput").value.toLowerCase(),s=p.professionals.filter(r=>{const o=r.name.toLowerCase().includes(a),n=t||r.status!=="inactive";return o&&n});e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=zn(s)}async function Et(){ne.clear(),_t.innerHTML=`
        ...
        `}).join(""),a.querySelectorAll(".tab-btn").forEach(s=>{s.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),pa(s.dataset.tab)})}))}async function pa(e){si(e);const t=document.getElementById("client-detail-content");if(t){t.innerHTML='<div class="flex h-64 items-center justify-center"><div class="loader"></div></div>';try{switch(e){case"cadastro":t.innerHTML=ri(U);break;case"agendamentos":case"historico":const a=await za(p.establishmentId,U.name,U.phone);t.innerHTML=ni(a,e);break;case"fidelidade":let s=[],r=[];try{s=await za(p.establishmentId,U.name,U.phone)}catch(o){console.error("Erro ao buscar histórico de vendas:",o)}if(ee&&ee.enabled)try{r=await er(p.establishmentId,U.name,U.phone)}catch(o){console.warn("Aviso: Histórico de fidelidade indisponível.",o)}t.innerHTML=li(U,s,r);break;default:t.innerHTML='<p class="p-6 text-center text-gray-500">Seção não implementada.</p>'}}catch(a){console.error("Erro crítico ao carregar aba:",a),t.innerHTML=`<div class="p-6 text-center text-red-500"><p>Erro ao carregar dados.</p><p class="text-xs mt-2">${f(a.message)}</p></div>`}}}function ri(e){const t=e?.dob?e.dob.split("/"):["",""],a=f(e?.name||""),s=f(e?.email||""),r=f(e?.phone||""),o=f(e?.notes||"");return`
    ...
    `}function ni(e,t){const a=t==="agendamentos"?"Próximos Agendamentos":"Histórico de Visitas",s=t==="agendamentos"?"Nenhum agendamento futuro.":"Nenhum histórico de visitas.",r=new Date;r.setHours(0,0,0,0);const o=t==="agendamentos",n=(e||[]).filter(i=>{const l=new Date(i.date),d=new Date(l);d.setHours(0,0,0,0);const c=d.getTime()===r.getTime(),u=d>r,m=d<r,b=["completed","finalized","finished","paid"].includes((i.status||"").toLowerCase());return o?!b&&(u||c):m||b});return n.sort((i,l)=>o?new Date(i.date).getTime()-new Date(l.date).getTime():new Date(l.date).getTime()-new Date(i.date).getTime()),n.length===0?`<div class="p-6 text-center text-gray-500">${s}</div>`:`
        <div class="p-4 space-y-3">
            <h4 class="font-semibold text-lg mb-2 pl-2">${a}</h4>
            ${n.map(i=>{const l=t==="historico",d=Pa(i),c=f(i.serviceName||"Serviço"),u=f(i.professionalName||"");return`
    ...
    `}function ii(e,t){const a=(e||[]).filter(l=>{const d=(l.status||"").toLowerCase();return d==="completed"||d==="finished"||d==="paid"||d==="finalized"});let s=0;a.forEach(l=>{s+=Pa(l)});const r=ee&&ee.conversionRate?parseFloat(ee.conversionRate):Ho,o=Math.floor(s*r);let n=0;(t||[]).forEach(l=>{l.type==="redeem"&&(n+=l.points||0)});const i=o-n;return{totalSpent:s,totalPointsEarned:o,totalPointsRedeemed:n,currentBalance:i}}function li(e,t,a){if(!ee||!ee.enabled)return`<div class="p-8 text-center text-gray-500">
        ...
        </div>`;const s=ii(t,a);e.loyaltyPoints=s.currentBalance,e.totalSpent=s.totalSpent;const r=(ee.tiers||[]).map(l=>{const d=s.currentBalance>=l.points,c=f(l.reward);return`
            <div class="flex justify-between items-center p-3 rounded-lg border ${d?"bg-green-50 border-green-200":"bg-gray-50 border-gray-200"}">
                <div>
                    <p class="font-bold text-sm ${d?"text-green-800":"text-gray-700"}">${c}</p>
                    <p class="text-xs ${d?"text-green-600":"text-gray-500"} font-medium">${l.points} pts</p>
                </div>
                <button data-action="redeem-reward" data-points="${l.points}" data-reward="${c}" ${d?"":"disabled"}
            ...
            </div>`}).join("")||'<p class="text-sm text-gray-400 italic">Nenhum prêmio configurado.</p>',o=[],n=ee.conversionRate||Ho;(t||[]).forEach(l=>{const d=(l.status||"").toLowerCase();if(d==="completed"||d==="finished"||d==="paid"||d==="finalized"){const c=Pa(l),u=Math.floor(c*n);u>0&&o.push({type:"earn",desc:`Serviço/Compra (R$ ${c.toFixed(2)})`,points:u,date:l.date||l.createdAt})}}),(a||[]).forEach(l=>{o.push({type:"redeem",desc:`Resgate: ${l.reward||"Prêmio"}`,points:l.points,date:l.timestamp||l.date})}),o.sort((l,d)=>new Date(d.date)-new Date(l.date));const i=o.length>0?o.map(l=>`
        <div class="flex justify-between items-center py-2 border-b last:border-0 border-gray-100">
            <div>
                <p class="text-xs font-semibold text-gray-700">${f(l.desc)}</p>
    ...
    `}function Oo(e){U=e,ua="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",s=`
    ...
    `,r=window.innerWidth<768,o=r?"max-w-full":"max-w-3xl";if(z({title:a,contentHTML:s,maxWidth:o}),r){const d=document.getElementById("genericModal");if(d){const c=d.querySelector(`.${o.replace(":","\\:")}`);c&&(c.style.height="auto",c.style.maxHeight="85vh",c.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&(n.onclick=async d=>{const c=d.target.closest("[data-action]");if(!c)return;switch(c.dataset.action){case"redeem-reward":{const m=parseInt(c.dataset.points,10),b=c.dataset.reward;if(await O("Confirmar Resgate",`Deseja resgatar "${b}" por ${m} pontos?`))try{await tr(p.establishmentId,U.name,U.phone,{points:m,reward:b}),g("Prêmio resgatado com sucesso!","success"),await pa("fidelidade")}catch(h){g(`Erro ao resgatar: ${h.message}`,"error")}break}case"open-comanda-from-history":{const m=c.dataset.appointmentId;m&&(document.getElementById("genericModal").style.display="none",Z("comandas-section",{selectedAppointmentId:m,initialFilter:"finalizada"}));break}case"view-appointment":{const m=c.dataset.appointmentId,b=c.dataset.appointmentDate;m&&b&&(document.getElementById("genericModal").style.display="none",Z("agenda-section",{targetDate:b,scrollToAppointmentId:m}));break}case"save-client":{d.preventDefault(),ao();break}}}),pa("cadastro"),setTimeout(()=>{const d=document.getElementById("client-form");d&&d.addEventListener("submit",c=>{c.preventDefault(),ao()})},500);const i=document.getElementById("cancelDetailViewBtn");i&&i.addEventListener("click",d=>{d.preventDefault(),document.getElementById("genericModal").style.display="none",zt()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await di()})}async function ao(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await Zs(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await Sa(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await zt()}catch(s){g("Erro",`Não foi possível salvar: ${s.message}`,"error")}}async function di(){if(!U||!U.id)return;if(await O("Excluir Cliente",`Tem certeza que deseja excluir ${U.name}? Esta ação é irreversível.`))try{await Ks(U.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await zt()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function wt(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",a.className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 content-start p-2 pb-20",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const s=Y==="inactive",r=Y==="birthdays";e.forEach(o=>{const n=document.createElement("div");n.className="bg-white rounded-lg border border-gray-200 shadow-sm p-3 hover:shadow-md cursor-pointer transition-all flex flex-col justify-between h-full relative group",n.dataset.clientId=o.id;const i=o.loyaltyPoints||0,l=o.totalSpent||0;let d="";const u=`https://wa.me/55${o.phone?o.phone.replace(/\D/g,""):""}?text=`,m=f(o.name),b=f(o.phone);if(s){const v=encodeURIComponent(Zn(o.name,ma));d=`<a href="${u+v}" target="_blank" class="absolute top-2 right-2 text-blue-500 bg-blue-50 p-1 rounded-full z-10 hover:bg-blue-100"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM16.64 16.64C16.64 16.64 15.11 17.58 14.54 17.76C13.97 17.94 13.06 18.06 10.66 17.06C8.26 16.06 6.38 13.62 6.38 13.62C6.38 13.62 4.96 11.72 4.96 9.76C4.96 7.8 6.04 6.88 6.04 6.88C6.04 6.88 6.32 6.56 6.6 6.56C6.88 6.56 7.16 6.56 7.16 6.56C7.38 6.56 7.62 6.46 7.86 7.02C8.1 7.58 8.68 9.02 8.68 9.02C8.68 9.02 8.78 9.24 8.64 9.48C8.5 9.72 8.36 9.88 8.16 10.1C7.96 10.32 7.74 10.4 8.02 10.88C8.3 11.36 9.26 12.92 10.68 14.18C11.62 15.02 12.56 15.36 12.94 15.54C13.32 15.72 13.6 15.66 13.84 15.38C14.08 15.1 14.62 14.34 14.62 14.34C14.62 14.34 14.88 14.06 15.18 14.12C15.48 14.18 16.94 14.9 16.94 14.9C16.94 14.9 17.2 15.04 17.3 15.22C17.4 15.4 17.4 16.28 16.64 16.64Z"/></svg></a>`}else if(r&&ti(o)){const v=encodeURIComponent(Xn(o.name,ma));d=`<a href="${u+v}" target="_blank" class="absolute top-2 right-2 text-green-500 bg-green-50 p-1 rounded-full z-10 hover:bg-green-100"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M... (mesmo icone)"/></svg></a>`}n.innerHTML=`
                ${d}
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-[10px] text-gray-500 truncate">${b}</p>
                    </div>
                </div>
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const s=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!s||!r){g("Erro","Preencha todos os campos.","error");return}const o=t.querySelector('button[form="change-email-form"]');o.disabled=!0,o.textContent="A verificar...";try{const n=G.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=es.credential(n.email,r);await ts(n,i),o.textContent="A enviar link...",await as(n,s),o.textContent="A atualizar BD...",await Ls(p.establishmentId,s),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{o.disabled=!1,o.textContent="Salvar Novo E-mail"}})}function Di(e,t){const a=f(e.welcomeMessage||"");t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;const a=e.loyaltyProgram||{};t.querySelector("#loyaltyEnabled").checked=a.enabled||!1,t.querySelector("#loyaltyPointsPerCurrency").value=a.pointsPerCurrency||10;const s=t.querySelector("#loyaltyTiersContainer"),r=(o={})=>{const n=document.createElement("div"),i=f(o.reward||"");return n.className="loyalty-tier-row",n.innerHTML=`
            <div>
    ...
    `;try{const[a,s]=await Promise.all([et(p.establishmentId),tt(p.establishmentId)]),r=e.financialIntegration||{},o=e.commissionConfig||{};t.querySelector("#financialNatureId").innerHTML=ut(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=ut(s,r.defaultCentroDeCustoId),t.querySelector("#commissionNatureId").innerHTML=ut(a,o.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=ut(s,o.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const s={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};qe(s,a)})}function qi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <h3 class="text-xl font-bold text-gray-800">${e}</h3>
            <p class="mt-4 text-gray-500">Esta secção ainda não foi implementada.</p>
        </div>
    `}function Go(e="indigo",t){const a=t.querySelector("#color-palette-container"),s=t.querySelector("#establishmentThemeColor");!a||!s||(a.innerHTML="",Object.entries(Ci).forEach(([r,o])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
    ...
    `,ve.querySelector('button[data-action="back-to-list"]').addEventListener("click",s=>{s.preventDefault(),Yo()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":Li(W,a);break;case"change-password":Ti(W,a);break;case"change-email":Bi(W,a);break;case"branding":Di(W,a);break;case"booking":Pi(W,a);break;case"working-hours":Mi(W,a);break;case"loyalty":Ai(W,a);break;case"financial":await Ni(W,a);break;default:qi(t?t.label:"Definições",a)}}async function Yo(){if(ve.innerHTML=`
    ...
    `,!W)try{W=await at(p.establishmentId)}catch{g("Erro Fatal","Não foi possível carregar os dados do estabelecimento.","error"),ve.innerHTML='<p class="text-red-500">Erro ao carregar dados.</p>';return}const e=G.currentUser;e&&e.displayName&&(p.userName=e.displayName);const t=f(p.userName||G.currentUser.email),a=t?t.charAt(0).toUpperCase():"U";ve.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd" /></svg>
                Definições
        ...
        <div data-action="go-to-my-profile" class="bg-white p-4 rounded-lg shadow-md mb-6 cursor-pointer hover:bg-gray-50 transition-all">
            <div class="text-center relative">
                ...
                <div class="absolute top-0 right-0 p-2 text-gray-400 hover:text-indigo-600" title="Ver Meu Perfil">
                 ...
                 ${p.userName&&p.userName!==G.currentUser.email?`<p class="text-sm text-gray-500">${f(G.currentUser.email)}</p>`:""}
    ...
    `,ve.querySelector("#settings-menu-list").addEventListener("click",r=>{const o=r.target.closest("button[data-section]");o&&(r.preventDefault(),Ri(o.dataset.section))});const s=ve.querySelector('[data-action="go-to-my-profile"]');s&&s.addEventListener("click",r=>{r.preventDefault(),Z("my-profile-section")})}const Ge=document.getElementById("content");async function Te(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,s=document.getElementById("filterEndDate")?.value,r=await Mt(p.establishmentId,a||new Date().toISOString().split("T")[0],s||new Date().toISOString().split("T")[0],e),o=document.getElementById("filterReason")?.value.toLowerCase(),n=o?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(o)):r,i=n.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let m=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <h4 class="font-bold text-gray-700">${f(l)} (${d.length})</h4>`;if(d.length>1){const b=JSON.stringify(d.map(v=>v.id));m+=`<button data-action="batch-delete-blockage" data-ids='${b}' class="text-xs text-red-600 hover:text-red-800 font-semibold flex items-center gap-1">
                                    Apagar Lote
                                </button>`}m+="</div>",c.innerHTML=m,d.forEach(b=>{const v=new Date(b.startTime),h=new Date(b.endTime),w=v.toLocaleDateString("pt-BR"),$=h.toLocaleDateString("pt-BR"),E=`
                    <div class="bg-white p-3 rounded-md flex items-center justify-between shadow-sm">
                        <div>
                            <p class="font-medium text-gray-800 text-sm">
                                ${w===$?`${w} | ${v.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${h.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`:`De ${w} às ${v.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}<br>Até ${$} às ${h.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`}
                            </p>
                        </div>
                    ...
                    </div>`;c.innerHTML+=E}),t.appendChild(c)})}catch(a){t.innerHTML=`<p class="text-center text-red-500">Erro: ${a.message}</p>`}}}async function ji(e){e.preventDefault();const t=e.target,a=t.querySelector("#blockageProfId").value,s=t.querySelector("#blockageDate").value,r=t.querySelector("#blockageEndDate").value||s,o=t.querySelector("#blockageStartTime").value,n=t.querySelector("#blockageEndTime").value,i={establishmentId:p.establishmentId,professionalId:a,startTime:new Date(`${s}T${o}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:t.querySelector("#blockageReason").value};try{await At(i),t.reset(),g("Sucesso","Bloqueio adicionado com sucesso!","success"),Te(a)}catch(l){g("Erro",`Não foi possível criar o bloqueio: ${l.message}`,"error")}}async function Hi(e){e.preventDefault();const t=e.target,a=Array.from(t.querySelectorAll('input[name="batch-professionals"]:checked')).map(c=>c.value);if(a.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const s=t.querySelector("#batchBlockageDate").value,r=t.querySelector("#batchBlockageEndDate").value||s,o=t.querySelector("#batchBlockageStartTime").value,n=t.querySelector("#batchBlockageEndTime").value,i=t.querySelector("#batchBlockageReason").value,l=t.querySelector('button[type="submit"]');l.disabled=!0,l.textContent="Aguarde...";const d=a.map(c=>{const u={establishmentId:p.establishmentId,professionalId:c,startTime:new Date(`${s}T${o}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:i};return At(u)});try{await Promise.all(d),g("Sucesso",`${a.length} bloqueios foram criados com sucesso!`,"success"),t.reset(),t.querySelectorAll('input[name="batch-professionals"]:checked').forEach(u=>u.checked=!1);const c=document.getElementById("blockageProfId").value;c&&Te(c)}catch(c){g("Erro",`Ocorreu um erro: ${c.message}`,"error")}finally{l.disabled=!1,l.textContent="Adicionar Bloqueio em Lote"}}function Oi(e){Ge.addEventListener("submit",t=>{t.target.id==="blockageForm"&&ji(t),t.target.id==="batchBlockageForm"&&Hi(t)}),Ge.addEventListener("input",t=>{t.target.matches("#filterStartDate, #filterEndDate, #filterReason")&&Te(e)}),Ge.addEventListener("click",async t=>{const a=t.target.closest("button[data-action]");if(!a)return;const s=a.dataset.action;if(s==="back-to-professionals")Z("profissionais-section");else if(s==="delete-blockage"){if(await O("Apagar Bloqueio","Tem a certeza que deseja apagar este bloqueio?"))try{await ka(a.dataset.id),g("Sucesso","Bloqueio removido.","success"),Te(e)}catch(o){g("Erro",`Não foi possível remover o bloqueio: ${o.message}`,"error")}}else if(s==="batch-delete-blockage"){const r=JSON.parse(a.dataset.ids);if(await O("Apagar Lote de Bloqueios",`Tem certeza que deseja apagar ${r.length} bloqueios de uma vez?`))try{await Eo(r),g("Sucesso",`${r.length} bloqueios removidos.`,"success"),Te(e)}catch(n){g("Erro",`Não foi possível apagar os bloqueios: ${n.message}`,"error")}}})}async function zi(e){const{professionalId:t,professionalName:a}=e;if(!t||!a){Ge.innerHTML='<p class="text-red-500 p-8">Erro: ID do profissional não fornecido.</p>';return}const s=f(a);Ge.innerHTML=`
        <section>
        ...
        </section>`,Oi(t),await Te(t);const r=document.getElementById("batchProfSelectionContainer");try{const o=await Q(p.establishmentId);r.innerHTML=o.map(n=>`
            <div class="flex items-center">
    ...
    `}).join("")}function St(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=p.users:t=p.users.filter(a=>a.status==="active"),Xi(t)}function Zi(e={}){return Object.entries(Yi).map(([t,a])=>{const s=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,o=Object.entries(Qi).map(([i,l])=>`
    ...
    `}).join("")}async function so(e=null){document.getElementById("user-list-view").classList.add("hidden");const t=document.getElementById("user-form-view");t.classList.remove("hidden");let a=p.professionals;if(!a||a.length===0)try{a=await Q(p.establishmentId),p.professionals=a}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error")}const s=$=>a.find(I=>I.id===$),r=($,I)=>{const k=s($)?.photo,C=I.charAt(0).toUpperCase();return{photoSrc:k||`https://placehold.co/128x128/E2E8F0/4A5568?text=${C}`,initials:C,photoUrl:k||""}},o=e?.professionalId,n=e?.name||"Novo Usuário",i=r(o,n),l=s(o),d=$=>{let I='<option value="">-- Não Associado a um Profissional --</option>';return I+=a.map(E=>`<option value="${E.id}" ${E.id===$?"selected":""}>${E.name} (${E.specialty||"N/A"})</option>`).join(""),I},c=e!==null;t.querySelector("#userFormTitle").textContent=c?`Editar Usuário: ${e.name}`:"Novo Usuário";const u=t.querySelector("#userForm");u.innerHTML=`
    ...
    `;const m=window.innerWidth<768,b=u.querySelector(".bg-white");if(m&&b){b.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const $=u.closest("section");$&&($.style.padding="0",$.style.margin="0"),b.classList.add("p-4")}const v=u.querySelector("#userProfessionalId"),h=u.querySelector("#userPhotoPreview"),w=u.querySelector("#profPhotoName");if(v.addEventListener("change",$=>{const I=$.target.value,E=s(I),k=E?E.name:"Selecione um profissional",C=r(I,n);h.src=C.photoSrc,w.textContent=k,u.querySelector("#professionalPhotoUrl").value=C.photoUrl}),u.addEventListener("submit",async $=>{$.preventDefault();const I=e?.email,E=u.querySelector("#userEmail").value,k={};u.querySelectorAll('input[type="checkbox"]').forEach(j=>{const L=j.dataset.module,T=j.dataset.permission;k[L]||(k[L]={}),k[L][T]=j.checked});const C=u.querySelector("#userProfessionalId").value||null,F={name:u.querySelector("#userName").value,permissions:k,professionalId:C,establishmentId:p.establishmentId};try{c?(I!==E&&(F.email=E),await Ji(e.id,F),g("Usuário atualizado com sucesso!","success")):(F.email=u.querySelector("#userEmail").value,F.password=u.querySelector("#userPassword").value,await Ui(F),g("Usuário criado com sucesso!","success")),It()}catch(j){g(`Erro: ${j.message}`,"error")}}),c){const $=u.querySelector("#password-change-container"),I=$.querySelector('[data-action="show-password-form"]'),E=$.querySelector("#password-form"),k=E.querySelector('[data-action="save-password"]'),C=E.querySelector('[data-action="cancel-password-change"]');I.addEventListener("click",()=>{I.classList.add("hidden"),E.classList.remove("hidden")}),C.addEventListener("click",()=>{I.classList.remove("hidden"),E.classList.add("hidden"),E.querySelector("#userNewPassword").value=""}),k.addEventListener("click",async()=>{const F=E.querySelector("#userNewPassword").value;if(!F||F.length<6){g("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await O("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{k.disabled=!0,k.textContent="Aguarde...",await Wi(e.id,F),g("Sucesso!","A senha do usuário foi alterada.","success"),I.classList.remove("hidden"),E.classList.add("hidden"),E.querySelector("#userNewPassword").value=""}catch(L){g("Erro",`Não foi possível alterar a senha: ${L.message}`,"error")}finally{k.disabled=!1,k.textContent="Salvar Nova Senha"}})}}async function Ki(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,a]=await Promise.all([Vi(p.establishmentId),Q(p.establishmentId)]);p.users=t,p.professionals=a,St()}catch{g("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function It(){ge.innerHTML=`
    ...
    `,He&&ge.removeEventListener("click",He),Oe&&ge.removeEventListener("change",Oe),He=async e=>{if(!document.getElementById("user-list-view")){ge.removeEventListener("click",He);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":so();break;case"edit-user":const s=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));so(s);break;case"back-to-list":It();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await O("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await _i(r),g("Usuário excluído com sucesso!","success"),It()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Oe=async e=>{if(!document.getElementById("user-list-view")){ge.removeEventListener("change",Oe);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")St();else if(t){e.stopPropagation();const a=t.dataset.userId,s=t.checked?"active":"inactive";try{await Gi(a,s),g(`Usuário ${s==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=p.users.findIndex(o=>o.id===a);r>-1&&(p.users[r].status=s,St())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,St()}}},ge.addEventListener("click",He),ge.addEventListener("change",Oe),await Ki()}const el=document.getElementById("content");let ro={},ga=null;function tl(){Object.values(ro).forEach(e=>e?.destroy()),ro={}}function al(e,t){if(!window.jspdf){g("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:a}=window.jspdf,s=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(s.setFontSize(18),s.text(e,s.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];s.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const o=s.lastAutoTable?s.lastAutoTable.finalY+20:60;s.text("Detalhes das Vendas",20,o),s.autoTable({html:`#${t}`,startY:o+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),s.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function no(e){const t=document.getElementById("genericModal"),a=f(e.client),s=f(e.items),r=f(e.responsavelCaixa||"N/A"),o=(e.payments||[]).map(n=>`
    ...
    `}).join(""),o.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=ga.transactions[l];d&&no(d)})}),n.innerHTML=a.map((i,l)=>{const d=f(i.client),c=f(i.items),u=f(i.type);return`
    ...
    `}).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const l=i.dataset.transactionIndex,d=ga.transactions[l];d&&no(d)})})}async function io(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const s=t.value,r=a.value;if(!s||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const o=document.getElementById("cashierSessionFilter").value,n=await Js({establishmentId:p.establishmentId,startDate:s,endDate:r,cashierSessionId:o});ga=n,e.innerHTML=`
        ...
        `,ol(n)}catch(o){g("Erro",`Não foi possível carregar o relatório: ${o.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${f(o.message)}</p>`}}async function sl(){tl();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];el.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",io),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const s=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,o=`Relatorio_Vendas_${s}_a_${r}`;al(o,"transactionsTable")});try{const s=await Pr(p.establishmentId),r=document.getElementById("cashierSessionFilter");s&&s.length>0&&s.forEach(o=>{const n=new Date(o.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=f(o.closedByName||"N/A");r.innerHTML+=`<option value="${o.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await io()}const rl=document.getElementById("content");let B={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},Qt=null,mt=null,pt=null;function Ma(e){const t=new Map,a=[];return e&&(e.forEach(s=>t.set(s.id,{...s,children:[]})),t.forEach(s=>{s.parentId&&t.has(s.parentId)?t.get(s.parentId).children.push(s):a.push(s)})),a}function Qo(e,t,a){if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const s=(r,o=0)=>{const n="— ".repeat(o);return`
    ...
    `,e.style.display="flex"}function gt(e,t="all"){let a='<option value="all">Todos</option>';const s=n=>{const i=new Map,l=[];return n&&(n.forEach(d=>i.set(d.id,{...d,children:[]})),i.forEach(d=>{d.parentId&&i.has(d.parentId)?i.get(d.parentId).children.push(d):l.push(d)})),l},r=(n,i=0)=>{const l=i>0?"— ".repeat(i):"",d=n.id===t?"selected":"";a+=`<option value="${n.id}" ${d}>${l}${n.name}</option>`,n.children.forEach(c=>r(c,i+1))};return s(e).forEach(n=>r(n)),a}async function xe(){const e=document.getElementById("financial-content"),t=document.getElementById("filterStartDate")?.value,a=document.getElementById("filterEndDate")?.value,s=document.getElementById("filterNaturezaId")?.value,r=document.getElementById("filterCostCenterId")?.value;if(!t||!a){try{const[i,l]=await Promise.all([et(p.establishmentId),tt(p.establishmentId)]);B={...B,natures:i,costCenters:l},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=gt(B.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=gt(B.costCenters))}catch(i){g("Erro",`Não foi possível carregar os dados base: ${i.message}`,"error")}ba(),po();return}const o=document.getElementById("payables-list"),n=document.getElementById("receivables-list");o&&(o.innerHTML='<div class="loader mx-auto"></div>'),n&&(n.innerHTML='<div class="loader mx-auto"></div>');try{const i={startDate:t,endDate:a,establishmentId:p.establishmentId};s&&s!=="all"&&(i.natureId=s),r&&r!=="all"&&(i.costCenterId=r);const[l,d,c,u]=await Promise.all([fi(i),wi(i),et(p.establishmentId),tt(p.establishmentId)]),m=d.previousBalance-l.previousBalance;B={...B,payables:l.entries,receivables:d.entries,natures:c,costCenters:u,previousBalance:m,filterNaturezaId:s,filterCostCenterId:r},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=gt(B.natures,B.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=gt(B.costCenters,B.filterCostCenterId)),ba(),po()}catch(i){g("Erro",`Não foi possível carregar os dados: ${i.message}`,"error"),e&&(e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function dl(e,t,a=null){e.preventDefault();const s=e.target,r=s.querySelector('[name="status"]').checked,o=s.querySelector('[name="paymentDate"]').value,n=parseFloat(s.querySelector('[name="amount"]').value),i=parseInt(s.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!o){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const l={description:s.querySelector('[name="description"]').value,amount:n,dueDate:s.querySelector('[name="dueDate"]').value,naturezaId:s.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:s.querySelector('[name="centroDeCustoId"]').value||null,notes:s.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?o:null,installments:a?1:i,establishmentId:p.establishmentId};try{a?(await(t==="payable"?vi(a,l):ki(a,l)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?bi(l):yi(l)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await xe()}catch(d){g("Erro",`Não foi possível salvar: ${d.message}`,"error")}}async function cl(e,t){if(await O("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?hi(t):Si(t)),g("Sucesso","Lançamento excluído!","success"),await xe()}catch(s){g("Erro",`Falha ao excluir: ${s.message}`,"error")}}async function ul(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?xi(t,a):$i(t,a)),g("Sucesso","Lançamento atualizado!","success"),await xe()}catch(s){g("Erro",`Falha ao atualizar status: ${s.message}`,"error")}}function mo(e){const t=B.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function ba(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(B.natures.map(i=>[i.id,i.name])),s=new Map(B.costCenters.map(i=>[i.id,i.name])),r=mo(B.payables),o=mo(B.receivables),n=(i,l)=>{const d=i.status==="paid",c=JSON.stringify(i).replace(/'/g,"&apos;"),u=i.naturezaId?a.get(i.naturezaId):"N/A",m=i.centroDeCustoId?s.get(i.centroDeCustoId):"N/A";let b=l==="payable"?"text-red-600":"text-green-600";const v=d?"bg-gray-200 text-gray-600":l==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",h=d?"Finalizado":"Pendente";return d&&(b="text-gray-500"),`
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2 text-right">
                    ...
                    <div class="flex flex-col items-center gap-1">
                        <span class="text-xs font-semibold px-2 py-1 rounded-full ${v}">${h}</span>
                        <div class="flex">
        ...
        </div>`};e.innerHTML=r.map(i=>n(i,"payable")).join("")||'<p class="text-center text-gray-500 py-4">Nenhuma conta a pagar.</p>',t.innerHTML=o.map(i=>n(i,"receivable")).join("")||'<p class="text-center text-gray-500 py-4">Nenhuma conta a receber.</p>'}function po(){const e=B.payables.filter(r=>r.status==="pending").reduce((r,o)=>r+o.amount,0),t=B.receivables.filter(r=>r.status==="pending").reduce((r,o)=>r+o.amount,0),a=t-e;document.getElementById("summary-pending-receivables").textContent=`R$ ${t.toFixed(2)}`,document.getElementById("summary-pending-payables").textContent=`R$ ${e.toFixed(2)}`,document.getElementById("summary-pending-balance").textContent=`R$ ${a.toFixed(2)}`;const s=document.getElementById("summary-pending-balance");s&&(s.className=`text-2xl font-bold ${a>=0?"text-green-600":"text-red-600"}`)}function Xt(e,t=null){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const a=document.getElementById("genericModal"),s=`${t?"Editar":"Nova"} ${e==="payable"?"Despesa":"Receita"}`,r=e==="payable"?"bg-red-600 hover:bg-red-700":"bg-green-600 hover:bg-green-700",o=m=>{let b='<option value="">-- Selecione (Opcional) --</option>';const v=Ma(m),h=(w,$="",I=0)=>{const E=I>0?"— ".repeat(I):"";b+=`<option value="${w.id}">${E}${w.name}</option>`,w.children.forEach(k=>h(k,$+"— "))};return v.forEach(w=>h(w)),b},n=o(B.natures),i=o(B.costCenters),l=t?"":`
        <div>
    ...
    `;const r=document.getElementById("main-fab-btn"),o=document.getElementById("fab-menu");if(r&&o){r.addEventListener("click",h=>{h.stopPropagation(),o.classList.toggle("hidden"),r.classList.toggle("rotate-45")});const m=o.querySelector('button[data-action="open-modal"][data-type="receivable"]'),b=o.querySelector('button[data-action="open-modal"][data-type="payable"]'),v=o.querySelector('button[data-action="open-cash-flow-modal"]');m&&m.addEventListener("click",h=>{h.stopPropagation(),Xt("receivable")}),b&&b.addEventListener("click",h=>{h.stopPropagation(),Xt("payable")}),v&&v.addEventListener("click",h=>{h.stopPropagation(),uo()})}mt&&document.body.removeEventListener("click",mt),pt&&document.getElementById("genericModal").removeEventListener("click",pt);const n=()=>{const m=document.getElementById("filterStartDate"),b=document.getElementById("filterEndDate"),v=document.getElementById("filterNaturezaId"),h=document.getElementById("filterCostCenterId");B.startDate=m.value,B.endDate=b.value,B.filterNaturezaId=v.value,B.filterCostCenterId=h.value;const w=document.getElementById("advanced-filters");w&&w.classList.contains("hidden")===!1&&window.innerWidth<768&&w.classList.add("hidden"),xe()},i=m=>{const b=m.target.closest("[data-status-filter]");if(!b)return;const v=b.dataset.statusFilter;B.currentFilter=v,document.querySelectorAll("[data-status-filter]").forEach(h=>{h.classList.remove("bg-blue-100","text-blue-800"),h.classList.add("bg-gray-100","text-gray-600")}),b.classList.remove("bg-gray-100","text-gray-600"),b.classList.add("bg-blue-100","text-blue-800"),ba()},l=m=>{const b=document.getElementById("payables-container"),v=document.getElementById("receivables-container"),h=document.getElementById("btn-payables-view"),w=document.getElementById("btn-receivables-view");window.innerWidth>=1024&&B.currentListView===m||(m==="payables"?(b.classList.remove("hidden"),v.classList.add("hidden"),h&&(h.classList.remove("bg-gray-200"),h.classList.add("bg-red-100","border","border-red-500")),w&&(w.classList.remove("bg-green-100","border","border-green-500"),w.classList.add("bg-gray-200"))):(b.classList.add("hidden"),v.classList.remove("hidden"),h&&(h.classList.remove("bg-red-100","border","border-red-500"),h.classList.add("bg-gray-200")),w&&(w.classList.remove("bg-gray-200"),w.classList.add("bg-green-100","border","border-green-500"))),B.currentListView=m)};document.getElementById("applyDateFilterBtn").addEventListener("click",n),document.getElementById("filterNaturezaId").addEventListener("change",()=>{B.filterNaturezaId=document.getElementById("filterNaturezaId").value}),document.getElementById("filterCostCenterId").addEventListener("change",()=>{B.filterCostCenterId=document.getElementById("filterCostCenterId").value}),document.querySelectorAll("[data-status-filter]").forEach(m=>{m.addEventListener("click",i)}),mt=m=>{const b=m.target.closest("button[data-action]");if(!b)return;const{action:v,type:h,id:w}=b.dataset;v==="edit"?Xt(h,JSON.parse(b.dataset.item.replace(/&apos;/g,"'"))):v==="delete"?cl(h,w):v==="mark-as-paid"?ul(h,w):v==="manage-natures"?lo("nature"):v==="manage-cost-centers"?lo("cost-center"):v==="open-cash-flow-modal"?uo():v==="toggle-filters"?document.getElementById("advanced-filters")?.classList.toggle("hidden"):v==="open-indicators-modal"?il():v==="open-settings-modal"?ll():v==="toggle-list-view"&&l(b.dataset.list)},pt=m=>{const b=m.target.closest('button[data-action^="delete-"]');if(b){const v=b.dataset.action.split("-")[1];d(v,b.dataset.id)}},document.body.addEventListener("click",mt),document.getElementById("genericModal").addEventListener("click",pt);async function d(m,b){const v=m==="nature",h=v?mi:gi,w=v?et:tt,$=v?"natures":"costCenters",I=document.getElementById("hierarchyList");if(await O("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await h(b);const k=await w(p.establishmentId);B[$]=k,Qo(I,Ma(k),m),await xe()}catch(k){g("Erro",`Não foi possível apagar: ${k.message}`,"error")}}const c=()=>{const m=window.innerWidth<1024,b=document.getElementById("payables-container"),v=document.getElementById("receivables-container"),h=document.getElementById("list-toggle-buttons");b&&v&&(b.classList.remove("hidden"),v.classList.remove("hidden"),m?(b.classList.remove("lg:col-span-1"),v.classList.remove("lg:col-span-1"),h?.classList.remove("hidden"),l(B.currentListView)):(b.classList.add("lg:col-span-1"),v.classList.add("lg:col-span-1"),h?.classList.add("hidden"),b.classList.remove("hidden"),v.classList.remove("hidden")))};c(),window.addEventListener("resize",c);const u=document.querySelector(`[data-status-filter="${B.currentFilter}"]`);u&&(document.querySelectorAll("[data-status-filter]").forEach(m=>{m.classList.remove("bg-blue-100","text-blue-800"),m.classList.add("bg-gray-100","text-gray-600")}),u.classList.remove("bg-gray-100","text-gray-600"),u.classList.add("bg-blue-100","text-blue-800"));try{const m=await Ii(p.establishmentId),b=document.getElementById("summary-today-payables");b&&(b.textContent=`R$ ${m.totalPayables.toFixed(2)}`);const v=document.getElementById("summary-today-receivables");v&&(v.textContent=`R$ ${m.totalReceivables.toFixed(2)}`)}catch{g("Erro","Não foi possível carregar o resumo do dia.","error")}await xe()}const pl=e=>y("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),gl=e=>y("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),bl=(e,t)=>{const a=new URLSearchParams({startDate:e,endDate:t}).toString();return y(`/api/commissions/stats?${a}`)},fl=(e={})=>{Object.keys(e).forEach(s=>(e[s]===void 0||e[s]===null||e[s]==="")&&delete e[s]);const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return y(a)},vl=e=>y(`/api/commissions/report/${e}`,{method:"DELETE"}),Ct=new Date,go=new Date(Ct.getFullYear(),Ct.getMonth(),1),P={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:go.toISOString().split("T")[0],dashEndDate:Ct.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:go.toISOString().split("T")[0],histEndDate:Ct.toISOString().split("T")[0],histProfessionalId:"all"};let bt=null;const ze=document.getElementById("content");async function hl(){try{P.professionals=await Q(p.establishmentId)}catch(e){console.error("Erro profissionais",e)}Tl(),xl(),Vt(),Tt("dashboard")}function xl(){bt&&ze.removeEventListener("click",bt),bt=e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.action,s=t.dataset.id,r=t.dataset.idx;switch(a){case"tab-nav":Tt(t.dataset.tab);break;case"toggle-all-profs":yl();break;case"back-to-filters":P.calculationResult=null,Lt(document.getElementById("commissions-content"));break;case"view-preview-items":Ll(r);break;case"save-final-report":kl();break;case"start-new-calc":Tt("calculator");break;case"print-receipt":Sl(s);break;case"delete-report":$l(s);break;case"filter-dashboard":Vt();break;case"filter-history":Aa();break}},ze.addEventListener("click",bt),ze.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;Il(t)}},ze.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),wl())}}async function Vt(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(P.dashStartDate=e.value),t&&(P.dashEndDate=t.value);const a=document.getElementById("dashboard-stats-container");a&&(a.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const s=await bl(P.dashStartDate,P.dashEndDate);P.dashStats={revenue:s.totalRevenue||0,commissions:s.totalCommissionsPaid||0},P.currentTab==="dashboard"&&Xo(document.getElementById("commissions-content"))}catch(s){console.error(s),a&&(a.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function Aa(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),a=document.getElementById("hist-prof");e&&(P.histStartDate=e.value),t&&(P.histEndDate=t.value),a&&(P.histProfessionalId=a.value);const s=document.getElementById("history-list-container");if(s){s.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await fl({startDate:P.histStartDate,endDate:P.histEndDate,professionalId:P.histProfessionalId});P.historyData=r,Zo(s,r)}catch{s.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function yl(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(a=>a.checked);e.forEach(a=>a.checked=!t)}async function wl(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(o=>o.value);if(e.length===0)return g("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},a=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),s=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");P.periodString=`${a} a ${s}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const o=await pl(t);P.calculationResult=o.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),Lt(r)}catch(o){g("Erro",o.message,"error"),P.calculationResult=null,Lt(r)}}async function kl(){const e=P.calculationResult.length;if(await O("Confirmar",`Gerar ${e} relatórios? Isso marcará as vendas como pagas.`))try{const a=P.calculationResult.map(s=>{const r=s.items.map(o=>o.originalSaleId).filter(o=>o!=null);return gl({professionalId:s.professionalId,professionalName:s.professionalName,period:P.periodString,processedSalesIds:r,reportData:{...s,summary:{...s.summary,finalValue:s.finalValue,extraDebit:s.extraDebit||0,extraCredit:s.extraCredit||0,notes:s.notes||""}}})});await Promise.all(a),g("Sucesso","Pagamentos registrados!","success"),P.calculationResult=null,Vt(),Tt("history")}catch(a){g("Erro",a.message,"error")}}function Sl(e){const t=P.historyData.find(a=>a.id===e);t&&El(t)}async function $l(e){if(await O("Excluir","Deseja remover este registro? As vendas voltarão a ficar disponíveis para cálculo."))try{await vl(e),g("Sucesso","Registro removido.","success"),Aa(),Vt()}catch(a){g("Erro",a.message,"error")}}function El(e){const{jsPDF:t}=window.jspdf;if(!t)return g("Erro","PDF lib não carregada.","error");const a=new t,s=a.internal.pageSize.getWidth()/2;a.setFontSize(18),a.setFont(void 0,"bold"),a.text("RECIBO DE PAGAMENTO DE COMISSÃO",s,20,{align:"center"}),a.setFontSize(12),a.setFont(void 0,"normal"),a.text(`Profissional: ${e.professionalName}`,15,40),a.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),a.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const o=a.lastAutoTable.finalY+10;a.setFontSize(14),a.setFont(void 0,"bold"),a.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,o,{align:"right"}),a.save(`Recibo_${e.professionalName}.pdf`)}function Il(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),a=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let s=0,r=0;if(t.forEach(o=>{o.value&&(s=parseFloat(o.value))}),a.forEach(o=>{o.value&&(r=parseFloat(o.value))}),P.calculationResult&&P.calculationResult[e]){const o=P.calculationResult[e];o.extraDebit=s,o.extraCredit=r,o.finalValue=o.summary.totalCommission-s+r,t.forEach(i=>{i!==document.activeElement&&(i.value=s||"")}),a.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${o.finalValue.toFixed(2)}`),Cl()}}function Cl(){const e=P.calculationResult.reduce((a,s)=>a+s.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(a=>a.innerText=`R$ ${e.toFixed(2)}`)}function Ll(e){const t=P.calculationResult[e];if(!t)return;const a=t.items.map(s=>`
    ...
    `,P.historyData.length>0?Zo(document.getElementById("history-list-container"),P.historyData):Aa()}function Zo(e,t){if(t.length===0){e.innerHTML=`
    ...
    `}const Zt=document.getElementById("content");let ye={allPackages:[],catalogForModal:{services:[],products:[]}},ft=null,Ce=null;function Dl(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Pl(){const e=document.getElementById("packagesListContainer");if(e){if(ye.allPackages.length===0){e.innerHTML=`
            ...
            </div>`;return}e.innerHTML=ye.allPackages.map(t=>{const a=t.status==="active",s=JSON.stringify(t).replace(/'/g,"&apos;"),r=t.price||0,o=t.originalPrice||0,n=o>r?o-r:0,i=o>0?(o-r)/o*100:0,l=f(t.name),d=f(t.description||"Sem descrição");return`
        ...
        `}).join("")}}function bo(){const e=document.getElementById("genericModal");e.style.display="none",Ce&&e.removeEventListener("click",Ce)}async function fo(e=null){const t=document.getElementById("genericModal"),a=!!e,s=e?JSON.parse(JSON.stringify(e.items||[])):[],r=f(e?.name||""),o=f(e?.description||""),n=e?.price||"",i=e?.commissionRate||0,l=e?.validityDays||30,d=`
    ...
    `;t.innerHTML=d,t.style.display="flex";const c=t.querySelector("#package-items-list"),u=(b,v)=>{const h=v.querySelector("#originalPrice"),w=b.reduce(($,I)=>$+I.price*I.quantity,0);h&&(h.textContent=`R$ ${w.toFixed(2)}`)},m=b=>{b.length===0?c.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':c.innerHTML=b.map((v,h)=>{const w=v.type==="service",$=w?"Serviço":"Produto",I=w?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
                <div class="flex items-center justify-between bg-white p-2 rounded shadow-sm border border-gray-200">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <span class="text-xs font-medium px-2 py-0.5 rounded-full ${I}">${$}</span>
            ...
            `}).join(""),u(b,t)};m(s),c.addEventListener("change",b=>{if(b.target.classList.contains("quantity-input")){const v=parseInt(b.target.dataset.index,10),h=parseInt(b.target.value,10);h>0&&s[v]&&(s[v].quantity=h,m(s))}}),c.addEventListener("click",b=>{if(b.target.classList.contains("remove-item-btn")){const v=parseInt(b.target.dataset.index,10);s.splice(v,1),m(s)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>Ml(b=>{const v=s.find(h=>h.id===b.id&&h.type===b.type);v?v.quantity++:s.push({...b,quantity:1}),m(s)}),Ce&&t.removeEventListener("click",Ce),Ce=async b=>{const v=b.target.closest("button[data-action]");if(!v)return;const h=v.dataset.action;if(b.stopPropagation(),h==="close-modal"&&bo(),h==="save-package"){const w=v,$={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:s,originalPrice:s.reduce((I,E)=>I+E.price*E.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null,establishmentId:p.establishmentId};if(!$.name||!$.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if($.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}w.disabled=!0,w.textContent="A salvar...";try{a?await Nr($.id,$):(delete $.id,await Ar($)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),bo(),await Na()}catch(I){g("Erro",`Não foi possível salvar o pacote: ${I.message}`,"error"),w.disabled=!1,w.textContent="Salvar Pacote"}}},t.addEventListener("click",Ce)}function Ml(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const s={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=l=>{const d=t.toLowerCase(),c=ye.catalogForModal.services.filter(v=>v.name.toLowerCase().includes(d)),u=ye.catalogForModal.products.filter(v=>v.name.toLowerCase().includes(d)),m=c.map(v=>`
    ...
    `,document.body.appendChild(a);const o=a.querySelector("#item-selection-list"),n=a.querySelector("#item-search-input"),i=()=>{a.remove()};r(o),n.addEventListener("input",()=>{t=n.value,r(o)}),a.addEventListener("click",l=>{const d=l.target.closest('[data-action="select-item"]'),c=l.target.closest('[data-action="close-selection-modal"]');if(d){const{itemType:u,itemId:m}=d.dataset,v=(ye.catalogForModal[u+"s"]||[]).find(h=>h.id===m);v&&(e({...v,type:u}),i())}else(c||l.target===a)&&i()})}async function Na(){Zt.innerHTML=`
    ...
    `,ft&&Zt.removeEventListener("click",ft),ft=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const s=e.target.closest('[data-action="delete-package"]');if(s){const r=s.dataset.id;O("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async o=>{if(o)try{await qr(r),g("Sucesso!","Pacote excluído.","success"),await Na()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")fo(null);else if(a==="edit-package"){const s=JSON.parse(t.dataset.package);fo(s)}},Zt.addEventListener("click",ft);try{const[e,t,a]=await Promise.all([Do(p.establishmentId),Ne(p.establishmentId),Ft(p.establishmentId)]);ye.allPackages=e,ye.catalogForModal={services:t.filter(s=>s.active),products:a},Pl()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const Al=document.getElementById("content");let Nl=null;async function ql(){const e=f(p.userName||"Usuário"),t=f(G.currentUser?.email||"E-mail não disponível"),a=p.userName?p.userName.charAt(0):"U";Al.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await Fl()}async function Fl(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=p.userProfessionalId;if(t){const a=await Ts(t);Nl=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo);const s=f(a.name);e.innerHTML=`
            ...
            `,Rl(a.id),document.getElementById("my-blocks-filter").addEventListener("change",o=>Bt(a.id,o.target.value)),Bt(a.id,"future")}else e.innerHTML=`
        ...
        `}}function Rl(e){const t=document.getElementById("block-schedule-form");t.addEventListener("submit",async a=>{a.preventDefault();const s=t.querySelector("#blockDate").value,r=t.querySelector("#blockStartTime").value,o=t.querySelector("#blockEndTime").value,n=t.querySelector("#blockReason").value;if(!s||!r||!o){g("Erro","Por favor, preencha a data e os horários de início e fim.","error");return}if(r>=o){g("Erro","A hora de início deve ser anterior à hora de fim.","error");return}const i=new Date(`${s}T${r}:00`),l=new Date(`${s}T${o}:00`),d=t.querySelector('button[type="submit"]');d.disabled=!0,d.textContent="A bloquear...";try{await At({establishmentId:p.establishmentId,professionalId:e,reason:n||"Bloqueado (Meu Perfil)",startTime:i.toISOString(),endTime:l.toISOString()}),g("Sucesso","Agenda bloqueada com sucesso!","success"),t.reset();const c=document.getElementById("my-blocks-filter").value;Bt(e,c)}catch(c){console.error("Erro ao bloquear agenda:",c),g("Erro",`Não foi possível bloquear a agenda: ${c.message}`,"error")}finally{d.disabled=!1,d.textContent="Bloquear Agenda"}})}async function Bt(e,t="future"){const a=document.getElementById("my-blocks-list");a.innerHTML='<p class="text-gray-500">A carregar bloqueios...</p>';try{const s=new Date;let r,o;t==="history"?(o=new Date,r=new Date,r.setFullYear(r.getFullYear()-1)):(r=new Date,o=new Date,o.setFullYear(o.getFullYear()+1));let i=(await Mt(p.establishmentId,r.toISOString(),o.toISOString(),e)).map(l=>({...l,startTime:new Date(l.startTime),endTime:new Date(l.endTime)}));t==="history"?i=i.filter(l=>l.endTime<s).sort((l,d)=>d.startTime-l.startTime):i=i.filter(l=>l.endTime>=s).sort((l,d)=>l.startTime-d.startTime),i.length>0?(a.innerHTML=i.map(l=>{const d=l.startTime.toLocaleDateString("pt-BR"),c=`${l.startTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${l.endTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`,u=l.endTime<new Date,m=f(l.reason||"Sem motivo");return`
                ...
                `}).join(""),a.querySelectorAll(".remove-block-btn").forEach(l=>{l.addEventListener("click",async d=>{const c=d.currentTarget.dataset.blockId;if(confirm("Tem certeza que deseja remover este bloqueio?"))try{await ka(c),g("Sucesso","Bloqueio removido.","success"),Bt(e,t)}catch(u){console.error("Erro ao remover bloqueio:",u),g("Erro",`Não foi possível remover o bloqueio: ${u.message}`,"error")}})})):a.innerHTML=`<p class="text-gray-500 py-4 text-center">Nenhum bloqueio ${t==="history"?"no histórico recente":"futuro agendado"}.</p>`}catch(s){console.error("Erro ao carregar bloqueios:",s),a.innerHTML=`<p class="text-red-500">Erro ao carregar bloqueios: ${f(s.message)}</p>`}}const vt=document.getElementById("loadingScreen"),Kt=document.getElementById("dashboardContent"),ea=document.getElementById("content"),vo=document.getElementById("notificationBell"),ta=document.getElementById("notificationBadge"),ht=document.getElementById("notificationPanel"),ho=document.getElementById("notificationList"),aa=document.getElementById("profileMenuButton"),K=document.getElementById("profileDropdown"),jl=document.getElementById("profileName"),Hl=document.getElementById("profileEmail"),Ol=document.getElementById("logoutButton"),zl=document.getElementById("cancellationHistoryBtn"),xo=document.getElementById("myProfileLink"),yo={indigo:{main:"#4f46e5",hover:"#4338ca",light:"#e0e7ff",text:"#ffffff"},blue:{main:"#2563eb",hover:"#1d4ed8",light:"#dbeafe",text:"#ffffff"},sky:{main:"#0284c7",hover:"#0369a1",light:"#e0f2fe",text:"#ffffff"},teal:{main:"#0d9488",hover:"#0f766e",light:"#ccfbf1",text:"#ffffff"},emerald:{main:"#059669",hover:"#047857",light:"#d1fae5",text:"#ffffff"},green:{main:"#16a34a",hover:"#15803d",light:"#dcfce7",text:"#ffffff"},lime:{main:"#65a30d",hover:"#4d7c0f",light:"#ecfccb",text:"#ffffff"},amber:{main:"#d97706",hover:"#b45309",light:"#fef3c7",text:"#1f2937"},orange:{main:"#ea580c",hover:"#c2410c",light:"#ffedd5",text:"#ffffff"},red:{main:"#dc2626",hover:"#b91c1c",light:"#fee2e2",text:"#ffffff"},rose:{main:"#e11d48",hover:"#be123c",light:"#ffe4e6",text:"#ffffff"},pink:{main:"#db2777",hover:"#be185d",light:"#fce7f3",text:"#ffffff"},fuchsia:{main:"#c026d3",hover:"#a21caf",light:"#fae8ff",text:"#ffffff"},purple:{main:"#7c3aed",hover:"#6d28d9",light:"#ede9fe",text:"#ffffff"},violet:{main:"#8b5cf6",hover:"#7c3aed",light:"#ddd6fe",text:"#ffffff"},gray:{main:"#4b5563",hover:"#374151",light:"#f3f4f6",text:"#ffffff"},black:{main:"#111827",hover:"#000000",light:"#e5e7eb",text:"#ffffff"}};let Ye=null,Qe=[];const Vl={"agenda-section":To,"comandas-section":Ea,"relatorios-section":Zr,"servicos-section":fn,"produtos-section":Bn,"suppliers-section":Hn,"profissionais-section":Et,"clientes-section":zt,"estabelecimento-section":Yo,"ausencias-section":zi,"users-section":It,"sales-report-section":sl,"financial-section":ml,"commissions-section":hl,"packages-section":Na,"my-profile-section":ql};function Ul(e){const t=yo[e]||yo.indigo,s=(o=>{const n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(o);return n?`${parseInt(n[1],16)}, ${parseInt(n[2],16)}, ${parseInt(n[3],16)}`:"79, 70, 229"})(t.main);document.body.style.setProperty("--theme-main",t.main);const r=document.getElementById("dynamic-theme-styles");r.innerHTML=`
        :root {
    ...
    `}function fa(){const e=Qe.filter(t=>!t.read).length;if(e>0?(ta.textContent=e,ta.classList.remove("hidden")):ta.classList.add("hidden"),Qe.length===0){ho.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}ho.innerHTML=Qe.map(t=>`
    ...
    `).join("")}function Jl(e){Ye&&Ye();const t=Ae(te,"establishments",e,"notifications"),a=ha(t,xa("timestamp",">=",new Date),ko("timestamp","desc"));Ye=ls(a,s=>{s.docChanges().forEach(r=>{if(r.type==="added"){const o=r.doc.data();Qe.unshift({title:o.title,message:o.message,time:o.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(o.title,o.message,"info",!0),fa();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(o.type==="cancellation"||o.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),To())}})},s=>{console.error("Erro no listener de notificações em tempo real:",s),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function Z(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=p.enabledModules?.[a]!==!1,o=p.userPermissions===null||p.userPermissions[e]?.view===!0;if(!r||!o){ea.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const s=Vl[e];s?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),ea.innerHTML="",s(t)):(ea.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function _l(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),s=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),o=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(o&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!o&&!n))try{const i=await _s();o&&s&&(s.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),o&&s&&(s.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function Wl(e){try{Xe.getPlatform()==="android"&&(await ie.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await ie.checkPermissions();if(t.receive==="prompt"&&(t=await ie.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await ie.register(),ie.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const s=Dt(te,"users",e);await So(s,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(s){console.error("Erro ao salvar token FCM:",s)}}),ie.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),ie.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),ie.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),Z("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function Gl(){Xe.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),ys(),vo.addEventListener("click",e=>{e.stopPropagation(),ht.classList.toggle("hidden"),ht.classList.contains("hidden")||(Qe.forEach(t=>t.read=!0),fa())}),zl.addEventListener("click",()=>{ws()}),aa.addEventListener("click",e=>{e.stopPropagation(),K.classList.toggle("active"),K.classList.contains("active")?K.classList.remove("hidden"):setTimeout(()=>K.classList.add("hidden"),200)}),xo&&xo.addEventListener("click",e=>{e.preventDefault(),Z("my-profile-section"),K.classList.remove("active"),K.classList.add("hidden")}),document.addEventListener("click",e=>{!ht.contains(e.target)&&e.target!==vo&&ht.classList.add("hidden"),!K.contains(e.target)&&e.target!==aa&&K.classList.contains("active")&&(K.classList.remove("active"),setTimeout(()=>K.classList.add("hidden"),200))}),rs(G,async e=>{if(e)try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const s=await at(a.establishmentId);p.enabledModules=s.modules,Ul(s.themeColor||"indigo");let r=null,o=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const d=Dt(te,"users",e.uid),c=await is(d);if(c.exists()){const u=c.data();r=a.role==="employee"?u.permissions||{}:null,o=u.name||o,n=u.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}p.userProfessionalId=n,Wl(e.uid);const i=o||e.email;ds(a.establishmentId,s.name,r),aa.textContent=i.charAt(0).toUpperCase(),jl.textContent=i,Hl.textContent=e.email;const l=()=>{Ye&&Ye(),qa(G).then(()=>window.location.href="/login.html")};Ol.addEventListener("click",d=>{d.preventDefault(),l()}),Es(Z,r,p.enabledModules),_l(r),Jl(a.establishmentId),fa(),vt.classList.add("fade-out"),Kt.style.display="flex",setTimeout(()=>{vt.style.display="none"},500),Z("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),vt.classList.add("fade-out"),setTimeout(()=>{vt.style.display="none"},500),Kt.innerHTML=`
                ...
                `,Kt.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{qa(G).then(()=>window.location.href="/login.html")})}else window.location.href="/login.html"})}Gl();
