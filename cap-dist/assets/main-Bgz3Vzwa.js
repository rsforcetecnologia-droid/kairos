...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${r.status}) em ${s}:`,n),new Error(n)}return r.json()}catch(r){throw console.error(`Falha de rede ao tentar acessar ${s}:`,r.message),r.message.includes("Failed to fetch")||r.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${Wt}. Verifique sua conexão com a internet.`):r}}const ts=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),w(r)},as=(e,t,a)=>{const s=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return w(s)},os=e=>w("/api/appointments",{method:"POST",body:JSON.stringify(e)}),da=(e,t)=>w(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),ss=e=>w(`/api/appointments/${e}`,{method:"DELETE"}),rs=e=>w(`/api/appointments/${e}/reopen`,{method:"POST"}),ns=(e,t)=>w(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let F;async function is(){if(!F)try{F=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function ls(){if(!F){console.warn("AudioContext não inicializado. O som não será tocado.");return}F.state==="suspended"&&F.resume();const e=F.createOscillator(),t=F.createGain();e.connect(t),t.connect(F.destination),e.type="sine",e.frequency.setValueAtTime(800,F.currentTime),t.gain.setValueAtTime(0,F.currentTime),t.gain.linearRampToValueAtTime(.3,F.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,F.currentTime+.2),e.start(F.currentTime),e.stop(F.currentTime+.2)}function g(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&ls();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},d={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;const d=r.querySelector("[data-close-modal]");d&&(d.onclick=n);const l=r.querySelector('[data-action="close-modal"]');return l&&(l.onclick=n),r.addEventListener("click",u=>{u.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function It(e){const t=document.getElementById(e);t&&(t.style.display="none")}function ds(){document.body.addEventListener("click",()=>{F||is()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const s=t.dataset.target;if(s){const r=document.getElementById(s);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const s=document.getElementById("genericModal");s&&(s.style.display="none")}})}async function Ea(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const s=await as(p.establishmentId,t,a);if(s.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=s.map(r=>`
        ...
        `).join("")}catch(s){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${s.message}</p>`}}function cs(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const s=`
    ...
    `,{modalElement:r}=N({title:"Histórico de Cancelamentos",contentHTML:s,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Ea),Ea()}const z=document.getElementById("sidebar"),$a=document.getElementById("sidebarToggle"),Fe=document.getElementById("mainContent"),us=document.querySelectorAll(".sidebar-link"),Ia=document.getElementById("hamburger-menu-btn"),Le=document.getElementById("mobile-overlay");function ft(e){!z||!Fe||(z.classList.toggle("collapsed",e),Fe.classList.toggle("sidebar-collapsed-shift",e))}function ms(){!z||!Le||(z.classList.add("mobile-open"),Le.classList.add("visible"))}function it(){!z||!Le||(z.classList.remove("mobile-open"),Le.classList.remove("visible"))}function ps(){ft(!z.classList.contains("collapsed"))}function gs(e,t,a){if(!z||!Fe)return;Fe.classList.add("main-content-shift"),window.innerWidth>=768?ft(z.classList.contains("collapsed")):(Fe.classList.remove("main-content-shift","sidebar-collapsed-shift"),it()),$a&&$a.addEventListener("click",r=>{r.stopPropagation(),ps()}),z.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&z.classList.contains("collapsed")&&ft(!1)}),z.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||ft(!0))}),Ia&&Ia.addEventListener("click",r=>{r.stopPropagation(),ms()}),Le&&Le.addEventListener("click",r=>{r.stopPropagation(),it()});let s=0;z.addEventListener("touchstart",r=>{s=r.changedTouches[0].screenX},{passive:!0}),z.addEventListener("touchend",r=>{const o=r.changedTouches[0].screenX;s-o>50&&it()},{passive:!0}),us.forEach(r=>{const o=r.getAttribute("data-target"),n=o.replace("-section",""),i=a?.[n]!==!1,d=t===null||t[o]?.view===!0;if(!i||!d){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",l=>{l.preventDefault(),o&&typeof e=="function"&&e(o),window.innerWidth<768&&it()})})}const et=e=>{const t=e||p.establishmentId;return t?w(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},bs=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},fs=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},vs=(e,t)=>{const a=e||p.establishmentId;return a?w(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},G=e=>w(`/api/professionals/${e}`),hs=e=>w(`/api/professionals/details/${e}`),ys=e=>w("/api/professionals",{method:"POST",body:JSON.stringify(e)}),xs=(e,t)=>w(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),ws=e=>w(`/api/professionals/${e}`,{method:"DELETE"});var Be;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Be||(Be={}));class qt extends Error{constructor(t,a,s){super(t),this.message=t,this.code=a,this.data=s}}const ks=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},Ss=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},s=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:ks(e),o=()=>r()!=="web",n=c=>{const m=l.get(c);return!!(m?.platforms.has(r())||i(c))},i=c=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===c)},d=c=>e.console.error(c),l=new Map,u=(c,m={})=>{const b=l.get(c);if(b)return console.warn(`Capacitor plugin "${c}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(c);let k;const h=async()=>(!k&&f in m?k=typeof m[f]=="function"?k=await m[f]():k=m[f]:t!==null&&!k&&"web"in m&&(k=typeof m.web=="function"?k=await m.web():k=m.web),k),y=(T,P)=>{var A,O;if(v){const ae=v?.methods.find(_=>P===_.name);if(ae)return ae.rtype==="promise"?_=>a.nativePromise(c,P.toString(),_):(_,rt)=>a.nativeCallback(c,P.toString(),_,rt);if(T)return(A=T[P])===null||A===void 0?void 0:A.bind(T)}else{if(T)return(O=T[P])===null||O===void 0?void 0:O.bind(T);throw new qt(`"${c}" plugin is not implemented on ${f}`,Be.Unimplemented)}},E=T=>{let P;const A=(...O)=>{const ae=h().then(_=>{const rt=y(_,T);if(rt){const nt=rt(...O);return P=nt?.remove,nt}else throw new qt(`"${c}.${T}()" is not implemented on ${f}`,Be.Unimplemented)});return T==="addListener"&&(ae.remove=async()=>P()),ae};return A.toString=()=>`${T.toString()}() { [capacitor code] }`,Object.defineProperty(A,"name",{value:T,writable:!1,configurable:!1}),A},$=E("addListener"),L=E("removeListener"),B=(T,P)=>{const A=$({eventName:T},P),O=async()=>{const _=await A;L({eventName:T,callbackId:_},P)},ae=new Promise(_=>A.then(()=>_({remove:O})));return ae.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await O()},ae},H=new Proxy({},{get(T,P){switch(P){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?B:$;case"removeListener":return L;default:return E(P)}}});return s[c]=H,l.set(c,{name:c,proxy:H,platforms:new Set([...Object.keys(m),...v?[f]:[]])}),H};return a.convertFileSrc||(a.convertFileSrc=c=>c),a.getPlatform=r,a.handleError=d,a.isNativePlatform=o,a.isPluginAvailable=n,a.registerPlugin=u,a.Exception=qt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},Es=e=>e.Capacitor=Ss(e),We=Es(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),ca=We.registerPlugin;class po{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let s=!1;this.listeners[t]||(this.listeners[t]=[],s=!0),this.listeners[t].push(a);const o=this.windowListeners[t];o&&!o.registered&&this.addWindowListener(o),s&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,s){const r=this.listeners[t];if(!r){if(s){let o=this.retainedEventArguments[t];o||(o=[]),o.push(a),this.retainedEventArguments[t]=o}return}r.forEach(o=>o(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:s=>{this.notifyListeners(a,s)}}}unimplemented(t="not implemented"){return new We.Exception(t,Be.Unimplemented)}unavailable(t="not available"){return new We.Exception(t,Be.Unavailable)}async removeListener(t,a){const s=this.listeners[t];if(!s)return;const r=s.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(s=>{this.notifyListeners(t,s)}))}}const Ca=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),La=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class $s extends po{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(s=>{if(s.length<=0)return;let[r,o]=s.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=La(r).trim(),o=La(o).trim(),a[r]=o}),a}async setCookie(t){try{const a=Ca(t.key),s=Ca(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,o=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${s||""}${r}; path=${o}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}ca("CapacitorCookies",{web:()=>new $s});const Is=async e=>new Promise((t,a)=>{const s=new FileReader;s.onload=()=>{const r=s.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},s.onerror=r=>a(r),s.readAsDataURL(e)}),Cs=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,o,n)=>(r[o]=e[t[n]],r),{})},Ls=(e,t=!0)=>e?Object.entries(e).reduce((s,r)=>{const[o,n]=r;let i,d;return Array.isArray(n)?(d="",n.forEach(l=>{i=t?encodeURIComponent(l):l,d+=`${o}=${i}&`}),d.slice(0,-1)):(i=t?encodeURIComponent(n):n,d=`${o}=${i}`),`${s}&${d}`},"").substr(1):null,Bs=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Cs(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const o=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))o.set(n,i);a.body=o.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const o=new FormData;if(e.data instanceof FormData)e.data.forEach((i,d)=>{o.append(d,i)});else for(const i of Object.keys(e.data))o.append(i,e.data[i]);a.body=o;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Ts extends po{async request(t){const a=Bs(t,t.webFetchExtra),s=Ls(t.params,t.shouldEncodeUrlParams),r=s?`${t.url}?${s}`:t.url,o=await fetch(r,a),n=o.headers.get("content-type")||"";let{responseType:i="text"}=o.ok?t:{};n.includes("application/json")&&(i="json");let d,l;switch(i){case"arraybuffer":case"blob":l=await o.blob(),d=await Is(l);break;case"json":d=await o.json();break;case"document":case"text":default:d=await o.text()}const u={};return o.headers.forEach((c,m)=>{u[m]=c}),{data:d,headers:u,status:o.status,url:o.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}ca("CapacitorHttp",{web:()=>new Ts});const oe=ca("PushNotifications",{}),Ms=(e,t,a)=>w(`/api/analytics/${e}?startDate=${t}&endDate=${a}`),Ds=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:s})=>{let r=`/api/reports/sales/${e}?startDate=${t}&endDate=${a}`;return s&&s!=="all"&&(r+=`&cashierSessionId=${s}`),w(r)},Ps=(e,t,a)=>w(`/api/analytics/${e}/monthly-details?year=${t}&month=${a}`),As=(e,t,a,s)=>{const r=`/api/analytics/${e}/daily-details?year=${t}&month=${a}&day=${s}`;return w(r)},qs=(e,t,a,s)=>{const r=`/api/analytics/${e}/professional-details?year=${t}&month=${a}&professionalId=${s}`;return w(r)},Ns=()=>w("/api/reports/summary",{method:"GET"}),De=e=>w(`/api/services/${e}`),Fs=e=>w("/api/services",{method:"POST",body:JSON.stringify(e)}),Rs=(e,t)=>w(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),js=e=>w(`/api/services/${e}`,{method:"DELETE"}),Hs=(e,t)=>w(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),Os=e=>w(`/api/services/stats/most-popular/${e}`),Ct=(e,t,a,s="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${s}`;return w(r)},Lt=e=>w("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ua=e=>w(`/api/blockages/${e}`,{method:"DELETE"}),go=e=>w("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),tt=e=>w(`/api/clients/${e}`),ma=e=>w("/api/clients",{method:"POST",body:JSON.stringify(e)}),zs=(e,t)=>w(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Vs=e=>w(`/api/clients/${e}`,{method:"DELETE"}),Us=(e,t,a)=>{const s=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return w(s)},Js=(e,t,a)=>{const s=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return w(s)},_s=(e,t,a,s)=>w("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:s})}),Ba=document.getElementById("content"),Ws=window.location.origin;let Ta=!1;const Gt=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let at=[],Bt=[],Ge={},ee=[],C={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},S={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Gs(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function bo(e){const t=new Date(e);if(t.setHours(0,0,0,0),C.currentView==="week"&&C.weekViewDays===7){const a=t.getDay(),s=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(s))}return t}function kt(){const e=document.getElementById("profSelectorContainer"),t=C.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(o=>C.showInactiveProfs||o.status!=="inactive");t&&(a=a.filter(o=>o.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(o=>{const n=C.selectedProfessionalId===o.id,i=o.name==="Todos"?"Todos":o.name.split(" ")[0],d=o.name==="Todos"?"T":o.name.charAt(0).toUpperCase(),l=o.status!=="inactive",u=Gt[0],c=o.id!=="all"&&p.professionalColors.get(o.id)||u,m=o.photo||`https://placehold.co/64x64/${c.main?.replace("#","")||"E0E7FF"}/${c.light?.replace("#","")||"4F46E5"}?text=${d}`,b=o.id==="all"?"#e0e7ff":c.light,f=o.id==="all"?"#4f46e5":c.main,k=`border: 3px solid ${n?c.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+c.border:"none"};`;return`
    ...
    `;return{title:a,content:s}}function lr(){const e="Selecionar Serviço",t=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${at.map(a=>{const s=S.data.selectedServiceIds.includes(a.id),r=a.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
    ...
    `;return{title:e,content:t}}function dr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
...
Aguardo confirmação.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(n)}&body=${encodeURIComponent(i)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=p.establishmentId,ln(s).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const a=e.target.closest(".supplier-order-card"),s=a.dataset.supplierName,r=[];a.querySelectorAll(".border-b").forEach(n=>{const i=n.querySelector(".text-gray-800"),d=n.querySelector(".text-gray-600");if(i&&d){const l=i.innerText,u=d.innerText.split(" x ")[0];r.push({name:l,qty:u})}}),ja("Pedido de Compra",s,r,!1)}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);In(s)}},ue.addEventListener("click",ne),ue.addEventListener("input",ne),ue.addEventListener("change",ne),lt("list")}function lt(e){vt=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),yn()}const Nt=document.getElementById("content"),Oa={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let te=new Set,dt=null,Se=null;function Ln(e=8){let t="";for(let a=0;a<e;a++)t+=`
    ...
    `,r=window.innerWidth<768,o=r?"max-w-full":"max-w-3xl";if(N({title:a,contentHTML:s,maxWidth:o}),r){const u=document.getElementById("genericModal");if(u){const c=u.querySelector(`.${o.replace(":","\\:")}`);c&&(c.style.height="auto",c.style.maxHeight="85vh",c.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&n.addEventListener("click",async u=>{const c=u.target.closest("[data-action]");if(!c)return;switch(c.dataset.action){case"redeem-reward":{const b=parseInt(c.dataset.points,10),f=c.dataset.reward;if(await M("Confirmar Resgate",`Deseja resgatar "${f}" por ${b} pontos?`))try{await _s(p.establishmentId,R.name,R.phone,{points:b,reward:f}),g("Prémio resgatado com sucesso!","success"),K=await tt(p.establishmentId);const h=K.find(y=>y.id===R.id);h&&(R=h),oa("fidelidade")}catch(k){g(`Erro ao resgatar: ${k.message}`,"error")}break}case"open-comanda-from-history":{const b=c.dataset.appointmentId;b&&(document.getElementById("genericModal").style.display="none",j("comandas-section",{selectedAppointmentId:b,initialFilter:"finalizada"}));break}case"view-appointment":{const b=c.dataset.appointmentId,f=c.dataset.appointmentDate;b&&f&&(document.getElementById("genericModal").style.display="none",j("agenda-section",{targetDate:f,scrollToAppointmentId:b}));break}}}),oa("cadastro");const i=document.getElementById("client-form");i&&i.addEventListener("submit",u=>{u.preventDefault(),Yn()});const d=document.getElementById("cancelDetailViewBtn");d&&d.addEventListener("click",u=>{u.preventDefault(),document.getElementById("genericModal").style.display="none",At()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await Xn()})}async function Yn(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await zs(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await ma(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await At()}catch(s){g("Erro",`Não foi possível salvar: ${s.message}`,"error")}}async function Xn(){if(!R||!R.id)return;if(await M("Excluir Cliente",`Tem certeza que deseja excluir ${R.name}? Esta ação é irreversível.`))try{await Vs(R.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await At()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function ht(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const s=U==="inactive",r=U==="birthdays";e.forEach(o=>{const n=document.createElement("div");n.className="client-card bg-white rounded-lg shadow p-4 flex flex-col cursor-pointer",n.dataset.clientId=o.id;const i=o.loyaltyPoints||0,d=ze.enabled?`${i} pts`:`R$ ${i.toFixed(2)}`;let l="";const c=`https://wa.me/55${o.phone?o.phone.replace(/\D/g,""):""}?text=`;if(s){const m=encodeURIComponent(jn(o.name,aa));l=`
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const s=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!s||!r){g("Erro","Preencha todos os campos.","error");return}const o=t.querySelector('button[form="change-email-form"]');o.disabled=!0,o.textContent="A verificar...";try{const n=J.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Ho.credential(n.email,r);await Oo(n,i),o.textContent="A enviar link...",await zo(n,s),o.textContent="A atualizar BD...",await vs(p.establishmentId,s),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{o.disabled=!1,o.textContent="Salvar Novo E-mail"}})}function hi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,await hl()}async function hl(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=p.userProfessionalId;if(t){const a=await hs(t);fl=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo),e.innerHTML=`
