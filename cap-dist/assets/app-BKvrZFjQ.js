import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{a as z,m as _a,d as V}from"./firebase-config-BeTHj08_.js";import{EmailAuthProvider as ks,reauthenticateWithCredential as Ss,verifyBeforeUpdateEmail as $s,updatePassword as Es,updateProfile as Is,setPersistence as Cs,browserLocalPersistence as Ls,onAuthStateChanged as Ao,signOut as Ja}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{doc as me,updateDoc as Ea,arrayUnion as Ts,addDoc as No,collection as pe,query as jt,where as st,getDocs as Ia,orderBy as qo,writeBatch as Fo,serverTimestamp as Wa,deleteDoc as Bs,getDoc as Ps,onSnapshot as Ds}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import{getToken as Ms,onMessage as As}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-messaging.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-storage.js";const m={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function Ns(e,t,a){m.establishmentId=e,m.establishmentName=t,m.userPermissions=a}const ua="https://kairos-app-407358446276.us-central1.run.app";console.log("🚀 API configurada para Produção (US):",ua);async function qs(){const e=z.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function y(e,t={}){const a=await qs();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const s=ua.replace(/\/$/,""),r=e.startsWith("/")?e:`/${e}`,o=`${s}${r}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${o}`);try{const n=await fetch(o,{...t,headers:{...a,...t.headers}});if(!n.ok){const l=(await n.json().catch(()=>({message:n.statusText}))).message||`Erro na API: ${n.status}`;if(l.includes("FAILED_PRECONDITION")&&l.includes("requires an index")){const d=/(https:\/\/[^\s]+)/,c=l.match(d),u=c?c[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${o}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${o}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${ua}. Verifique sua conexão com a internet.`):n}}const Ro=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),y(r)},Fs=(e,t,a)=>{const s=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return y(s)},Rs=({establishmentId:e,professionalId:t,serviceIds:a,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${s}`;return y(r)},js=e=>y("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Ca=(e,t)=>y(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),Hs=e=>y(`/api/appointments/${e}`,{method:"DELETE"}),Os=e=>y(`/api/appointments/${e}/reopen`,{method:"POST"}),zs=(e,t)=>y(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let U;async function Vs(){if(!U)try{U=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Us(){if(!U){console.warn("AudioContext não inicializado. O som não será tocado.");return}U.state==="suspended"&&U.resume();const e=U.createOscillator(),t=U.createGain();e.connect(t),t.connect(U.destination),e.type="sine",e.frequency.setValueAtTime(800,U.currentTime),t.gain.setValueAtTime(0,U.currentTime),t.gain.linearRampToValueAtTime(.3,U.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,U.currentTime+.2),e.start(U.currentTime),e.stop(U.currentTime+.2)}function g(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&Us();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;const l=r.querySelector("[data-close-modal]");l&&(l.onclick=n);const d=r.querySelector('[data-action="close-modal"]');return d&&(d.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Ht(e){const t=document.getElementById(e);t&&(t.style.display="none")}function _s(){document.body.addEventListener("click",()=>{U||Vs()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const s=t.dataset.target;if(s){const r=document.getElementById(s);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const s=document.getElementById("genericModal");s&&(s.style.display="none")}})}async function Ga(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const s=await Fs(m.establishmentId,t,a);if(s.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=s.map(r=>`
        ...
        `).join("")}catch(s){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${s.message}</p>`}}function Js(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const s=`
    ...
    `,{modalElement:r}=W({title:"Histórico de Cancelamentos",contentHTML:s,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Ga),Ga()}const Y=document.getElementById("sidebar"),Ya=document.getElementById("sidebarToggle"),Qe=document.getElementById("mainContent"),Ws=document.querySelectorAll(".sidebar-link"),Qa=document.getElementById("hamburger-menu-btn"),Fe=document.getElementById("mobile-overlay");function St(e){!Y||!Qe||(Y.classList.toggle("collapsed",e),Qe.classList.toggle("sidebar-collapsed-shift",e))}function Gs(){!Y||!Fe||(Y.classList.add("mobile-open"),Fe.classList.add("visible"))}function mt(){!Y||!Fe||(Y.classList.remove("mobile-open"),Fe.classList.remove("visible"))}function Ys(){St(!Y.classList.contains("collapsed"))}function Qs(e,t,a){if(!Y||!Qe)return;Qe.classList.add("main-content-shift"),window.innerWidth>=768?St(Y.classList.contains("collapsed")):(Qe.classList.remove("main-content-shift","sidebar-collapsed-shift"),mt()),Ya&&Ya.addEventListener("click",r=>{r.stopPropagation(),Ys()}),Y.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&Y.classList.contains("collapsed")&&St(!1)}),Y.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||St(!0))}),Qa&&Qa.addEventListener("click",r=>{r.stopPropagation(),Gs()}),Fe&&Fe.addEventListener("click",r=>{r.stopPropagation(),mt()});let s=0;Y.addEventListener("touchstart",r=>{s=r.changedTouches[0].screenX},{passive:!0}),Y.addEventListener("touchend",r=>{const o=r.changedTouches[0].screenX;s-o>50&&mt()},{passive:!0}),Ws.forEach(r=>{const o=r.getAttribute("data-target"),n=o.replace("-section",""),i=a?.[n]!==!1,l=t===null||t[o]?.view===!0;if(!i||!l){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",d=>{d.preventDefault(),o&&typeof e=="function"&&e(o),window.innerWidth<768&&mt()})})}const Ee=e=>{const t=e||m.establishmentId;return t?y(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},$t=(e,t)=>{const a=e||m.establishmentId;return a?y(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Xs=(e,t)=>{const a=e||m.establishmentId;return a?y(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Zs=(e,t)=>{const a=e||m.establishmentId;return a?y(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},Q=e=>y(`/api/professionals/${e}`),Ks=e=>y(`/api/professionals/details/${e}`),jo=e=>y("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Bt=(e,t)=>y(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Xa=(e,t)=>Bt(e,{services:t}),Ho=e=>y(`/api/professionals/${e}`,{method:"DELETE"}),er=e=>{const t=e.map(a=>Ho(a));return Promise.all(t)},Ie=e=>y(`/api/services/${e}`),Oo=e=>y("/api/services",{method:"POST",body:JSON.stringify(e)}),tr=(e,t)=>y(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),ar=e=>y(`/api/services/${e}`,{method:"DELETE"}),or=(e,t)=>y(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),sr=e=>y(`/api/services/stats/most-popular/${e}`),Ot=e=>y(`/api/products/${e}`),zo=e=>y("/api/products",{method:"POST",body:JSON.stringify(e)}),rr=(e,t)=>y(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),nr=e=>y(`/api/products/${e}`,{method:"DELETE"}),ir=(e,t)=>y(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),lr=({startDate:e,endDate:t,productId:a,categoryId:s,establishmentId:r})=>{const o=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&o.append("productId",a),s&&s!=="all"&&o.append("categoryId",s),r&&o.append("establishmentId",r),y(`/api/products/stock-history/report?${o.toString()}`)},dr={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}};function Za(e,t,a){return new Promise((s,r)=>{const o=new FileReader;o.readAsDataURL(e),o.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const l=document.createElement("canvas");let d=i.width,c=i.height;d>t&&(c*=t/d,d=t),l.width=d,l.height=c,l.getContext("2d").drawImage(i,0,0,d,c);const p=e.type==="image/png"&&t<500?"image/png":"image/jpeg";s(l.toDataURL(p,a))},i.onerror=l=>r(l)},o.onerror=n=>r(n)})}let ue=null;const Et=[{id:"company_data",title:"Identidade do Negócio",icon:"🏢",description:"Configure os dados da sua empresa."},{id:"branding",title:"Sua Marca",icon:"🎨",description:"Logo e cores (Opcional)."},{id:"time_config",title:"O Relógio",icon:"⏱️",description:"Tempo padrão entre agendamentos."},{id:"first_service",title:"O Menu",icon:"✂️",description:"Seu principal serviço."},{id:"first_prof",title:"Sua Equipe",icon:"💇",description:"Cadastre o primeiro profissional."},{id:"first_product",title:"O Estoque",icon:"🧴",description:"Cadastre um produto (opcional)."}];let X=0,Pt=[];async function cr(){try{console.log("Iniciando verificação de Onboarding para ID:",m.establishmentId);const e=await Ee(m.establishmentId),t=await Q(m.establishmentId),a=await Ie(m.establishmentId);Pt=a||[];const s=e&&e.name&&(e.phone||e.address),r=e&&(e.logo||e.themeColor&&e.themeColor!=="indigo"),o=e&&e.slotInterval>0,n=a&&a.length>0,i=t&&t.length>0;if(console.log("Status Onboarding:",{hasCompanyData:s,hasBranding:r,hasTimeConfig:o,hasService:n,hasProf:i}),s&&o&&i&&n)return;if(!s)X=0;else if(!r&&!o)X=1;else if(!o)X=2;else if(!n)X=3;else if(!i)X=4;else if(X===0)return;ur(),Ta(X)}catch(e){console.error("Erro ao verificar onboarding:",e)}}function ur(){document.getElementById("onboarding-overlay")||(ue=document.createElement("div"),ue.id="onboarding-overlay",ue.className="fixed inset-0 bg-gray-900 bg-opacity-95 z-[9999] flex items-center justify-center p-4 overflow-y-auto",ue.style.cssText="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(17, 24, 39, 0.95); z-index: 9999; display: flex; align-items: center; justify-content: center;",ue.innerHTML=`
    ...
    `,document.body.appendChild(ue),La())}function La(){const e=Math.round(X/Et.length*100),t=document.getElementById("progress-bar"),a=document.getElementById("progress-text");t&&(t.style.width=`${e}%`),a&&(a.innerText=`${e}%`)}function Ta(e){const t=document.getElementById("onboarding-step-content"),a=Et[e];if(!a){Ka(t);return}let s="";if(a.id==="company_data")s=`
        ...
        <div class="mt-6 flex justify-end gap-2">
            ...
            <button type="button" id="next-step-btn" class="bg-indigo-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-indigo-700 transition flex items-center gap-2 text-sm">
                ${e===Et.length-1?"Concluir":"Próximo"}
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"></path></svg>
            </button>
        </div>
    `,document.getElementById("next-step-btn").addEventListener("click",()=>mr(a.id)),document.getElementById("skip-btn")&&document.getElementById("skip-btn").addEventListener("click",()=>{e===Et.length-1?Ka(t):(X++,La(),Ta(X))}),a.id==="branding"){const r=document.getElementById("logo-input"),o=document.getElementById("bg-input");r&&(r.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Za(i,200,.8);document.getElementById("logo-base64").value=l,document.getElementById("logo-preview").innerHTML=`<img src="${l}" class="w-full h-full object-contain rounded">`}catch(l){console.error("Erro logo",l)}}),o&&(o.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Za(i,1024,.7);document.getElementById("bg-base64").value=l}catch(l){console.error("Erro bg",l)}})}}function Ka(e){e.innerHTML=`
    ...
    `;return{title:a,content:s}}function Jr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${dt.map(s=>{const r=E.data.selectedServiceIds.includes(s.id),o=s.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S",n=v(s.name);return`
    ...
    `;return{title:t,content:i}}function Yr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),s=dt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=E.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>Go(r.dataset.serviceId,r))})}function Qr(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),s=Ut.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=E.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",i=v(r.name);return`
    ...
    `,{modalElement:t}=W({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const s=document.getElementById("initial-amount"),r=s.value.trim(),o=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),s.focus();return}try{const i={establishmentId:m.establishmentId,initialAmount:parseFloat(n.toFixed(2))};o&&(i.notes=o);const l=await tn(i);x.isCashierOpen=!0,x.activeCashierSessionId=l.id,await Aa(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function vn(){const e=x.activeCashierSessionId;if(e)try{const t=await sn(e),a=`
        ...
        `,{modalElement:s}=W({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});s.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const o=parseFloat(document.getElementById("final-amount").value);if(isNaN(o)||o<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await an(e,o),x.isCashierOpen=!1,x.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await Aa(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function hn(e){x.activeFilter!==e&&(x.activeFilter=e,x.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),xe(),await se(),x.selectedComandaId=null,K())}function ba(e){x.selectedComandaId=e,ge(),dn(),K()}async function xn(e,t){const a=x.allComandas.find(r=>r.id===x.selectedComandaId);if(!a)return;const s=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...s),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),K(),ge();return}try{await Ca(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),K(),ge()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function yn(e,t){const a=x.allComandas.find(o=>o.id===x.selectedComandaId);if(!a)return;let s=!1,r=(a.comandaItems||[]).findIndex(o=>o.id===e&&o.type===t);if(r>-1)a.comandaItems.splice(r,1),s=!0;else{let o=(a.services||[]).findIndex(n=>n.id===e);if(o>-1)a.services.splice(o,1),s=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),s=!0)}}if(s){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),K(),ge();return}try{await Ca(a.id,a),g("Sucesso","Item removido!","success"),K(),ge()}catch(o){g("Erro",`Não foi possível remover o item: ${o.message}`,"error"),await se()}}}async function wn(e,t,a){const s=e.type==="appointment",r=_t(e),o=new Set,n=[];for(const l of r){if(l.type==="service"&&l.id){if(o.has(l.id))continue;o.add(l.id)}n.push(l)}const i={payments:a,totalAmount:t,items:n,cashierSessionId:x.activeCashierSessionId};try{s?await zs(e.id,i):(i.establishmentId=m.establishmentId,i.clientName=e.clientName,i.professionalId=e.professionalId,i.clientPhone=e.clientPhone,await Zr(i)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",xe(),x.selectedComandaId=null,await se()}catch(l){g("Erro no Checkout",l.message,"error")}}async function kn(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,s=x.clients.find(n=>n.id===t),r=m.professionals.find(n=>n.id===a);if(!s||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const o={id:`temp-${Date.now()}`,type:"walk-in",clientName:s.name,clientPhone:s.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};x.allComandas.unshift(o),x.selectedComandaId=o.id,document.getElementById("genericModal").style.display="none",ba(o.id)}async function se(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=x.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await Zo();if(x.isCashierOpen=!!a,x.activeCashierSessionId=a?a.id:null,un(),!x.isCashierOpen&&x.activeFilter==="atendimento"){ge(),K();return}const s=await Xr(m.establishmentId,t,x.paging.page,x.paging.limit);if(x.allComandas=s.data||s,x.paging.total=s.total||s.length,x.catalog.services.length===0){const[r,o,n,i,l]=await Promise.all([Ie(m.establishmentId),Ot(m.establishmentId),Ko(m.establishmentId),lt(m.establishmentId),Q(m.establishmentId)]);x.catalog={services:r,products:o,packages:n},x.clients=i,m.professionals=l}ge(),x.selectedComandaId,K()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function Aa(e={}){Be=document.getElementById("content");try{const t=await Zo();x.isCashierOpen=!!t,x.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),x.isCashierOpen=!1}x.selectedComandaId=e.selectedAppointmentId||null,cn(),Le&&(Be.removeEventListener("click",Le),Be.removeEventListener("change",Le)),Le=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&x.activeFilter==="finalizadas"){x.paging.page=1,await se();return}if(a){if(a.matches("[data-filter]"))hn(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}ba(a.dataset.comandaId)}else if(a.matches("[data-action]")){const s=a.dataset.action,r=a.dataset.id||x.selectedComandaId;switch(s){case"back-to-list":{xe(),x.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(o=>o.classList.remove("selected")),K();break}case"new-sale":es();break;case"add-item":gn();break;case"checkout":bn();break;case"open-cashier":fn();break;case"close-cashier":await vn();break;case"view-sales-report":ee("sales-report-section");break;case"remove-item":await yn(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await j("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await Os(r);const n=x.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete x.allComandas[n].transaction,delete x.allComandas[n].cashierSessionId,delete x.allComandas[n].redeemedReward,x.allComandas[n].status="confirmed"),x.selectedComandaId=null,xe(),g("Sucesso!","Comanda reaberta para edição.","success"),await se()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await j("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await Kr(r),g("Sucesso!","Venda revertida."),xe(),x.selectedComandaId=null,await se()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const o=a.dataset.id,n=a.dataset.date;ee("agenda-section",{scrollToAppointmentId:o,targetDate:new Date(n)});break}case"delete-walk-in":{if(await j("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))x.allComandas=x.allComandas.filter(n=>n.id!==r),x.selectedComandaId=null,ge(),K(),g("Sucesso","Venda avulsa removida.","success"),xe();else try{await en(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),x.selectedComandaId=null,xe(),await se()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},Be.addEventListener("click",Le),Be.addEventListener("change",Le),e.initialFilter&&(x.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(x.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${x.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",x.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await se(),x.selectedComandaId&&ba(x.selectedComandaId)}const fa=document.getElementById("content");let Kt={};const pt=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],N={startDate:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,appointmentsData:[],currentTab:"dashboards"};async function Sn(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function $n(){fa.innerHTML='<div class="flex flex-col items-center justify-center h-64"><div class="loader mb-4"></div><p class="text-gray-500">A carregar inteligência de dados...</p></div>';try{await Sn();const[e,t]=await Promise.all([Q(m.establishmentId),Er(m.establishmentId).catch(()=>[])]);N.professionalsList=e||[],N.costCentersList=t||[],En(),await ts()}catch(e){console.error("Erro no loadReportsPage:",e),fa.innerHTML=`
            ...
            </div>`}}function En(){const e=N.professionalsList.map(a=>`<option value="${a.id}">${v(a.name)}</option>`).join(""),t=N.costCentersList.map(a=>`<option value="${a.id}">${v(a.name)}</option>`).join("");fa.innerHTML=`
    ...
    `,document.getElementById("btn-filter").onclick=In,document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{N.currentTab=a.dataset.tab,io(),as()}}),io()}function io(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===N.currentTab;e.className=t?"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-bold bg-white text-indigo-600 shadow-sm transition-all whitespace-nowrap":"tab-btn flex-1 px-4 py-2 rounded-md text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-200 transition-all whitespace-nowrap"})}async function In(){N.startDate=document.getElementById("report-start").value,N.endDate=document.getElementById("report-end").value,N.selectedProfessional=document.getElementById("report-prof").value,N.selectedCostCenter=document.getElementById("report-cost").value,await ts()}async function ts(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader"></div></div>';try{const t=$r(N.startDate,N.endDate,N.selectedProfessional,N.selectedCostCenter),a=N.selectedProfessional==="all"?null:N.selectedProfessional,s=new Date(N.startDate+"T00:00:00").toISOString(),r=new Date(N.endDate+"T23:59:59").toISOString(),o=Ro(m.establishmentId,s,r,a).catch(l=>[]),[n,i]=await Promise.all([t,o]);N.data=n,N.appointmentsData=Array.isArray(i)?i:[],as()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function as(){const e=document.getElementById("report-content");if(N.data)switch(N.currentTab){case"dashboards":Cn(e);break;case"appointments":Ln(e);break;case"dre":Tn(e);break}}function Cn(e){const{dreSimple:t,charts:a}=N.data,s=t||{grossRevenue:0,netProfit:0,variableCosts:0},r=N.data.totalAppointments||0,o=r>0?s.grossRevenue/r:0;e.innerHTML=`
    ...
    `}async function ct(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,s,r]=await Promise.all([Ie(m.establishmentId),Q(m.establishmentId),Jt(m.establishmentId,"services"),sr(m.establishmentId)]);m.services=(t||[]).filter(Boolean),m.professionals=(a||[]).filter(Boolean),m.serviceCategories=(s||[]).filter(Boolean),m.mostPopularService=r||{name:"N/A",count:0},m.services.forEach(o=>{o.active===void 0&&(o.active=!0)}),rs(Xe)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function rs(e){if(document.getElementById("services-content-container")){if(Xe===e&&document.getElementById("services-content-container").children.length>1){Xe==="services"&&(qa(),Pe());return}Xe=e,ke="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?Nn():e==="reports"&&qn()}}function Fn(){le&&(he.removeEventListener("click",le),he.removeEventListener("input",le),he.removeEventListener("change",le)),le=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),o=r.dataset.id,n=r.checked;try{await or(o,n);const i=m.services.findIndex(l=>l.id===o);i>-1&&(m.services[i].active=n),He(m.establishmentId,Oe(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${o}) para ${n?"Ativo":"Inativo"}`),Pe(),qa()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,Pe()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){Pe();return}if(!a)return;if(a.hasAttribute("data-view")){rs(a.dataset.view);return}switch(a.dataset.action){case"new-service":lo();break;case"edit-service":const r=JSON.parse(a.dataset.service);lo(r);break;case"manage-categories":Mn();break;case"filter-service":const o=a.dataset.filterType;if(o==="popular")return;ke=o==="total"?"all":o,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,d=i===ke||i==="total"&&ke==="all";n.classList.toggle("ring-2",d),n.classList.toggle("ring-indigo-500",d),n.classList.toggle("shadow-lg",d)}),Pe();break}},he.addEventListener("click",le),he.addEventListener("input",le),he.addEventListener("change",le)}async function Rn(){he.innerHTML=`
        ...
        </section>`,Fn();try{(!m.professionals||m.professionals.length===0)&&(m.professionals=await Q(m.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),m.professionals=[]}Xe="services",ke="all",await ct()}const Wt="suppliers",Fa="purchases",ns="financial_payables",Ra=async e=>{try{const t=jt(pe(V,Wt),st("establishmentId","==",e)),a=await Ia(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar fornecedores:",t),t}},jn=async e=>{try{return{id:(await No(pe(V,Wt),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},Hn=async(e,t)=>{try{const a=me(V,Wt,e);return await Ea(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},On=async e=>{try{const t=me(V,Wt,e);return await Bs(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},zn=async(e,t=null)=>{try{const a=Fo(V),s=me(pe(V,Fa)),r={...e,createdAt:Wa()};if(a.set(s,r),t&&t.defaultNatureId&&t.defaultCostCenterId){const o=me(pe(V,ns)),n=new Date().toISOString().split("T")[0],i={establishmentId:e.establishmentId,description:`Compra - ${e.supplierName}`,amount:parseFloat(e.totalAmount),dueDate:n,naturezaId:t.defaultNatureId,centroDeCustoId:t.defaultCostCenterId,notes:`Gerado automaticamente pelo Pedido de Compra. Itens: ${e.items.length}`,status:"pending",paymentDate:null,purchaseId:s.id,createdAt:Wa()};a.set(o,i)}return await a.commit(),{id:s.id,...r}}catch(a){throw console.error("Erro ao registrar compra com integração:",a),a}},Vn=async(e,t)=>{try{const a=Fo(V),s=me(V,Fa,e);a.delete(s);const r=jt(pe(V,ns),st("purchaseId","==",e),st("establishmentId","==",t));return(await Ia(r)).forEach(n=>{a.delete(n.ref)}),await a.commit(),!0}catch(a){throw console.error("Erro ao excluir compra e financeiro:",a),a}},Un=async e=>{try{const t=jt(pe(V,Fa),st("establishmentId","==",e),qo("createdAt","desc")),a=await Ia(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},be=document.getElementById("content");let de=null,Ze="products",oe="all";async function _n(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await os({establishmentId:m.establishmentId,name:s},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await ja(),await ut()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function Jn(e){if(await j("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await ss(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await ja(),await ut()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function ja(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await Jt(m.establishmentId,"products");m.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${v(a.name)}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(m.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${v(a.name)}</option>`)),Ha(),rt()}function Xn(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const s=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function ut(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,s]=await Promise.all([Ot(m.establishmentId),Jt(m.establishmentId,"products"),Ra(m.establishmentId)]);m.products=(t||[]).filter(Boolean),m.categories=(a||[]).filter(Boolean),m.suppliers=(s||[]).filter(Boolean),is(Ze)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function is(e){if(document.getElementById("products-content-container")){if(Ze===e&&document.getElementById("products-content-container").children.length>1){Ze==="products"&&(Ha(),rt());return}Ze=e,oe="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?Qn():e==="movements"&&Xn()}}async function Kn(){be.innerHTML=`
    ...
    `;W({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",oi),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Ht("genericModal"))},50)}function li(){fe.innerHTML=`
...
Aguardo retorno.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order");if(a.disabled)return;const s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=m.establishmentId,a.disabled=!0,a.textContent="A processar...",Ee(m.establishmentId).then(r=>{const o=r.purchaseConfig||null;return zn(s,o)}).then(()=>{g("Sucesso","Compra registrada e integrada ao financeiro!","success"),a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{a.disabled=!1,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Salvar',g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-delete-purchase")){const s=e.target.closest(".btn-delete-purchase").dataset.id;j("Excluir Compra","Isto apagará o registo histórico E o lançamento financeiro associado. Deseja continuar?").then(async r=>{if(r)try{await Vn(s,m.establishmentId),g("Sucesso","Compra e financeiro excluídos.","success"),ls()}catch(o){g("Erro","Erro ao excluir: "+o.message,"error")}})}if(e.target.closest(".btn-print-order")){const s=e.target.closest(".supplier-order-card").dataset.supplierId,r=R.finalOrders[s];r?ii(r,R.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);ni(s)}},fe.addEventListener("click",ce),fe.addEventListener("input",ce),fe.addEventListener("change",ce),gt("list")}function gt(e){It=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),ei()}const ea=document.getElementById("content"),po={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let re=new Set,bt=null,De=null;function di(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function ci(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",s=v(t.name),r=v(t.specialty||"Especialidade"),o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,n=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function ta(){const e=document.getElementById("genericModal");e.style.display="none",De&&e.removeEventListener("click",De)}async function ui(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},s=v(a.name),r=m.services||await Ie(m.establishmentId),o=m.professionals||await Q(m.establishmentId),n=`
    ...
    `}function Ti(e,t){const a=(e||[]).filter(l=>{const d=(l.status||"").toLowerCase();return d==="completed"||d==="finished"||d==="paid"||d==="finalized"});let s=0;a.forEach(l=>{s+=Oa(l)});let r=ne&&ne.conversionRate?parseFloat(ne.conversionRate):hi;r<=0&&(r=1);const o=Math.floor(s/r);let n=0;(t||[]).forEach(l=>{l.type==="redeem"&&(n+=l.points?Math.abs(l.points):0)});const i=o-n;return{totalSpent:s,totalPointsEarned:o,totalPointsRedeemed:n,currentBalance:i,moneyPerPoint:r}}function Bi(e,t,a){if(!ne||!ne.enabled)return`<div class="p-8 text-center text-gray-500">
        ...
        </div>`;const s=Ti(t,a);e.loyaltyPoints=s.currentBalance,e.totalSpent=s.totalSpent;const r=(ne.tiers||[]).map(i=>{const l=s.currentBalance>=i.points,d=v(i.reward);return`
    ...
    `}function ds(e){J=e,xa="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",s=`
    ...
    `}function fs(e="indigo",t){const a=t.querySelector("#color-palette-container"),s=t.querySelector("#establishmentThemeColor");!a||!s||(a.innerHTML="",Object.entries(Yi).forEach(([r,o])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{s.value=r,fs(r,t)}),a.appendChild(i)}),s.value=e)}function il(e,t){const a=t.querySelector("#slotIntervalContainer"),s=t.querySelector("#establishmentSlotInterval");if(!a||!s)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(o=>{const n=o.value===e;return`<button type="button" data-value="${o.value}" 
    ...
    `,ye.querySelector('button[data-action="back-to-list"]').addEventListener("click",s=>{s.preventDefault(),vs()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":Qi(_,a);break;case"change-password":Xi(_,a);break;case"change-email":Zi(_,a);break;case"branding":Ki(_,a);break;case"booking":el(_,a);break;case"working-hours":tl(_,a);break;case"loyalty":al(_,a);break;case"financial":await ol(_,a);break;case"support":sl(_,a);break;case"cancellation":rl(_,a);break;default:nl(t?t.label:"Definições",a)}}async function vs(){if(ye.innerHTML=`
    ...
    `,ye.querySelector("#settings-menu-list").addEventListener("click",o=>{const n=o.target.closest("button[data-section]");n&&(o.preventDefault(),ll(n.dataset.section))});const r=ye.querySelector('[data-action="go-to-my-profile"]');r&&r.addEventListener("click",o=>{o.preventDefault(),ee("my-profile-section")})}const tt=document.getElementById("content");async function qe(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,s=document.getElementById("filterEndDate")?.value,r=await zt(m.establishmentId,a||new Date().toISOString().split("T")[0],s||new Date().toISOString().split("T")[0],e),o=document.getElementById("filterReason")?.value.toLowerCase(),n=o?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(o)):r,i=n.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let p=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
        ...
        </section>`,ul(t),await qe(t);const r=document.getElementById("batchProfSelectionContainer");try{const o=await Q(m.establishmentId);r.innerHTML=o.map(n=>`
            <div class="flex items-center">
    ...
    `,Je&&ve.removeEventListener("click",Je),We&&ve.removeEventListener("change",We),Je=async e=>{if(!document.getElementById("user-list-view")){ve.removeEventListener("click",Je);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":ho();break;case"edit-user":const s=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));ho(s);break;case"back-to-list":At();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await j("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await fl(r),g("Usuário excluído com sucesso!","success"),At()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},We=async e=>{if(!document.getElementById("user-list-view")){ve.removeEventListener("change",We);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")Tt();else if(t){e.stopPropagation();const a=t.dataset.userId,s=t.checked?"active":"inactive";try{await hl(a,s),g(`Usuário ${s==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=m.users.findIndex(o=>o.id===a);r>-1&&(m.users[r].status=s,Tt())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,Tt()}}},ve.addEventListener("click",Je),ve.addEventListener("change",We),await Sl()}const $l=document.getElementById("content");let xo={},ka=null;function El(){Object.values(xo).forEach(e=>e?.destroy()),xo={}}function Il(e,t){if(!window.jspdf){g("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:a}=window.jspdf,s=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(s.setFontSize(18),s.text(e,s.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];s.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const o=s.lastAutoTable?s.lastAutoTable.finalY+20:60;s.text("Detalhes das Vendas",20,o),s.autoTable({html:`#${t}`,startY:o+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),s.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function yo(e){const t=document.getElementById("genericModal"),a=v(e.client),s=v(e.items),r=v(e.responsavelCaixa||"N/A"),o=(e.payments||[]).map(n=>`
    ...
    `}).join(""),o.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=ka.transactions[l];d&&yo(d)})}),n.innerHTML=a.map((i,l)=>{const d=v(i.client),c=v(i.items),u=v(i.type);return`
    ...
    `,e.style.display="flex"}function ht(e,t="all"){let a='<option value="all">Todos</option>';const s=n=>{const i=new Map,l=[];return n&&(n.forEach(d=>i.set(d.id,{...d,children:[]})),i.forEach(d=>{d.parentId&&i.has(d.parentId)?i.get(d.parentId).children.push(d):l.push(d)})),l},r=(n,i=0)=>{const l=i>0?"— ".repeat(i):"",d=n.id===t?"selected":"";a+=`<option value="${n.id}" ${d}>${l}${n.name}</option>`,n.children.forEach(c=>r(c,i+1))};return s(e).forEach(n=>r(n)),a}async function Se(){const e=document.getElementById("financial-content");if(!e)return;const t=document.getElementById("filterStartDate"),a=document.getElementById("filterEndDate");if(!t||!a)return;const s=t.value,r=a.value,o=document.getElementById("filterNaturezaId")?.value,n=document.getElementById("filterCostCenterId")?.value;if(!s||!r){try{const[d,c]=await Promise.all([nt(m.establishmentId),it(m.establishmentId)]);if(!document.getElementById("financial-content"))return;B={...B,natures:d,costCenters:c},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=ht(B.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=ht(B.costCenters))}catch(d){g("Erro",`Não foi possível carregar os dados base: ${d.message}`,"error")}Sa(),Io();return}const i=document.getElementById("payables-list"),l=document.getElementById("receivables-list");i&&(i.innerHTML='<div class="loader mx-auto"></div>'),l&&(l.innerHTML='<div class="loader mx-auto"></div>');try{const d={startDate:s,endDate:r,establishmentId:m.establishmentId};o&&o!=="all"&&(d.natureId=o),n&&n!=="all"&&(d.costCenterId=n);const[c,u,p,b]=await Promise.all([Ri(d),Vi(d),nt(m.establishmentId),it(m.establishmentId)]);if(!document.getElementById("financial-content"))return;const f=(u.previousBalance||0)-(c.previousBalance||0);B={...B,payables:c.entries||[],receivables:u.entries||[],natures:p||[],costCenters:b||[],previousBalance:f,filterNaturezaId:o,filterCostCenterId:n},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=ht(B.natures,B.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=ht(B.costCenters,B.filterCostCenterId)),Sa(),Io()}catch(d){document.getElementById("financial-content")&&(g("Erro",`Não foi possível carregar os dados: ${d.message}`,"error"),e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function Ml(e,t,a=null){e.preventDefault();const s=e.target,r=s.querySelector('[name="status"]').checked,o=s.querySelector('[name="paymentDate"]').value,n=parseFloat(s.querySelector('[name="amount"]').value),i=parseInt(s.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!o){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const l={description:s.querySelector('[name="description"]').value,amount:n,dueDate:s.querySelector('[name="dueDate"]').value,naturezaId:s.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:s.querySelector('[name="centroDeCustoId"]').value||null,notes:s.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?o:null,installments:a?1:i,establishmentId:m.establishmentId};try{a?(await(t==="payable"?ji(a,l):Ui(a,l)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?Fi(l):zi(l)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await Se()}catch(d){g("Erro",`Não foi possível salvar: ${d.message}`,"error")}}async function Al(e,t){if(await j("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?Hi(t):_i(t)),g("Sucesso","Lançamento excluído!","success"),await Se()}catch(s){g("Erro",`Falha ao excluir: ${s.message}`,"error")}}async function Nl(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?Oi(t,a):Ji(t,a)),g("Sucesso","Lançamento atualizado!","success"),await Se()}catch(s){g("Erro",`Falha ao atualizar status: ${s.message}`,"error")}}function Eo(e){const t=B.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function Sa(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(B.natures.map(i=>[i.id,i.name])),s=new Map(B.costCenters.map(i=>[i.id,i.name])),r=Eo(B.payables),o=Eo(B.receivables),n=(i,l)=>{const d=i.status==="paid",c=JSON.stringify(i).replace(/'/g,"&apos;"),u=i.naturezaId?a.get(i.naturezaId):"N/A",p=i.centroDeCustoId?s.get(i.centroDeCustoId):"N/A";let b=l==="payable"?"text-red-600":"text-green-600";const f=d?"bg-gray-200 text-gray-600":l==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",h=d?"Finalizado":"Pendente";return d&&(b="text-gray-500"),`
    ...
    `,D.historyData.length>0?ys(document.getElementById("history-list-container"),D.historyData):Va()}function ys(e,t){if(t.length===0){e.innerHTML=`
    ...
    `}const na=document.getElementById("content");let $e={allPackages:[],catalogForModal:{services:[],products:[]}},yt=null,Ae=null;function td(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function ad(){const e=document.getElementById("packagesListContainer");if(e){if($e.allPackages.length===0){e.innerHTML=`
            ...
            </div>`;return}e.innerHTML=$e.allPackages.map(t=>{const a=t.status==="active",s=JSON.stringify(t).replace(/'/g,"&apos;"),r=t.price||0,o=t.originalPrice||0,n=o>r?o-r:0,i=o>0?(o-r)/o*100:0,l=v(t.name),d=v(t.description||"Sem descrição");return`
    ...
    `,document.body.appendChild(a);const o=a.querySelector("#item-selection-list"),n=a.querySelector("#item-search-input"),i=()=>{a.remove()};r(o),n.addEventListener("input",()=>{t=n.value,r(o)}),a.addEventListener("click",l=>{const d=l.target.closest('[data-action="select-item"]'),c=l.target.closest('[data-action="close-selection-modal"]');if(d){const{itemType:u,itemId:p}=d.dataset,f=($e.catalogForModal[u+"s"]||[]).find(h=>h.id===p);f&&(e({...f,type:u}),i())}else(c||l.target===a)&&i()})}async function Ua(){na.innerHTML=`
    ...
    `,await id()}async function id(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=m.userProfessionalId;if(t){const a=await Ks(t);rd=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo);const s=v(a.name);e.innerHTML=`
    ...
    `}function $a(){const e=ot.filter(t=>!t.read).length;if(e>0?(da.textContent=e,da.classList.remove("hidden")):da.classList.add("hidden"),ot.length===0){Po.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}Po.innerHTML=ot.map(t=>`
    ...
    `).join("")}function bd(e){at&&at();const t=pe(V,"establishments",e,"notifications"),a=jt(t,st("timestamp",">=",new Date),qo("timestamp","desc"));at=Ds(a,s=>{s.docChanges().forEach(r=>{if(r.type==="added"){const o=r.doc.data();ot.unshift({title:o.title,message:o.message,time:o.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(o.title,o.message,"info",!0),$a();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(o.type==="cancellation"||o.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),Xo())}})},s=>{console.error("Erro no listener de notificações em tempo real:",s),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function ee(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=m.enabledModules?.[a]!==!1,o=m.userPermissions===null||m.userPermissions[e]?.view===!0;if(!r||!o){la.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const s=pd[e];s?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),la.innerHTML="",s(t)):(la.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function fd(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),s=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),o=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(o&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!o&&!n))try{const i=await Cr();o&&s&&(s.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),o&&s&&(s.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function vd(e){try{we.getPlatform()==="android"&&(await ie.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await ie.checkPermissions();if(t.receive==="prompt"&&(t=await ie.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await ie.register(),ie.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const s=me(V,"users",e);await Ea(s,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(s){console.error("Erro ao salvar token FCM:",s)}}),ie.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),ie.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),ie.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),ee("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}async function hd(){try{await Cs(z,Ls),console.log("Persistência LOCAL configurada na inicialização.")}catch(e){console.error("Erro ao definir persistência no main.js",e)}we.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),_s(),Bo.addEventListener("click",e=>{e.stopPropagation(),kt.classList.toggle("hidden"),kt.classList.contains("hidden")||(ot.forEach(t=>t.read=!0),$a())}),md.addEventListener("click",()=>{Js()}),ca.addEventListener("click",e=>{e.stopPropagation(),te.classList.toggle("active"),te.classList.contains("active")?te.classList.remove("hidden"):setTimeout(()=>te.classList.add("hidden"),200)}),Do&&Do.addEventListener("click",e=>{e.preventDefault(),ee("my-profile-section"),te.classList.remove("active"),te.classList.add("hidden")}),document.addEventListener("click",e=>{!kt.contains(e.target)&&e.target!==Bo&&kt.classList.add("hidden"),!te.contains(e.target)&&e.target!==ca&&te.classList.contains("active")&&(te.classList.remove("active"),setTimeout(()=>te.classList.add("hidden"),200))}),Ao(z,async e=>{if(e){console.log("Usuário detectado:",e.email),we.isNativePlatform()||(console.log("Inicializando Web Push (PWA)..."),Uo());try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const s=await Ee(a.establishmentId);m.enabledModules=s.modules,gd(s.themeColor||"indigo");let r=null,o=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const d=me(V,"users",e.uid),c=await Ps(d);if(c.exists()){const u=c.data();r=a.role==="employee"?u.permissions||{}:null,o=u.name||o,n=u.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}m.userProfessionalId=n,we.isNativePlatform()&&vd(e.uid);const i=o||e.email;Ns(a.establishmentId,s.name,r),ca.textContent=i.charAt(0).toUpperCase(),dd.textContent=i,cd.textContent=e.email;const l=()=>{at&&at(),Ja(z).then(()=>window.location.href="/login.html")};ud.addEventListener("click",d=>{d.preventDefault(),l()}),Qs(ee,r,m.enabledModules),fd(r),bd(a.establishmentId),$a(),wt.classList.add("fade-out"),ia.style.display="flex",setTimeout(()=>{wt.style.display="none"},500),console.log("Verificando Onboarding..."),setTimeout(()=>{cr()},1500),ee("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),wt.classList.add("fade-out"),setTimeout(()=>{wt.style.display="none"},500),ia.innerHTML=`
                ...
                `,ia.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{Ja(z).then(()=>window.location.href="/login.html")})}}else window.location.href="/login.html"})}hd();Ao(z,e=>{e&&Uo()});
