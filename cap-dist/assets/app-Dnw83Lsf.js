const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-D_Jyh1D1.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/firebase-config-C2tbVz-J.js","assets/styles-C2XZ_b0c.css"])))=>i.map(i=>d[i]);
import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{a as U,d as _,m as Ra}from"./firebase-config-C2tbVz-J.js";import{EmailAuthProvider as Cs,reauthenticateWithCredential as Ls,verifyBeforeUpdateEmail as Ts,updatePassword as Ps,updateProfile as Bs,setPersistence as Ds,browserLocalPersistence as Ms,onAuthStateChanged as Lo,signOut as ja}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{doc as me,getDoc as To,updateDoc as ha,setDoc as As,addDoc as Po,collection as pe,query as Rt,where as rt,getDocs as xa,orderBy as Bo,writeBatch as Do,serverTimestamp as Fa,deleteDoc as qs,arrayUnion as Ns,onSnapshot as Rs}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import{getToken as js,onMessage as Fs}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-messaging.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-storage.js";const m={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,establishmentSettings:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function Hs(e,t,a){m.establishmentId=e,m.establishmentName=t,m.userPermissions=a}const Mo=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1",ia=Mo?"http://localhost:8080":"https://kairos-app-407358446276.us-central1.run.app";console.log(`🚀 API configurada para modo: ${Mo?"LOCAL (Dev)":"PRODUÇÃO (Cloud)"}`);console.log("📡 URL Base:",ia);async function Os(){const e=U.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function S(e,t={}){const a=await Os();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const s=ia.replace(/\/$/,""),r=e.startsWith("/")?e:`/${e}`,o=`${s}${r}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${o}`);try{const n=await fetch(o,{...t,headers:{...a,...t.headers}});if(!n.ok){const l=(await n.json().catch(()=>({message:n.statusText}))).message||`Erro na API: ${n.status}`;if(l.includes("FAILED_PRECONDITION")&&l.includes("requires an index")){const d=/(https:\/\/[^\s]+)/,c=l.match(d),u=c?c[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${o}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${o}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${ia}. Verifique se o servidor backend está rodando.`):n}}const Ao=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),S(r)},zs=({establishmentId:e,professionalId:t,serviceIds:a,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${s}`;return S(r)},Vs=e=>S("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Us=(e,t)=>S(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),Ha=e=>S(`/api/appointments/${e}`,{method:"DELETE"}),_s=e=>S(`/api/appointments/${e}/reopen`,{method:"POST"}),Ws=(e,t)=>S(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let W;async function Js(){if(!W)try{W=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Gs(){if(!W){console.warn("AudioContext não inicializado. O som não será tocado.");return}W.state==="suspended"&&W.resume();const e=W.createOscillator(),t=W.createGain();e.connect(t),t.connect(W.destination),e.type="sine",e.frequency.setValueAtTime(800,W.currentTime),t.gain.setValueAtTime(0,W.currentTime),t.gain.linearRampToValueAtTime(.3,W.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,W.currentTime+.2),e.start(W.currentTime),e.stop(W.currentTime+.2)}function g(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&Gs();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;const l=r.querySelector("[data-close-modal]");l&&(l.onclick=n);const d=r.querySelector('[data-action="close-modal"]');return d&&(d.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function jt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function Ys(){document.body.addEventListener("click",()=>{W||Js()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const s=t.dataset.target;if(s){const r=document.getElementById(s);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const s=document.getElementById("genericModal");s&&(s.style.display="none")}})}const Q=document.getElementById("sidebar"),Oa=document.getElementById("sidebarToggle"),Qe=document.getElementById("mainContent"),Qs=document.querySelectorAll(".sidebar-link"),za=document.getElementById("hamburger-menu-btn"),Ne=document.getElementById("mobile-overlay");function wt(e){!Q||!Qe||(Q.classList.toggle("collapsed",e),Qe.classList.toggle("sidebar-collapsed-shift",e))}function Xs(){!Q||!Ne||(Q.classList.add("mobile-open"),Ne.classList.add("visible"))}function mt(){!Q||!Ne||(Q.classList.remove("mobile-open"),Ne.classList.remove("visible"))}function Zs(){wt(!Q.classList.contains("collapsed"))}function Ks(e,t,a){if(!Q||!Qe)return;Qe.classList.add("main-content-shift"),window.innerWidth>=768?wt(Q.classList.contains("collapsed")):(Qe.classList.remove("main-content-shift","sidebar-collapsed-shift"),mt()),Oa&&Oa.addEventListener("click",r=>{r.stopPropagation(),Zs()}),Q.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&Q.classList.contains("collapsed")&&wt(!1)}),Q.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||wt(!0))}),za&&za.addEventListener("click",r=>{r.stopPropagation(),Xs()}),Ne&&Ne.addEventListener("click",r=>{r.stopPropagation(),mt()});let s=0;Q.addEventListener("touchstart",r=>{s=r.changedTouches[0].screenX},{passive:!0}),Q.addEventListener("touchend",r=>{const o=r.changedTouches[0].screenX;s-o>50&&mt()},{passive:!0}),Qs.forEach(r=>{const o=r.getAttribute("data-target"),n=o.replace("-section",""),i=a?.[n]!==!1,l=t===null||t[o]?.view===!0;if(!i||!l){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",d=>{d.preventDefault(),o&&typeof e=="function"&&e(o),window.innerWidth<768&&mt()})})}const Se=e=>{const t=e||m.establishmentId;return t?S(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},kt=(e,t)=>{const a=e||m.establishmentId;return a?S(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},er=(e,t)=>{const a=e||m.establishmentId;return a?S(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},tr=(e,t)=>{const a=e||m.establishmentId;return a?S(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},K=e=>S(`/api/professionals/${e}`),ar=e=>S(`/api/professionals/details/${e}`),qo=e=>S("/api/professionals",{method:"POST",body:JSON.stringify(e)}),Ct=(e,t)=>S(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),Va=(e,t)=>Ct(e,{services:t}),No=e=>S(`/api/professionals/${e}`,{method:"DELETE"}),or=e=>{const t=e.map(a=>No(a));return Promise.all(t)},$e=e=>S(`/api/services/${e}`),Ro=e=>S("/api/services",{method:"POST",body:JSON.stringify(e)}),sr=(e,t)=>S(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),rr=e=>S(`/api/services/${e}`,{method:"DELETE"}),nr=(e,t)=>S(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),ir=e=>S(`/api/services/stats/most-popular/${e}`),Ft=e=>S(`/api/products/${e}`),jo=e=>S("/api/products",{method:"POST",body:JSON.stringify(e)}),lr=(e,t)=>S(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),dr=e=>S(`/api/products/${e}`,{method:"DELETE"}),cr=(e,t)=>S(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),ur=({startDate:e,endDate:t,productId:a,categoryId:s,establishmentId:r})=>{const o=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&o.append("productId",a),s&&s!=="all"&&o.append("categoryId",s),r&&o.append("establishmentId",r),S(`/api/products/stock-history/report?${o.toString()}`)},mr={indigo:{name:"Padrão (Índigo)",main:"#4f46e5"},blue:{name:"Azul",main:"#2563eb"},sky:{name:"Céu",main:"#0284c7"},teal:{name:"Verde Água",main:"#0d9488"},emerald:{name:"Esmeralda",main:"#059669"},green:{name:"Verde",main:"#16a34a"},lime:{name:"Lima",main:"#65a30d"},amber:{name:"Âmbar",main:"#d97706"},orange:{name:"Laranja",main:"#ea580c"},red:{name:"Vermelho",main:"#dc2626"},rose:{name:"Rosa",main:"#e11d48"},pink:{name:"Pink",main:"#db2777"},fuchsia:{name:"Fúcsia",main:"#c026d3"},purple:{name:"Roxo",main:"#7c3aed"},violet:{name:"Violeta",main:"#8b5cf6"},gray:{name:"Cinza",main:"#4b5563"},black:{name:"Preto",main:"#111827"}};function Ua(e,t,a){return new Promise((s,r)=>{const o=new FileReader;o.readAsDataURL(e),o.onload=n=>{const i=new Image;i.src=n.target.result,i.onload=()=>{const l=document.createElement("canvas");let d=i.width,c=i.height;d>t&&(c*=t/d,d=t),l.width=d,l.height=c,l.getContext("2d").drawImage(i,0,0,d,c);const p=e.type==="image/png"&&t<500?"image/png":"image/jpeg";s(l.toDataURL(p,a))},i.onerror=l=>r(l)},o.onerror=n=>r(n)})}let ce=null;const St=[{id:"company_data",title:"Identidade do Negócio",icon:"🏢",description:"Configure os dados da sua empresa."},{id:"branding",title:"Sua Marca",icon:"🎨",description:"Logo e cores (Opcional)."},{id:"time_config",title:"O Relógio",icon:"⏱️",description:"Tempo padrão entre agendamentos."},{id:"first_service",title:"O Menu",icon:"✂️",description:"Seu principal serviço."},{id:"first_prof",title:"Sua Equipe",icon:"💇",description:"Cadastre o primeiro profissional."},{id:"first_product",title:"O Estoque",icon:"🧴",description:"Cadastre um produto (opcional)."}];let ee=0,Lt=[];async function pr(){try{console.log("Iniciando verificação de Onboarding para ID:",m.establishmentId);const e=await Se(m.establishmentId),t=await K(m.establishmentId),a=await $e(m.establishmentId);Lt=a||[];const s=e&&e.name&&(e.phone||e.address),r=e&&(e.logo||e.themeColor&&e.themeColor!=="indigo"),o=e&&e.slotInterval>0,n=a&&a.length>0,i=t&&t.length>0;if(console.log("Status Onboarding:",{hasCompanyData:s,hasBranding:r,hasTimeConfig:o,hasService:n,hasProf:i}),s&&o&&i&&n)return;if(!s)ee=0;else if(!r&&!o)ee=1;else if(!o)ee=2;else if(!n)ee=3;else if(!i)ee=4;else if(ee===0)return;gr(),wa(ee)}catch(e){console.error("Erro ao verificar onboarding:",e)}}function gr(){document.getElementById("onboarding-overlay")||(ce=document.createElement("div"),ce.id="onboarding-overlay",ce.className="fixed inset-0 bg-gray-900 bg-opacity-95 z-[9999] flex items-center justify-center p-4 overflow-y-auto",ce.style.cssText="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: rgba(17, 24, 39, 0.95); z-index: 9999; display: flex; align-items: center; justify-content: center;",ce.innerHTML=`
    ...
    `,document.body.appendChild(ce),ya())}function ya(){const e=Math.round(ee/St.length*100),t=document.getElementById("progress-bar"),a=document.getElementById("progress-text");t&&(t.style.width=`${e}%`),a&&(a.innerText=`${e}%`)}function wa(e){const t=document.getElementById("onboarding-step-content"),a=St[e];if(!a){_a(t);return}let s="";if(a.id==="company_data")s=`
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="themeColor" class="mt-1 w-full p-2 border rounded text-sm bg-white">
                            ${Object.entries(mr).map(([o,n])=>`<option value="${o}">${n.name}</option>`).join("")}
                        </select>
                    </div>
                    <div>
        ...
        `;else if(a.id==="first_prof"){const r=Lt.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""),o=Lt.length>0;s=`
            <form id="step-form" class="space-y-3">
    ...
    `,document.getElementById("next-step-btn").addEventListener("click",()=>br(a.id)),document.getElementById("skip-btn")&&document.getElementById("skip-btn").addEventListener("click",()=>{e===St.length-1?_a(t):(ee++,ya(),wa(ee))}),a.id==="branding"){const r=document.getElementById("logo-input"),o=document.getElementById("bg-input");r&&(r.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Ua(i,200,.8);document.getElementById("logo-base64").value=l,document.getElementById("logo-preview").innerHTML=`<img src="${l}" class="w-full h-full object-contain rounded">`}catch(l){console.error("Erro logo",l)}}),o&&(o.onchange=async n=>{const i=n.target.files[0];if(i)try{const l=await Ua(i,1024,.7);document.getElementById("bg-base64").value=l}catch(l){console.error("Erro bg",l)}})}}function _a(e){e.innerHTML=`
    ...
    `;const t=document.getElementById("progress-bar"),a=document.getElementById("progress-text");t&&(t.style.width="100%"),a&&(a.innerText="100%"),document.getElementById("finish-onboarding-btn").onclick=()=>{ce&&ce.remove(),window.location.reload()}}async function br(e){const t=document.getElementById("step-form");if(!t.reportValidity())return;const a=document.getElementById("next-step-btn"),s=a.innerHTML;a.disabled=!0,a.innerHTML="Salvando...";const r=new FormData(t),o=Object.fromEntries(r.entries());try{if(e==="company_data")await kt(m.establishmentId,{name:o.name,phone:o.phone,email:o.email,address:o.address,zipCode:o.zipCode});else if(e==="branding"){const n={};o.logoBase64&&(n.logo=o.logoBase64),o.bgBase64&&(n.backgroundImage=o.bgBase64),o.themeColor&&(n.themeColor=o.themeColor),o.primaryColor&&(n.primaryColor=o.primaryColor),Object.keys(n).length>0&&await kt(m.establishmentId,n)}else if(e==="time_config"){const n=parseInt(o.slotInterval);await kt(m.establishmentId,{slotInterval:n})}else if(e==="first_service"){const n=await Ro({establishmentId:m.establishmentId,name:o.name,price:parseFloat(o.price),duration:parseInt(o.duration),active:!0});n&&Lt.push(n)}else if(e==="first_prof"){const n=await qo({establishmentId:m.establishmentId,name:o.name,specialty:o.role,active:!0,commissionRate:0});if(o.serviceId&&n&&n.id)try{Va?await Va(n.id,[o.serviceId]):Ct&&await Ct(n.id,{services:[o.serviceId]})}catch(i){console.warn("Não foi possível vincular o serviço automaticamente.",i)}}else e==="first_product"&&await jo({establishmentId:m.establishmentId,name:o.name,price:parseFloat(o.salePrice),stock:parseInt(o.stock),active:!0});g("Sucesso","Passo concluído!","success"),ee++,ya(),wa(ee)}catch(n){g("Erro","Erro ao salvar: "+n.message,"error"),a.disabled=!1,a.innerHTML=s}}var Re;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Re||(Re={}));class Xt extends Error{constructor(t,a,s){super(t),this.message=t,this.code=a,this.data=s}}const fr=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},vr=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},s=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:fr(e),o=()=>r()!=="web",n=u=>{const p=d.get(u);return!!(p?.platforms.has(r())||i(u))},i=u=>{var p;return(p=a.PluginHeaders)===null||p===void 0?void 0:p.find(b=>b.name===u)},l=u=>e.console.error(u),d=new Map,c=(u,p={})=>{const b=d.get(u);if(b)return console.warn(`Capacitor plugin "${u}" already registered. Cannot register plugins twice.`),b.proxy;const h=r(),f=i(u);let y;const k=async()=>(!y&&h in p?y=typeof p[h]=="function"?y=await p[h]():y=p[h]:t!==null&&!y&&"web"in p&&(y=typeof p.web=="function"?y=await p.web():y=p.web),y),$=(P,B)=>{var R,j;if(f){const z=f?.methods.find(N=>B===N.name);if(z)return z.rtype==="promise"?N=>a.nativePromise(u,B.toString(),N):(N,Y)=>a.nativeCallback(u,B.toString(),N,Y);if(P)return(R=P[B])===null||R===void 0?void 0:R.bind(P)}else{if(P)return(j=P[B])===null||j===void 0?void 0:j.bind(P);throw new Xt(`"${u}" plugin is not implemented on ${h}`,Re.Unimplemented)}},w=P=>{let B;const R=(...j)=>{const z=k().then(N=>{const Y=$(N,P);if(Y){const Ee=Y(...j);return B=Ee?.remove,Ee}else throw new Xt(`"${u}.${P}()" is not implemented on ${h}`,Re.Unimplemented)});return P==="addListener"&&(z.remove=async()=>B()),z};return R.toString=()=>`${P.toString()}() { [capacitor code] }`,Object.defineProperty(R,"name",{value:P,writable:!1,configurable:!1}),R},E=w("addListener"),T=w("removeListener"),q=(P,B)=>{const R=E({eventName:P},B),j=async()=>{const N=await R;T({eventName:P,callbackId:N},B)},z=new Promise(N=>R.then(()=>N({remove:j})));return z.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await j()},z},F=new Proxy({},{get(P,B){switch(B){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return f?q:E;case"removeListener":return T;default:return w(B)}}});return s[u]=F,d.set(u,{name:u,proxy:F,platforms:new Set([...Object.keys(p),...f?[h]:[]])}),F};return a.convertFileSrc||(a.convertFileSrc=u=>u),a.getPlatform=r,a.handleError=l,a.isNativePlatform=o,a.isPluginAvailable=n,a.registerPlugin=c,a.Exception=Xt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},hr=e=>e.Capacitor=vr(e),oe=hr(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),Ht=oe.registerPlugin;class Fo{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let s=!1;this.listeners[t]||(this.listeners[t]=[],s=!0),this.listeners[t].push(a);const o=this.windowListeners[t];o&&!o.registered&&this.addWindowListener(o),s&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,s){const r=this.listeners[t];if(!r){if(s){let o=this.retainedEventArguments[t];o||(o=[]),o.push(a),this.retainedEventArguments[t]=o}return}r.forEach(o=>o(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:s=>{this.notifyListeners(a,s)}}}unimplemented(t="not implemented"){return new oe.Exception(t,Re.Unimplemented)}unavailable(t="not available"){return new oe.Exception(t,Re.Unavailable)}async removeListener(t,a){const s=this.listeners[t];if(!s)return;const r=s.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(s=>{this.notifyListeners(t,s)}))}}const Wa=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Ja=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class xr extends Fo{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(s=>{if(s.length<=0)return;let[r,o]=s.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Ja(r).trim(),o=Ja(o).trim(),a[r]=o}),a}async setCookie(t){try{const a=Wa(t.key),s=Wa(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,o=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${s||""}${r}; path=${o}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}Ht("CapacitorCookies",{web:()=>new xr});const yr=async e=>new Promise((t,a)=>{const s=new FileReader;s.onload=()=>{const r=s.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},s.onerror=r=>a(r),s.readAsDataURL(e)}),wr=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,o,n)=>(r[o]=e[t[n]],r),{})},kr=(e,t=!0)=>e?Object.entries(e).reduce((s,r)=>{const[o,n]=r;let i,l;return Array.isArray(n)?(l="",n.forEach(d=>{i=t?encodeURIComponent(d):d,l+=`${o}=${i}&`}),l.slice(0,-1)):(i=t?encodeURIComponent(n):n,l=`${o}=${i}`),`${s}&${l}`},"").substr(1):null,Sr=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=wr(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const o=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))o.set(n,i);a.body=o.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const o=new FormData;if(e.data instanceof FormData)e.data.forEach((i,l)=>{o.append(l,i)});else for(const i of Object.keys(e.data))o.append(i,e.data[i]);a.body=o;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class $r extends Fo{async request(t){const a=Sr(t,t.webFetchExtra),s=kr(t.params,t.shouldEncodeUrlParams),r=s?`${t.url}?${s}`:t.url,o=await fetch(r,a),n=o.headers.get("content-type")||"";let{responseType:i="text"}=o.ok?t:{};n.includes("application/json")&&(i="json");let l,d;switch(i){case"arraybuffer":case"blob":d=await o.blob(),l=await yr(d);break;case"json":l=await o.json();break;case"document":case"text":default:l=await o.text()}const c={};return o.headers.forEach((u,p)=>{c[p]=u}),{data:l,headers:c,status:o.status,url:o.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}Ht("CapacitorHttp",{web:()=>new $r});const G=Ht("PushNotifications",{}),Er="modulepreload",Ir=function(e){return"/"+e},Ga={},Cr=function(t,a,s){let r=Promise.resolve();if(a&&a.length>0){let l=function(d){return Promise.all(d.map(c=>Promise.resolve(c).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const n=document.querySelector("meta[property=csp-nonce]"),i=n?.nonce||n?.getAttribute("nonce");r=l(a.map(d=>{if(d=Ir(d),d in Ga)return;Ga[d]=!0;const c=d.endsWith(".css"),u=c?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${u}`))return;const p=document.createElement("link");if(p.rel=c?"stylesheet":Er,c||(p.as="script"),p.crossOrigin="",p.href=d,i&&p.setAttribute("nonce",i),document.head.appendChild(p),c)return new Promise((b,h)=>{p.addEventListener("load",b),p.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${d}`)))})}))}function o(n){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=n,window.dispatchEvent(i),!i.defaultPrevented)throw n}return r.then(n=>{for(const i of n||[])i.status==="rejected"&&o(i.reason);return t().catch(o)})},Ya=Ht("App",{web:()=>Cr(()=>import("./web-D_Jyh1D1.js"),__vite__mapDeps([0,1,2,3])).then(e=>new e.AppWeb)}),Lr="BDA-IaH_jjWBRwHbuFcB56I25jKHpchx34yZtv_6iIo_yV2tz_yIZYS3hfntDaN5Slf4ch8ZEJCIt4D7LIWX4mY";let Qa=!1;async function Ho(){if(window.Capacitor&&window.Capacitor.isNativePlatform()){try{await G.removeAllListeners(),await G.addListener("registration",async a=>{Vo(a.value,!0)}),await G.addListener("pushNotificationReceived",a=>{console.log("[Push Nativo] Recebido em Foreground (Visual gerido pelo Firestore):",a)}),await G.addListener("pushNotificationActionPerformed",a=>{const s=a.notification.data;console.log("Notificação clicada (Ação):",s)});let t=await G.checkPermissions();t.receive==="prompt"&&(t=await G.requestPermissions()),t.receive==="granted"&&await G.register()}catch(t){console.error("[Push Nativo] Erro:",t)}return}"Notification"in window&&Notification.permission==="granted"&&zo()}async function Oo(){try{return await Notification.requestPermission()==="granted"?(console.log("Permissão concedida pelo utilizador."),await zo(),!0):(console.warn("Permissão de notificações negada."),!1)}catch(e){return console.error("Erro ao pedir permissão Web:",e),!1}}async function zo(){if("serviceWorker"in navigator)try{const e=await navigator.serviceWorker.register("/firebase-messaging-sw.js");await e.update();const t=await js(Ra,{vapidKey:Lr,serviceWorkerRegistration:e});t?(console.log("[Push Web] Token validado."),await Vo(t,!1)):console.warn("[Push Web] Token veio vazio."),Qa||(Fs(Ra,a=>{console.log("[Push Web] Recebido em Foreground (Silencioso):",a)}),Qa=!0)}catch(e){console.error("[Push Web] Falha no registo:",e)}else console.warn("Navegador sem suporte a Service Worker.")}async function Vo(e,t){const a=U.currentUser;if(!a){console.warn("Usuário não logado. Token não salvo.");return}const s=me(_,"users",a.uid);try{const r=await To(s);if(r.exists()){const n=r.data().fcmTokens||[];if(n.length===1&&n[0]===e){console.log("Token já sincronizado e único. Nenhuma ação necessária.");return}}await ha(s,{fcmTokens:[e],lastLoginAt:new Date().toISOString(),platform:t?"android_native":"pwa_web"}),console.log("Token atualizado (Sessão Única garantida).")}catch(r){if(r.code==="not-found")try{await As(s,{email:a.email,fcmTokens:[e],platform:t?"android_native":"pwa_web",createdAt:new Date().toISOString()},{merge:!0}),console.log("Utilizador criado e token inicial salvo.")}catch(o){console.error("Erro ao criar user:",o)}else console.error("Erro ao atualizar token:",r)}}const Tr=(e,t,a="all",s="all")=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("professionalId",a),s&&s!=="all"&&r.append("costCenterId",s),S(`/api/reports/indicators?${r.toString()}`)},Pr=e=>e?S(`/api/financial/cost-centers/${e}`):Promise.resolve([]),Br=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:s})=>{const r=new URLSearchParams({startDate:t,endDate:a});return s&&s!=="all"&&r.append("cashierSessionId",s),e&&r.append("establishmentId",e),S(`/api/reports/sales?${r.toString()}`)},Dr=()=>S("/api/reports/summary",{method:"GET"}),Ot=(e,t,a,s="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${s}`;return S(r)},zt=e=>S("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ka=e=>S(`/api/blockages/${e}`,{method:"DELETE"}),Uo=e=>S("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),_o=e=>e?String(e).replace(/\D/g,""):"",lt=(e,t="",a=20,s={})=>{const r=new URLSearchParams;return t&&r.append("search",t),a&&r.append("limit",a),s&&s.hasLoyalty&&r.append("hasLoyalty","true"),s&&s.birthMonth&&r.append("birthMonth",s.birthMonth),s&&s.inactiveDays&&r.append("inactiveDays",s.inactiveDays),S(`/api/clients/${e}?${r.toString()}`)},Wo=(e,t)=>{const a=encodeURIComponent(t);return S(`/api/clients/details/${e}/${a}`)},Jo=e=>{const t=e.phone||e.id;if(!t)throw new Error("Telefone é obrigatório");const a=_o(t),s={...e,phone:a,id:a};return S(`/api/clients/${a}`,{method:"PUT",body:JSON.stringify(s)})},Sa=Jo,Mr=(e,t)=>Jo({...t,id:e}),Ar=e=>{const t=encodeURIComponent(e);return S(`/api/clients/${t}`,{method:"DELETE"})},qr=(e,t)=>Wo(e,_o(t));function v(e){return e==null?"":String(e).replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t])}function Go(e,t=800,a=800,s=.7){return new Promise((r,o)=>{if(!e.type.match(/image.*/))return o(new Error("O ficheiro selecionado não é uma imagem."));const n=new FileReader;n.readAsDataURL(e),n.onload=i=>{const l=new Image;l.src=i.target.result,l.onload=()=>{let d=l.width,c=l.height;d>c?d>t&&(c*=t/d,d=t):c>a&&(d*=a/c,c=a);const u=document.createElement("canvas");u.width=d,u.height=c,u.getContext("2d").drawImage(l,0,0,d,c);const b=u.toDataURL("image/jpeg",s);r(b)},l.onerror=d=>o(new Error("Erro ao carregar a imagem para processamento."))},n.onerror=i=>o(new Error("Erro ao ler o ficheiro."))})}const Xa=document.getElementById("content");let Za=!1;const la=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let dt=[],Vt=[],je={},Ae=[],C={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null,isSelectionMode:!1,selectedItems:new Set},I={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Nr(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function Yo(e){const t=new Date(e);if(t.setHours(0,0,0,0),C.currentView==="week"&&C.weekViewDays===7){const a=t.getDay(),s=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(s))}return t}function Tt(){const e=document.getElementById("profSelectorContainer"),t=C.profSearchTerm.toLowerCase();if(!e||!m.professionals)return;let a=m.professionals.filter(o=>C.showInactiveProfs||o.status!=="inactive");t&&(a=a.filter(o=>o.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(o=>{const n=C.selectedProfessionalId===o.id,i=o.name==="Todos"?"Todos":o.name.split(" ")[0],l=o.name==="Todos"?"T":o.name.charAt(0).toUpperCase(),d=o.status!=="inactive",c=v(i),u=la[0],p=o.id!=="all"&&m.professionalColors.get(o.id)||u,b=o.photo||`https://placehold.co/64x64/${p.main?.replace("#","")||"E0E7FF"}/${p.light?.replace("#","")||"4F46E5"}?text=${l}`,h=o.id==="all"?"#e0e7ff":p.light,f=o.id==="all"?"#4f46e5":p.main,k=`border: 3px solid ${n?p.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+p.border:"none"};`;return`
            <div class="prof-card ${n?"selected":""} ${d?"":"opacity-50"}" 
                 data-action="select-professional" 
                 data-prof-id="${o.id}">
                ${o.id==="all"?`<div class="prof-card-all-placeholder" style="background-color: ${h}; color: ${f}; ${k}">
                           ${l}
                          </div>`:`<img src="${b}" alt="${c}" class="prof-card-photo" style="${k}" />`}
                <span class="prof-card-name">${c}</span>
            </div>
        `}).join("")}function Rr(e,t,a,s,r){const o=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${s} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,d=encodeURIComponent(l);return`https://wa.me/${o}?text=${d}`}function jr(e){const t=document.getElementById("agenda-view");if(!t)return;if(e.sort((s,r)=>new Date(s.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(s=>{const r=new Date(s.startTime),o=new Date(s.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=o.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),l=m.professionalColors.get(s.professionalId)||{},d=v(s.reason),c=v(s.professionalName),u=v(s.clientName),p=v(s.serviceName),b=C.selectedItems.has(s.id),h=C.isSelectionMode?`<div class="flex items-center justify-center pr-3 border-r border-gray-200 mr-3">
                ...
                </div>`;const f=s.status==="completed",y=f?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",k=f?"Finalizado":"Aberto",$=JSON.stringify(s).replace(/'/g,"&apos;"),w=s.redeemedReward?.points>0,E=s.hasRewards&&!w,T=Rr(s.clientPhone,s.clientName,s.serviceName,s.professionalName,s.startTime),q=C.isSelectionMode?"":'data-action="open-comanda"';return`
            <div class="appointment-list-card" data-appointment='${$}' style="border-left-color: ${l.border};">
                ...
                <div class="status-info">
            ...
            </div>`}).join("");t.innerHTML=`<div class="list-view-container space-y-2 pb-24">${a}</div>`}function $a(){return window.innerWidth<768&&C.currentView==="week"?3:C.weekViewDays}function Fr(e){const t=document.getElementById("agenda-view");if(!t)return;const a=["Domingo","Segunda","Terça","Quarta","Quinta","Sexta","Sábado"],s=Yo(C.currentDate),r=$a();let o=`<div class="grid divide-x divide-gray-200 min-h-[60vh]" style="grid-template-columns: repeat(${r}, minmax(0, 1fr));">`;for(let n=0;n<r;n++){const i=new Date(s);i.setDate(i.getDate()+n);const l=new Date,d=i.toDateString()===l.toDateString(),c=e.filter(p=>new Date(p.startTime).toDateString()===i.toDateString()).sort((p,b)=>new Date(p.startTime)-new Date(b.startTime));let u='<div class="p-1 space-y-2">';c.length>0?u+=c.map(p=>{const h=new Date(p.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),f=m.professionalColors.get(p.professionalId)||{bg:"#e5e7eb",border:"#9ca3af"},y=v(p.reason),k=v(p.professionalName),$=v(p.clientName),w=v(p.serviceName);if(p.type==="blockage")return`
                    ...
                    `;const E=JSON.stringify(p).replace(/'/g,"&apos;"),T=p.redeemedReward?.points>0,q=p.hasRewards&&!T,F=p.status==="completed";return`
        ...
        `}o+="</div>",t.innerHTML=o}function Qo(){const e=m.allEvents.filter(t=>C.selectedProfessionalId==="all"||t.professionalId===C.selectedProfessionalId);C.currentView==="list"?jr(e):Fr(e),Ea()}function Ea(){const e=document.getElementById("batch-delete-container"),t=document.querySelector('[data-action="new-appointment"]');e&&(C.isSelectionMode&&C.selectedItems.size>0?(e.innerHTML=`
        ...
        `,e.style.display="block",t&&(t.style.display="none")):(e.style.display="none",t&&(t.style.display="flex")))}async function ae(){const e=document.getElementById("agenda-view");if(!e)return;C.selectedItems.clear(),Ea(),e.innerHTML='<div class="loader mx-auto my-10"></div>';let t,a;const s=document.getElementById("weekRange");if(s){if(C.currentView==="list")t=new Date(C.currentDate),t.setHours(0,0,0,0),a=new Date(C.currentDate),a.setHours(23,59,59,999),s.textContent=Nr(t);else{const r=$a();t=Yo(new Date(C.currentDate)),a=new Date(t),a.setDate(t.getDate()+(r-1)),a.setHours(23,59,59,999),s.textContent=`${t.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} - ${a.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}`}try{const r=await Ao(m.establishmentId,t.toISOString(),a.toISOString(),C.selectedProfessionalId==="all"?null:C.selectedProfessionalId),o=await Ot(m.establishmentId,t.toISOString(),a.toISOString(),C.selectedProfessionalId);if(!document.getElementById("agenda-view"))return;const n=o.map(l=>{let d=l.professionalName;if(!d&&l.professionalId){const c=m.professionals?m.professionals.find(u=>u.id===l.professionalId):null;c&&(d=c.name)}return{...l,type:"blockage",professionalName:d||"Não identificado"}}),i=[...r.map(l=>({...l,type:"appointment"})),...n];if(m.allEvents=i,Tt(),Qo(),C.scrollToAppointmentId){const l=document.querySelector(`[data-appointment*='"id":"${C.scrollToAppointmentId}"']`);l&&(l.scrollIntoView({behavior:"smooth",block:"center"}),l.style.transition="background-color 0.5s ease-in-out",l.style.backgroundColor="#e0e7ff",setTimeout(()=>{l.style.backgroundColor=""},2500)),C.scrollToAppointmentId=null}}catch(r){document.getElementById("agenda-view")&&(document.getElementById("agenda-view").innerHTML='<div class="p-6 text-center text-red-600">Falha ao carregar dados.</div>',g("Erro na Agenda",`Não foi possível carregar a agenda: ${r.message}`,"error"))}}}async function Hr(){try{const[e,t,a]=await Promise.all([m.professionals&&m.professionals.length>0?Promise.resolve(m.professionals):K(m.establishmentId),m.services&&m.services.length>0?Promise.resolve(m.services):$e(m.establishmentId),je.enabled!==void 0?Promise.resolve(null):Se(m.establishmentId)]);(!m.professionals||m.professionals.length===0)&&(m.professionals=e||[]),(!m.services||m.services.length===0)&&(m.services=t||[]),Ae=[],a&&(je=a.loyaltyProgram||{enabled:!1}),m.professionals.forEach((s,r)=>{m.professionalColors.set(s.id,la[r%la.length])}),Tt()}catch(e){console.error("Erro ao popular filtros e dependências do modal:",e),g("Atenção","Não foi possível pré-carregar os dados para agendamento. A abertura do modal pode ser lenta.","error")}}function da(e){e<1||e>4||(I.step=e,ca(null,!0))}function Xo(e,t){const a=document.getElementById("multiServiceToggle"),s=a&&a.checked,r=t.classList.contains("selected"),o=I.data.selectedServiceIds.indexOf(e);if(r)t.classList.remove("selected","border-blue-500"),o>-1&&I.data.selectedServiceIds.splice(o,1);else{if(!s){I.data.selectedServiceIds=[];const n=document.getElementById("apptServicesContainer");n&&n.querySelectorAll(".service-card.selected").forEach(i=>{i.classList.remove("selected","border-blue-500")})}t.classList.add("selected","border-blue-500"),I.data.selectedServiceIds.push(e)}}function Zo(e,t){const a=document.querySelector(".professional-step-cards");if(!a)return;a.querySelectorAll(".professional-modal-card").forEach(r=>r.classList.remove("selected","border-blue-500")),t.classList.add("selected","border-blue-500");const s=Vt.find(r=>r.id===e);I.data.professionalId=e,I.data.professionalName=s?s.name:"N/A"}function Or(e,t){const a=document.getElementById("availableTimesContainer");a&&(a.querySelectorAll(".time-slot-card").forEach(s=>s.classList.remove("selected")),t.classList.add("selected"),I.data.time=e)}async function Ka(){const e=document.getElementById("apptTotalDuration"),t=document.getElementById("availableTimesContainer");if(!e||!t)return;const a=I.data.professionalId,s=I.data.selectedServiceIds,r=document.getElementById("apptDate").value;I.data.date=r;const o=s.reduce((n,i)=>{const l=dt.find(d=>d.id===i);return n+(l?l.duration+(l.bufferTime||0):0)},0);if(e.textContent=`${o} min`,o===0||!a||!r){t.innerHTML='<p class="col-span-full text-center text-gray-500">Selecione serviço, profissional e data.</p>';return}t.innerHTML='<div class="loader mx-auto col-span-full"></div>';try{let n=await zs({establishmentId:m.establishmentId,professionalId:a,serviceIds:s,date:r});const i=new Date;if(new Date(r+"T00:00:00").toDateString()===i.toDateString()){const d=i.getHours()*60+i.getMinutes();n=n.filter(c=>{const[u,p]=c.split(":").map(Number);return u*60+p>=d})}if(t.innerHTML="",n.length>0){if(n.forEach(d=>{const c=document.createElement("button");c.type="button",c.className=`time-slot-card p-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200 transition ${I.data.time===d?"selected":""}`,c.textContent=d,c.addEventListener("click",()=>Or(d,c)),t.appendChild(c)}),I.data.time){const d=t.querySelector(`[data-action="time-slot"][data-time="${I.data.time}"]`);d&&d.classList.add("selected")}}else t.innerHTML='<p class="col-span-full text-center text-gray-500">Nenhum horário disponível.</p>'}catch(n){console.error("Erro ao buscar horários:",n),t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao buscar horários.</p>'}}function zr(){const e=document.getElementById("loyaltyRewardsContainer");if(!e)return;const{clientHasRewards:t,clientLoyaltyPoints:a,redeemedReward:s}=I.data,{enabled:r,rewards:o}=je;if(!r||!t||!o||o.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden");const n=o.filter(l=>a>=l.points);let i=`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",l=>{l.target.checked&&(I.data.redeemedReward=null)})}async function Vr(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!I.data.time||I.data.selectedServiceIds.length===0||!I.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const s=I.data.selectedServiceIds.map(d=>{const c=dt.find(u=>u.id===d);return{id:c.id,name:c.name,price:c.price,duration:c.duration,bufferTime:c.bufferTime||0,photo:c.photo||null}}),[r,o]=I.data.time.split(":"),n=new Date(`${I.data.date}T${r}:${o}:00`),i={establishmentId:m.establishmentId,clientName:I.data.clientName,clientPhone:I.data.clientPhone,services:s,professionalId:I.data.professionalId,startTime:n.toISOString(),redeemedReward:I.data.redeemedReward},l=t.querySelector("#appointmentId").value;l&&(i.id=l);try{l?await Us(l,i):await Vs(i),g(`Agendamento ${l?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",ae()}catch(d){g(d.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function Ko(e){const t=I.data.clientName===e.name&&I.data.clientPhone===e.phone,a=v(e.name),s=v(e.phone);return`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",_r)}function Jr(){Wr()}function Gr(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",s=`
    ...
    `;return{title:a,content:s}}function Yr(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${dt.map(s=>{const r=I.data.selectedServiceIds.includes(s.id),o=s.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S",n=v(s.name);return`
    ...
    `;return{title:e,content:a}}function Qr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Vt.map(a=>{const s=I.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",o=v(a.name);return`
    ...
    `;return{title:e,content:t}}function Xr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,s=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=I.data.date||s,o=v(I.data.clientName),n=v(I.data.professionalName),i=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:i}}function Zr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),s=dt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=I.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>Xo(r.dataset.serviceId,r))})}function Kr(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),s=Vt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=I.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",i=v(r.name);return`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(o=>{o.addEventListener("click",()=>{const n=parseInt(o.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),l=a.querySelector("#apptClientPhone");if(I.data.clientName=i.value.trim(),I.data.clientPhone=l.value.trim(),!I.data.clientName||!I.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if(I.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!I.data.professionalId)return g("Atenção","Selecione um profissional.","error");da(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(o=>{o.addEventListener("click",()=>da(parseInt(o.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if(I.step===4&&r&&r.addEventListener("submit",Vr),a.style.display="flex",I.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>Xo(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>Zr(i.target.value))}if(I.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>Zo(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>Kr(i.target.value))}if(I.step===1){const o=a.querySelector("#clientSearchInput");if(o&&(o.addEventListener("input",i=>Ur(i.target.value)),I.data.clientName&&I.data.clientPhone&&Ae.length>0)){const i=document.getElementById("clientSearchResults");i&&(i.innerHTML=Ae.map(Ko).join(""))}const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",Jr)}if(I.step===4){const o=a.querySelector("#apptDate");o&&o.addEventListener("change",Ka),Ka(),zr()}}async function es(e={}){C.currentDate=e.targetDate?new Date(e.targetDate):C.currentDate||new Date,C.scrollToAppointmentId=e.scrollToAppointmentId||null,C.profSearchTerm="",C.isSelectionMode=!1,C.selectedItems.clear(),window.innerWidth<768&&(C.currentView="list"),Xa.innerHTML=`
        <section>
        ...
        </section>`;const t=document.getElementById("btn-toggle-select");t.addEventListener("click",()=>{C.isSelectionMode=!C.isSelectionMode,C.isSelectionMode||C.selectedItems.clear(),t.classList.toggle("bg-blue-100",C.isSelectionMode),t.classList.toggle("text-blue-700",C.isSelectionMode),Qo()}),document.querySelectorAll(".view-btn[data-view]").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(o=>o.classList.remove("active")),s.classList.add("active"),C.currentView=s.dataset.view;const r=document.getElementById("week-days-toggle");if(C.currentView==="week"){if(r.style.display="flex",window.innerWidth<768){C.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(n=>n.classList.remove("active"));const o=document.querySelector('.week-days-btn[data-days="3"]');o&&o.classList.add("active")}}else r.style.display="none";ae()})}),document.querySelectorAll(".week-days-btn").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(r=>r.classList.remove("active")),s.classList.add("active"),C.weekViewDays=parseInt(s.dataset.days,10),ae()})}),document.getElementById("todayBtn").addEventListener("click",()=>{C.currentDate=new Date,ae()});const a=s=>{const r=parseInt(s.currentTarget.dataset.amount,10),o=C.currentView==="week"?$a():1,n=new Date(C.currentDate);n.setDate(n.getDate()+r*o),C.currentDate=n,ae()};document.getElementById("prevBtn").addEventListener("click",a),document.getElementById("nextBtn").addEventListener("click",a),document.getElementById("profSearchInput").addEventListener("input",s=>{C.profSearchTerm=s.target.value,Tt()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",s=>{C.showInactiveProfs=s.target.checked,Tt(),ae()}),Za||(Xa.addEventListener("click",async s=>{const r=s.target.closest("[data-action]");if(s.target.dataset.action==="toggle-select-item"){const l=s.target.dataset.id;s.target.checked?C.selectedItems.add(l):C.selectedItems.delete(l),Ea();return}if(r&&r.dataset.action==="batch-delete"){const l=C.selectedItems.size;if(await O("Excluir em Lote",`Tem certeza que deseja excluir ${l} agendamento(s)? Esta ação não pode ser desfeita.`)){const c=Array.from(C.selectedItems);let u=0;try{await Promise.all(c.map(async p=>{try{await Ha(p),u++}catch(b){console.error(`Falha ao excluir ${p}`,b)}})),g(`${u} agendamento(s) excluído(s).`,"success"),C.selectedItems.clear(),C.isSelectionMode=!1,document.getElementById("btn-toggle-select").classList.remove("bg-blue-100","text-blue-700"),ae()}catch{g("Erro ao processar exclusão em lote.","error")}}return}if(s.target.closest('[data-action="select-professional"]')){const d=s.target.closest('[data-action="select-professional"]').dataset.profId,c=C.selectedProfessionalId===d&&d!=="all";if(C.selectedProfessionalId=c?"all":d,d!=="all"){const u=document.getElementById("profSearchInput");u&&(u.value=""),C.profSearchTerm=""}await ae();return}if(!r)return;const o=r.dataset.action;let n=null;const i=s.target.closest("[data-appointment]");switch(i&&(n=JSON.parse(i.dataset.appointment.replace(/&apos;/g,"'"))),o){case"new-appointment":ca();break;case"edit-appointment":if(C.isSelectionMode||!n)return;if(n.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}n.hasRewards&&!n.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),ca(n);break;case"delete-appointment":{if(C.isSelectionMode)return;const l=r.dataset.id;if(await O("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await Ha(l),g("Agendamento apagado!","success"),ae()}catch(c){g(`Não foi possível apagar: ${c.message}`,"error")}break}case"open-comanda":if(C.isSelectionMode)return;if(n){n.hasRewards&&!n.redeemedReward&&n.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const l=n.status==="completed"?"finalizadas":"em-atendimento",d={selectedAppointmentId:n.id,initialFilter:l};if(l==="finalizadas"){let c=n.startTime;if(n.transaction&&n.transaction.paidAt){const u=n.transaction.paidAt;typeof u=="object"&&u._seconds?c=new Date(u._seconds*1e3):c=u}d.filterDate=c}Z("comandas-section",d)}break}}),Za=!0),await Hr(),await ae()}const en=(e,t=null,a=1,s=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${s}`;return t&&(r+=`&date=${t}`),S(r)},tn=(e,t)=>S(`/api/appointments/${e}/comanda`,{method:"POST",body:JSON.stringify({items:t})}),an=e=>S("/api/sales",{method:"POST",body:JSON.stringify(e)}),on=e=>S(`/api/sales/${e}`,{method:"DELETE"}),sn=()=>S("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),rn=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),S("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},nn=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),S(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},ln=()=>S("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),dn=e=>S(`/api/cashier/report/${e}`),ts=e=>S(`/api/packages/${e}`),cn=e=>S("/api/packages",{method:"POST",body:JSON.stringify(e)}),un=(e,t)=>S(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),mn=e=>S(`/api/packages/${e}`,{method:"DELETE"});let x={allComandas:[],catalog:{services:[],products:[],packages:[]},activeFilter:"atendimento",selectedComandaId:null,viewMode:"items",isCashierOpen:!1,activeCashierSessionId:null,loyaltySettings:null,pendingRedemption:null,paging:{page:1,limit:10,total:0},checkoutState:{payments:[],selectedMethod:"dinheiro",installments:1,amountReceived:"",discount:{type:"real",value:0},discountReason:""}},Ie=null,Le=null,eo=null;function as(e,t){return function(...a){clearTimeout(eo),eo=setTimeout(()=>e.apply(this,a),t)}}async function to(e,t="stay"){if(!e||!e.id)return;e._localUpdatedAt=Date.now(),e._cachedItems=null,e._hasUnsavedChanges=!1,nt(),t==="checkout"&&(x.viewMode="checkout",x.checkoutState.payments||(x.checkoutState.payments=[]),x.checkoutState.selectedMethod="dinheiro",x.checkoutState.amountReceived="",x.checkoutState.discount.value||(x.checkoutState.discount={type:"real",value:0},x.checkoutState.discountReason=""),V());const a=document.createElement("div");a.id="saving-overlay",a.className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm",a.innerHTML=`
    ...
    `,Ut()}function Ut(){const e=document.getElementById("cashier-alert-box"),t=document.getElementById("btn-new-sale");x.isCashierOpen?(e&&(e.innerHTML=""),t&&(t.classList.remove("opacity-50","cursor-not-allowed"),t.disabled=!1)):(e&&(e.innerHTML=`
        ...
        `),t&&(t.classList.add("opacity-50","cursor-not-allowed"),t.disabled=!0)),bn()}function bn(){const e=document.getElementById("cashier-controls");e&&(x.isCashierOpen?e.innerHTML=`
        ...
        `,t&&(t.innerHTML="");return}const s={atendimento:"confirmed",finalizadas:"completed"}[x.activeFilter],r=x.allComandas.filter(n=>n.status===s);if(r.length===0){e.innerHTML='<p class="text-center text-gray-400 py-10 text-sm">Nenhuma venda encontrada.</p>',ao(t);return}const o=document.createDocumentFragment();r.forEach(n=>{const l=ke(n).reduce((y,k)=>y+Number(k.price||0),0),d=n.id===x.selectedComandaId,c=new Date(n.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),u=n.type==="walk-in"||typeof n.id=="string"&&n.id.startsWith("temp-"),p=v(n.clientName||"Cliente sem nome"),b=v(n.professionalName||"Sem profissional"),h=u?'<span class="text-[10px] font-bold uppercase text-blue-600 bg-blue-100 px-2 py-0.5 rounded-md border border-blue-200">Avulso</span>':'<span class="text-[10px] font-bold uppercase text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-md border border-indigo-200">Agenda</span>',f=document.createElement("div");f.className=`comanda-card cursor-pointer ${d?"selected":""}`,f.dataset.action="select-comanda",f.dataset.comandaId=n.id,f.innerHTML=`
        ...
        `,o.appendChild(f)}),e.innerHTML="",e.appendChild(o),ao(t)}function ao(e){if(!e)return;e.innerHTML="";const{page:t,total:a,limit:s}=x.paging,r=Math.ceil((a||0)/s);if(r===0)return;const o=document.createElement("div");o.className="flex gap-2 justify-center items-center w-full",o.innerHTML=`
    ...
    `;if(!x.isCashierOpen){e.innerHTML=`
        ...
        `;return}const s=ke(t),r=t.status==="completed",o=t.type==="walk-in"||typeof t.id=="string"&&t.id.startsWith("temp-"),n=s.reduce((f,y)=>{const k=y._source==="original_service",$=y.id||y.name,w=k?`original-${$}`:`${y.type}-${$}`;return f[w]||(f[w]={...y,quantity:0,sources:[]}),f[w].quantity+=1,y._source&&f[w].sources.push(y._source),f},{}),i=Object.values(n).reduce((f,y)=>f+Number(y.price||0)*y.quantity,0),l=v(t.clientName||"Cliente sem nome"),d=v(t.professionalName||"Profissional não atribuído"),c=t._hasUnsavedChanges,b=`
    ...
    `;e.innerHTML=`
        ...
        ${r?"":h}
    `,!r&&(t.clientId||t.clientName)&&vn(t,e.querySelector("#loyalty-container"))}function fn(e,t){const s=ke(e).reduce((p,b)=>p+Number(b.price||0)*(b.quantity||1),0),r=x.checkoutState,o=r.discount||{type:"real",value:0};let n=0;o.type==="percent"?n=s*o.value/100:n=o.value,n>s&&(n=s);const i=s-n,l=r.payments.reduce((p,b)=>p+b.value,0),d=Math.max(0,i-l);(!r.amountReceived||d>0)&&(r.amountReceived=d.toFixed(2));const c=`
        <div class="mobile-only-header">
    ...
    `;const u=()=>{const p=x.checkoutState.discount.type,b=x.checkoutState.discount.value;let h=p==="percent"?s*b/100:b;h>s&&(h=s);const f=s-h,y=x.checkoutState.payments.reduce((T,q)=>T+q.value,0),k=Math.max(0,f-y),$=t.querySelector("#checkout-total-display");$&&($.textContent=`R$ ${f.toFixed(2)}`);const w=t.querySelector("#checkout-status-msg");w&&(k<=.01?w.innerHTML='<p class="text-green-600 font-bold text-lg">Pago</p>':w.innerHTML=`<p class="text-red-500 font-medium">Faltam: <span id="checkout-remaining-display">R$ ${k.toFixed(2)}</span></p>`);const E=t.querySelector("#checkout-amount");E&&k>0&&document.activeElement!==E&&(E.value=k.toFixed(2))};t.querySelector("#discount-value")?.addEventListener("input",p=>{const b=parseFloat(p.target.value)||0;x.checkoutState.discount.value=b,u()}),t.querySelector("#discount-type")?.addEventListener("change",p=>{x.checkoutState.discount.type=p.target.value,u()}),t.querySelector("#discount-reason")?.addEventListener("input",p=>{x.checkoutState.discountReason=p.target.value}),t.querySelector("#checkout-amount")?.addEventListener("input",p=>{x.checkoutState.amountReceived=p.target.value}),t.querySelector("#checkout-installments")?.addEventListener("change",p=>{x.checkoutState.installments=parseInt(p.target.value,10)})}async function vn(e,t){if(!t)return;const a=x.loyaltySettings;if(!a||!a.enabled)return;let s=null;try{if(e.clientId)s=await Wo(m.establishmentId,e.clientId);else if(e.clientName){const i=await lt(m.establishmentId,e.clientName,1);i&&i.length>0&&(s=i[0])}}catch(i){console.warn("Erro ao buscar dados de fidelidade",i)}if(!s||s.loyaltyPoints===void 0)return;const r=Number(s.loyaltyPoints)||0,n=(a.tiers||a.rewards||[]).filter(i=>{const l=Number(i.costPoints||i.points||0);return l>0&&r>=l});if(n.length>0){const i=document.createElement("div");i.className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-xl p-4 shadow-sm flex justify-between items-center animate-fade-in",i.innerHTML=`
            <div class="flex items-center gap-3">
        ...
        `;const l=document.createElement("button");l.innerText="Resgatar",l.className="text-xs font-bold bg-yellow-500 text-white px-4 py-2 rounded-lg shadow hover:bg-yellow-600 transition-colors",l.onclick=()=>hn(n,e),i.appendChild(l),t.innerHTML="",t.appendChild(i)}}function hn(e,t){const a=`
        <div class="space-y-4">
    ...
    `,{modalElement:s,close:r}=X({title:"🎁 Resgatar Prémio",contentHTML:a,maxWidth:"max-w-md"});s.addEventListener("click",o=>{const n=o.target.closest('[data-action="select-reward"]');if(n){const i=n.dataset.rewardId,l=e.find(d=>d.id&&d.id==i||(d.name||d.reward)==i);l&&(xn(l,t),r())}})}async function xn(e,t){const a=Number(e.costPoints||e.points||0),s=e.name||e.reward,r=ke(t),o=e.serviceId?String(e.serviceId):e.service_id?String(e.service_id):null,n=e.productId?String(e.productId):e.product_id?String(e.product_id):null,i=r.find(l=>{const d=l.id?String(l.id):l.itemId?String(l.itemId):null,c=l.serviceId?String(l.serviceId):l.service_id?String(l.service_id):null,u=l.productId?String(l.productId):l.product_id?String(l.product_id):null;return console.log("Verificando compatibilidade:",{rewardServiceId:o,itemId:d,itemServiceId:c,name:l.name}),!!(o&&(d===o||c===o)||n&&(d===n||u===n))});if(i){const l=Number(i.price||0);x.checkoutState.discount={type:"real",value:l},x.checkoutState.discountReason=`Resgate Fidelidade: ${s}`,x.pendingRedemption={rewardId:e.id||null,name:s,cost:a,appliedToServiceId:i.id},g("Sucesso",`Prémio "${s}" resgatado! O valor de R$ ${l.toFixed(2)} foi abatido.`,"success"),V()}else g("Resgate Não Permitido",`Para resgatar o prémio "${s}", o serviço correspondente deve constar na comanda. Adicione o serviço primeiro. Se o erro persistir, verifique se o prémio está vinculado a um serviço nas configurações.`,"warning")}function yn(){if(!x.isCashierOpen)return g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");const{modalElement:e,close:t}=X({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{const r=e.querySelector("#add-item-content");r.innerHTML=`
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{o>1&&(o--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{o++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await ss(r,o),t()}};e.addEventListener("click",r=>{const o=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(o){const{itemType:i,itemId:l}=o.dataset,c=(x.catalog[i+"s"]||[]).find(u=>u.id===l);c&&s({...c,type:i})}else n&&a()}),a()}async function ua(e=null){if(!x.isCashierOpen)return g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");if(!m.professionals||m.professionals.length===0)try{m.professionals=await K(m.establishmentId)}catch{return g("Erro","Não foi possível carregar profissionais.","error")}const a=`
    ...
    `,{modalElement:s}=X({title:"Nova Venda Avulsa",contentHTML:a,maxWidth:"max-w-md"}),r=s.querySelector("#client-search"),o=s.querySelector("#client-suggestions"),n=s.querySelector("#selected-client-id");e&&(n.value=e.id,r.value=`${e.name} (${e.phone||"Sem tel"})`,r.classList.add("bg-green-50","border-green-300","text-green-800")),r.addEventListener("input",as(async l=>{const d=l.target.value.trim();if(n.value="",r.classList.remove("bg-green-50","border-green-300","text-green-800"),d.length<2){o.classList.add("hidden");return}try{o.innerHTML='<li class="p-2 text-xs text-gray-500">Buscando...</li>',o.classList.remove("hidden");const c=await lt(m.establishmentId,d,10);c.length===0?o.innerHTML='<li class="p-2 text-xs text-gray-500">Nenhum cliente encontrado</li>':o.innerHTML=c.map(u=>`<li data-client-id="${u.id}" data-client-name="${u.name}" data-client-phone="${u.phone}" class="p-2 hover:bg-indigo-50 cursor-pointer border-b last:border-0 transition-colors"><div class="font-bold text-sm text-gray-800">${v(u.name)}</div><div class="text-xs text-gray-500">${u.phone||"Sem telefone"}</div></li>`).join("")}catch{o.classList.add("hidden")}},400)),o.addEventListener("click",l=>{const d=l.target.closest("li[data-client-id]");d&&(n.value=d.dataset.clientId,n.dataset.name=d.dataset.clientName,n.dataset.phone=d.dataset.clientPhone,r.value=`${d.dataset.clientName}`,r.classList.add("bg-green-50","border-green-300","text-green-800"),o.classList.add("hidden"))}),document.addEventListener("click",l=>{!r.contains(l.target)&&!o.contains(l.target)&&o.classList.add("hidden")}),s.querySelector("#new-sale-form").addEventListener("submit",Cn);const i=s.querySelector('[data-action="new-client-from-sale"]');i&&i.addEventListener("click",l=>{l.preventDefault(),s.style.display="none",wn()})}function wn(){X({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",kn)}async function kn(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector("#regClientName"),r=t.querySelector("#regClientPhone").value.replace(/\D/g,"");if(!a.value||!r)return g("Erro","Nome e Telefone são obrigatórios.","error");try{const o=await qr(m.establishmentId,r);if(o)g("Atenção","Cliente já cadastrado.","info"),document.getElementById("genericModal").style.display="none",ua(o);else{const n=await Sa({establishmentId:m.establishmentId,name:a.value,phone:r});g("Sucesso","Cliente cadastrado!","success"),document.getElementById("genericModal").style.display="none",ua(n)}}catch(o){g("Erro",o.message,"error")}}async function Sn(){const e=`
    ...
    `,{modalElement:t}=X({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const s=parseFloat(document.getElementById("initial-amount").value);if(isNaN(s)||s<0)return g("Valor Inválido","Insira um valor válido.","error");try{const r=await rn({establishmentId:m.establishmentId,initialAmount:parseFloat(s.toFixed(2))});x.isCashierOpen=!0,x.activeCashierSessionId=r.id,document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto (R$ ${s.toFixed(2)})`,"success"),Ut(),await re()}catch(r){g("Erro",`Falha ao abrir caixa: ${r.message}`,"error")}})}async function $n(){const e=x.activeCashierSessionId;if(e)try{const t=await dn(e),a=`
        ...
        `,{modalElement:s}=X({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});s.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const o=parseFloat(document.getElementById("final-amount").value);if(isNaN(o)||o<0)return g("Valor Inválido","Insira um valor final válido.","error");try{await nn(e,o),x.isCashierOpen=!1,x.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",Ut(),await re(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Falha ao fechar caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Falha ao carregar relatório: ${t.message}`,"error")}}async function En(e){if(x.activeFilter===e)return;x.activeFilter=e,x.paging.page=1,document.querySelectorAll(".filter-btn").forEach(a=>a.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),Te(),x.selectedComandaId=null,x.viewMode="items";const t=document.getElementById("comandas-list");t&&(t.innerHTML='<div class="loader mx-auto mt-10"></div>'),await re(),V()}function os(e){x.selectedComandaId=e,x.viewMode="items",x.pendingRedemption=null,x.checkoutState.discount={type:"real",value:0},x.checkoutState.discountReason="",nt(),pn(),V()}async function ss(e,t){const a=x.allComandas.find(o=>o.id===x.selectedComandaId);if(!a)return;if(!e.id||String(e.id)==="undefined"){console.error("Tentativa de adicionar item sem ID:",e),g("Erro","Item sem identificador. Não foi possível adicionar.","error");return}const s=parseFloat(e.price)||0,r=Array(t).fill(0).map(()=>{const o={id:String(e.id),name:e.name,price:s,type:e.type,isReward:e.isReward||!1,pointsCost:e.pointsCost||0};return e.type==="product"?(o.productId=o.id,o.product_id=o.id):e.type==="service"&&(o.serviceId=o.id,o.service_id=o.id),o});a.comandaItems=a.comandaItems||[],a.comandaItems.push(...r),a._cachedItems=null,a._hasUnsavedChanges=!0,V()}async function oo(e,t){const a=x.allComandas.find(o=>o.id===x.selectedComandaId);if(!a)return;let s=!1,r=(a.comandaItems||[]).findIndex(o=>o.id==e&&o.type===t);r>-1&&(a.comandaItems.splice(r,1),s=!0),s&&(a._cachedItems=null,a._hasUnsavedChanges=!0,V())}async function In(e){if(x.isProcessing)return;const t=ke(e),a=t.reduce((y,k)=>y+Number(k.price||0)*(k.quantity||1),0),s=x.checkoutState.discount||{type:"real",value:0};let r=s.type==="percent"?a*s.value/100:s.value;r>a&&(r=a);const o=a-r,{payments:n}=x.checkoutState,i=n.reduce((y,k)=>y+k.value,0),l=o-i;if(l>.01){if(!await O("Pagamento Parcial",`O valor de R$ ${l.toFixed(2)} não foi pago. Deseja registrar como DÍVIDA (Fiado) no cadastro do cliente?`))return;n.push({method:"fiado",value:l,installments:1})}x.isProcessing=!0;const d=e.type==="appointment",c=t;let u=0;const p=x.loyaltySettings;if(p&&p.enabled)if(p.type==="visit")u=Number(p.pointsPerVisit)||1;else{const y=Number(p.pointsPerCurrency)||10;y>0&&(u=Math.floor(o/y))}const b={...s,reason:x.checkoutState.discountReason||""},h={payments:n,totalAmount:Number(o),items:c,cashierSessionId:x.activeCashierSessionId,loyaltyPointsEarned:u,discount:b,loyaltyRedemption:x.pendingRedemption},f=document.createElement("div");f.className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm",f.innerHTML='<div class="bg-white p-6 rounded-2xl shadow-2xl flex flex-col items-center"><div class="loader mb-4 border-t-indigo-600"></div><p>Finalizando venda...</p></div>',document.body.appendChild(f);try{d?await Ws(e.id,h):(h.establishmentId=m.establishmentId,h.clientId=e.clientId,h.clientName=e.clientName,h.professionalId=e.professionalId,e.clientPhone&&(h.clientPhone=e.clientPhone),await an(h));let y="Venda finalizada com sucesso!";u>0&&(y+=` Cliente ganhou ${u} pontos!`),g("Sucesso!",y,"success"),Te(),x.selectedComandaId=null,x.viewMode="items",x.pendingRedemption=null,await re()}catch(y){g("Erro no Checkout",y.message,"error")}finally{document.body.contains(f)&&document.body.removeChild(f),x.isProcessing=!1}}async function Cn(e){e.preventDefault();const t=document.getElementById("selected-client-id"),a=document.getElementById("new-sale-professional").value,s=t.value,r=document.getElementById("client-search").value,o=t.dataset.phone||"";if(!s)return g("Erro","Selecione um cliente válido.","error");const n=m.professionals.find(l=>l.id===a);if(!n)return g("Erro","Selecione um profissional válido.","error");const i={id:`temp-${Date.now()}`,type:"walk-in",clientId:s,clientName:r.split("(")[0].trim(),clientPhone:o,professionalId:n.id,professionalName:n.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};x.allComandas.unshift(i),x.selectedComandaId=i.id,x.viewMode="items",document.getElementById("genericModal").style.display="none",os(i.id)}async function re(){const e=document.getElementById("comandas-list");(!e.hasChildNodes()||e.innerHTML.includes("loader"))&&(e.innerHTML='<div class="loader mx-auto mt-10"></div>');const t=x.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=sn(),s=en(m.establishmentId,t,x.paging.page,x.paging.limit),r=x.loyaltySettings?Promise.resolve(null):Se(m.establishmentId),[o,n,i]=await Promise.all([a,s,r]);if(x.isCashierOpen=!!o,x.activeCashierSessionId=o?o.id:null,Ut(),i&&i.loyaltyProgram&&(x.loyaltySettings=i.loyaltyProgram),!x.isCashierOpen&&x.activeFilter==="atendimento"){nt(),V();return}if(x.allComandas=n.data||n,x.paging.total=n.total||n.length,x.catalog.services.length===0){const[l,d,c,u]=await Promise.all([$e(m.establishmentId),Ft(m.establishmentId),ts(m.establishmentId),K(m.establishmentId)]);x.catalog={services:l,products:d,packages:c},m.professionals=u}nt(),V()}catch(a){g("Erro",`Não foi possível carregar os dados: ${a.message}`,"error")}}async function Ln(e={}){Le=document.getElementById("content"),x.selectedComandaId=e.selectedAppointmentId||null,x.viewMode="items",gn(),Ie&&(Le.removeEventListener("click",Ie),Le.removeEventListener("change",Ie)),Ie=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id]");if(t.target.id==="filter-date"&&x.activeFilter==="finalizadas"){x.paging.page=1,await re();return}if(a){if(a.matches("[data-filter]"))En(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}os(a.dataset.comandaId)}else if(a.matches("[data-action]")){const r=a.dataset.action,o=a.dataset.id||x.selectedComandaId,n=x.allComandas.find(i=>i.id===o);switch(r){case"back-to-list":Te(),x.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach($=>$.classList.remove("selected")),V();break;case"new-sale":ua();break;case"add-item":yn();break;case"open-cashier":Sn();break;case"close-cashier":await $n();break;case"view-sales-report":Z("sales-report-section");break;case"go-to-checkout":await to(n,"checkout");break;case"back-to-items":x.viewMode="items",V();break;case"save-comanda":await to(n,"stay");break;case"select-method":x.checkoutState.selectedMethod=a.dataset.method,x.checkoutState.installments=1,V();break;case"add-payment-checkout":const i=document.getElementById("checkout-amount");let l=parseFloat(i.value);const c=ke(n).reduce(($,w)=>$+(w.price||0),0),u=x.checkoutState.discount||{type:"real",value:0};let p=u.type==="percent"?c*u.value/100:u.value;p>c&&(p=c);const b=c-p,h=x.checkoutState.payments.reduce(($,w)=>$+w.value,0),f=b-h;if(isNaN(l)||l<=0){g("Valor inválido","Insira um valor maior que zero.","error");break}if(l>f+.05){g("Valor inválido","Valor excede o restante.","error");break}const y={method:x.checkoutState.selectedMethod,value:l};["credito","crediario"].includes(x.checkoutState.selectedMethod)&&x.checkoutState.installments>1&&(y.installments=x.checkoutState.installments),x.checkoutState.payments.push(y),x.checkoutState.selectedMethod="dinheiro",x.checkoutState.installments=1,x.checkoutState.amountReceived="",V();break;case"remove-payment-checkout":const k=parseInt(a.dataset.index,10);x.checkoutState.payments.splice(k,1),V();break;case"finalize-checkout":await In(n);break;case"increase-qty":{const $=a.dataset.itemId,w=a.dataset.itemType;if(!$||$==="undefined"||$==="null"){g("Erro","Item inválido para adição.","error");return}let T=ke(n).find(F=>F.id==$&&F.type===w);T||(T=(x.catalog[w+"s"]||[]).find(P=>P.id==$));const q=T?{id:T.id,name:T.name,price:Number(T.price),type:T.type}:{id:$,name:"Item Indisponível",price:0,type:w};await ss(q,1);break}case"decrease-qty":{await oo(a.dataset.itemId,a.dataset.itemType);break}case"remove-item":await oo(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await O("Reabrir Comanda","Tem certeza? O pagamento será estornado."))try{await _s(o);const w=x.allComandas.findIndex(E=>E.id===o);w!==-1&&(x.allComandas[w].status="confirmed",delete x.allComandas[w].transaction),x.selectedComandaId=null,Te(),await re(),g("Sucesso!","Comanda reaberta.","success")}catch(w){g("Erro",w.message,"error")}break}case"go-to-appointment":{const $=a.dataset.id,w=a.dataset.date;Z("agenda-section",{scrollToAppointmentId:$,targetDate:new Date(w)});break}case"delete-walk-in":{if(await O("Excluir Venda","Confirma a exclusão desta venda avulsa?"))if(o.startsWith("temp-"))x.allComandas=x.allComandas.filter(w=>w.id!==o),x.selectedComandaId=null,nt(),V(),Te();else try{await on(o),g("Sucesso","Venda excluída.","success"),x.selectedComandaId=null,Te(),await re()}catch(w){g("Erro",w.message,"error")}break}}}}},Le.addEventListener("click",Ie),Le.addEventListener("change",Ie),e.initialFilter&&(x.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(x.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${x.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",x.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await re()}const Ia=e=>S(`/api/financial/natures/${e}`),Tn=e=>S("/api/financial/natures",{method:"POST",body:JSON.stringify(e)}),Pn=e=>S(`/api/financial/natures/${e}`,{method:"DELETE"}),Ca=e=>S(`/api/financial/cost-centers/${e}`),Bn=e=>S("/api/financial/cost-centers",{method:"POST",body:JSON.stringify(e)}),Dn=e=>S(`/api/financial/cost-centers/${e}`,{method:"DELETE"}),rs=(e,t)=>S(`/api/financial/${e}`,{method:"POST",body:JSON.stringify(t)}),ns=(e,t={})=>{let a=`/api/financial/${e}`;const s=new URLSearchParams;t.establishmentId&&s.append("establishmentId",t.establishmentId),t.startDate&&s.append("startDate",t.startDate),t.endDate&&s.append("endDate",t.endDate),t.natureId&&s.append("natureId",t.natureId),t.costCenterId&&s.append("costCenterId",t.costCenterId),t.status&&s.append("status",t.status);const r=s.toString();return r&&(a+=`?${r}`),S(a)},is=(e,t,a)=>S(`/api/financial/${e}/${t}`,{method:"PUT",body:JSON.stringify(a)}),ls=(e,t)=>S(`/api/financial/${e}/${t}`,{method:"DELETE"}),ds=(e,t,a)=>S(`/api/financial/${e}/${t}/status`,{method:"PATCH",body:JSON.stringify({status:"paid",paymentDate:a})}),Mn=e=>rs("payables",e),An=e=>ns("payables",e),qn=(e,t)=>is("payables",e,t),Nn=e=>ls("payables",e),Rn=(e,t)=>ds("payables",e,t),jn=e=>rs("receivables",e),Fn=e=>ns("receivables",e),Hn=(e,t)=>is("receivables",e,t),On=e=>ls("receivables",e),zn=(e,t)=>ds("receivables",e,t),Vn=(e,t,a)=>S(`/api/financial/cash-flow?establishmentId=${e}&startDate=${t}&endDate=${a}`),ma=document.getElementById("content");let ue={};const so=["#4f46e5","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899","#06b6d4"],D={startDate:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],selectedProfessional:"all",selectedCostCenter:"all",professionalsList:[],costCentersList:[],data:null,appointmentsData:[],cashFlowData:null,currentTab:"dashboards",isFilterOpen:!1};async function Un(){if(!window.Chart)return new Promise((e,t)=>{const a=document.createElement("script");a.src="https://cdn.jsdelivr.net/npm/chart.js",a.onload=e,a.onerror=t,document.head.appendChild(a)})}async function _n(){ma.innerHTML=`
        <div class="flex flex-col items-center justify-center h-[calc(100vh-100px)]">
        ...
        </div>`;try{await Un();const[e,t]=await Promise.all([K(m.establishmentId),Pr(m.establishmentId).catch(()=>[])]);D.professionalsList=e||[],D.costCentersList=t||[],Wn(),await cs()}catch(e){console.error("Erro no loadReportsPage:",e),ma.innerHTML=`
            ...
            </div>`}}function Wn(){const e=D.professionalsList.map(a=>`<option value="${a.id}">${v(a.name)}</option>`).join(""),t=D.costCentersList.map(a=>`<option value="${a.id}">${v(a.name)}</option>`).join("");ma.innerHTML=`
    ...
    `,document.getElementById("toggle-filters-btn").onclick=ro,document.getElementById("btn-apply-filters").onclick=()=>{Jn(),ro()},document.querySelectorAll(".tab-btn").forEach(a=>{a.onclick=()=>{D.currentTab=a.dataset.tab,no(),us(),window.scrollTo({top:0,behavior:"smooth"})}}),no()}function ro(){const e=document.getElementById("filters-container"),t=document.getElementById("toggle-filters-btn");D.isFilterOpen=!D.isFilterOpen,D.isFilterOpen?(e.classList.remove("hidden"),t.classList.add("bg-indigo-100","text-indigo-800")):(e.classList.add("hidden"),t.classList.remove("bg-indigo-100","text-indigo-800"))}function no(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===D.currentTab;e.className=t?"tab-btn py-2 px-6 rounded-full text-xs md:text-sm font-bold bg-indigo-600 text-white shadow-md transform scale-105 transition-all whitespace-nowrap border-transparent":"tab-btn py-2 px-6 rounded-full text-xs md:text-sm font-medium text-gray-500 bg-white border-gray-200 hover:bg-gray-50 transition-all whitespace-nowrap"})}function Pt(e){if(!e)return"";const t=e.split("-");return`${t[2]}/${t[1]}`}async function Jn(){D.startDate=document.getElementById("report-start").value,D.endDate=document.getElementById("report-end").value,D.selectedProfessional=document.getElementById("report-prof").value,D.selectedCostCenter=document.getElementById("report-cost").value,document.getElementById("date-range-display").textContent=`${Pt(D.startDate)} até ${Pt(D.endDate)}`,await cs()}async function cs(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader border-t-indigo-600"></div></div>';try{const t=D.selectedProfessional==="all"?null:D.selectedProfessional,a=Tr(D.startDate,D.endDate,D.selectedProfessional,D.selectedCostCenter),s=Vn(m.establishmentId,D.startDate,D.endDate).catch(()=>({labels:[],payables:[],receivables:[],expectedBalance:[]})),r=new Date(D.startDate+"T00:00:00").toISOString(),o=new Date(D.endDate+"T23:59:59").toISOString(),n=Ao(m.establishmentId,r,o,t).catch(()=>[]),[i,l,d]=await Promise.all([a,s,n]);D.data=i,D.cashFlowData=l,D.appointmentsData=Array.isArray(d)?d:[],us()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function us(){const e=document.getElementById("report-content");if(D.data)switch(D.currentTab){case"dashboards":Gn(e);break;case"appointments":Yn(e);break;case"dre":Qn(e);break}}function Gn(e){const{dreSimple:t,charts:a}=D.data,s=t||{grossRevenue:0,netProfit:0,variableCosts:0};e.innerHTML=`
    ...
    `,D.cashFlowData&&Xn("chart-cashflow-modern",D.cashFlowData),io("chart-monthly","bar","Receita Mensal",a.salesMonthly.labels,a.salesMonthly.data,so[0]),io("chart-profs","doughnut","Total Vendas",a.professionals.labels,a.professionals.data,so)}function Yn(e){const t=D.appointmentsData,a=t.length;let s=0,r=0,o=0;const n={},i={};let l=new Date(D.startDate);const d=new Date(D.endDate);for(;l<=d;)n[l.toISOString().split("T")[0]]=0,l.setDate(l.getDate()+1);t.forEach(b=>{const h=parseFloat(b.totalAmount||b.price||0),f=(b.status||"").toLowerCase();let y=b.startTime?(b.startTime.toDate?b.startTime.toDate():new Date(b.startTime)).toISOString().split("T")[0]:"";const k=b.professionalName||"Sem Profissional";i[k]||(i[k]={name:k,count:0,value:0}),["cancelled","cancelado","no-show"].includes(f)?r++:(["completed","finalized","paid"].includes(f)&&s++,o+=h,y&&n.hasOwnProperty(y)&&n[y]++,i[k].count++,i[k].value+=h)});const c=Object.keys(n).sort(),u=c.map(b=>n[b]),p=Object.values(i).sort((b,h)=>h.value-b.value);e.innerHTML=`
            ...
            <div class="overflow-x-auto no-scrollbar w-full">
                <table class="w-full text-xs md:text-sm min-w-full">
                    <tbody class="divide-y divide-gray-100">
                        ${p.map((b,h)=>{const f=p[0]?.value||1,y=b.value/f*100;return`
                            <tr class="group">
    ...
    `,Zn("chart-appointments-daily",c,u)}function Qn(e){const{dreFinancial:t}=D.data;if(!t)return;const a=t.totalRevenues,s=(i,l,d,c=!1)=>{const u=a>0?l/a*100:0,p=c?"- ":"";return`
    ...
    `}function Xn(e,t){const a=document.getElementById(e);if(!a)return;const s=a.getContext("2d");ue[e]&&ue[e].destroy();const r=s.createLinearGradient(0,0,0,400);r.addColorStop(0,"rgba(59, 130, 246, 0.4)"),r.addColorStop(1,"rgba(59, 130, 246, 0.0)");const o=t.payables.map(i=>i*-1),n=t.labels.map(i=>i.split("-").reverse().slice(0,2).join("/"));ue[e]=new Chart(s,{type:"bar",data:{labels:n,datasets:[{label:"Saldo",data:t.expectedBalance,type:"line",borderColor:"#3b82f6",backgroundColor:r,borderWidth:3,pointRadius:3,pointBackgroundColor:"#fff",pointBorderColor:"#3b82f6",pointBorderWidth:2,pointHoverRadius:6,hitRadius:30,fill:!0,tension:.4,order:0,yAxisID:"y"},{label:"Entradas",data:t.receivables,backgroundColor:"#34d399",borderRadius:4,barPercentage:.6,order:1,yAxisID:"y"},{label:"Saídas",data:o,backgroundColor:"#f87171",borderRadius:4,barPercentage:.6,order:2,yAxisID:"y"}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{position:"top",align:"end",labels:{usePointStyle:!0,boxWidth:6,font:{family:"'Inter', sans-serif",size:10}}},tooltip:{backgroundColor:"#fff",titleColor:"#111",bodyColor:"#444",borderColor:"#eee",borderWidth:1,padding:10}},scales:{x:{grid:{display:!1},ticks:{font:{size:10}}},y:{display:!0,grid:{color:"#f3f4f6",borderDash:[4,4]},ticks:{callback:i=>new Intl.NumberFormat("pt-BR",{notation:"compact"}).format(i),font:{size:10}}}}}})}function Zn(e,t,a){const s=document.getElementById(e);if(!s)return;const r=s.getContext("2d");ue[e]&&ue[e].destroy();const o=r.createLinearGradient(0,0,0,300);o.addColorStop(0,"rgba(99, 102, 241, 0.5)"),o.addColorStop(1,"rgba(99, 102, 241, 0.0)");const n=t.map(i=>i.split("-").reverse().slice(0,2).join("/"));ue[e]=new Chart(r,{type:"line",data:{labels:n,datasets:[{label:"Agendamentos",data:a,borderColor:"#6366f1",backgroundColor:o,borderWidth:2,fill:!0,tension:.4,pointRadius:3,pointBackgroundColor:"#fff",pointBorderColor:"#6366f1",pointBorderWidth:2,pointHoverRadius:5,hitRadius:30}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!1},tooltip:{backgroundColor:"#fff",titleColor:"#111",bodyColor:"#6366f1",borderColor:"#eee",borderWidth:1}},scales:{x:{grid:{display:!1},ticks:{font:{size:10}}},y:{beginAtZero:!0,grid:{color:"#f3f4f6"},ticks:{stepSize:1,font:{size:10}}}}}})}function io(e,t,a,s,r,o){const n=document.getElementById(e);if(!n)return;const i=n.getContext("2d");ue[e]&&ue[e].destroy(),new Chart(i,{type:t,data:{labels:s,datasets:[{label:a,data:r,backgroundColor:o,borderColor:Array.isArray(o)?"#fff":o,borderWidth:1,tension:.3,fill:t==="line"}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:t==="doughnut",position:"right",labels:{usePointStyle:!0,boxWidth:8,font:{size:10}}}},scales:t==="doughnut"?{}:{x:{grid:{display:!1},ticks:{font:{size:9}}},y:{grid:{color:"#f3f4f6"},beginAtZero:!0,ticks:{font:{size:9}}}}}})}const _t=(e,t="products")=>S(`/api/${t}/categories/${e}`),ms=(e,t="products")=>S(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),ps=(e,t="products")=>S(`/api/${t}/categories/${e}`,{method:"DELETE"}),Kn="audit_logs",Fe=async(e,t,a,s,r,o=null)=>{try{if(!t)return;await Po(pe(_,Kn),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:s,description:r,details:o,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},ve=document.getElementById("content");let ie=null,Xe="services",ye="all";function He(){const e=U.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function ei(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await ms({establishmentId:m.establishmentId,name:s},"services"),Fe(m.establishmentId,He(),"Categorias (Serviços)","Criou",`Criou categoria: ${s}`),a.value="",g("Sucesso","Categoria criada!","success"),await La(),await ct()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function ti(e){if(await O("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await ps(e,"services"),Fe(m.establishmentId,He(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await La(),await ct()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function La(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await _t(m.establishmentId,"services");m.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                ...
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria criada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function ai(){X({title:"Gerir Categorias de Serviços",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",ei),t.addEventListener("click",s=>{const r=s.target.closest('button[data-action="delete-category"]');r&&(s.preventDefault(),ti(r.dataset.id))}))}La()}async function oi(e){e.preventDefault();const t=e.target.closest("#serviceModal"),a=t.querySelector("#serviceId").value,s={},r=t.querySelector('input[name="commissionType"]:checked').value;r==="custom"&&t.querySelectorAll(".professional-commission-row").forEach(n=>{const i=n.dataset.profId;if(n.querySelector('input[type="checkbox"]').checked){const d=parseFloat(n.querySelector('input[type="number"]').value);s[i]=isNaN(d)?0:d}});const o={establishmentId:m.establishmentId,name:t.querySelector("#serviceName").value,price:parseFloat(t.querySelector("#servicePrice").value),duration:parseInt(t.querySelector("#serviceDurationMinutes").value,10),bufferTime:parseInt(t.querySelector("#serviceBufferTimeMinutes").value,10)||0,categoryId:t.querySelector("#serviceCategory").value||null,commissionRate:parseFloat(t.querySelector("#serviceCommissionRate").value)||0,active:t.querySelector("#serviceStatus").value==="true",photo:t.querySelector("#servicePhotoBase64").value,notes:t.querySelector("#serviceNotes").value,commissionType:r,professionalCommissions:s};try{a?(await sr(a,o),Fe(m.establishmentId,He(),"Serviços","Editou",`Editou o serviço: ${o.name}`)):(await Ro(o),Fe(m.establishmentId,He(),"Serviços","Criou",`Criou novo serviço: ${o.name}`)),document.getElementById("serviceModal").style.display="none",g("Sucesso",`Serviço ${a?"atualizado":"adicionado"} com sucesso!`,"success"),await ct()}catch(n){g("Erro",n.message,"error")}}function lo(e=null){const t=document.getElementById("serviceModal"),a=m.serviceCategories||[],s=e?.duration||0,r=e?.bufferTime||0,o=v(e?.name||""),n=v(e?.notes||""),i=e?o:"Novo Serviço",l=a.map(E=>`<option value="${E.id}" ${e?.categoryId===E.id?"selected":""}>${v(E.name)}</option>`).join("");t.innerHTML=`
            ...
            <div class="mt-6 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-3">
    ...
    </div>`,t.style.display="flex",t.addEventListener("click",async E=>{const T=E.target.closest("button[data-action]");if(!T)return;const q=T.dataset.action,F=T.dataset.id;if(q==="close-modal"&&(t.style.display="none"),q==="delete-service"){if(!F)return;if(t.style.display="none",await O("Apagar Serviço","Tem a certeza que deseja apagar este serviço?"))try{const B=m.services.find(R=>R.id===F)?.name||"Desconhecido";await rr(F),Fe(m.establishmentId,He(),"Serviços","Excluiu",`Excluiu o serviço: ${B}`),g("Sucesso","Serviço apagado com sucesso!","success"),await ct()}catch(B){g("Erro",`Não foi possível apagar o serviço: ${B.message}`,"error")}else t.style.display="flex"}});const d=t.querySelectorAll(".tab-btn"),c=t.querySelectorAll(".tab-content");d.forEach(E=>{E.addEventListener("click",()=>{d.forEach(T=>{T.classList.remove("border-indigo-500","text-indigo-600"),T.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),E.classList.add("border-indigo-500","text-indigo-600"),E.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),c.forEach(T=>T.classList.add("hidden")),document.getElementById(`tab-content-${E.dataset.tab}`).classList.remove("hidden")})});const u=t.querySelectorAll('input[name="commissionType"]'),p=document.getElementById("defaultCommissionRateContainer"),b=document.getElementById("professionalCommissionsContainer");function h(){const E=t.querySelector('input[name="commissionType"]:checked').value;p&&(p.style.display=E==="default"?"block":"none"),b&&(b.style.display=E==="custom"?"block":"none")}u.forEach(E=>E.addEventListener("change",h));const f=document.getElementById("professionalCommissionsList");f&&(f.innerHTML=(m.professionals||[]).map(E=>{const T=e?.professionalCommissions?.[E.id]!==void 0,q=e?.professionalCommissions?.[E.id]||0;return`
                <div class="professional-commission-row flex items-center justify-between p-2 rounded-md ${T?"bg-blue-50":""}" data-prof-id="${E.id}">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <img src="${E.photo||`https://placehold.co/40x40/E2E8F0/4A5568?text=${v(E.name.charAt(0))}`}" class="w-8 h-8 rounded-full object-cover">
                        <span class="text-sm font-medium">${v(E.name)}</span>
                    </label>
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(m.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${v(a.name)}</option>`)),Ta(),Pe()}function ri(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function ct(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,s,r]=await Promise.all([$e(m.establishmentId),K(m.establishmentId),_t(m.establishmentId,"services"),ir(m.establishmentId)]);m.services=(t||[]).filter(Boolean),m.professionals=(a||[]).filter(Boolean),m.serviceCategories=(s||[]).filter(Boolean),m.mostPopularService=r||{name:"N/A",count:0},m.services.forEach(o=>{o.active===void 0&&(o.active=!0)}),gs(Xe)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function gs(e){if(document.getElementById("services-content-container")){if(Xe===e&&document.getElementById("services-content-container").children.length>1){Xe==="services"&&(Ta(),Pe());return}Xe=e,ye="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?si():e==="reports"&&ri()}}function ni(){ie&&(ve.removeEventListener("click",ie),ve.removeEventListener("input",ie),ve.removeEventListener("change",ie)),ie=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),o=r.dataset.id,n=r.checked;try{await nr(o,n);const i=m.services.findIndex(l=>l.id===o);i>-1&&(m.services[i].active=n),Fe(m.establishmentId,He(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${o}) para ${n?"Ativo":"Inativo"}`),Pe(),Ta()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,Pe()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){Pe();return}if(!a)return;if(a.hasAttribute("data-view")){gs(a.dataset.view);return}switch(a.dataset.action){case"new-service":lo();break;case"edit-service":const r=JSON.parse(a.dataset.service);lo(r);break;case"manage-categories":ai();break;case"filter-service":const o=a.dataset.filterType;if(o==="popular")return;ye=o==="total"?"all":o,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,d=i===ye||i==="total"&&ye==="all";n.classList.toggle("ring-2",d),n.classList.toggle("ring-indigo-500",d),n.classList.toggle("shadow-lg",d)}),Pe();break}},ve.addEventListener("click",ie),ve.addEventListener("input",ie),ve.addEventListener("change",ie)}async function ii(){ve.innerHTML=`
        ...
        </section>`,ni();try{(!m.professionals||m.professionals.length===0)&&(m.professionals=await K(m.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),m.professionals=[]}Xe="services",ye="all",await ct()}const Wt="suppliers",Pa="purchases",bs="financial_payables",Ba=async e=>{try{const t=Rt(pe(_,Wt),rt("establishmentId","==",e)),a=await xa(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar fornecedores:",t),t}},li=async e=>{try{return{id:(await Po(pe(_,Wt),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},di=async(e,t)=>{try{const a=me(_,Wt,e);return await ha(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},ci=async e=>{try{const t=me(_,Wt,e);return await qs(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},ui=async(e,t=null)=>{try{const a=Do(_),s=me(pe(_,Pa)),r={...e,createdAt:Fa()};if(a.set(s,r),t&&t.defaultNatureId&&t.defaultCostCenterId){const o=me(pe(_,bs)),n=new Date().toISOString().split("T")[0],i={establishmentId:e.establishmentId,description:`Compra - ${e.supplierName}`,amount:parseFloat(e.totalAmount),dueDate:n,naturezaId:t.defaultNatureId,centroDeCustoId:t.defaultCostCenterId,notes:`Gerado automaticamente pelo Pedido de Compra. Itens: ${e.items.length}`,status:"pending",paymentDate:null,purchaseId:s.id,createdAt:Fa()};a.set(o,i)}return await a.commit(),{id:s.id,...r}}catch(a){throw console.error("Erro ao registrar compra com integração:",a),a}},mi=async(e,t)=>{try{const a=Do(_),s=me(_,Pa,e);a.delete(s);const r=Rt(pe(_,bs),rt("purchaseId","==",e),rt("establishmentId","==",t));return(await xa(r)).forEach(n=>{a.delete(n.ref)}),await a.commit(),!0}catch(a){throw console.error("Erro ao excluir compra e financeiro:",a),a}},pi=async e=>{try{const t=Rt(pe(_,Pa),rt("establishmentId","==",e),Bo("createdAt","desc")),a=await xa(t),s=[];return a.forEach(r=>{s.push({id:r.id,...r.data()})}),s}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},ge=document.getElementById("content");let le=null,Ze="products",se="all";async function gi(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),s=a.value;if(s)try{await ms({establishmentId:m.establishmentId,name:s},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await Da(),await ut()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function bi(e){if(await O("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await ps(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await Da(),await ut()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function Da(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await _t(m.establishmentId,"products");m.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${v(a.name)}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function fi(){X({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
    ...
    <div class="modal-content max-w-3xl overflow-y-auto max-h-[90vh]">
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
    ...
    </div>`;const h=t.querySelector("#productCategory"),f=t.querySelector("#productPhotoInput");t.querySelector("#productPhotoButton").addEventListener("click",()=>f.click()),h.innerHTML='<option value="">Sem categoria</option>'+(m.categories||[]).map(P=>`<option value="${P.id}" ${e?.categoryId===P.id?"selected":""}>${v(P.name)}</option>`).join(""),e&&(h.value=e.categoryId||"");const y=t.querySelector("#productPhotoPreview");t.querySelector("#productPhotoBase64");const k=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",$=e?.photo||"";f.onchange=async()=>{const P=f.files[0];if(P){y.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const B=await co(P,800,800,"image/jpeg",.8),j=B.length*3/4,z=1e3*1024;if(j>z)throw new Error("A imagem é muito grande mesmo após a compressão.");preview.src=B,base64Input.value=B}catch(B){console.error("Erro ao processar imagem:",B),g("Erro de Imagem",B.message||"Não foi possível processar a imagem.","error"),preview.src=k,base64Input.value=$,F.value=""}}};const w=t.cloneNode(!0);t.parentNode.replaceChild(w,t);const E=()=>{const P=w.querySelector("#modalSupplierSearch"),B=w.querySelector("#supplierSearchResults"),R=w.querySelector("#selectedSuppliersList"),j=P.value.toLowerCase();if(j.length>0){const z=s.filter(N=>N.name.toLowerCase().includes(j)&&!o.has(N.id));z.length>0?(B.classList.remove("hidden"),B.innerHTML=z.map(N=>`
                ...
                `).join("")):(B.classList.remove("hidden"),B.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else B.classList.add("hidden");o.size>0?(R.innerHTML="",o.forEach(z=>{const N=s.find(Y=>Y.id===z);N&&(R.innerHTML+=`
                    ...
                    `)})):R.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};w.querySelector("#modalSupplierSearch").addEventListener("input",E),w.addEventListener("click",P=>{const B=P.target.closest("[data-add-supplier]");if(B){const j=B.dataset.addSupplier;o.add(j),w.querySelector("#modalSupplierSearch").value="",E()}const R=P.target.closest("[data-remove-supplier]");if(R){const j=R.dataset.removeSupplier;o.delete(j),E()}}),E(),w.addEventListener("click",async P=>{const B=P.target.closest("button[data-action]");if(!B)return;const R=B.dataset.action,j=w.querySelector("#productId").value;if(R==="close-modal"&&(w.style.display="none"),R==="delete-product"){if(!j)return;w.style.display="none",await vi(j)}if(R==="save-product-modal"){const z=w.querySelector("#productForm");if(z){if(!z.querySelector("#productName").value||!z.querySelector("#productPrice").value){g("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const N=B.closest('button[data-action="save-product-modal"]');N.disabled=!0,N.textContent="A salvar...";try{await hi(z)}catch(Y){g("Erro",`Falha ao salvar: ${Y.message}`,"error"),N.disabled=!1,N.textContent="Salvar Alterações"}}}if(R==="adjust-stock-modal"){P.preventDefault();const z=w.querySelector("#stockAdjustmentAmount"),N=w.querySelector("#stockAdjustmentReason"),Y=parseInt(z.value,10),Ee=parseInt(B.dataset.change,10);if(!Y||Y<=0){g("Erro","Por favor, insira uma quantidade válida.","error");return}const Qt=Y*Ee,Is=N.value||(Qt>0?"Entrada manual":"Saída manual");try{await cr(j,{change:Qt,reason:Is});const Ue=m.products.findIndex(_e=>_e.id===j);if(Ue>-1){const _e=m.products[Ue].currentStock+Qt;m.products[Ue].currentStock=_e,w.querySelector("#currentStockDisplay").textContent=_e,w.querySelector("#productCurrentStock").value=_e,z.value="",N.value="",g("Sucesso","Estoque atualizado!","success"),Ma(),it()}}catch(Ue){g("Erro de Stock",Ue.message,"error")}}});const T=w.querySelectorAll(".tab-btn"),q=w.querySelectorAll(".tab-content");T.forEach(P=>{P.addEventListener("click",B=>{B.preventDefault(),T.forEach(R=>{R.classList.remove("border-indigo-500","text-indigo-600"),R.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),P.classList.add("border-indigo-500","text-indigo-600"),P.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),q.forEach(R=>R.classList.add("hidden")),document.getElementById(`tab-content-${P.dataset.tab}`).classList.remove("hidden")})});const F=w.querySelector("#productPhotoInput");w.querySelector("#productPhotoButton").addEventListener("click",()=>F.click()),F.onchange=async()=>{const P=F.files[0];if(!P)return;const B=w.querySelector("#productPhotoPreview"),R=w.querySelector("#productPhotoBase64");B.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const j=await co(P,800,800,"image/jpeg",.8),N=j.length*3/4,Y=1e3*1024;if(N>Y)throw new Error("A imagem é muito grande mesmo após a compressão.");B.src=j,R.value=j}catch(j){console.error("Erro ao processar imagem:",j),g("Erro de Imagem",j.message||"Não foi possível processar a imagem.","error"),B.src=k,R.value=$,F.value=""}},w.style.display="flex"}function xi(){const e=document.getElementById("products-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(m.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${v(a.name)}</option>`)),Ma(),it()}function yi(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const s=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
        ...
        </div>`;const r=document.getElementById("productFilterReport"),o=document.getElementById("categoryFilterReport");r&&m.products&&(r.innerHTML+=m.products.map(n=>`<option value="${n.id}">${v(n.name)}</option>`).join("")),o&&m.categories&&(o.innerHTML+=m.categories.map(n=>`<option value="${n.id}">${v(n.name)}</option>`).join(""))}async function wi(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value,establishmentId:m.establishmentId};try{const a=await ur(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function ut(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,s]=await Promise.all([Ft(m.establishmentId),_t(m.establishmentId,"products"),Ba(m.establishmentId)]);m.products=(t||[]).filter(Boolean),m.categories=(a||[]).filter(Boolean),m.suppliers=(s||[]).filter(Boolean),fs(Ze)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function fs(e){if(document.getElementById("products-content-container")){if(Ze===e&&document.getElementById("products-content-container").children.length>1){Ze==="products"&&(Ma(),it());return}Ze=e,se="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?xi():e==="movements"&&yi()}}async function ki(){ge.innerHTML=`
        ...
        </section>`,le&&(ge.removeEventListener("click",le),ge.removeEventListener("input",le),ge.removeEventListener("change",le)),le=async e=>{const t=e.target;if(t.id==="productSearchInput"||t.id==="productCategoryFilter"){it();return}const a=e.target.closest("button[data-action], button[data-view], .indicator-card[data-action], .product-card[data-action]");if(!a||e.target.closest('[data-action-stop-propagation="true"]'))return;if(a.hasAttribute("data-view")){fs(a.dataset.view);return}switch(a.dataset.action){case"new-product":uo();break;case"edit-product":uo(JSON.parse(a.dataset.product));break;case"manage-product-categories":fi();break;case"generate-report":await wi();break;case"filter-stock":const r=a.dataset.filterType;se=se===r?"all":r,document.querySelectorAll(".indicator-card").forEach(o=>{o.classList.toggle("ring-2",o.dataset.filterType===se),o.classList.toggle("ring-indigo-500",o.dataset.filterType===se),o.classList.toggle("shadow-lg",o.dataset.filterType===se)}),it();break}},ge.addEventListener("click",le),ge.addEventListener("input",le),ge.addEventListener("change",le),Ze="products",se="all",await ut()}const be=document.getElementById("content");let de=null,$t="list",H={step:1,productsToBuy:[],allSuppliers:[],finalOrders:{},isQuoteMode:!1};async function Si(){$t==="list"?Jt():$t==="purchases"?(H.step=1,Ke()):$t==="history"&&vs()}async function $i(){try{const e=await Ba(m.establishmentId);return m.suppliers=e||[],H.allSuppliers=e,!0}catch(e){return console.error(e),!1}}async function Ei(e){if(await O("Excluir Fornecedor","Tem a certeza? Isso removerá o vínculo com os produtos."))try{await ci(e),g("Sucesso","Fornecedor excluído.","success"),jt("genericModal"),Jt()}catch(t){g("Erro","Erro ao excluir: "+t.message,"error")}}async function Ii(e){e.preventDefault();const t=e.target,a=t.querySelector("#supId").value,s={name:t.querySelector("#supName").value,contactName:t.querySelector("#supContact").value,email:t.querySelector("#supEmail").value,phone:t.querySelector("#supPhone").value,taxId:t.querySelector("#supTaxId").value,category:t.querySelector("#supCategory").value,establishmentId:m.establishmentId},r=t.querySelector('button[type="submit"]');r.disabled=!0,r.textContent="A salvar...";try{a?(await di(a,s),g("Sucesso","Fornecedor atualizado!","success")):(await li(s),g("Sucesso","Fornecedor criado!","success")),jt("genericModal"),Jt()}catch(o){g("Erro","Erro ao salvar: "+o.message,"error")}finally{r.disabled=!1,r.textContent="Salvar"}}async function Jt(){const e=document.getElementById("suppliersList");if(!e)return;e.innerHTML='<div class="loader mx-auto my-8"></div>',await $i();const t=document.getElementById("supplierSearchInput")?.value.toLowerCase()||"",a=m.suppliers.filter(o=>o.name.toLowerCase().includes(t)||o.contactName&&o.contactName.toLowerCase().includes(t));if(e.innerHTML="",a.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum fornecedor encontrado.</div>';return}let s='<div class="flex flex-col gap-2 md:hidden">';a.forEach(o=>{const n=JSON.stringify(o).replace(/"/g,"&quot;"),i=v(o.name),l=v(o.category||"Geral"),d=v(o.contactName||"");s+=`
    ...
    `;a.forEach(o=>{const n=JSON.stringify(o).replace(/"/g,"&quot;"),i=v(o.name),l=v(o.taxId||"Sem doc."),d=v(o.email||"-"),c=v(o.phone||"-"),u=v(o.category||"Geral");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=s+r}function Ci(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",s=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),o=v(e.name),n=v(e.category||"Fornecedor"),i=v(e.contactName||""),l=v(e.phone||""),d=`
    ...
    `;X({title:"",contentHTML:d,maxWidth:"max-w-md"})}async function Ke(){const e=document.getElementById("purchasesContainer");if(e)if(H.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Ft(m.establishmentId),Ba(m.establishmentId)]);H.allSuppliers=a||[];const s=t.filter(d=>{const c=parseInt(d.currentStock||0),u=parseInt(d.minStock||0);return c<=u});if(H.productsToBuy=s,s.length===0){e.innerHTML=`
                ...
                `;return}let r='<div class="flex flex-col gap-3 md:hidden">',o="";s.forEach(d=>{const c=parseInt(d.minStock)||0,u=parseInt(d.currentStock)||0,p=Math.max(c-u,1),b=parseFloat(d.costPrice||0),h=v(d.name);let f='<option value="">Selecione...</option>';H.allSuppliers.length>0?H.allSuppliers.forEach(y=>{const $=d.supplierIds&&d.supplierIds.includes(y.id)?"selected":"";f+=`<option value="${y.id}" ${$}>${v(y.name)}</option>`}):f='<option value="">Sem fornecedores</option>',r+=`
        ...
        `}).join("");s+=h;const y=encodeURIComponent(JSON.stringify({supplierId:p,supplierName:b.info.name,totalAmount:h,items:b.items})),k=encodeURIComponent(JSON.stringify({name:b.info.name,phone:b.info.phone,email:b.info.email})),$=encodeURIComponent(JSON.stringify(b.items)),w=v(b.info.name),E=v(b.info.email||"");a+=`
    ...
    `}async function vs(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await pi(m.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(o=>{const n=new Date(o.createdAt.seconds*1e3).toLocaleDateString("pt-BR"),i=v(o.supplierName);a+=`
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function Ti(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;X({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",Ii),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>jt("genericModal"))},50)}function Bi(){be.innerHTML=`
    ...
    `,de&&(be.removeEventListener("click",de),be.removeEventListener("input",de),be.removeEventListener("change",de)),de=e=>{if(e.target.closest("#tab-btn-list")&&pt("list"),e.target.closest("#tab-btn-purchases")&&pt("purchases"),e.target.closest("#tab-btn-history")&&pt("history"),e.target.id==="toggle-quote-mode"&&(H.isQuoteMode=e.target.checked,Ke()),e.target.id==="supplierSearchInput"&&Jt(),e.target.closest("#btn-new-supplier")&&mo(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),s=JSON.parse(a.dataset.supplier);Ci(s)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&Ei(t.dataset.id),a==="edit"&&mo(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&pa(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(s=>s.checked=a),pa()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),s={};let r=!1;if(a.forEach(o=>{if(o.offsetParent===null||!o.querySelector(".row-select").checked)return;r=!0;let i="Produto";const l=o.querySelector("td:nth-child(2)"),d=o.querySelector(".font-bold");l?i=l.innerText:d&&(i=d.innerText);const c=parseInt(o.querySelector(".qty-input").value),u=parseFloat(o.dataset.cost),b=o.querySelector(".supplier-select").value;if(b){if(!s[b]){const h=H.allSuppliers.find(f=>f.id===b);s[b]={info:h,items:[]}}s[b].items.push({name:i,qty:c,cost:u})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}H.finalOrders=s,H.step=2,Ke()}if(e.target.closest("#btn-back-step1")&&(H.step=1,Ke()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),s=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),o=parseFloat(a.dataset.total),n=H.isQuoteMode;if(s.phone){const i=s.phone.replace(/\D/g,"");let l="";n?(l=`Olá *${s.name}*, tudo bem?
...
Aguardo retorno.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order");if(a.disabled)return;const s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=m.establishmentId,a.disabled=!0,a.textContent="A processar...",Se(m.establishmentId).then(r=>{const o=r.purchaseConfig||null;return ui(s,o)}).then(()=>{g("Sucesso","Compra registrada e integrada ao financeiro!","success"),a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{a.disabled=!1,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Salvar',g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-delete-purchase")){const s=e.target.closest(".btn-delete-purchase").dataset.id;O("Excluir Compra","Isto apagará o registo histórico E o lançamento financeiro associado. Deseja continuar?").then(async r=>{if(r)try{await mi(s,m.establishmentId),g("Sucesso","Compra e financeiro excluídos.","success"),vs()}catch(o){g("Erro","Erro ao excluir: "+o.message,"error")}})}if(e.target.closest(".btn-print-order")){const s=e.target.closest(".supplier-order-card").dataset.supplierId,r=H.finalOrders[s];r?Pi(r,H.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);Ti(s)}},be.addEventListener("click",de),be.addEventListener("input",de),be.addEventListener("change",de),pt("list")}function pt(e){$t=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),Si()}const Zt=document.getElementById("content"),po={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let ne=new Set,gt=null,Be=null;function Di(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Mi(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",s=v(t.name),r=v(t.specialty||"Especialidade"),o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,n=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function Kt(){const e=document.getElementById("genericModal");e.style.display="none",Be&&e.removeEventListener("click",Be)}async function Ai(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},s=v(a.name),r=m.services||await $e(m.establishmentId),o=m.professionals||await K(m.establishmentId),n=`
        ...
        </div>`;t.innerHTML=n,t.style.display="flex",qi(a,r),Ni(a),Ri(a,o),Fi(a)}function qi(e,t){const a=document.getElementById("professionalForm"),s=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(y,k)=>{const $=k+1,w=$==s[1]?"selected":"",E=new Date(0,k).toLocaleString("pt-BR",{month:"long"});return`<option value="${$}" ${w}>${E.charAt(0).toUpperCase()+E.slice(1)}</option>`}).join(""),o=e.status||"active",n=v(e.name||""),i=v(e.specialty||""),l=v(e.phone||""),d=v(e.notes||"");a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${d}</textarea></div>`;const c=document.getElementById("profPhotoInput"),u=document.getElementById("profPhotoButton"),p=document.getElementById("profPhotoPreview"),b=document.getElementById("profPhotoBase64"),h=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,f=e.photo||"";u&&u.addEventListener("click",()=>c.click()),c&&(c.onchange=async()=>{const y=c.files[0];if(y){p.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const k=await Go(y,800,800,.8),w=k.length*3/4,E=1e3*1024;if(w>E)throw new Error("A imagem é muito grande mesmo após a compressão.");p.src=k,b.value=k}catch(k){g("Erro de Imagem",k.message||"Não foi possível processar a imagem.","error"),p.src=h,b.value=f,c.value=""}}})}function Ni(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',ji(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function Ri(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
        ...
        </section>`,gt&&Zt.removeEventListener("click",gt),gt=t=>{const a=t.target.closest('[data-action="open-professional-modal"]'),s=t.target.closest('[data-action="batch-delete"]');if(a){t.preventDefault();let o={};if(a.dataset.professional)try{o=JSON.parse(a.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}Ai(o);return}if(s){Hi();return}const r=t.target.closest(".professional-checkbox");if(r){const o=r.dataset.id;r.checked?ne.add(o):ne.delete(o),We(),ga();return}},Zt.addEventListener("click",gt),document.getElementById("profSearchInput").addEventListener("input",We),document.getElementById("showInactiveProfToggle").addEventListener("change",We);const e=document.getElementById("professionalsList");m.professionals=null,m.services=null,We();try{const[t,a]=await Promise.all([K(m.establishmentId),$e(m.establishmentId)]);m.professionals=t,m.services=a,We(),ga()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}let L={clients:[],selectedClient:null,activeTab:"profile",searchTerm:"",loading:!1,historyLimit:20,historySearchTerm:"",historyLoading:!1,historyData:{appointments:[],sales:[]}},hs=null;function Gt(){hs.innerHTML=`
        <section class="h-full flex flex-col bg-gray-50">
            ...
            <div id="clients-content-area" class="flex-grow overflow-y-auto p-2 sm:p-4 custom-scrollbar">
                ${L.loading?'<div class="loader mx-auto mt-10"></div>':""}
            </div>
        </section>
    `;const e=document.getElementById("btn-new-client"),t=document.getElementById("btn-back-list");e&&(e.onclick=Wi),t&&(t.onclick=()=>{L.selectedClient=null,Oe()})}function Oe(){Gt();const e=document.getElementById("clients-content-area"),t=L.clients.filter(o=>o.name.toLowerCase().includes(L.searchTerm.toLowerCase())||o.phone&&o.phone.includes(L.searchTerm)),a=`
            ...
            ${t.map(o=>`
                <div class="client-card bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition cursor-pointer group relative overflow-hidden active:bg-gray-50" data-id="${o.id}">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-grow min-w-0">
                            <h3 class="font-bold text-gray-800 truncate text-base sm:text-lg">${v(o.name)}</h3>
    ...
    `;e.innerHTML=a+s;const r=document.getElementById("client-search");r.focus(),r.setSelectionRange(r.value.length,r.value.length),r.addEventListener("input",o=>{L.searchTerm=o.target.value,Oe()}),e.querySelectorAll(".client-card").forEach(o=>{o.onclick=()=>xs(o.dataset.id)})}async function tt(){Gt();const e=document.getElementById("clients-content-area"),t=L.selectedClient;if(!t)return Oe();L.activeTab!=="profile"&&await _i(t.id);const a=`
        <div class="flex gap-2 mb-6 border-b overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
            <button class="tab-btn ${L.activeTab==="profile"?"active-tab":""}" data-tab="profile">
                👤 Perfil
            </button>
            <button class="tab-btn ${L.activeTab==="appointments"?"active-tab":""}" data-tab="appointments">
                📅 Agendamentos
            </button>
            <button class="tab-btn ${L.activeTab==="history"?"active-tab":""}" data-tab="history">
                💰 Histórico
            </button>
        </div>
    `;let s="";L.activeTab==="profile"?s=Oi(t):L.activeTab==="appointments"?s=zi():L.activeTab==="history"&&(s=Vi()),e.innerHTML=`
        <div class="max-w-4xl mx-auto w-full bg-white sm:rounded-2xl shadow-none sm:shadow-lg border-x-0 sm:border border-gray-200 overflow-hidden -mt-2 sm:mt-0">
            <div class="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white relative">
                <div class="flex flex-col md:flex-row items-center gap-4 relative z-10">
                    ...
                    <div class="text-center md:text-left">
                        <h2 class="text-2xl font-bold leading-tight">${v(t.name)}</h2>
            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="animate-fade-in relative min-h-[300px]">
                    ${L.historyLoading?'<div class="absolute inset-0 bg-white/80 flex items-start justify-center pt-10 z-20"><div class="loader"></div></div>':""}
                    ${s}
                </div>
            </div>
        </div>
    ...
    `,e.appendChild(r),e.querySelectorAll(".tab-btn").forEach(i=>{i.onclick=()=>{L.activeTab!==i.dataset.tab&&(L.historyLimit=20,L.historySearchTerm=""),L.activeTab=i.dataset.tab,tt()}}),L.activeTab==="profile"&&(document.getElementById("form-edit-client").onsubmit=Ji,document.getElementById("btn-delete-client").onclick=Gi);const o=document.getElementById("history-search-input");if(o){o.focus();const i=o.value;o.value="",o.value=i,o.addEventListener("input",l=>{L.historySearchTerm=l.target.value,tt()})}const n=document.getElementById("btn-load-more");n&&(n.onclick=()=>{L.historyLimit+=20,tt()}),e.querySelectorAll("[data-go-agenda]").forEach(i=>{i.onclick=l=>{const d=i.dataset.date,c=i.dataset.id;Z("agenda-section",{targetDate:new Date(d),scrollToAppointmentId:c})}}),e.querySelectorAll("[data-go-comanda]").forEach(i=>{i.onclick=l=>{const d=i.dataset.id;Z("comandas-section",{selectedAppointmentId:d,initialFilter:"finalizadas"})}})}function Oi(e){return`
        <form id="form-edit-client" class="space-y-4 pb-10">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700">Nome Completo</label>
                    <input type="text" name="name" value="${v(e.name)}" required class="mt-1 block w-full p-3 rounded-lg border border-gray-300 focus:ring-indigo-500 bg-gray-50 focus:bg-white transition text-base">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Telefone</label>
                    <input type="tel" name="phone" value="${v(e.phone||"")}" class="mt-1 block w-full p-3 rounded-lg border border-gray-300 focus:ring-indigo-500 bg-gray-50 focus:bg-white transition text-base">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">E-mail</label>
                    <input type="email" name="email" value="${v(e.email||"")}" class="mt-1 block w-full p-3 rounded-lg border border-gray-300 focus:ring-indigo-500 bg-gray-50 focus:bg-white transition text-base">
                </div>
                <div class="grid grid-cols-2 gap-2">
                    <div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3" class="mt-1 block w-full p-3 rounded-lg border border-gray-300 focus:ring-indigo-500 bg-gray-50 focus:bg-white transition text-base">${v(e.notes||"")}</textarea>
            </div>
            ...
            <div class="history-card bg-white border border-gray-200 rounded-lg p-3 sm:p-4 mb-3 transition-all cursor-pointer relative overflow-hidden group active:bg-gray-50"
                ...
                <div class="flex justify-between items-start pl-2">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-0.5 font-medium line-clamp-1">${v(r.serviceName||"Serviço")}</p>
                        <p class="text-xs text-gray-500 mt-0.5">Prof: ${v(r.professionalName||"N/A")}</p>
                    </div>
    ...
    `}function Vi(e){let t=L.historyData.sales||[];if(L.historySearchTerm){const a=L.historySearchTerm.toLowerCase();t=t.filter(s=>s.id.includes(a)||s.date&&new Date(s.date).toLocaleDateString().includes(a))}return t.sort((a,s)=>new Date(s.date)-new Date(a.date)),t.length===0&&!L.historySearchTerm?`
    ...
    `}async function Ui(){L.loading=!0,Gt();try{L.clients=await lt(m.establishmentId,""),Oe()}catch{g("Erro","Falha ao carregar clientes.","error")}finally{L.loading=!1,Oe()}}async function _i(e){const t=L.selectedClient;if(!(!t||!t.phone)){if(L.historyLoading=!0,document.getElementById("clients-content-area").querySelector(".tab-btn")){const a=document.createElement("div");a.className="absolute inset-0 bg-white/70 flex items-start justify-center pt-20 z-50 transition-opacity duration-300",a.innerHTML='<div class="loader"></div>';const s=document.querySelector(".animate-fade-in");s&&s.appendChild(a)}try{const a=new Date;a.setMonth(a.getMonth()+12);const s=new Date;s.setFullYear(s.getFullYear()-5);let r=`${m.apiBaseUrl||""}/api/appointments/${m.establishmentId}?startDate=${s.toISOString()}&endDate=${a.toISOString()}`;r+=`&clientPhone=${encodeURIComponent(t.phone)}`,r+=`&limit=${L.historyLimit}`;const o=await fetch(r,{headers:{Authorization:`Bearer ${m.token}`}});if(!o.ok)throw new Error("Falha API");const n=await o.json();L.historyData.appointments=n,L.historyData.sales=n.filter(i=>i.status==="completed").map(i=>({id:i.id,date:i.startTime,totalAmount:i.totalAmount||0,items:i.services||[]}))}catch(a){console.error("Erro ao buscar histórico otimizado",a),g("Erro","Falha ao carregar histórico.","error")}finally{L.historyLoading=!1,tt()}}}function xs(e){L.selectedClient=L.clients.find(t=>t.id===e),L.activeTab="profile",L.historyLimit=20,L.historySearchTerm="",tt()}function Wi(){const e=`
    ...
    `;const a=t.querySelector("#establishmentTimezone");if(e.timezone)a.value=e.timezone;else try{const o=Intl.DateTimeFormat().resolvedOptions().timeZone;Array.from(a.options).some(i=>i.value===o)?a.value=o:a.value="America/Sao_Paulo"}catch{a.value="America/Sao_Paulo"}const s=t.querySelector("#establishmentWorkingHoursContainer"),r=e.workingHours||{};Object.keys(ea).forEach(o=>{const n=r[o]||{},i=ea[o],l=n.active!==!1,d=document.createElement("div");d.className=`day-schedule-card p-4 rounded-lg ${l?"bg-gray-50":"bg-gray-100 disabled"}`,d.innerHTML=`
            ...
            </div>`,s.appendChild(d)}),s.addEventListener("change",o=>{const n=o.target.closest('.day-schedule-card input[type="checkbox"]');n&&n.closest(".day-schedule-card").classList.toggle("disabled",!n.checked)}),t.querySelector("#working-hours-form").addEventListener("submit",o=>{o.preventDefault();const n={};Object.keys(ea).forEach(l=>{n[l]={active:t.querySelector(`#est-${l}-active`).checked,start:t.querySelector(`#est-${l}-start`).value,end:t.querySelector(`#est-${l}-end`).value,breakStart:t.querySelector(`#est-${l}-breakStart`).value,breakEnd:t.querySelector(`#est-${l}-breakEnd`).value}});const i=t.querySelector("#establishmentTimezone").value;ze({workingHours:n,timezone:i},o)})}function ol(e,t){const a=e.loyaltyProgram||{},s=a.type||"amount",r=a.pointsPerCurrency||10,o=a.pointsPerVisit||1;t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
             <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#loyaltyEnabled").checked=a.enabled||!1;const n=t.querySelectorAll('input[name="loyaltyType"]'),i=t.querySelector("#loyalty-config-amount"),l=t.querySelector("#loyalty-config-visit");n.forEach(u=>{u.addEventListener("change",p=>{const b=p.target.value;n.forEach(h=>{const f=h.closest("label");h.checked?f.classList.add("border-indigo-500","bg-indigo-50","ring-1","ring-indigo-500"):f.classList.remove("border-indigo-500","bg-indigo-50","ring-1","ring-indigo-500")}),b==="amount"?(i.classList.remove("hidden"),l.classList.add("hidden")):(i.classList.add("hidden"),l.classList.remove("hidden"))})});const d=t.querySelector("#loyaltyTiersContainer"),c=(u={})=>{const p=document.createElement("div"),b=v(u.reward||"");return p.className="loyalty-tier-row group bg-white md:bg-transparent p-3 md:p-0 border md:border-0 rounded-lg shadow-sm md:shadow-none relative",p.innerHTML=`
        ...
        `,p};(a.tiers||[]).forEach(u=>{d.appendChild(c(u))}),t.querySelector("#add-loyalty-tier").addEventListener("click",()=>{d.appendChild(c())}),d.addEventListener("click",u=>{const p=u.target.closest(".remove-loyalty-tier");p&&p.closest(".loyalty-tier-row").remove()}),t.querySelector("#loyalty-form").addEventListener("submit",u=>{u.preventDefault();const p=t.querySelector('input[name="loyaltyType"]:checked').value,b=Array.from(t.querySelectorAll("#loyaltyTiersContainer .loyalty-tier-row")).map(f=>({points:parseInt(f.querySelector('input[data-field="points"]').value,10)||0,reward:f.querySelector('input[data-field="reward"]').value,discount:parseFloat(f.querySelector('input[data-field="discount"]').value)||0})),h={loyaltyProgram:{enabled:t.querySelector("#loyaltyEnabled").checked,type:p,pointsPerCurrency:parseFloat(t.querySelector("#loyaltyPointsPerCurrency").value)||10,pointsPerVisit:parseInt(t.querySelector("#loyaltyPointsPerVisit").value,10)||1,tiers:b.filter(f=>f.points>0&&f.reward).map(f=>({costPoints:f.points,reward:f.reward,name:f.reward,discount:f.discount}))}};ze(h,u)})}async function sl(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;try{const[a,s]=await Promise.all([Ia(m.establishmentId),Ca(m.establishmentId)]),r=e.financialIntegration||{},o=e.commissionConfig||{},n=e.purchaseConfig||{};t.querySelector("#financialNatureId").innerHTML=Ce(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=Ce(s,r.defaultCentroDeCustoId),t.querySelector("#purchaseNatureId").innerHTML=Ce(a,n.defaultNatureId),t.querySelector("#purchaseCostCenterId").innerHTML=Ce(s,n.defaultCostCenterId),t.querySelector("#commissionNatureId").innerHTML=Ce(a,o.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=Ce(s,o.defaultCostCenterId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const s={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},purchaseConfig:{defaultNatureId:t.querySelector("#purchaseNatureId").value||null,defaultCostCenterId:t.querySelector("#purchaseCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};ze(s,a)})}function rl(e,t){const a="5516997859430",s=encodeURIComponent("Olá, preciso de ajuda com o sistema Kairos."),r=`https://wa.me/${a}?text=${s}`;t.innerHTML=`
    ...
    `}function ws(e="indigo",t){const a=t.querySelector("#color-palette-container"),s=t.querySelector("#establishmentThemeColor");!a||!s||(a.innerHTML="",Object.entries(Qi).forEach(([r,o])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{s.value=r,ws(r,t)}),a.appendChild(i)}),s.value=e)}function ll(e,t){const a=t.querySelector("#slotIntervalContainer"),s=t.querySelector("#establishmentSlotInterval");if(!a||!s)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(o=>{const n=o.value===e;return`<button type="button" data-value="${o.value}" 
    ...
    `,he.querySelector('button[data-action="back-to-list"]').addEventListener("click",s=>{s.preventDefault(),ks()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":Xi(J,a);break;case"change-password":Zi(J,a);break;case"change-email":Ki(J,a);break;case"branding":el(J,a);break;case"booking":tl(J,a);break;case"working-hours":al(J,a);break;case"loyalty":ol(J,a);break;case"financial":await sl(J,a);break;case"support":rl(J,a);break;case"cancellation":nl(J,a);break;default:il(t?t.label:"Definições",a)}}async function ks(){if(he.innerHTML=`
    ...
    `,he.querySelector("#settings-menu-list").addEventListener("click",o=>{const n=o.target.closest("button[data-section]");n&&(o.preventDefault(),dl(n.dataset.section))});const r=he.querySelector('[data-action="go-to-my-profile"]');r&&r.addEventListener("click",o=>{o.preventDefault(),Z("my-profile-section")})}const at=document.getElementById("content");async function qe(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,s=document.getElementById("filterEndDate")?.value,r=await Ot(m.establishmentId,a||new Date().toISOString().split("T")[0],s||new Date().toISOString().split("T")[0],e),o=document.getElementById("filterReason")?.value.toLowerCase(),n=o?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(o)):r,i=n.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let p=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <h4 class="font-bold text-gray-700">${v(l)} (${d.length})</h4>`;if(d.length>1){const b=JSON.stringify(d.map(h=>h.id));p+=`<button data-action="batch-delete-blockage" data-ids='${b}' class="text-xs text-red-600 hover:text-red-800 font-semibold flex items-center gap-1">
                                    Apagar Lote
                                </button>`}p+="</div>",c.innerHTML=p,d.forEach(b=>{const h=new Date(b.startTime),f=new Date(b.endTime),y=h.toLocaleDateString("pt-BR"),k=f.toLocaleDateString("pt-BR"),w=`
                    <div class="bg-white p-3 rounded-md flex items-center justify-between shadow-sm">
                        <div>
                            <p class="font-medium text-gray-800 text-sm">
                                ${y===k?`${y} | ${h.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`:`De ${y} às ${h.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}<br>Até ${k} às ${f.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`}
                            </p>
                        </div>
        ...
        <section>
        ...
        </section>`,ml(t),await qe(t);const r=document.getElementById("batchProfSelectionContainer");try{const o=await K(m.establishmentId);r.innerHTML=o.map(n=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{r.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const gl=e=>S(`/api/users/${e}`),bl=e=>S("/api/users",{method:"POST",body:JSON.stringify(e)}),fl=(e,t)=>S(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),vl=e=>S(`/api/users/${e}`,{method:"DELETE"}),hl=(e,t)=>S(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),xl=(e,t)=>S(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),fe=document.getElementById("content"),yl={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},wl={view:"Visualizar",create:"Criar",edit:"Editar"};let Je=null,Ge=null;function kl(e){const t=document.getElementById("usersListContainer");if(!t)return;const a=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const s=a?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${s}</p>`;return}e.sort((s,r)=>(s.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(s=>{const r=JSON.stringify(s).replace(/'/g,"&apos;"),o=s.status==="active",n=m.professionals.find(c=>c.id===s.professionalId),i=n?n.name:"N/A",l=n?n.name.charAt(0):s.name.charAt(0),d=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(l)}`;return`
    ...
    `}).join("")}function Et(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=m.users:t=m.users.filter(a=>a.status==="active"),kl(t)}function Sl(e={}){return Object.entries(yl).map(([t,a])=>{const s=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,o=Object.entries(wl).map(([i,l])=>`
    ...
    `}).join("")}async function bo(e=null){document.getElementById("user-list-view").classList.add("hidden");const t=document.getElementById("user-form-view");t.classList.remove("hidden");let a=m.professionals;if(!a||a.length===0)try{a=await K(m.establishmentId),m.professionals=a}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error")}const s=k=>a.find($=>$.id===k),r=(k,$)=>{const E=s(k)?.photo,T=$.charAt(0).toUpperCase();return{photoSrc:E||`https://placehold.co/128x128/E2E8F0/4A5568?text=${T}`,initials:T,photoUrl:E||""}},o=e?.professionalId,n=e?.name||"Novo Usuário",i=r(o,n),l=s(o),d=k=>{let $='<option value="">-- Não Associado a um Profissional --</option>';return $+=a.map(w=>`<option value="${w.id}" ${w.id===k?"selected":""}>${w.name} (${w.specialty||"N/A"})</option>`).join(""),$},c=e!==null;t.querySelector("#userFormTitle").textContent=c?`Editar Usuário: ${e.name}`:"Novo Usuário";const u=t.querySelector("#userForm");u.innerHTML=`
    ...
    `;const p=window.innerWidth<768,b=u.querySelector(".bg-white");if(p&&b){b.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const k=u.closest("section");k&&(k.style.padding="0",k.style.margin="0"),b.classList.add("p-4")}const h=u.querySelector("#userProfessionalId"),f=u.querySelector("#userPhotoPreview"),y=u.querySelector("#profPhotoName");if(h.addEventListener("change",k=>{const $=k.target.value,w=s($),E=w?w.name:"Selecione um profissional",T=r($,n);f.src=T.photoSrc,y.textContent=E,u.querySelector("#professionalPhotoUrl").value=T.photoUrl}),u.addEventListener("submit",async k=>{k.preventDefault();const $=e?.email,w=u.querySelector("#userEmail").value,E={};u.querySelectorAll('input[type="checkbox"]').forEach(F=>{const P=F.dataset.module,B=F.dataset.permission;E[P]||(E[P]={}),E[P][B]=F.checked});const T=u.querySelector("#userProfessionalId").value||null,q={name:u.querySelector("#userName").value,permissions:E,professionalId:T,establishmentId:m.establishmentId};try{c?($!==w&&(q.email=w),await fl(e.id,q),g("Usuário atualizado com sucesso!","success")):(q.email=u.querySelector("#userEmail").value,q.password=u.querySelector("#userPassword").value,await bl(q),g("Usuário criado com sucesso!","success")),Dt()}catch(F){g(`Erro: ${F.message}`,"error")}}),c){const k=u.querySelector("#password-change-container"),$=k.querySelector('[data-action="show-password-form"]'),w=k.querySelector("#password-form"),E=w.querySelector('[data-action="save-password"]'),T=w.querySelector('[data-action="cancel-password-change"]');$.addEventListener("click",()=>{$.classList.add("hidden"),w.classList.remove("hidden")}),T.addEventListener("click",()=>{$.classList.remove("hidden"),w.classList.add("hidden"),w.querySelector("#userNewPassword").value=""}),E.addEventListener("click",async()=>{const q=w.querySelector("#userNewPassword").value;if(!q||q.length<6){g("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await O("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{E.disabled=!0,E.textContent="Aguarde...",await hl(e.id,q),g("Sucesso!","A senha do usuário foi alterada.","success"),$.classList.remove("hidden"),w.classList.add("hidden"),w.querySelector("#userNewPassword").value=""}catch(P){g("Erro",`Não foi possível alterar a senha: ${P.message}`,"error")}finally{E.disabled=!1,E.textContent="Salvar Nova Senha"}})}}async function $l(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,a]=await Promise.all([gl(m.establishmentId),K(m.establishmentId)]);m.users=t,m.professionals=a,Et()}catch{g("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function Dt(){fe.innerHTML=`
    ...
    `,Je&&fe.removeEventListener("click",Je),Ge&&fe.removeEventListener("change",Ge),Je=async e=>{if(!document.getElementById("user-list-view")){fe.removeEventListener("click",Je);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":bo();break;case"edit-user":const s=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));bo(s);break;case"back-to-list":Dt();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await O("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await vl(r),g("Usuário excluído com sucesso!","success"),Dt()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Ge=async e=>{if(!document.getElementById("user-list-view")){fe.removeEventListener("change",Ge);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")Et();else if(t){e.stopPropagation();const a=t.dataset.userId,s=t.checked?"active":"inactive";try{await xl(a,s),g(`Usuário ${s==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=m.users.findIndex(o=>o.id===a);r>-1&&(m.users[r].status=s,Et())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,Et()}}},fe.addEventListener("click",Je),fe.addEventListener("change",Ge),await $l()}const El=document.getElementById("content");let fo={},ba=null;function Il(){Object.values(fo).forEach(e=>e?.destroy()),fo={}}function Cl(e,t){if(!window.jspdf){g("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:a}=window.jspdf,s=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(s.setFontSize(18),s.text(e,s.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];s.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const o=s.lastAutoTable?s.lastAutoTable.finalY+20:60;s.text("Detalhes das Vendas",20,o),s.autoTable({html:`#${t}`,startY:o+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),s.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function vo(e){const t=document.getElementById("genericModal"),a=v(e.client),s=v(e.items),r=v(e.responsavelCaixa||"N/A"),o=(e.payments||[]).map(n=>`
    ...
    `}).join(""),o.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=ba.transactions[l];d&&vo(d)})}),n.innerHTML=a.map((i,l)=>{const d=v(i.client),c=v(i.items),u=v(i.type);return`
    ...
    `}).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const l=i.dataset.transactionIndex,d=ba.transactions[l];d&&vo(d)})})}async function ho(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const s=t.value,r=a.value;if(!s||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const o=document.getElementById("cashierSessionFilter").value,n=await Br({establishmentId:m.establishmentId,startDate:s,endDate:r,cashierSessionId:o});ba=n,e.innerHTML=`
        ...
        `;e.innerHTML=t.map(r=>s(r)).join("")}async function fa(e){const t=document.getElementById("genericModal"),a=e==="nature",s=a?"Naturezas Financeiras":"Centros de Custo",r=a?Ia:Ca,o=a?Tn:Bn,n=a?"natures":"costCenters";t.innerHTML=`
        ...
        </div>`,t.style.display="flex";const i=t.querySelector("#hierarchyList"),l=t.querySelector("#itemParent"),d=u=>{const p=Aa(u);Dl(i,p,e);const b=l.value;l.innerHTML='<option value="">-- Categoria Principal --</option>';const h=(f,y=0)=>{const k="  ".repeat(y)+(y>0?"↳ ":"");l.innerHTML+=`<option value="${f.id}">${k}${f.name}</option>`,f.children.forEach($=>h($,y+1))};p.forEach(f=>h(f)),l.value=b};try{const u=await r(m.establishmentId);M[n]=u,d(u)}catch(u){console.error(u)}const c=t.querySelector("#hierarchyForm");c.addEventListener("submit",async u=>{u.preventDefault();const p=t.querySelector("#itemName").value,b=l.value;try{await o({name:p,parentId:b||null,establishmentId:m.establishmentId});const h=await r(m.establishmentId);M[n]=h,d(h),c.reset(),await Ve(),g("Sucesso","Item adicionado.","success")}catch(h){g("Erro",h.message,"error")}})}async function Ml(){Al(),ql(),await Ve()}function Al(){Pl.innerHTML=`
    ...
    `}function ql(){document.getElementById("toggle-filter-btn").addEventListener("click",()=>{const a=document.getElementById("filter-panel"),s=document.getElementById("toggle-filter-btn");M.isFilterOpen=!M.isFilterOpen,M.isFilterOpen?(a.classList.remove("hidden"),s.classList.add("bg-indigo-100","text-indigo-600")):(a.classList.add("hidden"),s.classList.remove("bg-indigo-100","text-indigo-600"))}),document.getElementById("settings-btn").addEventListener("click",Ol),document.getElementById("fab-add").addEventListener("click",()=>{const a=M.currentTab==="payables"?"payable":"receivable";ta(a)});const e=document.getElementById("tab-receivables"),t=document.getElementById("tab-payables");e.addEventListener("click",()=>xo("receivables")),t.addEventListener("click",()=>xo("payables")),document.querySelectorAll(".status-filter-btn").forEach(a=>{a.addEventListener("click",s=>{document.querySelectorAll(".status-filter-btn").forEach(r=>{r.classList.remove("bg-indigo-100","text-indigo-700"),r.classList.add("text-gray-500")}),s.target.classList.add("bg-indigo-100","text-indigo-700"),s.target.classList.remove("text-gray-500"),M.statusFilter=s.target.dataset.status})}),document.getElementById("apply-filter-btn").addEventListener("click",()=>{M.startDate=document.getElementById("filterStartDate").value,M.endDate=document.getElementById("filterEndDate").value,M.filterNaturezaId=document.getElementById("filterNaturezaId").value,M.filterCostCenterId=document.getElementById("filterCostCenterId").value,document.getElementById("toggle-filter-btn").click(),Ve()}),bt&&document.body.removeEventListener("click",bt),bt=a=>{const s=a.target,r=s.closest("button[data-action]");if(r){const{action:n,type:i,id:l}=r.dataset;if(a.stopPropagation(),n==="delete"){Fl(i,l);return}if(n==="mark-as-paid"){jl(i,l);return}if(n==="manage-natures"){fa("nature");return}if(n==="manage-cost-centers"){fa("cost-center");return}if(n==="edit"){const d=JSON.parse(r.dataset.item.replace(/&apos;/g,"'"));ta(i,d);return}}const o=s.closest(".financial-card-item");if(o&&document.getElementById("list-container").contains(o)){const{type:n}=o.dataset,i=JSON.parse(o.dataset.item.replace(/&apos;/g,"'"));ta(n,i)}},document.body.addEventListener("click",bt),ft&&document.getElementById("genericModal").removeEventListener("click",ft),ft=a=>{if(a.target.closest('[data-action="close-modal"]')){document.getElementById("genericModal").style.display="none";return}const r=a.target.closest('button[data-action^="delete-"]');if(r){const o=r.dataset.action.split("-")[1];Hl(o,r.dataset.id)}},document.getElementById("genericModal").addEventListener("click",ft)}function xo(e){M.currentTab=e;const t=document.getElementById("tab-receivables"),a=document.getElementById("tab-payables"),s=document.getElementById("fab-add");e==="receivables"?(t.classList.add("bg-white","text-green-700","shadow-sm"),t.classList.remove("text-gray-500"),a.classList.remove("bg-white","text-red-700","shadow-sm"),a.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-green-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-green-700 hover:scale-105 transition-all z-40"):(a.classList.add("bg-white","text-red-700","shadow-sm"),a.classList.remove("text-gray-500"),t.classList.remove("bg-white","text-green-700","shadow-sm"),t.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-red-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-red-700 hover:scale-105 transition-all z-40"),Ss()}async function Ve(){const e=document.getElementById("list-container");e.innerHTML='<div class="loader mx-auto my-10"></div>';try{if(M.natures.length===0){const[r,o]=await Promise.all([Ia(m.establishmentId),Ca(m.establishmentId)]);M.natures=r,M.costCenters=o,Nl()}const t={startDate:M.startDate,endDate:M.endDate,establishmentId:m.establishmentId};M.filterNaturezaId!=="all"&&(t.natureId=M.filterNaturezaId),M.filterCostCenterId!=="all"&&(t.costCenterId=M.filterCostCenterId);const[a,s]=await Promise.all([An(t),Fn(t)]);M.payables=a.entries||[],M.receivables=s.entries||[],Rl(),Ss()}catch(t){e.innerHTML=`<p class="text-center text-red-500 mt-10">Erro ao carregar: ${t.message}</p>`}}function Nl(){const e=s=>{let r='<option value="all">Todas</option>';const o=Aa(s),n=(i,l=0)=>{const d="  ".repeat(l)+(l>0?"↳ ":"");r+=`<option value="${i.id}">${d}${i.name}</option>`,i.children.forEach(c=>n(c,l+1))};return o.forEach(i=>n(i)),r},t=document.getElementById("filterNaturezaId"),a=document.getElementById("filterCostCenterId");t&&(t.innerHTML=e(M.natures)),a&&(a.innerHTML=e(M.costCenters)),t&&(t.value=M.filterNaturezaId),a&&(a.value=M.filterCostCenterId)}function Rl(){const e=document.getElementById("summary-section");if(!e)return;const t=M.receivables.reduce((l,d)=>l+d.amount,0),a=M.receivables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),s=t-a,r=M.payables.reduce((l,d)=>l+d.amount,0),o=M.payables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),n=r-o,i=a-o;e.innerHTML=`
    ...
    `}function Ss(){const e=document.getElementById("list-container");if(!e)return;const t=M.currentTab==="receivables",a=t?M.receivables:M.payables;let s=a;if(M.statusFilter!=="all"&&(s=a.filter(n=>n.status===M.statusFilter)),s.sort((n,i)=>new Date(n.dueDate)-new Date(i.dueDate)),s.length===0){e.innerHTML=`
        ...
        `;return}const r=new Map(M.natures.map(n=>[n.id,n.name])),o=t?"receivable":"payable";e.innerHTML=s.map(n=>{const i=Bl(n.dueDate),l=n.status==="paid",d=l?"text-gray-400":t?"text-green-600":"text-red-500",c=n.naturezaId&&r.get(n.naturezaId)||"Geral",u=JSON.stringify(n).replace(/'/g,"&apos;");return`
        ...
        `}).join("")}async function jl(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?Rn(t,a):zn(t,a)),g("Sucesso","Lançamento baixado!","success"),await Ve()}catch(s){g("Erro",s.message,"error")}}async function Fl(e,t){if(await O("Excluir Lançamento","Tem certeza? Essa ação não pode ser desfeita."))try{await(e==="payable"?Nn(t):On(t)),g("Sucesso","Lançamento excluído.","success"),await Ve()}catch(s){g("Erro",s.message,"error")}}async function Hl(e,t){const s=e==="nature"?Pn:Dn;if(await O("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await s(t),fa(e==="nature"?"nature":"cost-center")}catch(o){g("Erro",o.message,"error")}}function Ol(){const e=document.getElementById("genericModal");e.innerHTML=`
    ...
    `,e.style.display="flex"}function ta(e,t=null){const a=document.getElementById("genericModal"),s=e==="payable",r=`${t?"Editar":"Nova"} ${s?"Despesa":"Receita"}`,o=s?"red":"green",n=(u,p)=>{let b='<option value="">-- Selecione --</option>';const h=Aa(u),f=(y,k=0)=>{const $="  ".repeat(k)+(k>0?"↳ ":""),w=y.id===p?"selected":"";b+=`<option value="${y.id}" ${w}>${$}${y.name}</option>`,y.children.forEach(E=>f(E,k+1))};return h.forEach(y=>f(y)),b};a.innerHTML=`
        ...
        </div>`,a.style.display="flex";const i=a.querySelector("#financial-form"),l=i.querySelector("#status-toggle"),d=i.querySelector("#payment-date-wrapper"),c=i.querySelector('[name="paymentDate"]');if(!t){const u=i.querySelector("#toggle-installments"),p=i.querySelector("#installments-container");u.addEventListener("change",()=>{p.classList.toggle("hidden",!u.checked)})}l.addEventListener("change",()=>{l.checked?(d.classList.remove("hidden"),c.required=!0):(d.classList.add("hidden"),c.required=!1)}),i.addEventListener("submit",async u=>{u.preventDefault();const p=new FormData(i),b=l.checked,h={description:p.get("description"),amount:parseFloat(p.get("amount")),dueDate:p.get("dueDate"),naturezaId:p.get("naturezaId")||null,centroDeCustoId:p.get("centroDeCustoId")||null,notes:p.get("notes"),status:b?"paid":"pending",paymentDate:b?p.get("paymentDate"):null,establishmentId:m.establishmentId},f=p.get("installments");!t&&f&&parseInt(f)>1?h.installments=parseInt(f):h.installments=1;try{t?(await(s?qn(t.id,h):Hn(t.id,h)),g("Sucesso","Atualizado com sucesso!","success")):(await(s?Mn(h):jn(h)),g("Sucesso","Criado com sucesso!","success")),document.getElementById("genericModal").style.display="none",Ve()}catch(y){g("Erro",y.message,"error")}})}const zl=e=>S("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),Vl=e=>S("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),Ul=(e,t)=>{const a=new URLSearchParams({startDate:e,endDate:t}).toString();return S(`/api/commissions/stats?${a}`)},_l=(e={})=>{Object.keys(e).forEach(s=>(e[s]===void 0||e[s]===null||e[s]==="")&&delete e[s]);const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return S(a)},Wl=e=>S(`/api/commissions/report/${e}`,{method:"DELETE"}),Mt=new Date,yo=new Date(Mt.getFullYear(),Mt.getMonth(),1),A={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:yo.toISOString().split("T")[0],dashEndDate:Mt.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:yo.toISOString().split("T")[0],histEndDate:Mt.toISOString().split("T")[0],histProfessionalId:"all"};let vt=null;const Ye=document.getElementById("content");async function Jl(){try{A.professionals=await K(m.establishmentId)}catch(e){console.error("Erro profissionais",e)}sd(),Gl(),Yt(),qt("dashboard")}function Gl(){vt&&Ye.removeEventListener("click",vt),vt=e=>{const t=e.target.closest("button");if(!t)return;const a=t.dataset.action,s=t.dataset.id,r=t.dataset.idx;switch(a){case"tab-nav":qt(t.dataset.tab);break;case"toggle-all-profs":Yl();break;case"back-to-filters":A.calculationResult=null,At(document.getElementById("commissions-content"));break;case"view-preview-items":od(r);break;case"save-final-report":Xl();break;case"start-new-calc":qt("calculator");break;case"print-receipt":Zl(s);break;case"delete-report":Kl(s);break;case"filter-dashboard":Yt();break;case"filter-history":qa();break}},Ye.addEventListener("click",vt),Ye.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;td(t)}},Ye.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),Ql())}}async function Yt(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(A.dashStartDate=e.value),t&&(A.dashEndDate=t.value);const a=document.getElementById("dashboard-stats-container");a&&(a.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const s=await Ul(A.dashStartDate,A.dashEndDate);A.dashStats={revenue:s.totalRevenue||0,commissions:s.totalCommissionsPaid||0},A.currentTab==="dashboard"&&$s(document.getElementById("commissions-content"))}catch(s){console.error(s),a&&(a.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function qa(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),a=document.getElementById("hist-prof");e&&(A.histStartDate=e.value),t&&(A.histEndDate=t.value),a&&(A.histProfessionalId=a.value);const s=document.getElementById("history-list-container");if(s){s.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await _l({startDate:A.histStartDate,endDate:A.histEndDate,professionalId:A.histProfessionalId});A.historyData=r,Es(s,r)}catch{s.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function Yl(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(a=>a.checked);e.forEach(a=>a.checked=!t)}async function Ql(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(o=>o.value);if(e.length===0)return g("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},a=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),s=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");A.periodString=`${a} a ${s}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const o=await zl(t);A.calculationResult=o.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),At(r)}catch(o){g("Erro",o.message,"error"),A.calculationResult=null,At(r)}}async function Xl(){const e=A.calculationResult.length;if(await O("Confirmar",`Gerar ${e} relatórios? Isso marcará as vendas como pagas.`))try{const a=A.calculationResult.map(s=>{const r=s.items.map(o=>o.originalSaleId).filter(o=>o!=null);return Vl({professionalId:s.professionalId,professionalName:s.professionalName,period:A.periodString,processedSalesIds:r,reportData:{...s,summary:{...s.summary,finalValue:s.finalValue,extraDebit:s.extraDebit||0,extraCredit:s.extraCredit||0,notes:s.notes||""}}})});await Promise.all(a),g("Sucesso","Pagamentos registrados!","success"),A.calculationResult=null,Yt(),qt("history")}catch(a){g("Erro",a.message,"error")}}function Zl(e){const t=A.historyData.find(a=>a.id===e);t&&ed(t)}async function Kl(e){if(await O("Excluir","Deseja remover este registro? As vendas voltarão a ficar disponíveis para cálculo."))try{await Wl(e),g("Sucesso","Registro removido.","success"),qa(),Yt()}catch(a){g("Erro",a.message,"error")}}function ed(e){const{jsPDF:t}=window.jspdf;if(!t)return g("Erro","PDF lib não carregada.","error");const a=new t,s=a.internal.pageSize.getWidth()/2;a.setFontSize(18),a.setFont(void 0,"bold"),a.text("RECIBO DE PAGAMENTO DE COMISSÃO",s,20,{align:"center"}),a.setFontSize(12),a.setFont(void 0,"normal"),a.text(`Profissional: ${e.professionalName}`,15,40),a.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),a.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const o=a.lastAutoTable.finalY+10;a.setFontSize(14),a.setFont(void 0,"bold"),a.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,o,{align:"right"}),a.save(`Recibo_${e.professionalName}.pdf`)}function td(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),a=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let s=0,r=0;if(t.forEach(o=>{o.value&&(s=parseFloat(o.value))}),a.forEach(o=>{o.value&&(r=parseFloat(o.value))}),A.calculationResult&&A.calculationResult[e]){const o=A.calculationResult[e];o.extraDebit=s,o.extraCredit=r,o.finalValue=o.summary.totalCommission-s+r,t.forEach(i=>{i!==document.activeElement&&(i.value=s||"")}),a.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${o.finalValue.toFixed(2)}`),ad()}}function ad(){const e=A.calculationResult.reduce((a,s)=>a+s.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(a=>a.innerText=`R$ ${e.toFixed(2)}`)}function od(e){const t=A.calculationResult[e];if(!t)return;const a=t.items.map(s=>`
    ...
    `).join("");X({title:"Detalhes da Comissão",contentHTML:`<div class="bg-gray-50 p-3 rounded-lg mb-4 flex justify-between items-center"><div><p class="text-xs text-gray-500">Profissional</p><p class="font-bold text-gray-800">${t.professionalName}</p></div><div class="text-right"><p class="text-xs text-gray-500">Total Itens</p><p class="font-bold text-gray-800">${t.items.length}</p></div></div><div class="border rounded-lg overflow-hidden max-h-[60vh] overflow-y-auto">${a}</div>`,maxWidth:"max-w-md"})}function At(e){if(A.calculationResult){const t=A.calculationResult,a=t.reduce((o,n)=>o+n.finalValue,0),s=t.map((o,n)=>`
            ...
            </div>`}else{const t=new Date().toISOString().split("T")[0],a=new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],s=A.professionals.map(r=>`
    ...
    `}function rd(e){const t=A.professionals.map(a=>`<option value="${a.id}" ${A.histProfessionalId===a.id?"selected":""}>${a.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `,A.historyData.length>0?Es(document.getElementById("history-list-container"),A.historyData):qa()}function Es(e,t){if(t.length===0){e.innerHTML=`
    ...
    `}const aa=document.getElementById("content");let we={allPackages:[],catalogForModal:{services:[],products:[]}},ht=null,De=null;function nd(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function id(){const e=document.getElementById("packagesListContainer");if(e){if(we.allPackages.length===0){e.innerHTML=`
        ...
        `}).join("")}}function wo(){const e=document.getElementById("genericModal");e.style.display="none",De&&e.removeEventListener("click",De)}async function ko(e=null){const t=document.getElementById("genericModal"),a=!!e,s=e?JSON.parse(JSON.stringify(e.items||[])):[],r=v(e?.name||""),o=v(e?.description||""),n=e?.price||"",i=e?.commissionRate||0,l=e?.validityDays||30,d=`
    ...
    `;t.innerHTML=d,t.style.display="flex";const c=t.querySelector("#package-items-list"),u=(b,h)=>{const f=h.querySelector("#originalPrice"),y=b.reduce((k,$)=>k+$.price*$.quantity,0);f&&(f.textContent=`R$ ${y.toFixed(2)}`)},p=b=>{b.length===0?c.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':c.innerHTML=b.map((h,f)=>{const y=h.type==="service",k=y?"Serviço":"Produto",$=y?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
            ...
            `}).join(""),u(b,t)};p(s),c.addEventListener("change",b=>{if(b.target.classList.contains("quantity-input")){const h=parseInt(b.target.dataset.index,10),f=parseInt(b.target.value,10);f>0&&s[h]&&(s[h].quantity=f,p(s))}}),c.addEventListener("click",b=>{if(b.target.classList.contains("remove-item-btn")){const h=parseInt(b.target.dataset.index,10);s.splice(h,1),p(s)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>ld(b=>{const h=s.find(f=>f.id===b.id&&f.type===b.type);h?h.quantity++:s.push({...b,quantity:1}),p(s)}),De&&t.removeEventListener("click",De),De=async b=>{const h=b.target.closest("button[data-action]");if(!h)return;const f=h.dataset.action;if(b.stopPropagation(),f==="close-modal"&&wo(),f==="save-package"){const y=h,k={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:s,originalPrice:s.reduce(($,w)=>$+w.price*w.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null,establishmentId:m.establishmentId};if(!k.name||!k.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(k.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}y.disabled=!0,y.textContent="A salvar...";try{a?await un(k.id,k):(delete k.id,await cn(k)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),wo(),await Na()}catch($){g("Erro",`Não foi possível salvar o pacote: ${$.message}`,"error"),y.disabled=!1,y.textContent="Salvar Pacote"}}},t.addEventListener("click",De)}function ld(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const s={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=l=>{const d=t.toLowerCase(),c=we.catalogForModal.services.filter(h=>h.name.toLowerCase().includes(d)),u=we.catalogForModal.products.filter(h=>h.name.toLowerCase().includes(d)),p=c.map(h=>`
    ...
    `,document.body.appendChild(a);const o=a.querySelector("#item-selection-list"),n=a.querySelector("#item-search-input"),i=()=>{a.remove()};r(o),n.addEventListener("input",()=>{t=n.value,r(o)}),a.addEventListener("click",l=>{const d=l.target.closest('[data-action="select-item"]'),c=l.target.closest('[data-action="close-selection-modal"]');if(d){const{itemType:u,itemId:p}=d.dataset,h=(we.catalogForModal[u+"s"]||[]).find(f=>f.id===p);h&&(e({...h,type:u}),i())}else(c||l.target===a)&&i()})}async function Na(){aa.innerHTML=`
    ...
    `,ht&&aa.removeEventListener("click",ht),ht=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const s=e.target.closest('[data-action="delete-package"]');if(s){const r=s.dataset.id;O("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async o=>{if(o)try{await mn(r),g("Sucesso!","Pacote excluído.","success"),await Na()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")ko(null);else if(a==="edit-package"){const s=JSON.parse(t.dataset.package);ko(s)}},aa.addEventListener("click",ht);try{const[e,t,a]=await Promise.all([ts(m.establishmentId),$e(m.establishmentId),Ft(m.establishmentId)]);we.allPackages=e,we.catalogForModal={services:t.filter(s=>s.active),products:a},id()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const dd=document.getElementById("content");let cd=null;async function ud(){const e=v(m.userName||"Usuário"),t=v(U.currentUser?.email||"E-mail não disponível"),a=m.userName?m.userName.charAt(0):"U";dd.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await md()}async function md(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=m.userProfessionalId;if(t){const a=await ar(t);cd=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo);const s=v(a.name);e.innerHTML=`
            ...
            `,pd(a.id),document.getElementById("my-blocks-filter").addEventListener("change",o=>Nt(a.id,o.target.value)),Nt(a.id,"future")}else e.innerHTML=`
    ...
    `}function va(){const e=st.filter(t=>!t.read).length;if(e>0?(ra.textContent=e,ra.classList.remove("hidden")):ra.classList.add("hidden"),st.length===0){$o.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}$o.innerHTML=st.map(t=>`
    ...
    `).join("")}function xd(e){ot&&ot();const t=pe(_,"establishments",e,"notifications"),a=Rt(t,rt("timestamp",">=",new Date),Bo("timestamp","desc"));ot=Rs(a,s=>{s.docChanges().forEach(r=>{if(r.type==="added"){const o=r.doc.data();st.unshift({title:o.title,message:o.message,time:o.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(o.title,o.message,"info",!0),va();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(o.type==="cancellation"||o.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),es())}})},s=>{console.error("Erro no listener de notificações em tempo real:",s)})}function Z(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=m.enabledModules?.[a]!==!1,o=m.userPermissions===null||m.userPermissions[e]?.view===!0;if(!r||!o){sa.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active")),xe.classList.contains("absolute")&&(xe.classList.add("hidden"),Me.classList.add("hidden"));return}}const s=vd[e];s?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),sa.innerHTML="",window.innerWidth<768&&(xe.classList.add("hidden"),Me.classList.add("hidden")),s(t)):sa.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`}async function yd(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),s=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),o=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(o&&t&&(t.classList.remove("hidden"),t.classList.add("inline-flex")),n&&a&&(a.classList.remove("hidden"),a.classList.add("inline-flex")),!(!o&&!n))try{const i=await Dr();o&&s&&(s.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i)}}async function wd(e){try{console.log("[Nativo] Iniciando configuração de Push..."),oe.getPlatform()==="android"&&(await G.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal Android criado."));let t=await G.checkPermissions();if(t.receive==="prompt"&&(t=await G.requestPermissions()),t.receive!=="granted")return;await G.register(),G.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const s=me(_,"users",e);await ha(s,{fcmTokens:Ns(a.value),platform:"native_mobile"}),console.log("Token FCM salvo no perfil do utilizador (Nativo).")}catch(s){console.error("Erro ao salvar token FCM:",s)}}),G.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),G.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),G.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),Z("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function kd(){const e=document.getElementById("exitConfirmationModal"),t=document.getElementById("btn-cancel-exit"),a=document.getElementById("btn-confirm-exit"),s=()=>e.style.display="block",r=()=>e.style.display="none",o=()=>e.style.display==="block";e&&(t.addEventListener("click",()=>{r(),oe.isNativePlatform()||history.pushState(null,document.title,location.href)}),a.addEventListener("click",()=>{r(),oe.isNativePlatform()?Ya.exitApp():history.back()}),oe.isNativePlatform()?Ya.addListener("backButton",({canGoBack:n})=>{if(o())r();else{const i=document.querySelectorAll('.modal[style*="display: block"]'),l=Array.from(i).filter(u=>u.id!=="exitConfirmationModal");if(l.length>0){l.forEach(u=>u.style.display="none");return}const d=document.getElementById("sidebar");if(d&&!d.classList.contains("hidden")&&window.innerWidth<768){d.classList.add("hidden"),document.getElementById("mobile-overlay").classList.add("hidden");return}const c=document.querySelector(".sidebar-link.active");c&&c.getAttribute("data-target")==="agenda-section"?s():Z("agenda-section")}}):(history.pushState(null,document.title,location.href),window.addEventListener("popstate",n=>{if(o()){r(),history.pushState(null,document.title,location.href);return}const i=document.querySelectorAll('.modal[style*="display: block"], .modal[style*="display: flex"]'),l=Array.from(i).filter(c=>c.id!=="exitConfirmationModal");if(l.length>0){l.forEach(c=>c.style.display="none"),history.pushState(null,document.title,location.href);return}const d=document.querySelector(".sidebar-link.active");d&&d.getAttribute("data-target")==="agenda-section"?s():(Z("agenda-section"),history.pushState(null,document.title,location.href))})))}async function Sd(){try{await Ds(U,Ms),console.log("Persistência LOCAL configurada na inicialização.")}catch(e){console.error("Erro ao definir persistência no main.js",e)}oe.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),Ys(),kd(),Io&&Io.addEventListener("click",e=>{e.stopPropagation(),xe.classList.remove("hidden"),xe.classList.add("absolute","inset-y-0","left-0","z-40","shadow-xl"),Me.classList.remove("hidden")}),Me&&Me.addEventListener("click",()=>{xe.classList.add("hidden"),xe.classList.remove("absolute","inset-y-0","left-0","z-40","shadow-xl"),Me.classList.add("hidden")}),So.addEventListener("click",e=>{e.stopPropagation(),yt.classList.toggle("hidden"),yt.classList.contains("hidden")||(st.forEach(t=>t.read=!0),va())}),na.addEventListener("click",e=>{e.stopPropagation(),te.classList.toggle("active"),te.classList.contains("active")?te.classList.remove("hidden"):setTimeout(()=>te.classList.add("hidden"),200)}),Eo&&Eo.addEventListener("click",e=>{e.preventDefault(),Z("my-profile-section"),te.classList.remove("active"),te.classList.add("hidden")}),document.addEventListener("click",e=>{!yt.contains(e.target)&&e.target!==So&&yt.classList.add("hidden"),!te.contains(e.target)&&e.target!==na&&te.classList.contains("active")&&(te.classList.remove("active"),setTimeout(()=>te.classList.add("hidden"),200))}),Lo(U,async e=>{if(e){if(console.log("Usuário detectado:",e.email),!oe.isNativePlatform()&&(console.log("Inicializando Web Push (PWA)..."),Ho(),"Notification"in window&&Notification.permission==="default")){const t=document.getElementById("toast-notification-request"),a=document.getElementById("btn-enable-toast"),s=document.getElementById("btn-deny-toast"),r=document.getElementById("btn-close-toast");setTimeout(()=>{t&&(t.style.display="block")},3500),a&&a.addEventListener("click",async()=>{await Oo()&&t&&(t.style.display="none")});const o=()=>{t&&(t.style.display="none")};s&&s.addEventListener("click",o),r&&r.addEventListener("click",o)}try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const s=await Se(a.establishmentId);m.enabledModules=s.modules,hd(s.themeColor||"indigo");let r=null,o=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const d=me(_,"users",e.uid),c=await To(d);if(c.exists()){const u=c.data();r=a.role==="employee"?u.permissions||{}:null,o=u.name||o,n=u.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}m.userProfessionalId=n,oe.isNativePlatform()&&wd(e.uid);const i=o||e.email;Hs(a.establishmentId,s.name,r),na.textContent=i.charAt(0).toUpperCase(),gd.textContent=i,bd.textContent=e.email;const l=()=>{ot&&ot(),ja(U).then(()=>window.location.href="/login.html")};fd.addEventListener("click",d=>{d.preventDefault(),l()}),Ks(Z,r,m.enabledModules),yd(r),xd(a.establishmentId),va(),xt.classList.add("fade-out"),oa.style.display="flex",setTimeout(()=>{xt.style.display="none"},500),console.log("Verificando Onboarding..."),setTimeout(()=>{pr()},1500),Z("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),xt.classList.add("fade-out"),setTimeout(()=>{xt.style.display="none"},500),oa.innerHTML=`
                ...
                `,oa.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{ja(U).then(()=>window.location.href="/login.html")})}}else window.location.href="/login.html"})}Sd();Lo(U,async e=>{if(e){await Ho();const a="Notification"in window&&Notification.permission==="default",s=window.Capacitor&&window.Capacitor.isNativePlatform();if(a&&!s){const r=document.getElementById("toast-notification-request"),o=document.getElementById("btn-enable-toast"),n=document.getElementById("btn-deny-toast"),i=document.getElementById("btn-close-toast");setTimeout(()=>{r&&(r.style.display="block")},3500),o&&o.addEventListener("click",async()=>{await Oo()&&r&&(r.style.display="none")});const l=()=>{r&&(r.style.display="none")};n&&n.addEventListener("click",l),i&&i.addEventListener("click",l)}}});export{Fo as W};
