const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-DowJ5ED4.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/firebase-config-C2tbVz-J.js","assets/styles-C2XZ_b0c.css"])))=>i.map(i=>d[i]);
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${a}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${a}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${io}. Verifique se o servidor backend está rodando.`):n}}const Fa=(e,t,o,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${o}`;return s&&(r+=`&professionalId=${s}`),E(r)},Ws=({establishmentId:e,professionalId:t,serviceIds:o,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${o.join(",")}&date=${s}`;return E(r)},Js=e=>E("/api/appointments",{method:"POST",body:JSON.stringify(e)}),Gs=(e,t)=>E(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),_o=e=>E(`/api/appointments/${e}`,{method:"DELETE"}),Ys=e=>E(`/api/appointments/${e}/reopen`,{method:"POST"}),Qs=(e,t)=>E(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let W;async function Xs(){if(!W)try{W=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function Zs(){if(!W){console.warn("AudioContext não inicializado. O som não será tocado.");return}W.state==="suspended"&&W.resume();const e=W.createOscillator(),t=W.createGain();e.connect(t),t.connect(W.destination),e.type="sine",e.frequency.setValueAtTime(800,W.currentTime),t.gain.setValueAtTime(0,W.currentTime),t.gain.linearRampToValueAtTime(.3,W.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,W.currentTime+.2),e.start(W.currentTime),e.stop(W.currentTime+.2)}function p(e,t,o="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&Zs();const a=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};a.className=`toast ${n[o]||n.info}`,a.innerHTML=`
        ...
        <div class="p-5 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="apptClientName" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="Nome Completo" value="${b(C.data.clientName)}">
                </div>
                <div>
                    <label for="apptClientPhone" class="block text-sm font-medium text-gray-700">Telemóvel</label>
                    <input type="tel" id="apptClientPhone" required class="mt-1 block w-full p-3 rounded-lg border-gray-300 shadow-sm" placeholder="(XX) XXXXX-XXXX" value="${b(C.data.clientPhone)}">
                </div>
            </div>
    ...
    `;return{title:o,content:s}}function Kr(){const e="Selecionar Serviço",o=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${ct.map(s=>{const r=C.data.selectedServiceIds.includes(s.id),a=s.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S",n=b(s.name);return`
    ...
    `;return{title:e,content:o}}function en(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Ut.map(o=>{const s=C.data.professionalId===o.id,r=o.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P",a=b(o.name);return`
    ...
    `;return{title:e,content:t}}function tn(e){const t=e?"Confirmar Edição":"Data e Horário",o=new Date,s=`${o.getFullYear()}-${String(o.getMonth()+1).padStart(2,"0")}-${String(o.getDate()).padStart(2,"0")}`,r=C.data.date||s,a=b(C.data.clientName),n=b(C.data.professionalName),i=`
        <div class="p-5 space-y-6">
    ...
    `;e.innerHTML=`
        ...
        ${r?"":h}
    `,!r&&(t.clientId||t.clientName)&&wn(t,e.querySelector("#loyalty-container"))}function yn(e,t){const s=Se(e).reduce((g,f)=>g+Number(f.price||0)*(f.quantity||1),0),r=x.checkoutState,a=r.discount||{type:"real",value:0};let n=0;a.type==="percent"?n=s*a.value/100:n=a.value,n>s&&(n=s);const i=s-n,l=r.payments.reduce((g,f)=>g+f.value,0),d=Math.max(0,i-l);(!r.amountReceived||d>0)&&(r.amountReceived=d.toFixed(2));const c=`
        <div class="mobile-only-header">
                ...
                ${["credito","crediario"].includes(r.selectedMethod)?`
                    <div class="mb-3">
                        ...
                        <select id="checkout-installments" class="w-full mt-1 p-2 border rounded-lg text-sm bg-gray-50">
                            ${Array.from({length:12},(g,f)=>`<option value="${f+1}" ${r.installments===f+1?"selected":""}>${f+1}x</option>`).join("")}
                        </select>
                    </div>
                `:""}
    ...
    `,{modalElement:t}=Y({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async o=>{o.preventDefault();const s=parseFloat(document.getElementById("initial-amount").value);if(isNaN(s)||s<0)return p("Valor Inválido","Insira um valor válido.","error");try{const r=await cn({establishmentId:m.establishmentId,initialAmount:parseFloat(s.toFixed(2))});x.isCashierOpen=!0,x.activeCashierSessionId=r.id,document.getElementById("genericModal").style.display="none",p("Sucesso!",`Caixa aberto (R$ ${s.toFixed(2)})`,"success"),_t(),await re()}catch(r){p("Erro",`Falha ao abrir caixa: ${r.message}`,"error")}})}async function Ln(){const e=x.activeCashierSessionId;if(e)try{const t=await pn(e),o=`
        ...
        <div class="flex flex-col items-center justify-center h-[calc(100vh-100px)]">
            ...
            </div>`}}function Qn(){const e=M.professionalsList.map(o=>`<option value="${o.id}">${b(o.name)}</option>`).join(""),t=M.costCentersList.map(o=>`<option value="${o.id}">${b(o.name)}</option>`).join("");po.innerHTML=`
    ...
    `,document.getElementById("toggle-filters-btn").onclick=ca,document.getElementById("btn-apply-filters").onclick=()=>{Xn(),ca()},document.querySelectorAll(".tab-btn").forEach(o=>{o.onclick=()=>{M.currentTab=o.dataset.tab,ua(),fs(),window.scrollTo({top:0,behavior:"smooth"})}}),ua()}function ca(){const e=document.getElementById("filters-container"),t=document.getElementById("toggle-filters-btn");M.isFilterOpen=!M.isFilterOpen,M.isFilterOpen?(e.classList.remove("hidden"),t.classList.add("bg-indigo-100","text-indigo-800")):(e.classList.add("hidden"),t.classList.remove("bg-indigo-100","text-indigo-800"))}function ua(){document.querySelectorAll(".tab-btn").forEach(e=>{const t=e.dataset.tab===M.currentTab;e.className=t?"tab-btn py-2 px-6 rounded-full text-xs md:text-sm font-bold bg-indigo-600 text-white shadow-md transform scale-105 transition-all whitespace-nowrap border-transparent":"tab-btn py-2 px-6 rounded-full text-xs md:text-sm font-medium text-gray-500 bg-white border-gray-200 hover:bg-gray-50 transition-all whitespace-nowrap"})}function Dt(e){if(!e)return"";const t=e.split("-");return`${t[2]}/${t[1]}`}async function Xn(){M.startDate=document.getElementById("report-start").value,M.endDate=document.getElementById("report-end").value,M.selectedProfessional=document.getElementById("report-prof").value,M.selectedCostCenter=document.getElementById("report-cost").value,document.getElementById("date-range-display").textContent=`${Dt(M.startDate)} até ${Dt(M.endDate)}`,await gs()}async function gs(){const e=document.getElementById("report-content");e.innerHTML='<div class="flex justify-center py-20"><div class="loader border-t-indigo-600"></div></div>';try{const t=M.selectedProfessional==="all"?null:M.selectedProfessional,o=Mr(M.startDate,M.endDate,M.selectedProfessional,M.selectedCostCenter),s=Jn(m.establishmentId,M.startDate,M.endDate).catch(()=>({labels:[],payables:[],receivables:[],expectedBalance:[]})),r=new Date(M.startDate+"T00:00:00").toISOString(),a=new Date(M.endDate+"T23:59:59").toISOString(),n=Fa(m.establishmentId,r,a,t).catch(()=>[]),[i,l,d]=await Promise.all([o,s,n]);M.data=i,M.cashFlowData=l,M.appointmentsData=Array.isArray(d)?d:[],fs()}catch(t){console.error(t),e.innerHTML=`
            ...
            </div>`}}function fs(){const e=document.getElementById("report-content");if(M.data)switch(M.currentTab){case"dashboards":Zn(e);break;case"appointments":Kn(e);break;case"dre":ei(e);break}}function Zn(e){const{dreSimple:t,charts:o}=M.data,s=t||{grossRevenue:0,netProfit:0,variableCosts:0};e.innerHTML=`
    ...
    `,M.cashFlowData&&ti("chart-cashflow-modern",M.cashFlowData),ma("chart-monthly","bar","Receita Mensal",o.salesMonthly.labels,o.salesMonthly.data,da[0]),ma("chart-profs","doughnut","Total Vendas",o.professionals.labels,o.professionals.data,da)}function Kn(e){const t=M.appointmentsData,o=t.length;let s=0,r=0,a=0;const n={},i={};let l=new Date(M.startDate);const d=new Date(M.endDate);for(;l<=d;)n[l.toISOString().split("T")[0]]=0,l.setDate(l.getDate()+1);t.forEach(f=>{const h=parseFloat(f.totalAmount||f.price||0),v=(f.status||"").toLowerCase();let y=f.startTime?(f.startTime.toDate?f.startTime.toDate():new Date(f.startTime)).toISOString().split("T")[0]:"";const w=f.professionalName||"Sem Profissional";i[w]||(i[w]={name:w,count:0,value:0}),["cancelled","cancelado","no-show"].includes(v)?r++:(["completed","finalized","paid"].includes(v)&&s++,a+=h,y&&n.hasOwnProperty(y)&&n[y]++,i[w].count++,i[w].value+=h)});const c=Object.keys(n).sort(),u=c.map(f=>n[f]),g=Object.values(i).sort((f,h)=>h.value-f.value);e.innerHTML=`
            ...
            <div class="overflow-x-auto no-scrollbar w-full">
                <table class="w-full text-xs md:text-sm min-w-full">
                    <tbody class="divide-y divide-gray-100">
                        ${g.map((f,h)=>{const v=g[0]?.value||1,y=f.value/v*100;return`
                            <tr class="group">
            ...
            <div class="mt-6 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-3">
                ...
                <div class="professional-commission-row flex items-center justify-between p-2 rounded-md ${L?"bg-blue-50":""}" data-prof-id="${I.id}">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <img src="${I.photo||`https://placehold.co/40x40/E2E8F0/4A5568?text=${b(I.name.charAt(0))}`}" class="w-8 h-8 rounded-full object-cover">
                        <span class="text-sm font-medium">${b(I.name)}</span>
                    </label>
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                ...
                <div class="flex flex-col-reverse sm:flex-row w-full sm:w-auto gap-3">
                    ...
                    <div class="p-2 hover:bg-indigo-50 cursor-pointer border-b last:border-0 text-sm flex justify-between items-center" data-add-supplier="${j.id}">
                        <span class="font-medium">${b(j.name)}</span>
                ...
                `).join("")):(D.classList.remove("hidden"),D.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else D.classList.add("hidden");a.size>0?(R.innerHTML="",a.forEach(z=>{const j=s.find(Q=>Q.id===z);j&&(R.innerHTML+=`
                        <div class="selected-supplier-item flex items-center justify-between bg-white border p-2 rounded shadow-sm" data-id="${j.id}">
                            <div class="text-sm">
                                <p class="font-bold text-gray-800">${b(j.name)}</p>
                                <p class="text-xs text-gray-500">${b(j.contactName||"")} - ${b(j.phone||"")}</p>
                            </div>
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(m.categories||[]).forEach(o=>t.innerHTML+=`<option value="${o.id}">${b(o.name)}</option>`)),Ro(),it()}function $i(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],o=new Date;o.setDate(o.getDate()-30);const s=o.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
    ...
    `;o.forEach(a=>{const n=JSON.stringify(a).replace(/"/g,"&quot;"),i=b(a.name),l=b(a.taxId||"Sem doc."),d=b(a.email||"-"),c=b(a.phone||"-"),u=b(a.category||"Geral");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=s+r}function Di(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",o=e.phone?`tel:${e.phone}`:"#",s=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),a=b(e.name),n=b(e.category||"Fornecedor"),i=b(e.contactName||""),l=b(e.phone||""),d=`
    ...
    `;Y({title:"",contentHTML:d,maxWidth:"max-w-md"})}async function et(){const e=document.getElementById("purchasesContainer");if(e)if(H.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,o]=await Promise.all([dt(m.establishmentId),qo(m.establishmentId)]);H.allSuppliers=o||[];const s=t.filter(d=>{const c=parseInt(d.currentStock||0),u=parseInt(d.minStock||0);return c<=u});if(H.productsToBuy=s,s.length===0){e.innerHTML=`
                ...
                `;return}let r='<div class="flex flex-col gap-3 md:hidden">',a="";s.forEach(d=>{const c=parseInt(d.minStock)||0,u=parseInt(d.currentStock)||0,g=Math.max(c-u,1),f=parseFloat(d.costPrice||0),h=b(d.name);let v='<option value="">Selecione...</option>';H.allSuppliers.length>0?H.allSuppliers.forEach(y=>{const $=d.supplierIds&&d.supplierIds.includes(y.id)?"selected":"";v+=`<option value="${y.id}" ${$}>${b(y.name)}</option>`}):v='<option value="">Sem fornecedores</option>',r+=`
                    <div class="product-row bg-white p-3 rounded-lg shadow-sm border border-gray-200" data-product-id="${d.id}" data-cost="${f}">
                        ...
                        <div class="mt-2 pt-2 border-t border-gray-100 flex justify-between items-center">
                            ...
                            <span class="row-subtotal font-bold text-indigo-600 text-sm">R$ ${(g*f).toFixed(2)}</span>
                        </div>
                    </div>
                `,a+=`
                    <tr class="hover:bg-gray-50 border-b border-gray-100 product-row" data-product-id="${d.id}" data-cost="${f}">
                        ...
                        <td class="p-3 text-right text-sm font-bold text-gray-800 row-subtotal">R$ ${(g*f).toFixed(2)}</td>
                        <td class="p-3 w-48">
        ...
        `}).join("");s+=h;const y=encodeURIComponent(JSON.stringify({supplierId:g,supplierName:f.info.name,totalAmount:h,items:f.items})),w=encodeURIComponent(JSON.stringify({name:f.info.name,phone:f.info.phone,email:f.info.email})),$=encodeURIComponent(JSON.stringify(f.items)),k=b(f.info.name),I=b(f.info.email||"");o+=`
    ...
    `;Y({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",Pi),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Ft("genericModal"))},50)}function qi(){be.innerHTML=`
    ...
    `,de&&(be.removeEventListener("click",de),be.removeEventListener("input",de),be.removeEventListener("change",de)),de=e=>{if(e.target.closest("#tab-btn-list")&&gt("list"),e.target.closest("#tab-btn-purchases")&&gt("purchases"),e.target.closest("#tab-btn-history")&&gt("history"),e.target.id==="toggle-quote-mode"&&(H.isQuoteMode=e.target.checked,et()),e.target.id==="supplierSearchInput"&&Gt(),e.target.closest("#btn-new-supplier")&&ba(),e.target.closest(".supplier-item-mobile")){const o=e.target.closest(".supplier-item-mobile"),s=JSON.parse(o.dataset.supplier);Di(s)}const t=e.target.closest("button[data-action]");if(t){const o=t.dataset.action;o==="delete"&&Ti(t.dataset.id),o==="edit"&&ba(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&go(),e.target.id==="check-all-rows"){const o=e.target.checked;document.querySelectorAll(".row-select").forEach(s=>s.checked=o),go()}if(e.target.closest("#btn-go-to-orders")){const o=document.querySelectorAll(".product-row"),s={};let r=!1;if(o.forEach(a=>{if(a.offsetParent===null||!a.querySelector(".row-select").checked)return;r=!0;let i="Produto";const l=a.querySelector("td:nth-child(2)"),d=a.querySelector(".font-bold");l?i=l.innerText:d&&(i=d.innerText);const c=parseInt(a.querySelector(".qty-input").value),u=parseFloat(a.dataset.cost),f=a.querySelector(".supplier-select").value;if(f){if(!s[f]){const h=H.allSuppliers.find(v=>v.id===f);s[f]={info:h,items:[]}}s[f].items.push({name:i,qty:c,cost:u})}}),!r){p("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}H.finalOrders=s,H.step=2,et()}if(e.target.closest("#btn-back-step1")&&(H.step=1,et()),e.target.closest(".btn-send-order")){const o=e.target.closest(".btn-send-order"),s=JSON.parse(decodeURIComponent(o.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(o.dataset.orderItems)),a=parseFloat(o.dataset.total),n=H.isQuoteMode;if(s.phone){const i=s.phone.replace(/\D/g,"");let l="";n?(l=`Olá *${s.name}*, tudo bem?
...
Aguardo retorno.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else p("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const o=e.target.closest(".btn-register-order");if(o.disabled)return;const s=JSON.parse(decodeURIComponent(o.dataset.order));s.establishmentId=m.establishmentId,o.disabled=!0,o.textContent="A processar...",$e(m.establishmentId).then(r=>{const a=r.purchaseConfig||null;return fi(s,a)}).then(()=>{p("Sucesso","Compra registrada e integrada ao financeiro!","success"),o.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',o.classList.replace("bg-blue-600","bg-green-600"),o.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{o.disabled=!1,o.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Salvar',p("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-delete-purchase")){const s=e.target.closest(".btn-delete-purchase").dataset.id;O("Excluir Compra","Isto apagará o registo histórico E o lançamento financeiro associado. Deseja continuar?").then(async r=>{if(r)try{await bi(s,m.establishmentId),p("Sucesso","Compra e financeiro excluídos.","success"),ws()}catch(a){p("Erro","Erro ao excluir: "+a.message,"error")}})}if(e.target.closest(".btn-print-order")){const s=e.target.closest(".supplier-order-card").dataset.supplierId,r=H.finalOrders[s];r?Ai(r,H.isQuoteMode):p("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const o=e.target.closest(".btn-view-purchase"),s=JSON.parse(o.dataset.purchase);Mi(s)}},be.addEventListener("click",de),be.addEventListener("input",de),be.addEventListener("change",de),gt("list")}function gt(e){Et=e,["list","purchases","history"].forEach(o=>{const s=document.getElementById(`tab-btn-${o}`),r=document.getElementById(`tab-content-${o}`);o===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),Ci()}const Zt=document.getElementById("content"),va={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let ne=new Set,ft=null,De=null;function Ni(e=8){let t="";for(let o=0;o<e;o++)t+=`
        ...
        </div>`;t.innerHTML=n,t.style.display="flex",Fi(o,r),Hi(o),Oi(o,a),Vi(o)}function Fi(e,t){const o=document.getElementById("professionalForm"),s=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(y,w)=>{const $=w+1,k=$==s[1]?"selected":"",I=new Date(0,w).toLocaleString("pt-BR",{month:"long"});return`<option value="${$}" ${k}>${I.charAt(0).toUpperCase()+I.slice(1)}</option>`}).join(""),a=e.status||"active",n=b(e.name||""),i=b(e.specialty||""),l=b(e.phone||""),d=b(e.notes||"");o.innerHTML=`
        ...
        </div>`;const s=document.getElementById("batchBlockageForm");s&&s.addEventListener("submit",async a=>{a.preventDefault();const n=Array.from(a.target.querySelectorAll('input[name="batch-professionals"]:checked')).map(f=>f.value);if(n.length===0)return p("Atenção","Selecione pelo menos um profissional.","error");const i=a.target.batchBlockageStartDate.value,l=a.target.batchBlockageEndDate.value||i,d=a.target.batchBlockageStartTime.value,c=a.target.batchBlockageEndTime.value,u=a.target.batchBlockageReason.value;if(!i||!d||!c)return p("Atenção","Preencha Data de Início, Início e Fim.","error");const g=n.map(f=>{const h={professionalId:f,establishmentId:m.establishmentId,startTime:new Date(`${i}T${d}`).toISOString(),endTime:new Date(`${l}T${c}`).toISOString(),reason:u};return zt(h)});try{await Promise.all(g),p("Sucesso!",`${n.length} bloqueios foram criados.`);const f=document.getElementById("prof-blockages-filter").value;tt(e.id,f)}catch(f){p("Erro",f.message,"error")}}),document.getElementById("prof-blockages-filter").addEventListener("change",a=>tt(e.id,a.target.value)),await tt(e.id,"future")}function zi(e,t){e.innerHTML=Object.keys(va).map(o=>{const s=t[o]||{},r=s.active!==!1;return`
        ...
        </section>`,ft&&Zt.removeEventListener("click",ft),ft=t=>{const o=t.target.closest('[data-action="open-professional-modal"]'),s=t.target.closest('[data-action="batch-delete"]');if(o){t.preventDefault();let a={};if(o.dataset.professional)try{a=JSON.parse(o.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}ji(a);return}if(s){Ui();return}const r=t.target.closest(".professional-checkbox");if(r){const a=r.dataset.id;r.checked?ne.add(a):ne.delete(a),Je(),fo();return}},Zt.addEventListener("click",ft),document.getElementById("profSearchInput").addEventListener("input",Je),document.getElementById("showInactiveProfToggle").addEventListener("change",Je);const e=document.getElementById("professionalsList");m.professionals=null,m.services=null,Je();try{const[t,o]=await Promise.all([K(m.establishmentId),ge(m.establishmentId)]);m.professionals=t,m.services=o,Je(),fo()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}let S={clients:[],selectedClient:null,activeTab:"profile",filters:{search:"",inactiveDays:"",birthMonth:"",hasLoyalty:!1,hasDebt:!1},showFilters:!1,loading:!1,historyLimit:20,historySearchTerm:"",historyLoading:!1,historyData:{appointments:[],sales:[],loyaltyLog:[]},modalOpen:!1},ks=null;const _i=e=>e?String(e).replace(/\D/g,""):"",Wi=e=>{if(!e)return"Nunca";let t;if(typeof e=="object"&&(e.seconds||e._seconds)){const o=e.seconds||e._seconds;t=new Date(o*1e3)}else t=new Date(e);return isNaN(t.getTime())?"Data Inválida":t.toLocaleDateString("pt-BR")};function jo(){ks.innerHTML=`
    ...
    `;const e=document.getElementById("btn-new-client");e&&(e.onclick=tl)}function lt(){if(S.modalOpen)return;jo();const e=document.getElementById("clients-content-area"),t=S.filters.inactiveDays||S.filters.birthMonth||S.filters.hasLoyalty||S.filters.hasDebt,o=`
    ...
    `,s=S.clients.length>0?`
        <div class="px-3 sm:px-4 pb-20 pt-2 w-full">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ${S.clients.map(c=>{const u=c.totalDebt&&parseFloat(c.totalDebt)>0,g=Wi(c.lastVisit);return`
    ...
    `;e.innerHTML=o+s;const r=document.getElementById("client-search"),a=document.getElementById("btn-toggle-filters"),n=document.getElementById("btn-apply-filters"),i=document.getElementById("btn-clear-search");a&&(a.onclick=()=>{S.showFilters=!S.showFilters,lt()});const l=document.getElementById("filter-birth-month");l&&(l.value=S.filters.birthMonth);const d=()=>{const c=document.getElementById("filter-inactive"),u=document.getElementById("filter-loyalty"),g=document.getElementById("filter-debt"),f=document.getElementById("filter-birth-month");S.filters={search:r.value,inactiveDays:c?c.value:"",birthMonth:f?f.value:"",hasLoyalty:u?u.checked:!1,hasDebt:g?g.checked:!1},bo()};n&&(n.onclick=d),i&&(i.onclick=()=>{S.filters={search:"",inactiveDays:"",birthMonth:"",hasLoyalty:!1,hasDebt:!1},S.showFilters=!1,bo()}),r.addEventListener("keypress",c=>{c.key==="Enter"&&d()}),e.querySelectorAll(".client-card").forEach(c=>{c.onclick=()=>Ss(c.dataset.id)})}function Ji(e){const t=`
    ...
    `;let o="";return S.activeTab==="profile"?o=Qi(e):S.activeTab==="appointments"?o=Xi():S.activeTab==="history"?o=Zi():S.activeTab==="loyalty"&&(o=Ki(e)),`
    ...
    `}function Gi(e,t){if(!document.getElementById("tabs-styles")){const a=document.createElement("style");a.id="tabs-styles",a.textContent=`
        ...
        `,t.onclick=s=>{s.target===t&&Ne()},document.body.appendChild(t),document.body.classList.add("overflow-hidden"),S.modalOpen=!0);const o=t.querySelector("#client-modal-content");o.innerHTML=Ji(e),Gi(o,e)}function Ne(){const e=document.getElementById("client-details-modal-overlay");e&&e.remove(),document.body.classList.remove("overflow-hidden"),S.modalOpen=!1,S.selectedClient=null,lt()}function Qi(e){return`
    ...
    `}function Xi(e){let t=S.historyData.appointments||[];if(S.historySearchTerm){const s=S.historySearchTerm.toLowerCase();t=t.filter(r=>r.serviceName&&r.serviceName.toLowerCase().includes(s)||r.professionalName&&r.professionalName.toLowerCase().includes(s))}t.sort((s,r)=>new Date(r.startTime)-new Date(s.startTime));const o=s=>{const r=new Date(s.startTime),a=r.toLocaleDateString("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).replace(".",""),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=r<new Date;let l=i?"Concluído":"Agendado",d=i?"bg-gray-200 text-gray-600":"bg-green-100 text-green-700";return s.status==="cancelled"&&(l="Cancelado",d="bg-red-100 text-red-600"),`
            <div class="relative bg-white border rounded-xl p-3 shadow-sm mb-3 flex gap-3 cursor-pointer active:scale-[0.99] transition w-full overflow-hidden"
                ...
                <div class="flex-grow min-w-0 flex flex-col justify-center">
                    <h4 class="font-bold text-gray-800 text-sm truncate">${b(s.serviceName||"Serviço")}</h4>
                    <p class="text-xs text-gray-500 truncate">Prof: ${b(s.professionalName||"N/A")}</p>
                    <div class="mt-1">
    ...
    `}function Zi(e){let t=S.historyData.sales||[];if(S.historySearchTerm){const o=S.historySearchTerm.toLowerCase();t=t.filter(s=>s.id.includes(o))}return t.sort((o,s)=>new Date(s.date)-new Date(o.date)),t.length===0&&!S.historySearchTerm?'<div class="text-center py-12 text-gray-400">Nenhum registro financeiro.</div>':`
        <div class="space-y-4 w-full">
            <div class="sticky top-0 bg-gray-50 pt-2 pb-2 z-10 w-full">
                <div class="relative w-full">
                    <input type="text" id="history-search-input" 
    ...
    `}function Ki(e){const t=S.historyData.loyaltyLog||[];t.sort((s,r)=>new Date(r.date)-new Date(s.date));const o=t.length>0?t.map(s=>{const r=s.type==="redemption";return`
            <div class="flex justify-between items-center py-3 border-b border-gray-100 last:border-0 w-full">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-700">${b(s.description||(r?"Resgate":"Acúmulo"))}</p>
    ...
    `,t.querySelector("#loyaltyEnabled").checked=o.enabled||!1;const i=t.querySelector("#loyaltyTiersContainer"),l=(d={})=>{const c=document.createElement("div");c.className="loyalty-tier-row group bg-white md:bg-transparent p-3 md:p-0 border md:border-0 rounded-lg shadow-sm md:shadow-none relative md:grid md:grid-cols-[0.8fr_1fr_1.5fr_1fr_auto] md:gap-2 md:items-start";const u=d.type||"money",g=d.itemId||"",f=d.reward||"",h=d.discount||"",v=d.points||d.costPoints||"";c.innerHTML=`
            ...
            <div class="mb-2 md:mb-0 relative">
                ...
                <input type="text" placeholder="Ex: Vale Compras R$ 20" data-field="rewardName" value="${b(f)}" class="desc-input w-full p-2 border rounded-md ${u!=="money"?"hidden":""}">
    ...
    `;try{const[o,s]=await Promise.all([Po(m.establishmentId),Do(m.establishmentId)]),r=e.financialIntegration||{},a=e.commissionConfig||{},n=e.purchaseConfig||{};t.querySelector("#financialNatureId").innerHTML=Ce(o,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=Ce(s,r.defaultCentroDeCustoId),t.querySelector("#purchaseNatureId").innerHTML=Ce(o,n.defaultNatureId),t.querySelector("#purchaseCostCenterId").innerHTML=Ce(s,n.defaultCostCenterId),t.querySelector("#commissionNatureId").innerHTML=Ce(o,a.defaultNatureId),t.querySelector("#commissionCostCenterId").innerHTML=Ce(s,a.defaultCostCenterId)}catch{p("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",o=>{o.preventDefault();const s={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null},purchaseConfig:{defaultNatureId:t.querySelector("#purchaseNatureId").value||null,defaultCostCenterId:t.querySelector("#purchaseCostCenterId").value||null},commissionConfig:{defaultNatureId:t.querySelector("#commissionNatureId").value||null,defaultCostCenterId:t.querySelector("#commissionCostCenterId").value||null}};Ve(s,o)})}function gl(e,t){const o="5516997859430",s=encodeURIComponent("Olá, preciso de ajuda com o sistema Kairos."),r=`https://wa.me/${o}?text=${s}`;t.innerHTML=`
    ...
    `}function fl(e,t){const o="5516997859430",s=encodeURIComponent("Olá, gostaria de solicitar o cancelamento da minha assinatura."),r=`https://wa.me/${o}?text=${s}`,a="sistemakairosagenda@gmail.com";t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex flex-col md:flex-row items-center justify-between mb-6 border-b pb-4">
                <div>
    ...
    `}function Es(e="indigo",t){const o=t.querySelector("#color-palette-container"),s=t.querySelector("#establishmentThemeColor");!o||!s||(o.innerHTML="",Object.entries(rl).forEach(([r,a])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{s.value=r,Es(r,t)}),o.appendChild(i)}),s.value=e)}function vl(e,t){const o=t.querySelector("#slotIntervalContainer"),s=t.querySelector("#establishmentSlotInterval");if(!o||!s)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];o.innerHTML=r.map(a=>{const n=a.value===e;return`<button type="button" data-value="${a.value}" 
    ...
    `,xe.querySelector('button[data-action="back-to-list"]').addEventListener("click",s=>{s.preventDefault(),Is()});const o=document.getElementById("settings-content-detail");switch(e){case"personal-data":nl(J,o);break;case"change-password":il(J,o);break;case"change-email":ll(J,o);break;case"branding":dl(J,o);break;case"booking":cl(J,o);break;case"working-hours":ul(J,o);break;case"loyalty":await ml(J,o);break;case"financial":await pl(J,o);break;case"support":gl(J,o);break;case"cancellation":fl(J,o);break;default:bl(t?t.label:"Definições",o)}}async function Is(){if(xe.innerHTML=`
        ...
        <div data-action="go-to-my-profile" class="bg-white p-4 rounded-lg shadow-md mb-6 cursor-pointer hover:bg-gray-50 transition-all">
            <div class="text-center relative">
                ...
                <div class="absolute top-0 right-0 p-2 text-gray-400 hover:text-indigo-600" title="Ver Meu Perfil">
                 ...
                 ${m.userName&&m.userName!==U.currentUser.email?`<p class="text-sm text-gray-500">${b(U.currentUser.email)}</p>`:""}
    ...
    `,xe.querySelector("#settings-menu-list").addEventListener("click",a=>{const n=a.target.closest("button[data-section]");n&&(a.preventDefault(),hl(n.dataset.section))});const r=xe.querySelector('[data-action="go-to-my-profile"]');r&&r.addEventListener("click",a=>{a.preventDefault(),Z("my-profile-section")})}const ot=document.getElementById("content");async function Re(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const o=document.getElementById("filterStartDate")?.value,s=document.getElementById("filterEndDate")?.value,r=await Ot(m.establishmentId,o||new Date().toISOString().split("T")[0],s||new Date().toISOString().split("T")[0],e),a=document.getElementById("filterReason")?.value.toLowerCase(),n=a?r.filter(l=>l.reason&&l.reason.toLowerCase().includes(a)):r,i=n.reduce((l,d)=>{const c=d.reason||"Sem motivo";return l[c]||(l[c]=[]),l[c].push(d),l},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([l,d])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let g=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
        ...
        <section>
        ...
        </section>`,wl(t),await Re(t);const r=document.getElementById("batchProfSelectionContainer");try{const a=await K(m.establishmentId);r.innerHTML=a.map(n=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{r.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const Sl=e=>E(`/api/users/${e}`),$l=e=>E("/api/users",{method:"POST",body:JSON.stringify(e)}),El=(e,t)=>E(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),Il=e=>E(`/api/users/${e}`,{method:"DELETE"}),Cl=(e,t)=>E(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),Ll=(e,t)=>E(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),ve=document.getElementById("content"),Tl={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},Pl={view:"Visualizar",create:"Criar",edit:"Editar"};let Ge=null,Ye=null;function Dl(e){const t=document.getElementById("usersListContainer");if(!t)return;const o=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const s=o?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${s}</p>`;return}e.sort((s,r)=>(s.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(s=>{const r=JSON.stringify(s).replace(/'/g,"&apos;"),a=s.status==="active",n=m.professionals.find(c=>c.id===s.professionalId),i=n?n.name:"N/A",l=n?n.name.charAt(0):s.name.charAt(0),d=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(l)}`;return`
    ...
    `}).join("")}function It(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=m.users:t=m.users.filter(o=>o.status==="active"),Dl(t)}function Bl(e={}){return Object.entries(Tl).map(([t,o])=>{const s=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,a=Object.entries(Pl).map(([i,l])=>`
    ...
    `;const g=window.innerWidth<768,f=u.querySelector(".bg-white");if(g&&f){f.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const w=u.closest("section");w&&(w.style.padding="0",w.style.margin="0"),f.classList.add("p-4")}const h=u.querySelector("#userProfessionalId"),v=u.querySelector("#userPhotoPreview"),y=u.querySelector("#profPhotoName");if(h.addEventListener("change",w=>{const $=w.target.value,k=s($),I=k?k.name:"Selecione um profissional",L=r($,n);v.src=L.photoSrc,y.textContent=I,u.querySelector("#professionalPhotoUrl").value=L.photoUrl}),u.addEventListener("submit",async w=>{w.preventDefault();const $=e?.email,k=u.querySelector("#userEmail").value,I={};u.querySelectorAll('input[type="checkbox"]').forEach(N=>{const P=N.dataset.module,D=N.dataset.permission;I[P]||(I[P]={}),I[P][D]=N.checked});const L=u.querySelector("#userProfessionalId").value||null,B={name:u.querySelector("#userName").value,permissions:I,professionalId:L,establishmentId:m.establishmentId};try{c?($!==k&&(B.email=k),await El(e.id,B),p("Usuário atualizado com sucesso!","success")):(B.email=u.querySelector("#userEmail").value,B.password=u.querySelector("#userPassword").value,await $l(B),p("Usuário criado com sucesso!","success")),Mt()}catch(N){p(`Erro: ${N.message}`,"error")}}),c){const w=u.querySelector("#password-change-container"),$=w.querySelector('[data-action="show-password-form"]'),k=w.querySelector("#password-form"),I=k.querySelector('[data-action="save-password"]'),L=k.querySelector('[data-action="cancel-password-change"]');$.addEventListener("click",()=>{$.classList.add("hidden"),k.classList.remove("hidden")}),L.addEventListener("click",()=>{$.classList.remove("hidden"),k.classList.add("hidden"),k.querySelector("#userNewPassword").value=""}),I.addEventListener("click",async()=>{const B=k.querySelector("#userNewPassword").value;if(!B||B.length<6){p("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await O("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{I.disabled=!0,I.textContent="Aguarde...",await Cl(e.id,B),p("Sucesso!","A senha do usuário foi alterada.","success"),$.classList.remove("hidden"),k.classList.add("hidden"),k.querySelector("#userNewPassword").value=""}catch(P){p("Erro",`Não foi possível alterar a senha: ${P.message}`,"error")}finally{I.disabled=!1,I.textContent="Salvar Nova Senha"}})}}async function Ml(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,o]=await Promise.all([Sl(m.establishmentId),K(m.establishmentId)]);m.users=t,m.professionals=o,It()}catch{p("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function Mt(){ve.innerHTML=`
    ...
    `,Ge&&ve.removeEventListener("click",Ge),Ye&&ve.removeEventListener("change",Ye),Ge=async e=>{if(!document.getElementById("user-list-view")){ve.removeEventListener("click",Ge);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":ya();break;case"edit-user":const s=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));ya(s);break;case"back-to-list":Mt();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await O("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await Il(r),p("Usuário excluído com sucesso!","success"),Mt()}catch(n){p(`Erro ao excluir: ${n.message}`,"error")}break}}},Ye=async e=>{if(!document.getElementById("user-list-view")){ve.removeEventListener("change",Ye);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")It();else if(t){e.stopPropagation();const o=t.dataset.userId,s=t.checked?"active":"inactive";try{await Ll(o,s),p(`Usuário ${s==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=m.users.findIndex(a=>a.id===o);r>-1&&(m.users[r].status=s,It())}catch(r){p(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,It()}}},ve.addEventListener("click",Ge),ve.addEventListener("change",Ye),await Ml()}const Al=document.getElementById("content");let wa={},vo=null;function ql(){Object.values(wa).forEach(e=>e?.destroy()),wa={}}function Nl(e,t){if(!window.jspdf){p("Erro","Biblioteca PDF não carregada.","error");return}const{jsPDF:o}=window.jspdf,s=new o({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(s.setFontSize(18),s.text(e,s.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];s.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const a=s.lastAutoTable?s.lastAutoTable.finalY+20:60;s.text("Detalhes das Vendas",20,a),s.autoTable({html:`#${t}`,startY:a+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),s.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function ka(e){const t=document.getElementById("genericModal"),o=b(e.client),s=b(e.items),r=b(e.responsavelCaixa||"N/A"),a=(e.payments||[]).map(n=>`
    ...
    `,t.style.display="flex"}function Rl(e){const{summary:t,transactions:o}=e;document.getElementById("summary-revenue").textContent=`R$ ${t.totalRevenue.toFixed(2)}`,document.getElementById("summary-transactions").textContent=t.totalTransactions,document.getElementById("summary-avg-ticket").textContent=`R$ ${t.averageTicket.toFixed(2)}`;const s=document.getElementById("paymentSummaryTableBody"),r=Object.entries(t.paymentMethodTotals).sort(([,i],[,l])=>l-i);s.innerHTML=r.map(([i,l])=>`
        <tr class="border-b">
    ...
    `}).join(""),a.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const l=i.dataset.transactionIndex,d=vo.transactions[l];d&&ka(d)})}),n.innerHTML=o.map((i,l)=>{const d=b(i.client),c=b(i.items),u=b(i.type);return`
    ...
    `}).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const l=i.dataset.transactionIndex,d=vo.transactions[l];d&&ka(d)})})}async function Sa(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),o=document.getElementById("reportEndDate");if(!e||!t||!o)return;const s=t.value,r=o.value;if(!s||!r)return p("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const a=document.getElementById("cashierSessionFilter").value,n=await qr({establishmentId:m.establishmentId,startDate:s,endDate:r,cashierSessionId:a});vo=n,e.innerHTML=`
        ...
        `,Rl(n)}catch(a){p("Erro",`Não foi possível carregar o relatório: ${a.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${b(a.message)}</p>`}}async function jl(){ql();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const o=t.toISOString().split("T")[0];Al.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Sa),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const s=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,a=`Relatorio_Vendas_${s}_a_${r}`;Nl(a,"transactionsTable")});try{const s=await mn(m.establishmentId),r=document.getElementById("cashierSessionFilter");s&&s.length>0&&s.forEach(a=>{const n=new Date(a.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=b(a.closedByName||"N/A");r.innerHTML+=`<option value="${a.id}">${i} - ${n}</option>`})}catch{p("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await Sa()}const Fl=document.getElementById("content");let A={payables:[],receivables:[],natures:[],costCenters:[],currentTab:"payables",statusFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],filterNaturezaId:"all",filterCostCenterId:"all",isFilterOpen:!1},bt=null,vt=null;function Fo(e){const t=new Map,o=[];return e&&(e.forEach(s=>t.set(s.id,{...s,children:[]})),t.forEach(s=>{s.parentId&&t.has(s.parentId)?t.get(s.parentId).children.push(s):o.push(s)})),o}function Hl(e){if(!e)return{day:"--",month:"---"};const t=new Date(e+"T00:00:00"),o=t.getDate().toString().padStart(2,"0"),s=t.toLocaleString("pt-BR",{month:"short"}).toUpperCase().replace(".","");return{day:o,month:s,full:t.toLocaleDateString("pt-BR")}}function Ct(e){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(e)}function Ol(e,t,o){if(!e)return;if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-400 text-sm py-4">Nenhum item criado.</p>';return}const s=(r,a=0)=>`
        ...
        </div>`,t.style.display="flex";const i=t.querySelector("#hierarchyList"),l=t.querySelector("#itemParent"),d=u=>{const g=Fo(u);Ol(i,g,e);const f=l.value;l.innerHTML='<option value="">-- Categoria Principal --</option>';const h=(v,y=0)=>{const w="  ".repeat(y)+(y>0?"↳ ":"");l.innerHTML+=`<option value="${v.id}">${w}${v.name}</option>`,v.children.forEach($=>h($,y+1))};g.forEach(v=>h(v)),l.value=f};try{const u=await r(m.establishmentId);A[n]=u,d(u)}catch(u){console.error(u)}const c=t.querySelector("#hierarchyForm");c.addEventListener("submit",async u=>{u.preventDefault();const g=t.querySelector("#itemName").value,f=l.value;try{await a({name:g,parentId:f||null,establishmentId:m.establishmentId});const h=await r(m.establishmentId);A[n]=h,d(h),c.reset(),await Ue(),p("Sucesso","Item adicionado.","success")}catch(h){p("Erro",h.message,"error")}})}async function zl(){Vl(),Ul(),await Ue()}function Vl(){Fl.innerHTML=`
    ...
    `}function Ul(){document.getElementById("toggle-filter-btn").addEventListener("click",()=>{const o=document.getElementById("filter-panel"),s=document.getElementById("toggle-filter-btn");A.isFilterOpen=!A.isFilterOpen,A.isFilterOpen?(o.classList.remove("hidden"),s.classList.add("bg-indigo-100","text-indigo-600")):(o.classList.add("hidden"),s.classList.remove("bg-indigo-100","text-indigo-600"))}),document.getElementById("settings-btn").addEventListener("click",Ql),document.getElementById("fab-add").addEventListener("click",()=>{const o=A.currentTab==="payables"?"payable":"receivable";to(o)});const e=document.getElementById("tab-receivables"),t=document.getElementById("tab-payables");e.addEventListener("click",()=>$a("receivables")),t.addEventListener("click",()=>$a("payables")),document.querySelectorAll(".status-filter-btn").forEach(o=>{o.addEventListener("click",s=>{document.querySelectorAll(".status-filter-btn").forEach(r=>{r.classList.remove("bg-indigo-100","text-indigo-700"),r.classList.add("text-gray-500")}),s.target.classList.add("bg-indigo-100","text-indigo-700"),s.target.classList.remove("text-gray-500"),A.statusFilter=s.target.dataset.status})}),document.getElementById("apply-filter-btn").addEventListener("click",()=>{A.startDate=document.getElementById("filterStartDate").value,A.endDate=document.getElementById("filterEndDate").value,A.filterNaturezaId=document.getElementById("filterNaturezaId").value,A.filterCostCenterId=document.getElementById("filterCostCenterId").value,document.getElementById("toggle-filter-btn").click(),Ue()}),bt&&document.body.removeEventListener("click",bt),bt=o=>{const s=o.target,r=s.closest("button[data-action]");if(r){const{action:n,type:i,id:l}=r.dataset;if(o.stopPropagation(),n==="delete"){Gl(i,l);return}if(n==="mark-as-paid"){Jl(i,l);return}if(n==="manage-natures"){ho("nature");return}if(n==="manage-cost-centers"){ho("cost-center");return}if(n==="edit"){const d=JSON.parse(r.dataset.item.replace(/&apos;/g,"'"));to(i,d);return}}const a=s.closest(".financial-card-item");if(a&&document.getElementById("list-container").contains(a)){const{type:n}=a.dataset,i=JSON.parse(a.dataset.item.replace(/&apos;/g,"'"));to(n,i)}},document.body.addEventListener("click",bt),vt&&document.getElementById("genericModal").removeEventListener("click",vt),vt=o=>{if(o.target.closest('[data-action="close-modal"]')){document.getElementById("genericModal").style.display="none";return}const r=o.target.closest('button[data-action^="delete-"]');if(r){const a=r.dataset.action.split("-")[1];Yl(a,r.dataset.id)}},document.getElementById("genericModal").addEventListener("click",vt)}function $a(e){A.currentTab=e;const t=document.getElementById("tab-receivables"),o=document.getElementById("tab-payables"),s=document.getElementById("fab-add");e==="receivables"?(t.classList.add("bg-white","text-green-700","shadow-sm"),t.classList.remove("text-gray-500"),o.classList.remove("bg-white","text-red-700","shadow-sm"),o.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-green-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-green-700 hover:scale-105 transition-all z-40"):(o.classList.add("bg-white","text-red-700","shadow-sm"),o.classList.remove("text-gray-500"),t.classList.remove("bg-white","text-green-700","shadow-sm"),t.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-red-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-red-700 hover:scale-105 transition-all z-40"),Cs()}async function Ue(){const e=document.getElementById("list-container");e.innerHTML='<div class="loader mx-auto my-10"></div>';try{if(A.natures.length===0){const[r,a]=await Promise.all([Po(m.establishmentId),Do(m.establishmentId)]);A.natures=r,A.costCenters=a,_l()}const t={startDate:A.startDate,endDate:A.endDate,establishmentId:m.establishmentId};A.filterNaturezaId!=="all"&&(t.natureId=A.filterNaturezaId),A.filterCostCenterId!=="all"&&(t.costCenterId=A.filterCostCenterId);const[o,s]=await Promise.all([jn(t),Vn(t)]);A.payables=o.entries||[],A.receivables=s.entries||[],Wl(),Cs()}catch(t){e.innerHTML=`<p class="text-center text-red-500 mt-10">Erro ao carregar: ${t.message}</p>`}}function _l(){const e=s=>{let r='<option value="all">Todas</option>';const a=Fo(s),n=(i,l=0)=>{const d="  ".repeat(l)+(l>0?"↳ ":"");r+=`<option value="${i.id}">${d}${i.name}</option>`,i.children.forEach(c=>n(c,l+1))};return a.forEach(i=>n(i)),r},t=document.getElementById("filterNaturezaId"),o=document.getElementById("filterCostCenterId");t&&(t.innerHTML=e(A.natures)),o&&(o.innerHTML=e(A.costCenters)),t&&(t.value=A.filterNaturezaId),o&&(o.value=A.filterCostCenterId)}function Wl(){const e=document.getElementById("summary-section");if(!e)return;const t=A.receivables.reduce((l,d)=>l+d.amount,0),o=A.receivables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),s=t-o,r=A.payables.reduce((l,d)=>l+d.amount,0),a=A.payables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),n=r-a,i=o-a;e.innerHTML=`
    ...
    `}function Cs(){const e=document.getElementById("list-container");if(!e)return;const t=A.currentTab==="receivables",o=t?A.receivables:A.payables;let s=o;if(A.statusFilter!=="all"&&(s=o.filter(n=>n.status===A.statusFilter)),s.sort((n,i)=>new Date(n.dueDate)-new Date(i.dueDate)),s.length===0){e.innerHTML=`
        ...
        `;return}const r=new Map(A.natures.map(n=>[n.id,n.name])),a=t?"receivable":"payable";e.innerHTML=s.map(n=>{const i=Hl(n.dueDate),l=n.status==="paid",d=l?"text-gray-400":t?"text-green-600":"text-red-500",c=n.naturezaId&&r.get(n.naturezaId)||"Geral",u=JSON.stringify(n).replace(/'/g,"&apos;");return`
        ...
        `}).join("")}async function Jl(e,t){const o=new Date().toISOString().split("T")[0];try{await(e==="payable"?On(t,o):Wn(t,o)),p("Sucesso","Lançamento baixado!","success"),await Ue()}catch(s){p("Erro",s.message,"error")}}async function Gl(e,t){if(await O("Excluir Lançamento","Tem certeza? Essa ação não pode ser desfeita."))try{await(e==="payable"?Hn(t):_n(t)),p("Sucesso","Lançamento excluído.","success"),await Ue()}catch(s){p("Erro",s.message,"error")}}async function Yl(e,t){const s=e==="nature"?An:Nn;if(await O("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await s(t),ho(e==="nature"?"nature":"cost-center")}catch(a){p("Erro",a.message,"error")}}function Ql(){const e=document.getElementById("genericModal");e.innerHTML=`
    ...
    `,e.style.display="flex"}function to(e,t=null){const o=document.getElementById("genericModal"),s=e==="payable",r=`${t?"Editar":"Nova"} ${s?"Despesa":"Receita"}`,a=s?"red":"green",n=(u,g)=>{let f='<option value="">-- Selecione --</option>';const h=Fo(u),v=(y,w=0)=>{const $="  ".repeat(w)+(w>0?"↳ ":""),k=y.id===g?"selected":"";f+=`<option value="${y.id}" ${k}>${$}${y.name}</option>`,y.children.forEach(I=>v(I,w+1))};return h.forEach(y=>v(y)),f};o.innerHTML=`
        ...
        </div>`,o.style.display="flex";const i=o.querySelector("#financial-form"),l=i.querySelector("#status-toggle"),d=i.querySelector("#payment-date-wrapper"),c=i.querySelector('[name="paymentDate"]');if(!t){const u=i.querySelector("#toggle-installments"),g=i.querySelector("#installments-container");u.addEventListener("change",()=>{g.classList.toggle("hidden",!u.checked)})}l.addEventListener("change",()=>{l.checked?(d.classList.remove("hidden"),c.required=!0):(d.classList.add("hidden"),c.required=!1)}),i.addEventListener("submit",async u=>{u.preventDefault();const g=new FormData(i),f=l.checked,h={description:g.get("description"),amount:parseFloat(g.get("amount")),dueDate:g.get("dueDate"),naturezaId:g.get("naturezaId")||null,centroDeCustoId:g.get("centroDeCustoId")||null,notes:g.get("notes"),status:f?"paid":"pending",paymentDate:f?g.get("paymentDate"):null,establishmentId:m.establishmentId},v=g.get("installments");!t&&v&&parseInt(v)>1?h.installments=parseInt(v):h.installments=1;try{t?(await(s?Fn(t.id,h):Un(t.id,h)),p("Sucesso","Atualizado com sucesso!","success")):(await(s?Rn(h):zn(h)),p("Sucesso","Criado com sucesso!","success")),document.getElementById("genericModal").style.display="none",Ue()}catch(y){p("Erro",y.message,"error")}})}const Xl=e=>E("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),Zl=e=>E("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),Kl=(e,t)=>{const o=new URLSearchParams({startDate:e,endDate:t}).toString();return E(`/api/commissions/stats?${o}`)},ed=(e={})=>{Object.keys(e).forEach(s=>(e[s]===void 0||e[s]===null||e[s]==="")&&delete e[s]);const t=new URLSearchParams(e).toString(),o=`/api/commissions/history${t?"?"+t:""}`;return E(o)},td=e=>E(`/api/commissions/report/${e}`,{method:"DELETE"}),At=new Date,Ea=new Date(At.getFullYear(),At.getMonth(),1),q={currentTab:"dashboard",professionals:[],calculationResult:null,historyData:[],periodString:"",dashStartDate:Ea.toISOString().split("T")[0],dashEndDate:At.toISOString().split("T")[0],dashStats:{revenue:0,commissions:0},histStartDate:Ea.toISOString().split("T")[0],histEndDate:At.toISOString().split("T")[0],histProfessionalId:"all"};let ht=null;const Qe=document.getElementById("content");async function od(){try{q.professionals=await K(m.establishmentId)}catch(e){console.error("Erro profissionais",e)}pd(),ad(),Yt(),Nt("dashboard")}function ad(){ht&&Qe.removeEventListener("click",ht),ht=e=>{const t=e.target.closest("button");if(!t)return;const o=t.dataset.action,s=t.dataset.id,r=t.dataset.idx;switch(o){case"tab-nav":Nt(t.dataset.tab);break;case"toggle-all-profs":sd();break;case"back-to-filters":q.calculationResult=null,qt(document.getElementById("commissions-content"));break;case"view-preview-items":md(r);break;case"save-final-report":nd();break;case"start-new-calc":Nt("calculator");break;case"print-receipt":id(s);break;case"delete-report":ld(s);break;case"filter-dashboard":Yt();break;case"filter-history":Ho();break}},Qe.addEventListener("click",ht),Qe.oninput=e=>{if(e.target.classList.contains("input-debit")||e.target.classList.contains("input-credit")){const t=e.target.dataset.idx;cd(t)}},Qe.onsubmit=e=>{e.target.id==="calc-form"&&(e.preventDefault(),rd())}}async function Yt(){const e=document.getElementById("dash-start"),t=document.getElementById("dash-end");e&&(q.dashStartDate=e.value),t&&(q.dashEndDate=t.value);const o=document.getElementById("dashboard-stats-container");o&&(o.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>');try{const s=await Kl(q.dashStartDate,q.dashEndDate);q.dashStats={revenue:s.totalRevenue||0,commissions:s.totalCommissionsPaid||0},q.currentTab==="dashboard"&&Ls(document.getElementById("commissions-content"))}catch(s){console.error(s),o&&(o.innerHTML='<p class="text-red-500 text-center">Erro ao carregar dados.</p>')}}async function Ho(){const e=document.getElementById("hist-start"),t=document.getElementById("hist-end"),o=document.getElementById("hist-prof");e&&(q.histStartDate=e.value),t&&(q.histEndDate=t.value),o&&(q.histProfessionalId=o.value);const s=document.getElementById("history-list-container");if(s){s.innerHTML='<div class="flex justify-center py-10"><div class="loader"></div></div>';try{const r=await ed({startDate:q.histStartDate,endDate:q.histEndDate,professionalId:q.histProfessionalId});q.historyData=r,Ts(s,r)}catch{s.innerHTML='<p class="text-red-500 text-center py-4">Erro ao buscar registros.</p>'}}}function sd(){const e=document.querySelectorAll(".prof-checkbox"),t=Array.from(e).every(o=>o.checked);e.forEach(o=>o.checked=!t)}async function rd(){const e=Array.from(document.querySelectorAll(".prof-checkbox:checked")).map(a=>a.value);if(e.length===0)return p("Atenção","Selecione profissionais","error");const t={professionalIds:e,startDate:document.getElementById("start-date").value,endDate:document.getElementById("end-date").value,calculationTypes:{services:document.getElementById("type-services").checked,products:document.getElementById("type-products").checked,packages:document.getElementById("type-packages").checked}},o=new Date(t.startDate+"T00:00:00").toLocaleDateString("pt-BR"),s=new Date(t.endDate+"T00:00:00").toLocaleDateString("pt-BR");q.periodString=`${o} a ${s}`;const r=document.getElementById("commissions-content");r.innerHTML='<div class="flex flex-col items-center justify-center py-20"><div class="loader mb-4"></div><p class="text-gray-500 animate-pulse">Calculando...</p></div>';try{const a=await Xl(t);q.calculationResult=a.map(n=>({...n,extraDebit:0,extraCredit:0,finalValue:n.summary.totalCommission,notes:""})),qt(r)}catch(a){p("Erro",a.message,"error"),q.calculationResult=null,qt(r)}}async function nd(){const e=q.calculationResult.length;if(await O("Confirmar",`Gerar ${e} relatórios? Isso marcará as vendas como pagas.`))try{const o=q.calculationResult.map(s=>{const r=s.items.map(a=>a.originalSaleId).filter(a=>a!=null);return Zl({professionalId:s.professionalId,professionalName:s.professionalName,period:q.periodString,processedSalesIds:r,reportData:{...s,summary:{...s.summary,finalValue:s.finalValue,extraDebit:s.extraDebit||0,extraCredit:s.extraCredit||0,notes:s.notes||""}}})});await Promise.all(o),p("Sucesso","Pagamentos registrados!","success"),q.calculationResult=null,Yt(),Nt("history")}catch(o){p("Erro",o.message,"error")}}function id(e){const t=q.historyData.find(o=>o.id===e);t&&dd(t)}async function ld(e){if(await O("Excluir","Deseja remover este registro? As vendas voltarão a ficar disponíveis para cálculo."))try{await td(e),p("Sucesso","Registro removido.","success"),Ho(),Yt()}catch(o){p("Erro",o.message,"error")}}function dd(e){const{jsPDF:t}=window.jspdf;if(!t)return p("Erro","PDF lib não carregada.","error");const o=new t,s=o.internal.pageSize.getWidth()/2;o.setFontSize(18),o.setFont(void 0,"bold"),o.text("RECIBO DE PAGAMENTO DE COMISSÃO",s,20,{align:"center"}),o.setFontSize(12),o.setFont(void 0,"normal"),o.text(`Profissional: ${e.professionalName}`,15,40),o.text(`Período: ${e.period}`,15,48);const r=[["Comissão Bruta",`R$ ${e.summary.totalCommission.toFixed(2)}`]];e.summary.extraCredit>0&&r.push(["(+) Bônus",`R$ ${e.summary.extraCredit.toFixed(2)}`]),e.summary.extraDebit>0&&r.push(["(-) Descontos",`R$ ${e.summary.extraDebit.toFixed(2)}`]),o.autoTable({startY:60,head:[["Descrição","Valor"]],body:r,theme:"grid"});const a=o.lastAutoTable.finalY+10;o.setFontSize(14),o.setFont(void 0,"bold"),o.text(`Total Líquido: R$ ${(e.summary.finalValue||e.summary.totalCommission).toFixed(2)}`,190,a,{align:"right"}),o.save(`Recibo_${e.professionalName}.pdf`)}function cd(e){const t=document.querySelectorAll(`.input-debit[data-idx="${e}"]`),o=document.querySelectorAll(`.input-credit[data-idx="${e}"]`);let s=0,r=0;if(t.forEach(a=>{a.value&&(s=parseFloat(a.value))}),o.forEach(a=>{a.value&&(r=parseFloat(a.value))}),q.calculationResult&&q.calculationResult[e]){const a=q.calculationResult[e];a.extraDebit=s,a.extraCredit=r,a.finalValue=a.summary.totalCommission-s+r,t.forEach(i=>{i!==document.activeElement&&(i.value=s||"")}),o.forEach(i=>{i!==document.activeElement&&(i.value=r||"")}),document.querySelectorAll(`.final-value-display[data-idx="${e}"]`).forEach(i=>i.innerText=`R$ ${a.finalValue.toFixed(2)}`),ud()}}function ud(){const e=q.calculationResult.reduce((o,s)=>o+s.finalValue,0);document.querySelectorAll("#grand-total-display").forEach(o=>o.innerText=`R$ ${e.toFixed(2)}`)}function md(e){const t=q.calculationResult[e];if(!t)return;const o=t.items.map(s=>`
    ...
    `}function gd(e){const t=q.professionals.map(o=>`<option value="${o.id}" ${q.histProfessionalId===o.id?"selected":""}>${o.name}</option>`).join("");e.innerHTML=`
        <div class="space-y-6">
    ...
    `}const oo=document.getElementById("content");let ke={allPackages:[],catalogForModal:{services:[],products:[]}},xt=null,Be=null;function fd(e=6){let t="";for(let o=0;o<e;o++)t+=`
        ...
        `}).join("")}}function Ia(){const e=document.getElementById("genericModal");e.style.display="none",Be&&e.removeEventListener("click",Be)}async function Ca(e=null){const t=document.getElementById("genericModal"),o=!!e,s=e?JSON.parse(JSON.stringify(e.items||[])):[],r=b(e?.name||""),a=b(e?.description||""),n=e?.price||"",i=e?.commissionRate||0,l=e?.validityDays||30,d=`
    ...
    `;t.innerHTML=d,t.style.display="flex";const c=t.querySelector("#package-items-list"),u=(f,h)=>{const v=h.querySelector("#originalPrice"),y=f.reduce((w,$)=>w+$.price*$.quantity,0);v&&(v.textContent=`R$ ${y.toFixed(2)}`)},g=f=>{f.length===0?c.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':c.innerHTML=f.map((h,v)=>{const y=h.type==="service",w=y?"Serviço":"Produto",$=y?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
            ...
            `}).join(""),u(f,t)};g(s),c.addEventListener("change",f=>{if(f.target.classList.contains("quantity-input")){const h=parseInt(f.target.dataset.index,10),v=parseInt(f.target.value,10);v>0&&s[h]&&(s[h].quantity=v,g(s))}}),c.addEventListener("click",f=>{if(f.target.classList.contains("remove-item-btn")){const h=parseInt(f.target.dataset.index,10);s.splice(h,1),g(s)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>vd(f=>{const h=s.find(v=>v.id===f.id&&v.type===f.type);h?h.quantity++:s.push({...f,quantity:1}),g(s)}),Be&&t.removeEventListener("click",Be),Be=async f=>{const h=f.target.closest("button[data-action]");if(!h)return;const v=h.dataset.action;if(f.stopPropagation(),v==="close-modal"&&Ia(),v==="save-package"){const y=h,w={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:s,originalPrice:s.reduce(($,k)=>$+k.price*k.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null,establishmentId:m.establishmentId};if(!w.name||!w.price){p("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(w.items.length===0){p("Erro","Adicione pelo menos um item ao pacote.","error");return}y.disabled=!0,y.textContent="A salvar...";try{o?await fn(w.id,w):(delete w.id,await gn(w)),p("Sucesso!",`Pacote ${o?"atualizado":"criado"} com sucesso.`,"success"),Ia(),await Oo()}catch($){p("Erro",`Não foi possível salvar o pacote: ${$.message}`,"error"),y.disabled=!1,y.textContent="Salvar Pacote"}}},t.addEventListener("click",Be)}function vd(e){let t="";const o=document.createElement("div");o.id="item-selection-modal",o.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const s={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=l=>{const d=t.toLowerCase(),c=ke.catalogForModal.services.filter(h=>h.name.toLowerCase().includes(d)),u=ke.catalogForModal.products.filter(h=>h.name.toLowerCase().includes(d)),g=c.map(h=>`
    ...
    `,await wd()}async function wd(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=m.userProfessionalId;if(t){const o=await nr(t);xd=o,o.photo&&(document.getElementById("user-profile-avatar").src=o.photo);const s=b(o.name);e.innerHTML=`
            ...
            `,kd(o.id),document.getElementById("my-blocks-filter").addEventListener("change",a=>Rt(o.id,a.target.value)),Rt(o.id,"future")}else e.innerHTML=`
