import"./modulepreload-polyfill-B5Qt9EMX.js";import{a as w,d as l}from"./firebase-config-C2tbVz-J.js";import{onAuthStateChanged as L,signOut as I}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{doc as T,getDoc as $}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import{collection as u,getDocs as c,query as k,where as B,doc as A,updateDoc as R,addDoc as M,serverTimestamp as P,deleteDoc as q}from"https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-storage.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-messaging.js";const C={super_admin:["dashboard","establishments","whatsapp","financial","team","settings"],support:["establishments","whatsapp"],financial:["dashboard","establishments","financial"],developer:["whatsapp","settings"]};let p=null;function D(){document.body.style.display="none",L(w,async t=>{if(t)try{const e=T(l,"admin_users",t.uid),o=await $(e);o.exists()?(p=o.data().role||"support",console.log("🛡️ Acesso concedido. Perfil:",p),N(p),document.body.style.display="flex"):(console.error("🚨 Intruso detectado: Usuário sem registro de Admin."),await I(w),window.location.href="admin-login.html")}catch(e){console.error("Erro ao verificar permissões de segurança:",e),window.location.href="admin-login.html"}else window.location.href="admin-login.html"})}function N(t){const e=C[t]||[];document.querySelectorAll("[data-module]").forEach(r=>{const a=r.getAttribute("data-module");e.includes(a)||r.remove()})}async function H(){try{await I(w),window.location.href="admin-login.html"}catch(t){console.error("Erro ao sair:",t)}}async function j(t){t.innerHTML=`
    ...
    `;try{const e=document.getElementById("dashboard-metrics"),o=u(l,"establishments"),[r,a,n]=await Promise.all([c(o),c(k(o,B("status","==","active"))),c(k(o,B("status","==","blocked")))]),i=r.size,s=a.size,d=n.size;let m=0;a.forEach(f=>{const y=f.data(),x=parseFloat(y.planPrice)||0;m+=x});const g=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(m);e.innerHTML=`
    ...
    `;try{const e=await c(u(l,"establishments")),o=document.getElementById("clients-tbody"),r=document.getElementById("clients-table"),a=document.getElementById("loading-msg");if(e.empty){a.innerText="Nenhum estabelecimento cadastrado ainda.";return}let n="";e.forEach(i=>{const s=i.data(),d=i.id,m=s.name||s.nomeFantasia||"Sem Nome Registrado",g=s.email||"Sem e-mail",f=s.phone||s.whatsapp||"Sem telefone",y=s.planName||"Plano Básico",x=s.status||"active";let h="",v="";x==="active"?(h='<span style="background: #d1fae5; color: #065f46; padding: 5px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 700;">ATIVO</span>',v=`<button class="btn-toggle-status" data-id="${d}" data-action="block" style="background: #fee2e2; color: #b91c1c; border: 1px solid #fca5a5; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 0.8rem; font-weight: 600; margin-right: 8px;">Bloquear</button>`):(h='<span style="background: #fee2e2; color: #991b1b; padding: 5px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 700;">BLOQUEADO</span>',v=`<button class="btn-toggle-status" data-id="${d}" data-action="unblock" style="background: #d1fae5; color: #047857; border: 1px solid #6ee7b7; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 0.8rem; font-weight: 600; margin-right: 8px;">Desbloquear</button>`),n+=`
            ...
            `}),o.innerHTML=n,a.style.display="none",r.style.display="table",F(t)}catch(e){console.error("Erro ao carregar clientes:",e),t.innerHTML='<div style="background: #fee2e2; color: #991b1b; padding: 20px; border-radius: 8px;">Erro ao carregar lista de clientes. Verifique o console.</div>'}}function F(t){document.querySelectorAll(".btn-toggle-status").forEach(r=>{r.addEventListener("click",async a=>{const n=a.target.getAttribute("data-id"),i=a.target.getAttribute("data-action");await O(n,i,t)})}),document.querySelectorAll(".btn-impersonate").forEach(r=>{r.addEventListener("click",a=>{const n=a.target.getAttribute("data-id");alert(`Acesso Administrativo ao cliente ID: ${n} 

(A lógica de login rápido será implementada aqui!)`)})})}async function O(t,e,o){const r=e==="block"?"blocked":"active";if(confirm(e==="block"?"Tem certeza que deseja SUSPENDER o acesso deste cliente?":"Tem certeza que deseja RESTAURAR o acesso deste cliente?"))try{const n=A(l,"establishments",t);await R(n,{status:r}),S(o)}catch(n){console.error("Erro ao alterar status:",n),alert("Falha ao atualizar o banco de dados: "+n.message)}}async function U(t){t.innerHTML=`
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
    ...
    `,await E(),V()}async function E(){const t=document.getElementById("plans-grid"),e=document.getElementById("loading-plans");try{const o=await c(u(l,"packages"));e.style.display="none";let r="";if(o.empty){t.innerHTML='<p style="color: #9ca3af; grid-column: span 3;">Nenhum plano cadastrado. Crie o primeiro!</p>';return}o.forEach(a=>{const n=a.data(),i=a.id,s=parseFloat(n.price||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"}),d=n.whatsappLimit?`${n.whatsappLimit} msgs/mês`:"💬 WhatsApp Ilimitado";r+=`
            ...
            `}),t.innerHTML=r,document.querySelectorAll(".btn-delete-plan").forEach(a=>{a.addEventListener("click",G)})}catch(o){console.error("Erro ao buscar planos:",o),e.innerText="Erro ao buscar os planos. Verifique o console.",e.style.color="red"}}function V(t){const e=document.getElementById("modal-new-plan"),o=document.getElementById("btn-new-plan"),r=document.getElementById("btn-cancel-plan"),a=document.getElementById("btn-save-plan");o.addEventListener("click",()=>{e.style.display="block",o.style.display="none"}),r.addEventListener("click",()=>{e.style.display="none",o.style.display="block"}),a.addEventListener("click",async()=>{const n=document.getElementById("plan-name").value,i=document.getElementById("plan-price").value,s=document.getElementById("plan-wa-limit").value;if(!n||!i){alert("Nome e Preço são obrigatórios!");return}a.innerText="Salvando...",a.disabled=!0;try{await M(u(l,"packages"),{name:n,price:parseFloat(i),whatsappLimit:s?parseInt(s):null,createdAt:P(),status:"active"}),document.getElementById("plan-name").value="",document.getElementById("plan-price").value="",document.getElementById("plan-wa-limit").value="",e.style.display="none",o.style.display="block",a.innerText="Salvar no Banco",a.disabled=!1,document.getElementById("loading-plans").style.display="block",await E()}catch(d){console.error("Erro ao criar plano:",d),alert("Erro ao criar plano: "+d.message),a.innerText="Salvar no Banco",a.disabled=!1}})}async function G(t){const e=t.target.getAttribute("data-id");if(confirm("ATENÇÃO: Tem certeza que deseja apagar este plano? Clientes que já o possuem podem ser afetados se você não migrá-los antes."))try{await q(A(l,"packages",e)),document.getElementById("loading-plans").style.display="block",await E()}catch(o){console.error("Erro ao deletar:",o),alert("Erro ao deletar o plano.")}}D();document.getElementById("btn-logout").addEventListener("click",H);const z=document.querySelectorAll(".sidebar-menu a"),b=document.getElementById("dynamic-content");z.forEach(t=>{t.addEventListener("click",async e=>{e.preventDefault(),z.forEach(r=>r.classList.remove("active")),e.target.classList.add("active"),document.getElementById("page-title").innerText=e.target.innerText.substring(2).trim(),p&&(document.getElementById("user-role-display").innerText=p);const o=e.target.getAttribute("href").replace("#","");o==="dashboard"?await j(b):o==="establishments"?await S(b):o==="financial"?await U(b):b.innerHTML=`<h3 style="color: #6b7280;">Módulo ${o} em desenvolvimento...</h3>`})});setTimeout(()=>{const t=document.getElementById("menu-dashboard");t&&t.click()},1e3);
