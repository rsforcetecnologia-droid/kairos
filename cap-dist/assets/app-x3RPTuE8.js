const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/web-BIj2Dc0k.js","assets/modulepreload-polyfill-B5Qt9EMX.js","assets/firebase-config-C2tbVz-J.js","assets/styles-C2XZ_b0c.css"])))=>i.map(i=>d[i]);
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${n.status}) em ${o}:`,l),new Error(l)}return n.json()}catch(n){throw console.error(`Falha de rede ao tentar acessar ${o}:`,n.message),n.message.includes("Failed to fetch")||n.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${ca}. Verifique se o servidor backend está rodando.`):n}}const Fo=(e,t,a,s=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return s&&(r+=`&professionalId=${s}`),I(r)},Zs=({establishmentId:e,professionalId:t,serviceIds:a,date:s})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${s}`;return I(r)},Ks=e=>I("/api/appointments",{method:"POST",body:JSON.stringify(e)}),er=(e,t)=>I(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),_a=e=>I(`/api/appointments/${e}`,{method:"DELETE"}),tr=e=>I(`/api/appointments/${e}/reopen`,{method:"POST"}),ar=(e,t)=>I(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let W;async function or(){if(!W)try{W=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function sr(){if(!W){console.warn("AudioContext não inicializado. O som não será tocado.");return}W.state==="suspended"&&W.resume();const e=W.createOscillator(),t=W.createGain();e.connect(t),t.connect(W.destination),e.type="sine",e.frequency.setValueAtTime(800,W.currentTime),t.gain.setValueAtTime(0,W.currentTime),t.gain.linearRampToValueAtTime(.3,W.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,W.currentTime+.2),e.start(W.currentTime),e.stop(W.currentTime+.2)}function p(e,t,a="info",s=!1){const r=document.getElementById("toast-container");if(!r)return;s&&sr();const o=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},l={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};o.className=`toast ${n[a]||n.info}`,o.innerHTML=`
    ...
    `;for(let c=0;c<r;c++){const u=new Date(s);u.setDate(u.getDate()+c);const g=new Date,b=u.toDateString()===g.toDateString(),v=e.filter(x=>new Date(x.startTime).toDateString()===u.toDateString()).sort((x,S)=>new Date(x.startTime)-new Date(S.startTime));let h='<div class="flex-grow overflow-y-auto overflow-x-hidden px-1.5 py-2 space-y-2.5 pb-24">';v.length>0?h+=v.map(x=>{const y=new Date(x.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),E=m.professionalColors.get(x.professionalId)||{main:"#9ca3af"},C=f(x.reason),B=f(x.professionalName||"Indefinido"),q=f(x.clientName),T=f(x.serviceName);if(x.type==="blockage")return`
                        <div class="relative p-2 rounded-xl bg-red-50 border border-red-100 shadow-sm overflow-hidden min-w-0">
                            ...
                            <div class="pl-1 min-w-0">
                                <span class="font-bold text-[11px] text-red-900 tracking-tight block">${y}</span>
                                <p class="font-bold text-xs text-red-800 truncate leading-tight mt-0.5">${C}</p>
                                <p class="text-[9px] text-red-600 truncate mt-1">${B}</p>
                            </div>
                        </div>
                    `;const M=JSON.stringify(x).replace(/'/g,"&apos;"),R=x.redeemedReward?.points>0,F=x.hasRewards&&!R,O=x.status==="completed";return`
                    <div class="relative p-2 rounded-xl bg-white border border-gray-100 shadow-sm cursor-pointer hover:shadow-md transition-all duration-200 overflow-hidden min-w-0" 
                        ...
                        <div class="pl-1 min-w-0 flex flex-col h-full justify-center">
                            <div class="flex justify-between items-center mb-0.5">
                                <span class="font-bold text-[11px] text-gray-800 tracking-tight">${y}</span>
                                ${O?'<span class="text-[8px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full leading-none">OK</span>':""}
                            </div>

                            <p class="font-bold text-xs text-gray-800 truncate leading-tight">${F?"🎁 ":""}${q}</p>
                            <p class="text-[10px] text-gray-500 truncate mt-0.5 leading-tight">${T}</p>
                            
                            <div class="flex items-center gap-1 mt-1.5 min-w-0">
                                <div class="w-3.5 h-3.5 rounded-full flex items-center justify-center text-[7px] font-bold text-white flex-shrink-0" style="background-color: ${E.main};">
                                    ${B.charAt(0).toUpperCase()}
                                </div>
                                <p class="text-[9px] text-gray-400 truncate leading-none">${B.split(" ")[0]}</p>
                            </div>
                        </div>
                    </div>
                `}).join(""):h+=`
            ...
            <div class="flex flex-col snap-start shrink-0 relative" style="${l}">
                <div class="sticky top-0 z-10 text-center py-2.5 ${b?"bg-indigo-600 text-white shadow-md":"bg-gray-50/95 backdrop-blur-sm text-gray-700 border-b border-gray-200"}">
                    <p class="text-[9px] uppercase tracking-widest font-bold opacity-80 mb-0.5">${a[u.getDay()]}</p>
                    <div class="flex items-baseline justify-center gap-0.5">
                        <span class="text-xl font-extrabold leading-none">${u.getDate()}</span>
                    </div>
                </div>
                ${h}
            </div>
    ...
    `;return{title:t,content:i}}function dn(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),s=mt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=s.map(r=>{const o=L.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap sm:justify-between sm:items-center mb-4 gap-3">
                    ...
                    <div class="flex flex-col sm:flex-row w-full sm:w-auto gap-3 order-2 sm:order-1">
                        ...
                        <div class="flex items-center justify-between sm:justify-start gap-2 w-full sm:w-auto">
                            ...
                            <div id="week-days-toggle" class="${D.currentView==="week"?"hidden sm:flex":"hidden"} items-center gap-1 rounded-lg bg-gray-200 p-1">
        ...
        </section>`;const t=document.getElementById("btn-toggle-select");t.addEventListener("click",()=>{D.isSelectionMode=!D.isSelectionMode,D.isSelectionMode||D.selectedItems.clear(),t.classList.toggle("bg-blue-100",D.isSelectionMode),t.classList.toggle("text-blue-700",D.isSelectionMode),as()}),document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),D.currentView=a.dataset.view;const s=document.getElementById("week-days-toggle");if(D.currentView==="week"){if(s.className="hidden sm:flex items-center gap-1 rounded-lg bg-gray-200 p-1",window.innerWidth<768){D.weekViewDays=7,document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="7"]');r&&r.classList.add("active")}}else s.className="hidden items-center gap-1 rounded-lg bg-gray-200 p-1";oe()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active")),a.classList.add("active"),D.weekViewDays=parseInt(a.dataset.days,10),oe()})}),document.getElementById("todayBtn").addEventListener("click",()=>{D.currentDate=new Date,oe()}),document.getElementById("dateFilterInput").addEventListener("change",a=>{if(a.target.value){const[s,r,o]=a.target.value.split("-");D.currentDate=new Date(s,r-1,o),oe()}}),document.getElementById("profSearchInput").addEventListener("input",a=>{D.profSearchTerm=a.target.value,Bt()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{D.showInactiveProfs=a.target.checked,Bt(),oe()}),oo||(ao.addEventListener("click",async a=>{const s=a.target.closest("[data-action]");if(a.target.dataset.action==="toggle-select-item"){const i=a.target.dataset.id;a.target.checked?D.selectedItems.add(i):D.selectedItems.delete(i),Ta();return}if(s&&s.dataset.action==="batch-delete"){const i=D.selectedItems.size;if(await H("Excluir em Lote",`Tem certeza que deseja excluir ${i} agendamento(s)? Esta ação não pode ser desfeita.`)){const d=Array.from(D.selectedItems);let c=0;try{await Promise.all(d.map(async u=>{try{await _a(u),c++}catch(g){console.error(`Falha ao excluir ${u}`,g)}})),p(`${c} agendamento(s) excluído(s).`,"success"),D.selectedItems.clear(),D.isSelectionMode=!1,document.getElementById("btn-toggle-select").classList.remove("bg-blue-100","text-blue-700"),oe()}catch{p("Erro ao processar exclusão em lote.","error")}}return}if(a.target.closest('[data-action="select-professional"]')){const l=a.target.closest('[data-action="select-professional"]').dataset.profId,d=D.selectedProfessionalId===l&&l!=="all";if(D.selectedProfessionalId=d?"all":l,l!=="all"){const c=document.getElementById("profSearchInput");c&&(c.value=""),D.profSearchTerm=""}await oe();return}if(!s)return;const r=s.dataset.action;let o=null;const n=a.target.closest("[data-appointment]");switch(n&&(o=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":pa();break;case"edit-appointment":if(D.isSelectionMode||!o)return;if(o.status==="completed"){p("Atenção","Agendamentos finalizados não podem ser editados.","error");return}o.hasRewards&&!o.redeemedReward&&p("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),pa(o);break;case"delete-appointment":{if(D.isSelectionMode)return;const i=s.dataset.id;if(await H("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await _a(i),p("Agendamento apagado!","success"),oe()}catch(d){p(`Não foi possível apagar: ${d.message}`,"error")}break}case"open-comanda":if(D.isSelectionMode)return;if(o){o.hasRewards&&!o.redeemedReward&&o.status!=="completed"&&p("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=o.status==="completed"?"finalizadas":"em-atendimento",l={selectedAppointmentId:o.id,initialFilter:i};if(i==="finalizadas"){let d=o.startTime;if(o.transaction&&o.transaction.paidAt){const c=o.transaction.paidAt;typeof c=="object"&&c._seconds?d=new Date(c._seconds*1e3):d=c}l.filterDate=d}K("comandas-section",l)}break}}),oo=!0),await Qr(),await oe()}const un=(e,t=null,a=1,s=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${s}`;return t&&(r+=`&date=${t}`),I(r)},mn=(e,t)=>I(`/api/appointments/${e}/comanda`,{method:"POST",body:JSON.stringify({items:t})}),pn=e=>I("/api/sales",{method:"POST",body:JSON.stringify(e)}),gn=e=>I(`/api/sales/${e}`,{method:"DELETE"}),bn=()=>I("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),fn=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),I("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},vn=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),I(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},hn=()=>I("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),xn=e=>I(`/api/cashier/report/${e}`),Da=e=>I(`/api/packages/${e}`),yn=e=>I("/api/packages",{method:"POST",body:JSON.stringify(e)}),wn=(e,t)=>I(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),kn=e=>I(`/api/packages/${e}`,{method:"DELETE"});let w={allComandas:[],catalog:{services:[],products:[],packages:[]},activeFilter:"atendimento",selectedComandaId:null,viewMode:"items",isCashierOpen:!1,activeCashierSessionId:null,loyaltySettings:null,pendingRedemption:null,paging:{page:1,limit:10,total:0},checkoutState:{payments:[],selectedMethod:"dinheiro",installments:1,amountReceived:"",discount:{type:"real",value:0},discountReason:""},isProcessing:!1},Le=null,De=null,ro=null;function is(e,t){return function(...a){clearTimeout(ro),ro=setTimeout(()=>e.apply(this,a),t)}}async function no(e,t="stay"){if(!e||!e.id)return;e._localUpdatedAt=Date.now(),e._cachedItems=null,e._hasUnsavedChanges=!1,lt(),t==="checkout"&&(w.viewMode="checkout",w.checkoutState.payments||(w.checkoutState.payments=[]),w.checkoutState.selectedMethod="dinheiro",w.checkoutState.amountReceived="",w.checkoutState.discount.value||(w.checkoutState.discount={type:"real",value:0},w.checkoutState.discountReason=""),V());const a=document.createElement("div");a.id="saving-overlay",a.className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center backdrop-blur-sm",a.innerHTML=`
    ...
    <div class="modal-content max-w-3xl overflow-y-auto max-h-[90vh]">
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                ...
                `).join("")):(M.classList.remove("hidden"),M.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else M.classList.add("hidden");o.size>0?(R.innerHTML="",o.forEach(O=>{const j=s.find(Q=>Q.id===O);j&&(R.innerHTML+=`
                    ...
                    `)})):R.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};y.querySelector("#modalSupplierSearch").addEventListener("input",E),y.addEventListener("click",T=>{const M=T.target.closest("[data-add-supplier]");if(M){const F=M.dataset.addSupplier;o.add(F),y.querySelector("#modalSupplierSearch").value="",E()}const R=T.target.closest("[data-remove-supplier]");if(R){const F=R.dataset.removeSupplier;o.delete(F),E()}}),E(),y.addEventListener("click",async T=>{const M=T.target.closest("button[data-action]");if(!M)return;const R=M.dataset.action,F=y.querySelector("#productId").value;if(R==="close-modal"&&(y.style.display="none"),R==="delete-product"){if(!F)return;y.style.display="none",await Di(F)}if(R==="save-product-modal"){const O=y.querySelector("#productForm");if(O){if(!O.querySelector("#productName").value||!O.querySelector("#productPrice").value){p("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const j=M.closest('button[data-action="save-product-modal"]');j.disabled=!0,j.textContent="A salvar...";try{await Pi(O)}catch(Q){p("Erro",`Falha ao salvar: ${Q.message}`,"error"),j.disabled=!1,j.textContent="Salvar Alterações"}}}if(R==="adjust-stock-modal"){T.preventDefault();const O=y.querySelector("#stockAdjustmentAmount"),j=y.querySelector("#stockAdjustmentReason"),Q=parseInt(O.value,10),Ce=parseInt(M.dataset.change,10);if(!Q||Q<=0){p("Erro","Por favor, insira uma quantidade válida.","error");return}const Kt=Q*Ce,Ns=j.value||(Kt>0?"Entrada manual":"Saída manual");try{await yr(F,{change:Kt,reason:Ns});const Je=m.products.findIndex(Ge=>Ge.id===F);if(Je>-1){const Ge=m.products[Je].currentStock+Kt;m.products[Je].currentStock=Ge,y.querySelector("#currentStockDisplay").textContent=Ge,y.querySelector("#productCurrentStock").value=Ge,O.value="",j.value="",p("Sucesso","Estoque atualizado!","success"),Ra(),dt()}}catch(Je){p("Erro de Stock",Je.message,"error")}}});const C=y.querySelectorAll(".tab-btn"),B=y.querySelectorAll(".tab-content");C.forEach(T=>{T.addEventListener("click",M=>{M.preventDefault(),C.forEach(R=>{R.classList.remove("border-indigo-500","text-indigo-600"),R.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),T.classList.add("border-indigo-500","text-indigo-600"),T.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),B.forEach(R=>R.classList.add("hidden")),document.getElementById(`tab-content-${T.dataset.tab}`).classList.remove("hidden")})});const q=y.querySelector("#productPhotoInput");y.querySelector("#productPhotoButton").addEventListener("click",()=>q.click()),q.onchange=async()=>{const T=q.files[0];if(!T)return;const M=y.querySelector("#productPhotoPreview"),R=y.querySelector("#productPhotoBase64");M.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const F=await go(T,800,800,"image/jpeg",.8),j=F.length*3/4,Q=1e3*1024;if(j>Q)throw new Error("A imagem é muito grande mesmo após a compressão.");M.src=F,R.value=F}catch(F){console.error("Erro ao processar imagem:",F),p("Erro de Imagem",F.message||"Não foi possível processar a imagem.","error"),M.src=x,R.value=S,q.value=""}},y.style.display="flex"}function Bi(){const e=document.getElementById("products-content-container");e.innerHTML=`
...
Aguardo retorno.`;const d=`mailto:${s.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(l)}`;window.location.href=d}else p("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order");if(a.disabled)return;const s=JSON.parse(decodeURIComponent(a.dataset.order));s.establishmentId=m.establishmentId,a.disabled=!0,a.textContent="A processar...",Ie(m.establishmentId).then(r=>{const o=r.purchaseConfig||null;return $i(s,o)}).then(()=>{p("Sucesso","Compra registrada e integrada ao financeiro!","success"),a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{a.disabled=!1,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Salvar',p("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-delete-purchase")){const s=e.target.closest(".btn-delete-purchase").dataset.id;H("Excluir Compra","Isto apagará o registo histórico E o lançamento financeiro associado. Deseja continuar?").then(async r=>{if(r)try{await Ei(s,m.establishmentId),p("Sucesso","Compra e financeiro excluídos.","success"),Es()}catch(o){p("Erro","Erro ao excluir: "+o.message,"error")}})}if(e.target.closest(".btn-print-order")){const s=e.target.closest(".supplier-order-card").dataset.supplierId,r=z.finalOrders[s];r?Vi(r,z.isQuoteMode):p("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),s=JSON.parse(a.dataset.purchase);zi(s)}},ve.addEventListener("click",ue),ve.addEventListener("input",ue),ve.addEventListener("change",ue),vt("list")}function vt(e){Lt=e,["list","purchases","history"].forEach(a=>{const s=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(s.classList.add("border-indigo-500","text-indigo-600"),s.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(s.classList.remove("border-indigo-500","text-indigo-600"),s.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),Ni()}const ta=document.getElementById("content"),vo={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let ie=new Set,ht=null,Me=null;function _i(e=8){let t="";for(let a=0;a<e;a++)t+=`
    ...
    `;const g=window.innerWidth<768,b=u.querySelector(".bg-white");if(g&&b){b.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const x=u.closest("section");x&&(x.style.padding="0",x.style.margin="0"),b.classList.add("p-4")}const v=u.querySelector("#userProfessionalId"),h=u.querySelector("#userPhotoPreview"),k=u.querySelector("#profPhotoName");if(v.addEventListener("change",x=>{const S=x.target.value,y=s(S),E=y?y.name:"Selecione um profissional",C=r(S,n);h.src=C.photoSrc,k.textContent=E,u.querySelector("#professionalPhotoUrl").value=C.photoUrl}),u.addEventListener("submit",async x=>{x.preventDefault();const S=e?.email,y=u.querySelector("#userEmail").value,E={};u.querySelectorAll('input[type="checkbox"]').forEach(q=>{const T=q.dataset.module,M=q.dataset.permission;E[T]||(E[T]={}),E[T][M]=q.checked});const C=u.querySelector("#userProfessionalId").value||null,B={name:u.querySelector("#userName").value,permissions:E,professionalId:C,establishmentId:m.establishmentId};try{c?(S!==y&&(B.email=y),await Al(e.id,B),p("Usuário atualizado com sucesso!","success")):(B.email=u.querySelector("#userEmail").value,B.password=u.querySelector("#userPassword").value,await Ml(B),p("Usuário criado com sucesso!","success")),qt()}catch(q){p(`Erro: ${q.message}`,"error")}}),c){const x=u.querySelector("#password-change-container"),S=x.querySelector('[data-action="show-password-form"]'),y=x.querySelector("#password-form"),E=y.querySelector('[data-action="save-password"]'),C=y.querySelector('[data-action="cancel-password-change"]');S.addEventListener("click",()=>{S.classList.add("hidden"),y.classList.remove("hidden")}),C.addEventListener("click",()=>{S.classList.remove("hidden"),y.classList.add("hidden"),y.querySelector("#userNewPassword").value=""}),E.addEventListener("click",async()=>{const B=y.querySelector("#userNewPassword").value;if(!B||B.length<6){p("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await H("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{E.disabled=!0,E.textContent="Aguarde...",await Nl(e.id,B),p("Sucesso!","A senha do usuário foi alterada.","success"),S.classList.remove("hidden"),y.classList.add("hidden"),y.querySelector("#userNewPassword").value=""}catch(T){p("Erro",`Não foi possível alterar a senha: ${T.message}`,"error")}finally{E.disabled=!1,E.textContent="Salvar Nova Senha"}})}}async function zl(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,a]=await Promise.all([Bl(m.establishmentId),ee(m.establishmentId)]);m.users=t,m.professionals=a,Tt()}catch{p("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function qt(){he.innerHTML=`
    ...
    `}function Zl(){document.getElementById("select-all-toggle").addEventListener("change",a=>{const s=a.target.checked,r=document.querySelectorAll(".item-checkbox");A.selectedIds.clear(),r.forEach(o=>{o.checked=s,s&&A.selectedIds.add(o.dataset.id)}),Ae()}),document.getElementById("cancel-selection-btn").addEventListener("click",()=>{A.selectedIds.clear(),document.getElementById("select-all-toggle").checked=!1,document.querySelectorAll(".item-checkbox").forEach(a=>a.checked=!1),Ae()}),document.getElementById("batch-delete-btn").addEventListener("click",async()=>{const a=A.selectedIds.size;if(a===0)return;if(await H("Excluir Selecionados",`Tem certeza que deseja excluir ${a} itens? Esta ação não pode ser desfeita.`))try{const r=A.currentTab==="payables"?"payables":"receivables";await gs(r,Array.from(A.selectedIds)),p("Sucesso",`${a} itens excluídos.`,"success"),A.selectedIds.clear(),Ae(),Ee()}catch(r){p("Erro","Falha ao excluir itens.","error"),console.error(r)}}),document.getElementById("toggle-filter-btn").addEventListener("click",()=>{const a=document.getElementById("filter-panel"),s=document.getElementById("toggle-filter-btn");A.isFilterOpen=!A.isFilterOpen,A.isFilterOpen?(a.classList.remove("hidden"),s.classList.add("bg-indigo-100","text-indigo-600")):(a.classList.add("hidden"),s.classList.remove("bg-indigo-100","text-indigo-600"))}),document.getElementById("settings-btn").addEventListener("click",rd),document.getElementById("fab-add").addEventListener("click",()=>{const a=A.currentTab==="payables"?"payable":"receivable";sa(a)});const e=document.getElementById("tab-receivables"),t=document.getElementById("tab-payables");e.addEventListener("click",()=>$o("receivables")),t.addEventListener("click",()=>$o("payables")),document.querySelectorAll(".status-filter-btn").forEach(a=>{a.addEventListener("click",s=>{document.querySelectorAll(".status-filter-btn").forEach(r=>{r.classList.remove("bg-indigo-100","text-indigo-700"),r.classList.add("text-gray-500")}),s.target.classList.add("bg-indigo-100","text-indigo-700"),s.target.classList.remove("text-gray-500"),A.statusFilter=s.target.dataset.status})}),document.getElementById("apply-filter-btn").addEventListener("click",()=>{A.startDate=document.getElementById("filterStartDate").value,A.endDate=document.getElementById("filterEndDate").value,A.filterNaturezaId=document.getElementById("filterNaturezaId").value,A.filterCostCenterId=document.getElementById("filterCostCenterId").value,document.getElementById("toggle-filter-btn").click(),Ee()}),xt&&document.body.removeEventListener("click",xt),xt=a=>{const s=a.target;if(s.classList.contains("item-checkbox")){const n=s.dataset.id;s.checked?A.selectedIds.add(n):A.selectedIds.delete(n),Ae(),a.stopPropagation();return}const r=s.closest("button[data-action]");if(r){const{action:n,type:i,id:l}=r.dataset;if(a.stopPropagation(),n==="delete"){const d=r.closest(".financial-card-item").dataset.item.replace(/&apos;/g,"'"),c=JSON.parse(d);ad(i,c);return}if(n==="mark-as-paid"){td(i,l);return}if(n==="manage-natures"){ya("nature");return}if(n==="manage-cost-centers"){ya("cost-center");return}if(n==="edit"){const d=JSON.parse(r.dataset.item.replace(/&apos;/g,"'"));sa(i,d);return}}const o=s.closest(".financial-card-item");if(o&&document.getElementById("list-container").contains(o)&&!s.closest(".checkbox-wrapper")){const{type:n}=o.dataset,i=JSON.parse(o.dataset.item.replace(/&apos;/g,"'"));sa(n,i)}},document.body.addEventListener("click",xt),yt&&document.getElementById("genericModal").removeEventListener("click",yt),yt=a=>{if(a.target.closest('[data-action="close-modal"]')){document.getElementById("genericModal").style.display="none";return}const r=a.target.closest('button[data-action^="delete-"]');if(r){const o=r.dataset.action.split("-")[1];sd(o,r.dataset.id)}},document.getElementById("genericModal").addEventListener("click",yt)}function Ae(){const e=document.getElementById("batch-action-bar"),t=document.getElementById("selected-count"),a=document.getElementById("fab-add"),s=A.selectedIds.size;t.textContent=s,s>0?(e.classList.remove("hidden"),a.classList.add("hidden")):(e.classList.add("hidden"),a.classList.remove("hidden"))}function $o(e){A.currentTab=e,A.selectedIds.clear(),Ae();const t=document.getElementById("tab-receivables"),a=document.getElementById("tab-payables"),s=document.getElementById("fab-add");e==="receivables"?(t.classList.add("bg-white","text-green-700","shadow-sm"),t.classList.remove("text-gray-500"),a.classList.remove("bg-white","text-red-700","shadow-sm"),a.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-green-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-green-700 hover:scale-105 transition-all z-40"):(a.classList.add("bg-white","text-red-700","shadow-sm"),a.classList.remove("text-gray-500"),t.classList.remove("bg-white","text-green-700","shadow-sm"),t.classList.add("text-gray-500"),s.className="fixed bottom-6 right-6 w-14 h-14 bg-red-600 text-white rounded-full shadow-xl flex items-center justify-center hover:bg-red-700 hover:scale-105 transition-all z-40"),Bs()}async function Ee(){const e=document.getElementById("list-container");e.innerHTML='<div class="loader mx-auto my-10"></div>';try{if(A.natures.length===0){const[r,o]=await Promise.all([Gt(m.establishmentId),Pa(m.establishmentId)]);A.natures=r,A.costCenters=o,Kl()}const t={startDate:A.startDate,endDate:A.endDate,establishmentId:m.establishmentId};A.filterNaturezaId!=="all"&&(t.natureId=A.filterNaturezaId),A.filterCostCenterId!=="all"&&(t.costCenterId=A.filterCostCenterId);const[a,s]=await Promise.all([fs(t),vs(t)]);A.payables=a.entries||[],A.receivables=s.entries||[],ed(),Bs()}catch(t){e.innerHTML=`<p class="text-center text-red-500 mt-10">Erro ao carregar: ${t.message}</p>`}}function Kl(){const e=s=>{let r='<option value="all">Todas</option>';const o=Fa(s),n=(i,l=0)=>{const d="  ".repeat(l)+(l>0?"↳ ":"");r+=`<option value="${i.id}">${d}${i.name}</option>`,i.children.forEach(c=>n(c,l+1))};return o.forEach(i=>n(i)),r},t=document.getElementById("filterNaturezaId"),a=document.getElementById("filterCostCenterId");t&&(t.innerHTML=e(A.natures)),a&&(a.innerHTML=e(A.costCenters)),t&&(t.value=A.filterNaturezaId),a&&(a.value=A.filterCostCenterId)}function ed(){const e=document.getElementById("summary-section");if(!e)return;const t=A.receivables.reduce((l,d)=>l+d.amount,0),a=A.receivables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),s=t-a,r=A.payables.reduce((l,d)=>l+d.amount,0),o=A.payables.filter(l=>l.status==="paid").reduce((l,d)=>l+d.amount,0),n=r-o,i=a-o;e.innerHTML=`
    ...
    `}function Bs(){const e=document.getElementById("list-container");if(!e)return;const t=A.currentTab==="receivables",a=t?A.receivables:A.payables;let s=a;if(A.statusFilter!=="all"&&(s=a.filter(n=>n.status===A.statusFilter)),s.sort((n,i)=>new Date(n.dueDate)-new Date(i.dueDate)),s.length===0){e.innerHTML=`
