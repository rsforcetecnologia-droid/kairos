import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css               */import{a as J,d as Q}from"./firebase-config-5GGiRrrP.js";import{EmailAuthProvider as Ro,reauthenticateWithCredential as Ho,verifyBeforeUpdateEmail as jo,updatePassword as Oo,updateProfile as zo,onAuthStateChanged as Vo,signOut as Ia}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";import{addDoc as ua,collection as De,getDocs as co,query as uo,orderBy as mo,doc as Ct,deleteDoc as Uo,updateDoc as po,getDoc as Jo,where as _o,onSnapshot as Wo}from"https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";const p={establishmentId:null,establishmentName:null,userName:null,userProfessionalId:null,userPermissions:null,enabledModules:{agenda:!0,comandas:!0,relatorios:!0,commissions:!0,packages:!0,financial:!0,servicos:!0,produtos:!0,profissionais:!0,clientes:!0,users:!0,estabelecimento:!0},professionals:[],services:[],professionalColors:new Map,allEvents:[]};function Go(e,t,a){p.establishmentId=e,p.establishmentName=t,p.userPermissions=a}const Qt="https://kairos-service-603994960586.southamerica-east1.run.app";console.log("🚀 API configurada para Produção:",Qt);async function Yo(){const e=J.currentUser;return e?{"Content-Type":"application/json",Authorization:`Bearer ${await e.getIdToken()}`}:(console.warn("Usuário não logado, tentando redirecionar para /login"),window.location.href="/login",null)}async function k(e,t={}){const a=await Yo();if(!a)throw new Error("Utilizador não autenticado. A requisição foi cancelada.");const o=`${Qt}${e}`;console.log(`AuthenticatedFetch: ${t.method||"GET"} ${o}`);try{const r=await fetch(o,{...t,headers:{...a,...t.headers}});if(!r.ok){const n=(await r.json().catch(()=>({message:r.statusText}))).message||`Erro na API: ${r.status}`;if(n.includes("FAILED_PRECONDITION")&&n.includes("requires an index")){const i=/(https:\/\/[^\s]+)/,d=n.match(i),l=d?d[0]:"URL não encontrada na mensagem de erro.";console.warn(`%c AVISO IMPORTANTE (FIREBASE): ÍNDICE NECESSÁRIO! %c
...
--------------------------------------------------------------------`,"background-color: #ffc; color: #663300; font-size: 14px; font-weight: bold; padding: 5px;","color: #663300; font-size: 12px;","color: #0000ff; font-size: 12px; font-weight: bold; text-decoration: underline;","color: #663300; font-size: 12px;")}throw console.error(`Erro na API (${r.status}) em ${o}:`,n),new Error(n)}return r.json()}catch(r){throw console.error(`Falha de rede ao tentar acessar ${o}:`,r.message),r.message.includes("Failed to fetch")||r.message.includes("NetworkError")?new Error(`Não foi possível conectar ao servidor em ${Qt}. Verifique sua conexão com a internet.`):r}}const Qo=(e,t,a,o=null)=>{let r=`/api/appointments/${e}?startDate=${t}&endDate=${a}`;return o&&(r+=`&professionalId=${o}`),k(r)},Xo=(e,t,a)=>{const o=`/api/appointments/cancelled/${e}?startDate=${t}&endDate=${a}`;return k(o)},Zo=({establishmentId:e,professionalId:t,serviceIds:a,date:o})=>{const r=`/api/availability?establishmentId=${e}&professionalId=${t}&serviceIds=${a.join(",")}&date=${o}`;return k(r)},Ko=e=>k("/api/appointments",{method:"POST",body:JSON.stringify(e)}),ma=(e,t)=>k(`/api/appointments/${e}`,{method:"PUT",body:JSON.stringify(t)}),es=e=>k(`/api/appointments/${e}`,{method:"DELETE"}),ts=e=>k(`/api/appointments/${e}/reopen`,{method:"POST"}),as=(e,t)=>k(`/api/appointments/${e}/checkout`,{method:"POST",body:JSON.stringify(t)});let F;async function os(){if(!F)try{F=new(window.AudioContext||window.webkitAudioContext)}catch(e){console.error("Não foi possível inicializar o áudio:",e)}}function ss(){if(!F){console.warn("AudioContext não inicializado. O som não será tocado.");return}F.state==="suspended"&&F.resume();const e=F.createOscillator(),t=F.createGain();e.connect(t),t.connect(F.destination),e.type="sine",e.frequency.setValueAtTime(800,F.currentTime),t.gain.setValueAtTime(0,F.currentTime),t.gain.linearRampToValueAtTime(.3,F.currentTime+.01),t.gain.exponentialRampToValueAtTime(1e-4,F.currentTime+.2),e.start(F.currentTime),e.stop(F.currentTime+.2)}function g(e,t,a="info",o=!1){const r=document.getElementById("toast-container");if(!r)return;o&&ss();const s=document.createElement("div"),n={success:"bg-green-50 border-green-400 text-green-700",error:"bg-red-50 border-red-400 text-red-700",info:"bg-blue-50 border-blue-400 text-blue-700"},i={success:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',error:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>',info:'<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>'},d={success:"bg-green-500",error:"bg-red-500",info:"bg-blue-500"};s.className=`toast ${n[a]||n.info}`,s.innerHTML=`
    ...
    `,r.appendChild(s),s.querySelector(".toast-close").addEventListener("click",()=>s.remove()),setTimeout(()=>{s.remove()},4e3)}function D(e,t){const a=document.getElementById("genericModal");return new Promise(o=>{a.innerHTML=`
    ...
    `;const d=r.querySelector("[data-close-modal]");d&&(d.onclick=n);const l=r.querySelector('[data-action="close-modal"]');return l&&(l.onclick=n),r.addEventListener("click",c=>{c.target.closest(".modal-content")||n()}),r.style.display="flex",{modalElement:r,close:n,setContent:i}}function Lt(e){const t=document.getElementById(e);t&&(t.style.display="none")}function rs(){document.body.addEventListener("click",()=>{F||os()},{once:!0}),document.addEventListener("click",e=>{const t=e.target.closest('[data-action="close-modal"]');if(t){const o=t.dataset.target;if(o){const r=document.getElementById(o);r&&(r.style.display="none")}}if(e.target.closest("[data-close-modal]")){const o=document.getElementById("genericModal");o&&(o.style.display="none")}})}async function Ca(){const e=document.getElementById("cancellationListContainer");if(!e)return;e.innerHTML='<div class="loader mx-auto"></div>';const t=document.getElementById("cancelStartDate").value,a=document.getElementById("cancelEndDate").value;try{const o=await Xo(p.establishmentId,t,a);if(o.length===0){e.innerHTML='<p class="text-center text-gray-500 py-4">Nenhum cancelamento encontrado para este período.</p>';return}e.innerHTML=o.map(r=>`
        ...
        `).join("")}catch(o){e.innerHTML=`<p class="text-red-500 text-center py-4">Erro ao carregar histórico: ${o.message}</p>`}}function ns(){const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const o=`
    ...
    `,{modalElement:r}=N({title:"Histórico de Cancelamentos",contentHTML:o,maxWidth:"max-w-3xl"});r.querySelector("#searchCancellationsBtn").addEventListener("click",Ca),Ca()}const z=document.getElementById("sidebar"),La=document.getElementById("sidebarToggle"),Re=document.getElementById("mainContent"),is=document.querySelectorAll(".sidebar-link"),Ba=document.getElementById("hamburger-menu-btn"),Le=document.getElementById("mobile-overlay");function vt(e){!z||!Re||(z.classList.toggle("collapsed",e),Re.classList.toggle("sidebar-collapsed-shift",e))}function ls(){!z||!Le||(z.classList.add("mobile-open"),Le.classList.add("visible"))}function it(){!z||!Le||(z.classList.remove("mobile-open"),Le.classList.remove("visible"))}function ds(){vt(!z.classList.contains("collapsed"))}function cs(e,t,a){if(!z||!Re)return;Re.classList.add("main-content-shift"),window.innerWidth>=768?vt(z.classList.contains("collapsed")):(Re.classList.remove("main-content-shift","sidebar-collapsed-shift"),it()),La&&La.addEventListener("click",r=>{r.stopPropagation(),ds()}),z.addEventListener("mouseenter",()=>{window.innerWidth>=1024&&z.classList.contains("collapsed")&&vt(!1)}),z.addEventListener("mouseleave",()=>{window.innerWidth>=1024&&(document.querySelector("#sidebarToggle:hover")||vt(!0))}),Ba&&Ba.addEventListener("click",r=>{r.stopPropagation(),ls()}),Le&&Le.addEventListener("click",r=>{r.stopPropagation(),it()});let o=0;z.addEventListener("touchstart",r=>{o=r.changedTouches[0].screenX},{passive:!0}),z.addEventListener("touchend",r=>{const s=r.changedTouches[0].screenX;o-s>50&&it()},{passive:!0}),is.forEach(r=>{const s=r.getAttribute("data-target"),n=s.replace("-section",""),i=a?.[n]!==!1,d=t===null||t[s]?.view===!0;if(!i||!d){r.style.display="none";return}r.style.display="flex",r.addEventListener("click",l=>{l.preventDefault(),s&&typeof e=="function"&&e(s),window.innerWidth<768&&it()})})}const et=e=>{const t=e||p.establishmentId;return t?k(`/api/establishments/${t}`):Promise.reject(new Error("ID do estabelecimento não fornecido."))},us=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}`,{method:"PUT",body:JSON.stringify(t)}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ms=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}/booking-status`,{method:"PATCH",body:JSON.stringify({publicBookingEnabled:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},ps=(e,t)=>{const a=e||p.establishmentId;return a?k(`/api/establishments/${a}/owner-email`,{method:"PATCH",body:JSON.stringify({newEmail:t})}):Promise.reject(new Error("ID do estabelecimento não fornecido."))},G=e=>k(`/api/professionals/${e}`),gs=e=>k(`/api/professionals/details/${e}`),bs=e=>k("/api/professionals",{method:"POST",body:JSON.stringify(e)}),fs=(e,t)=>k(`/api/professionals/${e}`,{method:"PUT",body:JSON.stringify(t)}),vs=e=>k(`/api/professionals/${e}`,{method:"DELETE"});var Be;(function(e){e.Unimplemented="UNIMPLEMENTED",e.Unavailable="UNAVAILABLE"})(Be||(Be={}));class Rt extends Error{constructor(t,a,o){super(t),this.message=t,this.code=a,this.data=o}}const hs=e=>{var t,a;return e?.androidBridge?"android":!((a=(t=e?.webkit)===null||t===void 0?void 0:t.messageHandlers)===null||a===void 0)&&a.bridge?"ios":"web"},ys=e=>{const t=e.CapacitorCustomPlatform||null,a=e.Capacitor||{},o=a.Plugins=a.Plugins||{},r=()=>t!==null?t.name:hs(e),s=()=>r()!=="web",n=u=>{const m=l.get(u);return!!(m?.platforms.has(r())||i(u))},i=u=>{var m;return(m=a.PluginHeaders)===null||m===void 0?void 0:m.find(b=>b.name===u)},d=u=>e.console.error(u),l=new Map,c=(u,m={})=>{const b=l.get(u);if(b)return console.warn(`Capacitor plugin "${u}" already registered. Cannot register plugins twice.`),b.proxy;const f=r(),v=i(u);let x;const h=async()=>(!x&&f in m?x=typeof m[f]=="function"?x=await m[f]():x=m[f]:t!==null&&!x&&"web"in m&&(x=typeof m.web=="function"?x=await m.web():x=m.web),x),y=(T,P)=>{var A,O;if(v){const oe=v?.methods.find(_=>P===_.name);if(oe)return oe.rtype==="promise"?_=>a.nativePromise(u,P.toString(),_):(_,rt)=>a.nativeCallback(u,P.toString(),_,rt);if(T)return(A=T[P])===null||A===void 0?void 0:A.bind(T)}else{if(T)return(O=T[P])===null||O===void 0?void 0:O.bind(T);throw new Rt(`"${u}" plugin is not implemented on ${f}`,Be.Unimplemented)}},S=T=>{let P;const A=(...O)=>{const oe=h().then(_=>{const rt=y(_,T);if(rt){const nt=rt(...O);return P=nt?.remove,nt}else throw new Rt(`"${u}.${T}()" is not implemented on ${f}`,Be.Unimplemented)});return T==="addListener"&&(oe.remove=async()=>P()),oe};return A.toString=()=>`${T.toString()}() { [capacitor code] }`,Object.defineProperty(A,"name",{value:T,writable:!1,configurable:!1}),A},$=S("addListener"),L=S("removeListener"),B=(T,P)=>{const A=$({eventName:T},P),O=async()=>{const _=await A;L({eventName:T,callbackId:_},P)},oe=new Promise(_=>A.then(()=>_({remove:O})));return oe.remove=async()=>{console.warn("Using addListener() without 'await' is deprecated."),await O()},oe},j=new Proxy({},{get(T,P){switch(P){case"$$typeof":return;case"toJSON":return()=>({});case"addListener":return v?B:$;case"removeListener":return L;default:return S(P)}}});return o[u]=j,l.set(u,{name:u,proxy:j,platforms:new Set([...Object.keys(m),...v?[f]:[]])}),j};return a.convertFileSrc||(a.convertFileSrc=u=>u),a.getPlatform=r,a.handleError=d,a.isNativePlatform=s,a.isPluginAvailable=n,a.registerPlugin=c,a.Exception=Rt,a.DEBUG=!!a.DEBUG,a.isLoggingEnabled=!!a.isLoggingEnabled,a},xs=e=>e.Capacitor=ys(e),Ge=xs(typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{}),pa=Ge.registerPlugin;class go{constructor(){this.listeners={},this.retainedEventArguments={},this.windowListeners={}}addListener(t,a){let o=!1;this.listeners[t]||(this.listeners[t]=[],o=!0),this.listeners[t].push(a);const s=this.windowListeners[t];s&&!s.registered&&this.addWindowListener(s),o&&this.sendRetainedArgumentsForEvent(t);const n=async()=>this.removeListener(t,a);return Promise.resolve({remove:n})}async removeAllListeners(){this.listeners={};for(const t in this.windowListeners)this.removeWindowListener(this.windowListeners[t]);this.windowListeners={}}notifyListeners(t,a,o){const r=this.listeners[t];if(!r){if(o){let s=this.retainedEventArguments[t];s||(s=[]),s.push(a),this.retainedEventArguments[t]=s}return}r.forEach(s=>s(a))}hasListeners(t){var a;return!!(!((a=this.listeners[t])===null||a===void 0)&&a.length)}registerWindowListener(t,a){this.windowListeners[a]={registered:!1,windowEventName:t,pluginEventName:a,handler:o=>{this.notifyListeners(a,o)}}}unimplemented(t="not implemented"){return new Ge.Exception(t,Be.Unimplemented)}unavailable(t="not available"){return new Ge.Exception(t,Be.Unavailable)}async removeListener(t,a){const o=this.listeners[t];if(!o)return;const r=o.indexOf(a);this.listeners[t].splice(r,1),this.listeners[t].length||this.removeWindowListener(this.windowListeners[t])}addWindowListener(t){window.addEventListener(t.windowEventName,t.handler),t.registered=!0}removeWindowListener(t){t&&(window.removeEventListener(t.windowEventName,t.handler),t.registered=!1)}sendRetainedArgumentsForEvent(t){const a=this.retainedEventArguments[t];a&&(delete this.retainedEventArguments[t],a.forEach(o=>{this.notifyListeners(t,o)}))}}const Ta=e=>encodeURIComponent(e).replace(/%(2[346B]|5E|60|7C)/g,decodeURIComponent).replace(/[()]/g,escape),Ma=e=>e.replace(/(%[\dA-F]{2})+/gi,decodeURIComponent);class ws extends go{async getCookies(){const t=document.cookie,a={};return t.split(";").forEach(o=>{if(o.length<=0)return;let[r,s]=o.replace(/=/,"CAP_COOKIE").split("CAP_COOKIE");r=Ma(r).trim(),s=Ma(s).trim(),a[r]=s}),a}async setCookie(t){try{const a=Ta(t.key),o=Ta(t.value),r=`; expires=${(t.expires||"").replace("expires=","")}`,s=(t.path||"/").replace("path=",""),n=t.url!=null&&t.url.length>0?`domain=${t.url}`:"";document.cookie=`${a}=${o||""}${r}; path=${s}; ${n};`}catch(a){return Promise.reject(a)}}async deleteCookie(t){try{document.cookie=`${t.key}=; Max-Age=0`}catch(a){return Promise.reject(a)}}async clearCookies(){try{const t=document.cookie.split(";")||[];for(const a of t)document.cookie=a.replace(/^ +/,"").replace(/=.*/,`=;expires=${new Date().toUTCString()};path=/`)}catch(t){return Promise.reject(t)}}async clearAllCookies(){try{await this.clearCookies()}catch(t){return Promise.reject(t)}}}pa("CapacitorCookies",{web:()=>new ws});const ks=async e=>new Promise((t,a)=>{const o=new FileReader;o.onload=()=>{const r=o.result;t(r.indexOf(",")>=0?r.split(",")[1]:r)},o.onerror=r=>a(r),o.readAsDataURL(e)}),Es=(e={})=>{const t=Object.keys(e);return Object.keys(e).map(r=>r.toLocaleLowerCase()).reduce((r,s,n)=>(r[s]=e[t[n]],r),{})},Ss=(e,t=!0)=>e?Object.entries(e).reduce((o,r)=>{const[s,n]=r;let i,d;return Array.isArray(n)?(d="",n.forEach(l=>{i=t?encodeURIComponent(l):l,d+=`${s}=${i}&`}),d.slice(0,-1)):(i=t?encodeURIComponent(n):n,d=`${s}=${i}`),`${o}&${d}`},"").substr(1):null,$s=(e,t={})=>{const a=Object.assign({method:e.method||"GET",headers:e.headers},t),r=Es(e.headers)["content-type"]||"";if(typeof e.data=="string")a.body=e.data;else if(r.includes("application/x-www-form-urlencoded")){const s=new URLSearchParams;for(const[n,i]of Object.entries(e.data||{}))s.set(n,i);a.body=s.toString()}else if(r.includes("multipart/form-data")||e.data instanceof FormData){const s=new FormData;if(e.data instanceof FormData)e.data.forEach((i,d)=>{s.append(d,i)});else for(const i of Object.keys(e.data))s.append(i,e.data[i]);a.body=s;const n=new Headers(a.headers);n.delete("content-type"),a.headers=n}else(r.includes("application/json")||typeof e.data=="object")&&(a.body=JSON.stringify(e.data));return a};class Is extends go{async request(t){const a=$s(t,t.webFetchExtra),o=Ss(t.params,t.shouldEncodeUrlParams),r=o?`${t.url}?${o}`:t.url,s=await fetch(r,a),n=s.headers.get("content-type")||"";let{responseType:i="text"}=s.ok?t:{};n.includes("application/json")&&(i="json");let d,l;switch(i){case"arraybuffer":case"blob":l=await s.blob(),d=await ks(l);break;case"json":d=await s.json();break;case"document":case"text":default:d=await s.text()}const c={};return s.headers.forEach((u,m)=>{c[m]=u}),{data:d,headers:c,status:s.status,url:s.url}}async get(t){return this.request(Object.assign(Object.assign({},t),{method:"GET"}))}async post(t){return this.request(Object.assign(Object.assign({},t),{method:"POST"}))}async put(t){return this.request(Object.assign(Object.assign({},t),{method:"PUT"}))}async patch(t){return this.request(Object.assign(Object.assign({},t),{method:"PATCH"}))}async delete(t){return this.request(Object.assign(Object.assign({},t),{method:"DELETE"}))}}pa("CapacitorHttp",{web:()=>new Is});const se=pa("PushNotifications",{}),Cs=(e,t,a)=>k(`/api/analytics/${e}?startDate=${t}&endDate=${a}`),Ls=({establishmentId:e,startDate:t,endDate:a,cashierSessionId:o})=>{let r=`/api/reports/sales/${e}?startDate=${t}&endDate=${a}`;return o&&o!=="all"&&(r+=`&cashierSessionId=${o}`),k(r)},Bs=(e,t,a)=>k(`/api/analytics/${e}/monthly-details?year=${t}&month=${a}`),Ts=(e,t,a,o)=>{const r=`/api/analytics/${e}/daily-details?year=${t}&month=${a}&day=${o}`;return k(r)},Ms=(e,t,a,o)=>{const r=`/api/analytics/${e}/professional-details?year=${t}&month=${a}&professionalId=${o}`;return k(r)},Ds=()=>k("/api/reports/summary",{method:"GET"}),Pe=e=>k(`/api/services/${e}`),Ps=e=>k("/api/services",{method:"POST",body:JSON.stringify(e)}),As=(e,t)=>k(`/api/services/${e}`,{method:"PUT",body:JSON.stringify(t)}),qs=e=>k(`/api/services/${e}`,{method:"DELETE"}),Ns=(e,t)=>k(`/api/services/${e}/status`,{method:"PATCH",body:JSON.stringify({active:t})}),Fs=e=>k(`/api/services/stats/most-popular/${e}`),Bt=(e,t,a,o="all")=>{const r=`/api/blockages/${e}?startDate=${t}&endDate=${a}&professionalId=${o}`;return k(r)},Tt=e=>k("/api/blockages",{method:"POST",body:JSON.stringify(e)}),ga=e=>k(`/api/blockages/${e}`,{method:"DELETE"}),bo=e=>k("/api/blockages/batch-delete",{method:"POST",body:JSON.stringify({ids:e})}),tt=e=>k(`/api/clients/${e}`),ba=e=>k("/api/clients",{method:"POST",body:JSON.stringify(e)}),Rs=(e,t)=>k(`/api/clients/${e}`,{method:"PUT",body:JSON.stringify(t)}),Hs=e=>k(`/api/clients/${e}`,{method:"DELETE"}),js=(e,t,a)=>{const o=`/api/clients/history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return k(o)},Os=(e,t,a)=>{const o=`/api/clients/loyalty-history/${e}?clientName=${encodeURIComponent(t)}&clientPhone=${encodeURIComponent(a)}`;return k(o)},zs=(e,t,a,o)=>k("/api/clients/redeem",{method:"POST",body:JSON.stringify({establishmentId:e,clientName:t,clientPhone:a,rewardData:o})}),Da=document.getElementById("content");let Pa=!1;const Xt=[{bg:"#e0e7ff",border:"#4f46e5",main:"#4f46e5"},{bg:"#d1fae5",border:"#059669",main:"#059669"},{bg:"#ffe4e6",border:"#e11d48",main:"#e11d48"},{bg:"#fef3c7",border:"#d97706",main:"#d97706"},{bg:"#cffafe",border:"#0e7490",main:"#0e7490"},{bg:"#e0f2fe",border:"#0284c7",main:"#0284c7"},{bg:"#ede9fe",border:"#7c3aed",main:"#7c3aed"},{bg:"#fce7f3",border:"#db2777",main:"#db2777"}];let at=[],Mt=[],Ye={},te=[],C={currentView:"list",weekViewDays:7,currentDate:new Date,selectedProfessionalId:"all",profSearchTerm:"",showInactiveProfs:!1,scrollToAppointmentId:null},E={step:1,data:{id:null,clientName:"",clientPhone:"",selectedServiceIds:[],professionalId:null,professionalName:"",date:null,time:null,redeemedReward:null,clientHasRewards:!1,clientLoyaltyPoints:0}};function Vs(e){return new Intl.DateTimeFormat("pt-BR",{weekday:"short",day:"2-digit",month:"short"}).format(e).replace(/\./g,"")}function fo(e){const t=new Date(e);if(t.setHours(0,0,0,0),C.currentView==="week"&&C.weekViewDays===7){const a=t.getDay(),o=t.getDate()-a+(a===0?-6:1);return new Date(t.setDate(o))}return t}function Et(){const e=document.getElementById("profSelectorContainer"),t=C.profSearchTerm.toLowerCase();if(!e||!p.professionals)return;let a=p.professionals.filter(s=>C.showInactiveProfs||s.status!=="inactive");t&&(a=a.filter(s=>s.name.toLowerCase().includes(t)));const r=[...[{id:"all",name:"Todos",photo:null,status:"active"}],...a];e.innerHTML=r.map(s=>{const n=C.selectedProfessionalId===s.id,i=s.name==="Todos"?"Todos":s.name.split(" ")[0],d=s.name==="Todos"?"T":s.name.charAt(0).toUpperCase(),l=s.status!=="inactive",c=Xt[0],u=s.id!=="all"&&p.professionalColors.get(s.id)||c,m=s.photo||`https://placehold.co/64x64/${u.main?.replace("#","")||"E0E7FF"}/${u.light?.replace("#","")||"4F46E5"}?text=${d}`,b=s.id==="all"?"#e0e7ff":u.light,f=s.id==="all"?"#4f46e5":u.main,x=`border: 3px solid ${n?u.border:"transparent"}; box-shadow: ${n?"0 0 0 2px "+u.border:"none"};`;return`
        ...
        `}).join("")}function Us(e,t,a,o,r){const s=(e||"").replace(/\D/g,""),n=new Date(r).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}),i=new Date(r).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=`Olá, ${t}! Você tem um agendamento de ${a} com o(a) profissional ${o} para o dia ${n} às ${i}. Podemos confirmar? Agradecemos a preferência!`,l=encodeURIComponent(d);return`https://wa.me/${s}?text=${l}`}function Js(e){const t=document.getElementById("agenda-view");if(e.sort((o,r)=>new Date(o.startTime)-new Date(r.startTime)),e.length===0){t.innerHTML='<div class="text-center p-10 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum agendamento ou bloqueio</h3><p class="mt-1 text-sm text-gray-500">Não há eventos para o dia e filtros selecionados.</p></div>';return}const a=e.map(o=>{const r=new Date(o.startTime),s=new Date(o.endTime),n=r.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),i=s.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),d=p.professionalColors.get(o.professionalId)||{};if(o.type==="blockage")return`
                ...
                </div>`;const l=o.status==="completed",c=l?"bg-green-100 text-green-800":"bg-yellow-100 text-yellow-800",u=l?"Finalizado":"Aberto",m=JSON.stringify(o).replace(/'/g,"&apos;"),b=o.redeemedReward?.points>0,f=o.hasRewards&&!b,v=Us(o.clientPhone,o.clientName,o.serviceName,o.professionalName,o.startTime);return`
            <div class="appointment-list-card" data-appointment='${m}' style="border-left-color: ${d.border};">
                ...
                <div class="status-info">
                    ...
                    `;const x=JSON.stringify(m).replace(/'/g,"&apos;"),h=m.redeemedReward?.points>0,y=m.hasRewards&&!h,S=m.status==="completed";return`
        ...
        `}s+="</div>",t.innerHTML=s}function Ws(){const e=p.allEvents.filter(t=>C.selectedProfessionalId==="all"||t.professionalId===C.selectedProfessionalId);C.currentView==="list"?Js(e):_s(e)}async function X(){const e=document.getElementById("agenda-view");if(!e)return;e.innerHTML='<div class="loader mx-auto my-10"></div>';let t,a;const o=document.getElementById("weekRange");if(C.currentView==="list")t=new Date(C.currentDate),t.setHours(0,0,0,0),a=new Date(C.currentDate),a.setHours(23,59,59,999),o.textContent=Vs(t);else{const r=fa();t=fo(new Date(C.currentDate)),a=new Date(t),a.setDate(t.getDate()+(r-1)),a.setHours(23,59,59,999),o.textContent=`${t.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} - ${a.toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}`}try{const r=await Qo(p.establishmentId,t.toISOString(),a.toISOString(),C.selectedProfessionalId==="all"?null:C.selectedProfessionalId),n=(await Bt(p.establishmentId,t.toISOString(),a.toISOString(),C.selectedProfessionalId)).map(d=>{let l=d.professionalName;if(!l&&d.professionalId){const c=p.professionals?p.professionals.find(u=>u.id===d.professionalId):null;c&&(l=c.name)}return{...d,type:"blockage",professionalName:l||"Não identificado"}}),i=[...r.map(d=>({...d,type:"appointment"})),...n];if(p.allEvents=i,Et(),Ws(),C.scrollToAppointmentId){const d=document.querySelector(`[data-appointment*='"id":"${C.scrollToAppointmentId}"']`);d&&(d.scrollIntoView({behavior:"smooth",block:"center"}),d.style.transition="background-color 0.5s ease-in-out",d.style.backgroundColor="#e0e7ff",setTimeout(()=>{d.style.backgroundColor=""},2500)),C.scrollToAppointmentId=null}}catch(r){g("Erro na Agenda",`Não foi possível carregar a agenda: ${r.message}`,"error"),e.innerHTML='<div class="p-6 text-center text-red-600">Falha ao carregar dados.</div>'}}async function Gs(){try{const[e,t,a,o]=await Promise.all([p.professionals&&p.professionals.length>0?Promise.resolve(p.professionals):G(p.establishmentId),p.services&&p.services.length>0?Promise.resolve(p.services):Pe(p.establishmentId),te.length>0?Promise.resolve(te):tt(p.establishmentId),Ye.enabled!==void 0?Promise.resolve(null):et(p.establishmentId)]);(!p.professionals||p.professionals.length===0)&&(p.professionals=e||[]),(!p.services||p.services.length===0)&&(p.services=t||[]),te.length===0&&(te=a||[]),o&&(Ye=o.loyaltyProgram||{enabled:!1}),p.professionals.forEach((r,s)=>{p.professionalColors.set(r.id,Xt[s%Xt.length])}),Et()}catch(e){console.error("Erro ao popular filtros e dependências do modal:",e),g("Atenção","Não foi possível pré-carregar os dados para agendamento. A abertura do modal pode ser lenta.","error")}}function Zt(e){e<1||e>4||(E.step=e,Kt(null,!0))}function vo(e,t){const a=document.getElementById("multiServiceToggle"),o=a&&a.checked,r=t.classList.contains("selected"),s=E.data.selectedServiceIds.indexOf(e);if(r)t.classList.remove("selected","border-blue-500"),s>-1&&E.data.selectedServiceIds.splice(s,1);else{if(!o){E.data.selectedServiceIds=[];const n=document.getElementById("apptServicesContainer");n&&n.querySelectorAll(".service-card.selected").forEach(i=>{i.classList.remove("selected","border-blue-500")})}t.classList.add("selected","border-blue-500"),E.data.selectedServiceIds.push(e)}}function ho(e,t){const a=document.querySelector(".professional-step-cards");if(!a)return;a.querySelectorAll(".professional-modal-card").forEach(r=>r.classList.remove("selected","border-blue-500")),t.classList.add("selected","border-blue-500");const o=Mt.find(r=>r.id===e);E.data.professionalId=e,E.data.professionalName=o?o.name:"N/A"}function Ys(e,t){const a=document.getElementById("availableTimesContainer");a&&(a.querySelectorAll(".time-slot-card").forEach(o=>o.classList.remove("selected")),t.classList.add("selected"),E.data.time=e)}async function Aa(){const e=document.getElementById("apptTotalDuration"),t=document.getElementById("availableTimesContainer");if(!e||!t)return;const a=E.data.professionalId,o=E.data.selectedServiceIds,r=document.getElementById("apptDate").value;E.data.date=r;const s=o.reduce((n,i)=>{const d=at.find(l=>l.id===i);return n+(d?d.duration+(d.bufferTime||0):0)},0);if(e.textContent=`${s} min`,s===0||!a||!r){t.innerHTML='<p class="col-span-full text-center text-gray-500">Selecione serviço, profissional e data.</p>';return}t.innerHTML='<div class="loader mx-auto col-span-full"></div>';try{let n=await Zo({establishmentId:p.establishmentId,professionalId:a,serviceIds:o,date:r});const i=new Date;if(new Date(r+"T00:00:00").toDateString()===i.toDateString()){const l=i.getHours()*60+i.getMinutes();n=n.filter(c=>{const[u,m]=c.split(":").map(Number);return u*60+m>=l})}if(t.innerHTML="",n.length>0){if(n.forEach(l=>{const c=document.createElement("button");c.type="button",c.className=`time-slot-card p-2 text-sm bg-gray-100 rounded-md hover:bg-gray-200 transition ${E.data.time===l?"selected":""}`,c.textContent=l,c.addEventListener("click",()=>Ys(l,c)),t.appendChild(c)}),E.data.time){const l=t.querySelector(`[data-action="time-slot"][data-time="${E.data.time}"]`);l&&l.classList.add("selected")}}else t.innerHTML='<p class="col-span-full text-center text-gray-500">Nenhum horário disponível.</p>'}catch(n){console.error("Erro ao buscar horários:",n),t.innerHTML='<p class="col-span-full text-center text-red-500">Erro ao buscar horários.</p>'}}function Qs(){const e=document.getElementById("loyaltyRewardsContainer");if(!e)return;const{clientHasRewards:t,clientLoyaltyPoints:a,redeemedReward:o}=E.data,{enabled:r,rewards:s,pointsPerCurrency:n}=Ye;if(!r||!t||!s||s.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden");const i=s.filter(l=>a>=l.points);let d=`
    ...
    `),e.querySelector('input[value="none"]').addEventListener("change",l=>{l.target.checked&&(E.data.redeemedReward=null)})}async function Xs(e){e.preventDefault();const t=e.target,a=t.querySelector('button[type="submit"]');if(!E.data.time||E.data.selectedServiceIds.length===0||!E.data.professionalId)return g("Erro de Validação","Por favor, selecione o horário, serviço(s) e profissional antes de confirmar.","error");a.disabled=!0,a.textContent="A confirmar...";const o=E.data.selectedServiceIds.map(l=>{const c=at.find(u=>u.id===l);return{id:c.id,name:c.name,price:c.price,duration:c.duration,bufferTime:c.bufferTime||0,photo:c.photo||null}}),[r,s]=E.data.time.split(":"),n=new Date(`${E.data.date}T${r}:${s}:00`),i={establishmentId:p.establishmentId,clientName:E.data.clientName,clientPhone:E.data.clientPhone,services:o,professionalId:E.data.professionalId,startTime:n.toISOString(),redeemedReward:E.data.redeemedReward},d=t.querySelector("#appointmentId").value;d&&(i.id=d);try{d?await ma(d,i):await Ko(i),g(`Agendamento ${d?"atualizado":"criado"} com sucesso!`,"success"),document.getElementById("appointmentModal").style.display="none",X()}catch(l){g(l.message,"error")}finally{a.disabled=!1,a.textContent="Confirmar Agendamento"}}function Zs(e){return`
    ...
    `}async function qa(e){const t=document.getElementById("clientSearchResults");if(!t)return;const a=e.toLowerCase().trim();if(a.length<3){t.innerHTML='<p class="text-sm text-gray-500">Digite pelo menos 3 caracteres para buscar clientes existentes.</p>';return}const o=te.filter(r=>r.name.toLowerCase().includes(a)||r.phone.includes(a));if(o.length===0){t.innerHTML='<p class="text-sm text-gray-500">Nenhum cliente encontrado com este termo.</p>';return}t.innerHTML=o.map(Zs).join(""),t.querySelectorAll('[data-action="select-client"]').forEach(r=>{r.addEventListener("click",s=>{const n=r.dataset.clientName,i=r.dataset.clientPhone,d=te.find(l=>l.phone===i&&l.name===n);if(E.data.clientName=n,E.data.clientPhone=i,d){const l=Ye,c=Math.min(...(l?.rewards||[]).map(u=>u.points));E.data.clientLoyaltyPoints=d.loyaltyPoints||0,E.data.clientHasRewards=l.enabled&&c!==1/0&&E.data.clientLoyaltyPoints>=c}else E.data.clientHasRewards=!1,E.data.clientLoyaltyPoints=0;document.getElementById("apptClientName").value=n,document.getElementById("apptClientPhone").value=i,document.querySelectorAll(".client-search-card").forEach(l=>l.classList.remove("selected","border-blue-500")),r.classList.add("selected","border-blue-500")})})}async function Ks(e){e.preventDefault();const t=document.getElementById("clientRegistrationForm"),a=t.querySelector('button[type="submit"]'),o={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim(),phone:t.querySelector("#regClientPhone").value.trim(),dobDay:t.querySelector("#regClientDobDay").value.trim(),dobMonth:t.querySelector("#regClientDobMonth").value.trim(),notes:t.querySelector("#regClientNotes").value.trim()};if(!o.name||!o.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{await ba(o),te.push({name:o.name,phone:o.phone,loyaltyPoints:0}),E.data.clientName=o.name,E.data.clientPhone=o.phone,E.data.clientHasRewards=!1,E.data.clientLoyaltyPoints=0,g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",Zt(1)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar"}}function er(){N({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("clientRegistrationForm");t&&t.addEventListener("submit",Ks)}function tr(){er()}function ar(e,t){const a=e?"Editar Agendamento":"Selecionar Cliente",o=`
    ...
    `;return{title:a,content:o}}function or(){const e="Selecionar Serviço",a=`
        <div class="p-5 space-y-6">
            ...
            <div id="apptServicesContainer" class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto p-1">
                 ${at.map(o=>{const r=E.data.selectedServiceIds.includes(o.id),s=o.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
    ...
    `;return{title:e,content:a}}function sr(){const e="Selecionar Profissional",t=`
        <div class="p-5 space-y-6">
             ...
             <div id="apptProfessionalContainer" class="mt-4 flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 professional-step-cards">
                 ${Mt.map(a=>{const o=E.data.professionalId===a.id,r=a.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
    ...
    `;return{title:e,content:t}}function rr(e){const t=e?"Confirmar Edição":"Data e Horário",a=new Date,o=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,r=E.data.date||o,s=`
        <div class="p-5 space-y-6">
    ...
    `;return{title:t,content:s}}function nr(e){const t=document.getElementById("apptServicesContainer");if(!t)return;const a=e.toLowerCase(),o=at.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=E.data.selectedServiceIds.includes(r.id),n=r.photo||"https://placehold.co/40x40/E0E7FF/4F46E5?text=S";return`
            <div class="service-card p-3 bg-white rounded-lg border-2 border-gray-200 cursor-pointer transition-all hover:bg-gray-50 ${s?"selected border-blue-500":""}" data-service-id="${r.id}">
            ...
            </div>`}).join(""),t.querySelectorAll(".service-card").forEach(r=>{r.addEventListener("click",()=>vo(r.dataset.serviceId,r))})}function ir(e){const t=document.getElementById("apptProfessionalContainer");if(!t)return;const a=e.toLowerCase(),o=Mt.filter(r=>r.name.toLowerCase().includes(a));t.innerHTML=o.map(r=>{const s=E.data.professionalId===r.id,n=r.photo||"https://placehold.co/60x60/E0E7FF/4F46E5?text=P";return`
             ...
             </div>`}).join(""),t.querySelectorAll(".professional-modal-card").forEach(r=>{r.addEventListener("click",()=>ho(r.dataset.professionalId,r))})}async function Kt(e=null,t=!1){const a=document.getElementById("appointmentModal");if(!t){const s=e?.startTime?new Date(e.startTime).toISOString().split("T")[0]:new Date().toISOString().split("T")[0],n=e?.startTime?new Date(e.startTime).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit",hour12:!1}):null;E={step:1,data:{id:e?.id||null,clientName:e?.clientName||"",clientPhone:e?.clientPhone||"",selectedServiceIds:e?.services?.map(i=>i.id)||[],professionalId:e?.professionalId||null,professionalName:e?.professionalName||"",date:s,time:n,redeemedReward:e?.redeemedReward||null,clientHasRewards:e?.hasRewards||!1,clientLoyaltyPoints:0}}}if(!p.services||!p.professionals||!te||Ye.enabled===void 0){g("Erro","Os dados da agenda ainda não foram carregados. Tente novamente em alguns segundos.","error");return}if(at=p.services,Mt=p.professionals.filter(s=>s.status==="active"),E.data.clientName&&E.data.clientPhone){const s=te.find(n=>n.phone===E.data.clientPhone&&n.name===E.data.clientName);s&&(E.data.clientLoyaltyPoints=s.loyaltyPoints||0)}let o={title:"Erro",content:"<p>Etapa não encontrada.</p>"};switch(E.step){case 1:o=ar(e);break;case 2:o=or();break;case 3:o=sr();break;case 4:o=rr(e);break}a.innerHTML=`
        ...
        </div>`,a.querySelectorAll('[data-action="next-step"]').forEach(s=>{s.addEventListener("click",()=>{const n=parseInt(s.dataset.currentStep,10);if(n===1){const i=a.querySelector("#apptClientName"),d=a.querySelector("#apptClientPhone");if(E.data.clientName=i.value.trim(),E.data.clientPhone=d.value.trim(),!E.data.clientName||!E.data.clientPhone)return g("Atenção","Nome e telefone do cliente são obrigatórios.","error")}else if(n===2){if(E.data.selectedServiceIds.length===0)return g("Atenção","Selecione pelo menos um serviço.","error")}else if(n===3&&!E.data.professionalId)return g("Atenção","Selecione um profissional.","error");Zt(n+1)})}),a.querySelectorAll('[data-action="prev-step"]').forEach(s=>{s.addEventListener("click",()=>Zt(parseInt(s.dataset.currentStep,10)-1))});const r=a.querySelector("#appointmentForm");if(E.step===4&&r&&r.addEventListener("submit",Xs),a.style.display="flex",E.step===2){a.querySelector("#apptServicesContainer").querySelectorAll(".service-card").forEach(i=>{i.addEventListener("click",()=>vo(i.dataset.serviceId,i))});const n=a.querySelector("#serviceSearchModalInput");n&&n.addEventListener("input",i=>nr(i.target.value))}if(E.step===3){a.querySelector("#apptProfessionalContainer").querySelectorAll(".professional-modal-card").forEach(i=>{i.addEventListener("click",()=>ho(i.dataset.professionalId,i))});const n=a.querySelector("#professionalSearchModalInput");n&&n.addEventListener("input",i=>ir(i.target.value))}if(E.step===1){const s=a.querySelector("#clientSearchInput");s&&(s.addEventListener("input",i=>qa(i.target.value)),E.data.clientName&&E.data.clientPhone&&qa(`${E.data.clientName} ${E.data.clientPhone}`));const n=a.querySelector('[data-action="open-client-registration"]');n&&n.addEventListener("click",tr)}if(E.step===4){const s=a.querySelector("#apptDate");s&&s.addEventListener("change",Aa),Aa(),Qs()}}async function yo(e={}){C.currentDate=e.targetDate?new Date(e.targetDate):C.currentDate||new Date,C.scrollToAppointmentId=e.scrollToAppointmentId||null,C.profSearchTerm="",window.innerWidth<768&&(C.currentView="list"),Da.innerHTML=`
        <section>
        ...
        </section>`,document.querySelectorAll(".view-btn[data-view]").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".view-btn[data-view]").forEach(r=>r.classList.remove("active")),a.classList.add("active"),C.currentView=a.dataset.view;const o=document.getElementById("week-days-toggle");if(C.currentView==="week"){if(o.style.display="flex",window.innerWidth<768){C.weekViewDays=3,document.querySelectorAll(".week-days-btn").forEach(s=>s.classList.remove("active"));const r=document.querySelector('.week-days-btn[data-days="3"]');r&&r.classList.add("active")}}else o.style.display="none";X()})}),document.querySelectorAll(".week-days-btn").forEach(a=>{a.addEventListener("click",()=>{document.querySelectorAll(".week-days-btn").forEach(o=>o.classList.remove("active")),a.classList.add("active"),C.weekViewDays=parseInt(a.dataset.days,10),X()})}),document.getElementById("todayBtn").addEventListener("click",()=>{C.currentDate=new Date,X()});const t=a=>{const o=parseInt(a.currentTarget.dataset.amount,10),r=C.currentView==="week"?fa():1,s=new Date(C.currentDate);s.setDate(s.getDate()+o*r),C.currentDate=s,X()};document.getElementById("prevBtn").addEventListener("click",t),document.getElementById("nextBtn").addEventListener("click",t),document.getElementById("profSearchInput").addEventListener("input",a=>{C.profSearchTerm=a.target.value,Et()}),document.getElementById("showInactiveProfsToggle").addEventListener("change",a=>{C.showInactiveProfs=a.target.checked,Et(),X()}),Pa||(Da.addEventListener("click",async a=>{const o=a.target.closest("[data-action]");if(a.target.closest('[data-action="select-professional"]')){const d=a.target.closest('[data-action="select-professional"]').dataset.profId,l=C.selectedProfessionalId===d&&d!=="all";if(C.selectedProfessionalId=l?"all":d,d!=="all"){const c=document.getElementById("profSearchInput");c&&(c.value=""),C.profSearchTerm=""}await X();return}if(!o)return;const r=o.dataset.action;let s=null;const n=a.target.closest("[data-appointment]");switch(n&&(s=JSON.parse(n.dataset.appointment.replace(/&apos;/g,"'"))),r){case"new-appointment":Kt();break;case"edit-appointment":if(!s)return;if(s.status==="completed"){g("Atenção","Agendamentos finalizados não podem ser editados.","error");return}s.hasRewards&&!s.redeemedReward&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos para resgatar. Verifique a Etapa 4 do agendamento.","info"),Kt(s);break;case"delete-appointment":{const i=o.dataset.id;if(await D("Confirmar Exclusão","Tem a certeza que deseja apagar este agendamento?"))try{await es(i),g("Agendamento apagado!","success"),X()}catch(l){g(`Não foi possível apagar: ${l.message}`,"error")}break}case"open-comanda":if(s){s.hasRewards&&!s.redeemedReward&&s.status!=="completed"&&g("🎁 Cliente com Prêmios!","Este cliente tem pontos de fidelidade para resgatar.","info");const i=s.status==="completed"?"finalizadas":"em-atendimento",d={selectedAppointmentId:s.id,initialFilter:i};if(i==="finalizadas"){let l=s.startTime;if(s.transaction&&s.transaction.paidAt){const c=s.transaction.paidAt;typeof c=="object"&&c._seconds?l=new Date(c._seconds*1e3):l=c}d.filterDate=l}H("comandas-section",d)}break}}),Pa=!0),await Gs(),await X()}const lr=(e,t=null,a=1,o=12)=>{let r=`/api/comandas/${e}?page=${a}&limit=${o}`;return t&&(r+=`&date=${t}`),k(r)},dr=e=>k("/api/sales",{method:"POST",body:JSON.stringify(e)}),cr=e=>k(`/api/sales/${e}/reopen`,{method:"POST"}),ur=e=>k(`/api/sales/${e}`,{method:"DELETE"}),Dt=e=>k(`/api/products/${e}`),mr=e=>k("/api/products",{method:"POST",body:JSON.stringify(e)}),pr=(e,t)=>k(`/api/products/${e}`,{method:"PUT",body:JSON.stringify(t)}),gr=e=>k(`/api/products/${e}`,{method:"DELETE"}),br=(e,t)=>k(`/api/products/${e}/stock`,{method:"PATCH",body:JSON.stringify(t)}),fr=({startDate:e,endDate:t,productId:a,categoryId:o})=>{const r=new URLSearchParams({startDate:e,endDate:t});return a&&a!=="all"&&r.append("productId",a),o&&o!=="all"&&r.append("categoryId",o),k(`/api/products/stock-history/report?${r.toString()}`)},xo=()=>k("/api/cashier/status").catch(e=>{if(e.message.includes("404")||e.message.includes("não encontrada"))return null;throw e}),vr=e=>{const t={establishmentId:e.establishmentId,initialAmount:Number(e.initialAmount),notes:e.notes||""};return console.log("Payload enviado para abrir caixa:",t),k("/api/cashier/open",{method:"POST",body:JSON.stringify(t)})},hr=(e,t)=>{const a={finalAmount:Number(t)};return console.log("Payload enviado para fechar caixa:",a),k(`/api/cashier/close/${e}`,{method:"PUT",body:JSON.stringify(a)})},yr=()=>k("/api/cashier/history").then(e=>e||[]).catch(e=>(console.error("Erro ao buscar histórico:",e),[])),xr=e=>k(`/api/cashier/report/${e}`),wo=e=>k(`/api/packages/${e}`),wr=e=>k("/api/packages",{method:"POST",body:JSON.stringify(e)}),kr=(e,t)=>k(`/api/packages/${e}`,{method:"PUT",body:JSON.stringify(t)}),Er=e=>k(`/api/packages/${e}`,{method:"DELETE"});let w={allComandas:[],catalog:{services:[],products:[],packages:[]},clients:[],activeFilter:"atendimento",selectedComandaId:null,isCashierOpen:!1,activeCashierSessionId:null,paging:{page:1,limit:12,total:0}},ye=null,xe=null;function Sr(){const e=document.getElementById("comandas-layout");if(e){e.classList.add("detail-view-active");const t=document.getElementById("comanda-detail-container");t&&(t.scrollTop=0)}}function ge(){const e=document.getElementById("comandas-layout");e&&e.classList.remove("detail-view-active")}function $r(){const e=new Date().toISOString().split("T")[0];xe.innerHTML=`
    ...
    `}function Ir(){const e=document.getElementById("cashier-controls");e&&(w.isCashierOpen?e.innerHTML=`
    ...
    `}function Cr(){N({title:"Cadastrar Novo Cliente",contentHTML:`
    ...
    `,maxWidth:"max-w-2xl"});const t=document.getElementById("comandas_clientRegistrationForm");t&&t.addEventListener("submit",Lr)}async function Lr(e){e.preventDefault();const t=document.getElementById("comandas_clientRegistrationForm");if(!t)return;const a=t.querySelector('button[type="submit"]'),o={establishmentId:p.establishmentId,name:t.querySelector("#regClientName").value.trim(),email:t.querySelector("#regClientEmail").value.trim()||null,phone:t.querySelector("#regClientPhone").value.trim(),dob:`${t.querySelector("#regClientDobDay").value.trim()}/${t.querySelector("#regClientDobMonth").value.trim()}`,notes:t.querySelector("#regClientNotes").value.trim()||null};if(!o.name||!o.phone)return g("Erro de Validação","Nome e Telefone são obrigatórios.","error");a.disabled=!0,a.textContent="A salvar...";try{const r=await ba(o);w.clients.push({id:r.id,...o}),g("Cliente cadastrado com sucesso!","success"),document.getElementById("genericModal").style.display="none",ko(r.id)}catch(r){g(`Erro ao cadastrar cliente: ${r.message}`,"error")}finally{a.disabled=!1,a.textContent="Salvar Cliente"}}function Br(){if(!w.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de adicionar itens.","error");return}const{modalElement:e,close:t}=N({title:"Adicionar Item à Comanda",contentHTML:'<div id="add-item-content"></div>',maxWidth:"max-w-4xl"}),a=()=>{let r="";const s=e.querySelector("#add-item-content"),n={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>',package:'<svg class="w-6 h-6 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14L4 7m8 4v10M4 7v10l8 4" /></svg>'},i=()=>{const d=r.toLowerCase(),l=u=>u.filter(m=>m.name.toLowerCase().includes(d)),c={"modal-service-list":{items:l(w.catalog.services),type:"service"},"modal-product-list":{items:l(w.catalog.products),type:"product"},"modal-package-list":{items:l(w.catalog.packages),type:"package"}};for(const[u,{items:m,type:b}]of Object.entries(c)){const f=s.querySelector(`#${u}`);f&&(f.innerHTML=m.map(v=>`
        ...
        `,document.getElementById("quantity-minus-btn").onclick=()=>{s>1&&(s--,i())},document.getElementById("quantity-plus-btn").onclick=()=>{s++,i()},document.querySelector('[data-action="confirm-add-item"]').onclick=async()=>{await Ar(r,s),t()}};e.addEventListener("click",r=>{const s=r.target.closest('[data-action="select-item-for-quantity"]'),n=r.target.closest('[data-action="back-to-catalog"]');if(s){const{itemType:i,itemId:d}=s.dataset,c=(w.catalog[i+"s"]||[]).find(u=>u.id===d);c&&o({...c,type:i})}else n&&a()}),a()}async function ko(e=null){if(!w.isCashierOpen){g("Caixa Fechado","Abra o caixa antes de criar uma nova venda.","error");return}if(!w.clients||w.clients.length===0)try{w.clients=await tt(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de clientes.","error");return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await G(p.establishmentId)}catch{g("Erro","Não foi possível carregar dados de profissionais.","error");return}const t=w.clients.map(n=>{const i=n.id===e?"selected":"";return`<option value="${n.id}" ${i}>${n.name} - ${n.phone}</option>`}).join(""),a=p.professionals.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""),o=`
    ...
    `,{modalElement:d}=N({title:"Finalizar Pagamento",contentHTML:i,maxWidth:"max-w-md"});document.getElementById("payment-value").value=r.remainingAmount.toFixed(2),d.addEventListener("click",l=>{const c=l.target.closest(".payment-method-btn");c&&(r.selectedMethod=c.dataset.method,r.installments=1,document.getElementById("installments-select").value=1,s()),l.target.closest("#add-payment-btn")&&n(),l.target.closest('[data-action="remove-payment"]')&&(o.splice(parseInt(l.target.closest('[data-action="remove-payment"]').dataset.paymentIndex,10),1),s()),l.target.closest("#finalize-checkout-btn")&&Nr(e,a,o)}),d.addEventListener("change",l=>{l.target.id==="installments-select"&&(r.installments=parseInt(l.target.value,10))}),d.addEventListener("input",l=>{l.target.id==="amount-received"&&(r.amountReceived=l.target.value,s())}),s()}async function Mr(){const e=`
    ...
    `,{modalElement:t}=N({title:"Abrir Caixa",contentHTML:e,maxWidth:"max-w-md"});t.querySelector("#open-cashier-form").addEventListener("submit",async a=>{a.preventDefault();const o=document.getElementById("initial-amount"),r=o.value.trim(),s=document.getElementById("cashier-notes").value.trim(),n=parseFloat(r);if(r===""||isNaN(n)||n<0){g("Valor Inválido","Por favor, insira um valor inicial válido (maior ou igual a R$ 0,00).","error"),o.focus();return}try{const i={establishmentId:p.establishmentId,initialAmount:parseFloat(n.toFixed(2))};s&&(i.notes=s);const d=await vr(i);w.isCashierOpen=!0,w.activeCashierSessionId=d.id,await va(),document.getElementById("genericModal").style.display="none",g("Sucesso!",`Caixa aberto com valor inicial de R$ ${n.toFixed(2)}`,"success")}catch(i){g("Erro",`Não foi possível abrir o caixa: ${i.message}`,"error")}})}async function Dr(){const e=w.activeCashierSessionId;if(e)try{const t=await xr(e),a=`
        ...
        `,{modalElement:o}=N({title:"Fechar Caixa",contentHTML:a,maxWidth:"max-w-md"});o.querySelector("#close-cashier-form").addEventListener("submit",async r=>{r.preventDefault();const s=parseFloat(document.getElementById("final-amount").value);if(isNaN(s)||s<0){g("Valor Inválido","Insira um valor final válido.","error");return}try{await hr(e,s),w.isCashierOpen=!1,w.activeCashierSessionId=null,document.getElementById("genericModal").style.display="none",await va(),g("Sucesso!","Caixa fechado com sucesso!","success")}catch(n){g("Erro",`Não foi possível fechar o caixa: ${n.message}`,"error")}})}catch(t){g("Erro",`Não foi possível carregar o relatório de fecho: ${t.message}`,"error")}}async function Pr(e){w.activeFilter!==e&&(w.activeFilter=e,w.paging.page=1,document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${e}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",e!=="finalizadas"),ge(),await K(),w.selectedComandaId=null,W())}function ea(e){w.selectedComandaId=e,de(),Sr(),W()}async function Ar(e,t){const a=w.allComandas.find(r=>r.id===w.selectedComandaId);if(!a)return;const o=Array(t).fill(0).map(()=>({id:e.id,name:e.name,price:e.price,type:e.type}));if(a.comandaItems=a.comandaItems||[],a.comandaItems.push(...o),a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),W(),de();return}try{await ma(a.id,a),g("Sucesso",`${t}x ${e.name} adicionado(s)!`,"success"),W(),de()}catch(r){g("Erro",`Não foi possível adicionar o item: ${r.message}`,"error"),a.comandaItems.splice(a.comandaItems.length-t,t)}}async function qr(e,t){const a=w.allComandas.find(s=>s.id===w.selectedComandaId);if(!a)return;let o=!1,r=(a.comandaItems||[]).findIndex(s=>s.id===e&&s.type===t);if(r>-1)a.comandaItems.splice(r,1),o=!0;else{let s=(a.services||[]).findIndex(n=>n.id===e);if(s>-1)a.services.splice(s,1),o=!0;else{let n=(a.items||[]).findIndex(i=>i.id===e&&i.type===t);n>-1&&(a.items.splice(n,1),o=!0)}}if(o){if(a.type==="walk-in"&&a.id.startsWith("temp-")){g("Sucesso","Item removido!","success"),W(),de();return}try{await ma(a.id,a),g("Sucesso","Item removido!","success"),W(),de()}catch(s){g("Erro",`Não foi possível remover o item: ${s.message}`,"error"),await K()}}}async function Nr(e,t,a){const o=e.type==="appointment",r=[...e.services||[],...e.comandaItems||[],...e.items||[]],s={payments:a,totalAmount:t,items:r,cashierSessionId:w.activeCashierSessionId};try{o?await as(e.id,s):(s.clientName=e.clientName,s.professionalId=e.professionalId,s.clientPhone=e.clientPhone,await dr(s)),g("Sucesso!","Venda finalizada com sucesso!","success"),document.getElementById("genericModal").style.display="none",ge(),w.selectedComandaId=null,await K()}catch(n){g("Erro no Checkout",n.message,"error")}}async function Fr(e){e.preventDefault();const t=document.getElementById("new-sale-client").value,a=document.getElementById("new-sale-professional").value,o=w.clients.find(n=>n.id===t),r=p.professionals.find(n=>n.id===a);if(!o||!r){g("Erro","Selecione um cliente e um profissional válidos.","error");return}const s={id:`temp-${Date.now()}`,type:"walk-in",clientName:o.name,clientPhone:o.phone,professionalId:r.id,professionalName:r.name,startTime:new Date,status:"confirmed",services:[],comandaItems:[]};w.allComandas.unshift(s),w.selectedComandaId=s.id,document.getElementById("genericModal").style.display="none",ea(s.id)}async function K(){const e=document.getElementById("comandas-list");e.innerHTML='<div class="loader mx-auto mt-10"></div>';const t=w.activeFilter==="finalizadas"?document.getElementById("filter-date").value:null;try{const a=await xo();if(w.isCashierOpen=!!a,w.activeCashierSessionId=a?a.id:null,Ir(),!w.isCashierOpen&&w.activeFilter==="atendimento"){de(),W();return}const o=await lr(p.establishmentId,t,w.paging.page,w.paging.limit);if(w.allComandas=o.data||o,w.paging.total=o.total||o.length,w.catalog.services.length===0){const[r,s,n,i,d]=await Promise.all([Pe(p.establishmentId),Dt(p.establishmentId),wo(p.establishmentId),tt(p.establishmentId),G(p.establishmentId)]);w.catalog={services:r,products:s,packages:n},w.clients=i,p.professionals=d}de(),w.selectedComandaId,W()}catch(a){g("Erro de Carregamento",`Não foi possível carregar os dados: ${a.message}`,"error"),e.innerHTML=`<p class="text-red-500 p-4">${a.message}</p>`}}async function va(e={}){xe=document.getElementById("content");try{const t=await xo();w.isCashierOpen=!!t,w.activeCashierSessionId=t?t.id:null}catch(t){console.error("Erro ao verificar caixa:",t),w.isCashierOpen=!1}w.selectedComandaId=e.selectedAppointmentId||null,$r(),ye&&(xe.removeEventListener("click",ye),xe.removeEventListener("change",ye)),ye=async t=>{const a=t.target.closest("[data-action], [data-filter], [data-comanda-id], [data-id]");if(t.target.id==="filter-date"&&w.activeFilter==="finalizadas"){w.paging.page=1,await K();return}if(a){if(a.matches("[data-filter]"))Pr(a.dataset.filter);else if(a.matches("[data-comanda-id]")){if(t.target.closest('[data-action="go-to-appointment"]')){t.stopPropagation();return}ea(a.dataset.comandaId)}else if(a.matches("[data-action]")){const o=a.dataset.action,r=a.dataset.id||w.selectedComandaId;switch(o){case"back-to-list":{ge(),w.selectedComandaId=null,document.querySelectorAll(".comanda-card").forEach(s=>s.classList.remove("selected")),W();break}case"new-sale":ko();break;case"add-item":Br();break;case"checkout":Tr();break;case"open-cashier":Mr();break;case"close-cashier":await Dr();break;case"view-sales-report":H("sales-report-section");break;case"remove-item":await qr(a.dataset.itemId,a.dataset.itemType);break;case"reopen-appointment":{if(await D("Reabrir Comanda","Tem certeza? O pagamento será estornado e os produtos devolvidos ao estoque."))try{await ts(r);const n=w.allComandas.findIndex(i=>i.id===r);n!==-1&&(delete w.allComandas[n].transaction,delete w.allComandas[n].cashierSessionId,delete w.allComandas[n].redeemedReward,w.allComandas[n].status="confirmed"),w.selectedComandaId=null,ge(),g("Sucesso!","Comanda reaberta para edição.","success"),await K()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"reopen-walk-in":{if(await D("Reabrir Venda","Tem certeza? A venda será cancelada e os produtos devolvidos ao estoque."))try{await cr(r),g("Sucesso!","Venda revertida."),ge(),w.selectedComandaId=null,await K()}catch(n){g("Erro",`Não foi possível reabrir: ${n.message}`,"error")}break}case"go-to-appointment":{const s=a.dataset.id,n=a.dataset.date;H("agenda-section",{scrollToAppointmentId:s,targetDate:new Date(n)});break}case"delete-walk-in":{if(await D("Excluir Venda","Tem certeza que deseja excluir esta venda avulsa? O estoque dos produtos será devolvido."))if(r.startsWith("temp-"))w.allComandas=w.allComandas.filter(n=>n.id!==r),w.selectedComandaId=null,de(),W(),g("Sucesso","Venda avulsa removida.","success"),ge();else try{await ur(r),g("Sucesso","Venda avulsa excluída com sucesso.","success"),w.selectedComandaId=null,ge(),await K()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}break}}}}},xe.addEventListener("click",ye),xe.addEventListener("change",ye),e.initialFilter&&(w.activeFilter=e.initialFilter==="finalizadas"?"finalizadas":"atendimento"),e.selectedAppointmentId&&(w.selectedComandaId=e.selectedAppointmentId),document.querySelectorAll(".filter-btn").forEach(t=>t.classList.remove("bg-white","text-indigo-600","shadow")),document.querySelector(`[data-filter="${w.activeFilter}"]`).classList.add("bg-white","text-indigo-600","shadow"),document.getElementById("finalizadas-datepicker").classList.toggle("hidden",w.activeFilter!=="finalizadas"),e.filterDate&&(document.getElementById("filter-date").value=new Date(e.filterDate).toISOString().split("T")[0]),await K(),w.selectedComandaId&&ea(w.selectedComandaId)}const Pt=document.getElementById("content");let le={},Rr=null,Hr=null,q={year:null,month:null,monthName:null,professionalId:null,professionalName:null},Fa=[];const we=["#4f46e5","#22c55e","#f97316","#06b6d4","#e11d48","#6366f1","#84cc16","#f59e0b"];function ha(){Object.values(le).forEach(e=>e?.destroy()),le={}}function jr(e){const t=document.getElementById("kpi-revenue"),a=document.getElementById("kpi-transactions"),o=document.getElementById("kpi-popular-item");t&&(t.textContent=`R$ ${e.totalRevenue.toFixed(2)}`),a&&(a.textContent=e.totalTransactions),o&&(o.textContent=e.mostPopularItem||"N/A"),document.querySelectorAll(".kpi-loader").forEach(r=>r.classList.add("hidden")),document.querySelectorAll(".kpi-value").forEach(r=>r.classList.remove("hidden"))}function Or(e){const t="Detalhes da Transação",a=`
    ...
    `;N({title:t,contentHTML:a,maxWidth:"max-w-md"})}async function Qe(e,t,a){q={year:e,month:t,monthName:a},Pt.innerHTML=`
        <section>
        ...
        </section>`,document.getElementById("backToMainBtn").addEventListener("click",ta);try{const o=await Bs(p.establishmentId,e,t);Hr=o,document.querySelector(".loader-container").classList.add("hidden"),document.getElementById("monthly-details-view").classList.remove("hidden");const r=o.revenueByDay.map(m=>m.day),s=o.revenueByDay.map(m=>m.revenue),n=o.salesByProfessional.map(m=>m.name),i=o.salesByProfessional.map(m=>m.count),d=o.topItems.map(m=>m.name),l=o.topItems.map(m=>m.count),c=["Agendamentos","Vendas Avulsas"],u=[o.revenueByTransactionType.appointment,o.revenueByTransactionType.sales];ha(),le.monthlyRevenue=new Chart(document.getElementById("monthlyRevenueChart").getContext("2d"),{type:"bar",data:{labels:r,datasets:[{label:"Receita Diária",data:s,backgroundColor:we[0]}]},options:{responsive:!0,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0}},onClick:(m,b)=>{if(b.length>0){const f=r[b[0].index];zr(e,t,f)}}}}),le.salesByProfessional=new Chart(document.getElementById("salesByProfessionalChart").getContext("2d"),{type:"pie",data:{labels:n,datasets:[{label:"Vendas",data:i,backgroundColor:we,hoverOffset:16}]},options:{responsive:!0,plugins:{legend:{position:"right"}},onClick:(m,b)=>{if(b.length>0){const f=b[0].index,v=o.salesByProfessional[f];v&&Vr(e,t,v.id,v.name)}}}}),le.topItems=new Chart(document.getElementById("topItemsChart").getContext("2d"),{type:"bar",data:{labels:d,datasets:[{label:"Itens Vendidos",data:l,backgroundColor:we[2]}]},options:{indexAxis:"y",responsive:!0,plugins:{legend:{display:!1}},scales:{x:{beginAtZero:!0}}}}),le.revenueByType=new Chart(document.getElementById("revenueByTypeChart").getContext("2d"),{type:"doughnut",data:{labels:c,datasets:[{label:"Receita por Tipo",data:u,backgroundColor:[we[1],we[3]],hoverOffset:16}]},options:{responsive:!0,plugins:{legend:{position:"right"}}}})}catch(o){g("Erro",`Não foi possível carregar os detalhes do mês: ${o.message}`,"error"),ta()}}async function zr(e,t,a){q={...q,day:a},Pt.innerHTML=`
        <section>
            <div class="flex justify-between items-center mb-6">
        ...
        `).join(""),document.querySelectorAll("#daily-transactions-list tr").forEach(s=>{s.addEventListener("click",n=>{const i=n.currentTarget.getAttribute("data-index"),d=Fa[i];d&&Or(d)})})}catch(o){g("Erro",`Não foi possível carregar os detalhes diários: ${o.message}`,"error"),Qe(q.year,q.month,q.monthName)}}async function Vr(e,t,a,o){q={year:e,month:t,professionalId:a,professionalName:o},Pt.innerHTML=`
        <section>
            <div class="flex justify-between items-center mb-6">
                <button id="backToMonthlyBtn" class="py-2 px-4 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300">
                    < Voltar
        ...
        `).join("")}catch(r){g("Erro",`Não foi possível carregar os detalhes do profissional: ${r.message}`,"error"),Qe(q.year,q.month,q.monthName)}}async function Ur(e,t,a){const o=t.getElementsAtEventForMode(e,"nearest",{intersect:!0},!1);if(o.length>0){const s=o[0].index,n=a[s];n&&await Qe(n.year,n.monthIndex,n.month)}}async function Ra(){const e=document.getElementById("main-reports-view"),t=document.getElementById("startDate"),a=document.getElementById("endDate");if(!e||!t||!a){console.error("Elementos essenciais para o relatório não foram encontrados no DOM.");return}e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';const o=t.value,r=a.value;if(!o||!r)return e.innerHTML='<p class="text-red-500 p-8 text-center">Selecione as datas para gerar o relatório.</p>',g("Atenção","Por favor, selecione as datas de início e fim.","error");try{const s=await Cs(p.establishmentId,o,r);Rr=s,e.innerHTML=`
            ...
            </div>`,jr(s.kpis),setTimeout(()=>{const n=document.getElementById("transactionsByMonthChart");n&&(ha(),le.main=new Chart(n.getContext("2d"),{type:"bar",data:{labels:s.transactionsByMonth.map(i=>i.month),datasets:[{label:"Receita Total",data:s.transactionsByMonth.map(i=>i.revenue),backgroundColor:we[0],borderRadius:5}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{callbacks:{label:i=>`R$ ${i.raw.toFixed(2)}`}}},scales:{y:{beginAtZero:!0}},onClick:(i,d)=>Ur(i,le.main,s.transactionsByMonth)}}))},50)}catch(s){g("Erro",`Não foi possível carregar os relatórios: ${s.message}`,"error"),e.innerHTML='<p class="text-red-500 p-8 text-center">Erro ao carregar os dados dos relatórios. Por favor, tente novamente.</p>'}}async function ta(){ha();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Pt.innerHTML=`
        <section>
            ...
            <div id="main-reports-view">
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Ra),await Ra()}const At=(e,t="products")=>k(`/api/${t}/categories/${e}`),Eo=(e,t="products")=>k(`/api/${t}/categories`,{method:"POST",body:JSON.stringify(e)}),So=(e,t="products")=>k(`/api/${t}/categories/${e}`,{method:"DELETE"}),Jr="audit_logs",Te=async(e,t,a,o,r,s=null)=>{try{if(!t)return;await ua(De(Q,Jr),{establishmentId:e,userId:t.uid,userName:t.name||t.email||"Utilizador",module:a,action:o,description:r,details:s,timestamp:new Date})}catch(n){console.error("Falha silenciosa ao registar log:",n)}},pe=document.getElementById("content");let re=null,He="services",fe="all";function Me(){const e=J.currentUser;return e?{uid:e.uid,name:e.displayName||e.email}:{uid:"unknown",name:"Desconhecido"}}async function _r(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await Eo({establishmentId:p.establishmentId,name:o},"services"),Te(p.establishmentId,Me(),"Categorias (Serviços)","Criou",`Criou categoria: ${o}`),a.value="",g("Sucesso","Categoria criada!","success"),await ya(),await ot()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function Wr(e){if(await D("Apagar Categoria","Tem a certeza? Os serviços nesta categoria ficarão sem categoria."))try{await So(e,"services"),Te(p.establishmentId,Me(),"Categorias (Serviços)","Excluiu",`Excluiu uma categoria (ID: ${e})`),g("Sucesso","Categoria apagada.","success"),await ya(),await ot()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function ya(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await At(p.establishmentId,"services");p.serviceCategories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                ...
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria criada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function Gr(){N({title:"Gerir Categorias de Serviços",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
    ...
    `,maxWidth:"max-w-xl"});const t=document.getElementById("genericModal");if(t){const a=t.querySelector("#categoryForm");a&&(a.addEventListener("submit",_r),t.addEventListener("click",o=>{const r=o.target.closest('button[data-action="delete-category"]');r&&(o.preventDefault(),Wr(r.dataset.id))}))}ya()}async function Yr(e){e.preventDefault();const t=e.target.closest("#serviceModal"),a=t.querySelector("#serviceId").value,o={},r=t.querySelector('input[name="commissionType"]:checked').value;r==="custom"&&t.querySelectorAll(".professional-commission-row").forEach(n=>{const i=n.dataset.profId;if(n.querySelector('input[type="checkbox"]').checked){const l=parseFloat(n.querySelector('input[type="number"]').value);o[i]=isNaN(l)?0:l}});const s={establishmentId:p.establishmentId,name:t.querySelector("#serviceName").value,price:parseFloat(t.querySelector("#servicePrice").value),duration:parseInt(t.querySelector("#serviceDurationMinutes").value,10),bufferTime:parseInt(t.querySelector("#serviceBufferTimeMinutes").value,10)||0,categoryId:t.querySelector("#serviceCategory").value||null,commissionRate:parseFloat(t.querySelector("#serviceCommissionRate").value)||0,active:t.querySelector("#serviceStatus").value==="true",photo:t.querySelector("#servicePhotoBase64").value,notes:t.querySelector("#serviceNotes").value,commissionType:r,professionalCommissions:o};try{a?(await As(a,s),Te(p.establishmentId,Me(),"Serviços","Editou",`Editou o serviço: ${s.name}`)):(await Ps(s),Te(p.establishmentId,Me(),"Serviços","Criou",`Criou novo serviço: ${s.name}`)),document.getElementById("serviceModal").style.display="none",g("Sucesso",`Serviço ${a?"atualizado":"adicionado"} com sucesso!`,"success"),await ot()}catch(n){g("Erro",n.message,"error")}}function Qr(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=d=>{const l=new Image;l.onload=()=>{let c=l.width,u=l.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(l,0,0,c,u);const f=m.toDataURL(o,r);s(f)},l.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),l.src=d.target.result},i.onerror=d=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Ha(e=null){const t=document.getElementById("serviceModal"),a=p.serviceCategories||[],o=e?.duration||0,r=e?.bufferTime||0,s=a.map(h=>`<option value="${h.id}" ${e?.categoryId===h.id?"selected":""}>${h.name}</option>`).join("");t.innerHTML=`
    ...
    `;const t=document.getElementById("serviceCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.serviceCategories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),xa(),ke()}function Zr(){const e=document.getElementById("services-content-container");e.innerHTML=`
    ...
    `}async function ot(){const e=document.getElementById("services-content-container");if(e){const t=e.querySelector(".loader");t&&(t.style.display="block")}try{const[t,a,o,r]=await Promise.all([Pe(p.establishmentId),G(p.establishmentId),At(p.establishmentId,"services"),Fs(p.establishmentId)]);p.services=(t||[]).filter(Boolean),p.professionals=(a||[]).filter(Boolean),p.serviceCategories=(o||[]).filter(Boolean),p.mostPopularService=r||{name:"N/A",count:0},p.services.forEach(s=>{s.active===void 0&&(s.active=!0)}),$o(He)}catch(t){e&&(e.innerHTML='<p class="text-red-500 col-span-full text-center py-10">Erro ao carregar dados. Verifique a conexão com o servidor.</p>'),g("Erro",`Não foi possível carregar os dados: ${t.message}`,"error")}}function $o(e){if(document.getElementById("services-content-container")){if(He===e&&document.getElementById("services-content-container").children.length>1){He==="services"&&(xa(),ke());return}He=e,fe="all",document.querySelectorAll("#services-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="services"?Xr():e==="reports"&&Zr()}}function Kr(){re&&(pe.removeEventListener("click",re),pe.removeEventListener("input",re),pe.removeEventListener("change",re)),re=async e=>{const t=e.target;if(t.closest('[data-action="toggle-service-status"]')){e.stopPropagation();const r=t.closest('[data-action="toggle-service-status"]'),s=r.dataset.id,n=r.checked;try{await Ns(s,n);const i=p.services.findIndex(d=>d.id===s);i>-1&&(p.services[i].active=n),Te(p.establishmentId,Me(),"Serviços","Atualizou Status",`Alterou status do serviço (ID: ${s}) para ${n?"Ativo":"Inativo"}`),ke(),xa()}catch(i){g("Erro",`Não foi possível atualizar o status: ${i.message}`,"error"),r.checked=!n,ke()}return}const a=t.closest("button[data-action], button[data-view], .indicator-card[data-action], .service-card[data-action]");if(t.id==="serviceSearchInput"||t.id==="serviceCategoryFilter"){ke();return}if(!a)return;if(a.hasAttribute("data-view")){$o(a.dataset.view);return}switch(a.dataset.action){case"new-service":Ha();break;case"edit-service":const r=JSON.parse(a.dataset.service);Ha(r);break;case"manage-categories":Gr();break;case"filter-service":const s=a.dataset.filterType;if(s==="popular")return;fe=s==="total"?"all":s,document.querySelectorAll('.indicator-card[data-action="filter-service"]').forEach(n=>{const i=n.dataset.filterType,l=i===fe||i==="total"&&fe==="all";n.classList.toggle("ring-2",l),n.classList.toggle("ring-indigo-500",l),n.classList.toggle("shadow-lg",l)}),ke();break}},pe.addEventListener("click",re),pe.addEventListener("input",re),pe.addEventListener("change",re)}async function en(){pe.innerHTML=`
        ...
        </section>`,Kr();try{(!p.professionals||p.professionals.length===0)&&(p.professionals=await G(p.establishmentId)||[])}catch(e){console.error("Falha ao carregar profissionais:",e),g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}He="services",fe="all",await ot()}const qt="suppliers",Io="purchases",wa=async()=>{try{const e=await co(De(Q,qt)),t=[];return e.forEach(a=>{t.push({id:a.id,...a.data()})}),t}catch(e){throw console.error("Erro ao buscar fornecedores:",e),e}},tn=async e=>{try{return{id:(await ua(De(Q,qt),e)).id,...e}}catch(t){throw console.error("Erro ao criar fornecedor:",t),t}},an=async(e,t)=>{try{const a=Ct(Q,qt,e);return await po(a,t),{id:e,...t}}catch(a){throw console.error("Erro ao atualizar fornecedor:",a),a}},on=async e=>{try{const t=Ct(Q,qt,e);return await Uo(t),!0}catch(t){throw console.error("Erro ao excluir fornecedor:",t),t}},sn=async e=>{try{const t={...e,createdAt:new Date};return{id:(await ua(De(Q,Io),t)).id,...t}}catch(t){throw console.error("Erro ao registrar compra:",t),t}},rn=async e=>{try{const t=uo(De(Q,Io),mo("createdAt","desc")),a=await co(t),o=[];return a.forEach(r=>{o.push({id:r.id,...r.data()})}),o}catch(t){throw console.error("Erro ao buscar histórico de compras:",t),t}},ce=document.getElementById("content");let ne=null,je="products",Z="all";async function nn(e){e.preventDefault();const a=e.target.closest("#categoryForm").querySelector("#categoryName"),o=a.value;if(o)try{await Eo({establishmentId:p.establishmentId,name:o},"products"),a.value="",g("Sucesso","Categoria de produto criada!","success"),await ka(),await st()}catch(r){g("Erro",`Não foi possível criar a categoria: ${r.message}`,"error")}}async function ln(e){if(await D("Apagar Categoria","Tem a certeza? Os produtos nesta categoria ficarão sem categoria."))try{await So(e,"products"),g("Sucesso","Categoria de produto apagada.","success"),await ka(),await st()}catch{g("Erro","Não foi possível apagar a categoria.","error")}}async function ka(){const e=document.getElementById("categoryList");if(e){e.innerHTML='<div class="loader mx-auto my-4"></div>';try{const t=await At(p.establishmentId,"products");p.categories=t,e.innerHTML="",t.length>0?e.innerHTML=t.map(a=>`
                <div class="flex justify-between items-center bg-gray-100 p-2 rounded">
                    <span>${a.name}</span>
                    <button data-action="delete-category" data-id="${a.id}" class="text-red-500 hover:text-red-700 font-semibold text-sm">Apagar</button>
                </div>`).join(""):e.innerHTML='<p class="text-center text-gray-500">Nenhuma categoria encontrada.</p>'}catch{e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar categorias.</p>'}}}function dn(){N({title:"Gerir Categorias de Produtos",contentHTML:`
        <div class="space-y-4">
            <div class="mb-4">
                <form id="categoryForm" class="flex flex-col sm:flex-row gap-4 sm:items-end">
                    <div class="flex-1 w-full">
        ...
        <form id="productForm">
            ...
            <div class="mt-8 pt-6 border-t flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
    ...
    </div>`;const n=t.querySelector("#productCategory"),i=t.querySelector("#productPhotoInput");t.querySelector("#productPhotoButton").addEventListener("click",()=>i.click()),n.innerHTML='<option value="">Sem categoria</option>'+(p.categories||[]).map(x=>`<option value="${x.id}" ${e?.categoryId===x.id?"selected":""}>${x.name}</option>`).join(""),e&&(n.value=e.categoryId||"");const d=t.querySelector("#productPhotoPreview");t.querySelector("#productPhotoBase64");const l=e?.photo||"https://placehold.co/128x128/E2E8F0/4A5568?text=Foto",c=e?.photo||"";i.onchange=async()=>{const x=i.files[0];if(x){d.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const h=await ja(x,800,800,"image/jpeg",.8),S=h.length*3/4,$=1e3*1024;if(S>$)throw new Error("A imagem é muito grande mesmo após a compressão.");preview.src=h,base64Input.value=h}catch(h){console.error("Erro ao processar imagem:",h),g("Erro de Imagem",h.message||"Não foi possível processar a imagem.","error"),preview.src=l,base64Input.value=c,v.value=""}}};const u=t.cloneNode(!0);t.parentNode.replaceChild(u,t);const m=()=>{const x=u.querySelector("#modalSupplierSearch"),h=u.querySelector("#supplierSearchResults"),y=u.querySelector("#selectedSuppliersList"),S=x.value.toLowerCase();if(S.length>0){const $=o.filter(L=>L.name.toLowerCase().includes(S)&&!s.has(L.id));$.length>0?(h.classList.remove("hidden"),h.innerHTML=$.map(L=>`
                ...
                `).join("")):(h.classList.remove("hidden"),h.innerHTML='<div class="p-2 text-xs text-gray-500 text-center">Nenhum resultado disponível.</div>')}else h.classList.add("hidden");s.size>0?(y.innerHTML="",s.forEach($=>{const L=o.find(B=>B.id===$);L&&(y.innerHTML+=`
                    ...
                    `)})):y.innerHTML='<p class="text-xs text-gray-400 text-center py-2">Nenhum fornecedor selecionado.</p>'};u.querySelector("#modalSupplierSearch").addEventListener("input",m),u.addEventListener("click",x=>{const h=x.target.closest("[data-add-supplier]");if(h){const S=h.dataset.addSupplier;s.add(S),u.querySelector("#modalSupplierSearch").value="",m()}const y=x.target.closest("[data-remove-supplier]");if(y){const S=y.dataset.removeSupplier;s.delete(S),m()}}),m(),u.addEventListener("click",async x=>{const h=x.target.closest("button[data-action]");if(!h)return;const y=h.dataset.action,S=u.querySelector("#productId").value;if(y==="close-modal"&&(u.style.display="none"),y==="delete-product"){if(!S)return;u.style.display="none",await cn(S)}if(y==="save-product-modal"){const $=u.querySelector("#productForm");if($){if(!$.querySelector("#productName").value||!$.querySelector("#productPrice").value){g("Erro","Nome e Preço de Venda são obrigatórios.","error");return}const L=h.closest('button[data-action="save-product-modal"]');L.disabled=!0,L.textContent="A salvar...";try{await un($)}catch(B){g("Erro",`Falha ao salvar: ${B.message}`,"error"),L.disabled=!1,L.textContent="Salvar Alterações"}}}if(y==="adjust-stock-modal"){x.preventDefault();const $=u.querySelector("#stockAdjustmentAmount"),L=u.querySelector("#stockAdjustmentReason"),B=parseInt($.value,10),j=parseInt(h.dataset.change,10);if(!B||B<=0){g("Erro","Por favor, insira uma quantidade válida.","error");return}const T=B*j,P=L.value||(T>0?"Entrada manual":"Saída manual");try{await br(S,{change:T,reason:P});const A=p.products.findIndex(O=>O.id===S);if(A>-1){const O=p.products[A].currentStock+T;p.products[A].currentStock=O,u.querySelector("#currentStockDisplay").textContent=O,u.querySelector("#productCurrentStock").value=O,$.value="",L.value="",g("Sucesso","Estoque atualizado!","success"),Ea(),Xe()}}catch(A){g("Erro de Stock",A.message,"error")}}});const b=u.querySelectorAll(".tab-btn"),f=u.querySelectorAll(".tab-content");b.forEach(x=>{x.addEventListener("click",h=>{h.preventDefault(),b.forEach(y=>{y.classList.remove("border-indigo-500","text-indigo-600"),y.classList.add("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300")}),x.classList.add("border-indigo-500","text-indigo-600"),x.classList.remove("border-transparent","text-gray-500","hover:text-gray-700","hover:border-gray-300"),f.forEach(y=>y.classList.add("hidden")),document.getElementById(`tab-content-${x.dataset.tab}`).classList.remove("hidden")})});const v=u.querySelector("#productPhotoInput");u.querySelector("#productPhotoButton").addEventListener("click",()=>v.click()),v.onchange=async()=>{const x=v.files[0];if(!x)return;const h=u.querySelector("#productPhotoPreview"),y=u.querySelector("#productPhotoBase64");h.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const S=await ja(x,800,800,"image/jpeg",.8),L=S.length*3/4,B=1e3*1024;if(L>B)throw new Error("A imagem é muito grande mesmo após a compressão.");h.src=S,y.value=S}catch(S){console.error("Erro ao processar imagem:",S),g("Erro de Imagem",S.message||"Não foi possível processar a imagem.","error"),h.src=l,y.value=c,v.value=""}},u.style.display="flex"}function mn(){const e=document.getElementById("products-content-container");e.innerHTML=`
    ...
    `;const t=document.getElementById("productCategoryFilter");t&&(t.innerHTML='<option value="all">Todas as categorias</option>',(p.categories||[]).forEach(a=>t.innerHTML+=`<option value="${a.id}">${a.name}</option>`)),Ea(),Xe()}function pn(){const e=document.getElementById("products-content-container"),t=new Date().toISOString().split("T")[0],a=new Date;a.setDate(a.getDate()-30);const o=a.toISOString().split("T")[0];e.innerHTML=`
        <div class="space-y-6">
        ...
        </div>`;const r=document.getElementById("productFilterReport"),s=document.getElementById("categoryFilterReport");r&&p.products&&(r.innerHTML+=p.products.map(n=>`<option value="${n.id}">${n.name}</option>`).join("")),s&&p.categories&&(s.innerHTML+=p.categories.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""))}async function gn(){const e=document.getElementById("report-results");e.innerHTML='<div class="loader mx-auto my-8"></div>';const t={startDate:document.getElementById("reportStartDate").value,endDate:document.getElementById("reportEndDate").value,productId:document.getElementById("productFilterReport").value,categoryId:document.getElementById("categoryFilterReport").value};try{const a=await fr(t);if(a.length===0){e.innerHTML=`
                <div class="bg-white border rounded-lg shadow-sm p-8">
            ...
            <div class="md:hidden space-y-3 pb-20">
                ...
                </div>`,e.appendChild(n)})):(e.className="",e.innerHTML='<p class="col-span-full text-center text-gray-500 py-10">Nenhum produto encontrado.</p>')}async function st(){const e=document.getElementById("products-content-container");e&&(e.innerHTML='<div class="loader col-span-full mx-auto my-8"></div>');try{const[t,a,o]=await Promise.all([Dt(p.establishmentId),At(p.establishmentId,"products"),wa()]);p.products=(t||[]).filter(Boolean),p.categories=(a||[]).filter(Boolean),p.suppliers=(o||[]).filter(Boolean),Co(je)}catch(t){e&&(e.innerHTML=`<p class="text-red-500 col-span-full text-center p-8">Erro ao carregar dados: ${t.message}</p>`)}}function Co(e){if(document.getElementById("products-content-container")){if(je===e&&document.getElementById("products-content-container").children.length>1){je==="products"&&(Ea(),Xe());return}je=e,Z="all",document.querySelectorAll("#products-tabs button.tab-button").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("border-indigo-500",a),t.classList.toggle("text-indigo-600",a),t.classList.toggle("border-transparent",!a),t.classList.toggle("text-gray-500",!a)}),e==="products"?mn():e==="movements"&&pn()}}async function bn(){ce.innerHTML=`
        ...
        </section>`,ne&&(ce.removeEventListener("click",ne),ce.removeEventListener("input",ne),ce.removeEventListener("change",ne)),ne=async e=>{const t=e.target;if(t.id==="productSearchInput"||t.id==="productCategoryFilter"){Xe();return}const a=e.target.closest("button[data-action], button[data-view], .indicator-card[data-action], .product-card[data-action]");if(!a||e.target.closest('[data-action-stop-propagation="true"]'))return;if(a.hasAttribute("data-view")){Co(a.dataset.view);return}switch(a.dataset.action){case"new-product":Oa();break;case"edit-product":Oa(JSON.parse(a.dataset.product));break;case"manage-product-categories":dn();break;case"generate-report":await gn();break;case"filter-stock":const r=a.dataset.filterType;Z=Z===r?"all":r,document.querySelectorAll(".indicator-card").forEach(s=>{s.classList.toggle("ring-2",s.dataset.filterType===Z),s.classList.toggle("ring-indigo-500",s.dataset.filterType===Z),s.classList.toggle("shadow-lg",s.dataset.filterType===Z)}),Xe();break}},ce.addEventListener("click",ne),ce.addEventListener("input",ne),ce.addEventListener("change",ne),je="products",Z="all",await st()}const ue=document.getElementById("content");let ie=null,ht="list",M={step:1,productsToBuy:[],allSuppliers:[],finalOrders:{},isQuoteMode:!1};async function fn(){ht==="list"?Nt():ht==="purchases"?(M.step=1,Oe()):ht==="history"&&kn()}async function vn(){try{const e=await wa();return p.suppliers=e||[],M.allSuppliers=e,!0}catch(e){return console.error(e),!1}}async function hn(e){if(await D("Excluir Fornecedor","Tem a certeza? Isso removerá o vínculo com os produtos."))try{await on(e),g("Sucesso","Fornecedor excluído.","success"),Lt("genericModal"),Nt()}catch(t){g("Erro","Erro ao excluir: "+t.message,"error")}}async function yn(e){e.preventDefault();const t=e.target,a=t.querySelector("#supId").value,o={name:t.querySelector("#supName").value,contactName:t.querySelector("#supContact").value,email:t.querySelector("#supEmail").value,phone:t.querySelector("#supPhone").value,taxId:t.querySelector("#supTaxId").value,category:t.querySelector("#supCategory").value},r=t.querySelector('button[type="submit"]');r.disabled=!0,r.textContent="A salvar...";try{a?(await an(a,o),g("Sucesso","Fornecedor atualizado!","success")):(await tn(o),g("Sucesso","Fornecedor criado!","success")),Lt("genericModal"),Nt()}catch(s){g("Erro","Erro ao salvar: "+s.message,"error")}finally{r.disabled=!1,r.textContent="Salvar"}}async function Nt(){const e=document.getElementById("suppliersList");if(!e)return;e.innerHTML='<div class="loader mx-auto my-8"></div>',await vn();const t=document.getElementById("supplierSearchInput")?.value.toLowerCase()||"",a=p.suppliers.filter(s=>s.name.toLowerCase().includes(t)||s.contactName&&s.contactName.toLowerCase().includes(t));if(e.innerHTML="",a.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum fornecedor encontrado.</div>';return}let o='<div class="flex flex-col gap-2 md:hidden">';a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;");o+=`
    ...
    `;a.forEach(s=>{const n=JSON.stringify(s).replace(/"/g,"&quot;");r+=`
        ...
        `}),r+="</tbody></table></div>",e.innerHTML=o+r}function xn(e){const t=e.phone?`https://wa.me/${e.phone.replace(/\D/g,"")}`:"#",a=e.phone?`tel:${e.phone}`:"#",o=e.email?`mailto:${e.email}`:"#",r=JSON.stringify(e).replace(/"/g,"&quot;"),s=`
    ...
    `;N({title:"",contentHTML:s,maxWidth:"max-w-md"})}async function Oe(){const e=document.getElementById("purchasesContainer");if(e)if(M.step===1){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const[t,a]=await Promise.all([Dt(p.establishmentId),wa()]);M.allSuppliers=a||[];const o=t.filter(l=>{const c=parseInt(l.currentStock||0),u=parseInt(l.minStock||0);return c<=u});if(M.productsToBuy=o,o.length===0){e.innerHTML=`
                    ...
                    <div class="bg-white p-3 md:p-4 rounded-lg border border-gray-200 shadow-sm">
                        <div class="flex flex-col md:flex-row justify-between items-center gap-3">
                            <div class="flex items-center gap-3 w-full md:w-auto">
                                <input type="checkbox" id="toggle-quote-mode" class="w-5 h-5 text-indigo-600 rounded" ${M.isQuoteMode?"checked":""}>
            ...
            `,aa()}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao calcular compras.</p>'}}else M.step===2&&wn(e)}function wn(e){if(!M.finalOrders||Object.keys(M.finalOrders).length===0){M.step=1,Oe();return}const t=M.isQuoteMode;let a="",o=0;const r=t?"border-indigo-100":"border-gray-200",s=t?"bg-indigo-50 border-indigo-100":"bg-gray-50 border-gray-200",n=t?"bg-blue-100 text-blue-700":"bg-green-100 text-green-700",i=t?"hidden":"flex",d=t?"Cotações Prontas":"Pedidos Prontos",l=t?"text-indigo-600":"text-green-600",c=t?"bg-indigo-50 border-indigo-100":"bg-green-50 border-green-100",u=t?"text-indigo-800":"text-green-800";for(const[m,b]of Object.entries(M.finalOrders)){let f=0,v=b.items.map(S=>{const $=S.qty*S.cost;return f+=$,`
            <div class="flex justify-between py-2 border-b border-gray-50 text-sm">
                ...
                <div class="text-right">
                    ...
                    <span class="text-indigo-600 font-bold block">R$ ${$.toFixed(2)}</span>
                </div>
            </div>
        `}).join("");o+=f;const x=encodeURIComponent(JSON.stringify({supplierId:m,supplierName:b.info.name,totalAmount:f,items:b.items})),h=encodeURIComponent(JSON.stringify({name:b.info.name,phone:b.info.phone,email:b.info.email})),y=encodeURIComponent(JSON.stringify(b.items));a+=`
    ...
    `}async function kn(){const e=document.getElementById("historyContainer");if(e){e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const t=await rn(p.establishmentId);if(t.length===0){e.innerHTML='<div class="text-center text-gray-500 py-8">Nenhum histórico encontrado.</div>';return}let a='<div class="flex flex-col gap-3 md:hidden">';t.forEach(s=>{const n=new Date(s.createdAt.seconds*1e3).toLocaleDateString("pt-BR");a+=`
        ...
        `;e.innerHTML=a+r}catch(t){console.error(t),e.innerHTML='<p class="text-red-500 text-center">Erro ao carregar histórico.</p>'}}}function En(e){const t=new Date(e.createdAt.seconds*1e3).toLocaleString("pt-BR"),a=e.items.map(r=>`
    ...
    `;N({title:e?"Editar Fornecedor":"Novo Fornecedor",contentHTML:t,maxWidth:"max-w-lg"}),setTimeout(()=>{document.getElementById("supplierForm").addEventListener("submit",yn),document.querySelector("#genericModal .modal-close").addEventListener("click",()=>Lt("genericModal"))},50)}function $n(){ue.innerHTML=`
    ...
    `,ie&&(ue.removeEventListener("click",ie),ue.removeEventListener("input",ie),ue.removeEventListener("change",ie)),ie=e=>{if(e.target.closest("#tab-btn-list")&&lt("list"),e.target.closest("#tab-btn-purchases")&&lt("purchases"),e.target.closest("#tab-btn-history")&&lt("history"),e.target.id==="toggle-quote-mode"&&(M.isQuoteMode=e.target.checked,Oe()),e.target.id==="supplierSearchInput"&&Nt(),e.target.closest("#btn-new-supplier")&&za(),e.target.closest(".supplier-item-mobile")){const a=e.target.closest(".supplier-item-mobile"),o=JSON.parse(a.dataset.supplier);xn(o)}const t=e.target.closest("button[data-action]");if(t){const a=t.dataset.action;a==="delete"&&hn(t.dataset.id),a==="edit"&&za(JSON.parse(t.dataset.supplier))}if((e.target.classList.contains("qty-input")||e.target.classList.contains("row-select"))&&aa(),e.target.id==="check-all-rows"){const a=e.target.checked;document.querySelectorAll(".row-select").forEach(o=>o.checked=a),aa()}if(e.target.closest("#btn-go-to-orders")){const a=document.querySelectorAll(".product-row"),o={};let r=!1;if(a.forEach(s=>{if(s.offsetParent===null||!s.querySelector(".row-select").checked)return;r=!0;let i="Produto";const d=s.querySelector("td:nth-child(2)"),l=s.querySelector(".font-bold");d?i=d.innerText:l&&(i=l.innerText);const c=parseInt(s.querySelector(".qty-input").value),u=parseFloat(s.dataset.cost),b=s.querySelector(".supplier-select").value;if(b){if(!o[b]){const f=M.allSuppliers.find(v=>v.id===b);o[b]={info:f,items:[]}}o[b].items.push({name:i,qty:c,cost:u})}}),!r){g("Atenção","Selecione pelo menos um item para gerar o pedido.","error");return}M.finalOrders=o,M.step=2,Oe()}if(e.target.closest("#btn-back-step1")&&(M.step=1,Oe()),e.target.closest(".btn-send-order")){const a=e.target.closest(".btn-send-order"),o=JSON.parse(decodeURIComponent(a.dataset.supplierInfo)),r=JSON.parse(decodeURIComponent(a.dataset.orderItems)),s=parseFloat(a.dataset.total),n=M.isQuoteMode;if(o.phone){const i=o.phone.replace(/\D/g,"");let d="";n?(d=`Olá *${o.name}*, tudo bem?
...
Aguardo retorno.`;const l=`mailto:${o.email}?subject=${encodeURIComponent(i)}&body=${encodeURIComponent(d)}`;window.location.href=l}else g("Erro","Fornecedor sem telefone ou email cadastrado.","error")}if(e.target.closest(".btn-register-order")){const a=e.target.closest(".btn-register-order"),o=JSON.parse(decodeURIComponent(a.dataset.order));o.establishmentId=p.establishmentId,sn(o).then(()=>{g("Sucesso","Compra registrada no histórico!","success"),a.disabled=!0,a.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Registrado',a.classList.replace("bg-blue-600","bg-green-600"),a.classList.replace("hover:bg-blue-700","hover:bg-green-700")}).catch(r=>{g("Erro","Falha ao registrar compra: "+r.message,"error")})}if(e.target.closest(".btn-print-order")){const o=e.target.closest(".supplier-order-card").dataset.supplierId,r=M.finalOrders[o];r?Sn(r,M.isQuoteMode):g("Erro","Dados do pedido não encontrados.","error")}if(e.target.closest(".btn-view-purchase")){const a=e.target.closest(".btn-view-purchase"),o=JSON.parse(a.dataset.purchase);En(o)}},ue.addEventListener("click",ie),ue.addEventListener("input",ie),ue.addEventListener("change",ie),lt("list")}function lt(e){ht=e,["list","purchases","history"].forEach(a=>{const o=document.getElementById(`tab-btn-${a}`),r=document.getElementById(`tab-content-${a}`);a===e?(o.classList.add("border-indigo-500","text-indigo-600"),o.classList.remove("border-transparent","text-gray-500"),r.classList.remove("hidden")):(o.classList.remove("border-indigo-500","text-indigo-600"),o.classList.add("border-transparent","text-gray-500"),r.classList.add("hidden"))});const t=document.getElementById("btn-new-supplier");t&&(e==="list"?t.classList.remove("hidden"):t.classList.add("hidden")),fn()}const Ht=document.getElementById("content"),Va={monday:"Segunda",tuesday:"Terça",wednesday:"Quarta",thursday:"Quinta",friday:"Sexta",saturday:"Sábado",sunday:"Domingo"};let ae=new Set,dt=null,Ee=null;function In(e=8){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function Cn(e){return e.length===0?'<p class="col-span-full text-center text-gray-500 py-10">Nenhum profissional encontrado.</p>':e.map(t=>{const a=t.status==="inactive",o=t.photo||`https://placehold.co/100x100/E2E8F0/4A5568?text=${encodeURIComponent(t.name?t.name.charAt(0):"P")}`,r=JSON.stringify(t).replace(/'/g,"&apos;");return`
            ...
            </div>`}).join("")}function jt(){const e=document.getElementById("genericModal");e.style.display="none",Ee&&e.removeEventListener("click",Ee)}async function Ln(e){const t=document.getElementById("genericModal"),a=e.id?e:{name:"Novo Profissional",specialty:"",status:"active",workingHours:{},services:[]},o=p.services||await Pe(p.establishmentId),r=p.professionals||await G(p.establishmentId),s=`
        ...
        </div>`;t.innerHTML=s,t.style.display="flex",Bn(a,o),Mn(a),Dn(a,r),An(a)}function Bn(e,t){const a=document.getElementById("professionalForm"),o=e.dob?e.dob.split("/"):["",""],r=Array.from({length:12},(m,b)=>{const f=b+1,v=f==o[1]?"selected":"",x=new Date(0,b).toLocaleString("pt-BR",{month:"long"});return`<option value="${f}" ${v}>${x.charAt(0).toUpperCase()+x.slice(1)}</option>`}).join(""),s=e.status||"active";a.innerHTML=`
        ...
        <div class="form-group"><label for="profNotes">Observações</label><textarea id="profNotes" rows="3" class="mt-1 w-full p-2 border rounded-md">${e.notes||""}</textarea></div>`;const n=document.getElementById("profPhotoInput"),i=document.getElementById("profPhotoButton"),d=document.getElementById("profPhotoPreview"),l=document.getElementById("profPhotoBase64"),c=e.photo||`https://placehold.co/128x128/E2E8F0/4A5568?text=${encodeURIComponent(e.name?e.name.charAt(0):"P")}`,u=e.photo||"";i&&i.addEventListener("click",()=>n.click()),n&&(n.onchange=async()=>{const m=n.files[0];if(m){d.src="https://placehold.co/128x128/E2E8F0/4A5568?text=...";try{const b=await Tn(m,800,800,"image/jpeg",.8);d.src=b,l.value=b}catch(b){g("Erro de Imagem",b.message||"Não foi possível processar a imagem.","error"),d.src=c,l.value=u,n.value=""}}})}function Tn(e,t=800,a=800,o="image/jpeg",r=.8){return new Promise((s,n)=>{if(!e.type.startsWith("image/"))return n(new Error("O ficheiro selecionado não é uma imagem."));const i=new FileReader;i.onload=d=>{const l=new Image;l.onload=()=>{let c=l.width,u=l.height;c>u?c>t&&(u*=t/c,c=t):u>a&&(c*=a/u,u=a);const m=document.createElement("canvas");m.width=c,m.height=u,m.getContext("2d").drawImage(l,0,0,c,u);const f=m.toDataURL(o,r);s(f)},l.onerror=c=>n(new Error("Não foi possível carregar a imagem.")),l.src=d.target.result},i.onerror=d=>n(new Error("Não foi possível ler o ficheiro.")),i.readAsDataURL(e)})}function Mn(e){const t=document.getElementById("jornada");t.innerHTML='<div><h3 class="text-xl font-semibold mb-4">Jornada de Trabalho Semanal</h3><p class="text-sm text-gray-600 mb-4">Defina os horários de trabalho padrão para este profissional.</p><div id="profScheduleContainer" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4"></div></div>',Pn(t.querySelector("#profScheduleContainer"),e.workingHours||{})}async function Dn(e,t){const a=document.getElementById("bloqueios");a.innerHTML=`
        ...
        `).join("")}catch(o){a.innerHTML=`<p class="text-red-500">${o.message}</p>`}}}function An(e){const t=document.getElementById("genericModal");Ee&&t.removeEventListener("click",Ee),Ee=async a=>{const o=a.target.closest("button[data-action]");if(!o){const s=a.target.closest(".tab-link");s&&(t.querySelectorAll(".tab-link").forEach(n=>n.classList.remove("active")),s.classList.add("active"),t.querySelectorAll(".tab-content").forEach(n=>n.classList.add("hidden")),document.getElementById(s.dataset.tab).classList.remove("hidden"));return}const r=o.dataset.action;switch(a.stopPropagation(),r){case"close-modal":jt();break;case"delete-professional":const s=o.dataset.id;if(await D("Excluir Profissional",`Tem certeza que deseja excluir ${e.name}? Esta ação não pode ser desfeita.`))try{await vs(s),g("Sucesso!","Profissional excluído.","success"),jt(),St()}catch(v){g("Erro",`Não foi possível excluir: ${v.message}`,"error")}break;case"save-professional":const i=document.getElementById("professionalForm"),d=o,l=document.getElementById("profScheduleContainer"),c=Array.from(i.querySelectorAll("#profServicesContainer input:checked")).map(v=>v.value),u={};l&&l.querySelectorAll(".day-schedule-card").forEach(v=>{const x=v.querySelector('[data-field="active"]').dataset.day;u[x]={active:v.querySelector('[data-field="active"]').checked,start:v.querySelector('[data-field="start"]').value,end:v.querySelector('[data-field="end"]').value,breakStart:v.querySelector('[data-field="breakStart"]').value,breakEnd:v.querySelector('[data-field="breakEnd"]').value}});const m={...e,id:i.querySelector("#professionalId").value||void 0,name:i.querySelector("#profName").value,specialty:i.querySelector("#profSpecialty").value,photo:i.querySelector("#profPhotoBase64").value,services:c,workingHours:u,phone:i.querySelector("#profPhone").value,dob:`${i.querySelector("#profDobDay").value}/${i.querySelector("#profDobMonth").value}`,receivesCommission:i.querySelector("#profCommission").value==="sim",showOnAgenda:i.querySelector("#profShowOnAgenda").value==="sim",orderOnAgenda:parseInt(i.querySelector("#profOrderOnAgenda").value)||1,notes:i.querySelector("#profNotes").value,status:i.querySelector("#profStatus").value};d.disabled=!0,d.textContent="A salvar...";try{m.id?(await fs(m.id,m),g("Sucesso!","Profissional atualizado.","success")):(delete m.id,await bs(m),g("Sucesso!","Profissional criado.","success")),jt(),St()}catch(v){g("Erro",v.message,"error"),d.disabled=!1,d.textContent="Salvar"}break;case"delete-blockage":const b=o.dataset.id;if(await D("Apagar Bloqueio","Tem certeza?"))try{await ga(b),g("Bloqueio removido.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";ze(e.id,v)}catch(v){g("Erro",v.message,"error")}break;case"batch-delete-blockage":const f=JSON.parse(o.dataset.ids);if(await D("Apagar em Lote",`Tem certeza que deseja apagar ${f.length} bloqueios com este motivo?`))try{await bo(f),g("Bloqueios removidos.","success");const v=document.getElementById("prof-blockages-filter")?document.getElementById("prof-blockages-filter").value:"future";ze(e.id,v)}catch(v){g("Erro",v.message,"error")}break}},t.addEventListener("click",Ee)}function oa(){const e=document.getElementById("batch-actions-container"),t=document.getElementById("selected-count");!e||!t||(ae.size>0?(t.textContent=`${ae.size} selecionado(s)`,e.classList.remove("hidden")):e.classList.add("hidden"))}function qn(){D("Excluir em Lote",`Tem certeza que deseja excluir ${ae.size} profissionais? Esta ação não pode ser desfeita.`).then(async e=>{if(e)try{await(void 0)(Array.from(ae)),g("Sucesso!",`${ae.size} profissionais foram excluídos.`,"success"),ae.clear(),oa(),St()}catch(t){g("Erro",`Não foi possível excluir em lote: ${t.message}`,"error")}})}function qe(){const e=document.getElementById("professionalsList");if(!e)return;if(!p.professionals){e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=In();return}const t=document.getElementById("showInactiveProfToggle").checked,a=document.getElementById("profSearchInput").value.toLowerCase(),o=p.professionals.filter(r=>{const s=r.name.toLowerCase().includes(a),n=t||r.status!=="inactive";return s&&n});e.className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 pb-20",e.innerHTML=Cn(o)}async function St(){ae.clear(),Ht.innerHTML=`
        ...
        </section>`,dt&&Ht.removeEventListener("click",dt),dt=t=>{const a=t.target.closest('[data-action="open-professional-modal"]'),o=t.target.closest('[data-action="batch-delete"]');if(a){t.preventDefault();let s={};if(a.dataset.professional)try{s=JSON.parse(a.dataset.professional)}catch(n){console.error("Erro ao fazer parse do professional data:",n);return}Ln(s);return}if(o){qn();return}const r=t.target.closest(".professional-checkbox");if(r){const s=r.dataset.id;r.checked?ae.add(s):ae.delete(s),qe(),oa();return}},Ht.addEventListener("click",dt),document.getElementById("profSearchInput").addEventListener("input",qe),document.getElementById("showInactiveProfToggle").addEventListener("change",qe);const e=document.getElementById("professionalsList");p.professionals=null,p.services=null,qe();try{const[t,a]=await Promise.all([G(p.establishmentId),Pe(p.establishmentId)]);p.professionals=t,p.services=a,qe(),oa()}catch{e.innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar dados da página.</p>'}}const Ot=document.getElementById("content");let ee=[],Ve={},R=null,sa="list",U="all",ra="O Estabelecimento";const Nn=(e,t)=>`Olá, ${e}! Nós da ${t} desejamos a você um Feliz Aniversário! Esperamos que seu dia seja maravilhoso. Venha comemorar conosco! 🎉🎂`,Fn=(e,t)=>`Oi, ${e}! Faz um tempo que não te vemos aqui no(a) ${t}. Sentimos sua falta! Temos novidades/ofertas especiais para você. Que tal agendar seu horário?`,Rn=[{value:30,label:"30 dias"},{value:60,label:"60 dias"},{value:90,label:"90 dias"},{value:120,label:"120 dias"}];function Hn(){return Math.floor(Math.random()*140)+10}function jn(e){if(!e.dob)return!1;const t=e.dob.split("/");if(t.length!==2)return!1;const a=new Date,o=a.getDate(),r=a.getMonth()+1,s=parseInt(t[0],10),n=parseInt(t[1],10);return s===o&&n===r}const On=[{value:99,label:"Aniversariantes de Hoje"},{value:0,label:"Todos os meses (com DOB)"},{value:1,label:"Janeiro"},{value:2,label:"Fevereiro"},{value:3,label:"Março"},{value:4,label:"Abril"},{value:5,label:"Maio"},{value:6,label:"Junho"},{value:7,label:"Julho"},{value:8,label:"Agosto"},{value:9,label:"Setembro"},{value:10,label:"Outubro"},{value:11,label:"Novembro"},{value:12,label:"Dezembro"}];function Ua(){return On.map(e=>{let t="";return e.value===99&&(t="selected"),`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function Ja(){return Rn.map(e=>{const t=e.value===90?"selected":"";return`<option value="${e.value}" ${t}>${e.label}</option>`}).join("")}function zn(e,t){const a=`w-5 h-5 ${t} mr-2`;switch(e){case"cadastro":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>`;case"agendamentos":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>`;case"historico":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v2a2 2 0 01-2 2h-2.586a1 1 0 00-.707.293l-1.414 1.414a1 1 0 01-1.414 0l-1.414-1.414A1 1 0 009.586 17H7a2 2 0 01-2-2v-2a2 2 0 012-2h12z" /></svg>`;case"fidelidade":return`<svg xmlns="http://www.w3.org/2000/svg" class="${a}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8a1 1 0 011 1v4a1 1 0 11-2 0v-4a1 1 0 011-1zm0 0a1 1 0 001-1V5a1 1 0 10-2 0v2a1 1 0 001 1zm0 0a1 1 0 011 1v2a1 1 0 11-2 0v-2a1 1 0 011-1z" /><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.5a9.5 9.5 0 1019 0 9.5 9.5 0 00-19 0z" /></svg>`;default:return""}}function Vn(e="cadastro"){const t=[{id:"cadastro",label:"Cadastro"},{id:"agendamentos",label:"Próximos Agend."},{id:"historico",label:"Histórico"},{id:"fidelidade",label:"Fidelidade"}],a=document.getElementById("client-detail-tabs");a&&(a.innerHTML=t.map(o=>{const r=e===o.id,s=r?"text-indigo-600":"text-gray-500";return`
        ...
        `}).join(""),a.querySelectorAll(".tab-btn").forEach(o=>{o.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),na(o.dataset.tab)})}))}async function na(e){Vn(e);const t=document.getElementById("client-detail-content");if(t)switch(t.innerHTML='<form id="client-form" class="p-6 space-y-4"><div class="loader mx-auto my-8"></div></form>',e){case"cadastro":t.innerHTML=Un(R);break;case"agendamentos":case"historico":try{const o=await js(p.establishmentId,R.name,R.phone);t.innerHTML=Jn(o,e)}catch(o){console.error("Erro ao carregar histórico do cliente:",o),t.innerHTML=`<form id="client-form" class="p-6 space-y-4"><p class="text-center text-red-500">Erro ao carregar o histórico: ${o.message}</p></form>`}break;case"fidelidade":const a=await Os(p.establishmentId,R.name,R.phone);t.innerHTML=_n(R,a);break;default:t.innerHTML='<form id="client-form" class="p-6 space-y-4"><p class="p-4 text-center text-gray-500">Secção não implementada.</p></form>'}}function Un(e){const t=e?.dob?e.dob.split("/"):["",""];return`
    ...
    `}function _n(e,t){const a=e.loyaltyPoints||0;let o='<p class="text-sm text-gray-500">O programa de fidelidade não está ativo.</p>';Ve.enabled&&Ve.tiers&&(o=Ve.tiers.map(s=>{const n=a>=s.points;return`
    ...
    `}function ia(e){R=e,sa="detail";const t=e!==null,a=t?"Editar Cliente":"Adicionar Cliente",o=`
    ...
    `,r=window.innerWidth<768,s=r?"max-w-full":"max-w-3xl";if(N({title:a,contentHTML:o,maxWidth:s}),r){const c=document.getElementById("genericModal");if(c){const u=c.querySelector(`.${s.replace(":","\\:")}`);u&&(u.style.height="auto",u.style.maxHeight="85vh",u.style.borderRadius="1rem")}}const n=document.getElementById("genericModal");n&&n.addEventListener("click",async c=>{const u=c.target.closest("[data-action]");if(!u)return;switch(u.dataset.action){case"redeem-reward":{const b=parseInt(u.dataset.points,10),f=u.dataset.reward;if(await D("Confirmar Resgate",`Deseja resgatar "${f}" por ${b} pontos?`))try{await zs(p.establishmentId,R.name,R.phone,{points:b,reward:f}),g("Prémio resgatado com sucesso!","success"),ee=await tt(p.establishmentId);const h=ee.find(y=>y.id===R.id);h&&(R=h),na("fidelidade")}catch(x){g(`Erro ao resgatar: ${x.message}`,"error")}break}case"open-comanda-from-history":{const b=u.dataset.appointmentId;b&&(document.getElementById("genericModal").style.display="none",H("comandas-section",{selectedAppointmentId:b,initialFilter:"finalizada"}));break}case"view-appointment":{const b=u.dataset.appointmentId,f=u.dataset.appointmentDate;b&&f&&(document.getElementById("genericModal").style.display="none",H("agenda-section",{targetDate:f,scrollToAppointmentId:b}));break}}}),na("cadastro");const i=document.getElementById("client-form");i&&i.addEventListener("submit",c=>{c.preventDefault(),Wn()});const d=document.getElementById("cancelDetailViewBtn");d&&d.addEventListener("click",c=>{c.preventDefault(),document.getElementById("genericModal").style.display="none",Ft()});const l=document.getElementById("deleteClientBtn");l&&l.addEventListener("click",async()=>{await Gn()})}async function Wn(){const e=document.getElementById("client-form");if(!e)return;const t=e.querySelector("#clientId").value,a={name:e.querySelector("#clientName").value,email:e.querySelector("#clientEmail").value,phone:e.querySelector("#clientPhone").value,dob:`${e.querySelector("#clientDobDay").value}/${e.querySelector("#clientDobMonth").value}`,notes:e.querySelector("#clientNotes").value,establishmentId:p.establishmentId};if(!a.name||!a.phone){g("Erro","Nome e Telefone são obrigatórios.","error");return}try{t?(await Rs(t,a),g("Sucesso","Cliente atualizado com sucesso!","success")):(await ba(a),g("Sucesso","Cliente cadastrado com sucesso!","success")),document.getElementById("genericModal").style.display="none",await Ft()}catch(o){g("Erro",`Não foi possível salvar: ${o.message}`,"error")}}async function Gn(){if(!R||!R.id)return;if(await D("Excluir Cliente",`Tem certeza que deseja excluir ${R.name}? Esta ação é irreversível.`))try{await Hs(R.id),g("Sucesso","Cliente excluído.","success"),document.getElementById("genericModal").style.display="none",await Ft()}catch(t){g("Erro",`Não foi possível excluir: ${t.message}`,"error")}}function yt(e,t){const a=document.getElementById("clientsList");if(a)if(a.innerHTML="",document.getElementById("client-count").textContent=`${e.length} cliente${e.length!==1?"s":""} | Total: ${t}`,e.length>0){const o=U==="inactive",r=U==="birthdays";e.forEach(s=>{const n=document.createElement("div");n.className="client-card bg-white rounded-lg shadow p-4 flex flex-col cursor-pointer",n.dataset.clientId=s.id;const i=s.loyaltyPoints||0,d=Ve.enabled?`${i} pts`:`R$ ${i.toFixed(2)}`;let l="";const u=`https://wa.me/55${s.phone?s.phone.replace(/\D/g,""):""}?text=`;if(o){const m=encodeURIComponent(Fn(s.name,ra));l=`
                    ...
                    </a>`}else if(r&&jn(s)){const b=encodeURIComponent(Nn(s.name,ra));l=`
    ...
    `,t.querySelector("#personal-data-form").addEventListener("submit",a=>{a.preventDefault();const o={ownerName:t.querySelector("#ownerName").value,name:t.querySelector("#establishmentName").value,phone:t.querySelector("#establishmentPhone").value,document:t.querySelector("#establishmentCnpjCpf").value,email:t.querySelector("#establishmentEmail").value,address:t.querySelector("#establishmentAddress").value,website:t.querySelector("#establishmentWebsite").value};Ae(o,a)})}function gi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `,t.querySelector("#change-email-form").addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#newEmail").value,r=t.querySelector("#currentPassword").value;if(!o||!r){g("Erro","Preencha todos os campos.","error");return}const s=t.querySelector('button[form="change-email-form"]');s.disabled=!0,s.textContent="A verificar...";try{const n=J.currentUser;if(!n)throw new Error("Usuário não autenticado.");const i=Ro.credential(n.email,r);await Ho(n,i),s.textContent="A enviar link...",await jo(n,o),s.textContent="A atualizar BD...",await ps(p.establishmentId,o),g("Sucesso","Link de verificação enviado! Por favor, verifique seu **novo e-mail** para confirmar a alteração.","success"),a.target.reset()}catch(n){let i="Não foi possível alterar o e-mail.";n.code==="auth/wrong-password"?i="A senha atual está incorreta.":n.code==="auth/email-already-in-use"?i="Este e-mail já está sendo usado por outra conta.":n.code==="auth/operation-not-allowed"?i="A troca de e-mail precisa ser habilitada no console do Firebase.":i=n.message,g("Erro",i,"error")}finally{s.disabled=!1,s.textContent="Salvar Novo E-mail"}})}function fi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
    ...
    `;const a=t.querySelector("#establishmentTimezone");if(e.timezone)a.value=e.timezone;else try{const s=Intl.DateTimeFormat().resolvedOptions().timeZone;Array.from(a.options).some(i=>i.value===s)?a.value=s:a.value="America/Sao_Paulo"}catch{a.value="America/Sao_Paulo"}const o=t.querySelector("#establishmentWorkingHoursContainer"),r=e.workingHours||{};Object.keys(zt).forEach(s=>{const n=r[s]||{},i=zt[s],d=n.active!==!1,l=document.createElement("div");l.className=`day-schedule-card p-4 rounded-lg ${d?"bg-gray-50":"bg-gray-100 disabled"}`,l.innerHTML=`
            ...
            </div>`,o.appendChild(l)}),o.addEventListener("change",s=>{const n=s.target.closest('.day-schedule-card input[type="checkbox"]');n&&n.closest(".day-schedule-card").classList.toggle("disabled",!n.checked)}),t.querySelector("#working-hours-form").addEventListener("submit",s=>{s.preventDefault();const n={};Object.keys(zt).forEach(d=>{n[d]={active:t.querySelector(`#est-${d}-active`).checked,start:t.querySelector(`#est-${d}-start`).value,end:t.querySelector(`#est-${d}-end`).value,breakStart:t.querySelector(`#est-${d}-breakStart`).value,breakEnd:t.querySelector(`#est-${d}-breakEnd`).value}});const i=t.querySelector("#establishmentTimezone").value;Ae({workingHours:n,timezone:i},s)})}function yi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
             <div class="flex justify-between items-center mb-6">
    ...
    `;try{const[a,o]=await Promise.all([Ze(),Ke()]),r=e.financialIntegration||{};t.querySelector("#financialNatureId").innerHTML=Wa(a,r.defaultNaturezaId),t.querySelector("#financialCostCenterId").innerHTML=Wa(o,r.defaultCentroDeCustoId)}catch{g("Erro","Não foi possível carregar os dados para a integração financeira.","error")}t.querySelector("#financial-form").addEventListener("submit",a=>{a.preventDefault();const o={financialIntegration:{defaultNaturezaId:t.querySelector("#financialNatureId").value||null,defaultCentroDeCustoId:t.querySelector("#financialCostCenterId").value||null}};Ae(o,a)})}function wi(e,t){t.innerHTML=`
        <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
            <h3 class="text-xl font-bold text-gray-800">${e}</h3>
            <p class="mt-4 text-gray-500">Esta secção ainda não foi implementada.</p>
        </div>
    `}function Ao(e="indigo",t){const a=t.querySelector("#color-palette-container"),o=t.querySelector("#establishmentThemeColor");!a||!o||(a.innerHTML="",Object.entries(mi).forEach(([r,s])=>{const n=r===e,i=document.createElement("div");i.className="w-24 text-center cursor-pointer mb-4",i.innerHTML=`
        ...
        `,i.addEventListener("click",()=>{o.value=r,Ao(r,t)}),a.appendChild(i)}),o.value=e)}function ki(e,t){const a=t.querySelector("#slotIntervalContainer"),o=t.querySelector("#establishmentSlotInterval");if(!a||!o)return;const r=[{label:"10 min",value:10},{label:"15 min",value:15},{label:"20 min",value:20},{label:"30 min",value:30},{label:"45 min",value:45},{label:"1 hora",value:60}];a.innerHTML=r.map(s=>{const n=s.value===e;return`<button type="button" data-value="${s.value}" 
    ...
    `,be.querySelector('button[data-action="back-to-list"]').addEventListener("click",o=>{o.preventDefault(),qo()});const a=document.getElementById("settings-content-detail");switch(e){case"personal-data":pi(V,a);break;case"change-password":gi(V,a);break;case"change-email":bi(V,a);break;case"branding":fi(V,a);break;case"booking":vi(V,a);break;case"working-hours":hi(V,a);break;case"loyalty":yi(V,a);break;case"financial":await xi(V,a);break;default:wi(t?t.label:"Definições",a)}}async function qo(){if(be.innerHTML=`
    ...
    `,be.querySelector("#settings-menu-list").addEventListener("click",o=>{const r=o.target.closest("button[data-section]");r&&(o.preventDefault(),Ei(r.dataset.section))});const a=be.querySelector('[data-action="go-to-my-profile"]');a&&a.addEventListener("click",o=>{o.preventDefault(),H("my-profile-section")})}const Ue=document.getElementById("content");async function Ie(e){const t=document.getElementById("blockagesList");if(t){t.innerHTML='<div class="loader mx-auto"></div>';try{const a=document.getElementById("filterStartDate")?.value,o=document.getElementById("filterEndDate")?.value,r=await Bt(p.establishmentId,a||new Date().toISOString().split("T")[0],o||new Date().toISOString().split("T")[0],e),s=document.getElementById("filterReason")?.value.toLowerCase(),n=s?r.filter(d=>d.reason&&d.reason.toLowerCase().includes(s)):r,i=n.reduce((d,l)=>{const c=l.reason||"Sem motivo";return d[c]||(d[c]=[]),d[c].push(l),d},{});if(t.innerHTML="",n.length===0){t.innerHTML='<p class="text-center text-gray-500">Nenhum bloqueio encontrado.</p>';return}Object.entries(i).forEach(([d,l])=>{const c=document.createElement("div");c.className="bg-gray-100 rounded-lg p-3 my-2 space-y-2";let u=`<div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                <h4 class="font-bold text-gray-700">${d} (${l.length})</h4>`;if(l.length>1){const m=JSON.stringify(l.map(b=>b.id));u+=`<button data-action="batch-delete-blockage" data-ids='${m}' class="text-xs text-red-600 hover:text-red-800 font-semibold flex items-center gap-1">
                                    Apagar Lote
                                </button>`}u+="</div>",c.innerHTML=u,l.forEach(m=>{const b=new Date(m.startTime),f=new Date(m.endTime),v=b.toLocaleDateString("pt-BR"),x=f.toLocaleDateString("pt-BR"),y=`
                    ...
                    </div>`;c.innerHTML+=y}),t.appendChild(c)})}catch(a){t.innerHTML=`<p class="text-center text-red-500">Erro: ${a.message}</p>`}}}async function Si(e){e.preventDefault();const t=e.target,a=t.querySelector("#blockageProfId").value,o=t.querySelector("#blockageDate").value,r=t.querySelector("#blockageEndDate").value||o,s=t.querySelector("#blockageStartTime").value,n=t.querySelector("#blockageEndTime").value,i={establishmentId:p.establishmentId,professionalId:a,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:t.querySelector("#blockageReason").value};try{await Tt(i),t.reset(),g("Sucesso","Bloqueio adicionado com sucesso!","success"),Ie(a)}catch(d){g("Erro",`Não foi possível criar o bloqueio: ${d.message}`,"error")}}async function $i(e){e.preventDefault();const t=e.target,a=Array.from(t.querySelectorAll('input[name="batch-professionals"]:checked')).map(c=>c.value);if(a.length===0)return g("Atenção","Selecione pelo menos um profissional.","error");const o=t.querySelector("#batchBlockageDate").value,r=t.querySelector("#batchBlockageEndDate").value||o,s=t.querySelector("#batchBlockageStartTime").value,n=t.querySelector("#batchBlockageEndTime").value,i=t.querySelector("#batchBlockageReason").value,d=t.querySelector('button[type="submit"]');d.disabled=!0,d.textContent="Aguarde...";const l=a.map(c=>{const u={establishmentId:p.establishmentId,professionalId:c,startTime:new Date(`${o}T${s}:00`).toISOString(),endTime:new Date(`${r}T${n}:00`).toISOString(),reason:i};return Tt(u)});try{await Promise.all(l),g("Sucesso",`${a.length} bloqueios foram criados com sucesso!`,"success"),t.reset(),t.querySelectorAll('input[name="batch-professionals"]:checked').forEach(u=>u.checked=!1);const c=document.getElementById("blockageProfId").value;c&&Ie(c)}catch(c){g("Erro",`Ocorreu um erro: ${c.message}`,"error")}finally{d.disabled=!1,d.textContent="Adicionar Bloqueio em Lote"}}function Ii(e){Ue.addEventListener("submit",t=>{t.target.id==="blockageForm"&&Si(t),t.target.id==="batchBlockageForm"&&$i(t)}),Ue.addEventListener("input",t=>{t.target.matches("#filterStartDate, #filterEndDate, #filterReason")&&Ie(e)}),Ue.addEventListener("click",async t=>{const a=t.target.closest("button[data-action]");if(!a)return;const o=a.dataset.action;if(o==="back-to-professionals")H("profissionais-section");else if(o==="delete-blockage"){if(await D("Apagar Bloqueio","Tem a certeza que deseja apagar este bloqueio?"))try{await ga(a.dataset.id),g("Sucesso","Bloqueio removido.","success"),Ie(e)}catch(s){g("Erro",`Não foi possível remover o bloqueio: ${s.message}`,"error")}}else if(o==="batch-delete-blockage"){const r=JSON.parse(a.dataset.ids);if(await D("Apagar Lote de Bloqueios",`Tem certeza que deseja apagar ${r.length} bloqueios de uma vez?`))try{await bo(r),g("Sucesso",`${r.length} bloqueios removidos.`,"success"),Ie(e)}catch(n){g("Erro",`Não foi possível apagar os bloqueios: ${n.message}`,"error")}}})}async function Ci(e){const{professionalId:t,professionalName:a}=e;if(!t||!a){Ue.innerHTML='<p class="text-red-500 p-8">Erro: ID do profissional não fornecido.</p>';return}Ue.innerHTML=`
        <section>
        ...
        </section>`,Ii(t),await Ie(t);const o=document.getElementById("batchProfSelectionContainer");try{const r=await G(p.establishmentId);o.innerHTML=r.map(s=>`
            <div class="flex items-center">
            ...
            </div>`).join("")}catch{o.innerHTML='<p class="text-red-500">Erro ao carregar profissionais.</p>'}}const Li=e=>k(`/api/users/${e}`),Bi=e=>k("/api/users",{method:"POST",body:JSON.stringify(e)}),Ti=(e,t)=>k(`/api/users/${e}`,{method:"PUT",body:JSON.stringify(t)}),Mi=e=>k(`/api/users/${e}`,{method:"DELETE"}),Di=(e,t)=>k(`/api/users/${e}/password`,{method:"PUT",body:JSON.stringify({password:t})}),Pi=(e,t)=>k(`/api/users/${e}/status`,{method:"PATCH",body:JSON.stringify({status:t})}),me=document.getElementById("content"),Ai={"agenda-section":"Agenda","comandas-section":"Comandas","relatorios-section":"Relatórios Gerais","sales-report-section":"Relatório de Vendas (Caixa)","financial-section":"Financeiro","servicos-section":"Serviços","produtos-section":"Produtos","suppliers-section":"Fornecedores","profissionais-section":"Profissionais","ausencias-section":"Ausências e Bloqueios","clientes-section":"Clientes","packages-section":"Pacotes","commissions-section":"Comissões","estabelecimento-section":"Configurações do Estabelecimento","users-section":"Usuários e Acessos"},qi={view:"Visualizar",create:"Criar",edit:"Editar"};let Ne=null,Fe=null;function Ni(e){const t=document.getElementById("usersListContainer");if(!t)return;const a=document.getElementById("showInactiveUsersToggle")?.checked;if(e.length===0){const o=a?"Nenhum usuário encontrado.":"Nenhum usuário ativo cadastrado.";t.innerHTML=`<p class="col-span-full text-center text-gray-500">${o}</p>`;return}e.sort((o,r)=>(o.status==="active"?-1:1)-(r.status==="active"?-1:1)),t.innerHTML=e.map(o=>{const r=JSON.stringify(o).replace(/'/g,"&apos;"),s=o.status==="active",n=p.professionals.find(c=>c.id===o.professionalId),i=n?n.name:"N/A",d=n?n.name.charAt(0):o.name.charAt(0),l=n?.photo||`https://placehold.co/64x64/E2E8F0/4A5568?text=${encodeURIComponent(d)}`;return`
    ...
    `}).join("")}function wt(){const e=document.getElementById("showInactiveUsersToggle")?.checked;let t;e?t=p.users:t=p.users.filter(a=>a.status==="active"),Ni(t)}function Fi(e={}){return Object.entries(Ai).map(([t,a])=>{const o=t==="agenda-section"||t==="comandas-section",r=e[t]?.view_all_prof===!0,s=Object.entries(qi).map(([i,d])=>`
    ...
    `;const m=window.innerWidth<768,b=u.querySelector(".bg-white");if(m&&b){b.classList.remove("rounded-xl","shadow-2xl","sm:p-6");const h=u.closest("section");h&&(h.style.padding="0",h.style.margin="0"),b.classList.add("p-4")}const f=u.querySelector("#userProfessionalId"),v=u.querySelector("#userPhotoPreview"),x=u.querySelector("#profPhotoName");if(f.addEventListener("change",h=>{const y=h.target.value,S=o(y),$=S?S.name:"Selecione um profissional",L=r(y,n);v.src=L.photoSrc,x.textContent=$,u.querySelector("#professionalPhotoUrl").value=L.photoUrl}),u.addEventListener("submit",async h=>{h.preventDefault();const y=e?.email,S=u.querySelector("#userEmail").value,$={};u.querySelectorAll('input[type="checkbox"]').forEach(j=>{const T=j.dataset.module,P=j.dataset.permission;$[T]||($[T]={}),$[T][P]=j.checked});const L=u.querySelector("#userProfessionalId").value||null,B={name:u.querySelector("#userName").value,permissions:$,professionalId:L};try{c?(y!==S&&(B.email=S),await Ti(e.id,B),g("Usuário atualizado com sucesso!","success")):(B.email=u.querySelector("#userEmail").value,B.password=u.querySelector("#userPassword").value,await Bi(B),g("Usuário criado com sucesso!","success")),$t()}catch(j){g(`Erro: ${j.message}`,"error")}}),c){const h=u.querySelector("#password-change-container"),y=h.querySelector('[data-action="show-password-form"]'),S=h.querySelector("#password-form"),$=S.querySelector('[data-action="save-password"]'),L=S.querySelector('[data-action="cancel-password-change"]');y.addEventListener("click",()=>{y.classList.add("hidden"),S.classList.remove("hidden")}),L.addEventListener("click",()=>{y.classList.remove("hidden"),S.classList.add("hidden"),S.querySelector("#userNewPassword").value=""}),$.addEventListener("click",async()=>{const B=S.querySelector("#userNewPassword").value;if(!B||B.length<6){g("Senha inválida","A nova senha deve ter pelo menos 6 caracteres.","error");return}if(await D("Alterar Senha","Tem a certeza que deseja alterar a senha deste usuário?"))try{$.disabled=!0,$.textContent="Aguarde...",await Di(e.id,B),g("Sucesso!","A senha do usuário foi alterada.","success"),y.classList.remove("hidden"),S.classList.add("hidden"),S.querySelector("#userNewPassword").value=""}catch(T){g("Erro",`Não foi possível alterar a senha: ${T.message}`,"error")}finally{$.disabled=!1,$.textContent="Salvar Nova Senha"}})}}async function Ri(){const e=document.getElementById("usersListContainer");e.innerHTML='<div class="loader col-span-full mx-auto"></div>';try{const[t,a]=await Promise.all([Li(p.establishmentId),G(p.establishmentId)]);p.users=t,p.professionals=a,wt()}catch{g("Erro ao carregar usuários.","error"),e.innerHTML='<p class="col-span-full text-center text-red-500">Não foi possível carregar os usuários.</p>'}}async function $t(){me.innerHTML=`
    ...
    `,Ne&&me.removeEventListener("click",Ne),Fe&&me.removeEventListener("change",Fe),Ne=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("click",Ne);return}const t=e.target.closest("[data-action]");if(!t)return;switch(t.dataset.action){case"new-user":Ga();break;case"edit-user":const o=JSON.parse(t.dataset.user.replace(/&apos;/g,"'"));Ga(o);break;case"back-to-list":$t();break;case"delete-user":{e.stopPropagation();const r=t.dataset.userId;if(await D("Excluir Usuário","Tem certeza que deseja excluir este usuário? Esta ação é irreversível."))try{await Mi(r),g("Usuário excluído com sucesso!","success"),$t()}catch(n){g(`Erro ao excluir: ${n.message}`,"error")}break}}},Fe=async e=>{if(!document.getElementById("user-list-view")){me.removeEventListener("change",Fe);return}const t=e.target.closest('input[data-action="toggle-user-status"]');if(e.target.id==="showInactiveUsersToggle")wt();else if(t){e.stopPropagation();const a=t.dataset.userId,o=t.checked?"active":"inactive";try{await Pi(a,o),g(`Usuário ${o==="active"?"ativado":"inativado"} com sucesso.`,"success");const r=p.users.findIndex(s=>s.id===a);r>-1&&(p.users[r].status=o,wt())}catch(r){g(`Erro ao atualizar status: ${r.message}`,"error"),t.checked=!t.checked,wt()}}},me.addEventListener("click",Ne),me.addEventListener("change",Fe),await Ri()}const Hi=document.getElementById("content");let Ya={},la=null;function ji(){Object.values(Ya).forEach(e=>e?.destroy()),Ya={}}function Oi(e,t){const{jsPDF:a}=window.jspdf,o=new a({orientation:"landscape",unit:"px",format:"a4"}),r=document.getElementById("salesReportSummaryCards");if(o.setFontSize(18),o.text(e,o.internal.pageSize.getWidth()/2,40,{align:"center"}),r){const n=[["Receita Total",r.querySelector("#summary-revenue").textContent],["Vendas Totais",r.querySelector("#summary-transactions").textContent],["Ticket Médio",r.querySelector("#summary-avg-ticket").textContent]];o.autoTable({startY:60,head:[["Métrica","Valor"]],body:n,theme:"striped",headStyles:{fillColor:[79,70,229]}})}const s=o.lastAutoTable?o.lastAutoTable.finalY+20:60;o.text("Detalhes das Vendas",20,s),o.autoTable({html:`#${t}`,startY:s+10,theme:"grid",headStyles:{fillColor:[22,163,74]}}),o.save(`${e.replace(/[\s/]/g,"_").toLowerCase()}.pdf`)}function Qa(e){const t=document.getElementById("genericModal"),a=(e.payments||[]).map(o=>`
    ...
    `).join(""),s.querySelectorAll("tr").forEach(i=>{i.addEventListener("dblclick",()=>{const d=i.dataset.transactionIndex,l=la.transactions[d];l&&Qa(l)})}),n.innerHTML=a.map((i,d)=>`
    ...
    `).join(""),n.querySelectorAll("div[data-transaction-index]").forEach(i=>{i.addEventListener("click",()=>{const d=i.dataset.transactionIndex,l=la.transactions[d];l&&Qa(l)})})}async function Xa(){const e=document.getElementById("main-reports-view"),t=document.getElementById("reportStartDate"),a=document.getElementById("reportEndDate");if(!e||!t||!a)return;const o=t.value,r=a.value;if(!o||!r)return g("Atenção","Por favor, selecione as datas de início e fim.","error");e.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A gerar relatório...</p></div>';try{const s=document.getElementById("cashierSessionFilter").value,n=await Ls({establishmentId:p.establishmentId,startDate:o,endDate:r,cashierSessionId:s});la=n,e.innerHTML=`
        ...
        `,zi(n)}catch(s){g("Erro",`Não foi possível carregar o relatório: ${s.message}`,"error"),e.innerHTML=`<p class="p-8 text-center text-red-500">${s.message}</p>`}}async function Vi(){ji();const e=new Date().toISOString().split("T")[0],t=new Date;t.setDate(t.getDate()-30);const a=t.toISOString().split("T")[0];Hi.innerHTML=`
        ...
        </section>`,document.getElementById("generateReportBtn").addEventListener("click",Xa),document.getElementById("exportPdfBtn").addEventListener("click",()=>{const o=document.getElementById("reportStartDate").value,r=document.getElementById("reportEndDate").value,s=`Relatorio_Vendas_${o}_a_${r}`;Oi(s,"transactionsTable")});try{const o=await yr(),r=document.getElementById("cashierSessionFilter");o.forEach(s=>{const n=new Date(s.openTime).toLocaleString("pt-BR",{dateStyle:"short"}),i=s.closedByName||"N/A";r.innerHTML+=`<option value="${s.id}">${i} - ${n}</option>`})}catch{g("Erro","Não foi possível carregar o histórico de caixas para o filtro.","error")}await Xa()}const Ui=document.getElementById("content");let I={payables:[],receivables:[],natures:[],costCenters:[],currentFilter:"pending",startDate:new Date(new Date().getFullYear(),new Date().getMonth()-1,1).toISOString().split("T")[0],endDate:new Date().toISOString().split("T")[0],previousBalance:0,filterNaturezaId:"all",filterCostCenterId:"all",currentListView:"receivables"},Vt=null,ct=null,ut=null;function Sa(e){const t=new Map,a=[];return e&&(e.forEach(o=>t.set(o.id,{...o,children:[]})),t.forEach(o=>{o.parentId&&t.has(o.parentId)?t.get(o.parentId).children.push(o):a.push(o)})),a}function No(e,t,a){if(!t||t.length===0){e.innerHTML='<p class="text-center text-gray-500">Nenhum item criado.</p>';return}const o=(r,s=0)=>{const n="— ".repeat(s);return`
        ...
        `};e.innerHTML=t.map(r=>o(r)).join("")}async function Za(e){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const t=document.getElementById("genericModal"),a=e==="nature",o=`Gerir ${a?"Naturezas Financeiras":"Centros de Custo"}`,r=a?Ze:Ke,s=a?"natures":"costCenters";t.innerHTML=`
    ...
    `,e.style.display="flex",e.querySelector("#generateCashFlowBtn").addEventListener("click",Ka),Ka()}function _i(){const e=document.getElementById("genericModal"),t=I.payables.filter(u=>u.status==="pending").reduce((u,m)=>u+m.amount,0),a=I.receivables.filter(u=>u.status==="pending").reduce((u,m)=>u+m.amount,0),o=a-t,r=I.payables.filter(u=>u.status==="paid").reduce((u,m)=>u+m.amount,0),s=I.receivables.filter(u=>u.status==="paid").reduce((u,m)=>u+m.amount,0),n=s-r,i=I.previousBalance||0,d=i+n,l=u=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(u),c=u=>u>=0?"text-green-600":"text-red-600";e.innerHTML=`
    ...
    `,e.style.display="flex"}function Wi(){const e=document.getElementById("genericModal");e.innerHTML=`
        <div class="modal-content max-w-lg">
            <div class="flex justify-between items-center mb-6">
    ...
    `,e.style.display="flex"}function mt(e,t="all"){let a='<option value="all">Todos</option>';const o=n=>{const i=new Map,d=[];return n&&(n.forEach(l=>i.set(l.id,{...l,children:[]})),i.forEach(l=>{l.parentId&&i.has(l.parentId)?i.get(l.parentId).children.push(l):d.push(l)})),d},r=(n,i=0)=>{const d=i>0?"— ".repeat(i):"",l=n.id===t?"selected":"";a+=`<option value="${n.id}" ${l}>${d}${n.name}</option>`,n.children.forEach(c=>r(c,i+1))};return o(e).forEach(n=>r(n)),a}async function ve(){const e=document.getElementById("financial-content"),t=document.getElementById("filterStartDate")?.value,a=document.getElementById("filterEndDate")?.value,o=document.getElementById("filterNaturezaId")?.value,r=document.getElementById("filterCostCenterId")?.value;if(!t||!a){try{const[i,d]=await Promise.all([Ze(),Ke()]);I={...I,natures:i,costCenters:d},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(I.natures)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(I.costCenters))}catch(i){g("Erro",`Não foi possível carregar os dados base: ${i.message}`,"error")}da(),ao();return}const s=document.getElementById("payables-list"),n=document.getElementById("receivables-list");s&&(s.innerHTML='<div class="loader mx-auto"></div>'),n&&(n.innerHTML='<div class="loader mx-auto"></div>');try{const i={startDate:t,endDate:a};o&&o!=="all"&&(i.natureId=o),r&&r!=="all"&&(i.costCenterId=r);const[d,l,c,u]=await Promise.all([ti(i),ni(i),Ze(),Ke()]),m=l.previousBalance-d.previousBalance;I={...I,payables:d.entries,receivables:l.entries,natures:c,costCenters:u,previousBalance:m,filterNaturezaId:o,filterCostCenterId:r},document.getElementById("filterNaturezaId")&&(document.getElementById("filterNaturezaId").innerHTML=mt(I.natures,I.filterNaturezaId)),document.getElementById("filterCostCenterId")&&(document.getElementById("filterCostCenterId").innerHTML=mt(I.costCenters,I.filterCostCenterId)),da(),ao()}catch(i){g("Erro",`Não foi possível carregar os dados: ${i.message}`,"error"),e&&(e.innerHTML='<p class="text-red-500 text-center">Falha ao carregar dados.</p>')}}async function Gi(e,t,a=null){e.preventDefault();const o=e.target,r=o.querySelector('[name="status"]').checked,s=o.querySelector('[name="paymentDate"]').value,n=parseFloat(o.querySelector('[name="amount"]').value),i=parseInt(o.querySelector('[name="installments"]')?.value,10)||1;if(isNaN(n)){g("Erro de Validação","O valor inserido é inválido.","error");return}if(r&&!s){g("Erro de Validação","Por favor, forneça a data de pagamento para um lançamento pago.","error");return}const d={description:o.querySelector('[name="description"]').value,amount:n,dueDate:o.querySelector('[name="dueDate"]').value,naturezaId:o.querySelector('[name="naturezaId"]').value||null,centroDeCustoId:o.querySelector('[name="centroDeCustoId"]').value||null,notes:o.querySelector('[name="notes"]').value,status:r?"paid":"pending",paymentDate:r?s:null,installments:a?1:i};try{a?(await(t==="payable"?ai(a,d):ii(a,d)),g("Sucesso","Lançamento atualizado!","success")):(await(t==="payable"?ei(d):ri(d)),g("Sucesso","Lançamento adicionado!","success")),document.getElementById("genericModal").style.display="none",await ve()}catch(l){g("Erro",`Não foi possível salvar: ${l.message}`,"error")}}async function Yi(e,t){if(await D("Confirmar Exclusão","Tem certeza? Esta ação é irreversível."))try{await(e==="payable"?oi(t):li(t)),g("Sucesso","Lançamento excluído!","success"),await ve()}catch(o){g("Erro",`Falha ao excluir: ${o.message}`,"error")}}async function Qi(e,t){const a=new Date().toISOString().split("T")[0];try{await(e==="payable"?si(t,a):di(t,a)),g("Sucesso","Lançamento atualizado!","success"),await ve()}catch(o){g("Erro",`Falha ao atualizar status: ${o.message}`,"error")}}function to(e){const t=I.currentFilter;return t==="all"?e:e.filter(a=>a.status===t)}function da(){const e=document.getElementById("payables-list"),t=document.getElementById("receivables-list");if(!e||!t)return;const a=new Map(I.natures.map(i=>[i.id,i.name])),o=new Map(I.costCenters.map(i=>[i.id,i.name])),r=to(I.payables),s=to(I.receivables),n=(i,d)=>{const l=i.status==="paid",c=JSON.stringify(i).replace(/'/g,"&apos;"),u=i.naturezaId?a.get(i.naturezaId):"N/A",m=i.centroDeCustoId?o.get(i.centroDeCustoId):"N/A";let b=d==="payable"?"text-red-600":"text-green-600";const f=l?"bg-gray-200 text-gray-600":d==="payable"?"bg-red-100 text-red-700":"bg-yellow-100 text-yellow-700",v=l?"Finalizado":"Pendente";return l&&(b="text-gray-500"),`
        ...
        </div>`};e.innerHTML=r.map(i=>n(i,"payable")).join("")||'<p class="text-center text-gray-500 py-4">Nenhuma conta a pagar.</p>',t.innerHTML=s.map(i=>n(i,"receivable")).join("")||'<p class="text-center text-gray-500 py-4">Nenhuma conta a receber.</p>'}function ao(){const e=I.payables.filter(r=>r.status==="pending").reduce((r,s)=>r+s.amount,0),t=I.receivables.filter(r=>r.status==="pending").reduce((r,s)=>r+s.amount,0),a=t-e;document.getElementById("summary-pending-receivables").textContent=`R$ ${t.toFixed(2)}`,document.getElementById("summary-pending-payables").textContent=`R$ ${e.toFixed(2)}`,document.getElementById("summary-pending-balance").textContent=`R$ ${a.toFixed(2)}`;const o=document.getElementById("summary-pending-balance");o&&(o.className=`text-2xl font-bold ${a>=0?"text-green-600":"text-red-600"}`)}function Ut(e,t=null){document.getElementById("fab-menu")?.classList.add("hidden"),document.getElementById("main-fab-btn")?.classList.remove("rotate-45");const a=document.getElementById("genericModal"),o=`${t?"Editar":"Nova"} ${e==="payable"?"Despesa":"Receita"}`,r=e==="payable"?"bg-red-600 hover:bg-red-700":"bg-green-600 hover:bg-green-700",s=m=>{let b='<option value="">-- Selecione (Opcional) --</option>';const f=Sa(m),v=(x,h="",y=0)=>{const S=y>0?"— ".repeat(y):"";b+=`<option value="${x.id}">${S}${x.name}</option>`,x.children.forEach($=>v($,h+"— "))};return f.forEach(x=>v(x)),b},n=s(I.natures),i=s(I.costCenters),d=t?"":`
        <div>
    ...
    `;a.innerHTML=`
        ...
        </div>`,a.style.display="flex",t&&(a.querySelector('[name="naturezaId"]').value=t.naturezaId||"",a.querySelector('[name="centroDeCustoId"]').value=t.centroDeCustoId||"");const l=a.querySelector("#status"),c=a.querySelector("#payment-date-container"),u=a.querySelector('[name="paymentDate"]');t?.status==="paid"&&(l.checked=!0,c.classList.remove("hidden"),u.value=t.paymentDate||new Date().toISOString().split("T")[0]),l.addEventListener("change",()=>{c.classList.toggle("hidden",!l.checked),u.required=l.checked}),a.querySelector("#financial-form").addEventListener("submit",m=>Gi(m,e,t?.id))}async function Xi(){const e=new Date,a=new Date(e.getFullYear(),e.getMonth()-1,1).toISOString().split("T")[0],o=e.toISOString().split("T")[0];I.startDate=a,I.endDate=o,I.currentFilter="pending",I.filterNaturezaId="all",I.filterCostCenterId="all",Ui.innerHTML=`
        <section>
    ...
    `;const r=document.getElementById("main-fab-btn"),s=document.getElementById("fab-menu");if(r&&s){r.addEventListener("click",v=>{v.stopPropagation(),s.classList.toggle("hidden"),r.classList.toggle("rotate-45")});const m=s.querySelector('button[data-action="open-modal"][data-type="receivable"]'),b=s.querySelector('button[data-action="open-modal"][data-type="payable"]'),f=s.querySelector('button[data-action="open-cash-flow-modal"]');m&&m.addEventListener("click",v=>{v.stopPropagation(),Ut("receivable")}),b&&b.addEventListener("click",v=>{v.stopPropagation(),Ut("payable")}),f&&f.addEventListener("click",v=>{v.stopPropagation(),eo()})}ct&&document.body.removeEventListener("click",ct),ut&&document.getElementById("genericModal").removeEventListener("click",ut);const n=()=>{const m=document.getElementById("filterStartDate"),b=document.getElementById("filterEndDate"),f=document.getElementById("filterNaturezaId"),v=document.getElementById("filterCostCenterId");I.startDate=m.value,I.endDate=b.value,I.filterNaturezaId=f.value,I.filterCostCenterId=v.value;const x=document.getElementById("advanced-filters");x&&x.classList.contains("hidden")===!1&&window.innerWidth<768&&x.classList.add("hidden"),ve()},i=m=>{const b=m.target.closest("[data-status-filter]");if(!b)return;const f=b.dataset.statusFilter;I.currentFilter=f,document.querySelectorAll("[data-status-filter]").forEach(v=>{v.classList.remove("bg-blue-100","text-blue-800"),v.classList.add("bg-gray-100","text-gray-600")}),b.classList.remove("bg-gray-100","text-gray-600"),b.classList.add("bg-blue-100","text-blue-800"),da()},d=m=>{const b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("btn-payables-view"),x=document.getElementById("btn-receivables-view");window.innerWidth>=1024&&I.currentListView===m||(m==="payables"?(b.classList.remove("hidden"),f.classList.add("hidden"),v&&(v.classList.remove("bg-gray-200"),v.classList.add("bg-red-100","border","border-red-500")),x&&(x.classList.remove("bg-green-100","border","border-green-500"),x.classList.add("bg-gray-200"))):(b.classList.add("hidden"),f.classList.remove("hidden"),v&&(v.classList.remove("bg-red-100","border","border-red-500"),v.classList.add("bg-gray-200")),x&&(x.classList.remove("bg-gray-200"),x.classList.add("bg-green-100","border","border-green-500"))),I.currentListView=m)};document.getElementById("applyDateFilterBtn").addEventListener("click",n),document.getElementById("filterNaturezaId").addEventListener("change",()=>{I.filterNaturezaId=document.getElementById("filterNaturezaId").value}),document.getElementById("filterCostCenterId").addEventListener("change",()=>{I.filterCostCenterId=document.getElementById("filterCostCenterId").value}),document.querySelectorAll("[data-status-filter]").forEach(m=>{m.addEventListener("click",i)}),ct=m=>{const b=m.target.closest("button[data-action]");if(!b)return;const{action:f,type:v,id:x}=b.dataset;f==="edit"?Ut(v,JSON.parse(b.dataset.item.replace(/&apos;/g,"'"))):f==="delete"?Yi(v,x):f==="mark-as-paid"?Qi(v,x):f==="manage-natures"?Za("nature"):f==="manage-cost-centers"?Za("cost-center"):f==="open-cash-flow-modal"?eo():f==="toggle-filters"?document.getElementById("advanced-filters")?.classList.toggle("hidden"):f==="open-indicators-modal"?_i():f==="open-settings-modal"?Wi():f==="toggle-list-view"&&d(b.dataset.list)},ut=m=>{const b=m.target.closest('button[data-action^="delete-"]');if(b){const f=b.dataset.action.split("-")[1];l(f,b.dataset.id)}},document.body.addEventListener("click",ct),document.getElementById("genericModal").addEventListener("click",ut);async function l(m,b){const f=m==="nature",v=f?Xn:Kn,x=f?Ze:Ke,h=f?"natures":"costCenters",y=document.getElementById("hierarchyList");if(await D("Apagar Item","Tem a certeza? Apagar um item principal também apagará os seus sub-itens."))try{await v(b);const $=await x();I[h]=$,No(y,Sa($),m),await ve()}catch($){g("Erro",`Não foi possível apagar: ${$.message}`,"error")}}const c=()=>{const m=window.innerWidth<1024,b=document.getElementById("payables-container"),f=document.getElementById("receivables-container"),v=document.getElementById("list-toggle-buttons");b&&f&&(b.classList.remove("hidden"),f.classList.remove("hidden"),m?(b.classList.remove("lg:col-span-1"),f.classList.remove("lg:col-span-1"),v?.classList.remove("hidden"),d(I.currentListView)):(b.classList.add("lg:col-span-1"),f.classList.add("lg:col-span-1"),v?.classList.add("hidden"),b.classList.remove("hidden"),f.classList.remove("hidden")))};c(),window.addEventListener("resize",c);const u=document.querySelector(`[data-status-filter="${I.currentFilter}"]`);u&&(document.querySelectorAll("[data-status-filter]").forEach(m=>{m.classList.remove("bg-blue-100","text-blue-800"),m.classList.add("bg-gray-100","text-gray-600")}),u.classList.remove("bg-gray-100","text-gray-600"),u.classList.add("bg-blue-100","text-blue-800"));try{const m=await ui(),b=document.getElementById("summary-today-payables");b&&(b.textContent=`R$ ${m.totalPayables.toFixed(2)}`);const f=document.getElementById("summary-today-receivables");f&&(f.textContent=`R$ ${m.totalReceivables.toFixed(2)}`)}catch{g("Erro","Não foi possível carregar o resumo do dia.","error")}await ve()}const Zi=e=>k("/api/commissions/calculate",{method:"POST",body:JSON.stringify(e)}),Ki=e=>k("/api/commissions/save",{method:"POST",body:JSON.stringify(e)}),el=(e={})=>{const t=new URLSearchParams(e).toString(),a=`/api/commissions/history${t?"?"+t:""}`;return k(a)},tl=e=>k(`/api/commissions/report/${e}`,{method:"DELETE"}),Ce=document.getElementById("content");let Je=[],Fo="",pt=null,Se={};function al(e){const{jsPDF:t}=window.jspdf,a=new t;a.setFontSize(18),a.text(`Recibo de Comissão - ${e.professionalName}`,105,20,null,null,"center"),a.setFontSize(12),a.text(`Período: ${e.period}`,105,30,null,null,"center"),a.autoTable({startY:40,head:[["Descrição","Valor (R$)"]],body:[["Total Comissionável",`R$ ${e.summary.totalCommissionableValue.toFixed(2)}`],["Total de Itens",e.summary.totalItems]],theme:"striped"});const o=a.lastAutoTable.finalY+20;a.setFontSize(14),a.setFont(void 0,"bold"),a.text("Valor Total da Comissão:",14,o),a.text(`R$ ${e.summary.totalCommission.toFixed(2)}`,190,o,null,null,"right");const r=o+80;a.line(40,r,170,r),a.setFontSize(10),a.setFont(void 0,"normal"),a.text(e.professionalName,105,r+10,null,null,"center"),a.save(`recibo_comissao_${e.professionalName}_${e.period.replace(/\//g,"-")}.pdf`)}function ol(){const e=new Date,t=new Date(e.getFullYear(),e.getMonth(),1).toISOString().split("T")[0],a=e.toISOString().split("T")[0],r=`
    ...
    `,{modalElement:s}=N({title:"✨ Novo Cálculo de Comissões",contentHTML:r,maxWidth:"max-w-xl"}),n=s.querySelector("#calculation-form"),i=s.querySelector("#calc-professionals-all"),d=s.querySelectorAll(".professional-checkbox");i.addEventListener("change",l=>{d.forEach(c=>{c.checked=l.target.checked})}),d.forEach(l=>{l.addEventListener("change",()=>{l.checked?Array.from(d).every(u=>u.checked)&&(i.checked=!0):i.checked=!1})}),n.addEventListener("submit",l=>{l.preventDefault();const c=Array.from(d).filter(m=>m.checked).map(m=>m.value);if(c.length===0){g("Atenção","Selecione pelo menos um profissional para o cálculo.","error");return}rl({professionalIds:c});const u=s.querySelector("[data-close-modal]");u?u.click():s.style.display="none"})}async function sl(e){if(await D("Excluir Relatório","Tem a certeza que deseja excluir permanentemente este relatório de comissão? Esta ação não pode ser desfeita."))try{await tl(e),g("Sucesso!","Relatório de comissão excluído.","success"),kt()}catch(a){g("Erro",`Não foi possível excluir: ${a.message}`,"error")}}async function rl(e){const{professionalIds:t}=e,a=document.getElementById("calc-start-date")?.value,o=document.getElementById("calc-end-date")?.value,r={services:document.getElementById("calc-type-services")?.checked,products:document.getElementById("calc-type-products")?.checked,packages:document.getElementById("calc-type-packages")?.checked};if(!a||!o){g("Erro","As datas não foram capturadas corretamente.","error");return}H("commissions-section",{view:"results",isLoading:!0});try{const s=await Zi({professionalIds:t,startDate:a,endDate:o,calculationTypes:r});Je=s;const n=`${new Date(a+"T00:00:00").toLocaleDateString("pt-BR")} a ${new Date(o+"T00:00:00").toLocaleDateString("pt-BR")}`;Fo=n,ll(s,n)}catch(s){g("Erro",`Não foi possível calcular: ${s.message}`,"error"),H("commissions-section",{view:"history"})}}async function nl(){if(Je.length===0){g("Erro","Não há resultados para salvar.","error");return}const e=Fo;if(await D("Salvar Relatórios",`Tem a certeza que deseja salvar ${Je.length} relatório(s) de comissão para o período de ${e}?`))try{const a=Je.map(o=>Ki({professionalId:o.professionalId,professionalName:o.professionalName,period:e,reportData:o}));await Promise.all(a),g("Sucesso!","Relatórios de comissão salvos.","success"),H("commissions-section",{view:"history"})}catch(a){g("Erro",`Não foi possível salvar: ${a.message}`,"error")}}async function kt(){const e=document.getElementById("commissionHistory");if(!e)return;const t=document.getElementById("filter-professional")?.value||"",a=document.getElementById("filter-month")?.value||"";Se={},t&&t!=="all"&&(Se.professionalId=t),a&&(Se.period=a),e.innerHTML='<div class="loader mx-auto my-8"></div>';try{const o=await el(Se);if(o.length===0){e.innerHTML='<p class="text-center text-gray-500 py-8">Nenhum relatório de comissão salvo encontrado para os filtros.</p>';return}e.innerHTML=`
            <div class="space-y-3">
                ${o.map(r=>`
            ...
            </div>`}catch(o){e.innerHTML=`<p class="text-red-500 text-center">Erro ao carregar histórico: ${o.message}</p>`}}function il(){if(!p.professionals){Ce.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A carregar dados...</p></div>';return}const e=p.professionals.map(s=>`<option value="${s.id}">${s.name}</option>`).join(""),t=[],a=new Date;for(let s=0;s<12;s++){const n=new Date(a.getFullYear(),a.getMonth()-s,1),i=n.getFullYear(),l=(n.getMonth()+1).toString().padStart(2,"0"),c=`${i}-${l}`,u=n.toLocaleDateString("pt-BR",{month:"long",year:"numeric"});t.push(`<option value="${c}">${u}</option>`)}const o=Se.professionalId||"all",r=Se.period||"";Ce.innerHTML=`
    ...
    `,document.getElementById("filter-professional").value=o,document.getElementById("filter-month").value=r,document.getElementById("filter-professional").addEventListener("change",kt),document.getElementById("filter-month").addEventListener("change",kt),kt()}function ll(e,t){Je=e;const a=e.reduce((o,r)=>o+r.summary.totalCommission,0);Ce.innerHTML=`
        <section>
    ...
    `}async function dl(e={}){const{view:t="history",isLoading:a=!1}=e;if(pt&&Ce.removeEventListener("click",pt),pt=o=>{const r=o.target.closest("button[data-action]");if(!r)return;switch(r.dataset.action){case"open-calculator":ol();break;case"back-to-history":H("commissions-section",{view:"history"});break;case"save-reports":nl();break;case"generate-receipt":const n=JSON.parse(r.dataset.report.replace(/&apos;/g,"'"));al(n);break;case"delete-report":const i=r.dataset.id;sl(i);break}},Ce.addEventListener("click",pt),a){Ce.innerHTML='<div class="p-8 text-center"><div class="loader mx-auto"></div><p class="mt-4">A calcular comissões...</p></div>';return}if(!p.professionals||p.professionals.length===0)try{p.professionals=await G(p.establishmentId)}catch{g("Erro","Não foi possível carregar a lista de profissionais.","error"),p.professionals=[]}(t==="history"||t==="main")&&il()}const Jt=document.getElementById("content");let he={allPackages:[],catalogForModal:{services:[],products:[]}},gt=null,$e=null;function cl(e=6){let t="";for(let a=0;a<e;a++)t+=`
        ...
        `;return t}function ul(){const e=document.getElementById("packagesListContainer");if(e){if(he.allPackages.length===0){e.innerHTML=`
        ...
        `}).join("")}}function oo(){const e=document.getElementById("genericModal");e.style.display="none",$e&&e.removeEventListener("click",$e)}async function so(e=null){const t=document.getElementById("genericModal"),a=!!e,o=e?JSON.parse(JSON.stringify(e.items||[])):[],r=`
    ...
    `;t.innerHTML=r,t.style.display="flex";const s=t.querySelector("#package-items-list"),n=(d,l)=>{const c=l.querySelector("#originalPrice"),u=d.reduce((m,b)=>m+b.price*b.quantity,0);c&&(c.textContent=`R$ ${u.toFixed(2)}`)},i=d=>{d.length===0?s.innerHTML='<p class="text-center text-gray-500 p-4">Nenhum item adicionado.</p>':s.innerHTML=d.map((l,c)=>{const u=l.type==="service",m=u?"Serviço":"Produto",b=u?"bg-indigo-100 text-indigo-800":"bg-green-100 text-green-800";return`
            ...
            `}).join(""),n(d,t)};i(o),s.addEventListener("change",d=>{if(d.target.classList.contains("quantity-input")){const l=parseInt(d.target.dataset.index,10),c=parseInt(d.target.value,10);c>0&&o[l]&&(o[l].quantity=c,i(o))}}),s.addEventListener("click",d=>{if(d.target.classList.contains("remove-item-btn")){const l=parseInt(d.target.dataset.index,10);o.splice(l,1),i(o)}}),t.querySelector("#add-item-to-package-btn").onclick=()=>ml(d=>{const l=o.find(c=>c.id===d.id&&c.type===d.type);l?l.quantity++:o.push({...d,quantity:1}),i(o)}),$e&&t.removeEventListener("click",$e),$e=async d=>{const l=d.target.closest("button[data-action]");if(!l)return;const c=l.dataset.action;if(d.stopPropagation(),c==="close-modal"&&oo(),c==="save-package"){const u=l,m={id:document.getElementById("packageId").value||null,name:document.getElementById("packageName").value,description:document.getElementById("packageDescription").value,status:document.getElementById("packageStatus").value,items:o,originalPrice:o.reduce((b,f)=>b+f.price*f.quantity,0),price:parseFloat(document.getElementById("finalPrice").value),commissionRate:parseFloat(document.getElementById("commissionRate").value)||0,validityDays:parseInt(document.getElementById("validityDays").value,10)||null};if(!m.name||!m.price){g("Erro","Nome do Pacote e Preço Final são obrigatórios.","error");return}if(m.items.length===0){g("Erro","Adicione pelo menos um item ao pacote.","error");return}u.disabled=!0,u.textContent="A salvar...";try{a?await kr(m.id,m):(delete m.id,await wr(m)),g("Sucesso!",`Pacote ${a?"atualizado":"criado"} com sucesso.`,"success"),oo(),await $a()}catch(b){g("Erro",`Não foi possível salvar o pacote: ${b.message}`,"error"),u.disabled=!1,u.textContent="Salvar Pacote"}}},t.addEventListener("click",$e)}function ml(e){let t="";const a=document.createElement("div");a.id="item-selection-modal",a.className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[110]";const o={service:'<svg class="w-6 h-6 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v4.512a9.04 9.04 0 00-3 5.012M12 12a9.04 9.04 0 01-3-5.012V5l-1-1z" /></svg>',product:'<svg class="w-6 h-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>'},r=d=>{const l=t.toLowerCase(),c=he.catalogForModal.services.filter(f=>f.name.toLowerCase().includes(l)),u=he.catalogForModal.products.filter(f=>f.name.toLowerCase().includes(l)),m=c.map(f=>`
    ...
    `,gt&&Jt.removeEventListener("click",gt),gt=e=>{if(e.target.closest('[data-action-stop-propagation="true"]')){e.stopPropagation();const o=e.target.closest('[data-action="delete-package"]');if(o){const r=o.dataset.id;D("Excluir Pacote","Tem a certeza que deseja excluir este pacote? Esta ação é irreversível.").then(async s=>{if(s)try{await Er(r),g("Sucesso!","Pacote excluído.","success"),await $a()}catch(n){g("Erro",`Não foi possível excluir: ${n.message}`,"error")}})}return}const t=e.target.closest('[data-action="new-package"], [data-action="edit-package"]');if(!t)return;const a=t.dataset.action;if(a==="new-package")so(null);else if(a==="edit-package"){const o=JSON.parse(t.dataset.package);so(o)}},Jt.addEventListener("click",gt);try{const[e,t,a]=await Promise.all([wo(p.establishmentId),Pe(p.establishmentId),Dt(p.establishmentId)]);he.allPackages=e,he.catalogForModal={services:t.filter(o=>o.active),products:a},ul()}catch{document.getElementById("packagesListContainer").innerHTML='<p class="text-red-500 col-span-full">Erro ao carregar pacotes.</p>'}}const pl=document.getElementById("content");let gl=null;async function bl(){pl.innerHTML=`
        <div class="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
    ...
    `,await fl()}async function fl(){const e=document.getElementById("professional-agenda-block");e.innerHTML="";try{const t=p.userProfessionalId;if(t){const a=await gs(t);gl=a,a.photo&&(document.getElementById("user-profile-avatar").src=a.photo),e.innerHTML=`
            ...
            `,vl(a.id),document.getElementById("my-blocks-filter").addEventListener("change",r=>It(a.id,r.target.value)),It(a.id,"future")}else e.innerHTML=`
        ...
        `}}function vl(e){const t=document.getElementById("block-schedule-form");t.addEventListener("submit",async a=>{a.preventDefault();const o=t.querySelector("#blockDate").value,r=t.querySelector("#blockStartTime").value,s=t.querySelector("#blockEndTime").value,n=t.querySelector("#blockReason").value;if(!o||!r||!s){g("Erro","Por favor, preencha a data e os horários de início e fim.","error");return}if(r>=s){g("Erro","A hora de início deve ser anterior à hora de fim.","error");return}const i=new Date(`${o}T${r}:00`),d=new Date(`${o}T${s}:00`),l=t.querySelector('button[type="submit"]');l.disabled=!0,l.textContent="A bloquear...";try{await Tt({establishmentId:p.establishmentId,professionalId:e,reason:n||"Bloqueado (Meu Perfil)",startTime:i.toISOString(),endTime:d.toISOString()}),g("Sucesso","Agenda bloqueada com sucesso!","success"),t.reset();const c=document.getElementById("my-blocks-filter").value;It(e,c)}catch(c){console.error("Erro ao bloquear agenda:",c),g("Erro",`Não foi possível bloquear a agenda: ${c.message}`,"error")}finally{l.disabled=!1,l.textContent="Bloquear Agenda"}})}async function It(e,t="future"){const a=document.getElementById("my-blocks-list");a.innerHTML='<p class="text-gray-500">A carregar bloqueios...</p>';try{const o=new Date;let r,s;t==="history"?(s=new Date,r=new Date,r.setFullYear(r.getFullYear()-1)):(r=new Date,s=new Date,s.setFullYear(s.getFullYear()+1));let i=(await Bt(p.establishmentId,r.toISOString(),s.toISOString(),e)).map(d=>({...d,startTime:new Date(d.startTime),endTime:new Date(d.endTime)}));t==="history"?i=i.filter(d=>d.endTime<o).sort((d,l)=>l.startTime-d.startTime):i=i.filter(d=>d.endTime>=o).sort((d,l)=>d.startTime-l.startTime),i.length>0?(a.innerHTML=i.map(d=>{const l=d.startTime.toLocaleDateString("pt-BR"),c=`${d.startTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - ${d.endTime.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}`;return`
                ...
                `}).join(""),a.querySelectorAll(".remove-block-btn").forEach(d=>{d.addEventListener("click",async l=>{const c=l.currentTarget.dataset.blockId;if(confirm("Tem certeza que deseja remover este bloqueio?"))try{await ga(c),g("Sucesso","Bloqueio removido.","success"),It(e,t)}catch(u){console.error("Erro ao remover bloqueio:",u),g("Erro",`Não foi possível remover o bloqueio: ${u.message}`,"error")}})})):a.innerHTML=`<p class="text-gray-500 py-4 text-center">Nenhum bloqueio ${t==="history"?"no histórico recente":"futuro agendado"}.</p>`}catch(o){console.error("Erro ao carregar bloqueios:",o),a.innerHTML=`<p class="text-red-500">Erro ao carregar bloqueios: ${o.message}</p>`}}const bt=document.getElementById("loadingScreen"),_t=document.getElementById("dashboardContent"),Wt=document.getElementById("content"),ro=document.getElementById("notificationBell"),Gt=document.getElementById("notificationBadge"),ft=document.getElementById("notificationPanel"),no=document.getElementById("notificationList"),Yt=document.getElementById("profileMenuButton"),Y=document.getElementById("profileDropdown"),hl=document.getElementById("profileName"),yl=document.getElementById("profileEmail"),xl=document.getElementById("logoutButton"),wl=document.getElementById("cancellationHistoryBtn"),io=document.getElementById("myProfileLink"),lo={indigo:{main:"#4f46e5",hover:"#4338ca",light:"#e0e7ff",text:"#ffffff"},blue:{main:"#2563eb",hover:"#1d4ed8",light:"#dbeafe",text:"#ffffff"},sky:{main:"#0284c7",hover:"#0369a1",light:"#e0f2fe",text:"#ffffff"},teal:{main:"#0d9488",hover:"#0f766e",light:"#ccfbf1",text:"#ffffff"},emerald:{main:"#059669",hover:"#047857",light:"#d1fae5",text:"#ffffff"},green:{main:"#16a34a",hover:"#15803d",light:"#dcfce7",text:"#ffffff"},lime:{main:"#65a30d",hover:"#4d7c0f",light:"#ecfccb",text:"#ffffff"},amber:{main:"#d97706",hover:"#b45309",light:"#fef3c7",text:"#1f2937"},orange:{main:"#ea580c",hover:"#c2410c",light:"#ffedd5",text:"#ffffff"},red:{main:"#dc2626",hover:"#b91c1c",light:"#fee2e2",text:"#ffffff"},rose:{main:"#e11d48",hover:"#be123c",light:"#ffe4e6",text:"#ffffff"},pink:{main:"#db2777",hover:"#be185d",light:"#fce7f3",text:"#ffffff"},fuchsia:{main:"#c026d3",hover:"#a21caf",light:"#fae8ff",text:"#ffffff"},purple:{main:"#7c3aed",hover:"#6d28d9",light:"#ede9fe",text:"#ffffff"},violet:{main:"#8b5cf6",hover:"#7c3aed",light:"#ddd6fe",text:"#ffffff"},gray:{main:"#4b5563",hover:"#374151",light:"#f3f4f6",text:"#ffffff"},black:{main:"#111827",hover:"#000000",light:"#e5e7eb",text:"#ffffff"}};let _e=null,We=[];const kl={"agenda-section":yo,"comandas-section":va,"relatorios-section":ta,"servicos-section":en,"produtos-section":bn,"suppliers-section":$n,"profissionais-section":St,"clientes-section":Ft,"estabelecimento-section":qo,"ausencias-section":Ci,"users-section":$t,"sales-report-section":Vi,"financial-section":Xi,"commissions-section":dl,"packages-section":$a,"my-profile-section":bl};function El(e){const t=lo[e]||lo.indigo,o=(s=>{const n=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(s);return n?`${parseInt(n[1],16)}, ${parseInt(n[2],16)}, ${parseInt(n[3],16)}`:"79, 70, 229"})(t.main);document.body.style.setProperty("--theme-main",t.main);const r=document.getElementById("dynamic-theme-styles");r.innerHTML=`
        :root {
    ...
    `}function ca(){const e=We.filter(t=>!t.read).length;if(e>0?(Gt.textContent=e,Gt.classList.remove("hidden")):Gt.classList.add("hidden"),We.length===0){no.innerHTML='<p class="text-center text-gray-500 p-4">Nenhuma notificação.</p>';return}no.innerHTML=We.map(t=>`
    ...
    `).join("")}function Sl(e){_e&&_e();const t=De(Q,"establishments",e,"notifications"),a=uo(t,_o("timestamp",">=",new Date),mo("timestamp","desc"));_e=Wo(a,o=>{o.docChanges().forEach(r=>{if(r.type==="added"){const s=r.doc.data();We.unshift({title:s.title,message:s.message,time:s.timestamp.toDate().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}),read:!1}),g(s.title,s.message,"info",!0),ca();const n=document.querySelector(".sidebar-link.active");n&&n.dataset.target==="agenda-section"&&(s.type==="cancellation"||s.type==="new_appointment")&&(console.log("Atualizando agenda em tempo real..."),yo())}})},o=>{console.error("Erro no listener de notificações em tempo real:",o),g("Erro de Conexão","Não foi possível receber atualizações em tempo real. Verifique as regras de segurança do Firestore.","error")})}function H(e,t={}){const a=e.replace("-section","");if(e!=="my-profile-section"){const r=p.enabledModules?.[a]!==!1,s=p.userPermissions===null||p.userPermissions[e]?.view===!0;if(!r||!s){Wt.innerHTML='<div class="p-8 text-center"><h2 class="text-2xl font-bold text-red-600">Acesso Negado</h2><p class="text-gray-600">Você não tem permissão para visualizar este módulo.</p></div>',document.querySelectorAll(".sidebar-link").forEach(n=>n.classList.remove("active"));return}}const o=kl[e];o?(document.querySelectorAll(".sidebar-link").forEach(r=>{r.classList.toggle("active",r.getAttribute("data-target")===e)}),e==="my-profile-section"&&document.querySelectorAll(".sidebar-link").forEach(r=>r.classList.remove("active")),Wt.innerHTML="",o(t)):(Wt.innerHTML=`<div class="p-8 text-center"><h2 class="text-2xl font-bold">Página em Construção</h2><p class="text-gray-600">O módulo para "${e}" ainda não foi implementado.</p></div>`,console.warn(`Nenhum carregador de página definido para: ${e}`))}async function $l(e){const t=document.getElementById("kpi-appointments-wrapper"),a=document.getElementById("kpi-financial-wrapper"),o=document.getElementById("kpi-today-appointments"),r=document.getElementById("kpi-today-revenue"),s=e===null||e["agenda-section"]?.view===!0,n=e===null||e["financial-section"]?.view===!0;if(s&&t&&t.classList.remove("hidden"),n&&a&&a.classList.remove("hidden"),!(!s&&!n))try{const i=await Ds();s&&o&&(o.textContent=i.todayAppointments.toString()),n&&r&&(r.textContent=`R$ ${i.todayRevenue.toFixed(2).replace(".",",")}`)}catch(i){console.error("Erro ao carregar KPIs do cabeçalho:",i),s&&o&&(o.textContent="Erro"),n&&r&&(r.textContent="Erro")}}async function Il(e){try{Ge.getPlatform()==="android"&&(await se.createChannel({id:"default",name:"Notificações Gerais",description:"Alertas de agendamentos e avisos",importance:5,visibility:1,vibration:!0}),console.log("Canal de notificação Android criado com sucesso."));let t=await se.checkPermissions();if(t.receive==="prompt"&&(t=await se.requestPermissions()),t.receive!=="granted"){console.warn("Permissão de notificação push foi negada pelo utilizador.");return}await se.register(),se.addListener("registration",async a=>{console.log("Push Token gerado:",a.value);try{const o=Ct(Q,"users",e);await po(o,{fcmToken:a.value}),console.log("Token FCM salvo no perfil do utilizador.")}catch(o){console.error("Erro ao salvar token FCM:",o)}}),se.addListener("registrationError",a=>{console.error("Erro no registo de push notifications:",a)}),se.addListener("pushNotificationReceived",a=>{console.log("Notificação Push recebida:",a),g(a.title,a.body,"info",!0)}),se.addListener("pushNotificationActionPerformed",a=>{console.log("Ação na notificação push:",a),H("agenda-section")})}catch(t){console.log("Push Notifications não suportado/inicializado:",t)}}function Cl(){Ge.isNativePlatform()&&(document.body.classList.add("is-app-native"),console.log("Modo App Nativo detectado: Layout ajustado para Safe Areas.")),rs(),ro.addEventListener("click",e=>{e.stopPropagation(),ft.classList.toggle("hidden"),ft.classList.contains("hidden")||(We.forEach(t=>t.read=!0),ca())}),wl.addEventListener("click",()=>{ns()}),Yt.addEventListener("click",e=>{e.stopPropagation(),Y.classList.toggle("active"),Y.classList.contains("active")?Y.classList.remove("hidden"):setTimeout(()=>Y.classList.add("hidden"),200)}),io&&io.addEventListener("click",e=>{e.preventDefault(),H("my-profile-section"),Y.classList.remove("active"),Y.classList.add("hidden")}),document.addEventListener("click",e=>{!ft.contains(e.target)&&e.target!==ro&&ft.classList.add("hidden"),!Y.contains(e.target)&&e.target!==Yt&&Y.classList.contains("active")&&(Y.classList.remove("active"),setTimeout(()=>Y.classList.add("hidden"),200))}),Vo(J,async e=>{if(e)try{const a=(await e.getIdTokenResult(!0)).claims;if((a.role==="owner"||a.role==="employee")&&a.establishmentId){const o=await et(a.establishmentId);p.enabledModules=o.modules,El(o.themeColor||"indigo");let r=null,s=e.displayName,n=null;if(a.role==="employee"||a.role==="owner"){const l=Ct(Q,"users",e.uid),c=await Jo(l);if(c.exists()){const u=c.data();r=a.role==="employee"?u.permissions||{}:null,s=u.name||s,n=u.professionalId||null}else if(a.role==="employee")throw new Error("Dados de permissão do funcionário não encontrados.")}p.userProfessionalId=n,Il(e.uid);const i=s||e.email;Go(a.establishmentId,o.name,r),Yt.textContent=i.charAt(0).toUpperCase(),hl.textContent=i,yl.textContent=e.email;const d=()=>{_e&&_e(),Ia(J).then(()=>window.location.href="/login.html")};xl.addEventListener("click",l=>{l.preventDefault(),d()}),cs(H,r,p.enabledModules),$l(r),Sl(a.establishmentId),ca(),bt.classList.add("fade-out"),_t.style.display="flex",setTimeout(()=>{bt.style.display="none"},500),H("agenda-section")}else throw new Error("Utilizador não tem permissão de 'owner' ou 'employee' ou 'establishmentId'.")}catch(t){console.error("Erro crítico na inicialização do painel:",t),bt.classList.add("fade-out"),setTimeout(()=>{bt.style.display="none"},500),_t.innerHTML=`
                ...
                `,_t.style.display="flex",document.getElementById("errorLogoutButton").addEventListener("click",()=>{Ia(J).then(()=>window.location.href="/login.html")})}else window.location.href="/login.html"})}Cl();
