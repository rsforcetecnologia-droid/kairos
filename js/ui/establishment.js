...
                            state.userName = ownerName;
                            // Atualiza o nome no menu de perfil
                            document.getElementById('profileName').textContent = ownerName; 
                            document.getElementById('profileMenuButton').textContent = ownerName.charAt(0).toUpperCase();
                        })
                );
             }
        }
...
        updatePromises.push(establishmentApi.updateEstablishment(state.establishmentId, updatedData));
...
            // 2. Chamar 'verifyBeforeUpdateEmail'
            saveButton.textContent = 'A enviar link...';
            await verifyBeforeUpdateEmail(user, newEmail); 

            // 3. Atualizar o e-mail no Firestore (backend)
...
// ####################################################################
// ### INÍCIO DA MODIFICAÇÃO (Add-on de Fidelidade) ###
// ####################################################################

function renderLoyaltySection(data, container) {
    // 1. Verifica se o módulo está ATIVO no plano do estabelecimento
    const loyaltyModuleEnabled = state.enabledModules?.['loyalty-section'] === true;

    if (loyaltyModuleEnabled) {
        // --- MÓDULO ATIVO: Renderiza o formulário de configuração ---
        ...
        container.querySelector('#loyalty-form').addEventListener('submit', e => {
            ...
            const formData = {
                loyaltyProgram: {
                    ...
                    tiers: loyaltyTiers.filter(t => t.points > 0 && (t.reward || t.discount > 0)) // (Corrigido para aceitar desconto sem descrição)
                }
            };
            handleSave(formData, e);
        });
        ...
        // --- MÓDULO INATIVO: Renderiza o card de "Contrate Agora" (Upsell) ---
        container.innerHTML = `
            <div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-gray-800">Plano de Fidelidade</h3>
                ...
                <div class="text-center p-4 bg-gray-50 rounded-lg border">
                    <svg class="mx-auto h-12 w-12 text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.31h5.518c.471 0 .664.621.32.951l-4.46 3.232a.563.563 0 00-.18.635l1.718 4.964a.562.562 0 01-.84.622l-4.48-3.232a.563.563 0 00-.65 0L5.64 19.34a.562.562 0 01-.84-.622l1.718-4.964a.563.563 0 00-.18-.635l-4.46-3.232c-.344-.25-.15-.951.32-.951h5.518a.563.563 0 00.475-.31L11.48 3.5z" />
                    </svg>
                    <h4 class="mt-3 font-semibold text-gray-800">Transforme clientes em fãs!</h4>
                    <p class="mt-2 text-sm text-gray-600">O módulo de Fidelidade é um add-on. Ative-o para criar prémios, atribuir pontos por compra e reter os seus melhores clientes.</p>
                    <button type="button" id="upgrade-loyalty-btn" class="mt-4 py-2 px-5 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700">
                        Contratar Add-on (Em breve)
                    </button>
                </div>
            </div>
        `;

        // Adiciona o listener para o botão de "Contratar"
        const upgradeButton = container.querySelector('#upgrade-loyalty-btn');
        if (upgradeButton) {
            upgradeButton.addEventListener('click', () => {
                showNotification('Em Breve', 'Esta funcionalidade estará disponível para contratação em breve.', 'info');
                // No futuro, isto pode abrir um modal do Stripe ou página de upgrade
            });
        }
    }
}
...
// --- 3. FUNÇÃO PRINCIPAL E NAVEGAÇÃO ---
...
        case 'financial': await renderFinancialIntegrationSection(establishmentData, detailContainer); break;
        case 'delete-account': renderPlaceholderSection(menuItem.label, detailContainer); break; // (Adicionado caso 'delete-account')
        default:
